 ...
 *
 * <p>Counts calls in one-minute windows. When the window limit is exhausted the
 * caller receives a {@link RateLimitException}. The window resets automatically
 * on the next call after the minute elapses.
 */
public final class ToolRateLimiter {
    ...
    public static final class RateLimitException extends RuntimeException {
        public RateLimitException(int limit) {
            super("Rate limit exceeded: " + limit + " tool calls per minute allowed. "
    ...
    /**
     * Records one tool invocation. Throws {@link RateLimitException} when the
     * per-minute cap is exceeded.
     */
    public void acquire() {
