...
/**
 * Wraps evidence-sourced text in a clearly delimited block to prevent prompt-injection attacks.
 *
 * <p>A forensic case may contain hostile documents that include text crafted to hijack an LLM's
 * instructions. By wrapping all evidence content in {@code <iped-evidence>} XML-like delimiters the
 * system prompt can instruct the model to treat everything inside as opaque data, not as
 * instructions.
 *
 ...
 * </pre>
 *
 * Content longer than {@value #MAX_CHARS} characters is truncated at a word boundary and a {@code
 * [TRUNCATED]} marker is appended before the closing tag.
 *
 * <h3>Binary / non-text content</h3>
 *
 * Binary items (identified by the {@code binary} flag on the caller side) are represented by a
 * single-line placeholder: the LLM cannot usefully read hex dumps, and sending them wastes context.
 * Use {@link #binaryPlaceholder} instead.
 */
public final class EvidenceGuard {
   ...
   */
  public static String binaryPlaceholder(
      String source, String itemId, String mediaType, long sizeBytes) {
