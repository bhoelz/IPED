 ...
 *
 * <p>Activated when {@code --transport=http}. The MCP endpoint is mounted at {@code /mcp} using the
 * streamable-HTTP protocol (SSE + POST on the same path). Auth and audit guarantees are identical
 * to the stdio transport because both transports share the same {@link ToolRegistry} and {@link
 * McpAuditLog}.
 *
...
public class HttpTransport {
  ...
  static void run(ToolRegistry registry, CliArgs cli) throws Exception {
    ...
    var transport =
        HttpServletStreamableServerTransportProvider.builder().jsonMapper(mapper).build();
    ...
    log.info(
        "iped-mcp HTTP server listening on port {} — endpoint: http://0.0.0.0:{}{} ",
        cli.port(),
        cli.port(),
        MCP_PATH);
