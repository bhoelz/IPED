 ...
 *
 * <p>Holds the session UUID used for request tracing, the case-ID allowlist (empty = all cases
 * accessible), the write-side capability grants, and the shared rate limiter. All tools receive
 * this context and call {@link #check(String)} before acting on a case-scoped request.
 */
public final class McpSessionContext {
  ...
  /**
   * Allowed case IDs for this session. Empty means no restriction. Populated from {@code
   * --allowed-cases=id1,id2} CLI arg.
   */
  public final Set<String> allowedCases;

  /**
   * Write-side capability grants for this session. Populated from {@code
   * --capabilities=bookmarks,jobs} CLI arg. Empty means read-only (no mutating tools registered).
   */
  ...
  public McpSessionContext(
      Set<String> allowedCases, int maxCallsPerMinute, Set<GrantedCapabilities> capabilities) {
   ...
   *
   * @throws CaseAccessDeniedException when an allowlist is configured and {@code caseId} is not in
   *     it
   */
  ...
  public static final class CaseAccessDeniedException extends RuntimeException {
    public CaseAccessDeniedException(String caseId, Set<String> allowed) {
      super(
          "Access denied: case \""
              + caseId
              + "\" is not in the allowed-cases list "
              + "for this session. Allowed: "
              + allowed);
    }
  }
}
