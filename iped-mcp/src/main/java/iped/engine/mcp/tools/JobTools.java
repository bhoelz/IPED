 ...
 *
 * <p>Jobs represent long-running operations (export, report) backed by the {@code POST/GET/DELETE
 * /v2/jobs} endpoints in iped-webapi. All tools require the {@code JOBS} capability.
 *
 * <ul>
 *   <li>{@code iped_job_export} — submit an export job for a set of items
 *   <li>{@code iped_job_status} — poll a job's current status and progress
 *   <li>{@code iped_job_cancel} — cancel a pending or running job
 * </ul>
 */
public class JobTools {
  ...
  private McpServerFeatures.SyncToolSpecification jobExport() {
    var props =
        Map.<String, Object>of(
            "sourceId",
                Map.of("type", "string", "description", "Source ID owning the items to export"),
            "itemIds",
                Map.of(
                    "type",
                    "array",
                    "description",
                    "List of integer docIds to include in the export",
                    "items",
                    Map.of("type", "integer")),
            "outputPath",
                Map.of(
                    ...
                    "string",
                    "description",
                    "(Optional) Absolute path for the output ZIP file. "
    ...
    return spec(
        ...
        new McpSchema.JsonSchema("object", props, List.of("sourceId", "itemIds"), null, null, null),
        (exchange, request) -> {
          ...
          List<Integer> itemIds = idList.stream().map(o -> ((Number) o).intValue()).toList();
          ...
          try {
            ...
            audit.success(
                "iped_job_export", Map.of("sourceId", sourceId, "itemCount", itemIds.size()));
  ...
  private McpServerFeatures.SyncToolSpecification jobStatus() {
    var props =
        Map.<String, Object>of(
            "jobId", Map.of("type", "string", "description", "Job ID returned by iped_job_export"));
    return spec(
        "iped_job_status",
        """
  ...
  private McpServerFeatures.SyncToolSpecification jobCancel() {
    var props =
        Map.<String, Object>of(
            "jobId",
            Map.of(
                "type", "string", "description", "Job ID of the pending or running job to cancel"));
    return spec(
        "iped_job_cancel",
