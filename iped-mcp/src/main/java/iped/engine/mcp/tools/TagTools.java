 ...
 *
 * <p>Tags are implemented as named bookmarks. {@code iped_item_tag} auto-creates
 * the tag bookmark on first use. Both tools require the {@code BOOKMARKS} capability.
 *
 * <ul>
 *   <li>{@code iped_item_tag}   — add an item to a named tag</li>
 *   <li>{@code iped_item_untag} — remove an item from a named tag</li>
 * </ul>
 */
public class TagTools {
    ...
    private McpServerFeatures.SyncToolSpecification itemTag() {
        var props = Map.<String, Object>of(
                "itemId", Map.of("type", "string",
                        "description", "Composite item ID \"{sourceId}:{docId}\" from iped_search"),
                "tag",    Map.of("type", "string",
                        "description", "Tag name to apply. The tag is created automatically if it does not exist.")
        );
        return spec("iped_item_tag",
                ...
                (exchange, request) -> {
                    ...
                    try {
                        ...
                        return CaseTools.ok("{\"tagged\":true,\"itemId\":\"" + itemId
                                + "\",\"tag\":\"" + tag + "\"}");
                    } catch (Exception e) {
    ...
    private McpServerFeatures.SyncToolSpecification itemUntag() {
        var props = Map.<String, Object>of(
                "itemId", Map.of("type", "string",
                        "description", "Composite item ID \"{sourceId}:{docId}\""),
                "tag",    Map.of("type", "string",
                        "description", "Tag name to remove from the item")
        );
        return spec("iped_item_untag",
                ...
                (exchange, request) -> {
                    ...
                    try {
                        ...
                        return CaseTools.ok("{\"untagged\":true,\"itemId\":\"" + itemId
                                + "\",\"tag\":\"" + tag + "\"}");
                    } catch (Exception e) {
