 ...
 *
 * <ul>
 *   <li>{@code iped_case_list} — list open cases (filtered to allowlist when set)
 *   <li>{@code iped_case_open} — open (mount) a case by file-system path
 *   <li>{@code iped_case_get} — get details for a single case
 *   <li>{@code iped_case_close} — close (unmount) a case
 * </ul>
 */
public class CaseTools {
        ...
        (exchange, request) -> {
          try {
            ...
            if (session.hasCaseRestriction()) {
              cases =
                  cases.stream()
                      .filter(c -> session.allowedCases.contains(String.valueOf(c.get("id"))))
        ...
        Map.<String, Object>of(
            "path",
                Map.of(
                    ...
                    "string",
                    "description",
                    "Absolute file-system path to a processed IPED case directory"),
            "id",
                Map.of(
                    "type",
                    "string",
                    "description",
                    "(Optional) Caller-chosen case ID. A UUID is derived from the path when omitted."));
    ...
    var props =
        Map.<String, Object>of(
            ...
            Map.of(
                ...
                "string",
                "description",
                "Case ID returned by iped_case_list or iped_case_open"));
  ...
  static McpSchema.CallToolResult err(String toolName, Exception e) {
    return McpSchema.CallToolResult.builder()
        .content(
            List.of(new McpSchema.TextContent("Tool " + toolName + " failed: " + e.getMessage())))
        .isError(true)
        .build();
  }
