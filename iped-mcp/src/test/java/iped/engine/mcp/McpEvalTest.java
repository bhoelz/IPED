...
public class McpEvalTest {
  ...
  void allExpectedToolsRegistered() {
    ...
    // Document
    assertContains(
        names, "iped_item_get", "iped_item_preview", "iped_item_related", "iped_category_list");
  ...
  void readOnlySessionHasNoWriteTools() {
    ...
    Set<String> names = Set.copyOf(roReg.tools().stream().map(s -> s.tool().name()).toList());
  ...
  void step4_itemPreviewIsGuarded() {
    ...
    assertTrue(
        preview.startsWith("<iped-evidence"),
        "EvidenceGuard opening tag missing: "
            + preview.substring(0, Math.min(50, preview.length())));
    assertTrue(preview.endsWith("</iped-evidence>"), "EvidenceGuard closing tag missing");
  ...
  void step6_submitAndPollExportJob() throws Exception {
    var exportResult =
        invoke("iped_job_export", Map.of("sourceId", "demo-1", "itemIds", List.of(5, 10, 15)));
  ...
  void invalidCursorReturnsError() {
    ...
    assertTrue(Boolean.TRUE.equals(result.isError()), "Expected error for invalid cursor");
  }
  ...
  private Set<String> toolNames() {
    return Set.copyOf(registry.tools().stream().map(s -> s.tool().name()).toList());
  }

  private static void assertSuccess(McpSchema.CallToolResult result) {
    assertFalse(Boolean.TRUE.equals(result.isError()), "Tool returned error: " + text(result));
  }
