...
/**
 * Fixture tests for DERCarver: verifies advertised CarverType metadata and that the DER/PKCS#12
 * header signatures decode without error.
 */
class DERCarverFixtureTest {
  ...
  void mimeTypeIsPkixCert() {
    assertEquals("application/pkix-cert", carver.getCarverTypes()[0].getMimeType().toString());
  }
  ...
  void carverClassIsSet() {
    assertEquals(DERCarver.class.getName(), carver.getCarverTypes()[0].getCarverClass());
  }
