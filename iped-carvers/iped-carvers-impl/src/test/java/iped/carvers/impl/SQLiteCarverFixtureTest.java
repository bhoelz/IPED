...
/**
 * Fixture tests for SQLiteCarver: verifies CarverType metadata and that the
 * SQLite magic signature decodes correctly via the CarverType API.
 */
class SQLiteCarverFixtureTest {
    ...
    void carverTypeHasSqliteMimeType() {
        assertEquals("application/x-sqlite3",
    ...
    void headerSignatureDecodesToSixteenBytes() {
        ...
        CarverType ct = carver.getCarverTypes()[0];
        Signature header = ct.getSignatures().stream()
                .filter(Signature::isHeader)
                .findFirst()
                .orElseThrow(() -> new AssertionError("no header signature"));
        assertEquals(16, header.getLength(),
                "SQLite magic 'SQLite format 3\\0' must decode to 16 bytes");
    }
