...
public class MOVCarver extends AbstractCarver {
  ...
  private static final String[] mp4AtomTypes = {
    "ftyp",
    "moov",
    "mdat",
    "skip",
    ...
    "wide",
    "pnot",
    "uuid", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$
    // //$NON-NLS-7$ //$NON-NLS-8$
    "meta",
    "pict",
    "PICT",
  ...
  public long getLengthFromHit(IItem parentEvidence, Hit header) throws IOException {
    ...
    try {
      ...
      while (true) {
        is.seek(atomStart + 4);
        int i = 0, off = 0;
        while (i != -1 && (off += i) < data.length) i = is.read(data, off, data.length - off);
        ...
        if (atomStart >= parentEvidence.getLength()) break;
      }
    } catch (Exception e) {
      e.printStackTrace();
