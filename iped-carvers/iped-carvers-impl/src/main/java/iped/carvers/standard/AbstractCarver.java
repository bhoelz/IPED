...
public abstract class AbstractCarver implements Carver {
  ...
  /**
   * This list stores headers with "stop on next header" option enabled. These types of carved items
   * do not have a footer nor a length defined in its content, so the end of the item is taken from
   * the defined maxLength configuration. Instead of processing them as soon as they are found (in
   * notifyHit()), they are added to this list. Whenever a new hit is notified, items in this list
   * will be processed if the new hit is of the same type (in this case, the end of the processed
   * hit will be limited to the start of the new hit), or when the offset of the new hit exceeds the
   * maximum offset of the hit in the list. (See #1816 discussion)
   */
  ...
  // getLengthFromHit
  private IItem carveFromLengthRef(IItem parentEvidence, Hit header, Hit lengthRef)
      throws IOException {
  ...
  public IItem carveFromHeader(IItem parentEvidence, Hit header, long len) throws IOException {
    ...
    if ((!ignoreCorrupted && !isSpecificIgnoreCorrupted())
        || isValid(parentEvidence, header, len)) {
      ...
      if (metadataObj instanceof Metadata metadata) {
        ...
        metadata.add(
            ExtraProperties.CARVEDOFFSET_METADATA_NAME, Long.toString(offsetFile.getFileOffset()));
      }

      for (Iterator<CarvedItemListener> iterator = carvedItemListeners.iterator();
          iterator.hasNext(); ) {
  ...
  /**
   * @return The prefix to be used in carved item name. Use a function to allow overriding.
   */
  ...
  /**
   * @return The suffix to be used in carved item name. Default is an empty string.
   */
  ...
  public boolean isValid(IItem parentEvidence, Hit headerOffset, long length) {
    ...
    for (iped.carvers.api.CarvedItemValidator v :
        headerOffset.getSignature().getCarverType().getValidators()) {
  ...
  public void notifyHit(IItem parentEvidence, Hit hit) throws IOException {
    ...
    if (hit.getSignature().isFooter()) {
      ...
      if (lastEscapeFooter == null
          || lastEscapeFooter.getOffset() + lastEscapeFooter.getSignature().getLength()
              != hit.getOffset() + hit.getSignature().getLength()) {
  ...
  protected void processHeadersWithStopOnNext(IItem parentEvidence, long currOffset)
      throws IOException {
