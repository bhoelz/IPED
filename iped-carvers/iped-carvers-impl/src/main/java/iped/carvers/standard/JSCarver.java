...
public class JSCarver extends DefaultCarver {
  ...
  public JSCarver(File scriptFile)
    ...
    String ext =
        scriptFile.getName().substring(scriptFile.getName().lastIndexOf('.') + 1); // $NON-NLS-1$
  ...
  public long getLengthFromHit(IItem parentEvidence, Hit header) throws IOException {
    try {
      ...
      return super.getDefaultLengthFromHeader(
          parentEvidence, header); // se o método não for implementado assume
  ...
  public void validateCarvedObject(IItem parentEvidence, Hit header, long length)
    ...
    try {
      ...
      super.validateCarvedObject(
          parentEvidence, header, length); // se o método de validação não for
  ...
  public IItem carveFromHeader(IItem parentEvidence, Hit header) throws IOException {
    try {
      ...
      return super.carveFromHeader(
          parentEvidence, header); // se o método de validação não for implementado
