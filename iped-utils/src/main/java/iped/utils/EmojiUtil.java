...
public class EmojiUtil {
  ...
  private static final Map<Integer, Character> specialCharsReplacement =
  ...
  public static byte[] replaceByImages(byte[] inBytes) {
    ...
    try {
      in =
          new BufferedReader(
              new InputStreamReader(new ByteArrayInputStream(inBytes), StandardCharsets.UTF_8));
      ...
      if (!replacedLines.isEmpty()) {
        ...
        ByteArrayOutputStream outBytes =
            new ByteArrayOutputStream(inBytes.length + inBytes.length / 4);
