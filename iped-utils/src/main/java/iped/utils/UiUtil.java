...
public class UiUtil {
  ...
  public static Color mix(Color c1, Color c2, double weight) {
    if (weight < 0) return c2;
    if (weight > 1) return c1;
  ...
  public static String getUIHtmlStyle() {
    ...
    sb.append("background-color:")
        .append(getHexRGB(background))
        .append(" !important;"); // $NON-NLS-1$ //$NON-NLS-2$
    sb.append("color:")
        .append(getHexRGB(foreground))
        .append(" !important;"); // $NON-NLS-1$ //$NON-NLS-2$
    sb.append("}"); // $NON-NLS-1$

    if (htmlLink != null) {
      sb.append(" .ipedtheme a:link, a:visited {color:")
          .append(getHexRGB(htmlLink))
          .append(" !important;}"); // $NON-NLS-1$ //$NON-NLS-2$
    }
    return sb.toString();
  }
