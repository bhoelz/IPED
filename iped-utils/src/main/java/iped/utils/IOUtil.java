...
public class IOUtil {
  ...
  private static final Set<String> DANGEROUS_EXTS =
      new HashSet<>(
          Arrays.asList(
              "0XE",
              "73K",
  ...
  public static boolean isToOpenExternally(String fileName, String trueExt) {
    return IOUtil.externalOpenConfig == ExternalOpenEnum.ALWAYS
        || (IOUtil.externalOpenConfig == ExternalOpenEnum.ASK_ALWAYS
            && IOUtil.confirmOpenDialog(fileName, trueExt))
        || (IOUtil.externalOpenConfig == ExternalOpenEnum.ASK_IF_EXE
            && (!IOUtil.isDangerousExtension(trueExt)
                || IOUtil.confirmOpenDialog(fileName, trueExt)));
  }
  ...
  public static String getValidFilename(String filename, int maxLength) {
    ...
    String[] invalidNames = {
      ...
      "PRN",
      "AUX",
      "NUL", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      "COM1",
      "COM2",
      "COM3",
      "COM4",
      ...
      "COM7",
      "COM8",
      "COM9", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$
    ...
    for (String name : invalidNames) {
      if (filename.equalsIgnoreCase(name)
          || filename.toUpperCase().startsWith(name + ".")) { // $NON-NLS-1$
    ...
    while (filename.length() > 0
        && ((c = filename.charAt(filename.length() - 1)) == ' ' || c == '.')) {
  ...
  public static final boolean isDiskFull(IOException e) {
    ...
    if (msg != null) {
      msg = msg.toLowerCase();
      if (msg.startsWith("espaço insuficiente no disco")
          || //$NON-NLS-1$
          msg.startsWith("não há espaço disponível no dispositivo")
          || //$NON-NLS-1$
          msg.startsWith("there is not enough space")
          || //$NON-NLS-1$
          msg.startsWith("not enough space")
          || //$NON-NLS-1$
  ...
  /**
   * A more reliable method than File.canWrite() to test for write permissions. It's slower because
   * the implementation opens a file handler internally. See more details on
