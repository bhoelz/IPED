...
public class ExternalImageConverter implements Closeable {
  ...
  private static String[] CMD = {
    ...
    "-density",
    DENSITY,
    "-sample", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
    SAMPLE_GEOMETRY,
    "-resize",
    RESIZE_GEOMETRY,
    INPUT,
  ...
  /**
   * Must be instantiated lazily, so command line adjustment will use the configured system
   * properties.
   */
  ...
  private ExternalImageConverter(ExecutorService executorService, boolean ownsExecutor) {
    ...
    highDensity =
        Integer.parseInt(System.getProperty(highDensityProp, "250").trim()); // $NON-NLS-1$
    magickAreaLimit =
        System.getProperty(magickAreaLimitProp, "32").trim() + "MP"; // $NON-NLS-1$  //$NON-NLS-2$
    minTimeout = Integer.parseInt(System.getProperty(minTimeoutProp, "20").trim()); // $NON-NLS-1$
    timeoutPerMB =
        Integer.parseInt(System.getProperty(timeoutPerMBProp, "2").trim()); // $NON-NLS-1$
    tmpDir =
        new File(
            System.getProperty(tmpDirProp, System.getProperty("java.io.tmpdir"))); // $NON-NLS-1$
    toolPath =
        winToolPathPrefix.isEmpty()
            ? "" //$NON-NLS-1$
            : winToolPathPrefix
                + File.separatorChar
                + "tools" //$NON-NLS-1$
                + File.separatorChar
                + (useGM ? "graphicsmagick" : "imagemagick"); // $NON-NLS-1$ //$NON-NLS-2$
  ...
  private String[] getCmd(int maxDimension, boolean highRes, Object objIn) {
    ...
    for (int i = 0; i < cmd.length; i++) {
      ...
      else if (c.equals(RESIZE_GEOMETRY)) cmd[i] = String.format("%1$dx%1$d>", maxDimension);
      else if (c.equals(NUM_THREADS)) cmd[i] = String.valueOf(numThreads);
      else if (c.equals(DENSITY)) cmd[i] = String.valueOf(highRes ? highDensity : lowDensity);
  ...
  public Dimension getDimension(InputStream in) {
    ...
    try {
      ...
      try (BufferedReader reader =
        ...
        String[] d = line.split(" ");
        if (d.length >= 2) return new Dimension(Integer.parseInt(d[0]), Integer.parseInt(d[1]));
      }
    } catch (IOException e) {
      e.printStackTrace();
    }
  ...
  public BufferedImage getImage(
      InputStream in, int maxDim, boolean highRes, Long imageSize, boolean throwTimeout)
      throws TimeoutException {
    return doGetImage(in, maxDim, highRes, imageSize, throwTimeout);
  }

  private BufferedImage doGetImage(
      Object objIn, int maxDim, boolean highRes, Long imageSize, boolean throwTimeout)
      throws TimeoutException {
    ...
    try {
      ...
      log.error(
          "Error executing "
              + (useGM ? "graphicsMagick" : "imageMagick")
              + ". " //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    ...
    if (p != null) {
      Future<?> sendFuture =
          objIn instanceof InputStream ? sendInputStream((InputStream) objIn, p) : null;
      ignoreErrorStream(p);
      Future<BufferedImage> resultFuture = getResultFuture(p);
      int timeout = getTotalTimeout(imageSize);
      if (highRes) timeout *= 2;
      try {
        if (sendFuture != null) sendFuture.get(timeout, TimeUnit.SECONDS);
  ...
  public int getTotalTimeout(Long imageSize) {
    if (imageSize == null) return minTimeout;
