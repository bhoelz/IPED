...
public class ImageUtil {
  ...
  /**
   * Redimensiona um imagem, mantendo sua proporção original se possível, mas utilizando mantendo
   * dimensões mínimas.
   */
  public static BufferedImage resizeImage(
      BufferedImage img, int maxW, int maxH, int minW, int minH, int imageType) {
  ...
  public static BufferedImage resizeImageFixed(
      BufferedImage img, int imgW, int imgH, int imageType) {
  ...
  public static BufferedImage getSubSampledImage(
      IItemReader itemReader, int size, String mimeType) {
  ...
  public static BufferedImage getSubSampledImage(
      InputStream inputStream, int size, String mimeType) {
  ...
  public static BufferedImage getSubSampledImage(
      IItemReader source, int size, BooleanWrapper renderException, String mimeType) {
  ...
  private static BufferedImage doGetSubSampledImage(
      Object source, int targetSize, BooleanWrapper renderException, String mimeType) {
  ...
  public static BufferedImage getBestFramesFit(
      BufferedImage img, String comment, int targetWidth, int targetHeight) {
  ...
  public static BufferedImage getBestFramesFit(
    ...
    BufferedImage fit =
        new BufferedImage(
            bestCols * (frameWidth + 1) + border * 2,
            bestRows * (frameHeight + 1) + border * 2,
  ...
  public static void saveJpegWithMetadata(
      BufferedImage img, File outFile, String data, int compression) throws IOException {
    ...
    IIOMetadata imageMetadata =
        writer.getDefaultImageMetadata(new ImageTypeSpecifier(img), jpgWriteParam);
  ...
  public static boolean isCompressedBMP(File file) throws FileNotFoundException, IOException {
    ...
    if (file.length() >= b.length) {
      try (FileInputStream is = new FileInputStream(file)) {
        is.read(b);
        if (b[30] == 0 && b[31] == 0 && b[32] == 0 && b[33] == 0) return false;
      }
    }
    return true;
  }

  public static void writeCompressedJPG(BufferedImage img, OutputStream baos, int compression)
      throws IOException {
  ...
  /**
   * @param intensity A proportion between the blurring window and the image dimensions. Typical
  ...
  public static BufferedImage grayscale(BufferedImage image) {
    if (image == null
        ...
        || image.getType() == BufferedImage.TYPE_USHORT_GRAY) return image;
