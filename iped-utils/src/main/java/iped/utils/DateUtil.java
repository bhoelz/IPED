...
public class DateUtil {
  ...
  private static final Pattern datePattern =
      Pattern.compile("\\d{4}([-:]\\d{2}){2}[T ](\\d{2}:){2}\\d{2}");
  ...
  /**
   * So we can return Date objects for these, this is the list (in preference order) of the various
   * ISO-8601 variants that we try when processing a date based property.
   */
   ...
   *
   * <p>This is not thread safe! Wrap in synchronized when sharing parser instances.
   *
   * @param dateString
   * @return
   */
