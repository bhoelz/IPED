...
/**
 * Creates seekable streams for items of a data source (image file, folder,
 * etc.), given each item's identifier within that source.
 */
public interface ISeekableInputStreamFactory {

    /**
     * Opens a new seekable stream over the content of the identified item. The
     * caller is responsible for closing it.
     *
     * @param identifier item identifier within the data source, e.g. a file
     *                   path or object id
    ...
    /**
     * @return {@code true} if this factory always returns empty streams, so
     *         callers can skip content processing
     */
     ...
     *
     * <p>Used by the distributed pipeline to serialize factory references into
