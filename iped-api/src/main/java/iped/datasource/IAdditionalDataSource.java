...
/**
 * Stores and retrieves results of additional processing tasks executed post-indexing on selected
 * items.
 *
 * <p>Each record is keyed by {@code (itemId, taskName)} and holds the extra attributes written by
 * the task. Calling {@link #storeTaskResult(int, String, Map)} with the same key overwrites any
...
public interface IAdditionalDataSource extends Closeable {
   ...
   */
  void storeTaskResult(int itemId, String taskName, Map<String, Object> extraAttrs)
      throws IOException;
  ...
  /**
   * Returns a merged map of all extra attributes contributed by every task that has been run on the
   * given item. When multiple tasks set the same key, the result of the most-recently stored task
   * wins.
   *
   * <p>The default implementation queries each task individually; concrete implementations may
   * override this with a single-query optimization.
   *
   * @param itemId IPED item identifier
  ...
  default Map<String, Object> getMergedExtraAttributes(int itemId) {
    ...
    for (String taskName : getExecutedTasks(itemId)) {
      getTaskResult(itemId, taskName).ifPresent(d -> merged.putAll(d.extraAttributes()));
    }
    return merged;
  }

  /**
   * Commits pending writes to persistent storage. Must be called after one or more {@link
   * #storeTaskResult} calls to guarantee durability.
   *
   * @throws IOException on I/O failure
   */
