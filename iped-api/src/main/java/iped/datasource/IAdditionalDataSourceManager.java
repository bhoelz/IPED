...
/**
 * Manages one or more {@link IAdditionalDataSource} instances associated with a case and provides
 * aggregated access to their contents.
 */
public interface IAdditionalDataSourceManager {

  /**
   * Registers an additional data source. Sources are consulted in registration order; later entries
   * win on attribute-key conflicts.
   *
  ...
  /**
   * Returns a merged map of extra attributes contributed by ALL registered sources for the given
   * item. Sources registered later win on key conflicts.
   *
  ...
  /**
   * Returns the union of all task names that have been executed on the item across all registered
   * sources.
   *
   * @param itemId IPED item identifier
