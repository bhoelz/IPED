...
/**
 * Holds the result of one additional-processing task run on a specific item. Instances are
 * immutable.
 *
 * @param taskName simple class name of the task that produced these results
...
public record AdditionalItemData(
    String taskName, Map<String, Object> extraAttributes, Instant processedAt) {

  /**
   * Canonical constructor that wraps {@code extraAttributes} in an unmodifiable view.
   *
   * @param taskName simple class name of the task that produced these results
   * @param extraAttributes extra attributes written by the task
   * @param processedAt when the task was executed
   */
  public AdditionalItemData(
      String taskName, Map<String, Object> extraAttributes, Instant processedAt) {
