 ...
 *
 * <p>Consumers (e.g., the distributed coordinator, the runner dashboard)
 * register an {@link IJobLifecycleListener} to receive these events without
 * depending on Kafka or engine internals.
 */
public final class JobLifecycleEvent {
    ...
    private JobLifecycleEvent(UUID caseId, JobPhase phase, Instant timestamp,
    ...
    public static JobLifecycleEvent failed(UUID caseId, Throwable cause) {
        return new JobLifecycleEvent(caseId, JobPhase.FAILED, Instant.now(),
                cause != null ? cause.getMessage() : null, cause);
    }

    @Override
    public String toString() {
        return "JobLifecycleEvent{caseId=" + caseId + ", phase=" + phase
                + ", timestamp=" + timestamp
