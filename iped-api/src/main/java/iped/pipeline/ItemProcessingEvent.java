...
/**
 * Immutable event recording the completion (or failure) of a single item's processing through the
 * task pipeline.
 *
 * <p>These events are the item-level counterpart of {@link JobLifecycleEvent}. They are used by the
 * distributed coordinator to track progress and drive completion detection without importing engine
 * or Kafka types.
 */
public final class ItemProcessingEvent {
  ...
  public static ItemProcessingEvent success(UUID caseId, String itemUuid, String taskName) {
    return new ItemProcessingEvent(
        caseId, itemUuid, taskName, Outcome.SUCCESS, Instant.now(), null);
  }

  /** Creates a failure event. */
  public static ItemProcessingEvent failure(
      UUID caseId, String itemUuid, String taskName, String errorMessage) {
    return new ItemProcessingEvent(
        caseId, itemUuid, taskName, Outcome.FAILURE, Instant.now(), errorMessage);
  }
  ...
  public static ItemProcessingEvent skipped(UUID caseId, String itemUuid, String taskName) {
    return new ItemProcessingEvent(
        caseId, itemUuid, taskName, Outcome.SKIPPED, Instant.now(), null);
  }

  @Override
  public String toString() {
    return "ItemProcessingEvent{caseId="
        + caseId
        + ", itemUuid='"
        + itemUuid
        + "', task='"
