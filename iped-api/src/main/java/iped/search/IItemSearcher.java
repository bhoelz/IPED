...
/**
 * Simplified search interface for use by processing tasks and scripts that need to query the case
 * index during processing.
 *
 * <p>Unlike {@link IIPEDSearcher}, this interface is deliberately narrow: it accepts a Lucene query
 * string and returns item readers directly, without exposing Lucene internals. It is safe to use
 * from tasks and parsers.
 *
...
public interface IItemSearcher extends Closeable {
  ...
  /**
   * Executes a Lucene query string and returns matches as a lazy iterable. Prefer this over {@link
   * #search(String)} for large result sets to avoid loading all results into memory at once.
   *
  ...
  /**
   * Escapes special Lucene characters in {@code string} so it can be used as a literal term inside
   * a larger query expression.
   *
