...
/** IPED-specific media types and helpers to query the Tika media type hierarchy. */
public class MediaTypes {

  /** Parent type of items that are metadata entries, not real files. */
  public static final MediaType METADATA_ENTRY =
      MediaType.application("x-metadata-entry"); // $NON-NLS-1$

  /** Chat message decoded from an application database. */
  public static final MediaType CHAT_MESSAGE_MIME =
      MediaType.parse("message/x-chat-message"); // $NON-NLS-1$
  ...
  public static final MediaType UFED_MESSAGE_MIME =
      MediaType.application("x-ufed-instantmessage"); // $NON-NLS-1$
  ...
  public static final MediaType UFED_CALL_MIME =
      MediaType.application("x-ufed-call"); // $NON-NLS-1$
  ...
  public static final MediaType UFED_CONTACT_MIME =
      MediaType.application("x-ufed-contact"); // $NON-NLS-1$

  /** User account decoded from a UFED extraction. */
  public static final MediaType UFED_USER_ACCOUNT_MIME =
      MediaType.application("x-ufed-useraccount");

  /** Device information entry decoded from a UFED extraction. */
  public static final MediaType UFED_DEVICE_INFO =
      MediaType.application("x-ufed-deviceinfo"); // $NON-NLS-1$
  ...
  public static final MediaType VMDK_DESCRIPTOR =
      MediaType.application("x-vmdk-descriptor"); // $NON-NLS-1$
  ...
  public static final MediaType MS_PUBLISHER =
      MediaType.application("x-mspublisher"); // $NON-NLS-1$
  ...
  /**
   * Declared last (after all the MediaType constants above) so that Tika's DefaultParser
   * construction — triggered by TikaConfig.getDefaultConfig() — can reflectively instantiate
   * parsers that read those constants in their own static initializers (e.g. UfedMessageParser) via
   * ServiceLoader without re-entering this class's still-incomplete <clinit> and seeing them as
   * null.
   */
  private static final MediaTypeRegistry MEDIA_TYPE_REGISTRY =
  ...
  /**
   * Gets the parent of a media type in the type hierarchy, skipping the generic {@code
   * application/octet-stream} ancestor. UFED types always resolve to {@link #METADATA_ENTRY}.
   *
   ...
   * @return {@code true} if {@code instance} is {@code parent} or one of its specializations
   */
  ...
  /**
   * Checks whether a media type equals or specializes another, accepting the candidate as any
   * object whose string form is a media type.
   *
   * @param instance the candidate type, as a {@link MediaType} or any object whose {@code
   *     toString()} is a media type string
   * @param parent the supertype to test against
   * @return {@code true} if {@code instance} is {@code parent} or one of its specializations,
   *     {@code false} if {@code instance} is null
   */
  ...
  /**
   * @param type the type to test, as any object whose string form is a media type
   * @return {@code true} if the type is a metadata entry, not a real file
   */
