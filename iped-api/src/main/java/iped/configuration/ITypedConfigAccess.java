 ...
 *
 * <p>Tasks, parsers, and scripts use this interface to retrieve their configuration without
 * importing the engine's {@code ConfigurationManager} or {@code ConfigurationRegistry} internals.
 * The engine binds a concrete implementation via dependency injection or a thread-local context.
 *
 ...
 * }</pre>
 *
 * <p>Implementations are expected to be thread-safe and to return the same instance on each call
 * for the same class within a case context.
 */
public interface ITypedConfigAccess {
   ...
   * @return the configuration wrapped in an {@link Optional}, or {@link Optional#empty()} if no
   *     configurable of that type has been registered with the engine
   */
  ...
  /**
   * Returns {@code true} if a named task-enable property is set to {@code true} in the loaded
   * configuration.
   *
   * @param propertyName the task-enable property name (as declared in the task configuration file)
   * @return {@code true} if the task is enabled, {@code false} if disabled or not found
   */
