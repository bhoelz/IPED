 ...
 *
 * <p>An {@code IIPEDSource} wraps a Lucene index and all case metadata for one processing output.
 * To search across multiple sources at once, see {@link IMultiBookmarks} and the multi-source
 * searchers.
 *
...
public interface IIPEDSource extends Closeable {
  ...
  /**
   * @return the Lucene {@code Analyzer} used for query parsing and indexing; returned as {@code
   *     Object} to avoid a compile-time Lucene dependency
   */
  Object getSearchAnalyzer();

  /**
   * @return the leaf-level Lucene {@code IndexReader}; returned as {@code Object} to avoid a
   *     compile-time Lucene dependency
   */
  Object getLeafIndexReader();

  /**
   * @return the atomic-level Lucene {@code IndexReader}; returned as {@code Object} to avoid a
   *     compile-time Lucene dependency
   */
  Object getAtomicIndexReader();

  /**
   * @return the case root folder (the folder that contains the {@code iped/} module directory)
   */
  File getCaseDir();
  ...
  /**
   * Returns all categories that are descendants of the given ancestral category in the category
   * hierarchy.
   *
  ...
  /**
   * @return the set of extra-attribute names stored by processing tasks in this case
   */
  ...
  /**
   * Converts an {@link IItemId} (source + item id pair) to the Lucene document id in this source.
   *
  ...
  /**
   * @return a handle to the underlying Lucene {@code IndexReader}; type is opaque to avoid a
   *     compile-time Lucene dependency
   */
  Object getIndexReaderHandle();

  /**
   * @return a handle to the underlying Lucene {@code IndexSearcher}; type is opaque to avoid a
   *     compile-time Lucene dependency
   */
  Object getIndexSearcherHandle();

  /**
   * @return the numeric source id assigned to this case within a multi-source session
   */
  int getSourceId();
