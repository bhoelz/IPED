...
/**
 * Read-only view of an evidence item: identity, names, dates, hashes, metadata and content streams.
 * This is the interface exposed to scripts and external consumers that must not modify the item.
 */
public interface IItemReader extends IStreamSource {
  ...
  /**
   * Returns a temp file with item content. This can spool data to the temp directory, so avoid
   * using this if you can work directly with item data streams.
   */
  File getTempFile() throws IOException;

  /**
   * @return this item's identifier within its data source, e.g. a file path or object id, used with
   *     {@link #getInputStreamFactory()}
   */
   ...
   * @throws SQLException if the preview is stored in a database and reading it fails
  ...
  /**
   * Opens a new buffered stream over this item's content. The caller is responsible for closing it.
   *
  ...
  /**
   * Opens a new image input stream over this item's content. The caller is responsible for closing
   * it.
   *
  ...
  /**
   * Gets the first value of a metadata entry reflectively, without a compile dependency on the
  ...
  /**
   * Gets all values of a metadata entry reflectively, without a compile dependency on the metadata
   * implementation.
   *
   * @param key the metadata entry name
