 ...
 *
 * <p>Bookmarks are named, ordered collections of items. Unlike tags (which are attached to
 * individual items), bookmarks are managed as named sets that can group items for reporting and
 * export.
 *
 * @since 4.2
 */
public interface IBookmarkService {
  ...
  /**
   * Creates a new, empty bookmark with the given name. If a bookmark with that name already exists
   * this is a no-op.
   *
  ...
  /**
   * Deletes the bookmark with the given name and removes all item associations. A no-op if the
  ...
  /**
   * Adds the item to the named bookmark. Creates the bookmark if it does not already exist.
   *
  ...
  /**
   * Removes the item from the named bookmark. A no-op if the item is not in the bookmark or the
   * bookmark does not exist.
   *
   * @param bookmarkName bookmark name; must not be {@code null}
  ...
  /**
   * Returns the item IDs belonging to the named bookmark, or an empty set if the bookmark does not
   * exist.
   *
   * @param bookmarkName bookmark name; must not be {@code null}
