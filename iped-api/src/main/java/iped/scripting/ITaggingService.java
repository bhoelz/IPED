 ...
 *
 * <p>Tags are arbitrary string labels attached to items. Each item may hold
 * zero or more tags simultaneously. Tag names are case-sensitive and must be
 * non-null, non-empty, and contain no {@code |} characters.
 *
 * @since 4.2
 */
public interface ITaggingService {
    ...
    /**
     * Removes the given tag from the item, if it was previously applied.
     * A no-op if the tag was not set.
     *
     * @param itemId  the target item; must not be {@code null}
