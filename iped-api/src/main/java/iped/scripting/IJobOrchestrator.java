 ...
 *
 * <p>Scripts use this interface to start new IPED processing jobs and observe their progress
 * without importing engine-internal types.
 *
 * @since 4.2
 */
public interface IJobOrchestrator {
  ...
  /**
   * Submits a new processing job for the given data source against the specified output directory
   * and returns a handle to the running job.
   *
   * <p>The call is non-blocking. Use the returned {@link Future} or register listeners via {@link
   * #addJobListener} to track completion.
   *
   ...
   * @return a {@link Future} whose value is the case UUID assigned to the job; never {@code null}
   */
  Future<UUID> submit(IDataSource dataSource, File outputDir, File profileDir);

  /**
   * Cancels the job identified by {@code caseId} if it is still running. A no-op if the job has
  ...
  /**
   * Blocks until the job identified by {@code caseId} has reached a terminal state ({@code
  ...
  /**
   * Returns the last known lifecycle event for the job, or {@code null} if no event has been fired
   * yet or the ID is unknown.
   *
  ...
  /**
   * Registers a listener that will receive job lifecycle events for all jobs managed by this
   * orchestrator. Idempotent if called twice with the same instance.
   *
  ...
  /**
   * Removes a previously registered job lifecycle listener. A no-op if the listener was not
   * registered.
   *
  ...
  /**
   * Registers a listener that will receive item processing events for all jobs managed by this
   * orchestrator. Idempotent if called twice with the same instance.
   *
   * @param listener listener to add; must not be {@code null}
   */
  ...
  /**
   * Removes a previously registered item processing listener. A no-op if the listener was not
   * registered.
   *
   * @param listener listener to remove; must not be {@code null}
   */
