...
- [ ] Add ArchUnit test forbidding dependencies from `iped-api` to any other IPED module.
- [ ] Javadoc coverage for every public type (this module is the documentation surface for
      the 5.0 scripting APIs).
- [ ] Mark internal-but-public types with a clear annotation (`@Internal`) so scripting
      docs can exclude them.
...
- [ ] Add API-level event/listener contracts for the distributed pipeline (job lifecycle,
      item processed, case completed) so `iped-distributed` and `iped-runner` don't bind to
      engine internals.

## Phase 3 — Modularization support
- [ ] Capability/SPI interfaces for pluggable datasource readers, so `iped-sleuthkit`,
      `iped-ufed`, `iped-ad1` register via `ServiceLoader` instead of hardcoded engine wiring.
- [ ] Configuration schema contracts aligned with the TOML config system (typed config
