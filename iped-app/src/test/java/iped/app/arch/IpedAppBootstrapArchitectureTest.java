...
/**
 * Verifies that the headless {@code Bootstrap} class (and everything in {@code iped.app.bootstrap}
 * except {@code BootstrapUI}) is free of AWT, Swing, and JavaFX dependencies, so the processing
 * entry-point can be loaded in a headless server JVM without a display.
 */
class IpedAppBootstrapArchitectureTest {

  private static final JavaClasses BOOTSTRAP_CLASSES =
  ...
  void bootstrapMustNotDependOnAwtOrSwing() {
    ArchRule rule =
        noClasses()
            .that()
            .resideInAPackage("iped.app.bootstrap..")
            .and()
            .haveSimpleNameNotContaining("BootstrapUI")
            .should()
            .dependOnClassesThat()
            .resideInAnyPackage("java.awt..", "javax.swing..", "sun.awt..")
            .because(
                "Bootstrap must be loadable without a graphical environment; "
  ...
  void bootstrapMustNotDependOnIpedUiPackages() {
    ArchRule rule =
        noClasses()
            ...
            .should()
            .dependOnClassesThat()
            .resideInAnyPackage("iped.app.ui..", "iped.app.splash..")
