...
public class GraphToolBar extends JToolBar implements GraphHistoryListener {
  ...
  private void init() {
    ...
    undoBtn =
        createButton(new UndoAction(app), Messages.getString("GraphAnalysis.UndoAction"), undoUrl);
    undoBtn.setEnabled(false);
    this.add(undoBtn);

    URL redoUrl = this.getClass().getResource("action-redo-2x.png");
    redoBtn =
        createButton(new RedoAction(app), Messages.getString("GraphAnalysis.RedoAction"), redoUrl);
    ...
    JButton fitToScreenBtn =
        createButton(
            new FitToScreenAction(app),
            Messages.getString("GraphAnalysis.FitToScreen"),
            fitToScreenUrl);
    ...
    JButton exportImageBtn =
        createButton(
            new ExportImageAction(app),
            Messages.getString("GraphAnalysis.SaveImage"),
            exportImageUrl);
    this.addBtn(exportImageBtn);

    URL searchEntityUrl = this.getClass().getResource("magnifying-glass-2x.png");
    JButton searchEntityBtn =
        createButton(
            new OpenEntitySearchAction(app),
            Messages.getString("GraphAnalysis.SearchEntities"),
            searchEntityUrl);
    ...
    JButton exportLinksBtn =
        createButton(
            new OpenExportLinkAction(app),
            Messages.getString("GraphAnalysis.ExportLinks"),
            exportLinksUrl);
    this.addBtn(exportLinksBtn);

    URL advancedSearchUrl = this.getClass().getResource("beaker-2x.png");
    JButton advancedSearchBtn =
        createButton(
            new OpenAdvancedSearchAction(app),
            Messages.getString("GraphAnalysis.AdvancedSearch"),
            advancedSearchUrl);
    ...
    for (GraphVizAlgorithm algo : GraphVizAlgorithm.values()) {
      ...
      JButton layoutHLBtn =
          createButton(
              new ApplyGraphLayoutAction(
                  app, new GraphVizLayoutUniqueEdges(algo, new GraphVizIpedResolver())),
              "Layout " + algo.name(),
              layoutHLUrl);
