...
public class AppGraphAnalytics extends JPanel implements ClearFilterListener {
  ...
  private void updateThemeColors() {
    ...
    if (defaultEdgeColor == null) defaultEdgeColor = Color.BLUE;
    ...
    if (selectedNodeBoxColor == null) selectedNodeBoxColor = Color.BLUE;
    Color selectionBoxColor = UIManager.getColor("Graph.selectionBox");
    if (selectionBoxColor == null) selectionBoxColor = Color.BLUE;
  ...
  private void init() {
    ...
    renderers.registerNodeRenderer(
        GraphConfiguration.ORGANIZATION_LABEL, new CompanyNodeRenderer());
    ...
    renderers.registerNodeRenderer(
        GraphConfiguration.BANK_ACCOUNT_LABEL, new MoneyBagNodeRenderer());
    renderers.registerNodeRenderer(
        GraphConfiguration.MONEY_TRANSFER_LABEL, new MoneyTransferNodeRenderer());
    renderers.registerNodeRenderer(GraphConfiguration.DOCUMENT_LABEL, new DocumentNodeRenderer());

    renderers.registerNodeRenderer(
        GraphConfiguration.DATASOURCE_LABEL + "," + GraphConfiguration.DOCUMENT_LABEL,
        new DocumentNodeRenderer());
    renderers.registerNodeRenderer(
        GraphConfiguration.DATASOURCE_LABEL + "," + GraphConfiguration.PERSON_LABEL,
    ...
    renderers.registerNodeRenderer(
        GraphConfiguration.CONTACT_GROUP_LABEL, new PeopleNodeRenderer());

    this.sidePanel = new GraphSidePanel(this.graphPane);

    JSplitPane splitPane =
        new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, this.graphPane, this.sidePanel);
  ...
  public void addEvidenceFilesToGraph(Collection<ItemId> items) {
    if (!items.isEmpty()) {
      String[] ids =
          items.stream()
              .map(s -> Integer.toString(s.getId()))
  ...
  public void expandSelectedWithLabelsOrTypes(
      Collection<String> labelsOrTypes, boolean isEdge, int maxNodes) {
    new ExpandConfigurationNodeWorker(
            labelsOrTypes, Long.parseLong(this.getContextMenuNodeId()), isEdge, maxNodes)
        .execute();
  }
  ...
  private class AppStageListerner extends StageAdapter {
    ...
    public void stageClicked(MouseEvent evt) {
      if (evt.isControlDown() || evt.isShiftDown()) return;
  ...
  private class AppNodeListener extends NodeAdapter {
    ...
    private void nodeRightClicked(Node node, MouseEvent evt) {
      boolean keepSelection =
          graphPane.isNodeSelected(node) || evt.isShiftDown() || evt.isControlDown();
      graphPane.selectNode(node.getId(), keepSelection);
  ...
  private class ShowMoreConnectedNode extends SwingWorker<Void, Void> {
    ...
    protected Void doInBackground() {
      try {
        ...
        log.info(
            "Query {} most connected nodes took {}s",
            ids.size(),
            (System.currentTimeMillis() - t) / 1000);
        for (Long id : ids) {
          AddNodeWorker worker =
              new AddNodeWorker(AppGraphAnalytics.this, Collections.singleton(id));
          worker.execute();
          worker.get();
          t = System.currentTimeMillis();
          ExpandNodeWorker expandWorker =
              new ExpandNodeWorker(
    ...
    protected void done() {
      if (!ids.isEmpty()) {
        AppGraphAnalytics.this.graphStatusBar.setStatus(
            Messages.getString("GraphAnalysis.InitialGraphMsg"));
      }
    }
  }
  ...
  private class AddEvidenceFileWorker extends ExpandNodeWorker {
    ...
    protected Void doInBackground() {
      AppGraphAnalytics.this.graphStatusBar.setStatus(
          Messages.getString("GraphAnalysis.Processing"));
    ...
    public boolean edgeFound(Relationship relationship) {
      found++;
      AppGraphAnalytics.this.graphStatusBar.increaseProgress(
          (int) ((found / this.ids.length) * 100));
  ...
  private class ExpandConfigurationNodeWorker extends ExpandNodeWorker {
    ...
    public ExpandConfigurationNodeWorker(
        Collection<String> labelsOrTypes, Long nodeId, boolean isEdge, int maxNodes) {
      super(null);
    ...
    protected Void doInBackground() {
      ...
      try {
        if (!isEdge) {
          ...
          else graphService.getNeighboursWithLabels(labelsOrTypes, nodeId, this, maxNodes);
        } else {
  ...
  private class ExpandNodeWorker extends SwingWorker<Void, Node> implements NodeEdgeQueryListener {
    ...
    protected Point getPosition(Node newNode) {
      return AppGraphAnalytics.this.graphModel.calculateRelativePosition(
          this.currentNode, newNode, currentDegree);
    }
        ...
        if (propertyContainer instanceof org.neo4j.graphdb.Relationship) {
          ...
          if (startNode.getId() != neo4jNode.getId() && endNode.getId() != neo4jNode.getId()) {
            ...
            if (newlyAdded.contains(endNodeId)) {
              ...
              Point position =
                  AppGraphAnalytics.this.graphModel.calculateRelativePosition(
                      currentSource, currentTarget, newNode, hop, length, degree);
              newNode.setX(position.x);
              newNode.setY(position.y);
            }
  ...
  public void exportImage(File selectedFile) {
    AppGraphAnalytics.this.graphStatusBar.setStatus(
        Messages.getString("GraphAnalysis.ExportingImage"));
    ...
    for (Path path : paths) {
      ...
      for (org.neo4j.graphdb.Relationship relationship : path.relationships()) {
        ...
        if (!relationshipInGraph && !relationshipInNewEdges) {
          ...
          boolean inNewNodes =
              graphElements.containsNode(startNodeId) && graphElements.containsNode(endNodeId);
  ...
  private void applyDefaultLayout() {
    if (!this.graph.isEmpty()) {
      HistoryEnabledLayout layout =
          new GraphVizLayoutUniqueEdges(GraphVizAlgorithm.NEATO, new GraphVizIpedResolver());
      applyLayout(layout);
    }
  }
