...
public class FilterSelectedEdges implements IResultSetFilterer {
    ...
    for (Edge edge : edges) {
      if (edge instanceof OverlappedEdges) {
        ...
        for (Edge e : overlapped.getEdges()) {
          if (selectedEdges.add(e)) added = true;
        }
      } else {
        if (selectedEdges.add(edge)) added = true;
      }
    }
    if (added) {
      updateResults();
  ...
  public IFilter getFilter() {
    if (selectedEdges != null && selectedEdges.size() > 0) {
      Set<IItemId> selectedEdgesItems =
          FilterSelectedEdges.getInstance().getItemIdsOfSelectedEdges();
...
class EdgeFilter implements IResultSetFilter {
  ...
  public EdgeFilter(Graph graph, Set<IItemId> itemIdsOfSelectedEdges, HashSet<Edge> edges) {
    ...
    for (Iterator iterator = selectedEdges.iterator(); iterator.hasNext(); ) {
      ...
      String filterInfo =
          graph.getNode(edge.getSource()).getLabel()
              + "->"
              + graph.getNode(edge.getTarget()).getLabel();
  ...
  public IMultiSearchResult filterResult(IMultiSearchResult src)
    ...
    result =
        new MultiSearchResult(
            filteredItems.toArray(new ItemId[0]),
