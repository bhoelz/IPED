...
class LoadGraphDatabaseWorker extends SwingWorker<Void, Void> {
  ...
  protected Void doInBackground() throws Exception {
    ...
    if (cases.size() == 1) {
      ...
      if (!multiCaseGraphPath.getParentFile().exists()
          && !multiCaseGraphPath.getParentFile().mkdirs()) {
    ...
    if (!graphDataDir.exists()) {
      int option =
          JOptionPane.showConfirmDialog(
              App.get(), Messages.getString("GraphAnalysis.CreateMultiGraph"));
      if (option != JOptionPane.YES_OPTION) return;
      try {
        if (multiCaseGraphPath.exists()) IOUtil.deleteDirectory(multiCaseGraphPath, false);
      ...
      JOptionPane.showMessageDialog(
          App.get(),
          Messages.getString("GraphAnalysis.MultiGraphError"),
          "Error",
  ...
  private class ImportWorker extends CancelableWorker<Boolean, Void> implements ImportListener {
    ...
    public void output(String line) {
      if (!line.trim().isEmpty()) progress.setNote(line.trim());
      else progress.setNote("Importing...");
    }

    @Override
    protected Boolean doInBackground() throws Exception {
      try {
        List<File> csvParents =
            cases.stream()
                .map(c -> new File(c.getModuleDir(), GraphTask.CSVS_PATH))
  ...
  private File copyToTempFolder(File dataDir) throws IOException {
    File tmpDb =
        new File(System.getProperty("java.io.tmpdir"), "iped-graph" + dataDir.lastModified());
