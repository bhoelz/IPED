...
public class MetadataSearch extends MetadataSearchable {
  ...
  public static final String RANGE_SEPARATOR =
      Messages.getString("MetadataPanel.RangeSeparator"); // $NON-NLS-1$
  ...
  public MultiSearchResult getIdsWithOrd(
      MultiSearchResult result, String field, Set<Integer> ordsToGet) throws IOException {
    ...
    if (isNumeric && numValues != null && !noRanges) {
        ...
        if (adv) {
          ...
          if (logScale) {
            if (val == Double.NEGATIVE_INFINITY) {
              ...
              if (ord > logScaleBins - 3) ord = logScaleBins - 3;
            }
          } else {
            ord = (int) ((val - min) / interval);
            if (val == max && min != max) ord--;
      ...
      for (int ord : ordsToGet) {
        if (ord >= 0 && ord < l.size()) set.add(l.get(ord));
      }
      if (!set.isEmpty()) {
        // must reset docValues to call advance again
        loadDocValues(field);
    ...
    return new MultiSearchResult(
        items.toArray(new ItemId[0]), ArrayUtils.toPrimitive(scores.toArray(new Float[0])));
  }
    ...
    if (isNumeric && numValues != null && !noRanges) {
        ...
        for (IItemId item : ipedResult.getIterator()) {
          ...
          if (adv) {
            ...
            if (val == Double.NEGATIVE_INFINITY) {
              ...
              hasNaN = true;
              ord = linearScaleBins - 1;
            } else if (ord >= linearScaleBins) ord = linearScaleBins - 1;
            if (!isFloat && !isDouble) {
              long lval = (long) val;
              ...
              if (lval < actualMin[ord]) actualMin[ord] = lval;
              if (lval > actualMax[ord]) actualMax[ord] = lval;
            }
            if (ord < 0) ord = 0;
            else if (ord >= linearScaleBins) ord = linearScaleBins - 1;
            valueCount[ord]++;
      ...
      for (IItemId item : ipedResult.getIterator()) {
        ...
        for (int i = 0; adv && i < numValuesSet.docValueCount(); i++) {
          ...
          if (v == null) map.put(val, v = new SingleValueCount(val));
          v.incrementCount();
        }
      }
      ...
      for (IItemId item : ipedResult.getIterator()) {
        if (isTimeEvent && item instanceof TimeItemId) {
          ...
          long ord, prevOrd = -1;
          while (adv && (ord = docValuesSet.nextOrd()) != -1L) {
            if (prevOrd != ord) valueCount[(int) ord]++;
            prevOrd = ord;
          }
        }
      }
    ...
    if (isNumeric && !noRanges) {
      for (int ord = 0; ord < valueCount.length; ord++) {
        if (valueCount[ord] > 0) {
          ...
          double end = 0;
          if (logScale) {
            if (ord == 0) start = end = Double.NEGATIVE_INFINITY;
            else if (ord == logScaleBins - 2) start = end = Double.POSITIVE_INFINITY;
            else if (ord == logScaleBins - 1) start = end = Double.NaN;
      ...
      for (int ord = 0; ord < valueCount.length; ord++)
        if (valueCount[ord] > 0) list.add(new ValueCount(lo, ord, valueCount[ord]));
      ...
      for (int ord = 0; ord < valueCount.length; ord++)
        if (valueCount[ord] > 0)
          if (!isMoney) list.add(new ValueCount(lo, ord, valueCount[ord]));
          else list.add(new MoneyCount(lo, ord, valueCount[ord]));
    }
