 ...
 *
 * <p>Uses JLine3's {@link Status} to pin two status lines at the bottom of the terminal while log
 * messages scroll above. Falls back gracefully to {@link ProgressConsole} when stdout is not a real
 * TTY (see {@link #tryCreate()}).
 */
public class ProgressJLine implements PropertyChangeListener {
   ...
   *
   * @return a new instance, or {@code null} if the current environment is not a real terminal
   *     (piped output, CI, dumb terminal).
   */
  ...
  private void redraw() {
    ...
    if (discoverEnded && processedMb > 0 && totalVolumeMb > processedMb) {
      long secsToEnd =
          (long) (totalVolumeMb - processedMb)
              * (System.currentTimeMillis() - processingStart)
              / ((processedMb + 1) * 1000L);
      etaStr =
          "  ETA " + secsToEnd / 3600 + "h " + (secsToEnd / 60) % 60 + "m " + secsToEnd % 60 + "s";
    }
    ...
    line1
        .append("[")
        .append(FILLED.repeat(filled))
        .append(EMPTY.repeat(barWidth - filled))
        .append("] ");
