...
/**
 * Log4j2 appender that routes log output through the JLine3 terminal writer so the scroll region
 * managed by {@link org.jline.utils.Status} is never corrupted by direct writes to System.out.
 *
 * <p>Install once before activating the TUI; uninstall on shutdown.
 */
public class JLineAppender extends AbstractAppender {
  ...
  /**
   * Creates, starts, and registers this appender with the root logger. Existing Console appenders
   * are removed so only JLine touches the terminal.
   */
