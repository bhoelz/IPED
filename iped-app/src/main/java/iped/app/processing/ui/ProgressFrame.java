...
public class ProgressFrame extends JFrame implements PropertyChangeListener, ActionListener {
  ...
  private void update() {
    ...
    int totalVolume =
        (int) (Statistics.get().getCaseData().getDiscoveredVolume() >>> 20); // Converted to MB
    ...
    if (processedItems > 0) openApp.setEnabled(true);
    ...
    if (processedItems > 0) {
      ...
      msg =
          Messages.getString("ProgressFrame.Found")
              + totalItems
              + Messages.getString("ProgressFrame.items");
    }

    if (discoverEnded && processingStart != 0) {
      long secsToEndVolume =
          (totalVolume - processedVolume)
      ...
      msg +=
          Messages.getString("ProgressFrame.FinishIn")
              + secsToEnd / 3600
              + "h "
              + (secsToEnd / 60) % 60
              + "m "
  ...
  private String getParserTimes() {
    ...
    if (timesPerParser.isEmpty()) return "";
    StringBuilder msg = new StringBuilder();
    startTable(msg);
  ...
  private String getStats() {
    ...
    finishRow(
        ...
        secsToEnd == 0
            ? "-"
            : secsToEnd / 3600 + "h " + (secsToEnd / 60) % 60 + "m " + secsToEnd % 60 + "s",
        Align.RIGHT);
  ...
  private void addEnvironmentInfo(StringBuilder msg) {
    ...
    if (workers != null) {
      try {
        ...
        FileStore tempVolume =
            Files.getFileStore(
                new File(System.getProperty("java.io.tmpdir")).getCanonicalFile().toPath());
  ...
  private void addCell(StringBuilder sb, Object content, Align align) {
    sb.append("<td");
    if (align == Align.CENTER) sb.append(" class=c");
    else if (align == Align.RIGHT) sb.append(" class=r");
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getSource().equals(pause) && workers != null) {
      paused = !paused;
      if (paused) pause.setText(Messages.getString("ProgressFrame.Continue")); // $NON-NLS-1$
      else pause.setText(Messages.getString("ProgressFrame.Pause")); // $NON-NLS-1$
    ...
    if (e.getSource().equals(openApp)) {
      if (!App.get().isVisible()) {
        JOptionPane.showMessageDialog(
            this, Messages.getString("ProgressFrame.IncompleteProcessing")); // $NON-NLS-1$
        new AppMain().start(workers[0].output.getParentFile(), workers[0].manager, null);
      } else
        JOptionPane.showMessageDialog(
            this, Messages.getString("ProgressFrame.AlreadyOpen")); // $NON-NLS-1$
    }
  }

  /**
   * This is a workaround to avoid weird line breaks caused by a JDK bug. It may be removed when it
   * is fixed there. See issue #2102.
   */
  ...
  private void updateTaskBar(int totalVolume, int processedVolume, boolean discoverEnded) {
    if (Taskbar.isTaskbarSupported()) {
      ...
      taskbar.setWindowProgressState(
          this, paused ? State.PAUSED : discoverEnded ? State.INDETERMINATE : State.NORMAL);
      if (discoverEnded) {
