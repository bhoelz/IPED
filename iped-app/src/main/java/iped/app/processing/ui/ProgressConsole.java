...
public class ProgressConsole implements PropertyChangeListener {
  ...
  private void update() {
    ...
    if (processedItems > 0) {
      ...
      int percent =
          discoverEnded && totalVolume != 0
              ? (int) Math.round(processedVolume * 100.0 / totalVolume)
              : 0;
      msg += " (" + percent + "%)" + " " + rate + "GB/h";
    } else if (totalItems > 0) {
      msg =
          Messages.getString("ProgressConsole.Found")
              + totalItems
    ...
    if (discoverEnded && processingStart != 0) {
      ...
      msg +=
          Messages.getString("ProgressConsole.FinishIn")
              + secsToEnd / 3600
              + "h "
              + (secsToEnd / 60) % 60
              + "m "
