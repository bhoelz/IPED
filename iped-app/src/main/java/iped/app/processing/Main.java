...
public class Main {
  ...
  public void setConfigPath() throws Exception {
    ...
    if (profile != null) {
      configPath =
          new File(configPath, Configuration.PROFILES_DIR + "/" + profile)
  ...
  /**
   * Applies the parsed command-line arguments to this Main instance. This logic was previously in
   * CmdLineArgsImpl.handleSpecificArgs().
   */
  private void applyArgs(CmdLineArgs args) {
    dataSource = new ArrayList<File>();

    if ((args.getDatasources() == null || args.getDatasources().isEmpty())
        && args.getEvidenceToRemove() == null) {
      throw new com.beust.jcommander.ParameterException(
          "parameter '-d' or '-r' required."); //$NON-NLS-1$
    }
    ...
    if (args.getOcr() != null) {
      String list = "";
      for (String o : args.getOcr()) list += (o + iped.parsers.ocr.OCRParser.SUBSET_SEPARATOR);
    ...
    if ((args.isAppendIndex() || args.isContinue() || args.isRestart())
        && outputDir != null
        && !(new File(outputDir, "iped").exists())) {
    ...
    System.setProperty(
        iped.engine.config.LocalConfig.SYS_PROP_APPEND, Boolean.toString(args.isAppendIndex()));
  ...
  private void checkIfAppendingToCompatibleCase(CmdLineArgs args) {
    ...
    cmd.addAll(
        java.util.Arrays.asList("java", "-cp", classpath, Main.class.getCanonicalName(), "-h"));
    ...
    String thisVersion =
        iped.engine.Version.APP_VERSION.substring(
            0, iped.engine.Version.APP_VERSION.lastIndexOf('.'));
    ...
    try {
      ...
      UIPropertyListenerProvider.getInstance()
          .firePropertyChange(
              "mensagem",
              "",
              Messages.getString("Main.Finished")); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      ...
      else LOGGER.error("Processing Error: ", e); // $NON-NLS-1$

      if (e instanceof OutOfMemoryError || (e.getCause() instanceof OutOfMemoryError))
        LOGGER.error(
            "Processing aborted because of OutOfMemoryError. See the possible workarounds at " //$NON-NLS-1$
                + "https://github.com/sepinf-inc/IPED/wiki/Troubleshooting"); //$NON-NLS-1$

    } finally {
      if (manager != null) manager.setProcessingFinished(true);
      if (manager == null || !manager.isSearchAppOpen()) logConfiguration.closeConsoleLogFile();
  ...
  public boolean execute() {
    ...
    if (!cmdLineParams.isNogui()) {
      ...
      ImagePathResolverProvider.set(
          (f, folder) -> new SelectImagePathWithDialog(f, folder).askImagePathInGUI());
  ...
  private static void interruptIfBootstrapDied(
      InputStream is, UIPropertyListenerProvider provider) {
    Thread t =
        new Thread() {
          public void run() {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      Configuration.getInstance()
          .loadConfigurables(iped.configPath, true, ProcessingConfigContributor.INSTANCE);
