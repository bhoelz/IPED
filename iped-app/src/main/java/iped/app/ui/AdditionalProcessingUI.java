...
public final class AdditionalProcessingUI {
    ...
    public static void show(Component parent, IPEDSource source, Collection<? extends IItemId> selectedItems) {
        if (selectedItems == null || selectedItems.isEmpty()) {
            return;
        }
        List<TaskOption> tasks = eligibleTasks();
        if (tasks.isEmpty()) {
            JOptionPane.showMessageDialog(parent, "No additional-processing tasks are available.",
                    "Additional processing", JOptionPane.INFORMATION_MESSAGE);
            return;
        }
        ...
        try {
            ...
            JOptionPane.showMessageDialog(parent, "Could not open the additional index: " + e.getMessage(),
                    "Additional processing", JOptionPane.ERROR_MESSAGE);
            return;
        }

        AdditionalProcessingDialog dialog = new AdditionalProcessingDialog(
                SwingUtilities.getWindowAncestor(parent), source, destination,
                new ArrayList<>(selectedItems), tasks);
    ...
    private static List<TaskOption> eligibleTasks() {
        ...
        try {
            for (AbstractTask task : config.getNewTaskInstances()) {
                AdditionalProcessingCapable annotation = task.getClass().getAnnotation(AdditionalProcessingCapable.class);
                if (annotation != null) {
                    result.add(new TaskOption(task.getClass(), annotation.displayName(), annotation.description()));
                }
            }
    ...
    record TaskOption(Class<? extends AbstractTask> taskClass, String displayName, String description) {
        @Override
    ...
    private static final class AdditionalProcessingDialog extends JDialog {
        ...
        AdditionalProcessingDialog(Window owner, IPEDSource source, IAdditionalDataSource destination,
                                   List<IItemId> selectedItems, List<TaskOption> tasks) {
            ...
            this.threadSpinner = new JSpinner(new SpinnerNumberModel(
        ...
        private void build() {
            ...
            taskBox.addActionListener(e -> description.setText(taskBox.getItemAt(taskBox.getSelectedIndex()).description()));
        ...
        private void start() {
            ...
            List<IItemId> items = skipExisting.isSelected()
                    ? selectedItems.stream().filter(id -> !hasResult(id, option)).toList()
                    : selectedItems;
            if (items.isEmpty()) {
                JOptionPane.showMessageDialog(this, "All selected items already have a result for this task.");
                return;
            }
            AdditionalTaskProgressPanel panel = new AdditionalTaskProgressPanel(this, items.size());
            dispose();
            panel.start(new AdditionalTaskRunner(source, destination, (Integer) threadSpinner.getValue())
    ...
    private static final class AdditionalTaskProgressPanel extends JDialog {
        ...
        void start(CompletableFuture<AdditionalTaskProgress> future) {
            this.future = future;
            setVisible(true);
            future.whenComplete((result, failure) -> SwingUtilities.invokeLater(() -> {
                cancel.setEnabled(false);
                if (failure != null) errors.append(failure.getMessage() + "\n");
                else if (result != null) progress.setValue(result.getProcessed() + result.getErrors());
            }));
        }
        ...
        void update(AdditionalTaskProgress update) {
            SwingUtilities.invokeLater(() -> {
                ...
                progress.setString(update.getProcessed() + update.getErrors() + "/" + update.getTotal());
