...
public class UICaseSearchFilterListener implements CaseSearchFilterListener {
      ...
      if (!caseSearcherFilter.isCancelled())
        try {
          ...
          App.get()
              ...
              .getColumnModel()
              .getColumn(0)
              .setHeaderValue(LocalizedFormat.format(caseSearcherFilter.get().getLength()));
          App.get().resultsTable.getTableHeader().repaint();
          if (App.get().ipedResult.getLength() < 1 << 24
  ...
  private void saveHighlightTerms() throws ParseException, QueryNodeException {
    Set<String> highlightTerms =
        new QueryBuilder(App.get().appCase).getQueryStrings(caseSearcherFilter.getQueryText());
    ...
    } else highlightQuery = caseSearcherFilter.getSearcher().getQuery();
  ...
  public void onStart() {
    progressDialog =
        new ProgressDialog(App.get(), caseSearcherFilter, true, 0, ModalityType.TOOLKIT_MODAL);
  }
}
