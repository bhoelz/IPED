...
public class AIFiltersLoader {
  public static void load() {
    ...
    sortChildrenNodes(
        root, (Comparator<SimpleFilterNode>) new SimpleFilterNode.LocalizedNameComparator());
  ...
  private static void sortChildrenNodes(
      SimpleFilterNode node, Comparator<SimpleFilterNode> comparator) {
  ...
  private static void expandDynamic(IPEDSource source, SimpleFilterNode node) {
    if (node.getDynamic()) {
      try {
        ...
        Collections.sort(
            node.getChildren(),
            new Comparator<SimpleFilterNode>() {
