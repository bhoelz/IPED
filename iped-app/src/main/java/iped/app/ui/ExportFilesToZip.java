...
@Slf4j
public class ExportFilesToZip extends SwingWorker<Boolean, Integer>
  ...
  public ExportFilesToZip(File file, ArrayList<ItemId> uniqueIds) {
    ...
    progressMonitor =
        new ProgressMonitor(App.get(), "", "", 0, uniqueIds.size()); // $NON-NLS-1$ //$NON-NLS-2$
  ...
  protected Boolean doInBackground() {
    ...
    try {
      ...
      for (ItemId item : uniqueIds) {
        ...
        if (e.getLength() != null) entry.setSize(e.getLength());
  ...
  public static void fillZipDates(ZipArchiveEntry entry, IItem item) {
    ...
    try {
      ...
      log.error(
          "Error exporting Dates of item {} to ZIP package: {}", item.getPath(), e.toString());
    }
  }
  ...
  public void propertyChange(PropertyChangeEvent evt) {
    if ("progress" == evt.getPropertyName()) { // $NON-NLS-1$
      ...
      progressMonitor.setNote(
          Messages.getString("ExportFilesToZip.Copying")
              + progress //$NON-NLS-1$
              + Messages.getString("ExportFilesToZip.from")
              + uniqueIds.size()); // $NON-NLS-1$
    }
