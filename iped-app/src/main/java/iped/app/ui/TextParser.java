...
public class TextParser extends CancelableWorker implements ITextParser {
  ...
  public void done() {
    ...
    App.get()
        .hitsDock
        .setTitleText(
            LocalizedFormat.format(hits.size())
  ...
  private ParseContext getTikaContext(IItem item) throws Exception {
    return ParsingTaskContextFactory.create(
        item,
        App.get().getAutoParser(),
        ConfigurationManager.get(),
        App.get().getLastSelectedSource(),
        false);
  }
    ...
    try {
      ...
      while (!this.isCancelled()) {
        ...
        while (!this.isCancelled()
          ...
          off += read;
          totalRead += read;
          if (cis == null) this.firePropertyChange("progress", 0, totalRead); // $NON-NLS-1$
        }
        ...
        TextFragment[] fragments =
            TextHighlighter.getHighlightedFrags(
                lastRowInserted == -1, contents, fieldName, App.FRAG_SIZE);
          ...
          if (numHits < App.MAX_HITS && frag.getScore() > 0) {
            ...
            if (viewRows.size() - 1 < ATextViewer.MAX_LINES) {
              App.get()
                  .getTextViewer()
                  .textViewerModel
                  .fireTableRowsInserted(lastRowInserted + 1, viewRows.size() - 2);
              lastRowInserted = viewRows.size() - 2;
            } else {
              int line_disk_size = ATextViewer.MAX_LINE_SIZE * ATextViewer.CHAR_BYTE_COUNT;
              int line =
                  ATextViewer.MAX_LINES
                      + (int)
                          ((parsedFile.size() - viewRows.get(ATextViewer.MAX_LINES))
                              / line_disk_size);
              App.get()
                  .getTextViewer()
                  .textViewerModel
                  .fireTableRowsInserted(lastRowInserted + 1, line);
        ...
        if (viewRows.size() - 1 < ATextViewer.MAX_LINES) {
          ...
          int line =
              ATextViewer.MAX_LINES
                  + (int)
                      ((parsedFile.size() - viewRows.get(ATextViewer.MAX_LINES)) / line_disk_size);
          App.get()
              .getTextViewer()
              .textViewerModel
              .fireTableRowsInserted(lastRowInserted + 1, line);
      ...
      if (lineBreak && viewRows.size() - 1 < ATextViewer.MAX_LINES) {
        ...
        App.get()
            .getTextViewer()
            .textViewerModel
            .fireTableRowsInserted(lastRowInserted, lastRowInserted);
      }
