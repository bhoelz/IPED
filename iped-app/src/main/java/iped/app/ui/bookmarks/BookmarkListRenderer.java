...
public class BookmarkListRenderer extends JLabel implements ListCellRenderer<BookmarkAndKey> {
  ...
  public void updateUI() {
    ...
    if (c != null) background = new Color(c.getRGB());

    c = UIManager.getColor("List.foreground");
    if (c != null) foreground = new Color(c.getRGB());

    c = UIManager.getColor("nimbusSelectionBackground");
    if (c != null) selBackground = new Color(c.getRGB());

    c = UIManager.getColor("nimbusSelectedText");
    if (c != null) selForeground = new Color(c.getRGB());
  }

  @Override
  public Component getListCellRendererComponent(
      JList<? extends BookmarkAndKey> list,
      BookmarkAndKey value,
      int index,
      boolean isSelected,
