...
public class BookmarksTreeListener
    implements TreeSelectionListener, TreeExpansionListener, IResultSetFilterer {
  ...
  public Set<String> getSelectedBookmarkNames() {
    return selection.stream()
        .filter(b -> b != BookmarksTreeModel.ROOT && b != BookmarksTreeModel.NO_BOOKMARKS)
        .map(b -> b.toString())
  ...
  public void valueChanged(TreeSelectionEvent evt) {
    ...
    for (TreePath path : evt.getPaths()) {
      if (selection.contains(path.getLastPathComponent())) {
        ...
        if (!bookmark.equals(BookmarksTreeModel.ROOT)
            && !bookmark.equals(BookmarksTreeModel.NO_BOOKMARKS)) {
  ...
  public void updateModelAndSelection() {
    ...
    if (bookmarkSet != null && !selection.isEmpty()) {
      ...
      for (Object path : tempSel) {
        if (path == BookmarksTreeModel.ROOT
            || path == BookmarksTreeModel.NO_BOOKMARKS
      ...
      for (Object name : selection) {
        Object[] path =
            name == BookmarksTreeModel.ROOT
                ? new Object[] {BookmarksTreeModel.ROOT}
...
class BookMarkFilter implements IResultSetFilter, IMutableFilter, IBitmapFilter {
  ...
  public IMultiSearchResult filterResult(IMultiSearchResult src)
    ...
    return (MultiSearchResult)
        App.get().appCase.getMultiBookmarks().filterBookmarks(src, (Set<String>) bookmark);
  }
