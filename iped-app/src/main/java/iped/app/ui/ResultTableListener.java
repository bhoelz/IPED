...
public class ResultTableListener implements ListSelectionListener, MouseListener, KeyListener {
  ...
  public void valueChanged(ListSelectionEvent evt) {
    ...
    if (!syncingSelectedItems) {
      ...
      App.get()
          .gallery
          .scrollRectToVisible(App.get().gallery.getCellRect(galleryRow, galleyCol, false));
  ...
  private synchronized void processSelectedFile(boolean isMouseEvent) {
    ...
    if (viewIndex != -1) {
      ...
      if (docId != lastAppDoc && (!isMouseEvent || docId == lastTableDoc)) {
        ...
        App.get()
            .subitemDock
            .setTitleText(Messages.getString("SubitemTableModel.Subitens")); // $NON-NLS-1$
        App.get()
            .duplicateDock
            .setTitleText(Messages.getString("DuplicatesTableModel.Duplicates")); // $NON-NLS-1$
        App.get()
            .parentDock
            .setTitleText(Messages.getString("ParentTableModel.ParentCount")); // $NON-NLS-1$
        App.get()
            .referencesDock
            .setTitleText(Messages.getString("ReferencesTab.Title")); // $NON-NLS-1$
        App.get()
            .referencedByDock
            .setTitleText(Messages.getString("ReferencedByTab.Title")); // $NON-NLS-1$
  ...
  private void showContextMenu(IItemId itemId, MouseEvent evt) {
    ...
    if (evt.getButton() == MouseEvent.BUTTON3
        && (evt.getModifiersEx() & MouseEvent.CTRL_DOWN_MASK) == MouseEvent.CTRL_DOWN_MASK) {
      ...
      (new FieldValuePopupMenu(id, field, value))
          .show((Component) evt.getSource(), evt.getX(), evt.getY());
      return;
    }
  ...
  public void keyPressed(KeyEvent evt) {
    ...
    if (App.get().resultsTable.getSelectedRow() == -1) return;

    if (evt.getKeyCode() == KeyEvent.VK_C
        && ((evt.getModifiersEx() & KeyEvent.CTRL_DOWN_MASK) != 0)) {
      int selCol = App.get().resultsTable.getSelectedColumn();
      if (selCol < 0) return;
      String value =
          getCell(App.get().resultsTable, App.get().resultsTable.getSelectedRow(), selCol);
      ...
      evt.consume();
        ...
        && ((evt.getModifiersEx() & KeyEvent.ALT_DOWN_MASK) != 0)) {
      ...
      evt.consume();
    } else BookmarksManager.get().keyPressed(evt);
  }

  private ProgressDialog createProgressDialog() {
    ProgressDialog d =
        new ProgressDialog(App.get(), null, false, 200, Dialog.ModalityType.APPLICATION_MODAL);
          ...
          try {
            ...
            for (int i = 0; i < selectedRows.length; i++) {
              ...
              Document selectedDoc =
                  App.get().appCase.getSearcher().storedFields().document(selectedDocId);
              ...
              if (query != null) {
                ...
                if (result.getLength() > 0) {
                  log.debug(
                      "Found {} results of sourceId {} id {}",
                      result.getLength(),
                      selectedItemId.getSourceId(),
                      selectedItemId.getId());
  ...
  /**
   * Add a simple "type-to-find" feature to the table. It works on the currently sorted column, if
   * it uses a String comparator, so it will not work on numeric and date columns.
   */
  @Override
  public void keyTyped(KeyEvent evt) {
    ...
    if (!((RowComparator) ((ResultTableRowSorter) table.getRowSorter()).getComparator(sortCol))
        .isStringComparator()) return;

    if (BookmarksManager.get().hasSingleKeyShortcut()) return;

    long t = System.currentTimeMillis();
    if (t - lastKeyTime > 500) lastKeyString = ""; // $NON-NLS-1$
  ...
  private int findRow(JTable table, int from, int to, int col, String search) {
    ...
    if (from == to && compare(getCell(table, from, col), search) == 0) return from;
    return -1;
  }
  ...
  private String getCell(JTable table, int row, int col) {
    ...
    if (App.get().getFontStartTag() != null) cell = cell.replace(App.get().getFontStartTag(), "");
    return cell.replace(HitsTableModel.htmlStartTag, "")
        .replace(HitsTableModel.htmlEndTag, "")
        .replace(ATextViewer.HIGHLIGHT_START_TAG, "")
        .replace(ATextViewer.HIGHLIGHT_END_TAG, "");
  }
}
