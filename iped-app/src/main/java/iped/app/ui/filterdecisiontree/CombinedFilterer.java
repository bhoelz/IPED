...
public class CombinedFilterer implements IResultSetFilterer, IFilterChangeListener {
  ...
  private final Map<IFilter, FutureBitSetResult> cachedBitSet =
      new HashMap<IFilter, FutureBitSetResult>();
  ...
  /**
   * Class that represents a future calculated bitset of a filter. When the filter is added to
   * Combined filter its bitset is calculated in background, so when it is effectivelly calculated,
   * it can be used to filter results.
   *
   * @author patrick.pdb
   */
    ...
    public RoaringBitmap[] get() throws InterruptedException, ExecutionException {
      try {
        ...
        if ((filter instanceof IMutableFilter)
            || (bitsets == null)) { // IMutableFilter aren't cached
  ...
  public static RoaringBitmap getAllSetBitSet(int length) {
