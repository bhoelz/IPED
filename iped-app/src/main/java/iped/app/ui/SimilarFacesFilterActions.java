...
public class SimilarFacesFilterActions {
  ...
  public static void searchSimilarFaces(boolean external) {
    ...
    if (external) {
      ...
      if (file != null) {
        ...
        try (BufferedInputStream bis =
          ...
          App.get()
              ...
              .parse(
                  ...
                  (Metadata) newSimilarFacesRefItem.getMetadata(),
                  new ParseContext());
        ...
        final WaitDialog wait =
            new WaitDialog(app, Messages.getString("FaceSimilarity.LoadingFace"));

        FaceFeatureExtractor callable =
            new FaceFeatureExtractor(newSimilarFacesRefItem, app.appCase.getModuleDir()) {
              @Override
        ...
        try {
          ...
          JOptionPane.showMessageDialog(
              App.get(),
              e.getMessage(),
              Messages.getString("FaceSimilarity.ExternalTitle"),
    ...
    if (newSimilarFacesRefItem != null) {
      SimilarFacesOptionsDialog opt =
          new SimilarFacesOptionsDialog(app, newSimilarFacesRefItem, minScore, mode);
      opt.setVisible(true);
      if (opt.isOk()) {
  ...
  private abstract static class FaceFeatureExtractor implements Callable<Void> {
    ...
    private static final String SCRIPT_PATH =
        TaskInstallerConfig.SCRIPT_BASE + "/FaceRecognitionTask.py";
    ...
    public Void call() throws Exception {
      ...
      try {
        if (FaceFeatureExtractor.task == null) {
          ...
          AbstractTaskPropertiesConfig taskConfig =
              (AbstractTaskPropertiesConfig)
                  ConfigurationManager.get().getTaskConfigurable(CONF_FILE);
        ...
        List<KnnVector> faces =
            (List<KnnVector>) item.getExtraAttribute(SimilarFacesSearch.FACE_FEATURES);
