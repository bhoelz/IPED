...
public class ColumnsManager implements Serializable, IColumnsManager {
  ...
  public static final String[] groupNames = {
    Messages.getString("ColumnsManager.Basic"),
    Messages.getString("ColumnsManager.HashDB"),
    Messages.getString("ColumnsManager.Advanced"), // $NON-NLS-2$ //$NON-NLS-2$
    Messages.getString("ColumnsManager.Common"), // $NON-NLS-2$
    Messages.getString("ColumnsManager.Communication"),
    Messages.getString("ColumnsManager.Audio"), // $NON-NLS-2$
    Messages.getString("ColumnsManager.Image"),
    Messages.getString("ColumnsManager.Video"), // $NON-NLS-1$
    Messages.getString("ColumnsManager.PDF"),
    Messages.getString("ColumnsManager.Office"), // $NON-NLS-1$ //$NON-NLS-2$
    Messages.getString("ColumnsManager.HTML"),
    Messages.getString("ColumnsManager.Regex"), // $NON-NLS-1$ //$NON-NLS-2$
    Messages.getString("ColumnsManager.Language"),
    Messages.getString("ColumnsManager.NamedEntity"), // $NON-NLS-1$ //$NON-NLS-2$
    Messages.getString("ColumnsManager.PeerToPeer"),
    Messages.getString("ColumnsManager.UFED"), // $NON-NLS-1$ //$NON-NLS-2$
    Messages.getString("ColumnsManager.WindowsEvt"), // $NON-NLS-1$ //$NON-NLS-2$
    Messages.getString("ColumnsManager.Other"),
    Messages.getString("ColumnsManager.All")
  }; //$NON-NLS-1$ //$NON-NLS-2$
  ...
  private static final File getGlobalColsFile() {
    ...
    name += ".dat"; // $NON-NLS-1$
    return new File(
        System.getProperty("user.home") + "/.iped/" + name); // $NON-NLS-1$ //$NON-NLS-2$
  }

  protected static final List<Integer> defaultWidths =
      Arrays.asList(50, 100, 200, 50, 50, 100, 60, 150, 155, 155, 155, 155, 155, 155, 250, 2000);

  protected static final String[] defaultFields = {
    ...
    IndexItem.CHANGED,
    IndexItem.TIMESTAMP,
    IndexItem.TIME_EVENT,
    IndexItem.HASH,
    IndexItem.PATH
  };

  protected static final String[] extraFields = {
    IndexItem.CARVED,
    ...
    StandardParser.PARSER_EXCEPTION,
    ...
    ExtraProperties.TIKA_PARSER_USED,
    IndexItem.META_ADDRESS,
    IndexItem.MFT_SEQUENCE,
    IndexItem.FILESYSTEM_ID
  };
  ...
  public static ColumnsManager getInstance() {
    if (instance == null) instance = new ColumnsManager();
    return instance;
  }
  ...
  protected File getColStateFile() {
    ...
    File cols = caseCols;
    if (!cols.exists()) cols = globalCols;
    return cols;
  }
  ...
  private void loadSavedCols() {
    ...
    if (cols.exists()) {
      try {
        ...
        if (loadedFields.size() > 0) {
          ...
          if (removed > 0) loadedFields = (ArrayList<String>) colState.visibleFields.clone();
        }
    ...
    if (!lastColsOk) {
      ...
      for (String col : defaultFields) loadedFields.add(col);
    ...
    for (i = 0; i < docs.length; i += interval) {
      ...
      try {
        ...
        for (String field : indexFields) {
          if (doc.getField(field) != null) dinamicFields.add(field);
        }
      } catch (IOException e) {
        e.printStackTrace();
      }
  ...
  void updateDinamicFields() {
    ...
    for (String f : allExtraAttrs) {
      if (f.startsWith(RegexTask.REGEX_PREFIX)) regexFields.add(f);
      else if (f.startsWith(LanguageDetectTask.LANGUAGE_PREFIX)) languageFields.add(f);
      else if (f.startsWith(ExtraProperties.HASHDB_PREFIX)
          || f.startsWith(PhotoDNALookup.PHOTO_DNA_HIT_PREFIX)) hashDbFields.add(f);
      else extraAttrs.add(f);
    }

    for (String f : indexFields) {
      if (f.startsWith(ExtraProperties.AUDIO_META_PREFIX)) audioFields.add(f);
      else if (f.startsWith(ExtraProperties.IMAGE_META_PREFIX)) imageFields.add(f);
      else if (f.startsWith(ExtraProperties.VIDEO_META_PREFIX)) videoFields.add(f);
      else if (f.startsWith(ExtraProperties.PDF_META_PREFIX)) pdfFields.add(f);
      else if (f.startsWith(ExtraProperties.OFFICE_META_PREFIX)) officeFields.add(f);
      else if (f.startsWith(ExtraProperties.HTML_META_PREFIX)) htmlFields.add(f);
      else if (f.startsWith(NamedEntityTask.NER_PREFIX)) nerFields.add(f);
      else if (f.startsWith(ExtraProperties.P2P_META_PREFIX)) p2pFields.add(f);
      else if (f.startsWith(ExtraProperties.UFED_META_PREFIX)) ufedFields.add(f);
      else if (StringUtils.startsWithAny(
              f,
              Message.MESSAGE_PREFIX,
              ExtraProperties.CONVERSATION_PREFIX,
              ExtraProperties.COMMUNICATION_PREFIX)
          || ExtraProperties.COMMUNICATION_BASIC_PROPS.contains(f)) communicationFields.add(f);
      else if (f.startsWith(ExtraProperties.COMMON_META_PREFIX)) commonFields.add(f);
    ...
    String[][] customGroups =
        new String[][] {
          defaultFields.clone(),
          hashDbFields.toArray(new String[0]),
          ...
          p2pFields.toArray(new String[0]),
          ufedFields.toArray(new String[0]),
          winEvtFields.toArray(new String[0])
        };
    ...
    for (String f : indexFields) {
      ...
      if (insertField) otherFields.add(f);
    }

    fieldGroups = new String[customGroups.length + 2][];
    for (int i = 0; i < customGroups.length; i++) fieldGroups[i] = customGroups[i];
    fieldGroups[fieldGroups.length - 2] = otherFields.toArray(new String[0]);
    fieldGroups[fieldGroups.length - 1] = indexFields.clone();

    for (String[] fields : fieldGroups) Arrays.sort(fields, Collator.getInstance());
  }
}
