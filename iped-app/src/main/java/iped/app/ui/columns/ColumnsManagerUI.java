...
public class ColumnsManagerUI implements ActionListener {
  ...
  protected JCheckBox autoManage =
      new JCheckBox(Messages.getString("ColumnsManager.AutoManageCols")); // $NON-NLS-1$
  protected JComboBox<Object> combo;
  protected JScrollPane scrollList;
  protected JLabel showColsLabel =
      new JLabel(Messages.getString("ColumnsManager.ShowCols")); // $NON-NLS-1$
  ...
  public static ColumnsManagerUI getInstance() {
    if (instance == null) instance = new ColumnsManagerUI();
    return instance;
  }
  ...
  public void updateDinamicCols() {
    if (!columnsManager.isAutoManageCols()) return;

    if (App.get().getResults().getLength() == App.get().appCase.getTotalItems()
        || App.get().getResults().getLength() == 0) return;

    final ProgressDialog progress =
        new ProgressDialog(App.get(), null, false, 100, ModalityType.TOOLKIT_MODAL);
  ...
  private void updateDinamicCols(Set<String> dinamicFields) {
    ...
    if (dinamicFields == null) return;

    Set<String> colNamesToPin = new HashSet<>();
    for (int i = 2;
        i < Math.min(firstColsToPin, App.get().getResultsTable().getColumnCount());
        i++) {
      colNamesToPin.add(
          App.get()
              .getResultsTable()
    ...
    for (String field : (List<String>) columnsManager.colState.visibleFields.clone()) {
      if (!dinamicFields.contains(field)
          && !colNamesToPin.contains(field.toLowerCase())
          && !field.equals(
              BasicProps.LENGTH)) // length header changes to listed items total size info
      {
    ...
    for (String field : dinamicFields) {
      if (!columnsManager.colState.visibleFields.contains(field)) updateGUICol(field, true);
    }
  ...
  public void resetColumns(List<String> newCols, List<Integer> widths) {
    for (String field : (List<String>) columnsManager.colState.visibleFields.clone())
      if (!newCols.contains(field)
          ...
          && !field.equals(ResultTableModel.BOOKMARK_COL)) updateGUICol(field, false);
  ...
  protected void updatePanelList() {
    ...
    for (String fieldName : fieldNames) {
      if (filter.isEmpty() || fieldName.toLowerCase().indexOf(filter) >= 0) {
        ...
        if (columnsManager.colState.visibleFields.contains(
            LocalizedProperties.getNonLocalizedField(fieldName))) check.setSelected(true);
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (e.getSource().equals(autoManage)) columnsManager.setAutoManageCols(autoManage.isSelected());
  ...
  private void updateGUICol(String colName, boolean insert) {
    ...
    if (insert) {
      ...
      } else modelIdx += ResultTableModel.fixedCols.length;

      TableColumn tc = new TableColumn(modelIdx);
      if (lastWidths.containsKey(colName)) tc.setPreferredWidth(lastWidths.get(colName));
  ...
  public void setColumnRenderer(TableColumn tc) {
    if (ResultTableModel.SCORE_COL.equals(
        columnsManager
            .getLoadedFields()
            .get(tc.getModelIndex() - ResultTableModel.fixedCols.length)))
