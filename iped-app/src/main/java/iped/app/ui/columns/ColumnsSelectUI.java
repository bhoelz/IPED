...
public class ColumnsSelectUI extends ColumnsManagerUI {
  ...
  protected JCheckBox toggleSelectUnselectAllCheckBox =
      new JCheckBox(Messages.getString("ColumnsManager.ToggleSelectAll"));
  protected JButton selectVisibleButton =
      new JButton(Messages.getString("ColumnsManager.SelectVisible"));
  protected JButton okButton = new JButton("OK");

  protected Map<String, CheckBoxState> allCheckBoxesState = new HashMap<>();

  public static ColumnsSelectUI getInstance() {
    if (instance == null) instance = new ColumnsSelectUI();
    return instance;
  }
    ...
    for (String fieldName : fieldNames) {
      if (filter.isEmpty() || fieldName.toLowerCase().indexOf(filter) >= 0) {
        ...
        CheckBoxState checkBoxState =
            allCheckBoxesState.get(LocalizedProperties.getNonLocalizedField(fieldName));
  ...
  private void initializeAllCheckBoxesState() {
    for (int i = 0; i < ColumnsManager.getInstance().fieldGroups.length; i++) {
      ...
      fieldNames.forEach(
          f ->
              allCheckBoxesState.putIfAbsent(
                  LocalizedProperties.getNonLocalizedField(f), new CheckBoxState(false)));
    }
  }
