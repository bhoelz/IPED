...
public class DuplicatesTableModel extends BaseTableModel {
  ...
  public Object getValueAt(int row, int col) {
    if (col == 3) {
      try {
        Document doc =
            App.get().appCase.getSearcher().storedFields().document(results.getLuceneIds()[row]);
  ...
  public void onListItemsResultsComplete() {
    App.get()
        .duplicateDock
        .setTitleText(results.getLength() + Messages.getString("DuplicatesTableModel.Duplicates"));
  }
