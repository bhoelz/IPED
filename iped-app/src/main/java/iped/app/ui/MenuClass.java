...
public class MenuClass extends JPopupMenu {
  ...
  JMenuItem exportHighlighted,
      ...
      saveBookmarks,
      loadBookmarks,
      checkHighlightedAndSubItems,
      uncheckHighlightedAndSubItems,
      checkHighlightedAndParent,
      uncheckHighlightedAndParent,
      checkHighlightedAndReferences,
      uncheckHighlightedAndReferences,
      checkHighlightedAndReferencedBy,
      uncheckHighlightedAndReferencedBy,
      ...
      saveColLayout,
      addToGraph,
      navigateToParentChat,
      pinFirstColumns,
      similarImagesCurrent,
      similarImagesExternal,
      similarFacesCurrent,
      similarFacesExternal,
      toggleTimelineView,
      uiZoom,
      catIconSize,
      savePanelsLayout,
  ...
  public MenuClass(IItem item) {
    ...
    checkHighlighted =
        new JMenuItem(Messages.getString("MenuClass.CheckHighlighted")); // $NON-NLS-1$
    ...
    uncheckHighlighted =
        new JMenuItem(Messages.getString("MenuClass.UnCheckHighlighted")); // $NON-NLS-1$
    ...
    checkHighlightedAndSubItems =
        new JMenuItem(Messages.getString("MenuClass.CheckHighlightedAndSubItems")); // $NON-NLS-1$
    checkHighlightedAndSubItems.addActionListener(menuListener);
    checkHighlightedAndSubItems.setAccelerator(
        KeyStroke.getKeyStroke(KeyEvent.VK_R, ActionEvent.CTRL_MASK));
    submenu.add(checkHighlightedAndSubItems);

    uncheckHighlightedAndSubItems =
        new JMenuItem(Messages.getString("MenuClass.UncheckHighlightedAndSubItems")); // $NON-NLS-1$
    uncheckHighlightedAndSubItems.addActionListener(menuListener);
    uncheckHighlightedAndSubItems.setAccelerator(
    ...
    checkHighlightedAndParent =
        new JMenuItem(Messages.getString("MenuClass.CheckHighlightedAndParent")); // $NON-NLS-1$
    checkHighlightedAndParent.addActionListener(menuListener);
    checkHighlightedAndParent.setAccelerator(
        KeyStroke.getKeyStroke(KeyEvent.VK_P, ActionEvent.CTRL_MASK));
    submenu.add(checkHighlightedAndParent);

    uncheckHighlightedAndParent =
        new JMenuItem(Messages.getString("MenuClass.UncheckHighlightedAndParent")); // $NON-NLS-1$
    uncheckHighlightedAndParent.addActionListener(menuListener);
    uncheckHighlightedAndParent.setAccelerator(
    ...
    checkHighlightedAndReferences =
        new JMenuItem(Messages.getString("MenuClass.CheckHighlightedAndReferences")); // $NON-NLS-1$
    checkHighlightedAndReferences.addActionListener(menuListener);
    checkHighlightedAndReferences.setAccelerator(
        KeyStroke.getKeyStroke(KeyEvent.VK_F, ActionEvent.CTRL_MASK));
    submenu.add(checkHighlightedAndReferences);

    uncheckHighlightedAndReferences =
        new JMenuItem(
            Messages.getString("MenuClass.UncheckHighlightedAndReferences")); // $NON-NLS-1$
    uncheckHighlightedAndReferences.addActionListener(menuListener);
    uncheckHighlightedAndReferences.setAccelerator(
    ...
    checkHighlightedAndReferencedBy =
        new JMenuItem(
            Messages.getString("MenuClass.CheckHighlightedAndReferencedBy")); // $NON-NLS-1$
    checkHighlightedAndReferencedBy.addActionListener(menuListener);
    checkHighlightedAndReferencedBy.setAccelerator(
        KeyStroke.getKeyStroke(KeyEvent.VK_D, ActionEvent.CTRL_MASK));
    submenu.add(checkHighlightedAndReferencedBy);

    uncheckHighlightedAndReferencedBy =
        new JMenuItem(
            Messages.getString("MenuClass.UncheckHighlightedAndReferencedBy")); // $NON-NLS-1$
    uncheckHighlightedAndReferencedBy.addActionListener(menuListener);
    uncheckHighlightedAndReferencedBy.setAccelerator(
    ...
    additionalProcessing.setEnabled(
        App.get().resultsTable.getSelectedRowCount() > 0
            && AdditionalProcessingUI.hasEligibleTasks());
    this.add(additionalProcessing);

    additionalResultsFilter =
        new JCheckBoxMenuItem(
            "Has additional task result",
            App.get().additionalResultsFilterer != null
                && App.get().additionalResultsFilterer.isEnabled());
    ...
    exportHighlighted =
        new JMenuItem(Messages.getString("MenuClass.ExportHighlighted")); // $NON-NLS-1$
    ...
    exportCheckedToZip =
        new JMenuItem(Messages.getString("MenuClass.ExportCheckedToZip")); // $NON-NLS-1$
    ...
    copyHighlighted =
        new JMenuItem(Messages.getString("MenuClass.ExportProps.Highlighed")); // $NON-NLS-1$
    ...
    clearSearchHistory =
        new JMenuItem(Messages.getString("MenuClass.ClearSearches")); // $NON-NLS-1$
    ...
    JMenu layoutAppearance =
        new JMenu(Messages.getString("MenuClass.LayoutAppearance")); // $NON-NLS-1$
    ...
    savePanelsLayout =
        new JMenuItem(Messages.getString("MenuClass.SavePanelsLayout")); // $NON-NLS-1$
    savePanelsLayout.addActionListener(menuListener);
    layoutAppearance.add(savePanelsLayout);

    loadPanelsLayout =
        new JMenuItem(Messages.getString("MenuClass.LoadPanelsLayout")); // $NON-NLS-1$
    ...
    changeGalleryColCount =
        new JMenuItem(Messages.getString("MenuClass.ChangeGalleryColCount")); // $NON-NLS-1$
      ...
      for (Theme theme : themes) {
        JRadioButtonMenuItem themeItem =
            new JRadioButtonMenuItem(
                theme.getName(), theme.equals(ThemeManager.getInstance().getCurrentTheme()));
    ...
    previewScreenshot =
        new JMenuItem(Messages.getString("MenuClass.CopyViewerImage")); // $NON-NLS-1$
      ...
      enableGoToChat =
          MediaTypes.isInstanceOf(item.getMediaType(), MediaTypes.CHAT_MESSAGE_MIME)
              ...
              || (VCardParser.VCARD_MIME.equals(item.getMediaType())
                  && item.getMetadataValue(ExtraProperties.COMMUNICATION_FROM) != null
                  && item.getMetadataValue(ExtraProperties.COMMUNICATION_TO) != null);
    }
    ...
    similarImagesCurrent =
        new JMenuItem(Messages.getString("MenuClass.FindSimilarImages.Current")); // $NON-NLS-1$
    similarImagesCurrent.addActionListener(menuListener);
    similarImagesCurrent.setEnabled(
        item != null && item.getExtraAttribute(ImageSimilarityTask.IMAGE_FEATURES) != null);
    submenu.add(similarImagesCurrent);

    similarImagesExternal =
        new JMenuItem(Messages.getString("MenuClass.FindSimilarImages.External")); // $NON-NLS-1$
    ...
    similarFacesCurrent =
        new JMenuItem(Messages.getString("MenuClass.FindSimilarFaces.Current")); // $NON-NLS-1$
    similarFacesCurrent.addActionListener(menuListener);
    similarFacesCurrent.setEnabled(
        item != null && item.getExtraAttribute(SimilarFacesSearch.FACE_FEATURES) != null);
    submenu.add(similarFacesCurrent);

    similarFacesExternal =
        new JMenuItem(Messages.getString("MenuClass.FindSimilarFaces.External")); // $NON-NLS-1$
    ...
    addToGraph.setEnabled(
        App.get().appGraphAnalytics.isEnabled()
            && item != null
            && item.getMetadataValue(ExtraProperties.COMMUNICATION_FROM) != null
            && item.getMetadataValue(ExtraProperties.COMMUNICATION_TO) != null);
  ...
  public void addExportTreeMenuItems(JComponent menu) {
    ...
    exportTreeChecked =
        new JMenuItem(Messages.getString("MenuClass.ExportTree.Checked")); // $NON-NLS-1$
    exportTreeChecked.addActionListener(menuListener);
    menu.add(exportTreeChecked);

    exportCheckedTreeToZip =
        new JMenuItem(Messages.getString("MenuClass.ExportTreeToZip.Checked")); // $NON-NLS-1$
