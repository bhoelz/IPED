    ...
    if (model instanceof SearchResultTableModel) {
      if (colName.equalsIgnoreCase(ResultTableModel.BOOKMARK_COL)) {
        ...
        if (value != null) {
          ...
          || (model instanceof DuplicatesTableModel
              && (colName.equalsIgnoreCase(IndexItem.PATH)
                  || colName.equalsIgnoreCase(
                      LocalizedProperties.getLocalizedField(IndexItem.PATH))))) {
        try {
          IItemId item = ((SearchResultTableModel) model).getSearchResult().getItem(idx);
          var executedTasks =
              App.get().appCase.getAdditionalDataSourceManager().getAllExecutedTasks(item.getId());
