...
public class SimilarFacesOptionsDialog extends JDialog {
  ...
  public SimilarFacesOptionsDialog(Window parent, IItem item, int minScore, int mode) {
    ...
    if (img != null) {
      ...
      if (rects.size() > 1) {
        ...
        multi.add(labelSel);
        JLabel labelClick =
            new JLabel("(" + Messages.getString("FaceSimilarity.ClickToSelect") + ")");
  ...
  private String getSelText() {
    return Messages.getString("FaceSimilarity.SelectedFaces")
        + ": "
        + selectedIdxs.size()
        + " / "
        + rects.size();
  ...
  private static final List<Rectangle2D> toRectList(
      Object faceLocations, double zoom, int w, int h) {
