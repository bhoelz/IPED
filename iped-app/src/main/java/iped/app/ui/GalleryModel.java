...
public class GalleryModel extends AbstractTableModel {
  ...
  /**
   * Max Sleuthkit connection pool size. Using more threads than this sometimes caused deadlock in
   * TSK if many streams are asked at the same time
   */
  ...
  public final Map<IItemId, GalleryValue> cache =
      new LinkedHashMap<IItemId, GalleryValue>() {
        private static final long serialVersionUID = 1L;
  ...
  private static final BufferedImage errorImg =
  ...
  private boolean isAnimationImage(Document doc, String mediaType) {
    return MetadataUtil.isImageSequence(mediaType)
        || doc.get(ExtraProperties.ANIMATION_FRAMES_PROP) != null;
  }
  ...
  public Object getValueAt(final int row, final int col) {
    ...
    if (imgThumbTask == null) {
      try {
        ...
        galleryThreads =
            Math.min(imgThumbTask.getImageThumbConfig().getGalleryThreads(), MAX_TSK_POOL_SIZE);
          ...
          public void run() {
            ...
            try {
              ...
              if (image == null) {
                ...
                if (Boolean.valueOf(doc.get(IndexItem.ISDIR))) {
                  ...
                  value.icon = IconManager.getFolderIconGallery();
                    ...
                    && !isSupportedImage(mediaType)
                    && !isSupportedVideo(mediaType)) {
                  value.unsupportedType = true;
                ...
                if (image == null
                    && stream != null
                    && imgThumbTask.getImageThumbConfig().isExtractThumb()
              ...
              if (image == null || image == errorImg) {
                if (value.icon == null) value.icon = errorIcon;
              } else {
                // Resize image only if it is too large (> 2x the desired thumbSize)
                if (image.getWidth() > getThumbSize() * 2
                    || image.getHeight() > getThumbSize() * 2) {
  ...
  private BufferedImage getViewImage(int docID, String hash, boolean isVideo) throws IOException {
    ...
    if (hashFile.exists()) {
      ...
      try {
        ...
        PreviewRepositoryManager.get(baseFolder.getParentFile())
            .consumePreview(
                key,
                inputStream -> {
