...
public class BookmarksController implements IBookmarksController {
  ...
  public static final String HISTORY_DIV =
      Messages.getString("BookmarksController.HistoryDelimiter"); // $NON-NLS-1$
  ...
  public void addToRecentSearches(String text) {
    ...
    if (!text.equals(HISTORY_DIV)
        && !text.trim().isEmpty()
        && !App.get().appCase.getKeywords().contains(text)) {
    ...
    SwingUtilities.invokeLater(
        new Runnable() {
          ...
          public void run() {
            ...
            App.get()
                .checkBox
                .setSelected(App.get().appCase.getMultiBookmarks().getTotalChecked() > 0);
    ...
    if (fileChooser.showOpenDialog(App.get()) == JFileChooser.APPROVE_OPTION) {
      ...
      try {
        ...
        JOptionPane.showMessageDialog(
            App.get(),
            Messages.getString("BookmarksController.LoadSuccess"), // $NON-NLS-1$
            Messages.getString("BookmarksController.LoadSuccess.Title"),
        ...
        JOptionPane.showMessageDialog(
            App.get(),
            Messages.getString("BookmarksController.LoadError"), // $NON-NLS-1$
            Messages.getString("BookmarksController.LoadError.Title"),
  ...
  public void updateUIHistory() {
    ...
    for (String word : App.get().appCase.getKeywords()) App.get().queryComboBox.addItem(word);
  ...
  public void askAndSaveState() {
    ...
    if (fileChooser.showSaveDialog(App.get()) == JFileChooser.APPROVE_OPTION) {
      File file = fileChooser.getSelectedFile();
      if (!file.getName().endsWith(Bookmarks.EXT)) file = new File(file.getPath() + Bookmarks.EXT);

      try {
        App.get().appCase.getMultiBookmarks().saveState(file);
        JOptionPane.showMessageDialog(
            App.get(),
            Messages.getString("BookmarksController.SaveSuccess"), // $NON-NLS-1$
            Messages.getString("BookmarksController.SaveSuccess.Title"),
            JOptionPane.INFORMATION_MESSAGE); // $NON-NLS-1$
        ...
        JOptionPane.showMessageDialog(
            App.get(),
            Messages.getString("BookmarksController.SaveError"), // $NON-NLS-1$
            Messages.getString("BookmarksController.SaveError.Title"),
            JOptionPane.ERROR_MESSAGE); // $NON-NLS-1$
      }
    }
  }
