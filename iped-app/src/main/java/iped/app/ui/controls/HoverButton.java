...
/**
 * A flat button with no background or border. Border and background are only painted when the mouse
 * is over it, and with a brighter color when it is pressed. It is "theme aware", so colors will
 * change if a new theme is activated. Currently it only displays icons, but it could handle text.
 */
