...
public class AppMain {
            ...
            public void run() {
              ...
              if (warn != null) {
                JOptionPane.showMessageDialog(
                    null,
                    warn,
                    Messages.getString("AppMain.warn.Title"), // $NON-NLS-1$
  ...
  private File detectLibDir() throws URISyntaxException {
    ...
    File jarFile = null;
    if (url.toURI().getAuthority() == null) jarFile = new File(url.toURI());
    else jarFile = new File(url.toURI().getSchemeSpecificPart());
  ...
  public void start(File casePath, Manager processingManager, String[] args) {
    ...
    try {
      ...
      if (casesPathFile == null) casesPathFile = casePath;

      File logParent = casesPathFile;
      if (isMultiCase && casesPathFile.isFile()) logParent = casesPathFile.getParentFile();

      File logFile = new File(logParent, appLogFileName).getCanonicalFile();
      if ((logFile.exists() && !IOUtil.canWrite(logFile))
          || !IOUtil.canCreateFile(logFile.getParentFile())) {
        logFile = new File(System.getProperty("java.io.tmpdir"), appLogFileName);
      }

      if (libDir == null) libDir = detectLibDir();
  ...
  private static void showError(Exception e) {
    ...
    JOptionPane.showMessageDialog(
        null,
        "Error: " + new String(baos.toByteArray()), // $NON-NLS-1$
