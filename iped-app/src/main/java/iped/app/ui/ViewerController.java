...
/**
 * Central controller for all viewers. To add a new viewer it should be added in createViewers()
 * method.
 *
 * @author Wladimir
 */
public class ViewerController {
  ...
  public ViewerController() {
    ...
    viewersRepository.addViewer(
        new ReferencedFileViewer(
            viewersRepository,
            new AttachmentSearcherImpl(),
            () -> {
      ...
      public void run() {
        try {
                ...
                new Runnable() {
                  ...
                  public void run() {
                    String nativeLibFolder =
                        new File(moduledir, "lib/nativeview").getAbsolutePath();
          ...
          JOptionPane.showMessageDialog(
              null, Messages.getString("ViewerController.OfficeViewerUnSupported")); // $NON-NLS-1$
    ...
    if (viewer.equals(viewersRepository)) {
      if (viewersRepository.getPanel().isShowing()) {
        if (officeViewer != null
            && viewersRepository.getCurrentViewer() != null
  ...
  public void loadFile(
      IStreamSource file, IStreamSource viewFile, String contentType, Set<String> highlightTerms) {
    if (!isInitialized()) {
      return;
    }
    this.file = file;
    ...
    if (file != null && !isFixed) {
      ...
      if (!bestViewer.getPanel().isShowing()) {
        ...
        if (!viewerDock.getExtendedMode().equals(ExtendedMode.MINIMIZED)
            && isContainerVisibleTab(viewerDock)) {
          if (viewerDock != null) {
  ...
  public void updateViewer(AbstractViewer viewer, boolean clean, boolean forceLoad) {
    if (viewer.getPanel().isShowing() || (viewer.equals(textViewer) && hasHits()) || forceLoad) {
      ...
      if (dock != null) {
        boolean hitsEnabled =
            viewFile != null
                && ((hasHits() && viewer.getHitsSupported() == 0)
                    || (viewer.getHitsSupported() == 1));
  ...
  public void notifyAppLoaded() {
    ...
    boolean faceRecognitionEnabled =
        Optional.ofNullable(ConfigurationManager.get())
            ...
            .map(e -> e.isEnabled())
            .orElse(false);
    boolean hasFaceRecognitionInIndex =
        App.get().appCase.getLeafReader().getFieldInfos().fieldInfo(ExtraProperties.FACE_COUNT)
            != null;
    boolean enableHighlightFacesButton = faceRecognitionEnabled || hasFaceRecognitionInIndex;

    boolean ageEstimationEnabled =
        Optional.ofNullable(ConfigurationManager.get())
            .map(cm -> cm.getEnableTaskConfigurable(AgeEstimationConfig.enableParam))
            .map(e -> e.isEnabled())
            .orElse(false);
    boolean hasAgeEstimationInIndex =
        App.get().appCase.getLeafReader().getFieldInfos().fieldInfo(ExtraProperties.FACE_AGE_LABELS)
            != null;
    boolean enableAgeEstimationCombo = ageEstimationEnabled || hasAgeEstimationInIndex;

    viewersRepository.getViewerList().stream()
