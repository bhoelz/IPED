...
public class FileProcessor extends CancelableWorker<Void, Void> implements IFileProcessor {
  ...
  private static volatile FileProcessor parsingTask;
    ...
    if (docId >= 0) {
      try {
        ...
        if (App.get().appCase.getAtomicSources().size() > 1) {
          String casePath =
              App.get().appCase.getAtomicSource(docId).getCaseDir().getCanonicalPath();
      ...
      String helpPath =
          IPEDSource.MODULE_DIR + "/help/Help_" + locale + ".htm"; // $NON-NLS-1$ // $NON-NLS-2$
  ...
  private void process() throws InterruptedException {
    ...
    if (App.get().similarFacesRefItem != null) {
      List<String> locations =
          SimilarFacesSearch.getMatchLocations(App.get().similarFacesRefItem, item);
  ...
  private void waitEvidenceOpening(final IItem item) throws InterruptedException {
    ...
    if (!(factory instanceof SleuthkitInputStreamFactory)
        && !(factory instanceof ZIPInputStreamFactory)
          ...
          new Runnable() {
            ...
            public void run() {
              if (visible) {
                ...
                App.get()
                    .progressBar
                    .setString(Messages.getString("FileProcessor.OpeningEvidence")); // $NON-NLS-1$
