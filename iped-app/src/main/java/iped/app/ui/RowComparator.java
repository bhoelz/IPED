...
public class RowComparator implements Comparator<Integer> {
  ...
  public RowComparator(int col) {
    ...
    if (col >= ResultTableModel.fixedCols.length) {
      ...
      if (fields[col].equals(ResultTableModel.BOOKMARK_COL)) bookmarkCol = true;
      else if (fields[col].equals(ResultTableModel.SCORE_COL)) scoreCol = true;
      else {
  ...
  public final int compare(Integer a, Integer b) {
    ...
    if (scoreCol) return Float.compare(app.ipedResult.getScore(a), app.ipedResult.getScore(b));
    ...
    if (col == 1) {
      if (app.appCase.getMultiBookmarks().isChecked(itemA)
          == app.appCase.getMultiBookmarks().isChecked(itemB)) return 0;
      else if (app.appCase.getMultiBookmarks().isChecked(itemA) == true) return -1;
      else return 1;
    ...
    try {
      ...
      if (v1 == null || v1.isEmpty()) {
        if (v2 == null || v2.isEmpty()) return 0;
        else return -1;
      } else if (v2 == null || v2.isEmpty()) return 1;
