...
public class HexSearcherImpl implements HexSearcher {
  ...
  public void doSearch(
    ...
    GetResultsSearch getSearch =
        new GetResultsSearch(
            codeArea,
            painter,
            hits,
            data,
  ...
  static class GetResultsSearch extends CancelableWorker<String, Integer> {
    ...
    protected String doInBackground() throws Exception {
      ...
      for (String texto : palavras) {
        if (maxTermLength < texto.length()) maxTermLength = texto.length();
      }
      ...
      while (posicao < dataSize - bufferLength) {
          ...
          if (n > 0) {
            ...
            hitsKMP =
                searchBytes(
                    bufferTexto,
                    buffer,
                    bufferLength + texto.length() - 1,
                    (searchString && ignoreCaseSearch));

            for (int off : hitsKMP) {

              HighlightCodeAreaPainter.SearchMatch match =
                  new HighlightCodeAreaPainter.SearchMatch();
        ...
        if (seconds >= 1000) {
          ...
          timeLeft =
              (bytesInOneSecond != 0)
                  ? (long)
                      (((double) dataSize - (double) posicao)
                          / (((double) bytesInOneSecond / 1000) / ((double) seconds / 1000)))
                  : 359999000;
          timeLeftString =
              Messages.getString("HexSearcherImpl.timeLeft") + ": " + formatTime(timeLeft);
          bytesInOneSecond = 0;
        ...
        if (progressMonitor != null) {
          ...
          progressMonitor.setNote(
              "<html><body>"
                  + hitsEncontrados.size()
                  + " "
                  + Messages.getString("HexSearcherImpl.hits")
                  + "<br>"
                  + timeLeftString
                  + "</body></html>");
      ...
      if (!interromper && tempLength > 0) {
        ...
        if (n > 0) {
          ...
          for (String texto : palavras) {
            ...
            hitsKMP =
                searchBytes(bufferTexto, buffer, tempLength, (searchString && ignoreCaseSearch));
      ...
      if (hits.totalHits > 0) {
        ...
        resultSearch.setText(
            Messages.getString("HexSearcherImpl.hit")
                + " "
                + (hits.currentHit + 1)
                + " "
                + Messages.getString("HexSearcherImpl.of")
                + " "
                + hits.totalHits);
      } else {
  ...
  /**
   * Searches for all occurances of the word in the sentence. Runs in O(n+k) where n is the word
   * length and k is the sentence length.
   *
   ...
   * @return The list of starting indices of the matched word in the sentence. Empty list in case of
   *     no match.
   */
  public static List<Integer> searchBytes(
      final byte[] word, final byte[] sentence, int length, boolean ignoreCaseSearch) {
...
class SearchMatchComparator implements Comparator<HighlightCodeAreaPainter.SearchMatch> {
  @Override
  public int compare(
      HighlightCodeAreaPainter.SearchMatch o1, HighlightCodeAreaPainter.SearchMatch o2) {
