...
public class ResultTableModel extends AbstractTableModel implements SearchResultTableModel {
  ...
  private static final List<String> basicDateFields =
      Arrays.asList(IndexItem.ACCESSED, IndexItem.MODIFIED, IndexItem.CREATED, IndexItem.CHANGED);

  private static final String lengthField = LocalizedProperties.getLocalizedField(IndexItem.LENGTH);
  public static String BOOKMARK_COL =
      Messages.getString("ResultTableModel.bookmark"); // $NON-NLS-1$
  public static String SCORE_COL = Messages.getString("ResultTableModel.score"); // $NON-NLS-1$

  private SimpleDateFormat df =
      new SimpleDateFormat(Messages.getString("ResultTableModel.DateFormat")); // $NON-NLS-1$
  private SimpleDateFormat fatAccessedDf =
      new SimpleDateFormat(Messages.getString("ResultTableModel.FATDateFormat")); // $NON-NLS-1$
            ...
            public void run() {
              ...
              for (int i = 0; i < fixedColdWidths.length; i++) {
                App.get()
                    ...
                    .getColumnModel()
                    .getColumn(i)
                    .setPreferredWidth(fixedColdWidths[i]);
              }

              for (int i = 0; i < ColumnsManager.getInstance().colState.initialWidths.size(); i++) {
                TableColumn tc =
                    App.get().resultsTable.getColumnModel().getColumn(i + fixedColdWidths.length);
      ...
      if (lengthField.equalsIgnoreCase(columnNames[i])) {
        ...
        if (mb == -1) {
          App.get()
              ...
              .getColumnModel()
              .getColumn(col)
              .setHeaderValue(columnNames[i] + " (...)"); // $NON-NLS-1$
        } else {
          App.get()
              .resultsTable
              .getColumnModel()
  ...
  public Object getValueAt(int row, int col) {
    ...
    if (col == 1) return app.appCase.getMultiBookmarks().isChecked(app.ipedResult.getItem(row));
    ...
    try {
      ...
      if (field.equals(BOOKMARK_COL)) {
        return Util.concatStrings(
            app.appCase.getMultiBookmarks().getBookmarkList(app.ipedResult.getItem(row)), true);
      }
      ...
      if (LocalizedProperties.getLocalizedField(BasicProps.CATEGORY)
          .equalsIgnoreCase(getColumnName(col))) {
      ...
      if (basicDateFields.contains(field))
        try {
          ...
          if (field.equals(IndexItem.ACCESSED)) {
            if (doc.get(SleuthkitReader.IN_FAT_FS) != null) return fatAccessedDf.format(date);
          }
      ...
      if (field.equals(IndexItem.NAME)) {
        ...
        if (fragments[0].getScore() > 0) {
          ...
          if (App.get().getFontStartTag() != null) s.append(App.get().getFontStartTag());
          s.append(fragments[0].toString());
          if (App.get().getFontStartTag() != null) s.append(ATextViewer.HIGHLIGHT_END_TAG);
