...
public class KeywordListImporter extends CancelableWorker {
  ...
  protected Object doInBackground() {
          ...
          if (addBookMarkList || addBookMarkWords) {
            ...
            if (addBookMarkList) {
              App.get()
                  .appCase
                  .getMultiBookmarks()
                  .addBookmark(uniqueSelectedIds, singleBookmarkName);
            }
            if (addBookMarkWords) {
    ...
    if (addBookMarkList && !result.isEmpty()) {
      App.get()
          .appCase
          .getMultiBookmarks()
          .setBookmarkComment(singleBookmarkName, PREFIX + " " + result.toString());
    }
  ...
  public void done() {
    ...
    for (String word : result) App.get().appCase.getMultiBookmarks().addToTypedWords(word);
    ...
    if (errors.size() > 0) {
      ...
      JOptionPane.showMessageDialog(
          null, Messages.getString("KeywordListImporter.SyntaxError") + errorTerms); // $NON-NLS-1$
    }

    JOptionPane.showMessageDialog(
        null,
        Messages.getString("KeywordListImporter.Msg.1")
            + result.size() // $NON-NLS-1$
            + Messages.getString("KeywordListImporter.Msg.2")
            + keywords.size()); // $NON-NLS-1$
  }
}
