...
public class BookmarksManager implements ActionListener, ListSelectionListener, KeyListener {
  ...
  JButton butUpdateComment =
      new JButton(Messages.getString("BookmarksManager.Update")); // $NON-NLS-1$
  ...
  public static void updateCounters() {
    instance.highlighted.setText(
        Messages.getString("BookmarksManager.Highlighted")
            + " (" //$NON-NLS-1$ //$NON-NLS-2$
            + LocalizedFormat.format(App.get().resultsTable.getSelectedRowCount())
            + ")"); //$NON-NLS-1$
    instance.checked.setText(
        Messages.getString("BookmarksManager.Checked")
            + " (" //$NON-NLS-1$ //$NON-NLS-2$
            + LocalizedFormat.format(App.get().appCase.getMultiBookmarks().getTotalChecked())
            + ")"); //$NON-NLS-1$
  }
  ...
  private BookmarksManager() {
    ...
    newBookmark.setToolTipText(
        Messages.getString("BookmarksManager.NewBookmark.Tip")); // $NON-NLS-1$
    comments.setToolTipText(Messages.getString("BookmarksManager.CommentsTooltip")); // $NON-NLS-1$
    butUpdateComment.setToolTipText(
        Messages.getString("BookmarksManager.UpdateTooltip")); // $NON-NLS-1$
  ...
  private void updateList(String oldBookmark, String newBookmark) {
    ...
    for (String bookmark : bookmarkSet) {
      ...
      iped.data.BookmarkShortcut bs =
          App.get().appCase.getMultiBookmarks().getBookmarkBookmarkShortcut(bookmark);
  ...
  public void actionPerformed(final ActionEvent evt) {
    if (evt.getSource() == butAdd
        || evt.getSource() == butRemove
        || evt.getSource() == butUpdateComment
        || evt.getSource() == butEdit
        || evt.getSource() == butDelete) {
    ...
    if (evt.getSource() == butNew) {
      ...
      if (!name.isEmpty() && !listModel.contains(new BookmarkAndKey(name))) {
        ...
        multiBookmarks.setBookmarkColor(
            name, BookmarkColorsUtil.getInitialColor(multiBookmarks.getUsedColors(), name));
        updateList();
      }
      list.clearSelection();
      for (int i = 0; i < listModel.size(); i++) {
    ...
    if (evt.getSource() == butAdd || evt.getSource() == butRemove || evt.getSource() == butNew) {
      ...
      int result =
          JOptionPane.showConfirmDialog(
              dialog,
              Messages.getString("BookmarksManager.ConfirmDelete"), // $NON-NLS-1$
              Messages.getString("BookmarksManager.ConfirmDelTitle"),
      ...
      if (newName != null) {
        if (!newName.isEmpty() && !newName.equals(currentName)) {
          if (!currentName.equalsIgnoreCase(newName)
              && listModel.contains(new BookmarkAndKey(newName))) {
            JOptionPane.showMessageDialog(
                dialog, Messages.getString("BookmarksManager.AlreadyExists"));
          } else {
  ...
  private void bookmark(
      ArrayList<IItemId> uniqueSelectedIds, List<String> bookmarks, boolean insert, boolean isNew) {
    if (uniqueSelectedIds.isEmpty() && !isNew) {
      showMessage(
          Messages.getString(
              checked.isSelected()
                  ? "BookmarksManager.AlertNoCheckedtems"
    ...
    new Thread() {
      public void run() {
        if (duplicates.isSelected()) includeDuplicates(uniqueSelectedIds);
  ...
  public void keyPressed(KeyEvent e) {
    ...
    if (e.getKeyCode() == KeyEvent.VK_SPACE
        || Arrays.asList('R', 'P', 'F', 'D').contains((char) e.getKeyCode())) {
      if (e.getSource() == list) {
        showMessage(Messages.getString("BookmarksManager.KeyStrokeAlert4"));
        e.consume();
      }
  ...
  private void removeKeyStroke(String bookmarkStr) {
    ...
    App.get().appCase.getMultiBookmarks().removeBookmarkBookmarkShortcut(bookmarkStr);
    App.get().appCase.getMultiBookmarks().saveState();
  }
