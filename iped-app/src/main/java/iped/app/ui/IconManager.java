...
public class IconManager {
  ...
  private static final Map<String, QualityIcon> catIconMap =
      loadIconsFromJar("cat", initialSizes[0]);
  private static final Map<String, QualityIcon> filterIconMap =
      loadIconsFromJar("filter", initialSizes[0]);
  private static final Map<String, QualityIcon> filterSmallIconMap =
      loadIconsFromJar("filter", smallSize(initialSizes[0]));
  private static final Map<String, QualityIcon> filterDisabledIconMap =
      loadIconsFromJar("filter", initialSizes[0], false);

  private static final Map<String, QualityIcon> extIconMap =
      loadIconsFromJar("file", currentIconSize);
  private static final Map<String, QualityIcon> treeIconMap =
      loadIconsFromJar("tree", currentIconSize);
  private static final Map<String, QualityIcon> mimeIconMap = initMimeToIconMap(currentIconSize);

  private static final Map<String, QualityIcon> extIconMapGallery =
      initIconsMapSize(extIconMap, initialSizes[1]);
  private static final Map<String, QualityIcon> mimeIconMapGallery =
      initIconsMapSize(mimeIconMap, initialSizes[1]);
  private static final Map<String, QualityIcon> treeIconMapGallery =
      initIconsMapSize(treeIconMap, initialSizes[1]);
  ...
  private static Map<String, QualityIcon> loadIconsFromJar(
      String iconPath, int size, boolean enabled) {
          ...
          while (true) {
            ...
            String path =
                IconManager.class
                        ...
                        .toString()
                        .replace(".", separator)
                        .replace(IconManager.class.getSimpleName(), "")
                    + iconPath
                    + separator;
            ...
            if (nameWithPath.startsWith(path) && name.toLowerCase().endsWith(ICON_EXTENSION)) {
              BufferedImage img =
                  ImageIO.read(IconManager.class.getResource(iconPath + separator + name));
  ...
  public static Icon getFileIconGallery(String mimeType, String extension) {
    return getFileIcon(
        mimeType, extension, mimeIconMapGallery, extIconMapGallery, extIconMapGallery.get(fileKey));
  }
  ...
  private static Icon getFileIcon(
      String mimeType,
      String extension,
      Map<String, QualityIcon> mimeIconMap,
  ...
  public static Icon getFilterIcon(SimpleFilterNode node, boolean enabled) {
    if (node != null) {
      if (node.isDynamicChild()) {
        return filterSmallIconMap.getOrDefault(
            node.getPrefix().toLowerCase() + ".dynamicchild", defaultCategoryIcon);
      }
      return (enabled ? filterIconMap : filterDisabledIconMap)
          .getOrDefault(node.getFullName().toLowerCase(), defaultCategoryIcon);
    }
    return defaultCategoryIcon;
  }
