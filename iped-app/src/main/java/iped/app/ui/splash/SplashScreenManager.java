...
public class SplashScreenManager {
  /**
   * This is the total progress, based on the number of the classes loaded in the current (parent)
   * process and in the child process (actual application). A rough estimation is enough to show
   * some progress while loading. This constant may be adjusted in the future, if a larger number of
   * classes are loaded during startup process.
   */
          ...
          public void run() {
            try {
              ...
              SplashScreenConfig config =
                  ConfigurationManager.get().findObject(SplashScreenConfig.class);
              if (config != null) {
              ...
              while (screen.isVisible()) {
                ...
                Rectangle rcFill =
                    new Rectangle(rc.x + 1, rc.y + 1, (int) ((rc.width - 2) * pct), rc.height - 2);
                g.fill(rcFill);
                if (screen.isVisible()) {
                  screen.update();
                }
