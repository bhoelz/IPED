...
public class App extends JFrame implements WindowListener, IMultiSearchResultProvider, GUIProvider {
  ...
  public static final boolean triageGui =
      System.getProperty("enableTriageGui") != null; // $NON-NLS-1$
  ...
  private DefaultSingleCDockable categoriesTabDock,
      aiFiltersTabDock,
      metadataTabDock,
      bookmarksTabDock,
      evidenceTabDock;
  ...
  public DefaultSingleCDockable hitsDock,
      subitemDock,
      parentDock,
      duplicateDock,
      referencesDock,
  ...
  private JScrollPane hitsScroll,
      subItemScroll,
      parentItemScroll,
      duplicatesScroll,
      referencesScroll,
  ...
  static final String FILTRO_TODOS = Messages.getString("App.NoFilter"); // $NON-NLS-1$
  static final String FILTRO_SELECTED = Messages.getString("App.Checked"); // $NON-NLS-1$
  public static final String SEARCH_TOOL_TIP =
      Messages.getString("App.SearchBoxTip"); // $NON-NLS-1$

  static int MAX_HITS = 10000;

  static final int FRAG_SIZE = 100, TEXT_BREAK_SIZE = 1000000;
  ...
  private void destroy() {
    boolean processingFinished =
        processingManager == null || processingManager.isProcessingFinished();
    try {
      ...
      if (processingFinished) {
        if (processingManager != null) processingManager.deleteTempDir();
        if (logConfiguration != null) logConfiguration.closeConsoleLogFile();
  ...
  public void createGUI() {
    ...
    if (!Util.isJavaFXPresent()) {
      JOptionPane.showMessageDialog(
          this, Messages.get("NoJavaFX.Error"), "Error", JOptionPane.ERROR_MESSAGE);
      return;
    }
    ...
    if (processingManager != null) topPanel.add(updateCaseData);
        ...
        new ComponentAdapter() {
          ...
          public void componentResized(ComponentEvent e) {
            ...
            if (selRow >= 0) {
              App.get()
                  .gallery
                  .scrollRectToVisible(App.get().gallery.getCellRect(selRow, 0, false));
            }
          }
        });
    ...
    dockingControl.putProperty(
        EclipseTheme.TAB_PAINTER,
        new TabPainter() {
          public Border getFullBorder(
              BorderedComponent owner, DockController controller, Dockable dockable) {
        ...
        .putClient(
            ...
            new ViewGenerator<ButtonDockAction, BasicTitleViewItem<JComponent>>() {
              public BasicTitleViewItem<JComponent> create(
                  ActionViewConverter converter, ButtonDockAction action, Dockable dockable) {
    ...
    dockingControl
        ...
        .putTheme(
            ...
            new ViewGenerator<SelectableDockAction, BasicTitleViewItem<JComponent>>() {
              public BasicTitleViewItem<JComponent> create(
                  ActionViewConverter converter, SelectableDockAction action, Dockable dockable) {
                BasicSelectableHandler.Check handler =
                    new BasicSelectableHandler.Check(action, dockable);
                CustomButton button = new CustomButton(handler, handler);
                handler.setModel(button.getModel());
                return handler;
              }
    ...
    defaultColor =
        dockingControl.getController().getColors().get(ColorMap.COLOR_KEY_TAB_BACKGROUND);
    defaultFocusedColor =
        dockingControl.getController().getColors().get(ColorMap.COLOR_KEY_TAB_BACKGROUND_FOCUSED);
    defaultSelectedColor =
        dockingControl.getController().getColors().get(ColorMap.COLOR_KEY_TAB_BACKGROUND_SELECTED);

    timelineButton =
        new CCheckBox(
            Messages.get("App.ToggleTimelineView"), IconUtil.getToolbarIcon("time", resPath)) {
          protected void changed() {
            if (timelineListener != null) timelineListener.setTimelineTableView(isSelected());
          }
        };
    timelineListener =
        new TimelineListener(timelineButton, IconUtil.getToolbarIcon("timeon", resPath));
    ...
    filterManager.getFilterers().stream()
        .forEach(
            new Consumer<IFilterer>() {
              @Override
  ...
  /**
   * Setup application global keyboard shortcuts. TODO: Check if other existing keyboard shortcuts
   * may be handled globally.
   */
  ...
  public void updateUI(boolean refresh) {
    ...
    if (foreground == null) fontStartTag = null;
    else
      fontStartTag =
          "<font color=" + UiUtil.getHexRGB(foreground) + ">"; // $NON-NLS-1$ //$NON-NLS-2$
    ...
    if (refresh) {
      ...
      if (viewerController != null) viewerController.reload();
    }
  }
  ...
  private void createAllDockables() {
    categoriesTabDock =
        createDockable(
            "categoriestab",
            Messages.getString("CategoryTreeModel.RootName"), // $NON-NLS-1$ //$NON-NLS-2$
            categoriesPanel);
    aiFiltersTabDock =
        createDockable("aifilterstab", Messages.getString("App.AIFilters"), aiFiltersPanel);
    filtersTabDock =
        createDockable(
            "filterstab",
            Messages.getString("App.appliedFilters"), // $NON-NLS-1$ //$NON-NLS-2$
            filtersPanel);
    metadataTabDock =
        createDockable(
            "metadatatab",
            Messages.getString("App.Metadata"),
            metadataPanel); //$NON-NLS-1$ //$NON-NLS-2$
    if (evidencePanel != null) {
      evidenceTabDock =
          createDockable(
              "evidencetab",
              Messages.getString("TreeViewModel.RootName"), // $NON-NLS-1$ //$NON-NLS-2$
              evidencePanel);
    }
    bookmarksTabDock =
        createDockable(
            "bookmarkstab",
            Messages.getString("BookmarksTreeModel.RootName"), // $NON-NLS-1$ //$NON-NLS-2$
            bookmarksPanel);

    tableTabDock =
        createDockable(
            "tabletab",
            Messages.getString("App.Table"),
            resultsScroll); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    galleryTabDock =
        createDockable(
            "galleryscroll",
            Messages.getString("App.Gallery"),
            galleryScroll); //$NON-NLS-1$ //$NON-NLS-2$

    graphDock = createDockable("graphtab", Messages.getString("App.Links"), appGraphAnalytics);

    CCheckBox butToggleVideoFramesMode =
        new CCheckBox(
            Messages.getString("Gallery.ToggleVideoFrames"),
    ...
    galleryGrayButton =
        new CCheckBox(
            Messages.getString("Gallery.GalleryGrayFilter"),
    ...
    galleryBlurButton =
        new CCheckBox(
            Messages.getString("Gallery.GalleryBlurFilter"),
    ...
    butSimSearch =
        new CButton(
            Messages.getString("MenuClass.FindSimilarImages"),
            IconUtil.getToolbarIcon("find", resPath));
    galleryTabDock.addAction(butSimSearch);
    butSimSearch.addActionListener(
        new ActionListener() {
    ...
    butFaceSearch =
        new CButton(
            Messages.getString("MenuClass.FindSimilarFaces"),
            IconUtil.getToolbarIcon("face", resPath));
    galleryTabDock.addAction(butFaceSearch);
    butFaceSearch.addActionListener(
        new ActionListener() {
          public void actionPerformed(ActionEvent e) {
    ...
    for (Iterator<ResultSetViewer> iterator = rsViewers.iterator(); iterator.hasNext(); ) {
      ...
      DefaultSingleCDockable tabDock =
          createDockable(
              resultSetViewer.getID(), resultSetViewer.getTitle(), resultSetViewer.getPanel());
      ...
      tabDock.addCDockableLocationListener(
          new CDockableLocationListener() {
            @Override
    ...
    hitsDock =
        createDockable(
            "tabbedhits", Messages.getString("App.Hits"), hitsScroll); // $NON-NLS-1$ //$NON-NLS-2$
    subitemDock =
        createDockable(
            "subitemstab",
            Messages.getString("SubitemTableModel.Subitens"),
            subItemScroll); //$NON-NLS-1$ //$NON-NLS-2$
    parentDock =
        createDockable(
            "parentitemtab",
            Messages.getString("ParentTableModel.ParentCount"), // $NON-NLS-1$ //$NON-NLS-2$
            parentItemScroll);
    duplicateDock =
        createDockable(
            "duplicatestab",
            Messages.getString("DuplicatesTableModel.Duplicates"), // $NON-NLS-1$ //$NON-NLS-2$
            duplicatesScroll);
    referencesDock =
        createDockable(
            "referencestab", Messages.getString("ReferencesTab.Title"), referencesScroll);
    referencedByDock =
        createDockable(
            "referencedbytab", Messages.getString("ReferencedByTab.Title"), referencedByScroll);
    ...
    for (AbstractViewer viewer : viewers) {
      DefaultSingleCDockable viewerDock =
          createDockable(viewer.getClass().getName(), viewer.getName(), viewer.getPanel());
  ...
  private void setupViewerDocks() {
    CCheckBox chkFixed =
        new CCheckBox(
            Messages.getString("ViewerController.FixViewer"),
    ...
    for (int i = 0; i < viewers.size(); i++) {
      ...
      viewerDock.addCDockableLocationListener(
          new CDockableLocationListener() {
            public void changed(CDockableLocationEvent event) {
      ...
      if (viewer == App.get().getViewerController().getMultiViewer()) {
        CButton searchInViewer =
            new CButton(
                Messages.getString("MenuClass.SearchInViewer"),
                IconUtil.getToolbarIcon("search", resPath));
        searchInViewer.addActionListener(
            new ActionListener() {
              public void actionPerformed(ActionEvent e) {
        ...
        CButton copyViewerImage =
            new CButton(
                Messages.getString("MenuClass.CopyViewerImage"),
                IconUtil.getToolbarIcon("copy", resPath));
        copyViewerImage.addActionListener(
            new ActionListener() {
              public void actionPerformed(ActionEvent e) {
                AbstractViewer viewer =
                    App.get().getViewerController().getMultiViewer().getCurrentViewer();
                viewer.copyScreen();
              }
            });
      ...
      CButton prevHit =
          new CButton(
              Messages.getString("ViewerController.PrevHit"),
              IconUtil.getToolbarIcon("prev", resPath));
      CButton nextHit =
          new CButton(
              Messages.getString("ViewerController.NextHit"),
              IconUtil.getToolbarIcon("next", resPath));
      prevHit.addActionListener(
          new ActionListener() {
            public void actionPerformed(ActionEvent e) {
              viewer.scrollToNextHit(false);
            }
          });
      nextHit.addActionListener(
          new ActionListener() {
            public void actionPerformed(ActionEvent e) {
      ...
      if (toolbarSupport >= 0) {
        ...
        CSelButton butToolbar =
            new CSelButton(Messages.getString("ViewerController.ShowToolBar"), upIcon, downIcon);
        butToolbar.addActionListener(
            new ActionListener() {
              public void actionPerformed(ActionEvent e) {
                butToolbar.toggle();
        ...
        File xml =
            new File(
                Configuration.getInstance().appRoot
                    + "/conf/ResultSetViewersConf.xml"); //$NON-NLS-1$
  ...
  private void setTabColor(
      DefaultSingleCDockable dock, Color colorBackground, Color colorFocused, Color colorSelected) {
    ...
    dock.getColors()
        .setColor(ColorMap.COLOR_KEY_MINIMIZED_BUTTON_BACKGROUND_SELECTED, colorSelected);
  ...
  public void adjustLayout(boolean isReset) {
    if (isReset) removeAllDockables();
    createAllDockables();

    tableTabDock.setLocation(
        verticalLayout ? CLocation.base().normalNorth(0.7) : CLocation.base().normalNorth(0.5));
    ...
    hitsDock.setLocation(
        verticalLayout
            ? CLocation.base().normalSouth(0.3)
            : CLocation.base().normalSouth(0.5).west(0.4));
    ...
    if (!verticalLayout) adjustViewerLayout();
    ...
    if (isReset) setGalleryColCount(GalleryModel.defaultColCount);
  }
  ...
  private void adjustViewerLayout() {
    ...
    prevDock.setLocation(
        verticalLayout
            ? CLocation.base().normalEast(0.35)
            : CLocation.base().normalSouth(0.5).east(0.6));
  ...
  class DuplicatesFilterer implements IResultSetFilterer {
    ...
    public void clearFilter() {
      ...
      if (filterDuplicates.isSelected()) filterDuplicates.doClick();
      appletListener.clearAllFilters = false;
    }
  }

  class SimilarFacesSearchFilter
      implements IResultSetFilter, IMiniaturizable, IItemRef, IQuantifiableFilter {
    IItem itemRef;
    IItemId itemIdRef;
    private String refName;
    private BufferedImage img;
  ...
  class SearchFilterer implements IQueryFilterer {
    ...
    public boolean hasFilters() {
      if (App.get().queryComboBox.getSelectedItem() != null) {
        ...
        if (searchText.equals(BookmarksController.HISTORY_DIV)
            || searchText.equals(App.SEARCH_TOOL_TIP)) {
          return false;
        }
  ...
  class SimilarDocumentFilter implements IQueryFilter, IItemRef, IQuantifiableFilter {
    ...
    public Query getQuery() {
      if (query == null) {
        query =
            new SimilarDocumentSearch()
                .getQueryForSimilarDocs(itemRefId, filterPercent, App.get().appCase);
      }
      return query;
    }
    ...
    public String toString() {
      return Messages.get("FilterValue.SimilarDocument")
          + " ("
          + Integer.toString(filterPercent)
          + "%): "
          + itemRef.getName();
    }
