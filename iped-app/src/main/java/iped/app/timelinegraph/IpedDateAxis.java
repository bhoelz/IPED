...
public class IpedDateAxis extends DateAxis implements MouseResponsiveChartEntity {
  ...
  public IpedDateAxis(String string, IpedChartsPanel ipedChartsPanel) {
    ...
    dateFormaters.put(
        DateTickUnitType.MILLISECOND, new SimpleDateFormat("HH:mm:ss.SSS\nd-MMM-yyyy"));
  }
  ...
  protected List refreshTicksHorizontal(Graphics2D g2, Rectangle2D dataArea, RectangleEdge edge) {
    ...
    while (tickDate.before(upperDate)) {
      ...
      for (int minorTick = 1; minorTick < minorTickSpaces; minorTick++) {
        ...
        if (minorTickTime > 0
            && getRange().contains(minorTickTime)
      ...
      if (!isHiddenValue(tickDate.getTime())) {
        ...
        if (formatter != null) {
          ...
          int upperTickUnitCalendarValue =
              cal.get(DateUtil.getUpperCalendarField(this.getTickUnit().getCalendarField()));
        ...
        for (int minorTick = 1; minorTick < minorTickSpaces; minorTick++) {
          long minorTickTime =
              currentTickTime + (nextTickTime - currentTickTime) * minorTick / minorTickSpaces;
   ...
   */
  private Date correctTickDateForPosition(
      Date time, DateTickUnit unit, DateTickMarkPosition position) {
    Date result = time;
    if (unit.getUnitType().equals(DateTickUnitType.MONTH)) {
      result =
          calculateDateForPosition(new Month(time, this.getTimeZone(), this.getLocale()), position);
    } else if (unit.getUnitType().equals(DateTickUnitType.YEAR)) {
      result =
          calculateDateForPosition(new Year(time, this.getTimeZone(), this.getLocale()), position);
    }
    return result;
  }

  /**
   * Returns a {@link java.util.Date} corresponding to the specified position within a {@link
   * RegularTimePeriod}.
   *
  ...
  protected double findMaximumTickLabelHeight(
    ...
    return super.findMaximumTickLabelHeight(ticks, g2, drawArea, vertical)
        * 2; // doubled as tick labels upper text are painted bellow main tick label
  }

  private Calendar calendarInstance;

  public TimePeriod getDateOnConfiguredTimePeriod(
      Class<? extends TimePeriod> timePeriodClass, Date date) {
  ...
  private TimePeriod _getDateOnConfiguredTimePeriod(
      Class<? extends TimePeriod> timePeriodClass, Date date, Calendar cal) {
    ...
    if (cplot != null) {
      ...
      for (Iterator<XYPlot> iterator = plots.iterator(); iterator.hasNext(); ) {
        ...
        if (ds instanceof AsynchronousDataset && notify) {
          ((AsynchronousDataset) ds)
              .notifyVisibleRange(range.getLowerBound(), range.getUpperBound());
        }
      }
    }
  }
  ...
  /**
   * Adjusts visible range to guarantee that the current minimum and maximum visible x values are
   * visible with the minimum and maximum dates informed
   *
  ...
  public void garanteeShowRange(Date min, Date max) {
    ...
    Class<? extends TimePeriod> tpclass =
        bestGranularityForRange(newRange.getLowerDate(), newRange.getUpperDate());
  ...
  public boolean canZoom(Range range) {
    ...
    if (ipedChartsPanel != null) {
      ...
      double java2dlower =
          valueToJava2D(
              range.getLowerBound(),
              ipedChartsPanel.getChartPanel().getScreenDataArea(),
      ...
      double java2dupper =
          valueToJava2D(
              ...
              RectangleEdge.BOTTOM);
      double barsize =
          ((java2dupper - java2dlower) / (range.getUpperBound() - range.getLowerBound()))
              * ChartTimePeriodConstraint.getTimePeriodUnit(
                  ipedChartsPanel.getTimePeriodClass()); // size in pixels

      if (c != null) {
        if (curRangeSize < rangeSize
            && (rangeSize > c.maxZoomoutRangeSize || barsize <= c.minBarSizeInPixels)) {
          result = false;
        }
        if (curRangeSize > rangeSize
            && (rangeSize < c.minZoominRangeSize || barsize >= ((java2dupper - java2dlower) / 3))) {
          result = false;
        }
      } else {
      ...
      if (!result) {
        ...
        try {
          if (curRangeSize > rangeSize && tpclass != Second.class) { // zoomIn
            ...
            String msg =
                String.format(
                    Messages.get("TimeLineGraph.visibleZoominForGranularity"),
                    tpClassName,
                    DateUtil.getTimePeriodName(downtpclass));
          ...
          if (curRangeSize < rangeSize) { // zoomIn
            ...
            String msg =
                String.format(
                    Messages.get("TimeLineGraph.visibleZoomoutForGranularity"),
                    tpClassName,
                    DateUtil.getTimePeriodName(uptpclass));
            int input = JOptionPane.showConfirmDialog(null, msg, "", JOptionPane.OK_CANCEL_OPTION);
            if (input == 0) {
              result = true;
  ...
  /**
   * Returns a subclass of {@link RegularTimePeriod} that is smaller than the specified class.
   *
  ...
  public AxisState draw(
    ...
    if (mouseOverPaint != null) {
      ...
      g2.fillRect(
          ...
          (int) plotArea.getMaxX() - (int) dataArea.getMinX(),
          (int) plotArea.getMaxY() - (int) dataArea.getMaxY());
    }
