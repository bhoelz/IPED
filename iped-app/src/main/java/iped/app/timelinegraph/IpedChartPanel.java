...
public class IpedChartPanel extends ChartPanel implements KeyListener {
  private Rectangle2D
      filterIntervalRectangle; // represents the filter selection rectangle while drawing/defining a
  // interval
  ...
  HashMap<Class<? extends TimePeriod>, ChartTimePeriodConstraint> timePeriodConstraints =
      new HashMap<Class<? extends TimePeriod>, ChartTimePeriodConstraint>();
  ...
  public IpedChartPanel(IpedChart chart, IpedChartsPanel ipedChartsPanel) {
    ...
    apllyFilters =
        new JButton(new ImageIcon(img.getScaledInstance(12, 12, java.awt.Image.SCALE_SMOOTH)));
    apllyFilters.setMaximumSize(new Dimension(16, 16));
    apllyFilters.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
            chartPanelPopupMenu.show(
                apllyFilters,
                apllyFilters.getX() - 2,
                apllyFilters.getY() + apllyFilters.getHeight() - 8);
          }
        });
    this.add(apllyFilters);

    this.addChartMouseListener(
        new ChartMouseListener() {
          ...
          public void chartMouseMoved(ChartMouseEvent event) {
            ...
            if (ce instanceof AxisEntity) {
              curMouseResponsiveChartEntity =
                  (MouseResponsiveChartEntity) ((AxisEntity) ce).getAxis();
            }

            if (lastMouseResponsiveChartEntity instanceof MouseResponsiveChartEntity
                && curMouseResponsiveChartEntity == null) {
            ...
            if (curMouseResponsiveChartEntity != lastMouseResponsiveChartEntity) {
              ...
              self.getChart()
                  .getPlot()
                  .notifyListeners(new PlotChangeEvent(self.getChart().getPlot()));
          ...
          public void chartMouseClicked(ChartMouseEvent event) {
                ...
                if (entities != null) {
                  ...
                  for (int i = entityCount - 1; i >= 0; i--) {
                    ...
                    if (entity instanceof XYItemEntity) {
                      if (entity.getArea().getBounds().getMaxX() > x
                          && entity.getArea().getBounds().getMinX() < x) {
            ...
            if (ce instanceof PlotEntity) {
              ...
              if (filteredDate != null) {
                ...
                timelineSelectionPopupMenu.show(
                    event.getTrigger().getComponent(),
                    event.getTrigger().getX(),
                    event.getTrigger().getY());
              } else {
                domainPopupMenu.setDate(
                    new Date(
                        (long)
                            ipedChartsPanel
                                .getDomainAxis()
            ...
            if (ce instanceof AxisEntity) {
              if (((AxisEntity) ce).getAxis() instanceof DateAxis) {
                timePeriodSelectionPopupMenu.show(
                    event.getTrigger().getComponent(),
                    event.getTrigger().getX(),
                    event.getTrigger().getY());
              } else {
                seriesAxisPopupMenu.show(
                    event.getTrigger().getComponent(),
                    event.getTrigger().getX(),
                    event.getTrigger().getY());
              }
      ...
      if ((mods & this.filterMask) == this.filterMask) {
        ...
        if (this.filterIntervalRectangle == null) {
          startFilterDate =
              new Date(
                  (long)
                      ipedChartsPanel.domainAxis.java2DToValue(
                          e.getX(),
                          this.getScreenDataArea(),
                          ipedChartsPanel.combinedPlot.getDomainAxisEdge()));
          ...
          int x =
              (int)
                  ipedChartsPanel.domainAxis.dateToJava2D(
                      startFilterDate,
                      this.getScreenDataArea(),
                      ipedChartsPanel.combinedPlot.getDomainAxisEdge());
  ...
  public void mouseDragged(MouseEvent e) {
    ...
    if ((mods & this.filterMask) == this.filterMask) {
      ...
      this.filterIntervalRectangle =
          new Rectangle2D.Double(
              this.filterIntervalPoint.getX(),
              scaledDataArea.getMinY(),
              xmax - this.filterIntervalPoint.getX(),
  ...
  public void mouseReleased(MouseEvent e) {
    ...
    if (this.filterIntervalRectangle != null) {
      ...
      boolean filterTrigger1 =
          Math.abs(e.getX() - this.filterIntervalPoint.getX()) >= this.filterTriggerDistance;
  ...
  private void drawFilterIntervalRectangle(Graphics2D g2, boolean xor) {
    if (this.filterIntervalRectangle != null) {
      ...
      int w =
          (int) this.filterIntervalRectangle.getMaxX()
              - (int) this.filterIntervalRectangle.getMinX();
      int h =
          (int) this.filterIntervalRectangle.getMaxY()
              - (int) this.filterIntervalRectangle.getMinY();
      if (w <= 0) {
        return;
      }
      ...
      g2.drawString(
          strEndDate,
          (int) this.filterIntervalRectangle.getMaxX() + 2,
          (int) this.filterIntervalRectangle.getMinY() + g2.getFontMetrics().getHeight() + 2);
      if (xor) {
  ...
  public Date removeNextFromDatePart(Date date) {
    ...
    try {
      t =
          (RegularTimePeriod)
              ipedChartsPanel.getTimePeriodClass().getConstructor(Date.class).newInstance(date);
  ...
  private void drawDefinedFiltersDates(Date[] dates, Graphics2D g2, boolean xor) {
    ...
    g2.drawString(
        strStartDate,
        (int) rectangle2d.getMinX() + 2,
        (int) rectangle2d.getMinY() + g2.getFontMetrics().getHeight() + 2);
    g2.drawString(
        strEndDate,
        (int) rectangle2d.getMaxX() + 2,
        (int) rectangle2d.getMinY() + g2.getFontMetrics().getHeight() + 2);
  ...
  /**
   * Draws defined filters dates text (if present). The drawing is performed in XOR mode, therefore
   * when this method is called twice in a row, the second call will completely restore the state of
   * the canvas.
   *
   * @param g2 the graphics device.
   * @param xor use XOR for drawing?
  ...
  public void removeAllFilters(boolean updateResult) {
    for (Date[] removedDates : definedFilters) {
      HighlightWorker sw =
          new HighlightWorker(
              ipedChartsPanel.getDomainAxis(),
              ipedChartsPanel.resultsProvider,
              removedDates[0],
              removedDates[1],
              false,
              false);
  ...
  public void setTimePeriodConstraints(
      Class<? extends TimePeriod> t, ChartTimePeriodConstraint timePeriodConstraint) {
