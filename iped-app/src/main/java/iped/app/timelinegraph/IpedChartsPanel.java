...
@Slf4j
public class IpedChartsPanel extends JPanel
  ...
  static ThreadPoolExecutor swExecutor =
      new ThreadPoolExecutor(
          1, 1, 20000, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>());
  ...
  private static HashMap<String, Integer> eventNameToOrd = new HashMap<>();
  ...
  class LegendCellRenderer extends JLabel implements ListCellRenderer<LegendItemBlockContainer> {
    ...
    public Component getListCellRendererComponent(
        JList<? extends LegendItemBlockContainer> list,
        LegendItemBlockContainer value,
        int index,
        boolean isSelected,
      ...
      IpedCombinedDomainXYPlot rootPlot =
          ((IpedCombinedDomainXYPlot) getChartPanel().getChart().getPlot());
  ...
  public HashMap<String, AbstractIntervalXYDataset> createDataSets() {
    HashMap<String, AbstractIntervalXYDataset> result =
        new HashMap<String, AbstractIntervalXYDataset>();
    try {
      ...
      if (selectedBookmarks.size() > 0 && chartPanel.getSplitByBookmark()) {
        for (String bookmark : selectedBookmarks) {
          result.put(
              bookmark, ipedTimelineDatasetManager.getBestDataset(timePeriodClass, bookmark));
        }
      } else if (selectedCategories.size() > 0 && chartPanel.getSplitByCategory()) {
        for (String category : selectedCategories) {
          result.put(
              category, ipedTimelineDatasetManager.getBestDataset(timePeriodClass, category));
        }
      } else {
          ...
          synchronized (timeEventColumnNamesList) {
            ...
            try {
              SortedSetDocValues timeEventGroupValues =
                  reader.getSortedSetDocValues(BasicProps.TIME_EVENT);
  ...
  public static Integer getEventOrd(String event) {
    return eventNameToOrd.get(event);
  }

  public static String getEventName(int ord) {
  ...
  private JFreeChart createChart(
      HashMap<String, AbstractIntervalXYDataset> datasets, boolean resetDomainRange) {
    domainAxis.setLabel(
        "Date ("
            + timePeriodString
            + ") ["
            ...
            + " "
            + DateUtil.getTimezoneOffsetInformation(timeZone)
            + "]");
