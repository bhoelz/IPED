...
public class IpedTimelineDataset extends AbstractIntervalXYDataset
  ...
  /**
   * A flag that indicates that the domain is 'points in time'. If this flag is true, only the
   * x-value (and not the x-interval) is used to determine the range of values in the domain.
   */
  ...
  static ThreadPoolExecutor queriesThreadPool =
      new ThreadPoolExecutor(
          ...
          new LinkedBlockingQueue<
              Runnable>()); // pool of concurrent group of itens in a dataset beeing populated
  static ThreadPoolExecutor datasetsThreadPool =
      new ThreadPoolExecutor(
          3,
          10,
          20000,
          TimeUnit.MILLISECONDS,
          new LinkedBlockingQueue<Runnable>()); // pool of concurrent datasets beeing populated
   ...
   */
  Semaphore memoryCacheReloadSem =
      new Semaphore(
          1); // semaphore that controls start and end of load of cache window contourning the
  ...
  public IpedTimelineDataset(
    ...
    this.workingCalendar =
        Calendar.getInstance(ipedChartsPanel.getTimeZone(), ipedChartsPanel.getLocale());
    ...
    if (splitValue != null
        && !splitValue.equals("Bookmarks")
        && ipedChartsPanel.getChartPanel().getSplitByBookmark()) {
      MultiBitmapBookmarks multiBookmarks =
          (MultiBitmapBookmarks) App.get().getIPEDSource().getMultiBookmarks();
  ...
  class PopulateDatasetTask implements Runnable {
    ...
    public PopulateDatasetTask(
        IMultiSearchResult result,
        CacheTimePeriodEntry[] threadLocalCts,
        int threadCtsEnd,
        Semaphore timeSem,
        Semaphore addValueSem) {
      this.result = result;
      ...
      try {
        ...
        for (int i = 0; i < threadCtsEnd; i++) {
          ...
          for (CacheEventEntry ce : ct.getEvents()) {
            if (ce.docIds != null) {
              RoaringBitmap rb =
                  RoaringBitmap.and(((MultiSearchResult) result).getDocIdBitSet(), ce.docIds);
              ...
              if (count.value > 0) {
                ...
                try {
                  TimePeriod t =
                      ipedChartsPanel
                          .getDomainAxis()
                          .getDateOnConfiguredTimePeriod(
                              ipedChartsPanel.getTimePeriodClass(), ct.getDate());
  ...
  public String getWindowQuery() {
    ...
    Date tmpDate =
        new Date(endDate.getTime() - visibleRangeLength * MEMORY_WINDOW_CACHE_PROPORTION);
  ...
  int MEMORY_WINDOW_CACHE_PROPORTION =
      2; // how many times the visible range the memory window will load, forward and
  // backward.
  List<CacheTimePeriodEntry> memoryWindowCache = new ArrayList<CacheTimePeriodEntry>();

  public static Date MIN_DATE = new Date(0, 0, 1); // 01/01/1900 is the min JfreeChart date
  public static Date MAX_DATE = new Date(8099, 11, 31); // 01/01/9999 is the maxJfreeChart date
  ...
  public void caseSearchFilterLoad() throws Exception {
    ...
    try {
      ...
      if (ipedChartsPanel.getChartPanel().getSplitByCategory()
          && splitValue != null
          && !splitValue.equals("Categories")) {
      ...
      try {
        ...
        JOptionPane.showMessageDialog(
            ipedChartsPanel,
            "Insufficient Memory to plot chart!",
            "Error",
      ...
      csf.applyUIQueryFilters(
          exceptThis); // apply all filters from others UI objects except the chart defined interval
      // filters
      if (unionsArray != null) {
        if (splitValue.equals(noBookmarksStr)) {
      ...
      if (result.getLength() > 0) {
        ...
        Date cacheWindowEndDate =
            new Date(ipedChartsPanel.getChartPanel().removeNextFromDatePart(endDate).getTime() - 1);
        long visibleRangeLength = cacheWindowEndDate.getTime() - cacheWindowStartDate.getTime();
        cacheWindowStartDate =
            new Date(endDate.getTime() - visibleRangeLength * MEMORY_WINDOW_CACHE_PROPORTION);
        cacheWindowEndDate =
        ...
        if (cacheWindowEndDate.after(MAX_DATE)) {
          ...
          cacheWindowEndDate =
              new Date(
                  ipedChartsPanel.getChartPanel().removeNextFromDatePart(endDate).getTime() - 1);
        }
        ...
        try (RandomAccessReadBufferedFile sfis = a.getTmpCacheSfis(className)) {
          if (sfis != null) {
            ...
            while (it != null && it.hasNext()) {
              ipedChartsPanel
                  .getIpedTimelineDatasetManager()
                  .waitMemory(); // method to wait available mem to continue.
              if (cancelled) {
                break;
              }
              ...
              if (remove) {
                TimePeriod t =
                    ipedChartsPanel
                        .getDomainAxis()
                        .getDateOnConfiguredTimePeriod(
                            ipedChartsPanel.getTimePeriodClass(), ctpe.getDate());
        ...
        populatesWithList(
            result,
            visibleIntervalCache,
            addValueSem); // creates first the visible interval itens to be plotted
  ...
  public void populatesWithBeforeAndAfterList(
      IMultiSearchResult result,
      List<CacheTimePeriodEntry> beforecache,
      List<CacheTimePeriodEntry> aftercache,
      Semaphore addValueSem) {
    try {
      int threadCount =
          (int)
              Math.ceil(
                  (double) (beforecache.size() + aftercache.size()) / (double) CTITEMS_PER_THREAD);
      ...
      int beforeindex = 0;
      int afterindex = 0;
      CacheTimePeriodEntry ctpeBefore =
          beforecache.size() > 0 ? beforecache.get(beforeindex) : null;
      ...
      while (ctpeBefore != null || ctpeAfter != null) {
        ...
        if (threadCtsCount >= CTITEMS_PER_THREAD || totalInCount <= 0) {
          ...
          Runnable r =
              new PopulateDatasetTask(result, threadCts, threadCtsEnd, timeSem, addValueSem);
          Thread t = new Thread(r);
  ...
  public void populatesWithList(
      IMultiSearchResult result, List<CacheTimePeriodEntry> cache, Semaphore addValueSem) {
    try {
      ...
      for (CacheTimePeriodEntry ctpe : cache) {
        ...
        if (threadCtsCount >= CTITEMS_PER_THREAD || totalInCount <= 0) {
          ...
          threadCtsCount = 0;
          Runnable r =
              new PopulateDatasetTask(result, threadLocalCts, threadCtsEnd, timeSem, addValueSem);
          Thread t = new Thread(r);
          queriesThreadPool.execute(t);
        }
      }
   ...
   *
   * <p>This flag is used when determining the max and min values for the domain. If true, then only
   * the x-values are considered for the max and min values. If false, then the start and end
   * x-values will also be taken into consideration.
   *
   * @return The flag.
  ...
  /**
   * Sets a flag that controls whether the domain is treated as 'points in time', or time periods. A
  ...
  /**
   * Sets the position within each time period that is used for the X values, then sends a {@link
  ...
  /**
   * Returns the number of items in a series. This is the same value that is returned by {@link
   * #getItemCount()} since all series share the same x-values (time periods).
   *
              ...
              synchronized (self) {
                ...
                if (!c) {
                  ipedChartsPanel
                      ...
                      .notifyListeners(
                          new PlotChangeEvent(
                              ipedChartsPanel.getChartPanel().getChart().getPlot()));
