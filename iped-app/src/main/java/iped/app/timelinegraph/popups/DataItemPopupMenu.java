...
public class DataItemPopupMenu extends JPopupMenu implements ActionListener {
  ...
  public DataItemPopupMenu(IpedChartPanel ipedChartPanel) {
    ...
    highlightEventItens =
        new JMenuItem(Messages.getString("TimeLineGraph.highlightEventItensOnPeriod"));
    highlightEventItens.addActionListener(this);
    add(highlightEventItens);

    highlightPeriodItens =
        new JMenuItem(Messages.getString("TimeLineGraph.highlightItensOnPeriod"));
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getSource() == highlightEventItens) {
      ...
      c.setTime(
          new Date(
              (long)
                  chartEntity
                      .getDataset()
                      .getXValue(chartEntity.getSeriesIndex(), chartEntity.getItem())));
      ...
      BitSetHighlightWorker bsHighlight =
          new BitSetHighlightWorker(
    ...
    if (e.getSource() == highlightPeriodItens) {
      ...
      ipedChartPanel
          .getIpedChartsPanel()
          .highlightItemsOnInterval(c.getTime(), cEnd.getTime(), true);
    }
    ...
    if (e.getSource() == checkPeriodItens) {
      ...
      Date end =
          new Date(
              timestamp + (long) ipedChartPanel.getIpedChartsPanel().getTimePeriodLength() - 1l);
