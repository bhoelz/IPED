...
public class TimePeriodSelectionPopupMenu extends JPopupMenu implements ActionListener {
  ...
  static HashMap<Class<? extends TimePeriod>, JTimePeriodMenuItem> miMap =
      new HashMap<Class<? extends TimePeriod>, JTimePeriodMenuItem>();
  ...
  public TimePeriodSelectionPopupMenu(IpedChartsPanel ipedChartsPanel) {
    ...
    for (String timeZoneStr : ZoneId.getAvailableZoneIds()) {
      ...
      String timezoneofssetformat =
          DateUtil.getTimezoneOffsetInformation(TimeZone.getTimeZone(timeZoneStr));

      JTimezoneMenuItem tzmi =
          new JTimezoneMenuItem(
              timeZoneStr + "(" + timezoneofssetformat + ")", TimeZone.getTimeZone(timeZoneStr));
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getSource() instanceof JTimePeriodMenuItem) {
      Class<? extends TimePeriod> tpclass =
          ((JTimePeriodMenuItem) e.getSource()).getTimePeriodClass();
      ...
      ChartTimePeriodConstraint c =
          ipedChartsPanel.getChartPanel().getTimePeriodConstraints(tpclass);
      ...
      double java2dupper =
          ipedChartsPanel
              ...
              .valueToJava2D(
                  ...
                  RectangleEdge.BOTTOM);
      double newbarsize =
          ((java2dupper - java2dlower) / (dateRange.getUpperBound() - dateRange.getLowerBound()))
              * ChartTimePeriodConstraint.getTimePeriodUnit(tpclass); // size in pixels

      if (c != null && rangeSize > c.getMaxZoomoutRangeSize()) {
        Date centerDate = new Date(startDate.getTime() + rangeSize / 2);
        String msg =
            String.format(
                Messages.get("TimeLineGraph.visibleRangeToGreat"),
                tpClassName,
                iped.utils.DateUtil.dateToString(centerDate));
        input = JOptionPane.showConfirmDialog(null, msg, "", JOptionPane.OK_CANCEL_OPTION);
        if (input == 0) {
          Date lstartDate =
              new Date((long) Math.floor(centerDate.getTime() - c.getMaxZoomoutRangeSize() / 2));
          Date lendDate =
              new Date((long) Math.ceil(centerDate.getTime() + c.getMaxZoomoutRangeSize() / 2));
          new Thread(
                  new Runnable() {
                    @Override
                    public void run() {
                      ipedChartsPanel
                          .getDomainAxis()
                          .forceRange(new DateRange(lstartDate, lendDate), false, false);
                    }
                  })
              .start();
        }
        ...
        Date centerDate = new Date(startDate.getTime() + rangeSize / 2);
        Date lstartDate =
            new Date((long) Math.floor(centerDate.getTime() - c.getMinZoominRangeSize() / 2));
        Date lendDate =
            new Date((long) Math.ceil(centerDate.getTime() + c.getMinZoominRangeSize() / 2));
        ...
        new Thread(
          ...
          > (java2dupper - java2dlower)
              / 3) { // if the bar size of the new timeperiod granularity is greater than 1/3 of the
        // total screen area
        ...
        startDate =
            new Date(
                (long)
                    Math.floor(
                        centerDate.getTime()
                            - ChartTimePeriodConstraint.getTimePeriodUnit(tpclass)));
        endDate =
            new Date(
                (long)
                    Math.ceil(
