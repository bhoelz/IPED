...
public class PersistedArrayList implements Set<CacheTimePeriodEntry> {
  ...
  private static int flushMaxSize =
      Runtime.getRuntime().maxMemory() > 1536 * 1024 * 1024
          ? 1000000
          : Runtime.getRuntime().maxMemory() > 1024 * 1024 * 1024 ? 100000 : 10000;
  ...
  public Iterator<CacheTimePeriodEntry> iterator() {
    ...
    if (flushCount <= 0) {
      ...
      log.info(
          "Number of intermediary flushes to create {} index: {}. Merging them.",
          timePeriod,
          flushCount);
  ...
  public boolean add(CacheTimePeriodEntry e) {
    ...
    if (flushSize.get() > flushMaxSize
        && IpedTimelineDatasetManager.getAvailableMemory() < Runtime.getRuntime().maxMemory() / 2) {
      flush();
      flushSize.set(0);
    }
  ...
  private void flush() {
    ...
    Future f =
        cp.cachePersistanceExecutor.submit(
            new Runnable() {
              @Override
              public void run() {
                try {
