...
public class TimeIndexedMap extends HashMap<String, Set<CacheTimePeriodEntry>> {
  HashMap<String, TreeMap<Long, Long>> upperPeriodIndex =
  ...
  public void setIndexFile(String string, File f) throws IOException {
    ...
    int committed = 0;
    try (RandomAccessReadBufferedFile lcacheSfis = new RandomAccessReadBufferedFile(cacheFile);
  ...
  public ResultIterator iterator(
      String className, RandomAccessReadBufferedFile lcacheSfis, Date startDate, Date endDate) {
    try {
      DataInputStream lcacheDis = new DataInputStream(new RandomAccessInputStream(lcacheSfis));
      ...
      try {
        ...
        if (cache != null) {
          index =
              timelineCache.getIndex(
                  className,
                  pos); // finds the correspondent index position in cache array for the startpos

          if (lastStartDate != null
              && startDate
                  != null) { // liberate some memory of entries in cache out of window of iteration
        ...
        if (endDate == null) {
          return new ResultIterator(
              pos, index, timelineCache, lcacheSfis, lcacheDis, Long.MAX_VALUE, className);
        } else {
          return new ResultIterator(
              pos, index, timelineCache, lcacheSfis, lcacheDis, endDate.getTime(), className);
        }
      } catch (IOException e) {
        e.printStackTrace();
      }
  ...
  class ResultIterator implements Iterator<CacheTimePeriodEntry> {
    ...
    int cacheCurrentIndex =
        0; // current index being iterated (sum with startIndex to identify correspondent
    // cache array index)
    private CacheTimePeriodEntry[] lcache; // cache array to iterate
    private RandomAccessReadBufferedFile
        lcacheSfis; // seekable stream to index file (to read from when entry not in cache)
    ...
    private Reference<CacheTimePeriodEntry>[]
        lsoftcache; // soft reference cache (a more complete cache but with SoftReferences)
    boolean hasSoftCache = false;
    Long nextSeekPos = 0l;
    private Integer
        startIndex; // start index in cache array in file to iterate (if not in cache array load
    ...
    public ResultIterator(
        ...
        RandomAccessReadBufferedFile lcacheSfis,
        DataInputStream lcacheDis,
        long endDate,
        String className) {
      this.startPos = pos;
      ...
      try {
        ...
        if (lastHasNext == null) { // not in cache so load from file
          ...
          if (lcache != null && useCache) {
            lcache[startIndex + cacheCurrentIndex] = lastHasNext;
            if (hasSoftCache) {
              lsoftcache[startIndex + cacheCurrentIndex] =
                  new SoftReference<CacheTimePeriodEntry>(lastHasNext);
            }
  ...
  public void createOrLoadUpperPeriodIndex(IndexTimeStampCache indexTimeStampCache) {
    ...
    if (upperPeriodIndex.size() == 0) {
      for (Iterator iterator = indexTimeStampCache.getPeriodClassesToCache().iterator();
          iterator.hasNext(); ) {
