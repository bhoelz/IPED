...
public class EventTimestampCache implements Runnable {
          ...
          while (doc != DocIdSetIterator.NO_MORE_DOCS) {
            ...
            while (ord != -1) {
              if (emptyValueOrd == null || ord != emptyValueOrd) {
                for (Class<? extends TimePeriod> timePeriodClass :
                    timeStampCache.getPeriodClassesToCache()) {
                  Date date = null;
  ...
  private static final boolean isPeriodToCache(Class<? extends TimePeriod> timePeriodClass) {
    return timePeriodClass == Year.class
        || timePeriodClass == Quarter.class
        || timePeriodClass == Month.class
        || timePeriodClass == Week.class
        || timePeriodClass == Day.class
        || timePeriodClass == Hour.class;
  }

  public static synchronized String cloneBr(BytesRef br) {
