...
public class TimelineCache {
  ...
  HashMap<String, TreeMap<Long, Integer>> cachesIndexes =
      new HashMap<String, TreeMap<Long, Integer>>();
  HashMap<String, CacheTimePeriodEntry[]> caches = new HashMap<String, CacheTimePeriodEntry[]>();
  HashMap<String, Reference<CacheTimePeriodEntry>[]> softCaches =
      new HashMap<String, Reference<CacheTimePeriodEntry>[]>();
  ...
  public static TimelineCache get() {
    return singleton;
  }
              ...
              for (Iterator<String> iterator = caches.keySet().iterator(); iterator.hasNext(); ) {
                ...
                if (lperiod.equals(period)) {
                  for (int i = 0; i < cache.length; i++) {
                    if (cache[i] != null
                        && cache[i].getDate().before(startDate)
                        && cache[i].getDate().after(endDate)) {
