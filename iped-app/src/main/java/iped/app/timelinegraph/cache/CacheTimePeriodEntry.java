...
public class CacheTimePeriodEntry implements Comparable<CacheTimePeriodEntry> {
  ...
  public synchronized CacheEventEntry[] getEvents() {
  ...
  public synchronized RoaringBitmap getEventDocIds(int ord) {
    if (docids == null) {
      return null;
    }
  ...
  public synchronized RoaringBitmap getEventDocIds(String event) {
    if (docids == null) {
      return null;
    }
    if (eventOrds.getCardinality() == 0) {
  ...
  public synchronized void addEventEntry(int ord, RoaringBitmap pdocids) {
    eventOrds.add(ord);
    int pos = 0;
    if (docids != null) {
  ...
  public synchronized void addEventEntry(String event, RoaringBitmap pdocids) {
    int ord = IpedChartsPanel.getEventOrd(event);
    addEventEntry(ord, pdocids);
  }

  public synchronized void addEventEntry(int ord, int doc) {
    eventOrds.add(ord);
    int pos = 0;
    if (docids != null) {
      pos = (int) eventOrds.rangeCardinality(0, ord);
