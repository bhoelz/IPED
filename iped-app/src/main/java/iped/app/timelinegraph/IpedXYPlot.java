...
public class IpedXYPlot extends XYPlot {
  ...
  public IpedXYPlot(
      IpedChartPanel ipedChartPanel,
      AbstractIntervalXYDataset timeTableCumulativeXYDataset,
      DateAxis domainAxis,
  ...
  /**
   * Draws defined filters rectangles (if present). The drawing is performed in XOR mode, therefore
   ...
   */
  private void drawDefinedFilterRectangles(
      Date[] dates, Graphics2D g2, Rectangle2D area, boolean xor) {
    if (xor) {
    ...
    int xstart =
        (int)
            ipedChartPanel.ipedChartsPanel.domainAxis.dateToJava2D(
                dates[0], area, ipedChartPanel.ipedChartsPanel.combinedPlot.getDomainAxisEdge());
    int xend =
        (int)
            ipedChartPanel.ipedChartsPanel.domainAxis.dateToJava2D(
                dates[1], area, ipedChartPanel.ipedChartsPanel.combinedPlot.getDomainAxisEdge());
  ...
  private void drawDefinedFiltersDates(Date[] dates, Graphics2D g2, Rectangle2D area, boolean xor) {
    ...
    String strStartDate =
        ipedChartPanel
            .getIpedChartsPanel()
            .getDomainAxis()
            .ISO8601DateFormat(dates[0]); // iped.utils.DateUtil.dateToString(dates[0]);
    String strEndDate =
        ipedChartPanel
            .getIpedChartsPanel()
            .getDomainAxis()
            .ISO8601DateFormat(dates[1]); // iped.utils.DateUtil.dateToString(dates[1]);
  ...
  /**
   * Finds a range of item indices that is guaranteed to contain all the x-values from x0 to x1
   * (inclusive).
   *
