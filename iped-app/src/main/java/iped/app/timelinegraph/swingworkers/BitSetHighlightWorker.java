...
public class BitSetHighlightWorker extends CancelableWorker<Void, Void> {
  ...
  class SelectionSlice implements Runnable {
    ...
    public void run() {
      try {
        for (int i = start;
            (i < start + sliceSize) && (i < resultsProvider.getResults().getLength());
            i++) {
          try {
            ...
            int docid =
                resultsProvider
                    .getIPEDSource()
                    .getLuceneId(resultsProvider.getResults().getItem(i));
  ...
  public void highlightDocIdsParallel(
      RoaringBitmap bs, boolean highlight, boolean clearPreviousSelection) {
    Date d1 = new Date();
    ...
    try {
      ...
      int nThreads =
          (int) Math.ceil((double) resultsProvider.getResults().getLength() / (double) sliceSize);
