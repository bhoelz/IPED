...
/**
 * Default {@link CompanionHandoff} implementation that POSTs to the companion app's loopback HTTP
 * server.
 *
 * <p>The companion listens on {@code http://127.0.0.1:<port>/open} (default port {@value
 * #DEFAULT_PORT}). Authentication uses an HMAC-SHA256 session token issued by the companion at
 * startup.
 */
public class HttpCompanionHandoff implements CompanionHandoff {
  ...
  public HttpCompanionHandoff(int port, String sessionToken) {
    ...
    this.http = HttpClient.newBuilder().connectTimeout(Duration.ofMillis(500)).build();
  }
