...
class DistributedStatusServiceTest {
    ...
    void aggregatesCaseProgressFromStatusEvents() {
        ...
        svc.handleMessage(event("COMPLETED", "case-1", 1, "\"taskType\":\"HashTask\",\"durationMs\":10"));
        svc.handleMessage(event("COMPLETED", "case-1", 2, "\"taskType\":\"IndexTask\",\"durationMs\":12"));
        svc.handleMessage(event("COMPLETED", "case-1", 2, "\"taskType\":\"IndexTask\",\"durationMs\":15"));
        svc.handleMessage(event("ERROR", "case-1", 1,
                "\"taskType\":\"HashTask\",\"errorMessage\":\"corrupt stream\""));
    ...
    void caseCompletedMarksJobDone() {
        ...
        svc.handleMessage("{\"type\":\"CASE_COMPLETED\",\"caseId\":\"case-2\",\"timestamp\":\"2026-06-11T12:05:00Z\"}");
    ...
    void ignoresMalformedAndUnknownEvents() {
        ...
        svc.handleMessage(event("DISCOVERED", "case-4", 0,
                "\"futureField\":\"ignored\""));                          // schema evolution
