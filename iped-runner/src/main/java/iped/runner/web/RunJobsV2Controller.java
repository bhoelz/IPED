...
/**
 * Control-plane adapter — exposes the runner's run state using the same JSON shape as {@code
 * iped-webapi}'s {@code /v2/jobs} resource.
 *
 * <p>This lets the MCP {@code job_*} tools and the web UI talk to either service with an identical
 * contract. Active runs map to status {@code "running"}; queued runs map to {@code "pending"};
 * terminal history entries map to {@code "completed"}, {@code "failed"}, or {@code "cancelled"}.
 *
 * <h2>Routes</h2>
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>POST is intentionally omitted — run submission goes through {@code POST /run} or {@code POST
 * /runs/batch} which carry runner-specific richer metadata.
 */
@RestController
...
public class RunJobsV2Controller {
  ...
  public List<Map<String, Object>> listJobs() {
    ...
    // 1. Running
    executionService.snapshots().forEach(snap -> jobs.add(snapshotToJob(snap)));

    // 2. Queued (pending)
    runQueueService.pending().forEach(q -> jobs.add(queuedToJob(q)));

    // 3. History (terminal), newest first
    runHistory.all().forEach(s -> jobs.add(summaryToJob(s)));
  ...
  public ResponseEntity<Map<String, Object>> getJob(@PathVariable String id) {
    ...
    if (active.isPresent()) {
      var snap = executionService.snapshots().stream().filter(s -> s.id().equals(id)).findFirst();
      ...
      return ResponseEntity.ok(
          Map.of("id", id, "type", "iped-run", "status", "running", "progress", 0, "message", ""));
    }

    // Queued?
    var queued = runQueueService.pending().stream().filter(q -> q.id().equals(id)).findFirst();
  ...
  private static Map<String, Object> snapshotToJob(JobSnapshot snap) {
    ...
    m.put(
        "message",
        snap.recentLines() != null && !snap.recentLines().isEmpty()
  ...
  private static Map<String, Object> queuedToJob(QueuedRun q) {
    ...
    m.put(
        ...
        Map.of(
            ...
            q.priority().name(),
            "profile",
            q.request().profile() != null ? q.request().profile() : ""));
    return m;
  }
