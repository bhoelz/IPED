 ...
 *
 * <p>{@code GET /metrics} returns a flat JSON object suitable for scraping by
 * Prometheus (via a custom exporter), Grafana, or just {@code curl | jq}.
 *
 * <h2>Metric groups</h2>
 * <ul>
 *   <li>{@code runner.*} — queue depth, active runs, slot availability.</li>
 *   <li>{@code distributed.*} — active distributed cases observed on Kafka.</li>
 *   <li>{@code jvm.*} — heap usage, uptime (informational).</li>
 * </ul>
 */
@RestController
