...
public class ExecutionController {
   ...
   *
   * <p>Body: {@code {"tokens":[...],"priority":"NORMAL","profile":"mobile"}} Returns: {@code
   * {"id":"<uuid>","queued":true}} — or {@code "queued":false} if a slot was available and the
  ...
  /**
   * Returns the current status of a run. Active runs return {@code status: "running"}; terminal
   * runs are looked up in history.
   */
  @GetMapping("/run/{id}")
  ...
  /**
   * SSE stream for a running job. Sends {@code log}, {@code done}, {@code error}, or {@code
   * aborted} events.
   */
  ...
  /**
   * Gracefully cancels a running job (SIGTERM → wait → SIGKILL). Returns 404 if the run is not
   * active (already finished or unknown).
   */
