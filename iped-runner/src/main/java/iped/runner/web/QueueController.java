...
public class QueueController {
  ...
  public ResponseEntity<List<Map<String, Object>>> listQueue() {
    var items = queueService.pending().stream().map(QueueController::toMap).toList();
  ...
  /**
   * Batch-submit: scans {@code sourceDir} for immediate children and enqueues one run per child
   * using the provided {@code tokens} as a template.
   *
   * <p>Request body:
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>Each child directory/file under {@code sourceDir} becomes one IPED run: {@code -d <child> -o
   * <outputDir>/<child-name>}.
   */
  ...
  public ResponseEntity<Map<String, Object>> batchSubmit(@RequestBody Map<String, Object> body) {
    ...
    List<Map<String, String>> extraRaw =
        body.containsKey("extraTokens")
            ? (List<Map<String, String>>) body.get("extraTokens")
            : List.of();
    List<RunRequest.RunToken> extraTokens =
        extraRaw.stream().map(m -> new RunRequest.RunToken(m.get("flag"), m.get("val"))).toList();
          ...
          .forEach(
              child -> {
                try {
                  ...
                  tokens.add(
                      new RunRequest.RunToken(
                          "-o", Path.of(outputDir, child.getFileName().toString()).toString()));
                  ...
                  var qr =
                      queueService.enqueue(new RunRequest(tokens, priority, profile), priority);
                  enqueued.add(qr.id());
                } catch (Exception e) {
