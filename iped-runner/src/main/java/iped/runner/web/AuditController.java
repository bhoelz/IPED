 ...
 *
 * <p>All endpoints return 404 when no records exist for the requested case, so the UI can
 * distinguish "case not found" from "case has no terminal events yet."
 */
@RestController
...
public class AuditController {
  ...
  public ResponseEntity<String> getCaseAuditCsv(@PathVariable String caseId) {
    ...
    return ResponseEntity.ok()
        ...
        .header("Content-Disposition", "attachment; filename=\"audit-" + caseId + ".csv\"")
        .body(auditLog.exportCsv(caseId));
  }

  /** The most recent terminal record for a specific item. */
  @GetMapping("/{caseId}/{itemUuid}")
  public ResponseEntity<ProcessingRecord> getItemAudit(
      @PathVariable String caseId, @PathVariable String itemUuid) {
