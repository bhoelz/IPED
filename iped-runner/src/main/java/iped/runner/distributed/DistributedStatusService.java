...
/**
 * Consumes the global {@code iped.status} Kafka topic and aggregates per-case progress for the
 * Processing Dashboard.
 *
 * <p>Disabled unless {@code runner.kafka.bootstrap-servers} is set. Each runner instance joins with
 * a unique consumer group and reads the topic from the beginning, so the dashboard rebuilds the
 * full cluster view after a restart without affecting the processing consumer groups.
 */
@Service
@Slf4j
public class DistributedStatusService implements JobSnapshotProvider, AutoCloseable {
  ...
  @Autowired private ProcessingAuditLog auditLog;
  ...
  void start() {
    ...
    pollThread.start();
    log.info(
        "Distributed status monitor started, topic '{}' on {}", STATUS_TOPIC, bootstrapServers);
  }
  ...
  void handleMessage(String json) {
    ...
    cases
        .computeIfAbsent(event.getCaseId(), id -> new CaseStats(id, event.getTimestamp()))
        .apply(event);
