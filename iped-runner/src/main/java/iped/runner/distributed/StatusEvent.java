...
/**
 * Mirror of {@code iped.distributed.status.ItemStatusEvent} as published on the global
 * {@code iped.status} Kafka topic.
 *
 * <p>Deliberately duplicated instead of depending on {@code iped-distributed}: the runner
 * stays free of engine dependencies and the JSON schema of the topic is the contract
 * between the two modules. Unknown fields are ignored so the producer can evolve.
 */
@JsonIgnoreProperties(ignoreUnknown = true)
public class StatusEvent {

    public enum Type {
        DISCOVERED, STARTED, COMPLETED, SKIPPED, ERROR, TIMEOUT,
        SUBITEM_DISCOVERED, CASE_COMPLETED
    }
