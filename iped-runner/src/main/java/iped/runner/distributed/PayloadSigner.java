 ...
 * </pre>
 *
 * Fields joined with {@code |}, null fields rendered as empty string. Result is lowercase hex
 * stored in {@link StatusEvent} (or a dedicated signature field).
 *
 * <p>When {@code payloadSigningSecret} is blank both {@link #sign} and {@link #verify} are no-ops,
 * enabling a rolling upgrade path: disable verification until all agents are updated, then
 * re-enable.
 */
public final class PayloadSigner {
  ...
  static String buildPayload(
      String caseId, String itemUuid, int pipelineStage, String itemPath, long lengthBytes) {
    return IPED_V1
        + "|"
        + nullSafe(caseId)
        + "|"
