...
/**
 * Consumes the {@code iped.agents} Kafka heartbeat topic and maintains a live inventory of remote
 * processing agents with computed health status.
 *
 * <p>Disabled unless {@code runner.kafka.bootstrap-servers} is set (same condition as {@link
 * DistributedStatusService}). Uses its own consumer group so agent heartbeats are read
 * independently from case-status events.
 */
@Service
@Slf4j
public class AgentInventoryService implements AutoCloseable {
  ...
  void start() {
    ...
    pollThread.start();
    log.info(
        "Agent inventory consumer started on topic '{}', {}",
        AgentHeartbeatEvent.TOPIC,
        bootstrapServers);
  }
  ...
  private void pollLoop() {
    try {
      ...
      while (running) {
        consumer.poll(Duration.ofSeconds(1)).forEach(r -> handleMessage(r.value()));
      }
  ...
  void handleMessage(String json) {
    ...
    lastHeartbeats.merge(
        event.agentId(),
        event,
        (existing, incoming) ->
            incoming.lastSeen() != null
                    && existing.lastSeen() != null
                    && incoming.lastSeen().isAfter(existing.lastSeen())
                ? incoming
                : existing);
  }
