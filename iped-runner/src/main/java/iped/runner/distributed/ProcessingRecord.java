 ...
 *
 * <p>Created from {@link StatusEvent} when the event type is COMPLETED, ERROR, or TIMEOUT.
 * All other event types (DISCOVERED, STARTED, SUBITEM_DISCOVERED, CASE_COMPLETED) return
 * {@code null} from {@link #from(StatusEvent, String)}.
 */
public record ProcessingRecord(
    ...
    public enum Outcome { COMPLETED, ERROR, TIMEOUT }
    ...
    public static ProcessingRecord from(StatusEvent event, String agentId) {
        ...
        Outcome outcome = switch (event.getType()) {
