...
/**
 * Fires a webhook HTTP POST whenever a run reaches a terminal state
 ...
 * </pre>
 *
 * <p>When {@code runner.webhook.url} is blank the notifier is a no-op.
 * Delivery failures are logged at WARN level and do not affect the run's
 * recorded state.
 */
@Component
@Slf4j
public class WebhookNotifier {
    ...
    private final HttpClient httpClient = HttpClient.newBuilder()
            ...
            .executor(deliveryPool)
            .build();

    /**
     * Fires the webhook asynchronously for a terminal run. Returns immediately;
     * delivery happens on a daemon thread.
     */
    ...
    private void deliver(RunSummary summary) {
        try {
            ...
            if (resp.statusCode() >= 200 && resp.statusCode() < 300) {
                ...
                log.warn("Webhook delivery for run {} returned HTTP {}",
