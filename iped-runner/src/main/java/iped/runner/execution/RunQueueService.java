...
/**
 * Priority run queue — limits concurrent IPED launches to {@code runner.max-concurrent} (default 2)
 * and drains queued {@link QueuedRun}s in priority order.
 *
 * <p>The queue is flushed on service shutdown; queued (not yet started) runs are abandoned — they
...
public class RunQueueService {
  ...
  private final PriorityBlockingQueue<QueuedRun> queue =
      new PriorityBlockingQueue<>(
          16,
          Comparator.comparingInt((QueuedRun r) -> -r.priority().weight())
  ...
  void init() {
    ...
    log.info(
        "RunQueueService started, max-concurrent={}, profiles-dir={}", maxConcurrent, profilesDir);
  }

  /**
   * Enqueues a run. If a slot is immediately available, the run may start before this method
   * returns.
   *
   * @return the {@link QueuedRun} token (its {@code id} tracks the run)
   */
  public QueuedRun enqueue(RunRequest req, RunPriority priority) {
    var run = new QueuedRun(UUID.randomUUID().toString(), req, priority, java.time.Instant.now());
    queue.add(run);
  ...
  /**
   * Returns the profile names available under {@code runner.profiles-dir}. Profile files are {@code
   * *.toml} files. Returns an empty list if the directory does not exist.
   */
