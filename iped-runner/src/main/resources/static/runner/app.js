// App shell: sidebar, topbar, command bar, run modal, file browser, language picker, tweaks
...
// ---------------- Command bar ----------------
function CommandBar({ cmd, issues, t, lang }) {
...
function FileBrowserModal({ dir: initialDir, accept, onSelect, onClose, t }) {
  ...
  const pick = (entry) => {
    // Dirs: navigate unless the caller only wants to browse dirs (then also allow navigating);
    // when accept==='dir', files are hidden so only dirs are shown.
...
function RunModal({ open, onClose, cmd, issues, s, t, lang }) {
  ...
  const start = async () => {
    ...
    // Strip shell-quoting added by buildCommand's q() before sending raw values to ProcessBuilder
    ...
    try {
      ...
      // Connection-level error (server unreachable, network drop).
      // If a terminal event already ran, esRef.current is null so this is a no-op.
      es.onerror = () => {
...
function App() {
  ...
  // Opens the file browser modal; accept: 'dir' | 'file' | 'any' (default)
  ...
  return (
    <div className={`app${sidebarCollapsed ? ' sidebar-collapsed' : ''}`}>
      ...
      <div className="main">
        ...
        <CommandBar cmd={cmd} issues={issues} t={t} lang={lang} />
      </div>
      ...
      {/* Tweaks panel (activated via postMessage from Claude Design host) */}
