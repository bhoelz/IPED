...
public abstract class AbstractViewer {
  ...
  public boolean isSupportedType(String contentType, boolean testParentTypes) {
    if (!testParentTypes) return this.isSupportedType(contentType);

    MediaType type = MediaType.parse(contentType);
    while (type != null) {
      if (this.isSupportedType(type.toString())) return true;
  ...
  /**
   * AbstractViewer lazy initialization method. Can be used to start heavy objects or components.
   * Must be called outside EDT to not block the UI.
   */
   ...
   *
   * @return -1: no support 0: default, has support, external control when there are hits 1: has
   *     support, always enable as hits control is internal in the viewer
   */
