...
/**
 * Visualizador Html específico que abre links apontando para arquivos do caso, como anexos
 * transferidos em chats ou itens transferidos via P2P.
 *
 * @author Nassif
 */
@Slf4j
public class HtmlLinkViewer extends HtmlViewer implements SelectionListener {
  ...
  public static final String PREVIEW_WITH_LINKS_MIME =
      "application/x-preview-with-links"; //$NON-NLS-1$

  public static final String PREVIEW_WITH_LINKS_HEADER = "<!--Preview With Links-->"; // $NON-NLS-1$

  public static final String UFED_HTML_REPORT_MIME =
      "application/x-ufed-html-report"; //$NON-NLS-1$
              ...
              new Runnable() {
                ...
                public void run() {
                  ...
                  JOptionPane.showMessageDialog(
                      null, Messages.getString("HtmlLinkViewer.AttachNotFound")); // $NON-NLS-1$
                }
              });
  ...
  public void setSelected(IItemId item, boolean value) {
    if (cheking || mediaHashesInView.isEmpty()) return;
    String hash = attachSearcher.getHash(item);
    if (!mediaHashesInView.contains(hash)) return;
    String script =
        "var x = document.getElementsByName(\""
            + hash
            + "\");"
            + "for(var i = 0; i < x.length; i++) x[i].checked = "
            + value
            + ";";
    runScript(script);
  }
