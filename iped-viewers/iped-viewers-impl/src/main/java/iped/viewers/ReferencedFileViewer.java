...
import iped.viewers.util.PreviewStreamSource;
import java.io.File;
...
public class ReferencedFileViewer extends AbstractViewer {
  ...
  private static final String REFERENCE_NOT_FOUND =
      Messages.getString("ReferenceViewer.FileNotFound");
  private static final String REFERENCE_NOT_SUPPORTED =
      Messages.getString("ReferenceViewer.NotSupported");
  ...
  public ReferencedFileViewer(
      MultiViewer multiViewer, AttachmentSearcher attachSearcher, Runnable actionIfNotLoaded) {
    super();
  ...
  public void loadFile(IStreamSource content, Set<String> highlightTerms) {
    ...
    if (content instanceof IItem) {
      ...
      if (lastItem == null) {
        ...
        load(viewContent, mediaType, highlightTerms);
      } else load(lastItem, lastItem.getMediaType().toString(), highlightTerms);
    }
  }
  ...
  private String appendUfedFileId(IItem item, String query) {
    ...
    if (ufedFileId != null) {
      String ufedQuery =
          attachSearcher.escapeQuery(ExtraProperties.UFED_ID) + ":\"" + ufedFileId + "\"";
      if (query != null) {
