...
public class EmailViewer extends HtmlLinkViewer {
  ...
  public boolean isSupportedType(String contentType) {
    return contentType.equals("message/rfc822")
        || contentType.equals("message/x-emlx"); // $NON-NLS-1$ //$NON-NLS-2$
  }
  ...
  public void loadFile(IStreamSource content, Set<String> highlightTerms) {
    ...
    try {
      ...
      if (content instanceof IItemReader
        ...
        stream =
            new ReplacingInputStream(new ReplacingInputStream(stream, "\r\n", "\n"), "\r", "\n");
      }
  ...
  public class AttachmentOpen extends FileHandler {
    ...
    public void open(boolean external, String item) {
      if (external) externalAttachOpen(item);
      else {
  ...
  class MailContentHandler implements ContentHandler {
    ...
    private DateFormat dateFormat =
        new SimpleDateFormat(Messages.getString("EmailViewer.DateFormat")); // $NON-NLS-1$
    ...
    private void createHeader(OutputStreamWriter writer) throws IOException {
      ...
      String[][] names = {
        ...
        {TikaCoreProperties.CREATED.getName(), Messages.getString("EmailViewer.Date")}
      }; //$NON-NLS-1$
      ...
      for (AttachInfo attach : attachments.values()) {
        if (attach.name != null) {
          if (!isPartialEmlx || (attach.tmpFile != null && attach.tmpFile.length() > 0)) {
            text +=
                "<a href=\"\" onclick=\"app.open(false,"
                    + i
                    + ")\">"
                    + SimpleHTMLEncoder.htmlEncode(attach.name)
                    + "</a><br>";
          } else {
            String query = null;
            ...
            for (int k = 0; k <= 1; k++) {
              for (IItem item : items.toArray(new IItem[0])) {
                String name =
                    item.getName().endsWith(":DECOMP")
                        ? item.getName().substring(0, item.getName().length() - 7)
                        : item.getName();
                if (k == 1) {
            ...
            if (query != null) {
              text +=
                  "<a href=\"\" onclick=\"app.open(true,'"
                      + SimpleHTMLEncoder.htmlEncode(query)
                      + "')\">"
                      + SimpleHTMLEncoder.htmlEncode(attach.name)
                      + "</a><br>";
            } else {
              text +=
                  SimpleHTMLEncoder.htmlEncode(attach.name)
                      + " <em>"
                      + Messages.getString("EmailViewer.NotFound")
                      + "</em><br>";
            }
          }
          count++;
      ...
      if (count > 0) {
        text =
            "<b style=\"font-weight:bold\">"
                + Messages.getString("EmailViewer.Attachments")
                + ": ("
                + count //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                + ")</b><br>"
                + text; //$NON-NLS-1$
        writer.write(text);
      }
    ...
    public void body(BodyDescriptor body, InputStream is) throws MimeException, IOException {
      ...
      if (type.equalsIgnoreCase("text/plain")
          || type.equalsIgnoreCase("text/html")) { // $NON-NLS-1$ //$NON-NLS-2$
      ...
      if (isAttach) {
        ...
        if (bodyList.isEmpty()
            || multiParts.isEmpty()
            || !isAlternative()
            || b.multipartDesc != bodyList.getLast().multipartDesc) {
          bodyList.addLast(b);
        }
        ...
        if (bodyList.isEmpty()
            ...
            || (!(alternative = isAlternative())
                && !(mixedInAlternative = isMixedInAlternative()))) {
          bodyList.addLast(b);
        } else {
          if ((alternative && b.multipartDesc == bodyList.getLast().multipartDesc)
              || (mixedInAlternative && !bodyList.getLast().isHtml)) {
            bodyList.removeLast();
          }
          bodyList.addLast(b);
        }
    ...
    private boolean isAlternative() {
      return !multiParts.isEmpty()
          && "alternative".equalsIgnoreCase(multiParts.getLast().getSubType());
    }
    ...
    private boolean isMixedInAlternative() {
      ...
      return ("mixed".equals(lastPart) || "related".equals(lastPart))
          && "alternative".equalsIgnoreCase(multiParts.get(multiParts.size() - 2).getSubType());
    }
      ...
      try (OutputStream outStream = new BufferedOutputStream(new FileOutputStream(previewFile));
        ...
        for (Body bodyObj : bodyList) {
          ...
          for (Entry<String, AttachInfo> e : attachments.entrySet()) {
            ...
            if (body.contains(cid)) {
              File tmpFile =
                  e.getValue().item != null
                      ? e.getValue().item.getTempFile()
                      : e.getValue().tmpFile;
        ...
        for (AttachInfo attach : notInlined.values()) {
          if (attach.mime.startsWith("image")) { // $NON-NLS-1$
            ...
            writer.write(
                "<img src=\""
                    + attach.tmpFile.toURI().toString()
                    + "\" style=\"max-width:100%;\">"); //$NON-NLS-1$ //$NON-NLS-2$
