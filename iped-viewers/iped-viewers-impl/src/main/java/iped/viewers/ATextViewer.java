...
public abstract class ATextViewer extends AbstractViewer implements KeyListener {
  ...
  public abstract void loadFile(
      IStreamSource content, String contentType, Set<String> highlightTerms);
        ...
        try {
          ...
          if (lines == MAX_LINES) {
            lines =
                MAX_LINES
                    + (int)
                        ((textParser.getParsedFile().size()
                                - textParser.getViewRows().get(MAX_LINES))
                            / (CHAR_BYTE_COUNT * MAX_LINE_SIZE))
                    + 1;
          }
          return lines;
    ...
    public Object getValueAt(int row, int col) {
      try {
        ...
        if (row < MAX_LINES) {
          ...
          off = textParser.getViewRows().get(MAX_LINES) + (long) (row - MAX_LINES) * len;
        ...
        data.flip();
        String line =
            (new String(data.array(), TEXT_ENCODING))
                .replaceAll("\n", " ")
      ...
      for (Integer row : textTable.getSelectedRows()) {
        ...
        value =
            value
                .replaceAll("<html><pre>", "")
                .replaceAll(
                    "</pre></html>", ""); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        value =
            value
                .replaceAll(HIGHLIGHT_START_TAG, "") // $NON-NLS-1$
