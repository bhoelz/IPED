...
public class MsgViewer extends HtmlViewer {
  ...
  private Pattern emaillPattern =
      Pattern.compile("<?([0-9a-zA-Z\\+\\.\\_\\%\\-\\#\\!]+\\@[a-zA-Z0-9.-]+)>?");
  private TreeSet<String> imageExts =
      new TreeSet<>(Arrays.asList(".jpg", ".jpeg", ".png", ".bmp", ".gif"));

  private ArrayList<Pair<File, String>> attachs = new ArrayList<>();
  private final DateFormat dateFormat =
      new SimpleDateFormat(Messages.getString("EmailViewer.DateFormat"));
  ...
  private synchronized void createEmailHtml(File msgFile, File emailHtmlFile) throws IOException {
    ...
    try {
      ...
      if (!started) {
        ...
        if (pos > 0) preview.insert(pos + bodyStart.length(), "class=\"ipedtheme\" ");
      }
  ...
  public void parseMsg(
      org.apache.poi.hsmf.MAPIMessage msg,
      StringBuilder preview,
      ArrayList<Pair<File, String>> attachs,
      Index index)
      throws IOException {
    ...
    preview.append(
        "<b>"
            + Messages.getString("EmailViewer.Subject")
            + ": "
            + SimpleHTMLEncoder.htmlEncode(value.trim())
            + "</b><br>");
    ...
    if (!from[1].isEmpty() && from[1].compareTo(from[0]) != 0) {
      preview.append(
          "<b>"
              + Messages.getString("EmailViewer.From")
              + ":</b> "
              + SimpleHTMLEncoder.htmlEncode(from[0])
              + SimpleHTMLEncoder.htmlEncode(" <" + from[1] + ">")
              + "<br>");
      ...
      preview.append(
          "<b>"
              + Messages.getString("EmailViewer.From")
              + ":</b> "
              + SimpleHTMLEncoder.htmlEncode(!from[0].isEmpty() ? from[0] : from[1])
              + "<br>");
    }
    ...
    for (int k = 0; k < RecipientList.size(); k++) {
      ...
      switch (type) {
        case 0:
          if (toValue.isEmpty()) toValue += display + " <" + address + ">";
          else toValue += separator + display + " <" + address + ">";
          break;
        case 1:
          if (ccValue.isEmpty()) ccValue += display + " <" + address + ">";
          else ccValue += separator + display + " <" + address + ">";
          break;
        case 2:
          if (bccValue.isEmpty()) bccValue += display + " <" + address + ">";
          else bccValue += separator + display + " <" + address + ">";
          break;
      }
    }
    ...
    if (!toValue.isEmpty())
      preview.append(
          "<b>"
              + Messages.getString("EmailViewer.To")
              + ":</b> "
              + SimpleHTMLEncoder.htmlEncode(toValue)
              + "<br>");

    if (!ccValue.isEmpty())
      preview.append(
          "<b>"
              + Messages.getString("EmailViewer.Cc")
              + ":</b> "
              + SimpleHTMLEncoder.htmlEncode(ccValue)
              + "<br>");

    if (!bccValue.isEmpty())
      preview.append(
          "<b>"
              + Messages.getString("EmailViewer.Bcc")
              + ":</b> "
              + SimpleHTMLEncoder.htmlEncode(bccValue)
              + "<br>");

    date = null;
    try {
      if (msg.getMessageDate() != null) date = msg.getMessageDate().getTime();
    } catch (ChunkNotFoundException e) {
      // ignore
    }
    if (date != null)
      preview.append(
          "<b>"
              + Messages.getString("EmailViewer.Date")
              + ":</b> "
    ...
    for (AttachmentChunks att : atts) {
      ...
      if (!att.isEmbeddedMessage()) {
        ...
        attachmentList.append(
            "<a href=\"\" onclick=\"app.open("
                + index.index
                + ")\">"
                + SimpleHTMLEncoder.htmlEncode(attachName)
                + "</a>");
        ...
        if (byteChunk != null) {
          ...
          if (isImageMimeType(att)) {
            ...
            String cid =
                cidChunk != null && !cidChunk.getValue().isEmpty()
                    ? cidChunk.getValue()
        ...
        if (msgEmbedded != null) {
          ...
          attachmentList.append(
              "<a href=\"#embedded_msg_" + count + "\">" + subject + ".msg" + "</a>");
        }
      }
      count++;
    }
    ...
    if (count > 0) {
      preview.append(
          "<b>" + Messages.getString("EmailViewer.Attachments") + "(" + (count) + "):</b> ");
    ...
    for (AttachmentChunks att : atts) {
      if (att.isEmbeddedMessage()) {
        ...
        if (msgEmbedded != null) {
          ...
          preview.append(
              "<hr><B><a id=\"embedded_msg_"
                  + count
                  + "\"></a>--- "
                  + Messages.getString("EmailViewer.EmbeddedMessage")
                  + ": '"
                  + subject
                  + ".msg"
                  + "' ---</B></BR>");
  ...
  private String getAttachName(AttachmentChunks att) {
    ...
    return ((attachChunk == null || attachChunk.getValue().isEmpty())
  ...
  private String inlineImages(
      String corpo, Map<String, Pair<File, String>> cids, Set<String> inlined) {
    for (String cid : cids.keySet()) {
  ...
  private byte[] getRTFData(MAPIMessage msg) throws IOException, ChunkNotFoundException {
    ...
    MAPIRtfAttribute rtf =
        new MAPIRtfAttribute(MAPIProperty.RTF_COMPRESSED, Types.BINARY.getId(), chunk.getValue());
    return rtf.getData();
  }
  ...
  private String getTikaRTFToHTML(MAPIMessage msg) {
    try {
      ...
      parser.parse(
          new ByteArrayInputStream(getRTFData(msg)), handler, new Metadata(), new ParseContext());
  ...
  public String parserEmail(String texto, int grupo) {
    ...
    if (matcher.find() && grupo >= 0) {
      if (grupo <= matcher.groupCount()) found = matcher.group(grupo);
    }
    if (grupo == -1) found = "";
