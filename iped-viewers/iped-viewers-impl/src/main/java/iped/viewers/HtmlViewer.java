...
public class HtmlViewer extends AbstractViewer {
  ...
  private static String LARGE_FILE_MSG =
      "<html><body>" //$NON-NLS-1$
              ...
              if (System.getProperty("os.name")
                ...
                Runtime.getRuntime()
                    .exec(
                        new String[] {
                          "rundll32",
                          "SHELL32.DLL,ShellExec_RunDLL", //$NON-NLS-1$ //$NON-NLS-2$
                          "\"" + file.getCanonicalFile() + "\""
                        }); //$NON-NLS-1$ //$NON-NLS-2$
              } else {
                Runtime.getRuntime()
                    .exec(
                        new String[] {"xdg-open", file.toURI().toURL().toString()}); // $NON-NLS-1$
              }
                    ...
                    new ChangeListener<Worker.State>() {
                      ...
                      public void changed(
                        ...
                        if (newState == Worker.State.SUCCEEDED || newState == Worker.State.FAILED) {
                          ...
                          if (tmpFile != null
                              && !webEngine.getLocation().endsWith(tmpFile.getName())) return;
          ...
          do {
            ...
            if (term != null) {
              ...
              if (isNavigableTree) {
                ...
                try {
                  webEngine.executeScript(
                      "for (let el = document.getElementById('ipedHit-"
                          + (totalHits - 1)
              ...
              if (totalHits == 1) {
                ...
                if (shouldScrollToHit()) {
                  webEngine.executeScript(
                      "x = document.getElementById(\"ipedHit-"
                          + ++currentHit
                          + "\"); "
                          + scrollIntoView("x"));
                }
  ...
  @Override
  public void scrollToNextHit(
      final boolean forward, final boolean wrap, final HitsUpdater hitsUpdater) {
    if (totalHits <= 0) {
      return;
    }
                ...
                while (currTerm < queryTerms.length && queryTerms.length > 0) {
                  ...
                  if ((Boolean)
                    ...
                    if (currTerm != queryTerms.length) {
                      webEngine.executeScript(
                          "window.getSelection().collapse(document.body,0)"); //$NON-NLS-1$
                    }
                  }
                }
              }
              ...
              if (doc != null) {
                ...
                while (currTerm > -1) {
                  ...
                  if ((Boolean)
                    ...
                    if (currTerm != -1) {
                      webEngine.executeScript(
                          "window.getSelection().selectAllChildren(document)"); //$NON-NLS-1$
                      webEngine.executeScript(
                          "window.getSelection().collapseToEnd()"); //$NON-NLS-1$
                    }
                  }
                }
              }
