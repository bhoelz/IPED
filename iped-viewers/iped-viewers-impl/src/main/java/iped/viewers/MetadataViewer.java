...
public abstract class MetadataViewer extends AbstractViewer {
  ...
  private final DecimalFormat df =
      LocalizedFormat.getDecimalInstance("#,###.############"); // $NON-NLS-1$
  private final DateFormat dateFormat =
      new SimpleDateFormat(Messages.getString("MetadataViewer.DateFormat"));
  ...
  public MetadataViewer() {
    ...
    for (int i = 0; i < 3; i++) htmlViewers.add(new HtmlViewer());
  ...
  @Override
  public void init() {
    for (HtmlViewer viewer : htmlViewers) viewer.init();
  }
          ...
          public void run() {
            ...
            for (HtmlViewer viewer : htmlViewers) {
              ...
              if (content instanceof IItemReader) {
                ...
                String preview =
                    generatePreview((IItemReader) content, htmlViewers.indexOf(viewer));
                try {
                  if (viewer.tmpFile == null) {
                    viewer.tmpFile =
                        File.createTempFile("metadata", ".html"); // $NON-NLS-1$ //$NON-NLS-2$
                    viewer.tmpFile.deleteOnExit();
                  }
                  Files.write(
                      viewer.tmpFile.toPath(),
                      preview.getBytes("UTF-8"), // $NON-NLS-1$
  ...
  private String generatePreview(IItemReader item, int tabIndex) {
    ...
    sb.append(
        "<style>table {border-collapse: collapse; font-size:11pt; font-family: arial, verdana, sans-serif; width:100%; align:center; } table.t {margin-bottom:20px;} td { padding: 2px; } th {");
    sb.append("background-color:")
        .append(UiUtil.getHexRGB(color1))
        .append("; "); // $NON-NLS-1$ //$NON-NLS-2$
    sb.append("border: 1px solid ")
        .append(borderColor)
        .append(
            "; padding: 3px; text-align: left; font-weight: normal;} td.s1 {font-size:10pt; "); //$NON-NLS-1$ //$NON-NLS-2$
    sb.append("background-color:")
        .append(UiUtil.getHexRGB(color2))
        .append("; "); // $NON-NLS-1$ //$NON-NLS-2$
    sb.append("width:170px; border: 1px solid ")
        .append(borderColor)
        .append("; text-align:left;} td.s2 {font-size:10pt; "); // $NON-NLS-1$ //$NON-NLS-2$
    sb.append("background-color:")
        .append(UiUtil.getHexRGB(color3))
        .append("; "); // $NON-NLS-1$ //$NON-NLS-2$
    sb.append("border: 1px solid ")
        .append(borderColor)
        .append(
    ...
    // //$NON-NLS-2$
    sb.append(
        "textarea {readonly: readonly; height: 60px; width: 100%; resize: none;}\n"); //$NON-NLS-1$
    ...
    if (background != null)
      sb.append("background-color:")
          .append(UiUtil.getHexRGB(background))
          .append(";"); // $NON-NLS-1$  //$NON-NLS-2$
    if (foreground != null)
      sb.append("color:")
          .append(UiUtil.getHexRGB(foreground))
          .append(";"); // $NON-NLS-1$  //$NON-NLS-2$
    sb.append("\">\n"); // $NON-NLS-1$

    if (tabIndex == 0) fillBasicProps(sb, item);
    if (tabIndex == 1) fillAdvancedProps(sb, item);
  ...
  private void fillMetadata(StringBuilder sb, Metadata metadata) {
    ...
    sb.append(
        "<tr><th colspan=2>"
            + Messages.getString("MetadataViewer.Metadata")
            + "</th></tr>"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    Arrays.sort(metas, comparator);
    for (String meta : metas) {
      if (MetadataUtil.ignorePreviewMetas.contains(meta)) continue;
  ...
  private void fillBasicProps(StringBuilder sb, IItemReader item) {
    ...
    sb.append(
        "<tr><th colspan=2>"
            + Messages.getString("MetadataViewer.BasicProps")
            + "</th></tr>"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
  ...
  private void fillAdvancedProps(StringBuilder sb, IItemReader item) {
    ...
    sb.append(
        "<tr><th colspan=2>"
            + Messages.getString("MetadataViewer.AdvancedProps")
            + "</th></tr>"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    ...
    if (item.getMetadata() instanceof Metadata metadata) {
      fillProp(
          sb, ExtraProperties.TIKA_PARSER_USED, metadata.get(ExtraProperties.TIKA_PARSER_USED));
    }
    sb.append("</table>"); // $NON-NLS-1$
  }
  ...
  @Override
  public void dispose() {
    for (HtmlViewer viewer : htmlViewers) viewer.dispose();
  }
