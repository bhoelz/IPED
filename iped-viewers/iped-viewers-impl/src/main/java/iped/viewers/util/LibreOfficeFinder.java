    ...
    if (detectedPath == null) {
      ...
      if (!System.getProperty("os.name").startsWith("Windows")) { // $NON-NLS-1$ //$NON-NLS-2$
        try {
          ...
          if (ila.length != 0) {
            log.info(
                "Detected LibreOffice {} {}",
                ila[0].getMajorVersion(),
                ila[0].getHome()); // $NON-NLS-1$
        ...
        if (winPathLO.exists() || compressedLO.exists()) { // $NON-NLS-1$ //$NON-NLS-2$
          ...
          if (extractor.decompressLO(isNogui)) detectedPath = winPathLO.getAbsolutePath();
        }
      }
      if (detectedPath != null) System.setProperty("libreOfficePath", detectedPath);
    }
    return detectedPath;
  }
}
