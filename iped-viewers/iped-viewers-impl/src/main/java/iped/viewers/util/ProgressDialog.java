...
public class ProgressDialog implements ActionListener, Runnable, IProgressMonitor {
  ...
  private void createDialog() {
    if (parent instanceof Frame) dialog = new JDialog((Frame) parent, modal);
    else if (parent instanceof Dialog) dialog = new JDialog((Dialog) parent, modal);
    else if (parent != null) dialog = new JDialog(SwingUtilities.windowForComponent(parent), modal);
    else {
    ...
    if (modal.equals(ModalityType.MODELESS)) dialog.setFocusableWindowState(false);
    dialog.setLocationRelativeTo(parent);
    dialog.setVisible(true);
    dialog.setLocationRelativeTo(parent);

    progressBar.setIndeterminate(indeterminate);
    if (indeterminate) progressBar.setValue(100);
  }
