...
public class ProcessUtil {
  ...
  public static int getPidWin(int port) {
    ...
    try {
      ...
      if (pid != null) return Integer.valueOf(pid);
  ...
  public static int getPidLinux(int port) {
    ...
    try {
      ...
      String pid = null;
      for (String conection : conections) {
        if (conection.contains(":" + port)
            && conection.contains("/soffice.bin")) { // $NON-NLS-1$ //$NON-NLS-2$
  ...
  public static void killProcess(int port) {
    ...
    try {
      ...
      System.out.println(
          "Killed pid " + pid + " exitValue: " + result); // $NON-NLS-1$ //$NON-NLS-2$
  ...
  private static int getPid(String name, String arg) {
    ...
    else return getPidLinux(name, arg);
  }
