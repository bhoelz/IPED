...
public class ImageMetadataUtil {
  ...
  public static final synchronized void updateExifReaderToLoadThumbData() {
    ...
    List<JpegSegmentMetadataReader> allReaders =
        (List<JpegSegmentMetadataReader>) JpegMetadataReader.ALL_READERS;
    for (int n = 0, cnt = allReaders.size(); n < cnt; n++) {
      ...
      allReaders.set(
          ...
          new ExifReader() {
            @Override
            public void readJpegSegments(
                @NotNull final Iterable<byte[]> segments,
              ...
              for (byte[] segmentBytes : segments) {
                ...
                try {
                  ExifThumbnailDirectory tnDirectory =
                  ...
                  if (tnDirectory != null
                    ...
                    if (length > 0 && length < 1 << 24) {
                      ...
                      System.arraycopy(
                          segmentBytes, JPEG_SEGMENT_PREAMBLE.length() + offset, tnData, 0, length);
