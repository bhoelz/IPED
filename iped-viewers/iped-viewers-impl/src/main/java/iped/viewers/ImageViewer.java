...
public class ImageViewer extends AbstractViewer implements ActionListener {
  ...
  public static final String HIGHLIGHT_LOCATION =
      ImageViewer.class.getName() + "HighlightLocation:";
  ...
  private static final String[] ageOptions = {
    AGE_OPTION_CHILD, AGE_OPTION_TEENAGER, AGE_OPTION_ADULT, AGE_OPTION_MIDDLE_AGE, AGE_OPTION_AGED
  };
  ...
  protected volatile BufferedImage originalImage;
  protected volatile int rotation;
  protected volatile boolean applyBlurFilter = false;
  protected volatile boolean applyGrayScale = false;
  protected volatile boolean applyHighlightFaces = false;
  protected volatile List<String> facesLocations;
  protected volatile List<String> faceAgeLabels;
  protected volatile Dimension originalDimension;
  ...
  protected void loadFacesAttributes(IItemReader item) {
    ...
    if (faceLocationsAttr instanceof List) {
      facesLocations =
          ((List<?>) faceLocationsAttr)
              .stream().map(location -> HIGHLIGHT_LOCATION + location).collect(Collectors.toList());
  ...
  protected void drawRectangles(
      BufferedImage img, Collection<String> highlights, Color rectColorMain, boolean expanded) {
    ...
    for (String str : highlights) {
      if (str.startsWith(HIGHLIGHT_LOCATION + "[") && str.endsWith("]")) {
        String[] vals =
            str.substring(HIGHLIGHT_LOCATION.length() + 1, str.length() - 1).split(", ");
        ...
        RoundRectangle2D rc =
            new RoundRectangle2D.Double(left, top, right - left, bottom - top, arc, arc);
  ...
  public void init() {
    ...
    externalImageConverter.setNumThreads(
  ...
  private void createToolBar() {
    ...
    sliderBrightness.addChangeListener(
        new ChangeListener() {
          @Override
  ...
  protected JButton createAgeSelectionToolBarButton() {
    ...
    for (String ageOption : ageOptions) {
      JCheckBox checkBox =
          new JCheckBox(Messages.getString("ImageViewer.AgeSelectionOption." + ageOption));
    ...
    JButton ageButton =
        new JButton(Messages.getString("ImageViewer.AgeSelectionOptionTitle") + " ▾");
    ageButton.setVisible(applyHighlightFaces);
    ageButton.setMargin(new Insets(3, 10, 3, 10));
    ageButton.addMouseListener(
        new MouseAdapter() {
          @Override
  ...
  protected void applyHighlightFaces(boolean restoreImageBeforeHighlight) {
    ...
    if (applyHighlightFaces) {
      if (faceAgeLabels != null
          && facesLocations != null
          && faceAgeLabels.size() == facesLocations.size()) {
        for (String ageOption : ageOptions) {
    ...
    if (videoComment != null) {
      image =
          ImageUtil.getBestFramesFit(
              image, videoComment, imagePanel.getWidth(), imagePanel.getHeight());
    }
  }
