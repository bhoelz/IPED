...
public class IcePDFViewer extends AbstractViewer {
  ...
  public IcePDFViewer() {
    ...
    System.setProperty(
        "org.icepdf.core.views.page.text.highlightColor", "0xFFFF00"); // $NON-NLS-1$ //$NON-NLS-2$

    // Set rendering hints to improve rendering quality
    System.setProperty(
        "org.icepdf.core.screen.alphaInterpolation",
        "VALUE_ALPHA_INTERPOLATION_QUALITY"); //$NON-NLS-1$ //$NON-NLS-2$
    System.setProperty(
        "org.icepdf.core.screen.colorRender",
        "VALUE_COLOR_RENDER_QUALITY"); //$NON-NLS-1$ //$NON-NLS-2$
    System.setProperty(
        "org.icepdf.core.screen.interpolation",
        "VALUE_INTERPOLATION_BILINEAR"); //$NON-NLS-1$ //$NON-NLS-2$
    System.setProperty(
        "org.icepdf.core.screen.render", "VALUE_RENDER_QUALITY"); // $NON-NLS-1$ //$NON-NLS-2$
    ...
    new File(System.getProperties().getProperty("user.home"), ".icesoft/icepdf-viewer")
        .mkdirs(); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    pdfController
        ...
        .setAnnotationCallback(
            new org.icepdf.ri.common.MyAnnotationCallback(
                pdfController.getDocumentViewController()));

    ViewerPropertiesManager propManager = ViewerPropertiesManager.getInstance();
    propManager.set(
        ViewerPropertiesManager.PROPERTY_SHOW_TOOLBAR_ANNOTATION, "false"); // $NON-NLS-1$
    ...
    try {
      ...
      Image image =
          pdfController
              .getDocument()
              .getPageImage(
                  currPage, GraphicsRenderingHints.PRINT, Page.BOUNDARY_CROPBOX, currRot, zoom);
      if (image != null) {
