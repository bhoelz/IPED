...
public class ImageViewPanel extends JPanel {
   ...
   *     ser aplicado.
   * @param reference Ponto de referência que deve permanecer o mais próximo possível da localização
   *     que se encontrava antes da aplicação do zoom. Se for <code>null</code> considera o centro
   *     da área visível como ponto de referência.
   */
  public void changeZoom(double factor, Point reference) {
    Point pt =
        (reference == null)
            ? new Point(
                (int) imgPanel.getVisibleRect().getCenterX(),
                (int) imgPanel.getVisibleRect().getCenterY())
            : reference;
  ...
  private void updateZoomFactor(double limit, boolean vert, boolean horiz) {
    ...
    Dimension d =
        new Dimension(
            (int) (image.getWidth() * zoomFactor), (int) (image.getHeight() * zoomFactor));
    imgPanel.setPreferredSize(d);
    ...
    if (image != null) {
      if (factor > 0 && factor <= 100) {
        ...
        if (src.getColorModel().hasAlpha())
          op =
              new RescaleOp(
                  new float[] {scale, scale, scale, 1},
                  new float[] {offset, offset, offset, 0},
                  null);
        else op = new RescaleOp(scale, offset, null);
