...
/**
 * Renders image items (including multi-page TIFF) as PNG using headless ImageIO. TwelveMonkeys
 * plugins on the classpath extend format support transparently.
 */
@Slf4j
public class ImageWebRenderer implements WebRenderer {
  ...
  private void renderImage(RenderRequest request, OutputStream out) throws IOException {
    ...
    try (InputStream in = request.getItem().getSeekableInputStream();
      ...
      log.warn(
          "Failed to decode image for item {}: {}", request.getItem().getName(), e.getMessage());
