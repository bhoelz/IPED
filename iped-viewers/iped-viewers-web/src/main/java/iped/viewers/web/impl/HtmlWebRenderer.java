...
/**
 * Serves HTML/XHTML items as-is and generates HTML for known chat/link formats. Highlighting and
 * in-document search are performed client-side (hitsMode=internal).
 */
public class HtmlWebRenderer implements WebRenderer {
  ...
  private static final Set<String> HANDLED_TYPES =
      Set.of("text/html", "application/xhtml+xml", "text/asp", "text/aspdotnet");
  ...
  private void streamContent(RenderRequest request, OutputStream out) throws IOException {
    ...
    if (length > MAX_HTML_BYTES) {
      String msg =
          "<html><body><p>File too large to display inline ("
              + (length / 1024 / 1024)
              + " MB). Download to view.</p></body></html>";
