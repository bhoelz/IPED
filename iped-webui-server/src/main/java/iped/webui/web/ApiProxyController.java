...
/**
 * Transparent JSON proxy: forwards {@code /api/**} to the backing Jersey
 * {@code iped-webapi} so the browser only ever talks to one origin.
 *
 * <p>This keeps the engine as the single source of truth — the SSR server never
 * imports engine classes, it only relays JSON. More specific mappings (e.g. the
 * temporary {@link SearchStubController}) take precedence over this catch-all.
 */
@RestController
    ...
    public ResponseEntity<byte[]> proxy(HttpServletRequest request,
        ...
        try {
            return spec.retrieve()
                    .onStatus(status -> true, (req, res) -> { /* relay backend errors verbatim, don't throw */ })
                    .toEntity(byte[].class);
