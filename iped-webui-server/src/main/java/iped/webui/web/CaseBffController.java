 ...
 * </pre>
 *
 * <p>All backend calls are proxied to {@code iped-webapi /v2/cases}. If the webapi is unreachable
 * the picker still renders with an appropriate banner.
 */
@Controller
@Slf4j
public class CaseBffController {
  ...
  public RedirectView openCase(
      @RequestParam String path, @RequestParam(required = false) String id) {
    ...
    try {
      ...
      return new RedirectView(
          "/workspace?caseId=" + URLEncoder.encode(caseId, StandardCharsets.UTF_8));
  ...
  public RedirectView closeCase(@PathVariable String id) {
    try {
      apiClient.delete().uri("/v2/cases/{id}", id).retrieve().toBodilessEntity();
    } catch (RestClientException e) {
  ...
  private FetchResult fetchOpenCases() {
    try {
      List<?> raw = apiClient.get().uri("/v2/cases").retrieve().body(List.class);
          ...
          raw.stream()
              ...
              .map(
                  e -> {
                    ...
                    return new CaseSummary(str(m, "id"), str(m, "path"), str(m, "openedAt"), count);
                  })
              .toList();
  ...
  private static RedirectView errorRedirect(String msg) {
    return new RedirectView("/cases?error=" + URLEncoder.encode(msg, StandardCharsets.UTF_8));
  }
