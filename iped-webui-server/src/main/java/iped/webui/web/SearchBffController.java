...
/**
 * Backend-for-Frontend: adapts the results-grid island's two-step search contract to the single
 * {@code GET /v2/search} call implemented by {@code iped-webapi}.
 *
 * <p>Island contract (what the Angular component calls):
 *
 * <ol>
 *   <li>{@code POST /api/cases/{caseId}/search} → {@code {"searchId":"…"}}
 *   <li>{@code GET /api/cases/{caseId}/search/{searchId}/results?offset=&limit=} → {@code
 *       {searchId, total, page:{offset,limit}, items[]}}
 * </ol>
 *
 * <p>Implementation:
 *
 * <ul>
 *   <li>Step 1 encodes {@code {caseId, query}} into a Base64 token as the {@code searchId} — no
 *       server state needed.
 *   <li>Step 2 decodes the token and proxies to {@code GET
 *       /v2/search?q=…&sourceId={caseId}&offset=&limit=}.
 *   <li>When webapi returns HTTP 503 (no sources open), step 2 returns 503 so the island can
 *       surface a "no cases open" state rather than an empty grid.
 * </ul>
 *
 * <p>These mappings are more specific than the {@link ApiProxyController} catch-all and take
 * precedence over it. They replace {@code SearchStubController} entirely.
 */
@RestController
@Slf4j
public class SearchBffController {
  ...
  /**
   * Step 1: accept a search query and return a stable {@code searchId} token. The token encodes
   * {@code sourceId:query} in Base64 — no session state required.
   */
  @PostMapping(
  ...
  public ResponseEntity<Map<String, Object>> createSearch(
      @PathVariable String caseId, @RequestBody(required = false) Map<String, Object> body) {

    String query = body != null && body.get("query") instanceof String q ? q : "*";
    String token =
        Base64.getUrlEncoder()
            .withoutPadding()
            .encodeToString((caseId + "\0" + query).getBytes(StandardCharsets.UTF_8));
    return ResponseEntity.ok(Map.of("searchId", token, "status", "ready"));
  }

  /**
   * Step 2: decode the {@code searchId} token and proxy to webapi v2 search. Returns the same page
   * shape the island expects: {@code {searchId, total, page:{offset,limit}, items[]}}.
   */
  @GetMapping(
