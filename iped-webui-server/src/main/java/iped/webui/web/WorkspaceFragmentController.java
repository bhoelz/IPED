...
/**
 * HTMX fragment endpoints: return HTML partials that HTMX swaps into server-owned regions of the
 * workspace page.
 *
 * <p>Boundary rule: everything here renders server-owned markup. Rich, self-contained widget state
 * lives in Angular islands instead and talks to {@code /api/**}; the two never co-own a DOM
 * subtree.
 */
@Controller
@Slf4j
public class WorkspaceFragmentController {
  ...
  private static final Set<String> INFO_TABS = Set.of("hits", "sub", "par", "dup", "ref", "refby");
  private static final Set<String> VIEWER_MODES = Set.of("hex", "text", "meta", "preview");
  ...
  record FetchResult<T>(T data, boolean fromBackend) {
    static <T> FetchResult<T> live(T data) {
      return new FetchResult<>(data, true);
    }

    static <T> FetchResult<T> demo(T data) {
  ...
  public record ItemRow(
      String iid, String title, String name, String ext, String sizeStr, String typeDisp) {}

  // ── Static demo / fallback data ────────────────────────────────────────

  static final List<CategoryEntry> DEMO_CATEGORIES =
      List.of(
  ...
  static final List<BookmarkEntry> DEMO_BOOKMARKS =
      List.of(
  ...
  static final List<AiClassifier> DEMO_AI_CLASSIFIERS =
      List.of(
          new AiClassifier(
              "nsfw",
              "NSFW",
  ...
  static final List<EvidenceNode> DEMO_EVIDENCE_ROOTS =
      List.of(new EvidenceNode("src0", "evidence.dd [disk image]", true, 2439));

  static final Map<String, List<EvidenceNode>> DEMO_EVIDENCE_CHILDREN =
      Map.of(
          "src0",
              List.of(
  ...
  public String sidebar(
      @RequestParam(defaultValue = "cat") String tab,
      @RequestParam(required = false) String caseId) {
    caseId = resolvedCaseId(caseId);
    String active = SIDEBAR_TABS.contains(tab) ? tab : "cat";
    var catsResult =
        active.equals("cat") ? fetchCategories(caseId) : FetchResult.demo(List.<CategoryEntry>of());
    var bmsResult =
        active.equals("coll") ? fetchBookmarks(caseId) : FetchResult.demo(List.<BookmarkEntry>of());
    ...
    String content =
        sidebar
            .template(
                active, caseId, catsResult.data(), bmsResult.data(), clf, evid, filterState.list())
            .render()
            .toString();
  ...
  @ResponseBody
  public String evidenceChildrenFragment(
      @RequestParam String nodeId, @RequestParam(required = false) String caseId) {
    caseId = resolvedCaseId(caseId);
  ...
  /**
   * Add a filter chip. Returns the updated chips fragment so HTMX can swap {@code #filter-chips} in
   * the top bar.
   */
  @PostMapping(path = "/workspace/filter", produces = MediaType.TEXT_HTML_VALUE)
  @ResponseBody
  public String addFilter(@RequestParam String type, @RequestParam String label) {
  ...
  public String infoPanel(
      @RequestParam(defaultValue = "hits") String tab,
  ...
  /**
   * Loads a paginated list of items related to {@code itemId} for one of the relational info-panel
   * tabs. Proxies to the webapi endpoints; gracefully returns an empty list when the endpoint is
   * unavailable (v2 API pending).
   */
  ...
  public String infoItems(
    ...
    List<ItemRow> items =
        ref != null ? fetchRelatedItems(ref, active, offset, ITEMS_PAGE_LIMIT) : List.of();
  ...
  public String startExport(
      @RequestParam(defaultValue = "checked") String scope,
    ...
    try {
      ...
      String status =
          resp != null && resp.get("status") != null ? resp.get("status").toString() : "pending";
  ...
  public String exportStatus(@PathVariable String jobId) {
    try {
      ...
      String status =
          body != null && body.get("status") != null ? body.get("status").toString() : "unknown";
      boolean terminal =
          Set.of("done", "completed", "failed", "error", "cancelled")
  ...
  /**
   * Parses "{sourceId}:{docId}" item IDs. Returns {@code null} for opaque stub IDs without a colon
   * separator.
   */
  ...
  private static DocRef parseItemId(String itemId) {
    ...
    try {
      return new DocRef(itemId.substring(0, colon), Integer.parseInt(itemId.substring(colon + 1)));
  ...
  private FetchResult<List<CategoryEntry>> fetchCategories(String caseId) {
    ...
    try {
      ...
      if (list != null && !list.isEmpty()) {
        return FetchResult.live(
            list.stream().map(s -> new CategoryEntry(s.toString(), -1L)).toList());
      }
    } catch (RestClientException e) {
    ...
    try {
      ...
      if (body != null && body.get("hits") instanceof List<?> rawHits) {
        ...
        for (Object h : rawHits) {
          ...
          result.add(new HitSnippet(str(hm, "before"), str(hm, "match"), str(hm, "after")));
        }
    ...
    try {
      Map<?, ?> body =
          apiClient
              .get()
              .uri(
                  "/sources/{src}/docs/{id}/{seg}?offset={off}&limit={lim}",
                  ref.sourceId(),
                  ref.docId(),
                  segment,
                  offset,
      ...
      for (Object entry : list) {
        ...
        m.forEach(
            (k, v) -> {
              if (v == null) return;
              String val =
                  (v instanceof List<?> l && !l.isEmpty()) ? l.get(0).toString() : v.toString();
  ...
  private static String exportModal(String jobId, String initialStatus) {
    return "<div class=\"modal-backdrop\" onclick=\"document.getElementById('modal').innerHTML=''\"></div>"
        ...
        + "<div class=\"modal-header\"><span style=\"font-weight:600\">Export queued</span>"
        + CLOSE_BTN
        + "</div>"
        + "<div id=\"export-body\" hx-get=\"/workspace/export/status/"
        + jobId
        + "\""
        + " hx-trigger=\"load, every 3s\" hx-swap=\"outerHTML\">"
        + exportStatusBody(jobId, initialStatus, false)
        + "</div>"
  ...
  private static String exportErrorModal(String message) {
    return "<div class=\"modal-backdrop\" onclick=\"document.getElementById('modal').innerHTML=''\"></div>"
        ...
        + "<div class=\"modal-header\"><span style=\"font-weight:600\">Export failed</span>"
        + CLOSE_BTN
        + "</div>"
  ...
  private static String exportStatusBody(String jobId, String status, boolean terminal) {
    ...
    String indicator =
            ...
            ? "<span style=\"color:var(--ok,#27ae60)\">✔</span>"
            : failed
                ? "<span style=\"color:var(--error,#e74c3c)\">✖</span>"
                : "<span class=\"pulse\"></span>";
    String body =
        "<div class=\"modal-body\" style=\"padding:16px 20px\">"
            + "<div style=\"display:flex;align-items:center;gap:10px\">"
            + indicator
            + "<span>Job <code class=\"mono\">"
            + jobId
            + "</code> — "
            + status
            + "</span>"
    ...
    return "<div id=\"export-body\" hx-get=\"/workspace/export/status/"
        + jobId
        + "\""
