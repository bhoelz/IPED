 ...
 *
 * <p>Requires a {@code caseId} query parameter that refers to a case already
 * opened in iped-webapi. Missing or blank → redirects to the case picker.
 * Switching to a different case clears the per-session filter state.
 */
@Controller
public class WorkspaceController {
    ...
    public WorkspaceController(IslandManifest manifest, FilterState filterState,
                               WebMapProperties mapProperties) {
    ...
    public ResponseEntity<String> workspace(
        ...
        String html = page.template(manifest.styles(), manifest.scripts(), caseId, username,
                        mapProperties.getTileUrl(), mapProperties.getTileAttribution(),
