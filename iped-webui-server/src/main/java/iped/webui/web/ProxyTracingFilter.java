...
/**
 * Injects a per-request trace ID into the MDC and propagates it to the
 * downstream iped-webapi proxy call via {@code X-Trace-Id}.
 *
 * <p>Trace flow:
 * <pre>
 ...
 * </pre>
 *
 * <p>If the incoming request already carries an {@code X-Trace-Id} header
 * (e.g. from a load-balancer or upstream gateway) it is reused unchanged,
 * so the full call chain is observable as a single trace across services.
 *
