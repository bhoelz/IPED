...
/**
 * Resolves the content-hashed Angular island bundle filenames produced by the production build
 * (e.g. {@code main-AB12CD34.js}) so templates can reference them without knowing the hash.
 *
 * <p>The Angular {@code islands} build stages its output under {@code classpath:/static/islands/};
 * this bean scans that location once at startup. When no bundle is present (e.g. a dev run where
 * the frontend build was skipped) the lists are empty and the SSR page still renders — the island
 * host element simply never upgrades.
 *
 * <p>If a {@code manifest.json} (written by {@code generate-islands-manifest.mjs}) is present,
 * per-island chunk URLs are available via {@link #chunkFor(String)} for targeted {@code <link
 * rel="modulepreload">} injection.
 */
@Component
@Slf4j
public class IslandManifest {
  ...
  public IslandManifest() {
    ...
    if (scripts.isEmpty()) {
      log.warn(
          "No Angular island bundles found under static/islands/. "
              + "Run the frontend build (npm run build:islands) so islands can mount.");
    } else {
      log.info(
          "Resolved island bundles: scripts={}, styles={}, manifest-chunks={}",
          scripts,
          styles,
          chunkByIsland.keySet());
    }
  }
  ...
  /**
   * Returns the hashed chunk URL for the named island, if the manifest is present. Use to emit
   * {@code <link rel="modulepreload" href="...">} only for the islands needed on a given page.
   *
  ...
  private static List<String> resolve(String pattern) {
    ...
    result.sort(
        Comparator.comparingInt(IslandManifest::loadRank).thenComparing(Comparator.naturalOrder()));
