...
/**
 * Wiring checks for the vertical-slice pilot, using MockMvc (WebEnvironment.MOCK) so no embedded
 * Tomcat is required — tests pass in environments where the JDK NIO selector fails to open a
 * loopback pipe (e.g. sandboxed JVMs on Windows).
 */
...
class WorkspacePilotTest {
  ...
  void searchBffCreatesSearchTokenAndReturnsSearchId() throws Exception {
    mvc.perform(
            post("/api/cases/demo-case/search")
