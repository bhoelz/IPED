 ...
 *
 * <p>These tests verify behavior when iped-webapi is NOT reachable (no webapi
 * process running). Controllers must return 200 with well-formed HTML, fall
 * back to demo data or show an error state, and never surface a 5xx.
 *
 * <p>Uses {@code WebEnvironment.MOCK} so no embedded Tomcat starts — tests
 * pass in environments where the JDK NIO selector is restricted.
 *
...
class WorkspaceBffContractTest {
    ...
    void infoPanelHitsTab_returnsFragmentForUnknownItem() throws Exception {
        mvc.perform(get("/workspace/info")
                        .param("tab", "hits")
                        .param("itemId", "src0:1"))
                .andExpect(status().isOk())
                .andExpect(content().string(not(containsString("<html"))));
