 ...
 * <p>
 * One instance is created per {@code IPEDSource} during case bootstrap. If
 * a {@code .additional-index/} directory exists inside the case's module
 * directory, a {@link LuceneAdditionalDataSource} is registered automatically.
 * </p>
 */
public class DefaultAdditionalDataSourceManager implements IAdditionalDataSourceManager {
    ...
    public void register(IAdditionalDataSource source) {
        if (source == null) throw new IllegalArgumentException("source must not be null"); //$NON-NLS-1$
        sources.add(source);
    }
