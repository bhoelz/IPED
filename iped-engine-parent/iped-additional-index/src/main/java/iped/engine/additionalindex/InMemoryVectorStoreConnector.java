...
public final class InMemoryVectorStoreConnector implements IVectorStoreConnector {
    ...
    @Override public void indexVector(int itemId, String model, float[] vector) {
        validate(vector);
        vectors.put(itemId, vector.clone());
    }
    @Override public List<VectorMatch> similaritySearch(float[] query, int limit) {
        validate(query);
    ...
    @Override public void close() { vectors.clear(); }
