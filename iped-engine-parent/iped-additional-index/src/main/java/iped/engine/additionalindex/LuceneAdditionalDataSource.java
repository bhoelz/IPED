...
/**
 * {@link IAdditionalDataSource} implementation backed by a separate Lucene index stored at {@code
 * <case>/iped/.additional-index/}.
 *
 * <h3>Schema</h3>
 *
 ...
 *
 * <ul>
 *   <li>{@code _docKey} – StringField (indexed + stored): synthetic PK {@code
 *       "<itemId>_<taskName>"} used for upsert.
 *   <li>{@code _itemId} – IntPoint (indexed) + StoredField.
 *   <li>{@code _taskName} – StoredField.
 *   <li>{@code _processedAt} – StoredField (epoch ms as long string).
 *   <li>{@code ea_<key>} – StoredField per extra attribute, value format: {@code
 *       "<typeCode>|<encodedValue>"}.
 * </ul>
 *
 * <h3>Type encoding</h3>
 *
 ...
 * <h3>Thread safety</h3>
 *
 * Concurrent {@link #storeTaskResult} calls are safe (Lucene IndexWriter is thread-safe). Reader
 * refresh is protected by a read-write lock.
 */
@Slf4j
public class LuceneAdditionalDataSource implements IAdditionalDataSource {
  ...
  /**
   * Guards {@link #reader} / {@link #searcher} access: write-lock for refresh, read-lock for
   * queries.
   */
    ...
    if (extraAttrs != null) {
      for (Map.Entry<String, Object> entry : extraAttrs.entrySet()) {
        if (entry.getValue() != null) {
          doc.add(new StoredField(ATTR_PREFIX + entry.getKey(), encodeValue(entry.getValue())));
        }
      }
    }
  ...
  /**
   * Overrides the default interface implementation with a single Lucene query that fetches all task
   * documents for the item at once.
   */
  @Override
  ...
  /**
   * Encodes an attribute value as {@code "<typeCode>|<stringRepresentation>"}. Unknown types fall
   * back to {@code "s|<toString()>"}.
   */
  ...
  /**
   * Decodes a value previously encoded by {@link #encodeValue}. Returns the raw string on parse
   * errors or unknown type codes.
   */
