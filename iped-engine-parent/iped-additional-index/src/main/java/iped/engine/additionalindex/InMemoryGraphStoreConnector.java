...
public final class InMemoryGraphStoreConnector implements IGraphStoreConnector {
    ...
    @Override public void index(int itemId, String taskName, Map<String, Object> attributes) { }
    @Override public void upsertRelationship(EvidenceRelationship relationship) {
        relationships.removeIf(r -> r.sourceItemId() == relationship.sourceItemId()
                && r.targetItemId() == relationship.targetItemId() && r.type().equals(relationship.type()));
        relationships.add(Objects.requireNonNull(relationship));
    }
    @Override public List<EvidenceRelationship> findRelationships(int itemId, int limit) {
        return relationships.stream().filter(r -> r.sourceItemId() == itemId || r.targetItemId() == itemId)
    ...
    @Override public void close() { relationships.clear(); }
}
