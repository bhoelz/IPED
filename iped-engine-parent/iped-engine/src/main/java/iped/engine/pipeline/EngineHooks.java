...
/**
 * Registry of job-lifecycle and item-processing listeners attached to the
 * processing engine.
 *
 * <p>This is the clean seam between the engine and distributed/monitoring
 * infrastructure. Instead of importing Kafka or coordinator types directly,
 * the engine fires events through these hooks and leaves the transport-layer
 * wiring to iped-distributed.
 *
 * <h2>Contract</h2>
 * <ul>
 *   <li>Listeners registered here will be invoked synchronously on the
 *       thread that triggers the event (worker thread for item events,
 *       manager thread for job events).</li>
 *   <li>Listener implementations MUST be non-blocking. If a listener needs
 *       to do I/O, it should dispatch to its own executor.</li>
 *   <li>Listener exceptions are caught and logged; they do not abort
 *       processing.</li>
 * </ul>
 *
 * <p>One {@code EngineHooks} instance is created per {@code CaseContext} so
 * listeners can be scoped to an individual case.
 */
public final class EngineHooks {
    ...
    /**
     * Fires a job lifecycle event to all registered job listeners.
     * Exceptions thrown by individual listeners are swallowed after logging.
     *
    ...
    /**
     * Fires an item processing event to all registered item listeners.
     * Exceptions thrown by individual listeners are swallowed after logging.
     *
