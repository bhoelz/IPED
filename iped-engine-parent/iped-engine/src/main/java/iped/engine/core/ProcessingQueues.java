...
public class ProcessingQueues {
  ...
  */
  ...
  private void initConfig() {
    ProcessingPriorityConfig config =
        ConfigurationManager.get().findObject(ProcessingPriorityConfig.class);
  ...
  public synchronized boolean isNoItemInQueueOrBeingProcessed() {
    return totalItemsBeingProcessed == 0
        && getItemQueueTop().isEmpty()
        && getItemQueueRest().isEmpty();
  }
