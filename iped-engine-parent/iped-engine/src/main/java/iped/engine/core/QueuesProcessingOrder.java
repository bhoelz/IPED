...
/**
 * Classe de definição de prioridade de processamento de itens com base no mimeType. Para cada
 * prioridade, é criada uma fila de processamento contendo os itens com tal prioridade. MimeTypes
 * sem prioridade definida recebem a prioridade padrão zero. Primeiro são processados os itens da
 * fila de prioridade 0, depois da fila de prioridade 1 e assim por diante. Assim é possível
 * configurar dependências de processamento entre os itens.
 *
 * @author Nassif
 */
public class QueuesProcessingOrder {
  ...
  public static Set<Integer> getProcessingQueues() {
    ...
    for (Integer p : mediaTypes.values()) priorities.add(p);
