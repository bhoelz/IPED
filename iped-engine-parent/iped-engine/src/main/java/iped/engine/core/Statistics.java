...
public class Statistics {
  ...
  public void incCarvedIgnored(IItem item) {
    ...
    HashValue parentPersistId =
        new HashValue((String) item.getExtraAttribute(IndexItem.PARENT_TRACK_ID));
    synchronized (ignoredMap) {
  ...
  public synchronized int getSplits() {
    return splits;
  }

  public synchronized void incSplits() {
    splits++;
  }
  ...
  public synchronized void incTimeouts() {
    timeouts++;
  }

  public synchronized void incProcessed() {
    processed++;
  }
  ...
  public synchronized void incIoErrors() {
    ioerrors++;
  }

  public synchronized int getIoErrors() {
    return ioerrors;
  }

  public synchronized void incActiveProcessed() {
    activeProcessed++;
  }

  public synchronized int getActiveProcessed() {
    return activeProcessed;
  }

  public synchronized void addVolume(long volume) {
  ...
  public synchronized int getCorruptCarveIgnored() {
  ...
  public synchronized void updateLastId(int id) {
    if (id > lastId) {
      lastId = id;
    }
  }
  ...
  public synchronized void setLastId(int id) {
    lastId = id;
  }
  ...
  public void logStatistics(Manager manager) throws Exception {
    ...
    log.info(
        "Total processed: {} items in {} seconds ({} MB)",
        processed, //$NON-NLS-1$
    ...
    if (processed != discovered) {
      log.error(
          "Alert: Processed " + processed + " items of " + discovered); // $NON-NLS-1$ //$NON-NLS-2$
    }
    ...
    ExportEnablementSettings exportByKeywords =
        ConfigurationManager.get().findObjectInstanceOf(ExportEnablementSettings.class);
    boolean automaticExportEnabled =
        ConfigurationManager.get().getEnableTaskProperty("enableAutomaticExportFiles");
      ...
      if (indexed != discovered - carvedIgnored - ignored) {
        log.error(
            "Alert: Indexed "
                + indexed
                + " items of "
                + (discovered - carvedIgnored - ignored)); // $NON-NLS-1$ //$NON-NLS-2$
      }
    } /*
    ...
    if (this.getIoErrors() > processed * IO_ERROR_RATE_TO_WARN)
      log.error(
          "Warning: IO Errors happened while reading {} items from {}!",
          getIoErrors(),
          processed); //$NON-NLS-1$
  }
  ...
  public void printSystemInfo() throws Exception {
    ...
    if (warn != null) log.error(warn); // $NON-NLS-1$ //$NON-NLS-2$

    String arch = System.getProperty("os.arch");
    log.info("Architecture: {}", arch); // $NON-NLS-1$
    if (!arch.contains("64")) {
      throw new IPEDException(
          "Java 32 bits not supported anymore. Please update to a 64 bits version.");
    }
    ...
    if (maxMemory / localConfig.getNumThreads() < minMemPerThread) {
      String memoryAlert =
          Messages.getString("Statistics.LowMemory.Msg")
              .replace(
                  "{}", //$NON-NLS-1$
      ...
      UIPropertyListenerProvider.getInstance()
          .firePropertyChange(
              ...
              null, //$NON-NLS-1$
              new EngineMessage(
                  Messages.getString("Statistics.LowMemory.Title"), memoryAlert)); // $NON-NLS-1$
