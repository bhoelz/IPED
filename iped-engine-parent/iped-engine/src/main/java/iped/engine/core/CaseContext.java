...
public class CaseContext {
  ...
  /**
   * Set of trackIDs for items already submitted to this case's processing queue. Used by {@code
   * Worker.processNewItem} to skip duplicate submissions, making item processing idempotent under
   * Kafka retry / crash-resume replay.
   */
  ...
  /**
   * Returns the event hooks for this case. External components (e.g. iped-distributed) register
   * their listeners here during case startup to receive job-lifecycle and item-processing events
   * without importing engine or Kafka types.
   *
  ...
  /**
   * Returns the per-case set of already-submitted trackIDs. Workers use this to implement
   * idempotent item processing: if {@code add(trackId)} returns {@code false} the item was already
   * queued and must be skipped.
   *
  ...
  public String toString() {
    return "CaseContext{" + "id=" + id + ", state=" + state + '}';
  }
}
