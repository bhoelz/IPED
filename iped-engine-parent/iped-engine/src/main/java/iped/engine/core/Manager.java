 ...
 * pós-processamento.
 *
 * <p>O contador apenas enumera e soma o tamanho dos itens que serão processados, permitindo que
 * seja estimado o progresso e término do processamento.
 *
 * <p>O produtor obtém os itens a partir de uma fonte de dados específica (relatório do UFED,
 * diretório, imagem), inserindo-os numa fila de processamento com tamanho limitado (para limitar o
 * uso de memória).
 *
 * <p>Os consumidores (workers) retiram os itens da fila e são responsáveis pelo seu processamento.
 ...
 *
 * <p>Após inicializar o processamento, o manager realiza o monitoramento, verificando se alguma
 ...
 *
 * <p>O pós-processamento inclui a pré-ordenação das propriedades dos itens, o armazenamento do
 * volume de texto indexado de cada item, do mapeamento indexId para id, dos ids dos itens
 * fragmentados, a filtragem de categorias e palavras-chave e o log de estatísticas do
 * processamento.
 */
@Log4j2
public class Manager implements iped.engine.datasource.IDatasourceRegistry {
  ...
  /**
   * Get the CaseContext that owns this Manager instance. Used during worker initialization to set
          ...
          return (iped.engine.datasource.IDatasourceRegistry)
              prodClass
                  .getDeclaredConstructor(
                      cfgClass, String.class, iped.engine.datasource.IDatasourceRegistry.class)
  ...
  public void process() throws Exception {
    ...
    for (File source : sources) {
      log.info(
          "Evidence " + (i++) + ": '{}'", source.getAbsolutePath()); // $NON-NLS-1$ //$NON-NLS-2$
    }
    ...
    TaskRuntime.invokeConstructedVoid(
        "iped.engine.task.P2PBookmarker",
        ICaseData.class,
        caseData,
        "createBookmarksForSharedFiles",
        File.class,
  ...
  private IndexWriterConfig getIndexWriterConfig() {
    ...
    if ((localConfig.isIndexTempOnSSD() && indexDir != finalIndexDir)
        || localConfig.isOutputOnSSD()) {
  ...
  private void removeEvidence(String evidenceName, EvidenceStatus status) throws Exception {
    ...
    try (IPEDSource ipedCase = new IPEDSource(output.getParentFile(), writer)) {
      String query =
          BasicProps.NAME + ":\"" + evidenceName + "\" AND " + BasicProps.ISROOT + ":true";
    ...
    int deletions =
        GraphFileWriter.removeDeletedRelationships(
            evidenceUUID, new File(output, GraphConstants.CSVS_PATH));
  ...
  private void openIndex() throws IOException {
    UIPropertyListenerProvider.getInstance()
        .firePropertyChange(
            "mensagem",
            "",
            Messages.getString("Manager.OpeningIndex")); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
  ...
  private void initWorkers() throws Exception {
    ...
    UIPropertyListenerProvider.getInstance()
        .firePropertyChange("workers", 0, workers); // $NON-NLS-1$
  }
      ...
      if (counter.isAlive() && currentDir != null && !currentDir.trim().isEmpty()) {
        UIPropertyListenerProvider.getInstance()
            .firePropertyChange(
                "decodingDir",
                0, //$NON-NLS-1$
                Messages.getString("Manager.Adding")
                    + currentDir.trim()
                    + "\""); //$NON-NLS-1$ //$NON-NLS-2$
      }
      ...
      if (changeToNextQueue) {
        ...
        if (!queueEnd.isQueueEnd()) {
          throw new IPEDException(
              "Tried to get queue end from queue, but failed! Please warn the dev team.");
        }
        if (processingQueues.changeToNextQueue() != null) {
          log.info(
              "Changed to processing queue with priority "
                  + processingQueues.getCurrentQueuePriority()); // $NON-NLS-1$
          caseData.putCaseObject(
              IItemSearcher.class.getName(), new ItemSearcher(output.getParentFile(), writer));
        ...
        new Thread() {
          ...
          public void run() {
            try {
              ...
              UIPropertyListenerProvider.getInstance()
                  .firePropertyChange("mensagem", "", Messages.getString("Manager.CommitStarted"));
              ...
              TaskRuntime.invokeStaticVoid(
                  "iped.engine.task.ExportCSVTask", "commit", File.class, output);

              TaskRuntime.invokeStaticVoid(
                  "iped.engine.task.index.ElasticSearchIndexTask", "commit");

              writer.commit();

              long end = System.currentTimeMillis() / 1000;
              UIPropertyListenerProvider.getInstance()
                  .firePropertyChange("mensagem", "", Messages.getString("Manager.CommitFinished"));
  ...
  private void finishProcessing() throws Exception {
    ...
    if (indexConfig.isForceMerge()) {
      UIPropertyListenerProvider.getInstance()
          .firePropertyChange(
              "mensagem",
              "",
              Messages.getString("Manager.Optimizing")); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    ...
    UIPropertyListenerProvider.getInstance()
        .firePropertyChange(
            "mensagem",
            "",
            Messages.getString("Manager.ClosingIndex")); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    ...
    if (!indexDir.getCanonicalPath().equalsIgnoreCase(finalIndexDir.getCanonicalPath())) {
      UIPropertyListenerProvider.getInstance()
          .firePropertyChange(
              ...
              "",
              Messages.getString(
                  "Manager.CopyingIndex")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
  ...
  private void filterKeywords() {
    ...
    try {
      ...
      ArrayList<String> palavras =
          Util.loadKeywords(
              output.getAbsolutePath() + "/palavras-chave.txt", // $NON-NLS-1$
      ...
      if (palavras.size() != 0) {
        ...
        Util.saveKeywords(
            palavrasFinais,
            output.getAbsolutePath() + "/palavras-chave.txt",
  ...
  private void removeEmptyTreeNodes() {
    ...
    try (IPEDSource ipedCase = new IPEDSource(output.getParentFile())) {
      ...
      for (int docID : result.getLuceneIds()) {
        String parentIds =
            ipedCase.getReader().storedFields().document(docID).get(IndexItem.PARENTIDs);
  ...
  private void prepareOutputFolder() throws Exception {
    if (output.exists()
      ...
      throw new IPEDException(
          "Directory already exists: " + output.getAbsolutePath()); // $NON-NLS-1$
    }
    ...
    if (export.exists()
        ...
        && args.getEvidenceToRemove() == null) {
      throw new IPEDException(
          "Directory already exists: " + export.getAbsolutePath()); // $NON-NLS-1$
    }
    ...
    if (!args.isAppendIndex()
      ...
      IOUtil.copyDirectory(
          new File(Configuration.getInstance().appRoot, "lib"),
          new File(output, "lib"),
          true); //$NON-NLS-1$ //$NON-NLS-2$
      IOUtil.copyDirectory(
          new File(Configuration.getInstance().appRoot, "scripts"),
          new File(output, "scripts"),
          true); //$NON-NLS-1$ //$NON-NLS-2$
      IOUtil.copyDirectory(
          new File(Configuration.getInstance().appRoot, "jre"),
          new File(output, "jre"),
          true); //$NON-NLS-1$ //$NON-NLS-2$
      IOUtil.copyDirectory(
          new File(Configuration.getInstance().appRoot, iped.localization.Messages.BUNDLES_FOLDER),
          new File(output, iped.localization.Messages.BUNDLES_FOLDER),
          true); // $NON-NLS-1$ //$NON-NLS-2$
      ...
      IOUtil.copyDirectory(
          new File(Configuration.getInstance().appRoot, "help"),
          new File(output, "help")); // $NON-NLS-1$ //$NON-NLS-2$
      IOUtil.copyDirectory(
          new File(Configuration.getInstance().appRoot, "htmlreport"), // $NON-NLS-1$
      ...
      if (!currentProfile.equals(defaultProfile)) {
        IOUtil.copyDirectory(
            currentProfile, new File(output, Configuration.CASE_PROFILE_DIR), true);
      ...
      if (caseData.isIpedReport()) {
        File caseProfile =
            new File(Configuration.getInstance().appRoot, Configuration.CASE_PROFILE_DIR);
      ...
      if (binDir.exists()) IOUtil.copyDirectory(binDir, output.getParentFile()); // $NON-NLS-1$
      else {
        // Copy only IPED Windows executable (#1698)
        File exe =
            new File(
                new File(Configuration.getInstance().appRoot).getParentFile(), appWinExeFileName);
        if (exe.exists()) {
