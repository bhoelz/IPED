 ...
 *
 * <p>Tracks memory usage per case and enforces quotas to prevent resource exhaustion when
 * processing multiple cases concurrently.
 */
public class ResourceManager {
  ...
  /**
   * Acquire resources for a case. Returns immediately if resources available, blocks or throws if
   * limits exceeded.
   *
  ...
  public synchronized void acquireResources(UUID caseId) {
    int activeCases =
        (int)
            caseMemoryUsage.keySet().stream()
                .filter(id -> !casePaused.getOrDefault(id, false))
                .count();

    if (activeCases >= maxConcurrentCases) {
      throw new IllegalStateException("Max concurrent cases (" + maxConcurrentCases + ") exceeded");
    }
  ...
  public synchronized boolean enforceQuotas() {
    ...
    for (UUID caseId : caseMemoryUsage.keySet()) {
      ...
      boolean shouldPause = caseUsage > maxMemoryPerCase || memoryUsed > (availableMemory * 0.9);
  ...
  public int getActiveCaseCount() {
    return (int)
        caseMemoryUsage.keySet().stream().filter(id -> !casePaused.getOrDefault(id, false)).count();
  }
