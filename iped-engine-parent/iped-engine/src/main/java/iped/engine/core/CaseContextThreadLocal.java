...
/**
 * Thread-local holder for CaseContext. Allows each worker thread to access its associated case
 * context without needing to pass it through every method call.
 *
 * <p>Usage: - In worker thread: CaseContextThreadLocal.set(caseContext) - In task/processor:
 * CaseContext ctx = CaseContextThreadLocal.get() - On thread cleanup:
...
public class CaseContextThreadLocal {
  ...
  /**
   * Clear the current thread's case context. Should be called in finally blocks to prevent context
   * leaks.
   */
