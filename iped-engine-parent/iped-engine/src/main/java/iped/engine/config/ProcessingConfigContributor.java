...
/**
 * Extends {@link EngineConfigContributor} with the task pipeline -- discovering and instantiating
 * every pipeline/plugin task and registering each task's own {@link Configurable}s. Pulls in a
 * dependency on every task/parser/carver plugin module, so this must only be used by an actual
 * case-processing entry point ({@code iped-app}'s {@code Main}), never by something that merely
 * opens an already-processed case to read results.
 */
