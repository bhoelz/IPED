...
class TaskRegistry {
  ...
  static final class TaskRegistration {
    ...
    static TaskRegistration provider(
        String id,
        TaskDescriptor descriptor,
        TaskFactory factory,
        String source,
      ...
      for (TaskDependency dep : registration.descriptor.dependencies()) {
        ...
        if (!registrations.containsKey(dependencyTaskId)) {
          ...
          throw new IllegalStateException(
              "Task '" + registration.id + "' has missing dependency '" + dependencyTaskId + "'");
        }
  ...
  private static void addEdge(
      Map<String, Set<String>> adjacency, Map<String, Integer> indegree, String from, String to) {
    if (adjacency.get(from).add(to)) {
      indegree.merge(to, 1, Integer::sum);
    }
  }

  static TaskRegistry merge(
      Collection<TaskRegistration> pipelineRegistrations, TaskRegistry pluginRegistry) {
