...
public class TimelineResults {
  ...
  /**
   * One greater than the maximum number of allowed timestamp values in the same DateTime property.
   */
  ...
  private void loadDocValues() throws IOException {
    ...
    timeEventGroupValues =
        ipedCase.getAtomicReader().getSortedSetDocValues(ExtraProperties.TIME_EVENT_GROUPS);
    eventsInDocOrdsValues =
        ipedCase.getAtomicReader().getBinaryDocValues(ExtraProperties.TIME_EVENT_ORDS);
  }
  ...
  public static class TimeItemId extends ItemId {
    ...
    public TimeItemId(
        TimelineResults timelineResults, int sourceId, int id, int timeStampOrd, int timeEventOrd) {
      super(sourceId, id);
