...
public class SimilarFacesSearch {
  ...
  public SimilarFacesSearch(IPEDSource ipedCase, IItem refImage) {
    this.ipedCase =
        ipedCase instanceof IPEDMultiSource
            ? (IPEDMultiSource) ipedCase
  ...
  public MultiSearchResult filter(MultiSearchResult result) throws IOException {
    ...
    return (MultiSearchResult)
        ImageSimilarityLowScoreFilter.filter(result, squaredDistToScore(minDistSquared));
  }
                ...
                public void run() {
                  ...
                  for (int i = i0; i < i1; i++) {
                    ...
                    try {
                      ...
                      while (hasVal && (ordinal = similarityFeaturesValues.nextOrd()) != -1) {
                        ...
                        for (int j = 0; j < numRefFaces; j++) {
                          float squaredDist =
                              distance(refSimilarityFeatures[j], currentFeatures, minDist);
  ...
  public static float distance(float[] a, float[] b, float cut) {
    ...
    for (int i = 0; i < a.length && distance <= cut; ) {
      float d = a[i] - b[i++];
      distance +=
          d * d + (d = a[i] - b[i++]) * d + (d = a[i] - b[i++]) * d + (d = a[i] - b[i++]) * d;
    }
    return distance;
  }
