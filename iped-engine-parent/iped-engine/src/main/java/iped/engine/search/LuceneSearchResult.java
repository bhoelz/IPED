...
/**
 * Classe que armazena os resultados de uma pesquisa. Decompõe ScoreDoc[] para descartar atributo
 * desnecessário e header do objeto, ocupando 1/3 da memória original.
 *
 * @author Nassif
 */
public class LuceneSearchResult {
  ...
  public LuceneSearchResult intersect(LuceneSearchResult items) {
    ...
    for (int i = 0; i < result.length; i++)
      if (Arrays.binarySearch(docs2, result.docs[i]) < 0) result.docs[i] = -1;
  ...
  private LuceneSearchResult intersect2(LuceneSearchResult items) {
    ...
    while (i < result.length) {
      if (j == docs2.length || result.docs[index[i]] < docs2[j]) result.docs[index[i++]] = -1;
      else if (result.docs[index[i]] > docs2[j]) j++;
      else {
        i++;
        j++;
      }
