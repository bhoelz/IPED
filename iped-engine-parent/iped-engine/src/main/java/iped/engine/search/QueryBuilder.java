...
public class QueryBuilder {
    ...
    if (query != null) {
      if (query instanceof BooleanQuery) {
        ...
        if (query instanceof MultiTermQuery) {
          ...
          try {
            TermsEnum terms =
                mtq.getTermsEnum(((IPEDSource) ipedCase).getLeafReader().terms(mtq.getField()));
  ...
  private Query getContentQuery(Query query, String field) {
    ...
    ToParentBlockJoinQuery blockJoinQuery =
        new ToParentBlockJoinQuery(query, getParentsFilter(), ScoreMode.Total);
  ...
  public Query rewriteQuery(Query query, boolean expandCategories) {
    ...
    if (query instanceof MatchAllDocsQuery) {
      ...
      return getContentQuery(
          new TermRangeQuery(
              getNonLocalizedField(q.getField()),
              q.getLowerTerm(),
              q.getUpperTerm(),
              q.includesLower(),
              q.includesUpper()),
  ...
  public Query getQuery(String texto, Analyzer analyzer) throws ParseException, QueryNodeException {
    if (texto.trim().startsWith("* ")) texto = texto.trim().replaceFirst("\\* ", "*:* ");

    if (texto.trim().isEmpty()) return new MatchAllDocsQuery();
    else {
  ...
  private HashMap<String, PointsConfig> getPointsConfigMap() {
    ...
    for (String field : LoadIndexFields.getFields(Arrays.asList(ipedCase))) {
      ...
      else if (type.equals(Long.class)) pointsConfigMap.put(field, configLong);
      else if (type.equals(Float.class)) pointsConfigMap.put(field, configFloat);
      else if (type.equals(Double.class)) pointsConfigMap.put(field, configDouble);
    }
