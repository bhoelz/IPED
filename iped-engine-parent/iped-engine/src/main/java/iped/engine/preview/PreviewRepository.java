...
/**
 * Handles storage and retrieval of item previews in an H2 database. Instances of this class are
 * managed by {@link PreviewRepositoryManager}.
 */
@Slf4j
public class PreviewRepository {
  ...
  /**
   * Package-private constructor. Instances should be obtained from {@link
   * PreviewRepositoryManager}.
   *
  ...
  public boolean previewExists(PreviewKey key) throws SQLException {
    try (Connection conn = dataSource.getConnection();
        PreparedStatement pstmt = conn.prepareStatement(CHECK_EXISTS_SQL)) {
  ...
  /**
   * Stores a preview for an item from a stream that is *already compressed* with DEFLATE. This
   * method is suitable for high-performance streaming when the caller produces a compressed stream
   * directly.
   *
   ...
   * @throws IOException
   */
  public void storeCompressedPreview(IItemReader evidence, InputStream compressedValueStream)
  ...
  /**
   * Stores a preview for an item from a *raw, uncompressed* stream. This method compresses the
   * content on-the-fly using DEFLATE in a fully streaming, thread-less, constant-memory way.
   *
   * @param evidence The item to store the preview for.
   ...
   */
  public void storeRawPreview(IItemReader evidence, InputStream rawValueStream)
      throws SQLException, IOException {
    PreviewKey key = PreviewKey.create(evidence);
   ...
   */
  public boolean consumePreview(PreviewKey key, InputStreamConsumer consumer)
      throws SQLException, IOException {
   ...
   * @throws IOException if an I/O error occurs during streaming.
   */
  public boolean consumePreview(IItemReader evidence, InputStreamConsumer consumer)
      throws SQLException, IOException {
   ...
   *
   * @param evidence The item whose preview is to be consumed.
   * @param decompress If true, the stream will be decompressed (DEFLATE); if false, the raw stored
   *     stream is provided.
   ...
   * @throws IOException if an I/O error occurs during streaming.
   */
  public boolean consumePreview(
      IItemReader evidence, boolean decompress, InputStreamConsumer consumer)
      throws SQLException, IOException {
    PreviewKey key = PreviewKey.create(evidence);
   ...
   * @throws IOException if an I/O error occurs during streaming.
   */
  public boolean consumePreview(PreviewKey key, boolean decompress, InputStreamConsumer consumer)
      throws SQLException, IOException {
    try (Connection conn = dataSource.getConnection();
        PreparedStatement pstmt = conn.prepareStatement(SELECT_DATA_SQL)) {
      pstmt.setBytes(1, key.getBytes());
      try (ResultSet rs = pstmt.executeQuery()) {
        if (rs.next()) {
  ...
  /**
   * Consumes a preview for an IItemReader from the repository and converts it to a
   * SeekableInputStream.
   *
   ...
   * @throws IOException
   */
  public SeekableFileInputStream readPreview(IItemReader item, boolean forceFile)
      throws SQLException, IOException {
    PreviewKey key = PreviewKey.create(item);
    return PreviewInputStreamFactory.consumePreviewToSeekableInputStream(
        this, key, item.getPreviewExt(), forceFile);
  }
}
