...
/**
 * This class is not indexed as value of IndexItem.SOURCE_DECODER field. It is used in retrieved
 * item from index, when: - the read item has no file AND the read item has no reference in evidence
...
public class PreviewInputStreamFactory extends SeekableInputStreamFactory {
  ...
  /**
   * Generates a unique string identifier for an item's preview, used by {@link
   * PreviewInputStreamFactory}.
   *
   ...
   * @throws IOException
   */
  static SeekableFileInputStream consumePreviewToSeekableInputStream(
      PreviewRepository repo, PreviewKey key, String ext, boolean forceFile)
  ...
  /**
   * Converts a standard InputStream into a SeekableFileInputStream, using either memory or a
   * temporary file based on a threshold.
   */
  private static SeekableFileInputStream toSeekableInputStream(
      InputStream sourceStream, String ext, boolean forceFile) throws IOException {
