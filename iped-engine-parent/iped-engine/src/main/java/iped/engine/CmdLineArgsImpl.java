 ...
 * </pre>
 *
 * <p>After calling {@link #takeArgs(String[])}, callers are responsible for applying any app-level
 * side effects (setting datasources, output, keywords on the main class).
 *
 * @author Nassif
 */
@Slf4j
public class CmdLineArgsImpl implements CmdLineArgs {
  ...
  @Parameter(
      names = {"-d", "-data"},
      description =
          "input data (can be used multiple times): "
              + "DD, 001, E01, Ex01, VHD, VHDX, VMDK, Physical Drive, ISO, AFF (on Linux), AD1, UFDR, folder "
              + "or *.iped file (with tagged files to export and reindex)",
      validateWith = DatasourceExistsValidator.class,
      order = 0,
  ...
  @Parameter(
      names = "-dname",
      description = "display name (optional) of data added with -d",
      order = 1,
      splitter = NoSplitter.class)
  ...
  @Parameter(
      names = {"-remove"},
      description = "removes the evidence with specified name")
  private String evidenceToRemove;

  @Parameter(
      names = {"-l", "-keywordlist"},
      description =
          "line file with keywords to be imported into case. "
              + "Keywords with no hits are filtered out.",
      validateWith = FileExistsValidator.class)
  private File keywords;

  @Parameter(
      names = "-ocr",
      description = "only run OCR on a specific category or bookmark (can be used multiple times)",
      splitter = NoSplitter.class)
  ...
  @Parameter(
      names = {"-tz", "-timezone"},
      description =
          "original timezone of FAT devices: GMT-3, GMT-4... "
              + "If unspecified, local system timezone is used.")
  private String timezone;

  @Parameter(
      names = {"-b", "-blocksize"},
      description = "sector block size (bytes), must set to 4k sector devices")
  private int blocksize;

  @Parameter(
      names = {"-p", "-password"},
      description = "password for encrypted images/volumes",
      splitter = NoSplitter.class)
  ...
  @Parameter(
      names = "-profile",
      description =
          "use a processing profile: forensic, pedo, "
              + "fastmode, blind, triage. More details in manual.")
  private String profile;

  @Parameter(
      names = "--addowner",
      description = "index file owner info when processing local folders (slow over network)")
  ...
  @Parameter(
      names = "--restart",
      description = "discard last aborted processing and start from beginning")
  ...
  @Parameter(
      names = "--nopstattachs",
      description = "do not export automatically to report PST/OST email attachments")
  private boolean nopstattachs;

  public static final String noLinkedItemsOption = "--nolinkeditems";

  @Parameter(
      names = noLinkedItemsOption,
      description = "do not export automatically to report items linked to chats")
  private boolean noLinkedItems = false;

  @Parameter(
      names = "--portable",
      description =
          "use relative references to forensic images, so case can be moved to other machines if the images are on the same volume")
  private boolean portable;

  @Parameter(
      names = "--downloadInternetData",
      description =
          "download Internet data to enrich evidence data processing. E.g. media files still available in WhatsApp servers and not found in the evidence")
  private boolean downloadInternetData;

  @Parameter(
      names = {"-splash"},
      description = "custom message to be shown in the splash screen")
  ...
  /** Saves parsed arguments into the case data so they can be retrieved by processing modules. */
  ...
  /**
   * Parses the command-line arguments. After this call the getters are populated. Callers are
   * responsible for any app-level side effects (populating datasources, output directory, etc. on
