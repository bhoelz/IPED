...
public class NoScoringCollector extends SimpleCollector {
  ...
  public void collect(int doc) throws IOException {
    if (canceled) throw new InterruptedIOException("Search canceled!"); // $NON-NLS-1$
  ...
  public LuceneSearchResult getSearchResults1() {
    ...
    int idx = 0;
    for (int i = 0; i < bits.length(); i++) if (bits.get(i)) docs[idx++] = i;
  ...
  public LuceneSearchResult getSearchResults() {
    ...
    for (int i = 0; i < array.length; i++)
      for (int j = 0; j < Long.SIZE; j++)
        if ((array[i] & (1L << j)) != 0) docs[idx++] = (i << 6) | j;
