 ...
 *
 * Manages registration of custom metadata properties from plugins,
 * with support for Elasticsearch indexing configuration and type validation.
 *
...
public class MetadataRegistry {
     ...
     */
    private final Map<String, MetadataPropertyDescriptor> properties =
    ...
    public String getElasticsearchMapping() {
        ...
        for (int i = 0; i < indexed.size(); i++) {
            ...
            sb.append("\"").append(prop.name()).append("\":")
              .append(prop.getElasticsearchFieldConfig());
    ...
    public Map<String, Object> getStatistics() {
        ...
        stats.put("faceted_properties", (int) properties.values().stream()
            .filter(MetadataPropertyDescriptor::faceted).count());

        Set<String> components = properties.values().stream()
            .map(MetadataPropertyDescriptor::componentId)
            .collect(Collectors.toSet());
        stats.put("components_with_properties", components.size());

        Set<String> categories = properties.values().stream()
    ...
    public String toString() {
        return "MetadataRegistry{" +
            ...
            ", indexed=" + getIndexedProperties().size() +
            '}';
    }
}
