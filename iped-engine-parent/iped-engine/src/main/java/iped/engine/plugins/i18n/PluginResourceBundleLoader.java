 ...
 *
 * <p>Plugins can provide translations in multiple languages by including resource bundle files in
 * their JAR:
 *
 * <p>plugin-xyz.jar ├── com/xyz/MyComponent.class └── resources/ ├── iped-plugin-xyz.properties
 * (PT-BR, default) ├── iped-plugin-xyz_en_US.properties (English) ├──
 * iped-plugin-xyz_es_AR.properties (Spanish) └── iped-plugin-xyz_de_DE.properties (German)
 *
...
public class PluginResourceBundleLoader {
  ...
  /** Cache of loaded resource bundles. Key format: "component_id|locale" */
  ...
  /**
   * Load resource bundle for a plugin component. Automatically tries multiple locales in order of
   * availability.
   *
  ...
  /**
   * Get a localized message for a key. Uses current locale, falls back to Portuguese (default).
   *
  ...
  /**
   * Set the current application locale. Affects all subsequent getMessage() calls without explicit
   * locale.
   *
