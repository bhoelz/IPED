...
/**
 * Executes one additional-processing task on a user-selected subset of
 * already-indexed items and persists the results to an
 * {@link IAdditionalDataSource}.
 *
 * <h3>Usage</h3>
 * <pre>{@code
 ...
 * <ul>
 *   <li>Items are partitioned into batches, one per thread.</li>
 *   <li>Each thread owns one task instance (initialised once via
 *       {@link AbstractTask#init}).</li>
 *   <li>Raw (non-enriched) items are fetched via
 *       {@link IPEDSource#getRawItemByID} so that additional processing starts
 *       from a clean state.</li>
 *   <li>After all threads finish, {@link IAdditionalDataSource#commit()} is
 *       called once to flush writes.</li>
 * </ul>
 *
 * <h3>Thread safety</h3>
 * Each thread has its own task instance and worker.  The only shared mutable
 * state is the {@link IAdditionalDataSource} (which must be thread-safe for
...
public class AdditionalTaskRunner {
     ...
     */
    public AdditionalTaskRunner(IPEDSource source, IAdditionalDataSource destination, int numThreads) {
        this.source      = source;
     ...
     * @param taskClass        task class; must be annotated with
     *                         {@link AdditionalProcessingCapable} and have a
     *                         public no-arg constructor
     * @param progressCallback called after each item completes (may be
     *                         {@code null})
     * @return a future that completes with the final
     *         {@link AdditionalTaskProgress} when all items are done
     */
    ...
    private AdditionalTaskProgress doRun(
            List<IItemId> items,
            Class<? extends AbstractTask> taskClass,
            Consumer<AdditionalTaskProgress> progressCallback) throws Exception {
        ...
        for (int t = 0; t < batches.size(); t++) {
            ...
            futures.add(pool.submit(() -> {
                processBatch(batch, taskClass, displayName, workerId,
                        processed, errors, total, cancelled, progressCallback);
                return null;
            }));
        }
    ...
    private void processBatch(
        ...
        try {
            ...
            AdditionalIndexTask sinkTask = new AdditionalIndexTask(destination, taskClass.getSimpleName());
    ...
    private static <T> List<List<T>> partition(List<T> list, int n) {
        ...
        for (int i = 0; i < size; i += batchSize) {
            parts.add(Collections.unmodifiableList(
                    list.subList(i, Math.min(i + batchSize, size))));
        }
        return parts;
    }
}
