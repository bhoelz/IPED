...
public class ZIPInputStreamFactory extends SeekableInputStreamFactory implements Closeable {
  ...
  private Map<String, Path> filesCache =
      new LinkedHashMap<String, Path>(128, 0.75f, true) {
  ...
  private synchronized void init() throws IOException {
    if (zip == null) {
      ...
      while (true) {
        File segment =
            new File(file.getParentFile(), namePrefix + ".z" + String.format("%02d", ++num));
        if (segment.exists()) {
      ...
      if (channels.size() == 1) {
        ...
        sbc =
            ZipSplitReadOnlySeekableByteChannel.forOrderedSeekableByteChannels(
  ...
  public SeekableInputStream getSeekableInputStream(String path) throws IOException {
    ...
    Future<Pair<Path, byte[]>> future =
        executor.submit(
            new Callable<Pair<Path, byte[]>>() {
              @Override
              public Pair<Path, byte[]> call() throws Exception {
                ...
                try {
                  ...
                  if (value != zae.getCrc()) {
                    log.error(
                        "CRC32 inconsistency! File: "
                            + zae.getName()
                            + ", Length: "
                            + zae.getSize()
                            + ", Original: "
                            + Long.toHexString(zae.getCrc())
                            + " != Calulated: "
