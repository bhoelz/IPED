...
public class Bookmarks implements IBookmarks {
  ...
  TreeMap<Integer, BookmarkShortcut> bookmarkBookmarkShortcuts =
      new TreeMap<Integer, BookmarkShortcut>();
  ...
  public ArrayList<Integer> getBookmarkIds(int id) {
    ...
    if (bookmarkNames.size() > 0)
      for (int i : bookmarkNames.keySet()) {
        if (hasBookmark(id, i)) bookmarkIds.add(i);
      }
  ...
  public synchronized int newBookmark(String bookmarkName) {
    ...
    if (bookmarkId != -1) return bookmarkId;
    ...
    if (bookmarkId == -1) bookmarkId = bookmarkNames.size();
    ...
    if (bookmarkColors == null) bookmarkColors = new TreeMap<Integer, Color>();
  ...
  public synchronized void renameBookmark(int bookmarkId, String newBookmark) {
    if (bookmarkId != -1) bookmarkNames.put(bookmarkId, newBookmark);
  }

  public int getBookmarkId(String bookmarkName) {
    for (int i : bookmarkNames.keySet()) {
      if (bookmarkNames.get(i).equals(bookmarkName)) return i;
    }
    return -1;
  }
  ...
  public Set<Color> getUsedColors() {
    return bookmarkColors == null
        ? new HashSet<Color>()
        : new HashSet<Color>(bookmarkColors.values());
  }
  ...
  public synchronized void setInReport(int bookmarkId, boolean inReport) {
    if (inReport) reportBookmarks.add(bookmarkId);
    else reportBookmarks.remove(bookmarkId);
  }
  ...
  public SearchResult filterBookmarks(SearchResult result, Set<String> bookmarkNames) {
    ...
    int i = 0;
    for (String bookmarkName : bookmarkNames) bookmarkIds[i++] = getBookmarkId(bookmarkName);
  ...
  public void saveState(boolean synchronous) {
    try {
      if (IOUtil.canWrite(stateFile)
          || (!stateFile.exists() && IOUtil.canCreateFile(stateFile.getParentFile())))
        saveState(stateFile, synchronous);
      else saveState(cookie, synchronous);
  ...
  public void loadState() {
    try {
      if (cookie.exists()
          && (!stateFile.exists() || cookie.lastModified() > stateFile.lastModified()))
        loadState(cookie);
      else if (stateFile.exists()) loadState(stateFile);
