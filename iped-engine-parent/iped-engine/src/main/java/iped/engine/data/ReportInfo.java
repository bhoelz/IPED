...
public class ReportInfo implements Serializable {
  ...
  private static final String EVIDENCE_PREFIX =
      Messages.getString("ReportInfo.EvidencePrefix"); // $NON-NLS-1$ ;
    ...
    BufferedReader in =
        new BufferedReader(
            new InputStreamReader(
                new FileInputStream(asap), Charset.forName("cp1252"))); // $NON-NLS-1$
    String str = null;
  ...
  public String getEvidenceDescHtml() {
    ...
    for (int i = 0; i < evidences.size(); i++) {
      ...
      mat.append(EVIDENCE_PREFIX)
          .append(evidences.get(i).id)
          .append(ID_DESC_DELIMITER)
  ...
  public String getExaminersText() {
    if (examiners.size() == 1) return examiners.get(0);
    String result = "";
    for (String examiner : examiners) result += examiner + "; ";
    return result;
  }
  ...
  public void fillExaminersFromText(String text) {
    ...
    for (String e : es) examiners.add(e.trim());
  }
  ...
  public void readJsonInfoFile(File file) throws IOException {
    ...
    for (int i = 0; i < array.length(); i++) examiners.add(array.getString(i));
  ...
  public static ReportInfo readReportInfoFile(File file)
