...
public class MultiBitmapBookmarks implements Serializable, IMultiBookmarks {
  ...
  private transient SoftReference<RoaringBitmap[]> unionsCache =
      new SoftReference<RoaringBitmap[]>(null);
  ...
  public MultiBitmapBookmarks(List<IPEDSource> cases) {
    for (IIPEDSource s : cases) map.put(s.getSourceId(), s.getBookmarks());
  }
  ...
  public int getTotalChecked() {
    int sum = 0;
    for (IBookmarks m : map.values()) sum += m.getTotalChecked();
    return sum;
  }

  public int getLastId() {
    int lastId = 0;
    for (IBookmarks m : map.values()) lastId = Math.max(lastId, m.getLastId());
    return lastId;
  }

  public void clearChecked() {
    for (IBookmarks m : map.values()) m.clearSelected();
    for (SelectionListener l : selectionListeners) l.clearAll();
  }

  public void checkAll() {
    for (IBookmarks m : map.values()) m.checkAll();
    for (SelectionListener l : selectionListeners) l.selectAll();
  }
  ...
  public void setChecked(boolean value, IItemId item) {
    ...
    for (SelectionListener l : selectionListeners) l.setSelected(item, value);
  }
  ...
  private static final int[] getBookmarkIds(IBookmarks m, Set<String> bookmarkNames) {
    ...
    for (String bookmarkName : bookmarkNames) {
      ...
      if (bookmarkIds[i++] != -1) hasBookmark = true;
    }
  ...
  public void addBookmark(Set<IItemId> ids, String bookmarkName) {
    ...
    for (Integer sourceId : itemsPerSource.keySet()) {
      ...
      int bookmarkId = m.getBookmarkId(bookmarkName);
      if (bookmarkId == -1) bookmarkId = m.newBookmark(bookmarkName);
  ...
  public void removeBookmark(Set<IItemId> ids, String bookmarkName) {
    ...
    for (Integer sourceId : itemsPerSource.keySet()) {
      IBookmarks m = map.get(sourceId);
      int bookmarkId = m.getBookmarkId(bookmarkName);
      if (bookmarkId != -1) m.removeBookmark(itemsPerSource.get(sourceId), bookmarkId);
    }
    invalidadeCaches();
  }

  public void newBookmark(String bookmarkName) {
    for (IBookmarks m : map.values()) m.newBookmark(bookmarkName);
    invalidadeCaches();
  }
  ...
  public void renameBookmark(String oldBookmark, String newBookmark) {
    for (IBookmarks m : map.values()) m.renameBookmark(m.getBookmarkId(oldBookmark), newBookmark);
  }

  public void setBookmarkComment(String bookmarkName, String comment) {
    for (IBookmarks m : map.values()) m.setBookmarkComment(m.getBookmarkId(bookmarkName), comment);
  }
  ...
  public boolean isInReport(String bookmarkName) {
    for (IBookmarks m : map.values()) {
      int bookmarkId = m.getBookmarkId(bookmarkName);
      if (m.isInReport(bookmarkId)) return true;
    }
    return false;
  }
  ...
  public TreeSet<String> getBookmarkSet() {
    ...
    for (IBookmarks m : map.values()) bookmarks.addAll(m.getBookmarkMap().values());
    return bookmarks;
  }
  ...
  public IMultiSearchResult filterBookmarks(IMultiSearchResult result, Set<String> bookmarkNames) {
    ...
    MultiSearchResult r =
        new MultiSearchResult(
            selectedItems.toArray(new ItemId[0]),
            ArrayUtils.toPrimitive(scores.toArray(new Float[0])));

    return r;
  }

  public IMultiSearchResult filterBookmarksOrNoBookmarks(
    ...
    for (Entry<Integer, IBookmarks> e : map.entrySet()) {
      ...
      if (bookmarks instanceof BitmapBookmarks) {
        unions.put(
            e.getKey(),
            ((BitmapBookmarks) bookmarks).getBookmarksOrNoBookmarksUnion(bookmarkNames));
      }
    }
  ...
  public void loadState() {
    for (IBookmarks m : map.values()) m.loadState();
  }
  ...
  public void loadState(File file) throws ClassNotFoundException, IOException {
    ...
    for (IBookmarks marcador : this.map.values()) marcador.updateCookie();
  }
  ...
  public void saveState(boolean sync) {
    for (IBookmarks m : map.values()) m.saveState(sync);
  }
  ...
  public LinkedHashSet<String> getTypedWords() {
    ...
    for (IBookmarks m : map.values())
      for (String s : m.getTypedWords()) if (!searches.contains(s)) searches.add(s);
    return searches;
  }

  public void clearTypedWords() {
    for (IBookmarks m : map.values()) m.getTypedWords().clear();
  }

  public void addToTypedWords(String texto) {
    for (IBookmarks m : map.values()) m.addToTypedWords(texto);
  }
