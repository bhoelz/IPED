...
public class IPEDMultiSource extends IPEDSource {
  ...
  public IPEDMultiSource(List<IIPEDSource> sources) {
    super(null);
    for (IIPEDSource src : sources) this.cases.add((IPEDSource) src);
    init();
  }
  ...
  public IPEDMultiSource(File file) {
    ...
    List<File> files;
    if (file.isDirectory()) files = searchCasesinFolder(file);
    else files = loadCasesFromTxtFile(file);

    ExecutorService executor =
        Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors());
    ArrayList<Future<IPEDSource>> futures = new ArrayList<>();
    for (final File src : files) {
      Callable<IPEDSource> openCase =
          new Callable<IPEDSource>() {
  ...
  private List<File> searchCasesinFolder(File folder) {
    ...
    if (subFiles != null)
      for (File file : subFiles) {
        if (file.isDirectory()) {
          if (checkIfIsCaseFolder(file)) files.add(file);
          else files.addAll(searchCasesinFolder(file));
        }
      }
    return files;
  }
  ...
  public void init() {
    ...
    int i = 0;
    for (IPEDSource iCase : cases) iCase.sourceId = i++;
    ...
    for (IPEDSource iCase : cases) totalItens += iCase.totalItens;

    for (IIPEDSource iCase : cases) baseDocCache.add(getBaseLuceneId(iCase));

    loadCategories();

    for (IPEDSource iCase : cases)
      for (String keyword : iCase.keywords) if (!keywords.contains(keyword)) keywords.add(keyword);
  ...
  private void openIndex() throws IOException {
    ...
    for (IPEDSource iCase : cases) readers[i++] = iCase.reader;
  ...
  public void close() {
    ...
    for (IIPEDSource iCase : cases) iCase.close();
  }

  @Override
  public void checkImagePaths() throws IPEDException, TskCoreException {
    for (IPEDSource iCase : cases) iCase.checkImagePaths();
  }

  public final IIPEDSource getAtomicSource(int luceneId) {
    int maxDoc = 0;
    for (IPEDSource iCase : cases) {
      maxDoc += iCase.reader.maxDoc();
      if (luceneId < maxDoc) return iCase;
    }
    return null;
  }

  public final IPEDSource getAtomicSourceBySourceId(int sourceId) {
    return cases.get(sourceId);
  }

  public final List<IPEDSource> getAtomicSources() {
    return this.cases;
  }
  ...
  public final int getBaseLuceneId(IIPEDSource atomicCase) {
    int maxDoc = 0;
    for (IPEDSource iCase : cases) {
      if (atomicCase == iCase) return maxDoc;
      maxDoc += iCase.reader.maxDoc();
    }
    return maxDoc;
  }

  private LuceneSearchResult rebaseLuceneIds(
      LuceneSearchResult resultsFromAtomicCase, IIPEDSource atomicCase) {
    LuceneSearchResult result = resultsFromAtomicCase.clone();
    int baseDoc = getBaseLuceneId(atomicCase);
    int[] docs = result.getLuceneIds();
    for (int i = 0; i < docs.length; i++) docs[i] += baseDoc;
    return result;
  }

  @Override
  public final IItem getItemByID(int id) {
    throw new RuntimeException(
        "Use getItemByItemId() from " + this.getClass().getSimpleName()); // $NON-NLS-1$
  }

  public final IItem getItemByItemId(IItemId item) {
    return getAtomicSourceBySourceId(item.getSourceId()).getItemByID(item.getId());
  }

  @Override
  public final IItem getItemByLuceneID(int luceneId) {
  ...
  @Override
  public final int getId(int luceneId) {
    throw new RuntimeException(
        "Use getItemId() from " + this.getClass().getSimpleName()); // $NON-NLS-1$
  }

  public final IItemId getItemId(int luceneId) {
  ...
  public final int getLuceneId(int id) {
    throw new RuntimeException(
        "Use getLuceneId(ItemId) from " + this.getClass().getSimpleName()); // $NON-NLS-1$
  }

  public final int getLuceneId(IItemId id) {
  ...
  public int getLastId() {
    throw new RuntimeException(
        "Forbidden call from " + this.getClass().getSimpleName()); // $NON-NLS-1$
  }
