...
public class SimpleFilterNode implements Serializable, Cloneable {
  ...
  @JsonIgnore private SimpleFilterNode parent;

  @JsonIgnore private boolean dynamicChild;

  @JsonIgnore private int numItems = -1;
    ...
    public LocalizedNameComparator() {
      if (resourceBundle == null) {
        synchronized (LocalizedNameComparator.class) {
          if (resourceBundle == null) {
            resourceBundle =
                iped.localization.Messages.getExternalBundle(
                    bundleName, LocaleResolver.getLocale());
          }
        }
      }
      if (collator == null) {
