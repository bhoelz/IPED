...
public class SaveStateThread extends Thread {
  ...
  private final ConcurrentHashMap<UUID, Queue<Map.Entry<IBookmarks, File>>> caseStateMap =
      new ConcurrentHashMap<>();

  private SaveStateThread() {}

  public static synchronized SaveStateThread getInstance() {
    if (instance == null) {
  ...
  public void saveState(UUID caseId, IBookmarks state, File file) {
    Queue<Map.Entry<IBookmarks, File>> caseQueue =
        caseStateMap.computeIfAbsent(caseId, k -> new ConcurrentLinkedQueue<>());
      ...
      try {
        ...
        if (state != null && file != null) {
          ...
          if (!file.exists()) {
            ...
            if (!tmp.renameTo(file)) bkp.renameTo(file);
          }
        } else {
          Thread.sleep(200);
        }
  ...
  private File backupAndDelete(File file) {
    ...
    if (numBkps < MAX_BACKUPS) {
      ...
      oldestBkp =
          new File(bkpDir, baseName + "." + numBkps + ".bkp.iped"); // $NON-NLS-1$ //$NON-NLS-2$
    }
    if (newestBkp == null
        || (System.currentTimeMillis() - newestBkp.lastModified()) / 1000 > BKP_INTERVAL) {
      oldestBkp.delete();
