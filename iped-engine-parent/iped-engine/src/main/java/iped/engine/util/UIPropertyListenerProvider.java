...
public class UIPropertyListenerProvider {
  ...
  private final ConcurrentHashMap<UUID, Set<PropertyChangeListener>> caseListeners =
      new ConcurrentHashMap<>();
  private final ConcurrentHashMap<UUID, Set<PropertyChangeListener>> caseUiListeners =
      new ConcurrentHashMap<>();
  ...
  /**
   * Dispatcher used to invoke UI listeners. Defaults to inline (synchronous) execution, suitable
   * for headless / test mode. In GUI mode, iped-app sets this to {@code
   * SwingUtilities::invokeLater} so events are marshalled to the EDT automatically.
   */
  ...
  /**
   * Sets the dispatcher used to invoke UI listeners. Call {@code
   * setUiDispatcher(SwingUtilities::invokeLater)} from iped-app before processing starts when
   * running in GUI mode.
   */
  ...
  public void addPropertyChangeListener(
      UUID caseId, PropertyChangeListener l, boolean isUIListener) {
    if (isUIListener) {
