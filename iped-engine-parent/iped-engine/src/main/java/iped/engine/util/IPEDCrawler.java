...
public class IPEDCrawler {
            ...
            public void run() {
              ...
              try (IPEDSource ipedCase = new IPEDSource(file, null, false)) {
                ...
                for (Integer id : itemIds) {
                  ...
                  parentDir.mkdirs();
                  File target =
                      new File(parentDir, Util.getValidFilename(Util.getNameWithTrueExt(item)));
                  String ext = "";
                  int idx, suffix = 0;
                  ...
                  synchronized (exported) {
                    while (target.exists()) {
                      target =
                          new File(
                              parentDir, target.getName().substring(0, idx) + (++suffix) + ext);
                    }
  ...
  private static void recurse(File folder) {
    if (SKIP_KNOWN_FOLDERS) {
      ...
      if (((name.equals("Exportados") || name.equals("Exported"))
              && (new File(folder.getParentFile(), IPEDSource.MODULE_DIR).exists()
                  || new File(folder.getParentFile(), "indexador").exists()))
          || ((name.equals("report") || name.equals("relatorio"))
              && new File(folder, "thumbs").exists())
