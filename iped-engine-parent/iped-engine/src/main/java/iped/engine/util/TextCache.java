...
public class TextCache implements Closeable, Cloneable {
  ...
  public void write(char[] buf, int off, int len) throws IOException {
    ...
    if (sb != null && sb.length() < MAX_MEMORY_CHARS) {
      if (sb.length() + len > MAX_MEMORY_CHARS) len = MAX_MEMORY_CHARS - sb.length();
  ...
  public Reader getTextReader() throws IOException {
    ...
    Reader reader = null;
    if (sb != null) reader = new StringReader(sb.toString());
        ...
        if (offset == -1) {
          ...
          reader =
              new InputStreamReader(
                  new RandomAccessFileInputStream(raf, true), StandardCharsets.UTF_8);
          reader = new BufferedReader(new BoundedReader(reader, (int) size));
        }

      } catch (FileSystemException | FileNotFoundException e) {
        log.error(
            "Error reading extracted text file{}, maybe your antivirus blocked or deleted it? {}",
            sourceItem != null ? " from " + sourceItem.getPath() : "",
            e.toString());
        e.printStackTrace();
  ...
  public void setTextBounds(long offset, int size) {
    ...
    if (offset + size > this.size) {
      throw new IllegalArgumentException(
          "offset + size must be less than or equal to original text size.");
    }
    this.offset = offset;
    this.size = size;
  }
  ...
  public void close() throws IOException {
    ...
    if (tmp != null && refCount.decrementAndGet() == 0) tmp.delete();
  }
