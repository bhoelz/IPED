...
public class Util {
  ...
  public static String getJavaVersionWarn() {
    ...
    if (dotIdx > -1) versionStr = versionStr.substring(0, dotIdx);
    Integer version = Integer.valueOf(versionStr);

    if (version < MIN_JAVA_VER) {
      return Messages.getString("JavaVersion.Error")
          .replace("{}", MIN_JAVA_VER.toString()); // $NON-NLS-1$
    }
    if (version > MAX_JAVA_VER) {
      return Messages.getString("JavaVersion.Warn")
          .replace("{}", version.toString()); // $NON-NLS-1$
    }
    for (String ver : buggedVersions) {
      if (System.getProperty("java.version").equals(ver))
        return Messages.getString("JavaVersion.Bug")
            .replace("{1}", ver)
            .replace(
                "{2}", //$NON-NLS-1$
  ...
  /**
   * Computes trackID and reassign the item ID if it was mapped to a different ID in a previous
   * processing, being resumed or restarted.
   *
   * @param item
   */
  public static void calctrackIDAndUpdateID(CaseData caseData, IItem item) {
    ...
    Map<HashValue, Integer> globalToIdMap =
        (Map<HashValue, Integer>) caseData.getCaseObject(SkipCommitDataKeys.TRACK_ID_ID_MAP);
  ...
  public static String getParentPath(IItem item) {
    ...
    if (end <= 0) return "";
    if (path.charAt(end) == '>' && path.charAt(end - 1) == '>') end--;
  ...
  /**
   * Path-based overload: unlike {@link #readUTF8Content(File)}, this works for any {@link Path}
   * implementation, including a {@code ZipPath} pointing at a resource inside a jar (e.g. a
   * module's classpath-default config). Calling {@code path.toFile()} on such a path throws {@link
  ...
  public static String getRelativePath(File baseFile, File file) {
    ...
    if (!base.getRoot().equals(path.getRoot())) return file.getAbsolutePath();
  ...
  public static void changeEncoding(File file) throws IOException {
    if (file.isDirectory()) {
      ...
      Reader reader =
          new BufferedReader(
              new InputStreamReader(new FileInputStream(file), "windows-1252")); // $NON-NLS-1$
  ...
  public static ArrayList<String> loadKeywords(String filePath, String encoding)
      throws IOException {
  ...
  public static void saveKeywords(ArrayList<String> keywords, String filePath, String encoding)
      throws IOException {
    File file = new File(filePath);
    file.delete();
    file.createNewFile();
  ...
  public static TreeSet<String> loadKeywordSet(String filePath, String encoding)
      throws IOException {
  ...
  public static void saveKeywordSet(TreeSet<String> keywords, String filePath, String encoding)
      throws IOException {
    File file = new File(filePath);
    file.delete();
    file.createNewFile();
  ...
  public static InputStream getPOIFSInputStream(TikaInputStream tin) throws IOException {
    ...
    if (!MediaType.OCTET_STREAM.equals(mime)
        && tin.getOpenContainer() != null
  ...
  private static long getMemorySize(String key) {
    try {
      ...
      Object attribute =
          mBeanServer.getAttribute(new ObjectName("java.lang", "type", "OperatingSystem"), key);
