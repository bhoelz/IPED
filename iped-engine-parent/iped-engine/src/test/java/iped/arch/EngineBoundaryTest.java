 ...
 *
 * <ul>
 *   <li>Engine orchestration code must not import UI/app types.
 *   <li>Engine must not import task or parser implementation types (only their SPI/API contracts).
 *   <li>Engine must not contain Kafka or distributed-infrastructure types; that coupling belongs in
 *       iped-distributed.
 * </ul>
 */
class EngineBoundaryTest {

  private static final JavaClasses ENGINE_CLASSES =
  ...
  void engineMustNotImportSwingOrAwtUiClasses() {
    ArchRule rule =
        noClasses()
            ...
            .because(
                "engine orchestration and indexing layers must be headless; "
  ...
  void engineMustNotImportKafkaTypes() {
    ArchRule rule =
        noClasses()
            ...
            .should()
            .dependOnClassesThat()
            .resideInAnyPackage("org.apache.kafka..", "iped.distributed.kafka..")
            .because(
                "Kafka coupling must live in iped-distributed, not the engine; "
  ...
  void engineCoreMustNotImportAppPackage() {
    ArchRule rule =
        noClasses()
            ...
            .because(
                "iped-engine must not depend on the application layer (iped-app); "
