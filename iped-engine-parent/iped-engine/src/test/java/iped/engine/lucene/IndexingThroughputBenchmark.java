...
/**
 * JMH skeleton benchmark for Lucene indexing throughput (the "items/sec" baseline tracked by the
 * iped-engine roadmap's Phase 4 performance-baseline item). Indexes synthetic documents shaped like
 * a minimal evidence item through the real {@link AppAnalyzer} analyzer chain — exercises the same
 * per-field analysis (tokenization, lowercasing, ASCII folding) every real item goes through,
 * without needing a full case/evidence-set fixture.
 *
 * <p>Run via Maven: mvn -pl iped-engine-parent/iped-engine test
 * -Dtest=IndexingThroughputBenchmark#main
 */
...
public class IndexingThroughputBenchmark {
  ...
  public static void main(String[] args) throws RunnerException {
    Options opt =
        new OptionsBuilder().include(IndexingThroughputBenchmark.class.getSimpleName()).build();
    new Runner(opt).run();
  }
}
