 ...
 *
 * <p>These tests verify that the API for saving per-case state is available and that the thread can
 * handle multiple case queues.
 */
public class SaveStateThreadTest {
  ...
  void testBackwardsCompatibilityWithGlobalStateMap() {
    ...
    assertTrue(
        true,
        "Original saveState(IBookmarks, File) method still exists for backward compatibility");
  }
