...
public class SchemaAPIControllerTest {
  ...
  public void testListSchemasContainsConfigurationSchemas() {
    ...
    boolean hasAnalysisConfig =
        schemas.stream().anyMatch(s -> "AnalysisConfig".equals(s.get("name")));
    assertTrue(hasAnalysisConfig, "Should contain AnalysisConfig schema");

    boolean hasTaskInstaller =
        schemas.stream().anyMatch(s -> "TaskInstallerConfig".equals(s.get("name")));
  ...
  public void testListSchemasContainsCLISchemas() {
    ...
    boolean hasCLI = schemas.stream().anyMatch(s -> s.get("name").toString().contains("CLI"));
  ...
  public void testValidateConfigurationWithInvalidJson() {
    ...
    assertTrue(
        ...
        "Response should be successful (error reported in validation result)");
  }
  ...
  public void testListCLISchemasContainsExpectedApps() {
    ...
    boolean hasProcessing =
        cliSchemas.stream().anyMatch(s -> "IPEDProcessingCLI".equals(s.get("name")));
    assertTrue(hasProcessing, "Should contain IPEDProcessingCLI");

    boolean hasWebAPI = cliSchemas.stream().anyMatch(s -> "IPEDWebAPICLI".equals(s.get("name")));
    assertTrue(hasWebAPI, "Should contain IPEDWebAPICLI");

    boolean hasSearchApp =
        cliSchemas.stream().anyMatch(s -> "IPEDSearchAppCLI".equals(s.get("name")));
  ...
  public void testGetSchemasByCategory() {
    ...
    boolean allAreConfiguration =
        configSchemas.stream().allMatch(s -> "configuration".equals(s.get("category")));
  ...
  public void testGetSchemasByCategoryCLI() {
    ...
    boolean allAreCLI = cliSchemas.stream().allMatch(s -> "cli".equals(s.get("category")));
  ...
  public void testSchemaMetadataURLsAreFormatted() {
    ...
    assertTrue(
        jsonUrl.startsWith("/api/schemas/"), "JSON schema URL should start with /api/schemas/");
    assertTrue(uiUrl.startsWith("/api/schemas/"), "UI schema URL should start with /api/schemas/");
    assertTrue(
        validateUrl.startsWith("/api/schemas/"), "Validation URL should start with /api/schemas/");
  }
}
