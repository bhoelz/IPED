...
class RegistrySchemaValidatorTest {
  ...
  void shouldFailOnDuplicatePluginIds() {
    ...
    RegistryIndex index =
        new RegistryIndex(
            ...
            new RegistryIndex.SignatureInline("none", "k1", "ignored"),
            List.of(p1, p2));
  ...
  private static RegistryIndex sampleIndex(String latestVersion) {
    ...
    RegistryIndex.Plugin plugin =
        samplePlugin("iped.tasks.example", latestVersion, List.of(version));
  ...
  private static RegistryIndex.Plugin samplePlugin(
      String id, String latestVersion, List<RegistryIndex.PluginVersion> versions) {
    return new RegistryIndex.Plugin(
        id,
        "name",
        "desc",
        "owner",
  ...
  private static RegistryIndex.PluginVersion sampleVersion(String version) {
    return new RegistryIndex.PluginVersion(
        version,
        "2026-05-14T12:00:00Z",
        new RegistryIndex.Compatibility("[4.4.0,5.0.0)", "[1.0.0,2.0.0)", "[25,26)"),
        new RegistryIndex.Artifact(
            "https://example.org/artifact.jar",
            10,
            "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"),
        null,
        new RegistryIndex.Provider("task.id", "a.b.Provider"),
        List.of(),
        new RegistryIndex.Security(List.of("network:outbound"), null),
        false);
  }
}
