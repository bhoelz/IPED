...
public class UfedModelHandlerChatTest {
  ...
  public static void setUp() throws Exception {
    ...
    UfedModelListener listener =
        new UfedModelHandler.UfedModelListener() {
          @Override
    ...
    InputStream xmlFile =
        UfedModelHandlerChatTest.class.getResourceAsStream("/ufed-model-chat.xml");
  ...
  public void testRootChatObjectIsParsed() {
    ...
    assertEquals(
        "Chat ID should be parsed correctly.",
        "5c1b7590-ec7d-43ec-8ae0-f1ff3967d30d",
        parsedChat.getId());
    assertEquals(
        "Deleted state attribute should be parsed.", "Intact", parsedChat.getDeletedState());
    assertEquals(
        "Decoding confidence attribute should be parsed.",
        "High",
        parsedChat.getDecodingConfidence());
    ...
    assertEquals(
        "StartTime field should be parsed as Instant.",
        DateUtil.tryToParseDate("2024-09-28T14:59:35.230Z"),
        parsedChat.getStartTime());
  }
  ...
  public void testParticipantsAreParsed() {
    ...
    Party clefable =
        parsedChat.getParticipants().stream()
    ...
    assertEquals(
        "Identifier should be correct.", "5511999998888@s.whatsapp.net", clefable.getIdentifier());
  ...
  public void testMessageWithAttachment() {
    Optional<InstantMessage> msgWithAttachment =
        parsedChat.getMessages().stream()
    ...
    assertFalse(
        "Attachments list should not be empty.",
        msgWithAttachment.get().getAttachments().isEmpty());

    Attachment attachment = msgWithAttachment.get().getAttachments().get(0);
    assertEquals(
        "Attachment file_id should be parsed.",
        "d70bf66b-49b6-4185-b2cd-1db458e19267",
        attachment.getFileId());
    assertEquals(
        "Attachment filename should be parsed.",
        "PTT-20241103-WA0101.opus",
        attachment.getFilename());
    assertEquals(
        "Attachment extracted path should be parsed.",
        "files/Audio/PTT-20241103-WA0101.opus",
        attachment.getAttachmentExtractedPath());
  }

  @Test
  public void testMessageWithSharedContact() {
    Optional<InstantMessage> msgWithContact =
        parsedChat.getMessages().stream()
    ...
    assertFalse(
        "Shared contacts list should not be empty.",
        msgWithContact.get().getSharedContacts().isEmpty());
    ...
    Optional<ContactEntry> userIdEntry =
        contact.getUserID().flatMap(list -> list.stream().findFirst());
    assertTrue(userIdEntry.isPresent());
    assertEquals("5511999997745@s.whatsapp.net", userIdEntry.get().getValue());

    Optional<ContactEntry> phoneEntry =
        contact.getPhoneNumber().flatMap(list -> list.stream().findFirst());
  ...
  public void testMessageWithJumpTargets() {
    Optional<InstantMessage> msgWithJumpTarget =
        parsedChat.getMessages().stream()
    ...
    assertFalse(
        "Jump targets list should not be empty.",
        msgWithJumpTarget.get().getJumpTargets().isEmpty());
  ...
  public void testMessageWithChatActivity() {
    Optional<InstantMessage> msgWithActivity =
        parsedChat.getMessages().stream()
  ...
  public void testMessageWithReplyMessageData() {
    Optional<InstantMessage> msgWithReply =
        parsedChat.getMessages().stream()
