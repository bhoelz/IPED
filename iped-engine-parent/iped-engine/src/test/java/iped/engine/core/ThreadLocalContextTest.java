...
/**
 * Tests verifying ThreadLocal isolation of CaseContext across threads and correct set/clear
 * lifecycle.
 */
public class ThreadLocalContextTest {
  ...
  void testIsolationAcrossThreads() throws InterruptedException {
    ...
    assertEquals(case1Id, thread1ContextId.get(), "Thread 1 should have case1 context");
    assertEquals(case2Id, thread2ContextId.get(), "Thread 2 should have case2 context");
  }
  ...
  void testContextIsolationNoLeakBetweenThreads() throws InterruptedException {
    ...
    assertNull(
        thread2InitialContext.get(), "Thread 2 should not have access to Thread 1's context");
  }
  ...
  @Disabled(
      "Manager constructor requires ConfigurationManager to be initialized — integration test only")
  @Test
  void testManagerInstanceDelegation() {
    ...
    assertEquals(
        manager, retrievedManager, "Manager.getInstance() should return ThreadLocal manager");
