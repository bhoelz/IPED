...
/**
 * Integration tests verifying multi-case processing with independent statistics, configurations,
 * and item counter sequences.
 */
public class MultiCaseProcessingTest {
  ...
  void testTwoCasesRunInParallel() throws InterruptedException {
    ...
    assertTrue(
        orchestrator.getActiveCaseIds().contains(case1Id)
  ...
  void testIndependentStatisticsPerCase() throws InterruptedException {
    ...
    assertNotSame(
        context1.getStatistics(),
        context2.getStatistics(),
  ...
  void testItemIdSequencePerCase() {
    ...
    assertEquals(
        id1_first,
        id2_first,
        "Case 1 and Case 2 should have independent sequences, each starting at 0");
  }
  ...
  void testPauseAndResumeOperations() throws InterruptedException {
    ...
    assertEquals(CaseContext.CaseState.PAUSED, context.getState(), "Case should be paused");

    orchestrator.resumeCase(caseId);
    assertEquals(
        CaseContext.CaseState.RUNNING, context.getState(), "Case should be running after resume");
  }
  ...
  void testResourceBackpressureWhenLimitExceeded() throws InterruptedException {
    ...
    CaseContext context3 =
        new CaseContext.Builder(case3Id)
            .withCaseData(new CaseData())
            .withConfigurationView(new ConfigurationView())
            .build();
    ...
    assertTrue(
        orchestrator.getActiveCaseCount() <= 2, "Should not exceed max concurrent cases limit");
  }
  ...
  void testCompleteCaseProcessingReleaseResources() throws InterruptedException {
    ...
    assertNull(
        orchestrator.getCaseContext(caseId), "Case context should be removed after completion");
  }
