 ...
 *
 * <p>These tests verify the ThreadLocal setup behavior and isolation pattern used by Worker to
 * ensure each worker thread has its own case context.
 */
public class WorkerThreadContextTest {
  ...
  void testMainThreadContextNotAffectedByWorkerThread() throws InterruptedException {
    ...
    CaseContext context1 =
        new CaseContext.Builder(mainCaseId)
            .withManager(null)
            .withCaseData(new CaseData())
            .withConfigurationView(new ConfigurationView())
            .build();
    ...
    CaseContext context2 =
        new CaseContext.Builder(workerCaseId)
            .withManager(null)
            .withCaseData(new CaseData())
            .withConfigurationView(new ConfigurationView())
            .build();
    ...
    assertEquals(
        mainCaseId,
        CaseContextThreadLocal.get().getId(),
