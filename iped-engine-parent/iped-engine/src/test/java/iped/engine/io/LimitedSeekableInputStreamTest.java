...
class LimitedSeekableInputStreamTest {
    ...
    void read_whenWithinLimit_thenReadsCorrectly() throws IOException {
        byte[] data = {1, 2, 3, 4, 5};
        LimitedSeekableInputStream lis = new LimitedSeekableInputStream(new ArraySeekableStream(data), 0, 3);
    ...
    void read_withStart_thenReadsFromOffset() throws IOException {
        ...
        LimitedSeekableInputStream lis = new LimitedSeekableInputStream(new ArraySeekableStream(data), 2, 2);
    ...
    void size_reflectsLimit() throws IOException {
        byte[] data = {1, 2, 3, 4, 5};
        LimitedSeekableInputStream lis = new LimitedSeekableInputStream(new ArraySeekableStream(data), 1, 3);
    ...
    void position_tracksProgress() throws IOException {
        byte[] data = {1, 2, 3, 4, 5};
        LimitedSeekableInputStream lis = new LimitedSeekableInputStream(new ArraySeekableStream(data), 0, 5);
