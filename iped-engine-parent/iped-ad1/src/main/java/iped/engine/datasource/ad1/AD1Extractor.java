...
public class AD1Extractor implements Closeable {
  ...
  public FileHeader readObject(long PC, FileHeader parent) throws IOException {
    ...
    if (header.getFileSize() != 0) {
      ...
      readBytesFromRelativeFilePos(
          vector_variavel, PC, (header.objectChunkSize * 8) + pedacos_tam_adicionais);
    ...
    if (header.objectChunkSize == 1) {
      ...
      for (int i = 0; i < header.objectChunkSize; i++) {
        ...
        if (i != header.objectChunkSize - 1) {
          ...
          header.addChunk(
              readLongFromBufLE(vector_variavel, ((i + 1) * 8), 8),
              readLongFromBufLE(vector_variavel, ((i + 2) * 8), 8));

        } else {

          header.addChunk(
              readLongFromBufLE(vector_variavel, ((i + 1) * 8), 8), header.object_PC_partial_end);
        }
      }
    }
  ...
  private int seekAndRead(int ad1Ord, long seekOff, byte[] buf, int off, int len)
      throws IOException {
    ...
    try {
      ...
      if (bbList == null) {
        File newAd1 =
            new File(
                file.getAbsolutePath().substring(0, file.getAbsolutePath().lastIndexOf(".") + 3)
                    + ad1Ord);
