...
public class SleuthkitClientInputStream extends SeekableInputStream {
  ...
  private byte waitServerResponse() throws IOException {
    ...
    try {
      ...
      if (b == -1) throw new IOException(getServerId() + " pipe closed!"); // $NON-NLS-1$
    ...
    byte cmd;
    long time = 0;
    while (FLAGS.isClientCmd(cmd = SleuthkitServer.getByte(client.mbb, 0))
      ...
      try {
        ...
        if (System.currentTimeMillis() - time >= TIMEOUT) {
          ...
          throw new IOException(
              "MemoryReadTimeout waiting " + getServerId() + ": " + path); // $NON-NLS-1$
        }
    ...
    if (cmd == FLAGS.EXCEPTION) {
      ...
      try {
        throw new IOException(
            getServerId() + " error: " + new String(b, "UTF-8")); // $NON-NLS-1$ //$NON-NLS-2$
