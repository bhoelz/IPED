...
public class TouchSleuthkitImages {
  ...
  private static final byte[] ewfSignature =
      new byte[] {0x45, 0x56, 0x46, 0x09, 0x0D, 0x0A, (byte) 0xFF, 0x00};

  private static final Set<Long> preOpenedEvidenceIDs = new HashSet<Long>();

  public static void preOpenImagesOnSleuth(
      SleuthkitCase sleuthCase, boolean cacheWarmUpEnabled, int maxThreads) {
    if (sleuthCase == null) return;
    log.info("Pre-opening Images on Sleuthkit"); // $NON-NLS-1$
    try {
      ...
      if (cacheWarmUpEnabled) {
        ...
        if (!paths.isEmpty()) {
                        ...
                        while (true) {
                          ...
                          try {
                            ...
                            if (isEwf) {
                              ...
                              log.debug(
                                  "Cache warm up for file "
                                      + img.getAbsolutePath() // $NON-NLS-1$
                                      + ", sections = "
                                      + sections
                                      + ", elapsed ms = "
                                      + tWarmUp); //$NON-NLS-1$ //$NON-NLS-2$
                              synchronized (paths) {
                        ...
                        while (!contents.isEmpty()) {
                          for (int i = 0; i < contents.size(); i++) {
                            ...
                            log.info(
                                "Evidence "
                                    + c.getName() // $NON-NLS-1$
                                    + " opened on Sleuth, elapsed time (ms) = "
                                    + tSleuthInit); //$NON-NLS-1$ //$NON-NLS-3$
                          }
                          Thread.sleep(100);
                        }
                      } catch (Exception e) {
          ...
          log.info(
              "Pre-open images on Sleuth: cache warm up = enabled, total time (ms) = " //$NON-NLS-1$
                  + tTotalOpenOnSleuth
                  + ", evidences = "
                  + imgPaths.size()
                  + ", segments = "
                  + totSegments //$NON-NLS-1$ //$NON-NLS-2$
                  + ", sections = "
                  + totSections.get()); // $NON-NLS-1$
        }
      } else {
        long tTotalOpenOnSleuth = System.currentTimeMillis();
        for (Content c : contents) {
          ...
          log.info(
              "Evidence "
                  + c.getName()
                  + " opened on Sleuth, elapsed time (ms) = "
                  + tSleuthInit); //$NON-NLS-1$ //$NON-NLS-2$
          // //$NON-NLS-3$
          synchronized (preOpenedEvidenceIDs) {
            preOpenedEvidenceIDs.add(id);
          }
        ...
        tTotalOpenOnSleuth = System.currentTimeMillis() - tTotalOpenOnSleuth;
        log.info(
            "Pre-open images on Sleuth: cache warm up = disabled, total time (ms) = " //$NON-NLS-1$
                + tTotalOpenOnSleuth
                + ", evidences = "
                + imgPaths.size()); // $NON-NLS-1$
      }
    } catch (Exception e) {
      e.printStackTrace();
    }
