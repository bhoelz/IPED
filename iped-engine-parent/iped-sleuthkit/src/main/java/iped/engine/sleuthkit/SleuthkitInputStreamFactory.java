...
public class SleuthkitInputStreamFactory extends SeekableInputStreamFactory {
  ...
  /**
   * Constructor used for optimization purposes. If you use this, this factory will be specific for
   * the item content passed and can't be reused for other items.
   *
   * @param sleuthkitCase
   * @param content
   */
  ...
  public static File getWriteableDBFile(File sleuthkitDB) throws IOException {
    if (!IOUtil.canWrite(sleuthkitDB) || !IOUtil.canCreateFile(sleuthkitDB.getParentFile())) {
      File tmpCaseFile =
          new File(
              System.getProperty("java.io.basetmpdir"), // $NON-NLS-1$
  ...
  public SeekableInputStream getSeekableInputStream(String identifier) throws IOException {
    ...
    if (!fsConfig.isRobustImageReading()) {
      ...
      try {
        SleuthkitClient.initSleuthkitServers(
            new File(getSleuthkitCase().getDbDirPath(), getSleuthkitCase().getDatabaseName()));
