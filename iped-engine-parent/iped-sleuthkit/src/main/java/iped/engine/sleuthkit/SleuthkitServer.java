...
public class SleuthkitServer {
  ...
  static class FLAGS {
    ...
    static final boolean isClientCmd(int cmd) {
      return cmd != FLAGS.DONE
          && cmd != FLAGS.ERROR
          && cmd != FLAGS.EOF
          && cmd != FLAGS.EXCEPTION
  ...
  private static SleuthkitInputStream getSis(
      MappedByteBuffer out, SleuthkitCase sleuthCase, HashMap<Long, SleuthkitInputStream> sisMap)
      throws Exception {
