...
public class SleuthkitReader extends DataSourceReader {
  ...
  public static boolean isPhysicalDrive(File file) {
    return DataSourceUtils.isPhysicalDrive(file);
  }
  ...
  private static synchronized void checkTSKVersion() throws Exception {
    ...
    if (tskVer.contains(patchSufix)) { // $NON-NLS-1$
      ...
      log.error(
          "We recommend to apply the iped patch on sleuthkit, see https://github.com/sepinf-inc/IPED/wiki/Linux#the-sleuthkit"); //$NON-NLS-1$
    }
    ...
    if (majorVerExpected != majorVerFound || minorVerFound < minorVerExpected)
      throw new Exception(
          "Sleuthkit version "
              + tskVer
              + " not supported. Install version "
              + MIN_TSK_VER_TESTED); //$NON-NLS-1$ //$NON-NLS-2$
    if (minorVerFound > maxMinorVerTested)
      log.error(
          "Sleuthkit version "
              + tskVer
              + " not tested! It may contain incompatibilities!"); //$NON-NLS-1$ //$NON-NLS-2$
  ...
  @Override
  public void read(File image, IItem parent) throws Exception {
    ...
    if (parent == null) {
      ...
      if (embeddedDisk) {
        ...
        inheritedPath = DataSourceUtils.getParentPath(parent);
        idTotrackIDMap.put(parent.getId(), (String) parent.getExtraAttribute(BasicProps.TRACK_ID));
      }
    }
      ...
      if (range != null && args.isContinue()) {
        ...
        DatasourceRegistry.get()
            .onProgress(
                "mensagem",
                "", //$NON-NLS-1$ //$NON-NLS-2$
                Messages.getString("SleuthkitReader.WaitDecode")
                    + image.getAbsolutePath()); // $NON-NLS-1$
    ...
    java.util.logging.Logger.getLogger("org.sleuthkit")
        .setLevel(java.util.logging.Level.SEVERE); // $NON-NLS-1$
    ...
    if (!(listOnly && fastmode) || embeddedDisk) {
      try {
        ...
        if (addImageFuture.get(image) != null) addImageFuture.get(image).cancel(true);
        throw e;
      }

    } else if (addImageFuture.get(image) != null) addImageFuture.get(image).get();
  }

  /**
   * Deleting previous incomplete TSK entries is needed when resuming processing, so items will get
   * the same tskID, this is one of the requirements to recognize the same item between processings.
   */
  ...
  /**
   * Deletes basic info about an added image from TSK DB. Not all references are deleted for now.
   */
  ...
  private static synchronized void setImagePassword(
      File output, String imageName, String password) {
  ...
  private static void setEnvVar(String envVar, String value) {
    DatasourceRegistry.get().setEnvVar(envVar, value);
  ...
  private static synchronized void saveDecodedRangeId(
      File image, Long start, Long last, File output) {
    File file = new File(output, RANGE_ID_FILE);
    UTF8Properties props = new UTF8Properties();
    try {
      if (file.exists()) {
  ...
  private void addImageBlocking(AddImageProcess addImage, File image, int sectorSize)
    ...
    try {
      addImage.run(
          UUID.randomUUID().toString(), new String[] {image.getAbsolutePath()}, sectorSize);
      ...
      for (String error : e.toString().split("\\n")) {
        error = error.trim();
        if (error.isEmpty() || error.contains(ignore)) continue;
  ...
  private void addItems(long start, long last) throws Exception {
    ...
    if (!listOnly) {
      ...
      for (int k = 0; k < ids.length; k++) ids[k] = start + k;
  ...
  private List<AbstractFile> findFilesWhere(String where)
  ...
  private void handleTskCoreException(TskCoreException e, long start)
      throws TskCoreException, InterruptedException {
    if (e.getCause() instanceof SQLException) {
      ...
      log.warn(
          "SQLite error "
              ...
              + " after "
              + (now - start)
              + "s reading sleuth.db, trying again...");
      if (now - start > 3600)
  ...
  private Integer getTskParentId(long id) throws TskCoreException, SQLException {
    ...
    if (parent == null) throw new IPEDException("No parent found for tsk objectId " + id);
    return parent;
  }
  ...
  private void cacheTskParentIds(long[] ids) throws TskCoreException, SQLException {
    StringBuilder query =
        new StringBuilder(
            "SELECT obj_id, par_obj_id  FROM tsk_objects WHERE obj_id in ("); //$NON-NLS-1$
    ...
    try {
      while (rs.next()) {
        ...
        while (objId >= tskParentIds.size()) tskParentIds.add(null);
  ...
  private IItem addContentAndGetItem(Content content) throws Exception {
    ...
    SplitLargeBinaryConfig splitConfig =
        ConfigurationManager.get().findObject(SplitLargeBinaryConfig.class);

    if (absFile != null
        && (absFile.getType() == TSK_DB_FILES_TYPE_ENUM.UNALLOC_BLOCKS
      ...
      for (long offset = 0; offset < absFile.getSize(); offset += fragSize) {
        ...
        IItem frag = DatasourceRegistry.get().createItem();
        ...
        frag.setLength(len);
        if (len >= splitConfig.getMinItemSizeToFragment()) frag.setHash(""); // $NON-NLS-1$
    ...
    if (absFile == null
        || absFile.getName().startsWith("$BadClus:$Bad")
        || absFile.getName().equals("$UPCASE_TABLE")) {
      return null;
    }

    if (!fsConfig.isToAddFileSlacks()
        && absFile.getType() == TSK_DB_FILES_TYPE_ENUM.SLACK
  ...
  private boolean isVolumeShadowCopy(AbstractFile absFile) {
    return absFile
        .getName()
        .toLowerCase()
        .contains("{3808876b-c176-4e48-b7ae-04046e6cc752}"); // $NON-NLS-1$
  }
  ...
  private void setPath(IItem evidence, String path) {
    if (deviceName != null) {
      path =
          path.replaceFirst(
              "img_.+?\\/",
              Matcher.quoteReplacement(deviceName) + "/"); // $NON-NLS-1$ //$NON-NLS-2$
    }
  ...
  private IItem addItem(AbstractFile absFile, IItem evidence, boolean unalloc) throws Exception {
    ...
    if (absFile.isDir()
        && (absFile.getName().equals(".")
            || absFile.getName().equals(".."))) { // $NON-NLS-1$ //$NON-NLS-2$
      return null;
    }
    ...
    if (absFile.getUniquePath().contains("/$OrphanFiles/")) { // $NON-NLS-1$
      if (absFile instanceof FsContent) {
        ...
        if (absFile.getSize() > fs.getSize()) return null;
        // ignore orphans greater than its partition
        if (fs.getParent() != null && fs.getParent().getSize() < absFile.getSize()) return null;
      }

      FileSystemConfig fsConfig = ConfigurationManager.get().findObject(FileSystemConfig.class);
      if (fsConfig.getMinOrphanSizeToIgnore() != -1
          && absFile.getSize() >= fsConfig.getMinOrphanSizeToIgnore()) return null;
    }

    if (evidence == null) {
      evidence = DatasourceRegistry.get().createItem();
    ...
    if (unalloc
        || absFile.isDirNameFlagSet(TSK_FS_NAME_FLAG_ENUM.UNALLOC)
    ...
    if (isFATFile(absFile)) evidence.setExtraAttribute(IN_FAT_FS, true);
    ...
    if (time != 0) evidence.setChangeDate(new Date(time * 1000));
  ...
  private IItem addEvidenceFile(Content content) throws Exception {
    ...
    IItem evidence = DatasourceRegistry.get().createItem();
    ...
    if (content.getName().isEmpty()) {
      if (content instanceof VolumeSystem) {
        evidence.setName(
            ((VolumeSystem) content).getType().getName() + "_Partition_Table"); // $NON-NLS-1$
  ...
  private void setSubitemProperties(IItem item) {
    item.setSubItem(true);
    item.setSubitemId(itemCount);
    item.setExtraAttribute(
        BasicProps.CONTAINER_TRACK_ID, DatasourceRegistry.get().getTrackID(parent));
  }

  private void addToProcessingQueue(ICaseData caseData, IItem item) throws InterruptedException {
    ...
    if (!item.isRoot()) {
      ...
      if (parenttrackID != null) {
        item.setExtraAttribute(BasicProps.PARENT_TRACK_ID, parenttrackID);
      } else {
        throw new RuntimeException(
            BasicProps.PARENT_TRACK_ID + " cannot be null: " + item.getPath());
      }
    }

    if (embeddedDisk) {
      // add embedded disk subitems to queue head to process them first
      DatasourceRegistry.get().addItemFirst(item);
    } else {
    ...
    if (content instanceof FsContent) {
      try {
        ...
        if (fsType == TSK_FS_TYPE_ENUM.TSK_FS_TYPE_FAT12
            ...
            || fsType == TSK_FS_TYPE_ENUM.TSK_FS_TYPE_FAT32) return true;
