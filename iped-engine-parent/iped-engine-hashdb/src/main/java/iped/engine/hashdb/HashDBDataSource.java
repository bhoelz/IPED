...
public class HashDBDataSource {
  ...
  public synchronized ArrayList<PhotoDnaItem> readPhotoDNA(Set<String> statusFilter)
      throws Exception {
  ...
  public synchronized LedHashDB readLedHashDB() throws Exception {
    ...
    for (int propId : propertyIdToName.keySet()) {
      ...
      if (propName.equalsIgnoreCase(propertyStatus)
          || propName.equalsIgnoreCase(ledMd5_512)
  ...
  public synchronized void lookup(byte[][] hashes, Map<String, String> properties)
      throws Exception {
    int mask = 0;
