...
public class HashDBTool {
  ...
  private static final String caidDataModelValue =
      "http://github.com/ICMEC/ProjectVic/DataModels/1.2.xml#Media";
  ...
  private static final String vicDataModelValue13 =
      "http://github.com/ICMEC/ProjectVic/DataModels/1.3.xml#Media";

  private static final String vicDataModelKey20 = "@odata.context";
  private static final String vicDataModelValue20 =
      "http://github.com/VICSDATAMODEL/ProjectVic/DataModels/2.0.xml/Default/$metadata#Media";
  ...
  private static final String icseHeaderColumns =
      "MD5;SHA1;PhotoDNA;Image ID;Is distributed;Victim identified;Offender identified;Series name;Number of submissions;File Available;Media Type;Baseline";
  private static final String icseHeaderRenamedColumns =
      ";;;ImageID;IsDistributed;VictimIdentified;OffenderIdentified;SeriesName;NumSubmissions;FileAvailable;MediaType;Baseline";
  private static final String icseFlagColumns =
      "IsDistributed;VictimIdentified;OffenderIdentified;FileAvailable;Baseline";
  ...
  private final Map<String, Map<String, String>> mapColValues =
  ...
  private boolean process(byte[][] newHashes, Map<Integer, Set<String>> newProperties)
      throws Exception {
  ...
  private void mergeProperties(
      Map<Integer, Set<String>> newProp,
      Map<Integer, Set<String>> oldProp,
      ProcessMode m,
      Map<Integer, Set<String>> propertiesToAdd,
      Set<Integer> propertiesToRemove) {
  ...
  private boolean createDatabase() {
    ...
    try {
      ...
      sb.delete(0, sb.length());
      sb.append(
          "create table HASHES_PROPERTIES (HASH_ID integer, PROPERTY_ID integer, VALUE text NOT NULL, ");
  ...
  private boolean readFile(File file) {
    ...
    System.out.println(
        "\nReading "
            + (type == FileType.INPUT ? "" : type.toString() + " ")
            + "file "
            + file.getPath()
            + "...");
    ...
    if (type == FileType.NSRL_DB) return readNsrlDb(file);
    ...
    try {
      int setPropertyId =
          type == FileType.NSRL_MAIN || type == FileType.NSRL_MAIN_ZIP
      ...
      if (type == FileType.NSRL_MAIN_ZIP) {
        ...
        if (in == null) {
          System.out.println(
              "ERROR: Invalid NSRL ZIP file "
                  + file.getPath()
                  + ", NSRL text file entry not found.");
          return false;
        }
      } else {
      ...
      if (delimiter != null) builder.setDelimiter(delimiter);
      ...
      List<Map<String, String>> mapValuesPerCol =
          mapColValues.isEmpty() ? null : new ArrayList<Map<String, String>>();
  ...
  private boolean readNsrlDb(File file) {
    ...
    try {
      ...
      int cnt = 0;
      while (rs.next()) {
        if ((cnt++ & 8191) == 0) updatePercentage(cnt / (double) tot);
            ...
            if (token == JsonToken.START_ARRAY) arrayDepth++;
              ...
              if (token == JsonToken.START_OBJECT) {
                ...
                if (value != null) {
                  ...
                  || "IsDistributed".equalsIgnoreCase(name)
                  || "Series".equalsIgnoreCase(name)
                  || "IsPrecategorized".equalsIgnoreCase(name)
                  || "Tags".equalsIgnoreCase(name)) {
                token = jp.nextToken();
                String value = null;
              ...
              } else if ("HashValue".equalsIgnoreCase(name)
                  && "PhotoDNA".equalsIgnoreCase(hashName)) {
                String value = jp.nextTextValue();
                if (value != null) {
                  value = value.trim();
                  if (value.length() == photoDnaBase64Len) {
  ...
  private boolean checkDataModel(String key, String value, FileType type) {
    if (type == FileType.PROJECT_VIC) {
      return (vicDataModelKey13.equalsIgnoreCase(key)
              && vicDataModelValue13.equalsIgnoreCase(value))
          || (vicDataModelKey20.equalsIgnoreCase(key)
              && vicDataModelValue20.equalsIgnoreCase(value));
    }
  ...
  private boolean readNsrlProd(File file) {
    ...
    try {
      ...
      if (colProdCode < 0 || colProdName < 0) {
        in.close();
        throw new RuntimeException(
            "Error: Product code and name must be present in header columns.");
      }
      int cnt = 0;
      int numCols = header.size();
      nsrlProdCodeToName = new HashMap<Integer, String>();
  ...
  private void printTotals() {
    ...
    if (totSkip > 0)
      System.out.println(
          totSkip + " hash" + (totSkip == 1 ? " was" : "es were") + " already in the database.");
    if (totIgn > 0)
      System.out.println(
          totIgn + " zero length hash" + (totIgn == 1 ? " was" : "es were") + " ignored.");
    if (totComb > 0)
      ...
      if (type == FileType.NSRL_MAIN || type == FileType.NSRL_MAIN_ZIP) {
        ...
        if (!prodFile.exists() || !prodFile.isFile()) {
          System.out.println(
              "ERROR: File "
                  + prodFile.getName()
                  + " must be present in the same folder of NSRL main file ("
                  + file.getName()
                  + ").");
          return false;
        }
        ...
        if (!checkInputHeader(file, type)) return false;
          ...
          || type == FileType.NIST_CAID
          || type == FileType.NSRL_DB) {
  ...
  private FileType getFileType(File file) {
    ...
    if (name.endsWith(".json") && isKnownJson(file, FileType.NIST_CAID)) return FileType.NIST_CAID;
    if (name.endsWith(".db") && isKnownDB(file, FileType.NSRL_DB)) return FileType.NSRL_DB;
  ...
  private boolean checkNsrlHeader(File file, FileType type) {
    ...
    try {
      ...
      if (!hasHash && (type == FileType.NSRL_MAIN || type == FileType.NSRL_MAIN_ZIP)) {
        System.out.println(
            "ERROR: Invalid NSRL file " + file + ", no hash was found in its header.");
        return false;
      }
      if (!hasProductCode) {
        System.out.println(
            "ERROR: Invalid NSRL file " + file + ", no product code was found in its header.");
        return false;
      }
      if (!hasProductName && type == FileType.NSRL_PROD) {
        System.out.println(
            "ERROR: Invalid NSRL file " + file + ", no product name was found in its header.");
        return false;
      }
    } catch (Exception e) {
      System.out.println("Error reading file: " + file);
  ...
  private boolean checkInputHeader(File file, FileType type) {
    ...
    try {
      ...
      if (!hasHash) {
        System.out.println(
            "ERROR: File " + file + " header must contain at least one hash column.");
        return false;
      }
      if (!hasProperty) {
        System.out.println(
            "ERROR: File " + file + " header must contain at least one property column.");
        return false;
      }
    } catch (Exception e) {
      System.out.println("Error reading file: " + file);
  ...
  private boolean prepare() {
    try {
      ...
      stmtInsertHashProperty =
          connection.prepareStatement(
              "insert into HASHES_PROPERTIES (HASH_ID, PROPERTY_ID, VALUE) values (?, ?, ?)");
      stmtUpdateHashProperty =
          connection.prepareStatement(
              "update HASHES_PROPERTIES set VALUE=? where HASH_ID=? and PROPERTY_ID=?");
      ...
      stmtRemoveHashProperties =
          connection.prepareStatement(
              "delete from HASHES_PROPERTIES where HASH_ID=? and PROPERTY_ID=?");
      stmtRemoveAllHashProperties =
          connection.prepareStatement("delete from HASHES_PROPERTIES where HASH_ID=?");

      stmtSelectHashProperties =
          connection.prepareStatement(
              "select PROPERTY_ID, VALUE from HASHES_PROPERTIES where HASH_ID=?");
    ...
    for (int i = 0; i < args.length; i++) {
      ...
      if (arg.equalsIgnoreCase("-d")) {
        ...
        if (value1 == null || value2 == null) {
          System.out.println(
              "ERROR: -renameCol must be followed by the current and the new column name.");
          return false;
        }
        ...
        if (value1 == null || value2 == null) {
          System.out.println(
              "ERROR: -addCol must be followed by the column name and its fixed value.");
          return false;
        }
  ...
  private void usage() {
    ...
    System.out.println(
        "    Allows importing CSV files with a set of hashes and associated properties");
    System.out.println(
        "    into a database that can be later used during IPED case processing, to");
    System.out.println(
        "    search for these hashes and add their properties to the case item when a");
    System.out.println(
        "    hit is found. NIST NSRL RDS files (v2), NIST NSRL RDS DBs (v3), NIST CAID");
    System.out.println(
        "    Non-RDS JSON, Project VIC JSON and INTERPOL ICSE database CSV can also be");
    System.out.println("    imported directly.");
    System.out.println();
    System.out.println(
        "Usage: java -jar iped-hashdb.jar -d <input file or folder> -o <output DB file>");
    ...
    System.out.println(
        "            [-renameCol <current name> <new name>] [-skipCol <column name>]");
    System.out.println("            [-mapValue <column name> <current value> <new value>]");
    System.out.println();
    System.out.println("  -d <input file or folder>");
    System.out.println(
        "    Input files (can be used multiple times). If a folder is used, it processes");
    System.out.println(
        "    all files with '.csv' extension, NSRL or Project Vic files. CSV input files");
    System.out.println(
        "    should use plain text format, one item per line, with columns separated by");
    System.out.println(
        "    commas (or another character specified by '-delimiter' parameter). The first");
    System.out.println(
        "    line must be a header, defining the columns names. There must be one or more");
    ...
    System.out.println(
        "    with a different value, the default is to merge, keeping both values. Use");
    System.out.println(
        "    this option to replace, instead of merging, the value associated with the");
    ...
    System.out.println(
        "    parameter to remove items. It will remove only properties/values present");
    System.out.println(
        "    in the current file. If there are no property remaining, the hash itself");
    System.out.println("    is removed.");
    System.out.println("  -removeAll");
    System.out.println("    Remove all references to the hashes present in the input files.");
    System.out.println("  -noOpt");
    System.out.println(
        "    Skip optimizations (reclaim empty space and database analisys) executed");
    System.out.println("    after processing input file(s).");
    System.out.println("  -delimiter <char>");
    System.out.println(
        "    Specify the column delimiter used in the CSV files to be imported. Default");
    System.out.println("    delimiter is comma (,).");
    System.out.println();
    System.out.println(
        "Optional parameters to modify columns and values when importing CSVs (not used");
    System.out.println(
        "for ProjectVIC, NSRL and ICSE). Can be used multiple times. Column names and");
    ...
    System.out.println(
        "    Add a fixed column (for all imported lines), with a given name and value.");
    System.out.println("  -mapValue <column name> <current value> <new value>");
    System.out.println(
        "    Replace the current value specified by the new value, when found in the");
