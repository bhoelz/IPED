...
/**
 * Tests parameter parsing in HashDBTool.run() without hitting the database. All test cases trigger
 * parseParameters() to return false, so the method returns before any file I/O or System.exit() is
 * called.
 */
class HashDBToolParametersTest {
  ...
  void run_whenDWithNonExistentFile_thenFalse() {
    assertFalse(
        new HashDBTool().run(new String[] {"-d", "/no/such/file/exists.csv", "-o", "out.db"}));
  }
  ...
  void run_whenDuplicateO_thenFalse() throws Exception {
    ...
    assertFalse(
        new HashDBTool().run(new String[] {"-d", input.getAbsolutePath(), "-o", out1, "-o", out2}));
  }
  ...
  void run_whenConflictingModes_thenFalse() throws Exception {
    ...
    assertFalse(
        new HashDBTool()
            .run(new String[] {"-d", input.getAbsolutePath(), "-o", out, "-replace", "-remove"}));
  }
