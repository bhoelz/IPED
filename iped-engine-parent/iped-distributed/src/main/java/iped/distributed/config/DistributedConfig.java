...
public class DistributedConfig extends AbstractPropertiesConfigurable {
    ...
    /**
     * Directory in which the coordinator persists case definitions for crash recovery
     * ({@code cases.json}).  Empty/blank disables persistence (in-memory only).  Should
     * point to durable storage that survives a coordinator restart (a mounted volume in
     * containerised deployments).
     */
    private String coordinatorStateDir = "";

    /**
     * Adaptive work-unit sizing — soft target for the weighted byte size of one work unit
     * (items below this accumulate together).  See {@code iped.distributed.workunit}.
     */
    ...
    /**
     * Adaptive work-unit sizing — a single item whose weighted size reaches this is
     * isolated into its own work unit. Must be ≥ {@code workUnitTargetBytes}.
     */
    ...
    /**
     * When > 0, the coordinator automatically requeues DLQ items whose {@code attempt}
     * count is below this threshold on every sweep cycle.  {@code 0} (default) disables
     * automatic DLQ requeue — operators drive requeue manually via the REST API.
     */
    private int dlqAutoRetryMaxAttempts = 0;

    /**
     * Milliseconds of cool-down before a DLQ auto-requeue item is eligible for agent
     * pickup.  Sets {@code notBeforeMs = now + dlqAutoRetryDelayMs} on the requeued
     * message.  {@code 0} = immediate.
     */
    private long dlqAutoRetryDelayMs = 60_000L;

    /**
     * How often (in seconds) the coordinator's DLQ auto-retry sweep runs.
     * Only effective when {@code dlqAutoRetryMaxAttempts > 0}.
     */
    private int dlqAutoRetryIntervalSeconds = 120;

    /**
     * Maximum number of cases a single multi-case agent may be subscribed to at once.
     * The coordinator's heartbeat response caps the topic list to the highest-priority
     * running cases (by {@link iped.distributed.scheduler.CasePriority}, then alphabetical).
     * {@code 0} (default) = unlimited.
     */
    ...
    /**
     * When true, the coordinator automatically opens a {@code DualRunSession} for every
     * new case and exposes the {@code /api/v1/dualrun/{caseId}} and
     * {@code /api/v1/dualrun/{caseId}/reference} endpoints.
     * A dual-run session can also be started manually via
     * {@code POST /api/v1/dualrun/{caseId}/start} regardless of this flag.
     */
    ...
    /**
     * SASL mechanism for broker authentication.  Supported values: {@code PLAIN},
     * {@code SCRAM-SHA-256}, {@code SCRAM-SHA-512}.  Empty/blank = SASL disabled.
     * Combine with {@code kafkaTlsEnabled=true} for encrypted credentials (SASL_SSL).
     */
    ...
    /**
     * Shared HMAC-SHA256 secret for signing/verifying Kafka work-unit payloads.
     * Empty/blank = payload signing disabled.  All agents in a cluster must share
     * the same secret.  Use a cryptographically random string of ≥ 32 characters.
     */
    ...
    /**
     * Path to the CSV file where processing outcomes are durably logged for
     * chain-of-custody purposes.  Empty/blank = in-memory only (lost on restart).
     * The file is opened in append mode; existing records are preserved across restarts.
     */
    ...
    public String  getCoordinatorStateDir()         { return coordinatorStateDir; }
    public long    getWorkUnitTargetBytes()         { return workUnitTargetBytes; }
    public int     getWorkUnitMaxItems()            { return workUnitMaxItems; }
    public long    getWorkUnitOversizedBytes()      { return workUnitOversizedBytes; }
    public int     getDlqAutoRetryMaxAttempts()     { return dlqAutoRetryMaxAttempts; }
    public long    getDlqAutoRetryDelayMs()         { return dlqAutoRetryDelayMs; }
    public int     getDlqAutoRetryIntervalSeconds() { return dlqAutoRetryIntervalSeconds; }
    public int     getMaxSubscribedCases()          { return maxSubscribedCases; }
    public double  getBackpressureHeapSoftRatio()   { return backpressureHeapSoftRatio; }
    public double  getBackpressureHeapHardRatio()   { return backpressureHeapHardRatio; }
    public long    getBackpressureDiskSoftFreeBytes() { return backpressureDiskSoftFreeBytes; }
    public long    getBackpressureDiskHardFreeBytes() { return backpressureDiskHardFreeBytes; }
    public boolean isDualRunEnabled()                 { return dualRunEnabled; }
    public boolean isKafkaTlsEnabled()                { return kafkaTlsEnabled; }
    public String  getKafkaTruststorePath()           { return kafkaTruststorePath; }
    public String  getKafkaTruststorePassword()       { return kafkaTruststorePassword; }
    public String  getKafkaKeystorePath()             { return kafkaKeystorePath; }
    ...
    public String  getKafkaSaslUsername()             { return kafkaSaslUsername; }
    public String  getKafkaSaslPassword()             { return kafkaSaslPassword; }
    public String  getPayloadSigningSecret()          { return payloadSigningSecret; }
