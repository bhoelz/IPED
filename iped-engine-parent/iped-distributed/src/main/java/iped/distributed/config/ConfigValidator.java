 ...
 *
 * <p>Callers should invoke {@link #validate} immediately after loading config and refuse to start
...
public final class ConfigValidator {
  ...
  /**
   * Validates the given config and returns a (possibly empty) list of actionable error messages. An
   * empty list means the config is ready to use.
   */
  public static List<String> validate(DistributedConfig cfg) {
    ...
    if (blank(cfg.getCoordinatorServerUrl())) errors.add("coordinatorServerUrl must not be blank");

    if (blank(cfg.getSharedStorageRoot())) errors.add("sharedStorageRoot must not be blank");

    // ---- Port / timing ----------------------------------------------------
    int port = cfg.getCoordinatorPort();
    if (port < 1 || port > 65535) errors.add("coordinatorPort must be in [1, 65535], got " + port);
    ...
    if (cfg.getHeartbeatIntervalSeconds() <= 0)
      errors.add(
          "heartbeatIntervalSeconds must be positive, got " + cfg.getHeartbeatIntervalSeconds());

    if (cfg.getHeartbeatIntervalSeconds() >= cfg.getAgentExpirySeconds())
      errors.add(
          "heartbeatIntervalSeconds ("
              + cfg.getHeartbeatIntervalSeconds()
              + ") must be less than agentExpirySeconds ("
              + cfg.getAgentExpirySeconds()
              + ")");
    ...
    if (cfg.getMaxRetries() < 0) errors.add("maxRetries must be >= 0, got " + cfg.getMaxRetries());
    ...
    if (cfg.getWorkUnitOversizedBytes() < cfg.getWorkUnitTargetBytes())
      errors.add(
          "workUnitOversizedBytes ("
              + cfg.getWorkUnitOversizedBytes()
              + ") must be >= workUnitTargetBytes ("
    ...
    if (heapSoft > 0 && heapHard > 0 && heapSoft >= heapHard)
      errors.add(
          "backpressureHeapSoftRatio ("
              + heapSoft
              + ") must be < backpressureHeapHardRatio ("
              + heapHard
              + ")");

    if (cfg.getBackpressureDiskSoftFreeBytes() <= 0)
      errors.add(
          "backpressureDiskSoftFreeBytes must be positive, got "
              + cfg.getBackpressureDiskSoftFreeBytes());

    if (cfg.getBackpressureDiskHardFreeBytes() <= 0)
      errors.add(
          "backpressureDiskHardFreeBytes must be positive, got "
              + cfg.getBackpressureDiskHardFreeBytes());

    if (cfg.getBackpressureDiskSoftFreeBytes() > 0
        && cfg.getBackpressureDiskHardFreeBytes() > 0
        && cfg.getBackpressureDiskHardFreeBytes() >= cfg.getBackpressureDiskSoftFreeBytes())
      errors.add(
          "backpressureDiskHardFreeBytes ("
              + cfg.getBackpressureDiskHardFreeBytes()
              + ") must be < backpressureDiskSoftFreeBytes ("
              + cfg.getBackpressureDiskSoftFreeBytes()
              + ")");
