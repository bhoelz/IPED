...
/**
 * Registry of {@link ISeekableInputStreamFactory} builders keyed by fully-qualified
 * class name.
 *
 * <p>Each worker node registers builders for the factory types it is capable of
 * reconstructing (e.g. SleuthkitInputStreamFactory, AD1InputStreamFactory).
 * During item deserialization from Kafka, {@link #reconstruct} is called with the
 * class name and parameters stored in the {@link iped.distributed.kafka.KafkaItemMessage}.
 *
 * <p>Registration must happen during node startup, before any
 * {@link iped.distributed.agent.TaskAgent} begins processing.
 *
...
public final class InputStreamFactoryRegistry {
     ...
     *
     * @param fqcn   fully-qualified class name stored in {@link iped.distributed.kafka.KafkaItemMessage}
     ...
     */
    public static ISeekableInputStreamFactory reconstruct(String fqcn,
            Map<String, String> params) {
        Function<Map<String, String>, ISeekableInputStreamFactory> builder = BUILDERS.get(fqcn);
        if (builder == null) {
            throw new IllegalArgumentException(
                    "No InputStreamFactory builder registered for class '" + fqcn
