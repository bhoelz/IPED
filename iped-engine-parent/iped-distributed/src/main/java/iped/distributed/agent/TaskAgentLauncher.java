 ...
 *
 * <ol>
 *   <li>Parses CLI arguments
 *   <li>Loads IPED configuration (so tasks can access FileSystemConfig, etc.)
 *   <li>Registers the known {@link InputStreamFactoryRegistry} builders
 *   <li>Resolves the pipeline stage from the Coordinator
 *   <li>Instantiates the configured {@code AbstractTask} via reflection
 *   <li>Wraps it in a {@link TaskAgent} and starts it
 * </ol>
 *
 * <h3>Usage</h3>
 *
...
public class TaskAgentLauncher {
  ...
  public static void main(String[] args) throws Exception {
    ...
    log.info(
        "Starting Task Agent: caseId={}, taskType={}, parallelism={}",
        cfg.caseId,
        cfg.taskType,
        cfg.parallelism);
    ...
    if (cfg.configPath != null) {
      ConfigurationDirectory dir =
          new ConfigurationDirectory(java.nio.file.Paths.get(cfg.configPath));
    ...
    if (!(taskInstance instanceof TaskAgent.TaskProcessor)) {
      ...
      // 6. Start the agent
      try (TaskAgent agent =
          new TaskAgent(cfg.caseId, cfg.taskType, stageNumber, processor, distCfg, coordinator)) {
        Runtime.getRuntime()
            .addShutdownHook(new Thread(agent::stop, "agent-shutdown-" + cfg.taskType));
      ...
      try (TaskAgent agent =
          new TaskAgent(
              ...
              cfg.taskType,
              stageNumber,
              (TaskAgent.TaskProcessor) taskInstance,
              distCfg,
              coordinator)) {
        Runtime.getRuntime()
            .addShutdownHook(new Thread(agent::stop, "agent-shutdown-" + cfg.taskType));
  ...
  /**
   * Builds a {@link TaskAgent.TaskProcessor} that delegates to an {@code AbstractTask} via
   * reflection, keeping the task code completely independent of this class.
   */
      ...
      InputStreamFactoryRegistry.register(
          ...
          params -> {
            try {
              ...
              return (iped.io.ISeekableInputStreamFactory)
                  ctor.newInstance(
      ...
      InputStreamFactoryRegistry.register(
          ...
          params -> {
            try {
              java.lang.Class<?> cls =
                  Class.forName("iped.engine.datasource.AD1DataSourceReader$AD1InputStreamFactory");
              java.lang.reflect.Constructor<?> ctor = cls.getConstructor(java.nio.file.Path.class);
              return (iped.io.ISeekableInputStreamFactory)
                  ctor.newInstance(java.nio.file.Paths.get(params.get("datasourcePath")));
            } catch (Exception e) {
      ...
      InputStreamFactoryRegistry.register(
          ...
          params -> {
            try {
              ...
              java.lang.reflect.Constructor<?> ctor = cls.getConstructor(java.nio.file.Path.class);
              return (iped.io.ISeekableInputStreamFactory)
                  ctor.newInstance(java.nio.file.Paths.get(params.get("ufdrPath")));
            } catch (Exception e) {
    ...
    for (int i = 0; i < args.length - 1; i += 2) {
      switch (args[i]) {
        case "--caseId":
          ...
          break;
        case "--taskType":
          cfg.taskType = args[i + 1];
          break;
        case "--taskClass":
          cfg.taskClass = args[i + 1];
          break;
        case "--kafka":
          cfg.kafkaBootstrapServers = args[i + 1];
          break;
        case "--coordinator":
          cfg.coordinatorUrl = args[i + 1];
          break;
        case "--config":
          cfg.configPath = args[i + 1];
          break;
        case "--parallelism":
          cfg.parallelism = Integer.parseInt(args[i + 1]);
          break;
        case "--sharedStorage":
          cfg.sharedStorageRoot = args[i + 1];
          break;
        default:
