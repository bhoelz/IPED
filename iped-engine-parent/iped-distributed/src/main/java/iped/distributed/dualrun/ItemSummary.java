 ...
 *
 * <p>On the <b>distributed side</b> this is extracted from {@link ItemStatusEvent}
 * (populated at discovery time). On the <b>reference side</b> items are submitted
 * by the operator (or by the monolithic IPED run's output scanner) via the REST API.
 *
 * <p>The {@link #path} field is the canonical matching key between the two sides;
 * {@link #uuid} is the distributed-assigned UUID (null on the reference side when
 * the monolithic path uses a different ID scheme).  {@link #mediaType} and
 * {@link #lengthBytes} are optional enrichment used for attribute comparison.
 */
