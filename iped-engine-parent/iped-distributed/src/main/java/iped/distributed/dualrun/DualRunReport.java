 ...
 *
 * <p>The discrepancy lists ({@link #missingFromDistributed}, {@link #extraInDistributed}, {@link
 * #attributeMismatches}) are capped at {@link #MAX_DISCREPANCY_LIST} entries each to keep API
 * responses bounded; {@link #totalMissing} / {@link #totalExtra} / {@link
 * #totalAttributeMismatches} hold the true counts.
 */
  ...
  public String summary() {
    return switch (verdict) {
      ...
      case MISMATCH -> {
        ...
        if (totalMissing > 0) sb.append(" missing=").append(totalMissing);
        if (totalExtra > 0) sb.append(" extra=").append(totalExtra);
        if (totalAttributeMismatches > 0) sb.append(" attr-diff=").append(totalAttributeMismatches);
        sb.append(" (dist=")
            .append(distributedItemCount)
            .append(" ref=")
            .append(referenceItemCount)
            .append(')');
        yield sb.toString();
      }
    };
