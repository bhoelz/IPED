 ...
 *
 * <p>Both maps are keyed by the canonical item <b>path</b> (the matching key
 * between the distributed and reference paths).  The distributed side is built from
 * {@link ItemStatusEvent} DISCOVERED events; the reference side is submitted by the
 * operator from the monolithic run's output.
 *
 * <p>Comparison rules:
 * <ol>
 *   <li><b>Coverage</b>: items in reference but absent from distributed →
 *       {@link DualRunReport#missingFromDistributed}; items in distributed but absent
 *       from reference → {@link DualRunReport#extraInDistributed}.</li>
 *   <li><b>Attribute agreement</b>: for items present in both paths, {@code mediaType}
 *       and {@code lengthBytes} are compared when <em>both</em> sides have a non-null
 *       value.  If only one side has the field it is treated as unknown (no mismatch).</li>
 *   <li><b>Verdict</b>: {@link DualRunVerdict#INCOMPLETE} if either side is not finished;
 *       {@link DualRunVerdict#MATCH} if all three discrepancy lists are empty;
 *       {@link DualRunVerdict#MISMATCH} otherwise.</li>
 * </ol>
 */
public final class DualRunComparator {
     ...
     */
    public static DualRunReport compare(String caseId,
        ...
        List<String> allMissing = referenceByPath.keySet().stream()
                .filter(p -> !distributedByPath.containsKey(p))
                .sorted()
                .collect(Collectors.toList());

        // Coverage: paths in distributed but not in reference
        List<String> allExtra = distributedByPath.keySet().stream()
        ...
        for (Map.Entry<String, ItemSummary> entry : distributedByPath.entrySet()) {
            ...
            if (dist.mediaType() != null && ref.mediaType() != null
                    && !dist.mediaType().equals(ref.mediaType())) {
                allMismatches.add(new AttributeMismatch(path, "mediaType",
                        dist.mediaType(), ref.mediaType()));
            }
            if (dist.lengthBytes() != null && ref.lengthBytes() != null
                    && !dist.lengthBytes().equals(ref.lengthBytes())) {
                allMismatches.add(new AttributeMismatch(path, "lengthBytes",
                        String.valueOf(dist.lengthBytes()), String.valueOf(ref.lengthBytes())));
            }
        }
        ...
        return new DualRunReport(
                caseId, verdict,
                distributedByPath.size(), referenceByPath.size(),
                ...
                allMismatches.size(),
                distributedComplete, referenceSubmitted,
                Instant.now());
    }
}
