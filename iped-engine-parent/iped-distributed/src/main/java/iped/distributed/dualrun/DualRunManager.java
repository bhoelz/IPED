 ...
 *
 * <p>One {@link DualRunSession} is maintained per case.  Sessions are started
 * explicitly via {@link #startSession(String)} — typically when a case is registered
 * and dual-run mode is enabled.  Event routing ({@link #recordEvent}) is a no-op
 * for cases without an active session, so this manager can be wired into the event
 * consumer unconditionally.
 *
 * <p>The {@link #submitReference} and {@link #getReport} methods back the two
 * dual-run REST endpoints on the coordinator.
 */
@Slf4j
public class DualRunManager {
    ...
    /**
     * Create (or return the existing) dual-run session for the given case.
     * Safe to call multiple times for the same case.
     */
    public DualRunSession startSession(String caseId) {
        return sessions.computeIfAbsent(caseId, id -> {
    ...
    public boolean hasSession(String caseId) { return sessions.containsKey(caseId); }

    public List<String> activeSessionCaseIds() { return new ArrayList<>(sessions.keySet()); }
}
