 ...
 *
 * <p>Each Task Agent and each datasource reader holds one instance. Events are sent asynchronously
 * (fire-and-forget) — the status topic is observability infrastructure, not a critical data path.
 */
@Slf4j
public class ItemStatusProducer implements AutoCloseable {
  ...
  public void publishError(
      KafkaItemMessage msg, String taskType, long durationMs, Throwable cause) {
  ...
  public void publish(ItemStatusEvent event) {
    ...
    try {
      ...
      producer.send(
          new ProducerRecord<>(STATUS_TOPIC, event.getCaseId(), json),
          (meta, ex) -> {
            if (ex != null) {
              log.warn(
                  "Failed to publish status event type={} item={}",
                  event.getType(),
                  event.getItemUuid(),
                  ex);
            }
