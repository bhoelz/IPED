...
/**
 * Groups raw items into adaptively-sized work units instead of emitting one fixed-size work unit
 * per item.
 *
 * <h2>Why adaptive</h2>
 *
 * <p>A forensic case is a mix of extremes: millions of tiny files (registry keys, cached
 * thumbnails, log lines) alongside a handful of multi-gigabyte disk images and videos. Dispatching
 * one work unit per item wastes throughput on the small files (per-message, per-poll, per-commit
 * overhead dominates) while dispatching a <i>fixed</i> count per unit risks bundling several huge
 * items together, producing units that blow an agent's memory budget and skew load across the pool.
 *
 * <p>This planner packs items greedily by <b>weighted size</b> — raw bytes scaled by a {@link
 * MediaCostModel} per-type cost — under a {@link WorkUnitSizingPolicy}:
 *
 * <ul>
 *   <li>cheap, small items accumulate until the unit reaches {@code targetUnitBytes} or {@code
 *       maxUnitItems};
 *   <li>a single item at or above {@code oversizedItemBytes} is isolated into its own unit (never
 *       sharing, never co-located with others);
 *   <li>expensive types (archives, videos, disk images) reach the target with fewer items because
 *       their weight is higher.
 * </ul>
 *
 * <h2>Determinism</h2>
 *
 * <p>The plan is a pure function of the input order, the policy, and the cost model: the same input
 * always yields the same units. This keeps work-unit identity stable across reader restarts and is
 * what allows the rest of the pipeline (deterministic sub-item UUIDs, idempotent indexing) to keep
 * its exactly-once-effect guarantee.
 *
...
public class AdaptiveWorkUnitPlanner {
  ...
  /**
   * Groups the given items into work units according to the policy. Null items are skipped. The
   * returned list preserves input order: a unit's items are contiguous in the input, and units are
   * numbered in the order they are flushed.
   *
  ...
  public List<WorkUnit> plan(List<KafkaItemMessage> items) {
    ...
    for (KafkaItemMessage item : items) {
      ...
      boolean wouldExceedBytes =
          !current.isEmpty() && current.weightedBytes + weighted > policy.targetUnitBytes();
