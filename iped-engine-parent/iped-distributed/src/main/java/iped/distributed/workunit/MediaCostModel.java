...
/**
 * Assigns a relative <b>processing-cost weight</b> to an item based on its evidence type, used by
 * {@link AdaptiveWorkUnitPlanner} to size work units.
 *
 * <p>The weight multiplies an item's byte size to produce its "weighted size": types that are
 * expensive to process <i>per byte</i> (archives that expand into thousands of sub-items, videos
 * that get transcoded/thumbnailed) fill a work unit faster and are therefore batched into
 * <i>smaller</i> groups, balancing per-agent load. Cheap, dense types (plain text, directories)
 * batch into larger groups, amortising the fixed per-message and per-poll overhead.
 *
 * <p>The model is deliberately a coarse heuristic — exact per-item cost is unknowable before
 * processing. Weights are matched first on a normalised media type, then on the file extension as a
 * fallback for items whose type has not yet been detected (raw stage-0 items often carry only an
 * extension).
 */
