...
/**
 * Probes real connectivity to the Kafka broker cluster, used to turn the coordinator's {@code
 * /health} endpoint from a liveness check into a readiness check (PI-1-F3-S1).
 *
 * <p>Reuses the same fail-open {@link AdminClient} discipline already established by {@link
 * iped.distributed.metrics.ConsumerLagProvider}: a single long-lived {@code AdminClient} is created
 * once (avoiding a TCP handshake per {@code /health} call) and {@link #probe()} never throws — any
 * failure (timeout, broker down, unexpected error such as a SASL auth failure) is caught and
 * reported as a {@link ProbeResult#degraded}.
 *
 * <h2>Timeout discipline (NFR-E2)</h2>
 *
 * <p>The probe applies an explicit 3-second timeout on both sides: the {@code
 * DescribeClusterOptions} sent to the broker, and the client-side {@code Future.get(...)} wait. The
 * client-side timeout is what actually bounds the call — it guarantees the handler cannot hang even
 * if a given {@code AdminClient} implementation ignored {@code timeoutMs} internally.
 */
@Slf4j
public class BrokerHealthProbe implements AutoCloseable {
  ...
  /**
   * Probes the broker cluster. Never throws — any failure (timeout, connection refused, unexpected
   * broker-side error such as a SASL auth failure) is logged and reported as a {@link
   * ProbeResult#degraded} result with a sanitized reason, so the caller can fail open on the HTTP
   * response instead of propagating a 500.
   */
  public ProbeResult probe() {
    try {
      admin
          .describeCluster(new DescribeClusterOptions().timeoutMs(PROBE_TIMEOUT_MS))
          .clusterId()
