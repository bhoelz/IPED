...
/**
 * Computes which Kafka input-stage topics a registered agent should subscribe to, given the
 * currently active case set and the agent's task type.
 *
 * <p>Used by the coordinator heartbeat endpoint to return steering directives to agents. Only
 * {@code RUNNING} cases that contain the agent's task type in their pipeline are included. The
 * returned list is sorted lexicographically so agents can compare it cheaply with {@code
 * List.equals}.
 *
 * <p>When a {@code maxSubscribedCases} cap is configured, the list is truncated to the
 * highest-priority running cases: cases are first ordered by {@link CasePriority} (descending) then
 * lexicographically by case ID (ascending) for determinism within the same priority. The result is
 * re-sorted lexicographically after capping so the consumer-side equals check remains reliable.
 *
...
public class AgentTopicAssigner {
   ...
   */
  public AgentTopicAssigner(
      AgentRegistry registry, CaseLifecycleManager lifecycle, int maxSubscribedCases) {
  ...
  /**
   * Returns the sorted list of Kafka input-stage topics the given agent should currently subscribe
   * to.
   *
  ...
  public List<String> topicsForAgent(String agentId) {
    ...
    Stream<CaseLifecycleManager.CaseStatus> runningCases =
        lifecycle.allCases().stream()
            .filter(
                c -> c.state == CaseLifecycleManager.CaseStatus.State.RUNNING) // excludes PAUSED
    ...
    if (maxSubscribedCases > 0) {
      runningCases =
          runningCases
              .sorted(
                  Comparator.<CaseLifecycleManager.CaseStatus>comparingInt(
                          c ->
                              -(c.priority != null
                                  ? c.priority.weight()
                                  : CasePriority.NORMAL.weight()))
