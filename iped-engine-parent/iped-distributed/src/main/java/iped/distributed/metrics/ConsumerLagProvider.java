 ...
 * <p>For each consumer group whose name starts with {@code {caseId}.} (the naming convention
 * used by {@link iped.distributed.agent.TaskAgent}):
 * <ol>
 *   <li>Fetch committed offsets via {@code AdminClient.listConsumerGroupOffsets}.</li>
 *   <li>Fetch log-end offsets for the same partitions via {@code AdminClient.listOffsets}
 *       with {@code OffsetSpec.latest()}.</li>
 *   <li>Lag = max(0, endOffset − committedOffset).</li>
 * </ol>
 *
 * <h2>Failure handling</h2>
 * <p>{@link #getLag} never throws; it returns an empty map on any error so the caller's
 * {@code /metrics} scrape succeeds even when the broker is temporarily unavailable.
 *
 * <h2>Lifecycle</h2>
 * <p>This class is {@link AutoCloseable}.  The owner ({@link iped.distributed.coordinator.CoordinatorServer})
 * must call {@link #close()} on shutdown to release the underlying {@code AdminClient}.
 */
@Slf4j
public class ConsumerLagProvider implements AutoCloseable {
    ...
    /**
     * Returns the current consumer lag for all pipeline consumer groups that belong to
     * {@code caseId}.  Returns an empty map on any error (broker unreachable, timeout, …).
     *
    ...
    private Map<DistributedMetrics.LagKey, Long> computeLag(String caseId) throws Exception {
        ...
        for (String group : groups) {
            ...
            for (var entry : committed.entrySet()) {
                ...
                result.put(new DistributedMetrics.LagKey(
                        caseId, group, tp.topic(), tp.partition()), lag);
            }
        }
        return result;
    }
