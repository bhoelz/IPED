 ...
 * <ul>
 *   <li><b>Counters</b> (driven by {@link ItemStatusEvent}s from the {@code iped.status}
 *       topic, consumed by the coordinator):
 *       <ul>
 *         <li>{@code iped_distributed_items_processed_total} — COMPLETED events</li>
 *         <li>{@code iped_distributed_items_failed_total} — ERROR events</li>
 *         <li>{@code iped_distributed_processing_duration_ms_total} — cumulative processing
 *             time from COMPLETED events; divide by processed count for average latency</li>
 *       </ul>
 *   </li>
 *   <li><b>Gauges</b> (live snapshot from {@link AgentRegistry} at scrape time):
 *       <ul>
 *         <li>{@code iped_distributed_agent_inflight} — items being processed per agent</li>
 *         <li>{@code iped_distributed_agent_free_slots} — available parallelism per agent</li>
 *       </ul>
 *   </li>
 *   <li><b>Consumer lag</b> (optional; passed in from {@link ConsumerLagProvider} at scrape
 *       time; omitted when Kafka is unavailable):
 *       <ul>
 *         <li>{@code iped_distributed_consumer_lag} — messages behind end-of-partition</li>
 *       </ul>
 *   </li>
 * </ul>
 *
 * <h2>Thread safety</h2>
 * <p>{@link #recordEvent} is safe to call from any thread.  {@link #scrape} produces a
 * consistent snapshot: each individual counter is read atomically; there is no cross-counter
 * transaction, which is the standard trade-off for Prometheus-style instrumentation.
 *
 * <h2>Prometheus text format</h2>
 * <p>{@link #scrape} returns a {@code text/plain; version=0.0.4} body.  Feed it directly
 * to a {@code /metrics} endpoint or a Prometheus {@code textfile} collector.
 */
public class DistributedMetrics {
    ...
    /**
     * Identifies one lag data-point: a single Kafka partition for a given consumer
     * group.  Produced by {@link ConsumerLagProvider}; consumed by {@link #scrape}.
     */
    ...
    /**
     * Records a single pipeline status event.  Increments the appropriate counter.
     * Events without a {@code taskType} (e.g. DISCOVERED, CASE_COMPLETED) are skipped
     * because they carry no per-stage breakdown.
     *
    ...
    public void recordEvent(ItemStatusEvent event) {
        ...
        switch (event.getType()) {
            ...
            default    -> { /* other event types not individually metered */ }
        }
    }
     ...
     *                  pass {@code Map.of()} when Kafka is unavailable — the lag
     *                  metric family is simply omitted
    ...
    public String scrape(AgentRegistry registry, Map<LagKey, Long> lagByKey,
        ...
        writeCounterFamily(sb, processed, "iped_distributed_items_processed_total",
                "Items successfully processed per case/task-type/pipeline-stage.");
        writeCounterFamily(sb, failed, "iped_distributed_items_failed_total",
                "Items that encountered a processing error per case/task-type/stage.");
        writeCounterFamily(sb, durationMs, "iped_distributed_processing_duration_ms_total",
        ...
        if (!agents.isEmpty()) {
            appendHelp(sb, "iped_distributed_agent_inflight", "gauge",
                    "Items currently being processed by each agent.");
            appendHelp(sb, "iped_distributed_agent_free_slots", "gauge",
        ...
        if (!lagByKey.isEmpty()) {
            appendHelp(sb, "iped_distributed_consumer_lag", "gauge",
    ...
    /**
     * Returns a human-readable counter snapshot for a specific case.
     * Useful for log output and the coordinator's case-status endpoint.
     */
    public Map<String, Long> countersForCase(String caseId) {
        Map<String, Long> out = new LinkedHashMap<>();
        processed.forEach((k, v) -> {
    ...
    private static void writeCounterFamily(StringBuilder sb,
                                            ConcurrentHashMap<MetricKey, LongAdder> map,
    ...
    private static String agentLabels(AgentRegistration a) {
        return String.format(
                ...
                escape(a.getAgentId()), escape(a.getTaskType()),
                a.getStageNumber(), escape(a.getHostname()));
    }

    /**
     * Escapes a Prometheus label value per the text-format spec:
     * backslash → {@code \\}, double-quote → {@code \"}, newline → {@code \n}.
     */
