 ...
 * <p>This deserializer <em>never throws</em>.  If the byte payload cannot be parsed
 * (malformed JSON, incompatible schema, truncated data …) it returns a synthetic
 * {@link KafkaItemMessage} sentinel rather than propagating an exception.  Throwing
 * from inside {@code deserialize()} would crash the {@code consumer.poll()} call in
 * {@link iped.distributed.agent.TaskAgent}, stalling the partition permanently.
 *
 * <p>The sentinel is detected by {@link #isPoison(KafkaItemMessage)} and carries
 * diagnostic information in its {@code extraAttributes} map:
 * <ul>
 *   <li>{@value #POISON_ERROR_KEY} — exception message from the failed parse.</li>
 *   <li>{@value #POISON_RAW_SNIPPET_KEY} — hex-encoded prefix of the raw bytes
 *       (up to {@value #RAW_SNIPPET_MAX_BYTES} bytes) to aid operator investigation.</li>
 * </ul>
 * {@link iped.distributed.agent.TaskAgent} detects the sentinel early in
 * {@code processRecord} and routes it directly to the stage's dead-letter queue
 * without invoking any task logic, then commits the offset so the partition continues.
 */
@Slf4j
public class KafkaItemDeserializer implements Deserializer<KafkaItemMessage> {
    ...
    public KafkaItemMessage deserialize(String topic, byte[] data) {
        ...
        try {
            ...
            log.error("Poison message on topic '{}' — deserialization failed: {}; "
    ...
    /**
     * Returns {@code true} if {@code msg} is a poison-message sentinel produced by
     * this deserializer, i.e. the original Kafka record could not be parsed.
     */
    ...
    private static KafkaItemMessage buildSentinel(String topic, byte[] data, Exception cause) {
        ...
        attrs.put(POISON_ERROR_KEY, cause.getMessage() != null ? cause.getMessage() : cause.getClass().getName());
