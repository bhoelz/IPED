...
/**
 * Coordinator-side background service that automatically requeues DLQ items whose {@code attempt}
 * count is below the configured {@link DistributedConfig#getDlqAutoRetryMaxAttempts()} threshold.
 *
 * <p>On each sweep the retrier:
 *
 * <ol>
 *   <li>Lists all running cases from {@link CaseLifecycleManager}.
 *   <li>For each case, peeks at DLQ entries.
 *   <li>Requeues those with {@code attempt < maxAttempts} using the configured delay ({@link
 *       DistributedConfig#getDlqAutoRetryDelayMs()}).
 * </ol>
 *
 * <p>Disabled when {@code dlqAutoRetryMaxAttempts = 0} (the default). The coordinator schedules
 * {@link #sweep()} via its periodic executor.
 *
...
public class DlqAutoRetrier {
  ...
  public DlqAutoRetrier(
      DlqManager dlqManager, CaseLifecycleManager lifecycle, DistributedConfig cfg) {
  ...
  /**
   * Sweeps all RUNNING cases and requeues eligible DLQ items. Called periodically by the
   * coordinator's scheduler.
   */
  public void sweep() {
    if (!isEnabled()) return;

    List<String> runningCases =
  ...
  private void sweepCase(String caseId) throws Exception {
    ...
    log.info(
        "DLQ auto-retry: requeuing {}/{} items for case '{}' (attempt < {}, delayMs={})",
        eligible.size(),
        entries.size(),
        caseId,
        maxAttempts,
