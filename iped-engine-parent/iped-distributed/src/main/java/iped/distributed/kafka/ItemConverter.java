...
/**
 * Converts between {@link IItem} (engine in-memory representation) and {@link KafkaItemMessage}
 * (serializable Kafka payload).
 *
 * <p><b>Content streams</b>: The actual binary content of an item is NOT embedded in the message.
 * Instead {@link KafkaItemMessage#getInputStreamFactoryClass()} and {@link
 * KafkaItemMessage#getInputStreamFactoryParams()} are populated so that any worker node can call
 * {@link InputStreamFactoryRegistry#reconstruct} to obtain a live factory — provided the datasource
 * files are reachable via shared storage.
 */
@Slf4j
public final class ItemConverter {
  ...
  /**
   * Returns a deterministic UUID for a sub-item derived from its parent's UUID and its ordinal
   * position in the sub-item sequence emitted by the task.
   *
   * <p>This is the foundation of sub-item idempotency in the distributed pipeline. When a parent
   * item is re-delivered after an agent restart, the task re-executes and emits sub-items in the
   * same order. Using this method ensures every sub-item receives the same UUID across deliveries,
   * so downstream stages treat the second write as an upsert (overwrite) rather than a new item,
   * preventing duplicates in the case index.
   *
   * <p>The UUID is version 3 (MD5 name-based) computed from {@code parentItemUuid + ":" + ordinal}
   * encoded as UTF-8. It is stable across JVM restarts and Java versions.
   *
  ...
  /**
   * Creates a new {@link KafkaItemMessage} from an {@link IItem}. The message stage is set to 0
   * (raw — just produced by a reader).
   *
  ...
  /**
   * Merges updated item state back into an existing message (used by Task Agents after running a
   * task: the original message provides identity/stage, the item provides the updated analysis
   * state).
   */
  ...
  /**
   * Reconstructs an {@link IItem} from a {@link KafkaItemMessage}. The returned item is a live,
   * locally addressable object that tasks can use without knowing they are in a distributed
   * pipeline.
   */
    ...
    if (msg.getInputStreamFactoryClass() != null
      ...
      try {
        ISeekableInputStreamFactory factory =
            InputStreamFactoryRegistry.reconstruct(
                msg.getInputStreamFactoryClass(), msg.getInputStreamFactoryParams());
        item.setInputStreamFactory(factory);
      } catch (Exception e) {
        log.warn(
            "Could not reconstruct InputStreamFactory '{}' for item '{}': {}",
