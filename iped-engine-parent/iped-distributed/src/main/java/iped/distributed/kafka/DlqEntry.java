...
/**
 * Read-only view of a single dead-letter-queue record returned by {@link DlqManager#list}.
 *
 * <p>An entry carries enough information for an operator to understand why the item failed and to
 * decide whether to requeue or discard it:
 *
 * <ul>
 *   <li>{@link #dlqTopic}, {@link #partition}, {@link #offset} — the DLQ coordinates required by
 *       {@link DlqManager#requeue} / {@link DlqManager#discard}.
 *   <li>{@link #itemUuid}, {@link #path} — human-readable identity of the failed item.
 *   <li>{@link #pipelineStage}, {@link #attempt} — where in the pipeline the item failed and how
 *       many times it was retried before landing here.
 * </ul>
 */
...
public class DlqEntry {
  ...
  public static DlqEntry from(String dlqTopic, int partition, long offset, KafkaItemMessage msg) {
  ...
  /**
   * Derives the original stage topic name from a DLQ topic name by stripping the DLQ suffix. The
   * original topic is where this item should be republished on a requeue operation.
   *
