 ...
 * <dl>
 *   <dt>{@link #list}</dt>
 *   <dd>Peek at DLQ entries for a case without consuming them.  Safe to call at any time.</dd>
 *   <dt>{@link #requeue}</dt>
 *   <dd>Move specific DLQ items back to their original stage topic with a fresh attempt
 *       counter so agents will retry them.  Commits the DLQ offset to mark them as handled.</dd>
 *   <dt>{@link #discard}</dt>
 *   <dd>Permanently skip specific DLQ items without reprocessing.  Commits the DLQ offset
 *       to mark them as handled.</dd>
 * </dl>
 *
 * <h2>DLQ topic naming</h2>
 * <p>DLQ topics follow the pattern {@code {stageTopic}{dlqSuffix}}, e.g.
 * {@code iped.case1.stage.2.dlq} for the DLQ of stage&nbsp;2 in case {@code case1}
 * with the default suffix {@code .dlq}.  The original stage topic is derived by stripping
 * the suffix; see {@link DlqEntry#originalTopic}.
 *
 * <h2>Consumer group</h2>
 * <p>Requeue and discard operations use the consumer group
 * {@code iped.dlq.ops.{caseId}} to commit which DLQ offsets have been handled.
 * List uses a temporary read-only consumer that does not commit any offset.
 *
 * <h2>Thread safety</h2>
 * <p>Each operation creates a short-lived Kafka consumer and/or producer.  The only
 * shared state is the {@code AdminClient} used to discover DLQ topics; it is thread-safe.
 */
@Slf4j
public class DlqManager implements AutoCloseable {
    ...
    /**
     * Returns up to {@code maxPerTopic} DLQ entries from every DLQ topic belonging to
     * {@code caseId}.  This is a non-destructive peek: no consumer offset is committed,
     * so calling this method multiple times always returns the same view.
     *
    ...
    public List<DlqEntry> list(String caseId, int maxPerTopic) throws Exception {
        ...
        try (KafkaConsumer<String, KafkaItemMessage> consumer = buildConsumer(peekGroup)) {
            ...
            while (System.currentTimeMillis() < deadline) {
                ...
                if (dlqTopics.stream().allMatch(t -> countPerTopic.getOrDefault(t, 0) >= maxPerTopic)) break;
            }
        }

        result.sort(Comparator.comparing(DlqEntry::getDlqTopic)
    ...
    /**
     * Requeues the DLQ items at the given positions by republishing them to their
     * original stage topic with {@code attempt} reset to 0 (as if arriving fresh).
     * Commits the DLQ offsets so these items are not returned by future list/requeue
     * calls.
     *
     ...
     * @param delayMs   milliseconds to delay before the agent picks up the item;
     *                  sets {@code notBeforeMs = now + delayMs}.  {@code 0} = immediate.
     * @return number of items successfully requeued
     */
    public int requeue(String caseId, List<DlqPosition> positions, long delayMs) {
        if (positions.isEmpty()) return 0;
        int count = 0;
        try (KafkaConsumer<String, KafkaItemMessage> consumer = buildConsumer(OPS_GROUP_PREFIX + caseId);
        ...
        log.info("Requeued {}/{} DLQ items for case '{}' (delayMs={})",
                count, positions.size(), caseId, delayMs);
        return count;
    }
    ...
    /**
     * Discards the DLQ items at the given positions without republishing them.
     * Commits the DLQ offsets so these items are not returned by future list/discard
     * calls.
     *
     * @param caseId    the case identifier (used for the ops consumer group)
     * @param positions DLQ coordinates obtained from a prior {@link #list} call
    ...
    /**
     * Returns per-topic statistics for all DLQ topics of the given case.
     * Empty list when no DLQ topics exist or the broker is unreachable.
     */
            ...
            try (KafkaConsumer<String, KafkaItemMessage> consumer = buildConsumer(peekGroup)) {
                for (String dlqTopic : dlqTopics) {
                    ...
                    for (TopicPartition tp : tps) {
                        ...
                        if (e > b) { oldest = Math.min(oldest, b); newest = Math.max(newest, e - 1); }
                    }
                    if (oldest == Long.MAX_VALUE) oldest = -1;
                    if (newest == Long.MIN_VALUE) newest = -1;
                    result.add(new DlqTopicStats(
                            dlqTopic, DlqEntry.originalTopic(dlqTopic, dlqSuffix),
                            count, oldest, newest));
                }
            }
    ...
    /**
     * Seeks to each requested position, reads the record, optionally publishes it to
     * the original topic ({@code producer != null} → requeue, {@code null} → discard),
     * and commits the DLQ offset.
     *
     * @param producer null means discard (no republishing)
     * @return count of successfully handled items
     */
    private int processItems(KafkaConsumer<String, KafkaItemMessage> consumer,
                              List<DlqPosition> positions,
                              KafkaProducer<String, KafkaItemMessage> producer,
                              long delayMs) {
        // Group by partition for efficient seeking
        Map<TopicPartition, List<DlqPosition>> byPartition = positions.stream()
                .collect(Collectors.groupingBy(
                        p -> new TopicPartition(p.getDlqTopic(), p.getPartition())));
        ...
        for (Map.Entry<TopicPartition, List<DlqPosition>> e : byPartition.entrySet()) {
            ...
            List<DlqPosition> sorted = e.getValue().stream()
            ...
            for (DlqPosition pos : sorted) {
                ...
                for (ConsumerRecord<String, KafkaItemMessage> rec : records) {
                    ...
                    if (producer != null) {
                        ...
                        try {
                            producer.send(
                                    new ProducerRecord<>(origTopic, msg.getItemUuid(), msg)).get();
                            log.info("Requeued item '{}' → '{}' (was stage={}, attempt={})",
                                    msg.getItemUuid(), origTopic,
                                    rec.value().getPipelineStage(), rec.value().getAttempt());
                        } catch (Exception sendEx) {
                            log.error("Failed to requeue item '{}': {}",
                                    msg.getItemUuid(), sendEx.getMessage());
                            break; // don't commit — let operator retry
                        }
                    } else {
                        log.info("Discarded item '{}' from '{}' (stage={}, attempt={})",
                                msg.getItemUuid(), pos.getDlqTopic(),
                                msg.getPipelineStage(), msg.getAttempt());
                    }
    ...
    /**
     * Returns the names of all DLQ topics that belong to the given case.
     * A DLQ topic is any topic whose name starts with {@code iped.{caseId}.stage.}
     * and ends with the configured DLQ suffix.
     */
    ...
    private boolean allAtEnd(KafkaConsumer<?, ?> consumer,
