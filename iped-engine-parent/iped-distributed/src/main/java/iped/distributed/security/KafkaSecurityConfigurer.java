...
/**
 * Applies Kafka security configuration (TLS / SASL) to a {@link Properties} map that will be passed
 * to a Kafka producer, consumer, or AdminClient.
 *
 * <p>Supported configurations (controlled by {@link DistributedConfig}):
 *
 * <ul>
 *   <li><b>Plain (no auth)</b> — {@code kafkaTlsEnabled=false}, blank SASL mechanism: no changes to
 *       the properties.
 *   <li><b>TLS only</b> — {@code kafkaTlsEnabled=true}, blank SASL mechanism: sets {@code
 *       security.protocol=SSL}; optional truststore and keystore paths.
 *   <li><b>SASL over plaintext</b> — {@code kafkaTlsEnabled=false}, SASL mechanism set: sets {@code
 *       security.protocol=SASL_PLAINTEXT}.
 *   <li><b>SASL over TLS</b> — {@code kafkaTlsEnabled=true}, SASL mechanism set: sets {@code
 *       security.protocol=SASL_SSL} plus all TLS and SASL properties.
 * </ul>
 *
...
public final class KafkaSecurityConfigurer {
  ...
  /**
   * Apply security properties derived from {@code cfg} to {@code props}. No-op when neither TLS nor
   * SASL is configured.
   */
  public static void apply(Properties props, DistributedConfig cfg) {
    ...
    if (sasl) {
      ...
      props.put(
          "sasl.jaas.config",
          buildJaas(mechanism, cfg.getKafkaSaslUsername(), cfg.getKafkaSaslPassword()));
    }
  }
  ...
  /**
   * Escape double-quotes in JAAS config values (simple pass-through — production values should not
   * contain them).
   */
