 ...
 *
 * <p>The signature covers the immutable structural identity fields of a message — the fields that
 * do not change as an item moves through the pipeline:
 *
 * <pre>
 *   "iped-v1" | caseId | itemUuid | pipelineStage | path | lengthBytes
 * </pre>
 *
 * Fields are joined with {@code "|"}. {@code null} values are represented as the empty string. The
 * resulting HMAC is stored in {@link KafkaItemMessage#getSignature()} as a lowercase hex string.
 *
 * <p>Usage:
 *
 * <ul>
 *   <li>Before producing a message: call {@link #sign(KafkaItemMessage, String)}.
 *   <li>After consuming a message: call {@link #verify(KafkaItemMessage, String)}. Returns {@code
 *       false} if the signature is absent, tampered, or the wrong secret is used.
 *   <li>When no secret is configured (empty/blank string): {@link #isEnabled(String)} returns
 *       {@code false}; callers should skip both sign and verify.
 * </ul>
 *
 * <p>A missing signature on a message received when signing is enabled is treated as a verification
 * failure — callers should route such messages to the DLQ.
 */
public final class PayloadSigner {
  ...
  /**
   * Compute and store the HMAC-SHA256 signature on {@code msg}. After this call {@code
   * msg.getSignature()} carries the hex-encoded HMAC.
   */
