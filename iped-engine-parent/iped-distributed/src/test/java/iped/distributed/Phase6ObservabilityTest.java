 ...
 *
 * <ul>
 *   <li>KafkaItemProducer work-unit tagging logic via AdaptiveWorkUnitPlanner (5 tests)
 *   <li>CaseLifecycleManager.pauseCase / resumeCase (5 tests)
 *   <li>CaseLifecycleManager.updateTags (4 tests)
 *   <li>CaseCompletionMonitor.isStalled / lastEventAtMs (6 tests)
 *   <li>CoordinatorEventLog ring buffer (8 tests)
 *   <li>AgentTopicAssigner excludes PAUSED cases (3 tests)
 *   <li>DistributedConfig DLQ auto-retry config keys (4 tests)
 * </ul>
 */
class Phase6ObservabilityTest {
  ...
  void plannerAssignsWorkUnitIdToMessages() {
    ...
    List<KafkaItemMessage> items =
        List.of(smallMsg("item-1"), smallMsg("item-2"), smallMsg("item-3"));
    ...
    assertEquals(
        items.size() >= unit.itemCount() ? unit.itemCount() : items.size(),
        unit.itemUuids().size());
  }
  ...
  void plannerSplitsWhenExceedingMaxItems() {
    ...
    WorkUnitSizingPolicy policy = new WorkUnitSizingPolicy(Long.MAX_VALUE, 3, Long.MAX_VALUE);
  ...
  void workUnitIndexMatchesPositionWithinUnit() {
    ...
    AdaptiveWorkUnitPlanner planner = new AdaptiveWorkUnitPlanner(policy, new MediaCostModel());
    List<KafkaItemMessage> items = List.of(smallMsg("a"), smallMsg("b"), smallMsg("c"));
    List<WorkUnit> units = planner.plan(items);
    assertEquals(1, units.size());
  ...
  void pauseRunningCaseSucceeds() {
    lifecycle.addCase("c1", List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.RUNNING);
    assertTrue(lifecycle.pauseCase("c1"));
    assertEquals(CaseLifecycleManager.CaseStatus.State.PAUSED, lifecycle.getStatus("c1").state);
  }
  ...
  void resumePausedCaseSetsRunning() {
    lifecycle.addCase("c2", List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.PAUSED);
    assertTrue(lifecycle.resumeCase("c2"));
    assertEquals(CaseLifecycleManager.CaseStatus.State.RUNNING, lifecycle.getStatus("c2").state);
  }

  @Test
  void resumeRunningCaseReturnsFalse() {
    lifecycle.addCase("c3", List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.RUNNING);
  ...
  void updateTagsOnExistingCaseSucceeds() {
    lifecycle.addCase("c4", List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.RUNNING);
  ...
  void updateTagsMergesWithExistingTags() {
    lifecycle.addCase("c5", List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.RUNNING);
  ...
  void updateTagsOverwritesExistingKey() {
    lifecycle.addCase("c6", List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.RUNNING);
  ...
  void lastEventAtMsUpdatesOnEachEvent() throws InterruptedException {
    ...
    assertTrue(
        recorded >= before && recorded <= after + 5,
        "lastEventAtMs should be between "
            + before
            + " and "
            + (after + 5)
            + " but was "
  ...
  void pausedCaseExcludedFromTopics() {
    ...
    lifecycle.addCase(
        "c-paused", List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.PAUSED);
  ...
  void resumedPausedCaseAppearsInTopics() {
    registry.register(AgentRegistration.of("agent-1", "HashTask", 0, 4, "host"));
    lifecycle.addCase(
        "c-was-paused", List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.PAUSED);
  ...
  private static AdaptiveWorkUnitPlanner tinPlanner() {
    ...
    WorkUnitSizingPolicy policy = new WorkUnitSizingPolicy(1L * 1024 * 1024, 5, 2L * 1024 * 1024);
