...
/**
 * Tests for {@link DistributedMetrics}: event recording, Prometheus text format, label escaping,
 * and thread safety.
 *
...
class DistributedMetricsTest {
  ...
  void completedEventIncrementsProcessedCounter() {
    ...
    assertTrue(
        scrape.contains("iped_distributed_items_processed_total"),
        "processed metric must appear after a COMPLETED event");
    assertTrue(
        scrape.contains("{case_id=\"case1\",task_type=\"HashTask\",stage=\"1\"} 1"),
  ...
  void discoveredEventIgnored() {
    ...
    assertEquals(
        "",
        metrics.scrape(registry, Map.of()),
  ...
  void scrapeHasHelpAndTypeLines() {
    ...
    String scrape = metrics.scrape(registry, Map.of());
    assertTrue(
        scrape.contains("# HELP iped_distributed_items_processed_total"), "HELP line required");
    assertTrue(
        scrape.contains("# TYPE iped_distributed_items_processed_total counter"),
        "TYPE line required");
  }
  ...
  void scrapeMetricNamesAreLowercaseUnderscored() {
    ...
    for (String line : scrape.split("\n")) {
      ...
      assertEquals(name.toLowerCase(), name, "metric name must be lowercase: " + name);
    }
  }
  ...
  void agentGaugesAbsentWhenNoAgentsRegistered() {
    String scrape = metrics.scrape(registry, Map.of());
    assertFalse(
        scrape.contains("iped_distributed_agent_inflight"),
  ...
  void lagFamilyAppearsWhenNonEmpty() {
    DistributedMetrics.LagKey key =
        new DistributedMetrics.LagKey("case1", "case1.HashTask", "iped.case1.stage.1", 0);
  ...
  void lagFamilyAbsentWhenEmpty() {
    String scrape = metrics.scrape(registry, Map.of());
    assertFalse(
        scrape.contains("iped_distributed_consumer_lag"),
        "lag family must be omitted when lagByKey is empty");
  }

  @Test
  void multipleLagPartitions() {
    Map<DistributedMetrics.LagKey, Long> lag =
        Map.of(
    ...
    long sampleCount =
        scrape.lines().filter(l -> l.startsWith("iped_distributed_consumer_lag{")).count();
  ...
  void dlqCountFamilyAbsentWhenEmpty() {
    ...
    assertFalse(
        scrape.contains("iped_distributed_dlq_count"),
  ...
  void caseStalledFamilyAbsentWhenEmpty() {
    String scrape = metrics.scrape(registry, Map.of(), Map.of(), Map.of());
    assertFalse(
        scrape.contains("iped_distributed_case_stalled"),
  ...
  void escapeQuotesAreEscaped() {
    assertEquals(
        "a\\\"b", DistributedMetrics.escape("a\"b"), "double-quote must be escaped as \\\"");
  }

  @Test
  void escapeBackslashesAreEscaped() {
    assertEquals("a\\\\b", DistributedMetrics.escape("a\\b"), "backslash must be escaped as \\\\");
  }
  ...
  void countersForCaseOnlyReturnsRequestedCase() {
    ...
    caseA.forEach(
        (k, v) ->
            assertFalse(
                k.contains("caseB"), "countersForCase must not leak entries from other cases"));
  }
  ...
  private static ItemStatusEvent completedEvent(
      String caseId, String taskType, int stage, long durationMs) {
