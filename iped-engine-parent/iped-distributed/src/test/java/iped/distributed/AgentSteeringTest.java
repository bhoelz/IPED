 ...
 * <p>Covers:
 * <ul>
 *   <li>{@link HeartbeatResponse} — construction, field access, JSON round-trip</li>
 *   <li>{@link AgentTopicAssigner} — topic computation for various case/agent combinations</li>
 *   <li>{@link iped.distributed.coordinator.CoordinatorClient} — heartbeat response parsing</li>
 * </ul>
 *
...
class AgentSteeringTest {
    ...
    void heartbeatResponseRoundTrip() throws Exception {
        HeartbeatResponse original = new HeartbeatResponse(
                List.of("iped.case-X.stage.1", "iped.case-Y.stage.1"));
    ...
    private void addRunningCase(String caseId, List<String> tasks) {
        ((StubLifecycleManager) lifecycle).addCase(caseId, tasks,
    ...
    /**
     * Stub that bypasses Kafka topic provisioning for unit tests.
     * Backed by the same CaseLifecycleManager internal maps via registerCase reflection,
     * but we instead replicate the minimal API here for test clarity.
     */
