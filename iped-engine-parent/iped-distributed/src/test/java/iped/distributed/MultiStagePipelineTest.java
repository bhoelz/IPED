 ...
 *
 * <p>Verifies the behaviour of {@link CaseCompletionMonitor} across a 3-task pipeline ({@code
 * HashTask → SignatureTask → IndexTask} at stages 0, 1, 2). The key invariant is that {@code
 * isFinalStage} fires {@code completedFinal} <em>only</em> at stage 2 (IndexTask); completions at
 * stages 0 and 1 do not count toward case completion.
 *
...
class MultiStagePipelineTest {
  ...
  void stage0Completion_doesNotTriggerCaseComplete() {
    ...
    assertEquals(
        0,
        monitor.completedFinalCount(CASE_ID),
        "stage-0 (HashTask) completion must not count toward completedFinal");
    assertFalse(
        monitor.isCompleted(CASE_ID), "case must not be complete after only the first stage");
  }
  ...
  void fullPipelineRun_multipleItems_caseCompletesOnlyWhenAllPassFinalStage() {
    ...
    assertFalse(
        monitor.isCompleted(CASE_ID), "case still open — item-2 has not passed the final stage");
  ...
  void inFlight_clearedOnSkip() {
    ...
    assertEquals(0, monitor.inFlightCount(CASE_ID), "skipped item must be removed from in-flight");
  }
  ...
  void inFlight_clearedOnError() {
    ...
    monitor.onEvent(ItemStatusEvent.started(msg("item-1", 0), HASH));
    monitor.onEvent(
        ItemStatusEvent.error(msg("item-1", 0), HASH, 5L, new RuntimeException("hash failed")));

    assertEquals(0, monitor.inFlightCount(CASE_ID), "errored item must be removed from in-flight");
    assertEquals(1, monitor.failedCount(CASE_ID), "error must be counted in failedCount");
  }
  ...
  void timeout_atIntermediateStage_doesNotIncrementCompletedFinal() {
    ...
    assertEquals(
        0,
        zeroTimeout.completedFinalCount(CASE_ID),
        "timeout at stage 0 must not count toward completedFinal");
    assertFalse(
        zeroTimeout.isCompleted(CASE_ID),
  ...
  void twoSeparateCases_finalStageCountsAreIsolated() {
    ...
    assertEquals(
        0,
        monitor.completedFinalCount(caseB),
