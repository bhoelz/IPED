...
/**
 * Tests for {@link CaseScheduler}: strict priority across classes, round-robin within a class,
 * demand caps, and the no-preemption-of-free-only contract.
 *
...
class CaseSchedulerTest {
  ...
  void urgentCaseTakesAllSlotsBeforeNormal() {
    ...
    assertEquals(8, m.getOrDefault("urgent", 0), "all 8 slots go to the urgent case");
  ...
  void inFlightWorkDoesNotAffectAssignmentAndIsNeverReduced() {
    ...
    assertTrue(
        plan.stream().allMatch(a -> a.slots() > 0),
  ...
  void scheduleIsDeterministic() {
    ...
    assertEquals(
        a.stream().map(SlotAssignment::caseId).toList(),
