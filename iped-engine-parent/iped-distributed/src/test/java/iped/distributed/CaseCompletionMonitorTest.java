...
class CaseCompletionMonitorTest {
    ...
    void setUp() {
        ...
        lifecycle = new CaseLifecycleManager(new TopicProvisioner("localhost:1") {
            @Override
            public void provisionCase(String caseId, int stages, int partitions, short replication) {
                // no-op for tests
            }
        });
    ...
    void publishesCaseCompletedWhenAllItemsFinishFinalStage() {
        ...
        assertEquals(CaseLifecycleManager.CaseStatus.State.COMPLETED,
                lifecycle.getStatus(CASE).state);
    }
    ...
    void caseCompletedIsPublishedOnlyOnce() {
        ...
        long completions = published.stream()
    ...
    void sweepPublishesTimeoutForStuckItemsExactlyOnce() {
        ...
        long timeouts = published.stream()
                .filter(e -> e.getType() == ItemStatusEvent.Type.TIMEOUT)
                .count();
