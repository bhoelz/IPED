 ...
 *
 * <p>None of these tests require a running Kafka broker; they exercise only the deserializer's
 * parse-then-sentinel logic.
 */
class KafkaItemDeserializerTest {
  ...
  void malformedJsonReturnsSentinelNotException() {
    ...
    KafkaItemMessage sentinel = assertDoesNotThrow(() -> deserializer.deserialize(TOPIC, garbage));
  ...
  void truncatedJsonReturnsSentinel() {
    ...
    KafkaItemMessage sentinel =
        assertDoesNotThrow(() -> deserializer.deserialize(TOPIC, truncated));
    assertTrue(KafkaItemDeserializer.isPoison(sentinel));
  }
  ...
  void emptyBytesReturnsSentinel() {
    ...
    KafkaItemMessage sentinel = assertDoesNotThrow(() -> deserializer.deserialize(TOPIC, empty));
    assertTrue(KafkaItemDeserializer.isPoison(sentinel));
  }
  ...
  void wrongTypeJsonReturnsSentinel() {
    ...
    KafkaItemMessage sentinel = assertDoesNotThrow(() -> deserializer.deserialize(TOPIC, array));
    assertTrue(KafkaItemDeserializer.isPoison(sentinel));
  }
  ...
  void randomBinaryReturnsSentinel() {
    ...
    KafkaItemMessage sentinel = assertDoesNotThrow(() -> deserializer.deserialize(TOPIC, binary));
    assertTrue(KafkaItemDeserializer.isPoison(sentinel));
  }
  ...
  void sentinelPathContainsTopic() {
    ...
    KafkaItemMessage sentinel = deserializer.deserialize(TOPIC, garbage);
    assertTrue(
        sentinel.getPath().contains(TOPIC),
  ...
  void sentinelCarriesRawSnippetKey() {
    ...
    assertNotNull(attrs);
    assertTrue(
        attrs.containsKey(KafkaItemDeserializer.POISON_RAW_SNIPPET_KEY),
    ...
    assertTrue(
        snippet.matches("[0-9a-f…()0-9 bytes total]+"),
  ...
  void rawSnippetIsTruncatedForLargePayloads() {
    ...
    String snippet =
        (String) sentinel.getExtraAttributes().get(KafkaItemDeserializer.POISON_RAW_SNIPPET_KEY);
    // 100 bytes = 200 hex chars, plus ellipsis marker
    assertTrue(
        snippet.contains("…")
            || snippet.length() <= KafkaItemDeserializer.RAW_SNIPPET_MAX_BYTES * 2 + 50,
