 ...
 *
 * <p>Verifies that {@link ItemStatusEvent.Type#SUBITEM_DISCOVERED} events correctly grow {@code
 * discovered} after the initial set of root items is registered, preventing premature case
 * completion. A case is only complete when <em>all</em> items — root and sub-items alike — have
 * passed the final pipeline stage.
 *
 * <p>Key behaviours tested:
 *
 * <ul>
 *   <li>{@code SUBITEM_DISCOVERED} increments {@code discovered} just like {@code DISCOVERED}.
 *   <li>The case stays open until every sub-item also completes the final stage.
 *   <li>Sub-items from different parents are independently tracked.
 *   <li>Deterministic sub-item UUIDs (via {@link ItemConverter#deterministicSubitemUuid}) ensure
 *       re-delivery does not duplicate the discovered count.
 *   <li>A case where all root items produce sub-items completes only when sub-items clear the final
 *       stage — even if the parents' final-stage events arrived earlier.
 * </ul>
 *
...
class SubitemDiscoveryTest {
  ...
  void subitemDiscovered_incrementsDiscoveredCount() {
    ...
    assertEquals(
        2, monitor.discoveredCount(CASE_ID), "SUBITEM_DISCOVERED must increment discovered");
  }
  ...
  void multipleSubitemsFromSameParent_eachIncrementsDiscovered() {
    ...
    assertEquals(
        4, monitor.discoveredCount(CASE_ID), "1 root + 3 sub-items must give discovered=4");
  }
  ...
  void deterministicSubitemUuid_discoveredTwice_countsOnce() {
    ...
    assertEquals(
        3,
        monitor.discoveredCount(CASE_ID),
  ...
  void subitemError_incrementsFailedCount() {
    ...
    monitor.onEvent(
        ItemStatusEvent.error(
            msg("sub-bad", 0), TASK, 5L, new RuntimeException("corrupt sub-item")));

    assertEquals(1, monitor.failedCount(CASE_ID), "sub-item error must increment failedCount");
    assertEquals(
        0, monitor.inFlightCount(CASE_ID), "errored sub-item must be removed from in-flight");
  }
