...
/**
 * Verifies that sub-item UUIDs are deterministic — same parent UUID + same ordinal
 * always yields the same sub-item UUID — which is the idempotency guarantee needed so
 * that re-delivering a parent item on agent restart does not create duplicate evidence
 * items in the case index.
 */
class DeterministicSubitemUuidTest {
    ...
    void uuidIsStableAcrossMultipleCalls() {
        for (int i = 0; i < 5; i++) {
            ...
            assertEquals(expected, ItemConverter.deterministicSubitemUuid(PARENT_A, i),
    ...
    void redeliveryProducesIdenticalSubitemUuidSequence() {
        ...
        assertEquals(firstRunUuids, secondRunUuids,
    ...
    void twoParentsWithSameSubitemCountProduceDifferentSequences() {
        ...
        assertNotEquals(uuidsA, uuidsB,
                "sub-item sequences from different parents must not be equal");
    }
