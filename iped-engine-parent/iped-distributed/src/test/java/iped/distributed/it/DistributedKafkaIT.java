...
class DistributedKafkaIT {
    ...
    try (TopicProvisioner provisioner = new TopicProvisioner(bootstrapServers())) {
      ...
      try (KafkaProducer<String, KafkaItemMessage> producer = newItemProducer();
          KafkaConsumer<String, KafkaItemMessage> consumer =
              newItemConsumer("roundtrip-" + UUID.randomUUID())) {
        consumer.subscribe(topics);
        awaitAssignment(consumer);

        producer
            .send(new ProducerRecord<>(topics.get(0), sent.getItemUuid(), sent))
    ...
    try (TopicProvisioner provisioner = new TopicProvisioner(bootstrapServers())) {
      ...
      ItemStatusConsumer consumer =
          new ItemStatusConsumer(
              bootstrapServers(), "status-" + UUID.randomUUID(), true, monitor::onEvent);
      ...
      try {
        ...
        assertTrue(
            completed.await(30, TimeUnit.SECONDS), "case completion must flow through Kafka");
        ...
        assertTrue(
            observed.stream().anyMatch(e -> e.getType() == ItemStatusEvent.Type.CASE_COMPLETED));
      } finally {
        consumer.close();
        producer.close();
      }
  ...
  void caseTopicsAreProvisionedAndDeletedByCoordinator() throws Exception {
    ...
    try (TopicProvisioner provisioner = new TopicProvisioner(bootstrapServers())) {
      CaseLifecycleManager lifecycle = new CaseLifecycleManager(provisioner);
      lifecycle.startCase(
          caseId, List.of(HASH_TASK, INDEX_TASK), 1, (short) 1, CasePriority.NORMAL);
      ...
      assertEquals(
          Set.of(
              ...
              TopicProvisioner.stageTopic(caseId, 2)),
          topics);
  ...
  void malformedPayloadIsConvertedToPoisonSentinelOnConsumption() throws Exception {
    ...
    try (TopicProvisioner provisioner = new TopicProvisioner(bootstrapServers())) {
      ...
      try (KafkaProducer<String, byte[]> producer = newRawProducer();
          KafkaConsumer<String, KafkaItemMessage> consumer =
              newPoisonConsumer("poison-" + UUID.randomUUID())) {
        ...
        producer
            .send(
                new ProducerRecord<>(
                    topic, "poison-key", "{ not valid json".getBytes(StandardCharsets.UTF_8)))
            .get(30, TimeUnit.SECONDS);
        ...
        assertTrue(
            poison.getExtraAttributes().containsKey(KafkaItemDeserializer.POISON_RAW_SNIPPET_KEY));
      }
    }
  }
  ...
  private static <K, V> ConsumerRecord<K, V> pollForRecord(
