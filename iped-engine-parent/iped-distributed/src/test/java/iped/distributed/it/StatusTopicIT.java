...
/**
 * Integration tests for the status topic ({@code iped.status}) — producer/consumer round-trip,
 * consumer-group fan-out, agentId stamping, event-type coverage, and monitor recovery from a
 * replayed topic.
 */
@Tag("integration")
...
class StatusTopicIT {
  ...
  void twoConsumerGroups_bothReceiveAllEvents() throws Exception {
    ...
    ItemStatusConsumer consumer1 =
        new ItemStatusConsumer(
            bootstrapServers(),
            "fanout-A-" + UUID.randomUUID(),
            true,
            e -> {
              if (caseId.equals(e.getCaseId())) latch1.countDown();
            });
    ItemStatusConsumer consumer2 =
        new ItemStatusConsumer(
            bootstrapServers(),
            "fanout-B-" + UUID.randomUUID(),
            true,
            e -> {
              if (caseId.equals(e.getCaseId())) latch2.countDown();
            });
    ...
    try {
      ...
      assertTrue(latch1.await(30, TimeUnit.SECONDS), "consumer group A must receive all 3 events");
      assertTrue(
  ...
  void agentId_isStampedOnEventsWhenProducerHasAgentId() throws Exception {
    ...
    try {
      ...
      assertEquals(
          agentId,
          discovered.getAgentId(),
          "DISCOVERED published by agent producer must carry agentId");
      assertNull(
          started.getAgentId(), "STARTED published by coordinator producer must have null agentId");
    } finally {
      consumer.close();
    }
  }
  ...
  void allMajorEventTypes_roundTripThroughStatusTopic() throws Exception {
    ...
    Set<ItemStatusEvent.Type> expected =
        EnumSet.of(
    ...
    try {
      ...
      assertTrue(
          ...
          "all 7 event types must be received; missing: "
              + expected.stream().filter(t -> !observed.contains(t)).collect(Collectors.toSet()));
    } finally {
      consumer.close();
    }
  }
  ...
  void monitorRecovery_fromBeginning_rebuildsCorrectState() throws Exception {
    ...
    TopicProvisioner noopProvisioner =
        new TopicProvisioner(bootstrapServers()) {
          @Override
    ...
    try {
      ...
      assertTrue(monitor.isCompleted(caseId), "monitor.isCompleted must be true after replay");
  ...
  void eventsForSameCaseId_landInSamePartition() throws Exception {
    ...
    assertEquals(
        1, partsByCaseId.get(caseB).size(), "all caseB events must land in the same partition");
  }
  ...
  /**
   * Creates the {@code iped.status} topic with {@code partitions} partitions if it doesn't already
   * exist. No-op if the topic already exists (the broker returns a TopicExistsException which is
   * silently swallowed).
   */
  private static void ensureStatusTopicWithPartitions(int partitions) throws Exception {
    ...
    try (AdminClient admin = AdminClient.create(p)) {
      List<NewTopic> newTopics =
          List.of(new NewTopic(ItemStatusProducer.STATUS_TOPIC, partitions, (short) 1));
      try {
