 ...
 *
 * <p>The broker-free chaos tests ({@code MultiAgentChaosTest}) verify the watermark arithmetic in
 * isolation. These tests verify the full at-least-once guarantee end-to-end: a consumer that
 * commits only the watermark returned by {@link PartitionOffsetTracker#drainCommittable} causes
 * exactly the uncommitted records to be redelivered to a new consumer in the same group.
 */
@Tag("integration")
...
class AtLeastOnceDeliveryIT {
  ...
  /**
   * Publishes 5 items, processes only the first 3 (commits watermark offset 3), then verifies that
   * a new consumer in the same group is delivered exactly items 3 and 4 — neither more (items 0-2
   * are not redelivered) nor fewer.
   */
  @Test
  @Timeout(120)
  void uncommittedRecords_areRedeliveredToNextConsumerInSameGroup() throws Exception {
    ...
    try (KafkaConsumer<String, KafkaItemMessage> consumer1 = newConsumer(groupId)) {
      ...
      assertEquals(
          expectedCommitOffset,
          toCommit.get(tp).offset(),
    ...
    try (KafkaConsumer<String, KafkaItemMessage> consumer2 = newConsumer(groupId)) {
      ...
      while (System.nanoTime() < deadline) {
        ConsumerRecords<String, KafkaItemMessage> records = consumer2.poll(Duration.ofMillis(500));
  ...
  /**
   * Verifies the gap-stop invariant: when an out-of-order completion leaves a gap, the watermark
   * stops at the gap and the records after the gap are re-delivered on restart — even though they
   * were individually completed before the gap was filled.
   *
   * <p>Setup: 4 records at offsets 0-3. Offsets 0, 2, 3 are marked done; offset 1 is NOT. The
   * watermark can only advance to 0 (the consecutive run stops at 1). Committed offset = 1
   * (next-to-read). On restart, records 1, 2, 3 are redelivered.
   */
  @Test
  @Timeout(120)
  void gap_stopsWatermark_andGapRecordIsRedeliveredWithSuccessors() throws Exception {
    ...
    try (KafkaProducer<String, KafkaItemMessage> producer = newProducer()) {
      for (int i = 0; i < 4; i++) {
        uuids[i] = UUID.randomUUID().toString();
        producer
            .send(new ProducerRecord<>(topic, uuids[i], item(uuids[i])))
    ...
    assertFalse(
        redeliveredUuids.contains(uuids[0]), "offset 0 was committed and must not be redelivered");
    assertTrue(redeliveredUuids.contains(uuids[1]), "gap record (offset 1) must be redelivered");
    assertTrue(
        redeliveredUuids.contains(uuids[2]),
        "record after gap (offset 2) must be redelivered — watermark stopped before it");
    assertTrue(
        redeliveredUuids.contains(uuids[3]), "record after gap (offset 3) must be redelivered");
  }

  /**
   * Verifies that after all records are marked done, {@link
   * PartitionOffsetTracker#drainCommittable} returns the full-commit offset and a restarted
   * consumer in the same group receives nothing.
   */
  @Test
  @Timeout(120)
  void allRecordsCommitted_noRedeliveryOnRestart() throws Exception {
    ...
    assertTrue(
        redelivered.isEmpty(), "no records must be redelivered when all offsets were committed");
  }
  ...
  private static List<ConsumerRecord<String, KafkaItemMessage>> drainN(
    ...
    while (result.size() < n && System.nanoTime() < deadline) {
      for (ConsumerRecord<String, KafkaItemMessage> rec : consumer.poll(Duration.ofMillis(500))) {
