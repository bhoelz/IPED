...
class DualRunComparatorTest {
    ...
    void matchWhenIdentical() {
        Map<String, ItemSummary> dist = Map.of(
                "/a", item("u1", "text/plain", 100L),
                "/b", item("u2", "image/jpeg", 200L));
        Map<String, ItemSummary> ref = Map.of(
    ...
    void managerSubmitReferenceAutoStartsSession() {
        ...
        boolean ok = mgr.submitReference("new-case", List.of(
                ItemSummary.reference("/a", "text/plain", 10L)));
        assertFalse(ok);  // no session yet
        assertFalse(mgr.hasSession("new-case"));

        // Start then submit
        mgr.startSession("new-case");
        ok = mgr.submitReference("new-case", List.of(
                ItemSummary.reference("/a", "text/plain", 10L)));
        assertTrue(ok);
    }
    ...
    private static ItemStatusEvent discoveredEvent(String caseId, String uuid,
                                                    String path, String mediaType, Long length) {
        ItemStatusEvent e = new ItemStatusEvent();
        e.setCaseId(caseId);
        e.setItemUuid(uuid);
        e.setItemPath(path);
