 ...
 * <p>Covers:
 * <ul>
 *   <li>ConfigValidator (15 tests)</li>
 *   <li>KafkaItemMessage work-unit tagging (4 tests)</li>
 *   <li>CaseCompletionMonitor error tracking (4 tests)</li>
 *   <li>AgentTopicAssigner subscription cap (6 tests)</li>
 *   <li>CoordinatorClient registerWithRetry (3 tests)</li>
 *   <li>CaseLifecycleManager.deleteCase (3 tests)</li>
 * </ul>
 */
class Phase5HardeningTest {
    ...
    void cap1_returnsSingleHighestPriorityCase() {
        ...
        registerAgent("agent-1", "HashTask", 0);
        lifecycle.addCase("case-low",    List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.RUNNING, CasePriority.LOW);
        lifecycle.addCase("case-urgent", List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.RUNNING, CasePriority.URGENT);
        lifecycle.addCase("case-normal", List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.RUNNING, CasePriority.NORMAL);
    ...
    void cap2_returnsTwoHighestPriorityCases() {
        ...
        registerAgent("agent-1", "HashTask", 0);
        lifecycle.addCase("case-low",  List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.RUNNING, CasePriority.LOW);
        lifecycle.addCase("case-high", List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.RUNNING, CasePriority.HIGH);
        lifecycle.addCase("case-urg",  List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.RUNNING, CasePriority.URGENT);
        List<String> topics = assigner.topicsForAgent("agent-1");
    ...
    void capLargerThanCaseCount_returnsAllCases() {
        ...
        registerAgent("agent-1", "HashTask", 0);
        lifecycle.addCase("case-A", List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.RUNNING, CasePriority.NORMAL);
        lifecycle.addCase("case-B", List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.RUNNING, CasePriority.NORMAL);
    ...
    void capSamePriority_tieBreaksByAlpha() {
        AgentTopicAssigner assigner = new AgentTopicAssigner(registry, lifecycle, 2);
        registerAgent("agent-1", "HashTask", 0);
        lifecycle.addCase("case-Z", List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.RUNNING, CasePriority.NORMAL);
        lifecycle.addCase("case-A", List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.RUNNING, CasePriority.NORMAL);
        lifecycle.addCase("case-M", List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.RUNNING, CasePriority.NORMAL);
        List<String> topics = assigner.topicsForAgent("agent-1");
        assertEquals(2, topics.size());
    ...
    void capResult_isSortedLexicographically() {
        ...
        registerAgent("agent-1", "HashTask", 0);
        lifecycle.addCase("case-C", List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.RUNNING, CasePriority.HIGH);
        lifecycle.addCase("case-A", List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.RUNNING, CasePriority.HIGH);
        lifecycle.addCase("case-B", List.of("HashTask"), CaseLifecycleManager.CaseStatus.State.RUNNING, CasePriority.HIGH);
        List<String> topics = assigner.topicsForAgent("agent-1");
    ...
    void registerWithRetry_succeedsOnFirstAttempt() {
        ...
        StubClient client = new StubClient(attempts -> {
            callCount.add(1);
            // always succeed
        });
    ...
    void deleteExistingCaseReturnsTrue() {
        lifecycle.addCase("case-del", List.of("HashTask"),
    ...
    void deletedCaseIsRemovedFromAllCases() {
        ...
        assertTrue(lifecycle.allCases().stream()
                .noneMatch(s -> "case-del".equals(s.caseId)));
    ...
    private static boolean hasError(DistributedConfig cfg, String keyword) {
        return ConfigValidator.validate(cfg).stream()
                .anyMatch(e -> e.contains(keyword));
    }

    private CaseCompletionMonitor buildMonitor(List<String> tasks,
    ...
    static class StubLifecycleManager extends CaseLifecycleManager {
        ...
        StubLifecycleManager() { super(null); }
        ...
        void addCase(String caseId, java.util.List<String> tasks,
                     CaseStatus.State state, CasePriority priority) {
        ...
        @Override
        public java.util.Collection<CaseStatus> allCases() { return cases; }
        ...
        public int getStageForTask(String caseId, String taskType) {
            ...
            if (stage == null) throw new IllegalArgumentException(
                    "Task '" + taskType + "' not in case '" + caseId + "'");
            return stage;
        }
        ...
        public boolean pauseCase(String caseId) {
            CaseStatus s = getStatus(caseId);
            if (s == null || s.state == CaseStatus.State.COMPLETED
                    || s.state == CaseStatus.State.FAILED) return false;
