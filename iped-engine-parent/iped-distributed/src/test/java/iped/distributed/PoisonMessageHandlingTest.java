 ...
 *
 * <p>These tests verify the <em>full chain</em>: malformed bytes enter the {@link
 * KafkaItemDeserializer}, a sentinel is produced, and {@link KafkaItemDeserializer#isPoison}
 * correctly identifies it for DLQ routing by {@link
 * iped.distributed.agent.TaskAgent#processRecord}.
 *
 ...
 *
 * <ul>
 *   <li>A poison sentinel is <em>never</em> {@code null}.
 *   <li>A poison sentinel always has a non-null UUID (for DLQ keying).
 *   <li>{@link KafkaItemDeserializer#isPoison} is the single point of truth for whether {@code
 *       processRecord} should skip task execution.
 *   <li>A valid message that happens to have extra attributes is NOT flagged as poison unless it
 *       carries the specific {@link KafkaItemDeserializer#POISON_ERROR_KEY}.
 * </ul>
 */
class PoisonMessageHandlingTest {
  ...
  void malformedBytesProduceDetectablePoison() {
    ...
    assertTrue(
        KafkaItemDeserializer.isPoison(result),
  ...
  void validMessageIsNotPoison() throws Exception {
    ...
    assertFalse(
        KafkaItemDeserializer.isPoison(deserialized),
  ...
  void sentinelHasUuidSuitableForDlqKey() {
    ...
    assertFalse(sentinel.getItemUuid().isBlank(), "DLQ producer requires a non-blank key");
  }
  ...
  void sentinelErrorMessageIsReadable() {
    ...
    assertFalse(
        errMsg.isBlank(), "error message must not be blank — operators rely on it for diagnosis");
  }
  ...
  void multiplePoisonMessagesInSequenceAllDetected() {
    ...
    for (int i = 0; i < payloads.length; i++) {
      ...
      assertTrue(
          KafkaItemDeserializer.isPoison(result), "poison message #" + i + " was not detected");
    }
  }
  ...
  void isPoisonReturnsFalseForManuallyConstructedMessageWithUnrelatedKey() {
    ...
    assertFalse(
        KafkaItemDeserializer.isPoison(msg),
  ...
  void poisonErrorKeyConstantMatchesUsedKey() {
    ...
    assertTrue(
        sentinel.getExtraAttributes().containsKey(KafkaItemDeserializer.POISON_ERROR_KEY),
