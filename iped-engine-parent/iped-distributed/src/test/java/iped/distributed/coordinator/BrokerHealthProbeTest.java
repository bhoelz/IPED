...
/**
 * PI-1-F3-S1 — unit tests for {@link BrokerHealthProbe}, the readiness probe used by {@code
 * CoordinatorServer.handleHealth}.
 *
 * <p>Covers the 3 branches required by the story's DoD, with a mocked {@link AdminClient}:
 *
 * <ol>
 *   <li>broker reachable → {@code ok}
 *   <li>broker probe times out → {@code degraded}
 *   <li>broker probe throws an unexpected (non-timeout) exception → {@code degraded}, never
 *       propagated
 * </ol>
 */
class BrokerHealthProbeTest {
  ...
  void givenProbeTimesOut_whenProbed_thenResultIsDegradedWithReason() throws Exception {
    ...
    assertTrue(
        outcome.reason().contains("3000ms"),
  ...
  void givenUnexpectedException_whenProbed_thenResultIsDegradedAndNotPropagated() throws Exception {
    ...
    BrokerHealthProbe.ProbeResult outcome =
        assertDoesNotThrow(
            probe::probe, "probe() must never propagate — fail-open per PI-1-F3-S1 AC 4");
    ...
    assertFalse(
        outcome.reason().contains("super-sensitive-credential"),
