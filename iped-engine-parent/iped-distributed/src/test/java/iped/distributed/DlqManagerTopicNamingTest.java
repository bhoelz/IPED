 ...
 *
 * <p>These tests do not require a running Kafka broker; they exercise only the static naming logic
 * shared between {@link TopicProvisioner} and the DLQ manager.
 */
class DlqManagerTopicNamingTest {
  ...
  void dlqTopicRoundTripThroughOriginalTopic() {
    for (int stage = 0; stage <= 5; stage++) {
      ...
      assertEquals(
          stageTopic,
          DlqEntry.originalTopic(dlqTopic, SUFFIX),
  ...
  void filterIdentifiesDlqTopicsForCorrectCase() {
    ...
    assertFalse(
        dlqForCase.contains("iped.other-case.stage.0.dlq"),
        "must not include topics from a different case");
    assertFalse(
        dlqForCase.contains("iped.case-abcx.stage.0.dlq"),
