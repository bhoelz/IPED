 ...
 *
 * <p>Validates three invariants that must hold regardless of which agents crash and at which commit
 * offset:
 *
 * <ol>
 *   <li><b>No item loss</b> — every input UUID (and its deterministic sub-item UUID) is present in
 *       the final index.
 *   <li><b>No duplicates</b> — upsert semantics ensure each UUID appears exactly once in the index
 *       even if the same item is processed more than once.
 *   <li><b>Case completion</b> — {@link CaseCompletionMonitor} detects completion once all items
 *       have passed the final pipeline stage, even after crash-and-restart cycles.
 * </ol>
 *
 * <p>Simulation model:
 *
 * <ul>
 *   <li>One topic partition per simulated agent; items are distributed round-robin.
 *   <li>Each agent processes its partition sequentially, calling {@code recordPolled/markDone} and
 *       committing via {@code drainCommittable} after every {@code COMMIT_EVERY} items — matching
 *       the production {@link PartitionOffsetTracker} usage pattern.
 *   <li>A "crash" resets the agent's read position to its last committed watermark so uncommitted
 *       items are re-delivered (at-least-once).
 *   <li>The "index" is a {@code Map<UUID, String>} with upsert semantics, modelling Lucene's {@code
 *       updateDocument}: re-writing the same UUID is idempotent.
 *   <li>Sub-items are derived via {@link ItemConverter#deterministicSubitemUuid} — the same parent
 *       always produces the same sub-item UUID, so re-delivery never duplicates them.
 * </ul>
 *
...
class MultiAgentChaosTest {
  ...
  /**
   * Simulated agent that processes one partition sequentially. Commits after {@code commitEvery}
   * items; crash resets to last committed watermark.
   */
  private static final class SimulatedAgent {
    ...
    /**
     * Simulates a crash: resets the read position to the last committed offset so uncommitted items
     * are re-delivered.
     */
    void crash() {
  ...
  private static List<SimulatedPartition> buildPartitions(int totalItems, int partCount) {
    ...
    for (int p = 0; p < partCount; p++) {
      parts.add(
          new SimulatedPartition(
              new TopicPartition(TOPIC, p), Collections.unmodifiableList(byPart[p])));
    }
    return parts;
  }
  ...
  void partitionTrackerWatermarkStopsAtFirstUncommittedGap() {
    ...
    assertTrue(committable.containsKey(tp), "some offsets must be committable (0 and 1 are done)");
