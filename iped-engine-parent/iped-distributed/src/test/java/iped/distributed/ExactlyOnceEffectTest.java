...
/**
 * Exactly-once-effect validation: a distributed run with agent interruptions must produce the same
 * indexed output as a monolithic (uninterrupted) run.
 *
 * <h2>What is being tested</h2>
 *
 * <p>Three mechanisms work together to guarantee exactly-once <em>observable effect</em> even
 * though the delivery guarantee is only <em>at-least-once</em>:
 *
 * <ol>
 *   <li><b>At-least-once delivery</b> — {@link PartitionOffsetTracker} advances the Kafka offset
 *       watermark only through consecutive completed offsets; uncommitted items are re-delivered
 *       after an agent restart.
 *   <li><b>Sub-item UUID stability</b> — {@link ItemConverter#deterministicSubitemUuid} derives
 *       each sub-item UUID from {@code (parentUuid, ordinal)}; re-delivering the parent therefore
 *       re-emits sub-items with the same UUIDs.
 *   <li><b>Idempotent indexing</b> — the downstream Lucene index uses {@code updateDocument}
 *       (upsert), so re-processing an item overwrites rather than duplicates its entry.
 * </ol>
 *
 * <h2>Simulation model</h2>
 *
 * <p>The simulated pipeline has one partition with N items. The agent processes them in order:
 * items 0..{@code commitAfter}-1 are fully processed and committed; items {@code commitAfter}..N-1
 * are processed but <em>not</em> committed (agent killed). On restart, items from the committed
 * watermark onward are re-delivered. A simulated Lucene index (a {@code Map<uuid, data>}) receives
 * all writes via upsert semantics.
 *
...
class ExactlyOnceEffectTest {
   ...
   *
   * <p>Phase 1: process all N items; mark done only items 0..{@code commitAfter}-1. Drain committed
   * offsets (watermark = {@code commitAfter}-1 if commitAfter > 0).
   *
   * <p>Phase 2 (restart): re-deliver every item at or after the committed watermark (the semantics
   * Kafka gives on consumer restart) and re-index them with upsert.
   *
  ...
  private Map<String, List<String>> distributedRunWithInterruption(
    ...
    long restartFromOffset =
        committed.containsKey(PARTITION)
  ...
  private List<String> makeParentUuids(int count) {
    return IntStream.range(0, count)
        .mapToObj(
            i ->
                UUID.nameUUIDFromBytes(("parent-" + i).getBytes(StandardCharsets.UTF_8)).toString())
        .collect(Collectors.toList());
  }
  ...
  void distributedOutputMatchesMonolithicForVariousCrashPoints(int commitAfter) {
    ...
    Map<String, List<String>> distributed =
        distributedRunWithInterruption(parentUuids, subitemsPerParent, commitAfter);
    ...
    for (String parentUuid : monolithic.keySet()) {
      assertEquals(
          monolithic.get(parentUuid),
          distributed.get(parentUuid),
          "sub-item UUIDs mismatch for parent "
              + parentUuid
              + " (commitAfter="
              + commitAfter
              + ")");
  ...
  void subitemUuidsAreStableAcrossRedeliveries() {
    ...
    assertEquals(
        first, second, "same parent + same ordinals must always produce the same sub-item UUIDs");
  }
  ...
  void uncommittedGapCausesRedeliveryOfAllItemsFromGapOnward() {
    ...
    assertEquals(2L, nextOffset, "watermark stops at gap; restart re-delivers offsets 2, 3, 4");
  }
  ...
  void poisonItemsAreDlqdAndDoNotAppearInOutputIndex() {
    ...
    assertNotNull(
        sentinel.getItemUuid(),
        "DLQ routing still requires a non-null UUID for the Kafka record key");
    assertFalse(
        KafkaItemDeserializer.isPoison(new KafkaItemMessage()),
  ...
  void poisonMessageDoesNotBlockSubsequentValidItems() {
    ...
    String parentUuid =
        UUID.nameUUIDFromBytes("parent-after-poison".getBytes(StandardCharsets.UTF_8)).toString();
