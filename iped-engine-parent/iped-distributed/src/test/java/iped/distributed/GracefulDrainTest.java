 ...
 * </pre>
 *
 * Items whose {@code producer.send} callback had not yet fired by the time {@code
 * executor.awaitTermination} returned would have their {@code markDone} call arrive <em>after</em>
 * the final {@code commitSync}, so their offsets were never committed — causing unnecessary
 * re-delivery on restart.
 *
 * <h2>The fix</h2>
 *
 * <p>Insert {@code producer.flush()} between {@code executor.awaitTermination} and {@code
 * drainCommittable()} in all exit paths. The flush blocks until every pending send and its callback
 * completes, so all {@code markDone} calls fire before the final commit.
 *
 * <h2>Graceful drain</h2>
 *
 * <p>{@code drain()} sets a flag that causes the poll loop to: pause Kafka partitions (stop
 * accepting new records), wait for in-flight items to complete, call flush, then exit — so items
 * in-flight at drain time finish cleanly.
 *
...
class GracefulDrainTest {
  ...
  /**
   * Demonstrates the pre-fix bug: calling {@code drainCommittable()} before the producer callback
   * fires misses the completed offset.
   */
  @Test
  void withoutFlush_asyncCallbackMissedByCommit() {
    ...
    assertEquals(
        2L,
        tooLate.get(P0).offset(),
        "the offset that arrived late is visible in the next drain, "
            + "but the prior commitSync already ran without it");
  }

  /**
   * The fix: calling {@code drainCommittable()} AFTER flush ensures all callbacks are captured in a
   * single commit.
   */
  @Test
  void withFlush_asyncCallbackCapturedBeforeCommit() {
    ...
    assertEquals(
        5L, committed.get(P0).offset(), "with flush: all 5 offsets committed in a single pass");
  }
  ...
  /**
   * Simulates the producer callback arriving on a separate thread shortly after the executor task
   * returns. {@code drainCommittable()} called after a brief wait (simulating flush) captures it;
   * called immediately (no flush) misses it.
   */
  @Test
  void flushMakesAsyncMarkDoneVisibleToCommit() throws InterruptedException {
    ...
    assertEquals(2L, afterFlush.get(P0).offset(), "after flush: offset 1 committed");
  ...
  /**
   * Simulates the consumer-loop drain check: once {@code draining=true} and {@code inFlight=0}, the
   * loop sets {@code running=false} and exits.
   */
  @Test
  void drainFlagWithZeroInFlightExitsLoop() throws InterruptedException {
    ...
    inFlight.decrementAndGet();
    assertTrue(
        exited.await(2, TimeUnit.SECONDS), "loop must exit once inFlight reaches 0 during drain");
  ...
  /**
   * After drain completes, new records must not be submitted to the executor. The drain flag
   * doubles as a "no new intake" gate in the record dispatch loop.
   */
  @Test
  ...
  void partialFlush_onlyConsecutiveCompletionsCommitted() {
    ...
    assertEquals(
        5L, afterLateFlush.get(P0).offset(), "after gap filled: all 5 offsets committable");
  }
  ...
  void multipleCallsToDrainAreIdempotent() throws InterruptedException {
    ...
    assertTrue(
        exited.await(1, TimeUnit.SECONDS), "loop exits cleanly despite multiple drain calls");
  ...
  /**
   * Full shutdown path simulation: 1. awaitTermination (all executor tasks returned) 2.
   * flushProducer (all async callbacks fired → all markDone called) 3. drainCommittable (all
   * committable offsets captured) 4. commitSync
   *
   * <p>Verifies that the offset captured in step 3 includes callbacks that fired during step 2
   * (i.e., after executor tasks returned but before flush completed).
   */
  @Test
