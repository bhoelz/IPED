...
/**
 * Tests for adaptive work-unit sizing: {@link AdaptiveWorkUnitPlanner}, {@link
 * WorkUnitSizingPolicy}, and {@link MediaCostModel}.
 *
...
class AdaptiveWorkUnitPlannerTest {
  ...
  void policyRejectsNonPositiveTarget() {
    assertThrows(IllegalArgumentException.class, () -> new WorkUnitSizingPolicy(0, 10, 100));
  }

  @Test
  void policyRejectsNonPositiveMaxItems() {
    assertThrows(IllegalArgumentException.class, () -> new WorkUnitSizingPolicy(100, 0, 100));
  }

  @Test
  void policyRejectsOversizedBelowTarget() {
    assertThrows(IllegalArgumentException.class, () -> new WorkUnitSizingPolicy(100, 10, 50));
  }
  ...
  void expensiveTypesFillUnitFasterThanCheapTypes() {
    ...
    List<KafkaItemMessage> videos =
        List.of(item(10 * MB, "video/mp4", "mp4"), item(10 * MB, "video/mp4", "mp4"));
  ...
  void costModelFallsBackToExtensionWhenTypeMissing() {
    MediaCostModel m = new MediaCostModel();
    assertEquals(4.0, m.weight(item(1 * MB, null, "mkv")), 0.0001, "no media type → use extension");
  ...
  void costModelHandlesNullAndUnknown() {
    ...
    assertEquals(
        MediaCostModel.DEFAULT_WEIGHT,
        m.weight(item(1 * MB, "application/x-unknown-thing", "qqq")),
        0.0001);
  }
  ...
  void costModelStripsMediaTypeParameters() {
    ...
    assertEquals(
        1.0,
        m.weight(item(1 * MB, "text/plain; charset=utf-8", "txt")),
        0.0001,
  ...
  void mixedWorkloadSeparatesHugeFromSmall() {
    ...
    units.stream().filter(u -> !u.oversized()).forEach(u -> assertTrue(u.itemCount() <= 256));
