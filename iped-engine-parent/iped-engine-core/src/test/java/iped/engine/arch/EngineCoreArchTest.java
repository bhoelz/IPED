 ...
 * viewers-implementation layers, which would create hidden coupling and make
 * headless/server deployments impossible.
 */
@AnalyzeClasses(packages = "iped.engine", importOptions = ImportOption.DoNotIncludeTests.class)
public class EngineCoreArchTest {

    /**
     * The engine must never reach into {@code iped.app}. Application-layer classes
     * (Swing UI, desktop bootstrapping) cannot be on the classpath in server mode.
     */
    @ArchTest
    ...
    /**
     * Engine classes may use the viewers <em>API</em> but must not bind to concrete
     * viewer implementations. Binding to {@code iped.viewers.impl} would prevent
     * swapping or omitting the viewer stack in headless deployments.
     */
    @ArchTest
    static final ArchRule engine_must_not_depend_on_viewers_impl =
        noClasses()
            .should().dependOnClassesThat().resideInAPackage("iped.viewers.impl..")
