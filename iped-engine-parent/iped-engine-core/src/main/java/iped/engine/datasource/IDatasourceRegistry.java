 ...
 *
 * <p>Implementations live in iped-engine (Manager) and are registered via DatasourceRegistry at
 * startup. This breaks the compile-time dependency from datasource modules onto iped-engine.
 */
public interface IDatasourceRegistry {
  ...
  /**
   * Computes the trackID for the item and, when resuming a previous processing, reassigns the item
   * ID to the one from the previous run. Must be called before adding the item to the queue when
   * --continue is active.
   */
