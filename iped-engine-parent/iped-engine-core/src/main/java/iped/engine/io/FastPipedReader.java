...
public class FastPipedReader extends Reader {
   ...
   * . Data written to <code>src</code> will then be available as input from this stream.
   *
  ...
  /**
   * Creates a <code>PipedReader</code> so that it is not yet {@linkplain #connect(FastPipedWriter)
   * connected}. It must be {@linkplain java.io.PipedWriter#connect(java.io.PipedReader) connected}
  ...
  /**
   * Creates a <code>PipedReader</code> so that it is not yet {@link #connect(FastPipedWriter)
   * connected} and uses the specified pipe size for the pipe's buffer. It must be {@linkplain
   * java.io.PipedWriter#connect(java.io.PipedReader) connected} to a <code>PipedWriter</code>
   * before being used.
   *
   * @param pipeSize the size of the pipe's buffer.
   * @exception IllegalArgumentException if <code>pipeSize less than 1</code>.
   ...
   *
   * <p>If <code>src</code> is an unconnected piped writer and <code>snk</code> is an unconnected
   * piped reader, they may be connected by either the call:
   *
   * <p>
   *
   * <pre>
   ...
   * @exception IOException if the pipe is <a href=PipedInputStream.html#BROKEN><code>broken</code>
   *     </a>, {@link #connect(FastPipedWriter) unconnected}, closed, or an I/O error occurs.
   */
  @Override
   ...
   *
   * @exception IOException if the pipe is <a href=PipedInputStream.html#BROKEN><code>broken</code>
   *     </a>, {@link #connect(FastPipedWriter) unconnected}, or closed.
   */
  @Override
