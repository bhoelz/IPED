...
public class SequenceSeekableByteChannel implements SeekableByteChannel {
  ...
  public void close() throws IOException {
    for (SeekableByteChannel c : channels) c.close();
    closed = true;
  }
  ...
  public int read(ByteBuffer dst) throws IOException {
    ...
    if (read == -1) {
      if (channel == channels.length - 1) return -1;
      else {
        channel++;
        return 0;
      }
  ...
  public SeekableByteChannel position(long newPosition) throws IOException {
    ...
    long prevSize = 0;
    channel = 0;
    while ((prevSize += channels[channel].size()) <= pos) channel++;
  ...
  public long size() throws IOException {
    long total = 0;
    for (SeekableByteChannel c : channels) total += c.size();
    return total;
  }
