...
/**
 * API controller for configuration management. Provides REST endpoints for managing configurations.
 */
@Slf4j
public class ConfigurationAPIController {
  ...
  public Map<String, Object> exportConfiguration(String componentName) {
    try {
      ...
      String json =
          mapper.writerWithDefaultPrettyPrinter().writeValueAsString(configData.get("data"));
  ...
  public Map<String, Object> exportAllConfigurations() {
    try {
      ...
      String json = mapper.writerWithDefaultPrettyPrinter().writeValueAsString(allConfigs);
