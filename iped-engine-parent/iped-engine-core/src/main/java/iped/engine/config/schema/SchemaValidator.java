...
/**
 * Validates configurations against their JSON schemas. Provides detailed validation error messages.
 */
public class SchemaValidator {
  private final ObjectMapper mapper =
      new ObjectMapper()
          ...
          .registerModule(
              new SimpleModule()
                  .addSerializer(org.apache.tika.mime.MediaType.class, ToStringSerializer.instance))
  ...
  private void validateNode(JsonNode node, JsonNode schema, String path, List<String> errors) {
    ...
    if (schema.has("enum")) {
      ...
      if (!found) {
        errors.add(
            String.format(
                "%s: Value '%s' is not one of the allowed values",
