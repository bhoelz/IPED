...
public class Configuration {
  ...
  private static final File ipedRoot =
      new File(System.getProperty("user.home"), ".iped/ipedRoot.txt");
  ...
  private String getAppRoot(String configPath) {
    ...
    if (appRoot.contains(PROFILES_DIR)) appRoot = new File(appRoot).getParentFile().getParent();
    return appRoot;
  }
  ...
  private void configureLogger(String configPath) {
    ...
    LogFactory.getFactory()
        .setAttribute("org.apache.commons.logging.Log", NoOpLog.class.getName()); // $NON-NLS-1$
  ...
  private void loadClasspathDefault(String fileName) throws IOException {
    Enumeration<URL> resources =
        Configuration.class
            .getClassLoader()
            .getResources(
                ConfigurationDirectory.DEFAULTS_RESOURCE_DIR + "/" + fileName); // $NON-NLS-1$
  ...
  /**
   * @param extraConfigs optional hook invoked just before the batched {@code
   *     configManager.loadConfigs()}, for registering {@link Configurable}s that {@code
   *     iped-engine-core} cannot construct directly because their implementation classes live in a
   *     module that depends on this one (e.g. {@code iped-engine}). Ignored when {@code loadAll} is
   *     {@code false}.
   */
  public void loadConfigurables(
      String configPathStr, boolean loadAll, ConfigContributor extraConfigs) throws IOException {
  ...
  private void addPluginJarsToConfigurationLookup(
      ConfigurationDirectory configDirectory, PluginConfig pluginConfig) {
