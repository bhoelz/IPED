...
public class ConfigurationManager implements ObjectManager<Configurable<?>>, ITypedConfigAccess {
  ...
  /**
   * Process-global singleton used by the legacy monolithic (single-case) code path. Multi-case
   * callers should use {@link #createCaseInstance} instead so each {@code CaseContext} gets its own
  ...
  /**
   * Returns the process-global singleton. Only valid in single-case (monolithic) mode. Multi-case
   * code must not use this — use the instance stored on the {@code CaseContext} instead.
   *
   * @return the singleton, or {@code null} if {@link #createInstance} was never called
   * @deprecated Use {@link #createCaseInstance(IConfigurationDirectory)} for multi-case isolation
   *     and store the result on the {@code CaseContext}.
   */
  @Deprecated
  ...
  /**
   * Creates (or returns the existing) process-global singleton. Retained for the monolithic
   * single-case launch path.
   *
   * @deprecated Prefer {@link #createCaseInstance(IConfigurationDirectory)} for new
   *     multi-case-aware code.
   */
  @Deprecated
  ...
  /**
   * Creates a fresh {@code ConfigurationManager} instance scoped to one case. The instance is not
   * registered as the process-global singleton, so each {@code CaseContext} can hold its own
   * independent configuration without cross-case contamination.
   *
   * <p>Callers are responsible for storing and propagating the returned instance via {@code
   * CaseContext} (or an equivalent per-case container) rather than relying on {@link #get()}.
   *
  ...
  public void loadConfigs() throws IOException {
    for (Iterator<Configurable<?>> iterator = loadedConfigurables.keySet().iterator();
        iterator.hasNext(); ) {
  ...
  /**
   * Validates all loaded configurables against their JSON schemas and fails fast if any schema
   * violations are found. Validation is best-effort: a missing schema is not an error (many configs
   * predate the schema system).
   *
  ...
  private void validateAllOrFail() {
    ...
    if (stats.getTotalValidated() > 0) {
      log.info(
          "All {} validated configuration(s) passed schema checks.", stats.getTotalValidated());
    }
  }
  ...
  /**
   * Finds a registered configurable by interface, matching via {@code instanceof} rather than exact
   * class. Lets a caller depend on an interface declared here in {@code iped-engine-core} without
   * needing the concrete {@link Configurable} implementation's class on its classpath — useful when
   * the implementation is owned by a module that would otherwise create a dependency cycle (e.g. a
   * task module that depends on the engine).
   */
