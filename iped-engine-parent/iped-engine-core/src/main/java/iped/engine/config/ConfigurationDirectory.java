...
public class ConfigurationDirectory implements IConfigurationDirectory {
    ...
    public static final String DEFAULTS_INDEX_RESOURCE = "META-INF/iped/config-defaults.idx"; //$NON-NLS-1$

    /**
     * Lowest precedence layer: built-in defaults shipped on the classpath by each
     * module and configs inside plugin JARs. Local directories always override
     * these.
     */
    private List<Path> defaultDirs = new ArrayList<Path>();

    /**
     * Local layers in increasing precedence order: app conf dir, selected profile,
     * case profile. Within properties-based configurables the merge is key-level
     * last-wins, so these only need to contain deviations from the defaults.
     */
    ...
    /**
     * Mounts a plugin JAR/ZIP and adds its root to the defaults layer, so local
     * configuration files and profiles can override plugin shipped configs.
     */
    ...
    /**
     * Discovers every module on the classpath shipping built-in default configs
     * (marked by {@value #DEFAULTS_INDEX_RESOURCE}) and registers their
     * {@value #DEFAULTS_RESOURCE_DIR} directory in the defaults layer.
     */
