...
public class PluginVerifier {
  ...
  public void verifyIndexSignature(
      String rawJson, RegistryIndex.SignatureInline sig, PublicKeyStore keyStore) {
  ...
  public void verifyArtifactSignature(
      Path jar, Path sigFile, RegistryIndex.SignatureExternal sig, PublicKeyStore keyStore)
      throws IOException {
    PublicKey key = keyStore.find(sig.keyId());
    if (key == null) {
  ...
  public void verifyServiceLoaderContract(
      Path jar, String expectedProviderClass, String expectedTaskId) throws Exception {
    try (URLClassLoader cl =
        new URLClassLoader(new URL[] {jar.toUri().toURL()}, TaskProvider.class.getClassLoader())) {
      List<TaskProvider> providers =
          ServiceLoader.load(TaskProvider.class, cl).stream()
      ...
      boolean providerClassFound =
          providers.stream().anyMatch(p -> p.getClass().getName().equals(expectedProviderClass));
      if (!providerClassFound) {
        throw new IllegalStateException(
            "Expected providerClass not found: " + expectedProviderClass);
      }

      boolean taskIdFound =
          providers.stream()
              .map(p -> p.descriptor().id())
              .anyMatch(id -> id.equals(expectedTaskId));
      if (!taskIdFound) {
