...
public interface RenditionService {
    ...
    /**
     * Lists rendition kinds available for the given item together with their
     * URL paths so the caller can build absolute URLs.
     *
    ...
    /**
     * Returns the viewer identifier and capabilities for the given MIME type.
     * Map keys: "viewerId", "search" (bool), "hitsMode" (none|external|internal),
     * "toolbarSupported" (bool), "toolbarVisibleByDefault" (bool).
     */
     ...
     */
    void writeRendition(String sourceId, int itemId, String kind, int page, int width, OutputStream out)
            throws Exception;
}
