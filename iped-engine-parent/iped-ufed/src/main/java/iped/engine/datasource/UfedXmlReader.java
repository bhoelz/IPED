...
public class UfedXmlReader extends DataSourceReader {
  ...
  private static final String[] HEADER_STRINGS = {
    "project id", "extractionType", "sourceExtractions"
  }; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

  private static final String AVATAR_PATH_META =
      ExtraProperties.UFED_META_PREFIX + "contactphoto_extracted_path"; // $NON-NLS-1$
  private static final String ATTACH_PATH_META =
      ExtraProperties.UFED_META_PREFIX + "attachment_extracted_path"; // $NON-NLS-1$

  private static final String MEDIA_CLASSES_PROPERTY =
      ExtraProperties.UFED_META_PREFIX + "mediaClasses"; // $NON-NLS-1$
  private static final String MEDIA_CLASSES_SCORE_PREFIX =
      ExtraProperties.UFED_META_PREFIX + "mediaClassScore:"; // $NON-NLS-1$
  ...
  public static final String UFED_NATIVE_SCENE_CLASSIFICATION =
      ExtraProperties.UFED_META_PREFIX + "Native Scene Classification";
  public static final String MSISDN_PROP = "MSISDN";

  private static final String EMPTY_EXTRACTION_STR = "-";

  private static final String FILE_ID_ATTR =
      ExtraProperties.UFED_META_PREFIX + "file_id"; // $NON-NLS-1$
  private static final String LOCAL_PATH_META =
      ExtraProperties.UFED_META_PREFIX + "local_path"; // $NON-NLS-1$
  ...
  private Set<String> supportedApps =
      new HashSet<String>(
          Arrays.asList(
              WhatsAppParser.WHATSAPP,
              TelegramParser.TELEGRAM,
              WhatsAppParser.WHATSAPP + " Business",
              WhatsAppParser.WHATSAPP + " (Dual App)"));
  ...
  HashMap<String, String> ufedFileIdToLocalPath =
      new HashMap<>(); // used to replace non-existent attachment extracted path by local path
  ...
  private InputStream getXmlInputStream(File file) {
    if (file.getName().toLowerCase().endsWith(".xml")) { // $NON-NLS-1$
      try (InputStreamReader reader =
        ...
        int off = 0, i = 0;
        while (off < cbuf.length && (i = reader.read(cbuf, off, cbuf.length - off)) != -1) off += i;
        String header = new String(cbuf, 0, off);
        for (String str : HEADER_STRINGS) if (!header.contains(str)) return null;
  ...
  private InputStream lookUpXmlReportInputStream(File root) {
    if (root.isFile()) return getXmlInputStream(root);
  ...
  @Override
  public void read(File root, IItem parent) throws Exception {
  ...
  private void configureParsers(boolean isIOS) {
    ...
    try {
      ...
      log.warn(
          "Failed to parse {} parameter from {}. Using default internal value: {}",
          ParsingTaskConfig.SOURCES_WITH_PARSERS,
          ParsingTaskConfig.CONF_FILE,
  ...
  private void addRootItem(IItem parent) throws InterruptedException {
    ...
    rootItem = DatasourceRegistry.get().createItem();
    ...
    DatasourceRegistry.get().addItem(rootItem);
  }
  ...
  private void addVirtualDecodedFolder() throws InterruptedException {
    ...
    decodedFolder = DatasourceRegistry.get().createItem();
    ...
    decodedFolder.setExtraAttribute(
        ExtraProperties.DATASOURCE_READER, UfedXmlReader.class.getSimpleName());

    pathToParent.put(decodedFolder.getPath(), decodedFolder);

    caseData.incDiscoveredEvidences(1);
    DatasourceRegistry.get().addItem(decodedFolder);
  }
  ...
  private class XMLContentHandler implements ContentHandler, UfedModelListener {
    ...
    HashSet<String> ignoreNameAttrs =
        new HashSet<>(
            Arrays.asList(
                "Tags", //$NON-NLS-1$
    ...
    HashSet<String> mergeInParentNode =
        new HashSet<>(
            Arrays.asList(
                "Party", //$NON-NLS-1$
    ...
    private IItem getParent(String path) throws SAXException {
      ...
      if (idx < 1) return rootItem;

      String parentPath = path.substring(0, idx);
      IItem parent = pathToParent.get(parentPath);
      if (parent != null) return parent;

      parent = DatasourceRegistry.get().createItem();
      ...
      try {
        caseData.incDiscoveredEvidences(1);
        DatasourceRegistry.get().addItem(parent);
    ...
    public void startElement(String uri, String localName, String qName, Attributes atts)
      ...
      if (qName.equals("modelType")
          && StringUtils.equalsAny(
              atts.getValue("type"), "Chat", "InstantMessage", "Contact", "UserAccount", "Email")) {
      ...
      if (!listOnly) elements.add(qName);
      ...
      if (qName.equals("extractionInfo")) { // $NON-NLS-1$
        ...
        Long size = null;
        if (len != null) size = Long.valueOf(len.trim());
        ...
        boolean deleted =
            "deleted".equalsIgnoreCase(atts.getValue("deleted")); // $NON-NLS-1$ //$NON-NLS-2$
        ...
        if (prevNode.element.equals("modelType")) { // $NON-NLS-1$
          ...
          IItem item = createModelItem(atts);
          itemSeq.push(item);

        } else if (prevNode.element.equals("modelField")
            || prevNode.element.equals("multiModelField")) { // $NON-NLS-1$ //$NON-NLS-2$

          String type = atts.getValue("type"); // $NON-NLS-1$
          if (listOnly) {
            if (!mergeInParentNode.contains(type)) caseData.incDiscoveredEvidences(1);
            return;
          }
          ...
          if ("Location".equals(type)
              && ("FromPoint".equals(prevNameAtt)
                  || "ToPoint".equals(prevNameAtt))) // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
          ...
          item.setInputStreamFactory(new MetadataInputStreamFactory((Metadata) item.getMetadata()));
          item.setHash(""); // $NON-NLS-1$

          DatasourceRegistry.get().prepareForQueue(parent, caseData);
          item.setParent(parent);
          if (!mergeInParentNode.contains(type)) parent.setHasChildren(true);

          boolean deleted =
              "deleted"
                  .equalsIgnoreCase(atts.getValue("deleted_state")); // $NON-NLS-1$ //$NON-NLS-2$
          item.setDeleted(deleted);
    ...
    private IItem createModelItem(Attributes atts) throws SAXException {
      IItem item = DatasourceRegistry.get().createItem();
      item.setExtraAttribute(
          ExtraProperties.DATASOURCE_READER, UfedXmlReader.class.getSimpleName());
    ...
    private void fillCommonMeta(IItem item, Attributes atts) {
      ...
      ((Metadata) item.getMetadata())
          .add(ExtraProperties.UFED_META_PREFIX + "extractionName", extractionName); // $NON-NLS-1$
      ...
      for (int i = 0; i < atts.getLength(); i++) {
        ...
        if (!ignoreAttrs.contains(attName)) {
          ...
          ((Metadata) item.getMetadata()).add(ExtraProperties.UFED_META_PREFIX + attName, value);
        }
      }
      if (item.getMetadataValue(UFED_ID) != null) {
        item.setIdInDataSource(item.getMetadataValue(UFED_ID));
      } else {
    ...
    public void endElement(String uri, String localName, String qName) throws SAXException {
      ...
      IItem item = null;
      if (!itemSeq.empty()) item = itemSeq.peek();

      XmlNode parentNode = null;
      if (!nodeSeq.empty()) parentNode = nodeSeq.peek();
      ...
      if (("MSISDN".equals(nameAttr) || "LastUsedMSISDN".equals(nameAttr))
          && parentNode != null
        ...
        if ("Tags".equals(nameAttr)
            && "Configuration".equals(chars.toString())) { // $NON-NLS-1$ //$NON-NLS-2$
          ...
          ((Metadata) item.getMetadata()).add(LOCAL_PATH_META, normalizedPath);

          // Add key to map item id to "Local Path"
          ufedFileIdToLocalPath.put(item.getMetadataValue(UFED_ID), normalizedPath);

          if (item.getPath().endsWith("wireless/Library/Databases/CellularUsage.db")) {
            parseIphoneSimSwitch(item);
          }

        } else if (!ignoreNameAttrs.contains(nameAttr)
            && !nameAttr.toLowerCase().startsWith("exif")) // $NON-NLS-1$
        if (item != null && !chars.toString().trim().isEmpty())
            ((Metadata) item.getMetadata())
                .add(ExtraProperties.UFED_META_PREFIX + nameAttr, chars.toString().trim());
        ...
        try {
          ...
          if (!value.isEmpty()) {
            ...
            item.setAccessDate(parseDate(value));
              ...
              ((Metadata) item.getMetadata())
                  .add(ExtraProperties.UFED_META_PREFIX + nameAttr, value);
          }
        ...
        if (parentNode.element.equals("field")
            || parentNode.element.equals("multiField")) { // $NON-NLS-1$ //$NON-NLS-2$
          ...
          if (!ignoreNameAttrs.contains(parentNameAttr)) {
            ...
            try {
                ((Metadata) item.getMetadata()).add(meta, out.format(parseDate(value)));
              } catch (ParseException e) {
                throw new SAXException(e);
              }
              ...
              if ("Base64String".equalsIgnoreCase(currentNode.atts.get("format"))) {
                ...
                if (isString) {
                  ...
                  ((Metadata) item.getMetadata()).add(meta + ":format", "base64");
                }
              }
              ((Metadata) item.getMetadata()).add(meta, value);
            }
          }
        }
      } else if (qName.equals("targetid")
          && parentNode.element.equals("jumptargets")) { // $NON-NLS-1$ //$NON-NLS-2$
        ((Metadata) item.getMetadata())
            .add(ExtraProperties.UFED_JUMP_TARGETS, chars.toString().trim());

      } else if (qName.equals("ownerid") && parentNode.element.equals("sourcemodels")) {
        ((Metadata) item.getMetadata())
            .add(ExtraProperties.UFED_SOURCE_MODELS, chars.toString().trim());
        ...
        String md5 = item.getMetadataValue(ExtraProperties.UFED_META_PREFIX + "MD5");
        String localPath = item.getMetadataValue(LOCAL_PATH_META);
        if (StringUtils.isNotBlank(md5) && md5.length() == 32) {
          if (item.getInputStreamFactory() != null
              && !md5ToLocalPath.containsKey(md5)
        ...
        if (!itemSeq.isEmpty()) {
          ...
          if (parentItem.getMediaType() != null
              && UFED_CONTACTPHOTO_MIME.equals(
                  ((MediaType) parentItem.getMediaType()).getSubtype())) {
            String[] split =
                item.getIdInDataSource().split(UFDRInputStreamFactory.UFDR_PATH_PREFIX);
            String exportPath = split[split.length - 1];
            ((Metadata) parentItem.getMetadata()).set(AVATAR_PATH_META, exportPath);
            caseData.incDiscoveredEvidences(-1);
            merged = true;
          }
        }

        String sceneClassifications = item.getMetadataValue(UFED_NATIVE_SCENE_CLASSIFICATION);
        if (sceneClassifications != null) {
          ((Metadata) item.getMetadata()).remove(UFED_NATIVE_SCENE_CLASSIFICATION);
          for (String sceneClass : sceneClassifications.split(",")) {
            sceneClass = MetadataUtil.normalizeTerm(sceneClass);
            if (!sceneClass.isEmpty()) {
              ((Metadata) item.getMetadata()).add(UFED_NATIVE_SCENE_CLASSIFICATION, sceneClass);
            }
          }
        }
        ...
        if (!merged) {
          ...
          try {
            DatasourceRegistry.get().addItem(item);
          } catch (Exception e) {
            throw new SAXException(e);
          }
        }
        ...
        if ("Call".equals(type)
            || "SMS".equals(type) // $NON-NLS-4$
            || "MMS".equals(type)) { // $NON-NLS-1$
          String date =
              item.getMetadataValue(ExtraProperties.UFED_META_PREFIX + "TimeStamp"); // $NON-NLS-1$
          ((Metadata) item.getMetadata())
              .remove(ExtraProperties.UFED_META_PREFIX + "TimeStamp"); // $NON-NLS-1$
          ((Metadata) item.getMetadata()).set(ExtraProperties.COMMUNICATION_DATE, date);

          String subject =
              item.getMetadataValue(ExtraProperties.UFED_META_PREFIX + "Subject"); // $NON-NLS-1$
          ((Metadata) item.getMetadata())
              .remove(ExtraProperties.UFED_META_PREFIX + "Subject"); // $NON-NLS-1$
          ((Metadata) item.getMetadata()).set(ExtraProperties.MESSAGE_SUBJECT, subject);

          String body =
              item.getMetadataValue(ExtraProperties.UFED_META_PREFIX + "Body"); // $NON-NLS-1$
          ((Metadata) item.getMetadata())
              .remove(ExtraProperties.UFED_META_PREFIX + "Body"); // $NON-NLS-1$
          if (body == null) {
            body =
                item.getMetadataValue(ExtraProperties.UFED_META_PREFIX + "Snippet"); // $NON-NLS-1$
            ((Metadata) item.getMetadata())
                .remove(ExtraProperties.UFED_META_PREFIX + "Snippet"); // $NON-NLS-1$
          }
          ((Metadata) item.getMetadata()).set(ExtraProperties.MESSAGE_BODY, body);
        }
        if (mergeInParentNode.contains(type) && !itemSeq.empty()) {
          IItem parentItem = itemSeq.peek();
          if ("Party".equals(type)) { // $NON-NLS-1$
            String role =
                item.getMetadataValue(ExtraProperties.UFED_META_PREFIX + "Role"); // $NON-NLS-1$
            String parentNameAttr = parentNode.atts.get("name"); // $NON-NLS-1$
            if (role == null || role.equals("General")) // $NON-NLS-1$
            role = parentNameAttr;
            if (role.equals("To")
                && (parentNameAttr.equals("Bcc")
                    || parentNameAttr.equals("Cc"))) // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            role = parentNameAttr;
            ...
            String identifier =
                item.getMetadataValue(
                    ExtraProperties.UFED_META_PREFIX + "Identifier"); // $NON-NLS-1$
            String name =
                item.getMetadataValue(ExtraProperties.UFED_META_PREFIX + "Name"); // $NON-NLS-1$
            String value =
                name == null || name.equals(identifier)
                    ? identifier
                    : identifier == null
                        ? name
            ...
            if (value != null) {
              ...
              ((Metadata) parentItem.getMetadata()).add(ExtraProperties.COMMUNICATION_FROM, value);
              else if ("To".equalsIgnoreCase(role)) // $NON-NLS-1$
              ((Metadata) parentItem.getMetadata()).add(ExtraProperties.COMMUNICATION_TO, value);
              else if ("Cc".equalsIgnoreCase(role)) // $NON-NLS-1$
              ((Metadata) parentItem.getMetadata()).add(Message.MESSAGE_CC, value);
              else if ("Bcc".equalsIgnoreCase(role)) // $NON-NLS-1$
              ((Metadata) parentItem.getMetadata()).add(Message.MESSAGE_BCC, value);
              else
                ((Metadata) parentItem.getMetadata())
                    .add(ExtraProperties.UFED_META_PREFIX + role, value);
            }
            boolean isOwner =
                Boolean.valueOf(
                    item.getMetadataValue(
                        ExtraProperties.UFED_META_PREFIX + "IsPhoneOwner")); // $NON-NLS-1$
            if (value != null && isOwner) { // $NON-NLS-1$
              ownerParties.add(value);
              if (parentItem.getMediaTypeString().contains("chat"))
                ((Metadata) parentItem.getMetadata()).add(META_PHONE_OWNER, value);
            }
            if (isOwner && "From".equals(role)) // $NON-NLS-1$
            ((Metadata) parentItem.getMetadata()).add(META_FROM_OWNER, Boolean.TRUE.toString());

          } else if ("PhoneNumber".equals(type)
              || "EmailAddress".equals(type)) { // $NON-NLS-1$ //$NON-NLS-2$
            String category =
                item.getMetadataValue(ExtraProperties.UFED_META_PREFIX + "Category"); // $NON-NLS-1$
            String value =
                item.getMetadataValue(ExtraProperties.UFED_META_PREFIX + "Value"); // $NON-NLS-1$
            if (value != null && !value.trim().isEmpty()) {
              if (category != null) value += " (" + category + ")"; // $NON-NLS-1$ //$NON-NLS-2$
              ((Metadata) parentItem.getMetadata())
                  .add(ExtraProperties.UFED_META_PREFIX + type, value);
            }
            ...
            ((Metadata) parentItem.getMetadata()).set(lat, item.getMetadataValue(lat));
            ((Metadata) parentItem.getMetadata()).set(lon, item.getMetadataValue(lon));
            ((Metadata) parentItem.getMetadata())
                .set(
                    ExtraProperties.UFED_COORDINATE_ID,
                    item.getMetadataValue(ExtraProperties.UFED_ID));
            ...
            if (value != null) {
              String position =
                  item.getMetadataValue(
                      ExtraProperties.UFED_META_PREFIX + "Position"); // $NON-NLS-1$
              if (position != null) value += " (" + position + ")"; // $NON-NLS-1$ //$NON-NLS-2$
              ((Metadata) parentItem.getMetadata())
                  .add(ExtraProperties.UFED_META_PREFIX + type, value);
            }
            ...
            String avatarPath = item.getMetadataValue(AVATAR_PATH_META);
            if (avatarPath != null) {
              avatarPath = normalizePaths(avatarPath);
              ((Metadata) parentItem.getMetadata()).add(AVATAR_PATH_META, avatarPath);
            }
          } else if ("StreetAddress".equals(type)) { // $NON-NLS-1$
            for (String meta : ((Metadata) item.getMetadata()).names()) {
              if (meta.equals(ExtraProperties.UFED_META_PREFIX + "id")) continue;
              String[] vals = item.getMetadataValues(meta);
              for (String val : vals) ((Metadata) parentItem.getMetadata()).add(meta, val);
            }
            ...
            for (String meta : ((Metadata) item.getMetadata()).names()) {
              ...
              String[] vals = item.getMetadataValues(meta);
              for (String val : vals) {
                ((Metadata) parentItem.getMetadata()).add(meta, val);
              }
            }
          } else if ("KeyValueModel".equals(type)) {
            String key = item.getMetadataValue(ExtraProperties.UFED_META_PREFIX + "Key");
            if (key != null) {
              String value = item.getMetadataValue(ExtraProperties.UFED_META_PREFIX + "Value");
              if (value != null) {
                ((Metadata) parentItem.getMetadata())
                    .add(ExtraProperties.UFED_META_PREFIX + key, value);
              }
            }
          } else if ("MessageLabel".equals(type)) {
            String prop = ExtraProperties.UFED_META_PREFIX + "Label";
            String label = item.getMetadataValue(prop);
            if (label != null) {
              ((Metadata) parentItem.getMetadata()).add(prop, label);
            }
          } else if ("ContactEntry".equals(type)
              || "ProfilePicture".equals(type)
              || "WebAddress".equals(type)) {
            String value = item.getMetadataValue(ExtraProperties.UFED_META_PREFIX + "Value");
            if (value != null) {
              String category =
                  item.getMetadataValue(ExtraProperties.UFED_META_PREFIX + "Category");
              if (category != null) {
                value += " (" + category + ")";
              }
              ((Metadata) parentItem.getMetadata())
            ...
            String reaction =
                item.getMetadataValue(ExtraProperties.UFED_META_PREFIX + "ReactionType");
            if (reaction != null) {
              ((Metadata) parentItem.getMetadata())
                  .add(ExtraProperties.UFED_META_PREFIX + "Reaction", reaction);
            }
          } else if ("Price".equals(type)) {
            String prop = ExtraProperties.UFED_META_PREFIX + "Amount";
            String amount = item.getMetadataValue(prop);
            if (amount != null) {
              try {
              ...
              if (amount != null) {
                ((Metadata) parentItem.getMetadata()).add(prop, amount);
              }
            }
            prop = ExtraProperties.UFED_META_PREFIX + "Currency";
            String currency = item.getMetadataValue(prop);
            if (currency != null) {
              ((Metadata) parentItem.getMetadata()).add(prop, currency);
            }
          } else if ("QuotedMessageData".equals(type)) {
            String refId = item.getMetadataValue(ExtraProperties.UFED_META_PREFIX + "ReferenceId");
            if (refId != null) {
              ((Metadata) parentItem.getMetadata())
                  .add(ExtraProperties.UFED_META_PREFIX + "QuotedReferenceId", refId);
            }
          }
        } else {
          processItem(item);
      ...
      if ("Device Info".equals(currentNode.atts.get("section"))) {
        if (!deviceInfoData.isEmpty()) {
          item = DatasourceRegistry.get().createItem();
          item.setExtraAttribute(
              ExtraProperties.DATASOURCE_READER, UfedXmlReader.class.getSimpleName());
    ...
    private void processItem(IItem item) throws SAXException {
      try {
        caseData.incDiscoveredVolume(item.getLength());
    ...
    private void fillMissingInfo(IItem item) {
      String from = item.getMetadataValue(ExtraProperties.COMMUNICATION_FROM);
      String[] to = item.getMetadataValues(ExtraProperties.COMMUNICATION_TO);
      boolean fromOwner =
          Boolean.valueOf(item.getMetadataValue(ExtraProperties.UFED_META_PREFIX + "fromOwner"));
      if (to == null || to.length != 1) {
        if (item.getMediaType() != null
          ...
          if (to != null && to.length > 0) {
            ...
            String[] parties =
                parentChat.getMetadataValues(ExtraProperties.UFED_META_PREFIX + "Participants");
            for (String party : parties) {
              if ((from != null && !party.equals(from))
                  || (fromOwner && !ownerParties.contains(party))) toList.add(party);
            }
          }
          if (toList.size() == 1) {
            ((Metadata) item.getMetadata()).set(toProperty, toList.get(0));
          } else if (toList.size() > 1) {
            ((Metadata) item.getMetadata()).set(toProperty, parentChat.getName());
            ((Metadata) item.getMetadata()).set(ExtraProperties.IS_GROUP_MESSAGE, "true");
          }
        }
      }
      if (!msisdns.isEmpty()
          && (MediaTypes.UFED_CALL_MIME.equals(item.getMediaType())
        ...
        String direction = item.getMetadataValue(ExtraProperties.UFED_META_PREFIX + "Type");
        String status = item.getMetadataValue(ExtraProperties.UFED_META_PREFIX + "Status");
        if (item.getMetadataValue(ExtraProperties.COMMUNICATION_TO) == null
            && likelyMsisdn != null
            && ("Incoming".equals(direction)
                || "Missed".equals(direction)
                || (!fromOwner && ("Read".equals(status) || "Unread".equals(status))))) {
          ((Metadata) item.getMetadata()).set(ExtraProperties.COMMUNICATION_TO, likelyMsisdn);
        } else if (from == null
            && likelyMsisdn != null
            && ("Outgoing".equals(direction)
                || fromOwner
                || "Sent".equals(status)
                || "Unsent".equals(status))) {
          ((Metadata) item.getMetadata()).set(ExtraProperties.COMMUNICATION_FROM, likelyMsisdn);
        }
      }
    }
    ...
    private String getLikelyMSISDN(IItem item) {
      ...
      Date date = ((Metadata) item.getMetadata()).getDate(time);
    ...
    private void parseIphoneSimSwitch(IItem item) {
      ...
      try {
        ...
        try (Connection conn =
          ...
          String sql =
              "SELECT subscriber_mdn, last_update_time FROM subscriber_info"; //$NON-NLS-1$
    ...
    private void setContent(IItem item, String path) {
      item.setMediaType(null);
      item.setHash(null);
      item.setInputStreamFactory(null);
      if (path == null) return;
      String ufedId = item.getMetadataValue(UFED_ID);
      ...
      if (ufdrFile == null) {
        ...
        try {
          if (getUISF().entryExists(path)) {
            ...
            String id =
                item.getIdInDataSource() != null ? item.getIdInDataSource() + "_" + path : path;
    ...
    private void setMediaResult(IItem item) {
      String ufedId = item.getMetadataValue(UFED_ID);
      if (ufedId == null) {
        return;
      }
      ...
      if (mediaResults != null) {
        ...
        if (mediaResults.classifications != null && !mediaResults.classifications.isEmpty()) {
          ...
          mediaResults.classifications.stream()
              .forEach(
                  classification -> {
                    ...
                    double score =
                        TaskRuntime.invokeVideoScoreList(
                            ...
                            entry.getValue().stream()
                                ...
                                .max()
                                .orElse(0d));
                    finalScorePerCat.put(entry.getKey(), (float) score);
                  });
        ...
        finalScorePerCat.entrySet().stream()
            .forEach(
                e ->
                    item.setExtraAttribute(
                        MEDIA_CLASSES_SCORE_PREFIX + e.getKey(), shorten(e.getValue())));
    ...
    private String handleAttachment(IItem item) {
      String name =
          item.getMetadataValue(ExtraProperties.UFED_META_PREFIX + "Filename"); // $NON-NLS-1$
      if (name != null) updateName(item, name);
      String extracted_path = item.getMetadataValue(ATTACH_PATH_META);
      String ufedId = null;
      if (extracted_path != null) {
        extracted_path = normalizePaths(extracted_path);
        ufedId = ufdrPathToUfedId.get(extracted_path);

        // If extracted path doesn't exist, replace non-existent extracted path by attached file's
        // local path
        try {
        ...
        if (ufedId == null
            && ufedFileIdToLocalPath.containsKey(item.getMetadataValue(FILE_ID_ATTR))) {
          ufedId = item.getMetadataValue(FILE_ID_ATTR);
        }
      }
    ...
    private void createDeviceInfoPreview(IItem deviceInfo) {
      Collections.sort(
          deviceInfoData,
      ...
      try (BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(baos, "UTF-8"))) {
        ...
        bw.write(
            "<th class=\"h\" colspan=\"2\">"
                + Messages.getString("UfedXmlReader.DeviceInfo")
                + "</th>");
    ...
    public void onModelStarted(BaseModel model, Attributes attr) {
      ...
      try {
        IItem item = createModelItem(attr);
