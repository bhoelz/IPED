...
public class UfedModelHandler extends DefaultHandler {
  ...
  public static UfedModelHandler create(
      XMLReader xmlReader,
  ...
  public UfedModelHandler(
      XMLReader xmlReader, ContentHandler parentHandler, UfedModelListener listener) {
  ...
  public void startElement(String uri, String localName, String qName, Attributes attributes)
    ...
    if ("model".equalsIgnoreCase(qName)) {
      ...
      modelStack.push(newModel);
        ...
        qName, "field", "modelField", "multiModelField", "nodeField")) {
  ...
  private Object parseValue(String value, String type) {
    ...
    try {
      ...
      log.error(
          "Could not parse value '{}' for type '{}'. Defaulting to String. [{}]",
          value,
          type,
          e.getMessage());
      return value;
  ...
  private BaseModel createModelByType(String type) {
    ...
    switch (type) {
      ...
      case "Party":
        return new Party();
      case "QuotedMessageData":
        return new QuotedMessageData();
      case "Attachment":
        return new Attachment();
      case "ContactPhoto":
        return new ContactPhoto();
      case "MessageLabel":
      ...
      case "ForwardedMessageData":
        return new ForwardedMessageData();
      case "ReplyMessageData":
      ...
      default:
        {
          if (!modelStack.isEmpty()
              && modelStack.peek() instanceof Accountable
              && !fieldNameStack.isEmpty()
              && "Entries".equals(fieldNameStack.peek())) {
    ...
    if (parent instanceof Email) {
      ...
      if ("From".equals(fieldName) && child instanceof Party) {
        ...
        if (child instanceof MessageLabel) {
          ...
          log.error(
              "Unknown InstantMessageExtraData child '{}' => {} (id={}). Ignoring...",
              fieldName,
      ...
      if ("Entries".equals(fieldName) && child instanceof ContactEntry) {
        accountable
            .getContactEntries()
            .computeIfAbsent(child.getModelType(), k -> new ArrayList<>())
            .add((ContactEntry) child);
      } else if ("Photos".equals(fieldName) && child instanceof ContactPhoto) {
  ...
  private static class UfedModelHandlerListOnly extends UfedModelHandler {
    ...
    public UfedModelHandlerListOnly(
        XMLReader xmlReader,
        ContentHandler parentHandler,
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes)
      ...
      if ("model".equalsIgnoreCase(qName)
          || StringUtils.equalsAnyIgnoreCase(qName, "field", "value") && modelStack.size() == 1) {
    ...
    public void characters(char[] ch, int start, int length) throws SAXException {
      if (modelStack.size() == 1
          && !fieldNameStack.isEmpty()
          && fieldNameStack.peek().equals("Source")) {
