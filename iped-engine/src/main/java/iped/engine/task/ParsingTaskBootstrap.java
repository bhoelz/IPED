    ...
    public static int configure(ConfigurationManager configurationManager) {
        ...
        if (parsingConfig.isEnableExternalParsing()) {
            ...
            if (maxExpandingContainers == 0) {
                throw new IPEDException("You must have a minimum of 2 external parsing processes! Adjust the '"
                        + ParsingTaskConfig.NUM_EXTERNAL_PARSERS + "' option.");
            }
            ...
            if (localConfig.getNumThreads() < 2) {
                throw new IPEDException("You should have at least 2 processing workers! Please adjust '"
                        + LocalConfig.NUM_THREADS + "' option.");
            }
        ...
        String value = parsingConfig.isEnableExternalParsing() ? Boolean.FALSE.toString()
