...
public class MapViewer implements ResultSetViewer, TableModelListener, ListSelectionListener {
  ...
  public void tableChanged(TableModelEvent e) {
    ...
    if (e.getColumn() == 1) { // if the event was fired by checkbox on column 1
      if (!desabilitaTemp) { // the change was fired by an event inside the map itself, so, do not
        // repeat
        // operation
  ...
  public void valueChanged(ListSelectionEvent e) {
    if (e.getValueIsAdjusting() || resultsTable.getRowSorter() == null || desabilitaTemp) return;
