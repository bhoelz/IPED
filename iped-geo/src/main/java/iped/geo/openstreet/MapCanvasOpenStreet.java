...
public class MapCanvasOpenStreet extends AbstractMapCanvas {
    ...
    Platform.runLater(
        new Runnable() {
          public void run() {
            ...
            browser.setOnMouseReleased(
                e -> {
                  dragging = false;
                });

            browser.setOnMouseDragged(
                e -> {
                  if (dragging) {
            ...
            webEngine.setOnError(
                new EventHandler<WebErrorEvent>() {
            ...
            onLoadChange =
                new ChangeListener<State>() {
                  @Override
                  public void changed(
                      ObservableValue<? extends State> observable, State oldState, State newState) {
  ...
  public void load() {
    try {
      ...
      String b64_selecionado =
          "data:image/png;base64,"
              + Base64.getEncoder() // $NON-NLS-1$
                  .encodeToString(
                      IOUtils.toByteArray(
                          getClass()
                              .getResourceAsStream("marcador_selecionado.png"))); // $NON-NLS-1$
      String b64_selecionado_m =
          "data:image/png;base64,"
              + Base64.getEncoder() // $NON-NLS-1$
                  .encodeToString(
                      IOUtils.toByteArray(
                          getClass()
                              .getResourceAsStream("marcador_selecionado_m.png"))); // $NON-NLS-1$
      String b64_normal =
          "data:image/png;base64,"
              + Base64.getEncoder() // $NON-NLS-1$
                  .encodeToString(
                      IOUtils.toByteArray(
                          getClass().getResourceAsStream("marcador_normal.png"))); // $NON-NLS-1$
      String b64_marcado =
          "data:image/png;base64,"
              + Base64.getEncoder() // $NON-NLS-1$
                  .encodeToString(
                      IOUtils.toByteArray(
                          getClass().getResourceAsStream("marcador_marcado.png"))); // $NON-NLS-1$

      String kml =
          "<?xml version=\"1.0\" encoding=\"UTF-8\"?><kml xmlns=\"http://www.opengis.net/kml/2.2\">";
      kml += "<Document>";
      kml +=
          "<Style id=\"item\"><IconStyle><Icon><href>{{icone_base64}}</href></Icon></IconStyle></Style>";
      kml +=
          "<Style id=\"itemSelecionado\"><IconStyle><Icon><href>{{b64_selecionado}}</href></Icon></IconStyle></Style>";
      kml +=
          "<Style id=\"itemSelecionadoMarcado\"><IconStyle><Icon><href>{{b64_selecionado_m}}</href></Icon></IconStyle></Style>";
      kml +=
          "<Style id=\"itemMarcado\"><IconStyle><Icon><href>{{b64_marcado}}</href></Icon></IconStyle></Style>";
  ...
  public String getMainHtml() throws IOException {
    ...
    String html =
        IOUtils.toString(
            getClass().getResourceAsStream("main.html"), "UTF-8"); // $NON-NLS-1$ //$NON-NLS-2$
    String js =
        IOUtils.toString(
            getClass().getResourceAsStream("L.KML.js"), "UTF-8"); // $NON-NLS-1$ //$NON-NLS-2$
    String markerclusterjs =
        IOUtils.toString(
            getClass().getResourceAsStream("leaflet.markercluster.js"), // $NON-NLS-1$
            "UTF-8"); //$NON-NLS-1$
    String leafletarrowheads =
        IOUtils.toString(
            getClass().getResourceAsStream("leaflet-arrowheads.js"), // $NON-NLS-1$
            "UTF-8"); //$NON-NLS-1$
    String leafletgeometryutil =
        IOUtils.toString(
            getClass().getResourceAsStream("leaflet.geometryutil.js"), // $NON-NLS-1$
            "UTF-8"); //$NON-NLS-1$
    ...
    String layers_img =
        "data:image/png;base64,"
            + Base64.getEncoder() // $NON-NLS-1$
                .encodeToString(
                    IOUtils.toByteArray(
                        getClass().getResourceAsStream("layers.png"))); // $NON-NLS-1$
    ...
    html =
        html.replace(
            "{{googlemaps_scripts}}",
            "<script id=\"mapsapi\" src=\"https://maps.googleapis.com/maps/api/js?key="
                + apikey
                + "\" async defer></script>\n"
          ...
          new Runnable() {
            public void run() {
              ...
              for (int i = 0; i < marks.length; i++) {
                ...
                try {
                  script.append(
                      "track.selecionaMarcador([\"" + marks[i] + "\"],'" + b + "', false);");
                } catch (Exception e) {
                  e.printStackTrace();
                }
              }
              if (marcadorselecionado) {
                webEngine.executeScript(
                    script.toString()); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                try {
            ...
            public void run() {
              try {
                ...
                if (hasCheckedMarker) {
                  ...
                  webEngine.executeScript(
                      scriptCheck.toString()); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      ...
      Platform.runLater(
          new Runnable() {
            public void run() {
              try {
                webEngine.executeScript(
                    "track.updateLeadMarker(\"" + leadSelectionToApplyCopy + "\");");
  ...
  public void updateUI() {
    ...
    // Dark theme disabled for now until it is optimized, see
    ...
    Platform.runLater(
        new Runnable() {
          public void run() {
            try {
              webEngine.executeScript(
                  "asyncCreatePaths();track.setAllRange('"
                      + minlongit
                      + "','"
                      + minlat
                      + "','"
  ...
  public boolean setTileServerUrl(String url) {
    ...
    if (url.contains("googleapis")
        && url.contains("key")
        && !url.substring(url.indexOf("key=")).equals(lastGoogleApiKey)) {
      lastGoogleApiKey = url.substring(url.indexOf("key="));
      return true;
    } else {
      if (isLoaded()) {
          ...
          public void run() {
            try {
              if (jsonFeature != null && jsonFeature.length() > 3) {
                webEngine.executeScript(
                    "try{track.hideLastFeature();track.drawFeature("
                        + jsonFeature
                        + ");}catch(e){alert(e);}");
              } else {
          ...
          public void run() {
            try {
              if (jsonFeature.length > 0) {
                ...
                if (str.length() > 3) {
                  webEngine.executeScript(
                      "try{track.hideLastFeature();+" + str.toString() + "}catch(e){alert(e);}");
                } else {
                  webEngine.executeScript("try{track.hideLastFeature();}catch(e){alert(e);}");
                }
              }
