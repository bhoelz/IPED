 ...
 *
 * <p>Reads {@code ExtraProperties.LOCATIONS} values ({@code "lat;lon"} pairs) from the item
 * metadata map (via the reflective {@code IItem.getMetadataMap()} API so that no compile dependency
 * on Tika is required from this layer). Altitude is read from {@code
 * ExtraProperties.COMMON_META_PREFIX + "altitude"}.
 *
 * <p><strong>UI-toolkit free:</strong> this class imports no {@code java.awt}, {@code javax.swing},
 * or {@code javafx} types and can be loaded in a headless JVM.
 */
public class GeoDataService implements IGeoDataService {
  ...
  @Override
  public List<GeoFeature> extractFeatures(IIPEDSource source, Iterable<? extends IItemId> itemIds) {
    ...
    for (IItemId itemId : itemIds) {
      ...
      try {
        ...
        log.warn(
            "Skipping geo extraction for item {}:{} — {}",
            itemId.getSourceId(),
            itemId.getId(),
            e.getMessage());
      }
