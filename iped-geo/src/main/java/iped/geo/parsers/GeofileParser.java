...
public class GeofileParser extends AbstractParser {
  ...
  public void parse(
    ...
    try {
      ...
      List<Object> featureList =
          FeatureListFactoryRegister.getFeatureList(mimeType).parseFeatureList(file);
      ...
      for (Iterator<Object> iterator = featureList.iterator(); iterator.hasNext(); ) {
        ...
        if (o instanceof Folder) {
          ...
          virtualId =
              recursiveFolderParse(
                  virtualId, folder, handler, metadata, extractor, context, virtualId);
        }
      }
    } catch (Throwable e) {
  ...
  public int recursiveFolderParse(
      int parentId,
      Folder folder,
    ...
    for (Iterator<Object> iterator = featureList.iterator(); iterator.hasNext(); ) {
      ...
      if (o instanceof Folder) {
        ...
        virtualId =
            recursiveFolderParse(
                virtualId, subfolder, handler, metadata, extractor, context, virtualId);
      }
    }
    return virtualId;
  }

  private int folderParser(
      Folder folder,
      int parentId,
      ContentHandler handler,
      Metadata metadata,
      ...
      try {
        ...
        if (g instanceof Point) {
          ...
          feature.setAttribute(
              "description",
              StringEscapeUtils.escapeEcmaScript(feature.getAttribute("description").toString()));
          fjson.writeFeature(feature, writer);
