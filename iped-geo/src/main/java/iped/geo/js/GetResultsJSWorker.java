...
public class GetResultsJSWorker extends iped.viewers.api.CancelableWorker<KMLResult, Integer> {
    ...
    if (consumer != null) {
      ...
      try {
        ...
        if (lastResultBitmap != null) {
          ...
          result[1] =
              createCasesEmptyBitmapArray(msource); // creates an empty bitmap array to avoid NPE
        }
      } catch (Exception e) {
    ...
    try {
      ...
      for (int row = 0; row < results.getLength(); row++) {
        ...
        if (row % batchSize == 0) {
          ...
          sem.acquire(
              Math.min(batchSize, results.getLength() - (((int) row / batchSize) * batchSize)));
        ...
        final IItemId item =
            results.getItem(app.getResultsTable().convertRowIndexToModel(finalRow));
              ...
              public void run() {
                try {
                  ...
                  if (locations != null && locations.length == 1) {
                    ...
                    String gid =
                        item.getSourceId() + "_" + item.getId(); // $NON-NLS-1$ //$NON-NLS-2$
  ...
  private synchronized void updateViewableRegion(String longit, String lat) {
    try {
