...
public class KMLResult {
  ...
  public void saveKML() {
    if (fDialog == null)
      fDialog =
          guiProvider.createFileDialog(
              Messages.getString("KMLResult.Save"), FileDialog.SAVE); // $NON-NLS-1$
  ...
  public static String converteCoordFormat(String coord) {
    ...
    double dec =
        (double) Integer.parseInt(min) * (double) 60
            + Double.parseDouble(resto.replace(",", ".")); // $NON-NLS-1$ //$NON-NLS-2$
    if (grau > 0) {
