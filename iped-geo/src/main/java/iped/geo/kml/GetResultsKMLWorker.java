...
public class GetResultsKMLWorker extends iped.viewers.api.CancelableWorker<KMLResult, Integer> {
  ...
  protected KMLResult doInBackground() throws Exception {
    ...
    try {
      ...
      kml.append(
          "<kml xmlns=\"http://www.opengis.net/kml/2.2\" xmlns:gx=\"http://www.google.com/kml/ext/2.2\" >"); //$NON-NLS-1$
      kml.append("<Document>"); // $NON-NLS-1$
      kml.append(
          "<name>"
              + Messages.getString("KMLResult.SearchResults")
      ...
      kml.append(
          "<Style id=\"basico\"><BalloonStyle><![CDATA[" //$NON-NLS-1$
              + " $[name] <br/> $[description] <br/> "
              + Messages.getString("KMLResult.ShowInTree") // $NON-NLS-1$ //$NON-NLS-2$
              + "]]>" //$NON-NLS-1$
      ...
      kml.append(
          "<name>"
              + Messages.getString("KMLResult.Results")
              + "</name>"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      ...
      for (int row = 0; row < results.getLength(); row++) {
        ...
        doc =
            ((IndexSearcher) app.getIPEDSource().getIndexSearcherHandle())
                .storedFields()
                .document(luceneId);
        ...
        if (locations != null && locations.length == 1) {
          ...
          for (String location : locations) {
            ...
            longit = locs[1].trim();
            generateLocationKML(
                tourPlayList, kml, coluna, doc, df, row, item, lat, longit, alt, ++subitem);
  ...
  private void generateLocationKML(
      StringBuilder tourPlayList,
      StringBuilder outerKml,
      String coluna,
    ...
    if (progress != null)
      progress.setString(
          Messages.getString("KMLResult.LoadingGPSData")
              + ": "
              + (++itemsWithGPS)); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    if (subitem < 0) {
      ...
      gid =
          "marker_"
              ...
              + item.getId()
              + "_"
              + subitem; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    }
    ...
    kml.append(
        "<name>" + htmlFormat(doc.get(BasicProps.NAME)) + "</name>"); // $NON-NLS-1$ //$NON-NLS-2$
    if (!BasicProps.ID.equals(coluna))
      kml.append(
          "<description>"
              + htmlFormat(coluna)
              + ":"
              + htmlFormat(doc.get(coluna))
              + "</description>"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    else
      kml.append(
          "<description>"
    ...
    if (alt == null) {
      kml.append(
          "<Point><coordinates>"
              ...
              + ","
              + lat
              + ",0</coordinates></Point>"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    } else {
      kml.append(
          "<Point><altitudemode>absolute</altitudemode><extrude>1</extrude><coordinates>"
              + longit
              + "," //$NON-NLS-1$ //$NON-NLS-2$
              + lat
              + ","
              + alt
              + "</coordinates></Point>"); //$NON-NLS-1$
    }
    ...
    tourPlayList.append(
        "<gx:FlyTo>" //$NON-NLS-1$
            ...
            + " <Placemark targetId=\""
            + gid
            + "\"><gx:balloonVisibility>1</gx:balloonVisibility></Placemark>" //$NON-NLS-1$
            // //$NON-NLS-2$
            ...
            + " <Placemark targetId=\""
            + gid
            + "\"><gx:balloonVisibility>0</gx:balloonVisibility></Placemark>" //$NON-NLS-1$
            // //$NON-NLS-2$
            + " </Change></Update></gx:AnimatedUpdate>"); //$NON-NLS-1$
    ...
    for (int j = 0; j < colunas.length; j++) {
      if (!BasicProps.ID.equals(colunas[j]))
        kml.append(
            "<Data name=\""
                + htmlFormat(colunas[j])
                + "\"><value>"
                + htmlFormat(doc.get(colunas[j])) // $NON-NLS-1$ //$NON-NLS-2$
        ...
        kml.append(
            "<Data name=\""
                ...
                + "\"><value>"
                + gid
                + "</value></Data>"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    }
    ...
    kml.append(
        "<Data name=\"checked\"><value>"
            + checked
            + "</value></Data>"); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    kml.append(
        "<TimeSpan><begin>" + dataCriacao + "</begin></TimeSpan>"); // $NON-NLS-1$ //$NON-NLS-2$
  ...
  public static String resolveAltitude(Document doc) {
