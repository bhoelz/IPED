...
public class EventTranscriptParserTest {
  ...
  public void testEventranscriptHistory() throws IOException, SAXException, TikaException {
    ...
    assertTrue(
        tracker.histEventNames.contains("HJ_HistoryAddUrl")
            || tracker.histEventNames.contains("HJ_HistoryAddUrlEx"));
    assertTrue(tracker.pageTitles.contains("Search · \"eventtranscript.db\" \"query\""));
    assertTrue(
        tracker.urls.contains("https://github.com/search?q=%22eventtranscript.db%22&type=issues"));
    ...
    assertTrue(
        tracker
            ...
            .get(97)
            .contains(
                "https://www.google.com/search?q=%22inventoryapplicationadd%22&oq=%22inventoryapplicationadd"));
  ...
  public void testEventTranscriptInventoryApps() throws IOException, SAXException, TikaException {
    ...
    assertEquals(
        "%ProgramFiles% (x86)\\microsoft\\edgewebview\\application", tracker.rootDirPaths.get(0));
    ...
    assertEquals(
        "%ProgramFiles%\\windowsapps\\microsoft.microsoftedge.stable_104.0.1293.63_neutral__8wekyb3d8bbwe",
        tracker.rootDirPaths.get(2));
  ...
  public void testEventTranscriptAppInteract() throws IOException, SAXException, TikaException {
    ...
    assertEquals(
        "Microsoft.DiagnosticDataViewer_4.2007.21653.0_x64__8wekyb3d8bbwe!App",
        tracker.intAppNames.get(58));
  ...
  private static class EmbeddedEventTranscriptParser extends AbstractParser {
    ...
    public void parse(
      ...
      if (metadata
          .get(StandardParser.INDEXER_CONTENT_TYPE)
          .equals(EventTranscriptParser.EVENT_TRANSCRIPT_HIST_REG.toString())) {
      ...
      if (metadata
          .get(StandardParser.INDEXER_CONTENT_TYPE)
          .equals(EventTranscriptParser.EVENT_TRANSCRIPT_INVENTORY_APP_REG.toString())) {
      ...
      if (metadata
          .get(StandardParser.INDEXER_CONTENT_TYPE)
          .equals(EventTranscriptParser.EVENT_TRANSCRIPT_APP_INTERACT_REG.toString())) {
      ...
      if (metadata
          .get(StandardParser.INDEXER_CONTENT_TYPE)
          .equals(EventTranscriptParser.EVENT_TRANSCRIPT_DEVICES_REG.toString())) {
      ...
      if (metadata
          .get(StandardParser.INDEXER_CONTENT_TYPE)
          .equals(EventTranscriptParser.EVENT_TRANSCRIPT_CENSUS_REG.toString())) {
        censusEventNames.add(metadata.get("eventName"));
        censusTimesTamp.add(
            metadata.get(censusEventNames.get(censusEventNames.size() - 1) + "Event"));
