...
public class ThreemaParser extends SQLite3DBParser {
  ...
  public static final MediaType CHAT_STORAGE =
      MediaType.application("x-threema-chatstorage"); // $NON-NLS-1$

  public static final MediaType CHAT_STORAGE_F = MediaType.application("x-threema-chatstorage-f");

  public static final MediaType THREEMA_CHAT =
      MediaType.parse("application/x-threema-chat"); // $NON-NLS-1$

  public static final MediaType THREEMA_MESSAGE =
      MediaType.parse("message/x-threema-message"); // $NON-NLS-1$

  public static final MediaType THREEMA_ATTACHMENT =
      MediaType.parse("message/x-threema-attachment"); // $NON-NLS-1$

  public static final MediaType THREEMA_CALL =
      MediaType.parse("call/x-threema-call"); // $NON-NLS-1$
  ...
  public void parse(
    ...
    try (TemporaryResources tmp = new TemporaryResources()) {
      ...
      if (mimetype.equals(CHAT_STORAGE.toString())) {
        extractMediaFilesFromDatabase(
            stream, handler, metadata, context, new ExtractorIOSFactory(), tmp);
  ...
  private List<Chat> extractChatList(
      ...
      ExtractorFactory extractorIOSFactory,
    ...
    if (extractor.shouldParseEmbedded(metadata)) {
      ...
      try {
        ...
        Extractor threemaExtractor =
            extractorIOSFactory.createMessageExtractor(
                tmp, filePath, tempDbFile, account, recoverDeletedRecords);
        ...
        metadata.set(
            ParserConstants.INDEXER_CONTENT_TYPE, extractorIOSFactory.getType2().toString());
      }
    }
    return Collections.emptyList();
  }

  private void extractMediaFilesFromDatabase(
      InputStream stream,
      ContentHandler handler,
      Metadata metadata,
      ParseContext context,
    ...
    EmbeddedDocumentExtractor extractor =
        context.get(EmbeddedDocumentExtractor.class, new ParsingEmbeddedDocumentExtractor(context));
    List<Chat> chatList =
        extractChatList(stream, handler, metadata, context, new ExtractorIOSFactory(), tmp);
    for (Chat c : chatList) {
      ...
      for (Message m : c.getMessages()) {
        ...
        if (m.getData() != null) {
          try (InputStream is = new FileInputStream(m.getData())) {
            ...
            embedFileData.set(ParserConstants.INDEXER_CONTENT_TYPE, m.getMediaMime());
  ...
  private void parseThreemaMessages(
      InputStream stream,
      ContentHandler handler,
      Metadata metadata,
      ParseContext context,
    ...
    if (extractor.shouldParseEmbedded(metadata)) {
      try {
        ...
        else throw new TikaException("ThreemaParser Exception", e); // $NON-NLS-1$
      }
    }
  }
  ...
  private void extractMessages(
    ...
    for (Message m : messages) {
      ...
      meta.set(ParserConstants.INDEXER_CONTENT_TYPE, THREEMA_MESSAGE.toString());
      ...
      if (m.getMediaItem() != null && m.getMediaItem().getThumb() != null) {
        meta.set(
            ...
            Base64.getEncoder().encodeToString(m.getMediaItem().getThumb()));
      }
  ...
  private abstract static class ExtractorFactory {
    ...
    private void setConnectionParams(
        InputStream is, Metadata metadata, ParseContext context, ThreemaParser connFactory) {
      this.is = is;
  ...
  private void searchMediaFilesForMessagesInBatches(
      List<Message> messages, IItemSearcher searcher) {
    ...
    for (Message m : messages) {
      ...
      if (!fileName.isEmpty() && fileSize > 0) {
        ...
        List<Message> messageList =
            fileNameAndSizeToSearchFor.computeIfAbsent(key, k -> new ArrayList<>());
        messageList.add(m);
      }
    }
    ...
    if (!fileNameAndSizeToSearchFor.isEmpty()) {
      ...
      List<IItemReader> result =
          (searcher == null
              ...
              : searcher.search(fileNameAndSizeQuery));
      for (IItemReader item : result) {
