...
public class ReportGenerator {
  ...
  private static String format(String s) {
    ...
    else return SimpleHTMLEncoder.htmlEncode(s.trim());
  }
        ...
        () -> {
          ...
          if (currentMsg > 0)
            out.println(
                "<div class=\"linha\"><div class=\"date\">" //$NON-NLS-1$
                    + Messages.getString("ThreemaReport.ChatContinuation")
          ...
          while (currentMsg < c.getMessages().size()) {
            ...
            if (currentMsg != c.getMessages().size() && bout.size() >= minChatSplitSize) {
              out.println(
                  "<div class=\"linha\"><div class=\"date\">" //$NON-NLS-1$
                      + Messages.getString("ThreemaReport.ChatContinues")
                      + "</div></div>"); //$NON-NLS-1$ //$NON-NLS-2$
              break;
            }
          }
  ...
  private synchronized void printMessage(PrintWriter out, Message message, ThreemaAccount account) {
    ...
    switch (message.getMessageType()) {
      case UNKNOWN_MESSAGE:
        ...
        out.println(
            "<i>"
                + Messages.getString("ThreemaReport.UnknownMessage")
                + " [ID: "
                + message.getId()
      ...
      case THREEMA_CALL:
        ...
        out.println(
            Messages.getString("ThreemaReport.ThreemaCall") + "<br>"); // $NON-NLS-1$ //$NON-NLS-2$
        out.println(
            Messages.getString("ThreemaReport.Duration")
                + ": "
                + message.getMediaDuration()); // $NON-NLS-1$ //$NON-NLS-2$
        break;
      default:
        ...
        if (sender != null && !sender.isEmpty()) {
          out.println(
              "<span style=\"font-family: Arial; color: #b4c74b;\">" //$NON-NLS-1$
                  + format(sender)
                  + "</span><br/>"); //$NON-NLS-1$
        }
        ...
        switch (message.getMessageType()) {
          ...
          case UNKNOWN_MEDIA_MESSAGE:
            if (message.getMediaDescription() != null) {
              out.println(
                  "<i>" + Messages.getString("ThreemaReport.UnknownMediaMessage") + "</i><br/>");
          ...
          case URL_MESSAGE:
            out.println(
                "<a href=\""
                    + format(message.getUrl())
                    + "\">"
                    + format(message.getUrl()) // $NON-NLS-1$ //$NON-NLS-2$
                    + "</a><br/>"); //$NON-NLS-1$
            if (message.getText() != null) {
              out.print(format(message.getText()) + "<br/>"); // $NON-NLS-1$
            }
          ...
          case LOCATION_MESSAGE:
            out.println(
                "<i>"
                    + locationIcon
                    + Messages.getString("ThreemaReport.LocationMessage")
                    + "</i><br/>"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            out.println(
                "Latitude: " + message.getLatitude() + "<br/>"); // $NON-NLS-1$ //$NON-NLS-2$
            out.println(
                "Longitude: " + message.getLongitude() + "<br/>"); // $NON-NLS-1$ //$NON-NLS-2$
            if (message.getText() != null) {
              out.print(format(message.getText()) + "<br/>"); // $NON-NLS-1$
            }
            break;
          ...
          case APP_MESSAGE:
            ...
            if (mediaItem != null) { // media file found
              ...
              if (message.getMessageType() == MessageType.AUDIO_MESSAGE
                ...
                if (message.getMessageType() == MessageType.AUDIO_MESSAGE) {
                  out.println(
                      Messages.getString("ThreemaReport.AudioMessageTitle")
                          + "<br>"); //$NON-NLS-1$
                  out.println(
                      "<div class=\"audioImg iped-audio\" " //$NON-NLS-1$
                          + " title=\"Audio\" "
                          + "data-src1=\""
                          + format(exportPath)
                          + "\" "
                  ...
                  out.println(
                      Messages.getString("ThreemaReport.VideoMessageTitle")
                          + "<br>"); //$NON-NLS-1$
                  if (thumb != null) {
                ...
                if (thumb != null) {
                  ...
                  out.println(
                        Messages.getString("ThreemaReport.Video")
                  ...
                  if (message.getMessageType() == MessageType.IMAGE_MESSAGE) { // $NON-NLS-1$
                    ...
                    out.println(
                        "Attachment:<br><div class=\"attachImg\" title=\"Doc\"></div>"); //$NON-NLS-1$
                }
                out.println("</a><br>"); // $NON-NLS-1$
              }
            ...
            if (message.getMediaDescription() != null)
              out.println(
                  "<br><i>\""
                      + format(message.getMediaDescription())
                      + "\"</i><br>"); //$NON-NLS-1$ //$NON-NLS-2$
            break;
    ...
    if (!message.getChildPornSets().isEmpty()) {
      out.print(
          "<p><i>"
              + Messages.getString("ThreemaReport.FoundInPedoHashDB")
              + " "
  ...
  private static String getTitle(Message message) {
    if (message.getMediaDescription() != null) return message.getMediaDescription();
    else return "Attachment"; // $NON-NLS-1$
  }

  private void printMessageFile(
      PrintWriter out, String title, String id, byte[] avatar, Supplier<String> messages) {
    String strAvatar;
    if (avatar == null || avatar.length == 0) {
      ...
      strAvatar =
          "data:image/jpg;base64,"
              + Util.encodeBase64(Arrays.copyOfRange(avatar, 1, avatar.length));
    }
