...
public class UsnJrnlParserTest extends AbstractPkgTest {
  ...
  @Test
  public void testUsnJrnlParsingHTML()
    ...
    try (InputStream stream = getStream(testPath)) {
      ...
      for (int i = 1; i <= 4; i++) {
        ...
        for (String reason : reasons) {
          ...
          if (getVersion() < 9) assertEquals("20/05/2021 14:52:07", df.format(date));
          if (getVersion() >= 9 && getVersion() < 12)
            assertEquals("20 de mai de 2021 14:52:07", df.format(date));
          if (getVersion() >= 12) assertEquals("20 de mai. de 2021 14:52:07", df.format(date));
        }
      }
      ...
      for (int i = 3082; i <= 3083; i++) {
        ...
        for (String reason : reasons) {
          date =
              new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
                  .parse(metadata.get(UsnJrnlParser.USN_REASON_PREFIX + reason));
          if (getVersion() < 9) assertEquals("20/05/2021 14:55:03", df.format(date));
          if (getVersion() >= 9 && getVersion() < 12)
            assertEquals("20 de mai de 2021 14:55:03", df.format(date));
          if (getVersion() >= 12) assertEquals("20 de mai. de 2021 14:55:03", df.format(date));
        }
      }
  ...
  @Test
  public void testUsnJrnlParsingCSV()
      throws IOException, SAXException, TikaException, ParseException {
