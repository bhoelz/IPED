...
public class GenericPartyStringBuilder extends PartyStringBuilder {
  ...
  public String build() {
    ...
    if (StringUtils.isNotBlank(phoneNumber)
        && !StringUtils.equalsAny(phoneNumber, username, name)) {
      phoneNumberStr = "📞:" + phoneNumber;
    }
    if (StringUtils.isNotBlank(userId)
        && !StringUtils.equalsAny(userId, phoneNumber, username, name)) {
      userIdStr = "ID:" + userId;
    }

    String fieldsStr =
        Arrays.asList(usernameStr, phoneNumberStr, userIdStr).stream()
