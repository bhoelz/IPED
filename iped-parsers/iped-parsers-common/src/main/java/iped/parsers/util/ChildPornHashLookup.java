...
public class ChildPornHashLookup {
  ...
  /**
   * Lookup a hash in the database and merge the result with a optionally provided list (returned
   * from a previous call to hash lookup). The hash algorithm (md5, sha-1 or sha-256) is guessed
  ...
  public static List<String> lookupHashAndMerge(String mediaHash, List<String> prev) {
    ...
    String guessedAlgo = "";
    if (mediaHash.length() == 32) guessedAlgo = "md5";
    else if (mediaHash.length() == 40) guessedAlgo = "sha-1";
    else if (mediaHash.length() == 64) guessedAlgo = "sha-256";
  ...
  /**
   * Lookup a hash in the database and merge the result with a optionally provided list (returned
   * from a previous call to hash lookup), using a defined algorithm.
   */
  ...
  public abstract static class LookupProvider {
