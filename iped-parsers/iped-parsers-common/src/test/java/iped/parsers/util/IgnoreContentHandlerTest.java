...
class IgnoreContentHandlerTest {
  ...
  void startElement_noException() {
    assertDoesNotThrow(
        () -> new IgnoreContentHandler().startElement("", "tag", "tag", new AttributesImpl()));
  }
  ...
  void ignorableWhitespace_noException() {
    assertDoesNotThrow(
        () -> new IgnoreContentHandler().ignorableWhitespace(" ".toCharArray(), 0, 1));
  }
  ...
  void startPrefixMapping_noException() {
    assertDoesNotThrow(
        () -> new IgnoreContentHandler().startPrefixMapping("ns", "http://example.com"));
  }
