...
class P2PUtilTest {
    ...
    private static IItemSearcher stubSearcher(List<IItemReader> result) {
        return new IItemSearcher() {
            @Override
            public List<IItemReader> search(String query) { return result; }
            @Override
            public Iterable<IItemReader> searchIterable(String query) { return result; }
            @Override
            public String escapeQuery(String query) { return query; }
            @Override
            public void close() {}
        };
    }
