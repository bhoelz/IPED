...
public class LNKShortcut {
  ...
  public static enum FileAttributeFlags {
    ...
    HIDDEN(0x00000002),
    SYSTEM(0x00000004),
    RESERVED_1(0x00000008), // Reserved, not used by
    // the LNK format Is a
    // volume label
  ...
  public static enum DataFlags {
    ...
    IsUnicode(0x00000080), // The data strings in the LNK file are stored in Unicode (UTF-16
    // little-endian)
    // instead of ASCII
    ...
    RunInSeparateProcess(
        0x00000400), // A 16-bit target application is run in a separate virtual machine.
    Reserved_1(0x00000800),
    HasDarwinID(0x00001000), // The LNK file contains a Darwin (Mac OS-X) properties data
    // block
    ...
    NoPidlAlias(
        0x00008000), // The file system location is represented in the shell namespace when the path
    // to an item is parsed into the link target identifiers Contains a known folder
    // location data block?
    Reserved_2(0x00010000),
    RunWithShimLayer(0x00020000), // The target application is run with the shim layer. The
    ...
    DisableLinkPathTracking(
        0x00100000), // The environment variables location block should be ignored
