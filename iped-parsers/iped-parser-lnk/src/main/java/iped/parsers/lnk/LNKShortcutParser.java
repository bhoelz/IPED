...
/**
 * Parser para arquivos de atalho (LNK) do Windows Referencias utilizadas sobre o formato:
...
public class LNKShortcutParser extends AbstractParser {
  ...
  public static final Property LNK_METADATA_CREATED =
      Property.internalDate(LNK_METADATA_PREFIX + BasicProps.CREATED);
  public static final Property LNK_METADATA_MODIFIED =
      Property.internalDate(LNK_METADATA_PREFIX + BasicProps.MODIFIED);
  public static final Property LNK_METADATA_ACCESSED =
      Property.internalDate(LNK_METADATA_PREFIX + BasicProps.ACCESSED);
  public static final String LNK_METADATA_TARGET_REFERENCED =
      LNK_METADATA_PREFIX + "targetReferenced";
  public static final String LNK_METADATA_TARGET_METADATA_DIFFERENT =
      LNK_METADATA_PREFIX + "targetDifferentMetadata";
  public static final String LNK_METADATA_TARGET_REF_STRATEGY =
      LNK_METADATA_PREFIX + "targetReferenceStrategy";
  ...
  public void parse(
    ...
    final DateFormat df =
        new SimpleDateFormat(Messages.getString("LNKShortcutParser.DateFormat")); // $NON-NLS-1$
    ...
    try {
      ...
      if (lnkObj.hasName()
          || lnkObj.hasRelativePath()
          || lnkObj.hasWorkingDir()
          || lnkObj.hasArguments()
          || lnkObj.hasIconLocation()) showDataStrings(lnkObj, df, xhtml);

      // HasTargetIDList
      if (lnkObj.hasTargetIDList()) showTargetIDList(lnkObj, df, xhtml);

      // Extra data: distributed link tracker properties
      if (lnkObj.hasLinkTracker()) showLinkTracker(lnkObj, df, xhtml);
  ...
  private void setMetadata(Metadata metadata, ParseContext context, LNKShortcut lnkObj) {
    ...
    if (lnkObj.hasLinkLocation()) {
      ...
      metadata.add(
          LNK_METADATA_PREFIX + "localPath",
          getUnicodeValue(lnkObj, lnkLoc.getLocalPathUnicode(), lnkLoc.getLocalPath()));
      metadata.add(
          LNK_METADATA_PREFIX + "commonPath",
          getUnicodeValue(lnkObj, lnkLoc.getCommonPathUnicode(), lnkLoc.getCommonPath()));
      metadata.add(
          LNK_METADATA_PREFIX + "volumeLabel",
          getUnicodeValue(lnkObj, lnkLoc.getVolumeLabelUnicode(), lnkLoc.getVolumeLabel()));
      metadata.add(
          LNK_METADATA_PREFIX + "networkShare",
          getUnicodeValue(lnkObj, lnkLoc.getNetShareUnicode(), lnkLoc.getNetShare()));
      metadata.set(
          LNK_METADATA_PREFIX + "networkDeviceName",
          getUnicodeValue(lnkObj, lnkLoc.getNetDevNameUnicode(), lnkLoc.getNetDevName()));
  ...
  private IItemReader makeReference(
      Metadata metadata, ParseContext context, LNKShortcut lnkObj, String fullLocalPath) {
  ...
  /**
   * Looks up items using a relative path specified in a LNK shortcut object. <br>
   * It constructs an absolute target path by normalizing the the parent directory of {@code
   * lnkItem}'s path combined with the LNK relative path. <br>
   * In the end it is checked if results path exactly matches with the constructed absolute path.
   *
   * @param lnkObj The LNK object parsed from lnk file.
   * @param lnkItem The LNK item to be added to the case.
   * @return The corresponding target files, or {@code null} if preconditions are not met.
   */
  private List<IItemReader> lookupUsingRelativePath(
      IItemSearcher searcher, LNKShortcut lnkObj, IItemReader lnkItem) {
    ...
    String fullPathInCase =
        Paths.get(lnkItem.getPath(), "..", lnkRelativePath).normalize().toString();
    List<IItemReader> items =
        searcher.search(BasicProps.PATH + ":\"" + searcher.escapeQuery(fullPathInCase) + "\"");
    items.removeIf(item -> !item.getPath().equals(fullPathInCase)); // must match exactly the path
    return items;
  }

  /**
   * Attempts to find the file targeted by the LNK object ({@code lnkObj}) within the case.
   * Identification primarily relies on the NTFS file reference (MFT entry index and sequence
   * number) obtained from the {@code lnkObj}. <br>
   * A file is considered the target if its NTFS file reference matches that from the {@code
   * lnkObj}, and, additionally, the creation timestamp of the target file, as recorded in the
   * {@code lnkObj}, matches the creation timestamp of the candidate target file (useful to find
   * files that might have originated from different volumes).
   *
   * @param lnkObj The LNK object parsed from lnk file.
   * @param lnkItem The LNK item to be added to the case.
   * @return The corresponding target files, or {@code null} if preconditions are not met.
   */
  private List<IItemReader> lookupUsingMFT(
      IItemSearcher searcher, LNKShortcut lnkObj, IItemReader lnkItem) {
    ...
    LNKShellItem lastTarget =
        lnkObj.getShellTargetIDList().get(lnkObj.getShellTargetIDList().size() - 1);
    ...
    return searcher.search(
        BasicProps.META_ADDRESS
            + ":"
            + fEntry.getIndMft() //
            + " && "
            + BasicProps.MFT_SEQUENCE
            + ":"
            + fEntry.getSeqMft() //
            + " && "
            + BasicProps.CREATED
            + ":\""
            + DateUtil.dateToString(lnkObj.getCreateDate())
            + "\"");
  }

  private List<IItemReader> lookupUsingFullLocalPath(
      IItemSearcher searcher, LNKShortcut lnkObj, IItemReader lnkItem, String fullLocalPath) {
    ...
    List<IItemReader> items =
        searcher.search(BasicProps.PATH + ":\"" + searcher.escapeQuery(fullLocalPath) + "\"");
    items.removeIf(
        item ->
            !item.getPath().endsWith(fullLocalPathFinal)); // path must ends with fullLocalPathFinal

    // Disconsider if fullPath is not relative to a root item (named as "/").
    // This avoid a fullLocalPath = "/x/y/a.png" to wrongly reference a file like
    // "/image.E01/vol_2/Images/foo/x/y/a.png"
    ...
    for (Iterator<IItemReader> iter = items.iterator(); iter.hasNext(); ) {
      ...
      for (int i = 0; i < fullLocalPathParts; i++) {
        List<IItemReader> parents =
            searcher.search(BasicProps.ID + ":" + currentItem.getParentId());
        if (parents.isEmpty()) {
          currentItem = null;
          break;
        }
  ...
  private IItemReader setReferenceMetadata(
      Metadata metadata,
    ...
    if (items.size() > 1) {
      ...
      log.warn(
          "More than one file referenced to the link. Using only the best one. {}",
    ...
    if (item.getCreationDate() != null && lnkObj.getCreateDate() != null) {
      boolean sameCreated =
          DateUtils.truncatedEquals(
              item.getCreationDate(), lnkObj.getCreateDate(), Calendar.SECOND);
  ...
  private void showHeader(LNKShortcut lnkObj, DateFormat df, XHTMLContentHandler xhtml)
      throws Exception {
    ...
    addRow(
        xhtml,
        Messages.getString("LNKShortcutParser.Created"),
        lnkObj.getCreateDate(df),
        "a",
        "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    addRow(
        xhtml,
        Messages.getString("LNKShortcutParser.Accessed"),
        lnkObj.getAccessDate(df),
        "a",
        "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    addRow(
        xhtml,
        Messages.getString("LNKShortcutParser.Modified"),
        lnkObj.getModifiedDate(df),
        "a",
        "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    addRow(
        xhtml,
        Messages.getString("LNKShortcutParser.TargetSize"), // $NON-NLS-1$
        String.format("%,d bytes", lnkObj.getFileSize()),
        "a",
        "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    ...
    addRow(
        xhtml,
        Messages.getString("LNKShortcutParser.TargetAttr"),
        strFlAtt + String.format(" (0x%08x)", flAtt), // $NON-NLS-1$ //$NON-NLS-2$
        "a",
    ...
    addRow(
        xhtml,
        Messages.getString("LNKShortcutParser.LinkAttr"), // $NON-NLS-1$
        strDataFlag + String.format(" (0x%08x)", dataLnkFlags),
        "a",
        "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    addRow(
        xhtml,
        Messages.getString("LNKShortcutParser.IconIndex"),
        String.valueOf(lnkObj.getIconIndex()),
        "a", //$NON-NLS-1$ //$NON-NLS-2$
        "b"); //$NON-NLS-1$
    addRow(
        xhtml,
        Messages.getString("LNKShortcutParser.WindowAttr"),
        String.valueOf(lnkObj.getShowWindow()),
        "a", //$NON-NLS-1$ //$NON-NLS-2$
        "b"); //$NON-NLS-1$
    addRow(
        xhtml,
        Messages.getString("LNKShortcutParser.HotKey"),
        String.valueOf(lnkObj.getHotKey()),
        "a",
        "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    xhtml.endElement("table"); // $NON-NLS-1$
  }

  private void showDataStrings(LNKShortcut lnkObj, DateFormat df, XHTMLContentHandler xhtml)
      throws Exception {
    xhtml.startElement("table", "class", "t"); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    addRowHeader(
        xhtml,
        Messages.getString("LNKShortcutParser.DataStrings")
            + lnkObj.getStringDataFlags()
            + ")"); //$NON-NLS-1$//$NON-NLS-2$
    addRow(
        xhtml,
        Messages.getString("LNKShortcutParser.Description"),
        lnkObj.getDescription(),
        "a",
        "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    addRow(
        xhtml,
        Messages.getString("LNKShortcutParser.RelativePath"),
        lnkObj.getRelativePath(),
        "a",
        "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    addRow(
        xhtml,
        Messages.getString("LNKShortcutParser.WorkingDir"),
        lnkObj.getWorkingDir(),
        "a",
        "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    addRow(
        xhtml,
        Messages.getString("LNKShortcutParser.CmdLineArgs"),
        lnkObj.getCommandLineArgs(),
        "a",
        "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    addRow(
        xhtml,
        Messages.getString("LNKShortcutParser.IconLocation"),
        lnkObj.getIconLocation(),
        "a",
        "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    xhtml.endElement("table"); // $NON-NLS-1$
  }

  private void showLinkTracker(LNKShortcut lnkObj, DateFormat df, XHTMLContentHandler xhtml)
      throws Exception {
    LNKLinkTracker lnkTracker = lnkObj.getLinkTracker();
    if (lnkTracker != null) {
      xhtml.startElement("table", "class", "t"); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      addRowHeader(
          xhtml, Messages.getString("LNKShortcutParser.DistributedLinkTrackProps")); // $NON-NLS-1$
      addRow(
          xhtml,
          Messages.getString("LNKShortcutParser.MachineId"),
          lnkTracker.getMachineId(),
          "a",
          "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      addRow(
          xhtml,
          Messages.getString("LNKShortcutParser.DroidVolId"),
          lnkTracker.getDroidVolumeId(),
          "a",
          "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      addRow(
          xhtml,
          Messages.getString("LNKShortcutParser.DroidFileId"),
          lnkTracker.getDroidFileId(),
          "a",
          "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      addRow(
          xhtml,
          Messages.getString("LNKShortcutParser.BirthDroidVolId"),
          lnkTracker.getBirthDroidVolumeId(), // $NON-NLS-1$
          "a",
          "b"); //$NON-NLS-1$ //$NON-NLS-2$
      addRow(
          xhtml,
          Messages.getString("LNKShortcutParser.BirthDroidFileId"),
          lnkTracker.getBirthDroidFileId(), // $NON-NLS-1$
          "a",
          "b"); //$NON-NLS-1$ //$NON-NLS-2$
      xhtml.endElement("table"); // $NON-NLS-1$
    }
  ...
  private void showTargetIDList(LNKShortcut lnkObj, DateFormat df, XHTMLContentHandler xhtml)
      throws Exception {
    xhtml.startElement("table", "class", "t"); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    ...
    for (int i = 0; i < lstTarget.size(); i++) {
      ...
      if (lnkShell.hasFileEntry()) {
        ...
        if (fEntry.isDirectory()) {
          addRow(
              xhtml,
              Messages.getString("LNKShortcutParser.EntryType"), // $NON-NLS-1$
              Messages.getString("LNKShortcutParser.Directory"),
              "s1",
              "s2"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        } else if (fEntry.isFile()) {
          addRow(
              xhtml,
              Messages.getString("LNKShortcutParser.EntryType"), // $NON-NLS-1$
              Messages.getString("LNKShortcutParser.File"),
              "s1",
              "s2"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        }
        ...
        addRow(
            xhtml,
            Messages.getString("LNKShortcutParser.FileAttr"), // $NON-NLS-1$
            strFlAtt + String.format(" (0x%08x)", flAtt),
            "s1",
            "s2"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        addRow(
            xhtml,
            Messages.getString("LNKShortcutParser.PrimaryName"),
            fEntry.getPrimaryName(),
            "s1",
            "s2"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        if (fEntry.getFileSize() > 0) {
          addRow(
              xhtml,
              Messages.getString("LNKShortcutParser.Size"), // $NON-NLS-1$
              String.format("%,d bytes", fEntry.getFileSize()),
              "s1",
              "s2"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        }
        addRow(
            xhtml,
            Messages.getString("LNKShortcutParser.LastMod"),
            fEntry.getModifiedDate(df),
            "s1",
            "s2"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        addRow(
            xhtml,
            Messages.getString("LNKShortcutParser.Extensions"),
            fEntry.getExtensionsSig(),
            "s1", //$NON-NLS-1$ //$NON-NLS-2$
            "s2"); //$NON-NLS-1$
        addRow(
            xhtml,
            Messages.getString("LNKShortcutParser.Created"),
            fEntry.getCreateDate(df),
            "s1",
            "s2"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        addRow(
            xhtml,
            Messages.getString("LNKShortcutParser.Accessed"),
            fEntry.getAccessDate(df),
            "s1",
            "s2"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        addRow(
            xhtml,
            Messages.getString("LNKShortcutParser.SecondaryName"),
            fEntry.getSecondaryName(),
            "s1", //$NON-NLS-1$ //$NON-NLS-2$
            "s2"); //$NON-NLS-1$
        addRow(
            xhtml,
            Messages.getString("LNKShortcutParser.LocalizedNames"),
            fEntry.getLocalizedNames(),
            "s1", //$NON-NLS-1$ //$NON-NLS-2$
            "s2"); //$NON-NLS-1$
        addRow(
            xhtml,
            Messages.getString("LNKShortcutParser.NtfsRef"),
            fEntry.getNtfsRef(),
            "s1",
            "s2"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        addRow(
            xhtml,
            Messages.getString("LNKShortcutParser.UnknownData"),
            fEntry.getUnknown(),
            "s1",
            "s2"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        ...
        addRow(
            xhtml,
            Messages.getString("LNKShortcutParser.NetItemType"),
            netLoc.getTypeStr(),
            "s1",
            "s2"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        addRow(
            xhtml,
            Messages.getString("LNKShortcutParser.NetItemName"),
            netLoc.getLocation(),
            "s1",
            "s2"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        addRow(
            xhtml,
            Messages.getString("LNKShortcutParser.NetDescription"),
            netLoc.getDescription(),
            "s1", //$NON-NLS-1$ //$NON-NLS-2$
            "s2"); //$NON-NLS-1$
        addRow(
            xhtml,
            Messages.getString("LNKShortcutParser.NetComments"),
            netLoc.getComments(),
            "s1",
            "s2"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        ...
        for (int j = 0; j < lstTmp.size(); j++) {
          if (lnkShell.isUnparsed()) {
            xhtml.startElement(
                "textarea", "readonly", "readonly"); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
  ...
  private void showLinkLocation(LNKShortcut lnkObj, DateFormat df, XHTMLContentHandler xhtml)
      throws Exception {
    ...
    if (objItem != null) {
      ...
      addRow(
          xhtml,
          Messages.getString("LNKShortcutParser.DriveType"),
          objItem.getDriveTypeStr(),
          "a",
          "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      addRow(
          xhtml,
          Messages.getString("LNKShortcutParser.DriveSerial"),
          objItem.getDriveSerial(),
          "a",
          "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      addRow(
          xhtml,
          Messages.getString("LNKShortcutParser.VolumeLabel"),
          objItem.getVolumeLabel(),
          "a",
          "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      addRow(
          xhtml,
          Messages.getString("LNKShortcutParser.VolumeLabelUnicode"),
          objItem.getVolumeLabelUnicode(), // $NON-NLS-1$
          "a",
          "b"); //$NON-NLS-1$ //$NON-NLS-2$
      addRow(
          xhtml,
          Messages.getString("LNKShortcutParser.LocalPath"),
          objItem.getLocalPath(),
          "a",
          "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      addRow(
          xhtml,
          Messages.getString("LNKShortcutParser.CommonPath"),
          objItem.getCommonPath(),
          "a",
          "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      addRow(
          xhtml,
          Messages.getString("LNKShortcutParser.LocalPathUnicode"),
          objItem.getLocalPathUnicode(),
          "a", //$NON-NLS-1$ //$NON-NLS-2$
          "b"); //$NON-NLS-1$
      addRow(
          xhtml,
          Messages.getString("LNKShortcutParser.CommonPathUnicode"),
          objItem.getCommonPathUnicode(), // $NON-NLS-1$
          "a",
          "b"); //$NON-NLS-1$ //$NON-NLS-2$
      addRow(
          xhtml,
          Messages.getString("LNKShortcutParser.NetDeviceName"),
          objItem.getNetDevName(),
          "a",
          "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      addRow(
          xhtml,
          Messages.getString("LNKShortcutParser.NetDevNameUnicode"),
          objItem.getNetDevNameUnicode(), // $NON-NLS-1$
          "a",
          "b"); //$NON-NLS-1$ //$NON-NLS-2$
      addRow(
          xhtml,
          Messages.getString("LNKShortcutParser.NetShare"),
          objItem.getNetShare(),
          "a",
          "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      addRow(
          xhtml,
          Messages.getString("LNKShortcutParser.NetShareUnicode"),
          objItem.getNetShareUnicode(),
          "a", //$NON-NLS-1$ //$NON-NLS-2$
          "b"); //$NON-NLS-1$
      addRow(
          xhtml,
          Messages.getString("LNKShortcutParser.NetProviderType"), // $NON-NLS-1$
          String.format(" (0x%08x)", objItem.getNetProviderType()),
          "a",
          "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      addRow(
          xhtml,
          Messages.getString("LNKShortcutParser.LocationFlags"), // $NON-NLS-1$
          String.format(" (0x%08x)", objItem.getFlagsLocation()),
          "a",
          "b"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      xhtml.endElement("table"); // $NON-NLS-1$
    }
  ...
  private static final void addRow(
      XHTMLContentHandler xhtml, String nmStr, String vlStr, String classNm, String classVl)
      throws Exception {
