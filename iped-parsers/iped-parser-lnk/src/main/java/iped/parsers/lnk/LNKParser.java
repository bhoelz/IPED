...
public class LNKParser {
  ...
  private static void parseHeader(byte[] b, LNKShortcut lnkObj) throws Exception {
    ...
    if (sizeHeader != 0X4c) {
      ...
      throw new Exception(
          "Header size (" + sizeHeader + ") unsupported."); // $NON-NLS-1$ //$NON-NLS-2$
    }
    ...
    while (posBlock < endBlock) {
      ...
      if (btInt == 0X1F) {
        ...
        if ((netFlags & 0X80) == 0X80) {
          ...
          String strDesc =
              toStr(
                  b, posBlock + offSet, sizeItem - offSet, (byte) 0X00, "ISO8859_1"); // $NON-NLS-1$
          if (strDesc != null) {
        ...
        if (sizeData > 0) {
          offSet +=
              36; // 4(Unknown) + 4(Unknown) + 8(Unknown Timestamp) + 4(Unknown) + 12(Unknown) +
          // 4(Unknown)
        ...
        // outro tipo de item
        String hexStr =
            toHex(b, posBlock + 3, sizeItem - 3); // tamanho do item inclui o proprio tamanho do
  ...
  private static int parseLinkLocation(byte[] b, int offset, LNKShortcut lnkObj) throws Exception {
    ...
    if (sizeHeader < 28)
      throw new Exception(
          "LinkLocation section header size ("
              + sizeHeader
              + ") unsupported."); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    if ((locFlags & 0x0001)
        == 0x0001) { // Flag 0x0001 - VolumeIDAndLocalBasePath - The linked file is on a volume.
      ...
      if (iniVol > 0 && iniVol < sizeBlock) {
        ...
        if (sizeTmp > sizeBlock)
          throw new Exception(
              "LinkLocation Volume Information size ("
                  + sizeTmp
                  + ") exceeds section size (" //$NON-NLS-1$ //$NON-NLS-2$
                  + sizeBlock
                  + ")."); //$NON-NLS-1$
        ...
        String strTmp =
            toStr(b, iniVol + offVolLabel, sizeTmp - 16, (byte) 0X00, "ISO8859_1"); // $NON-NLS-1$
        if (strTmp != null) {
    ...
    if (iniNet > 0 && iniNet < sizeBlock) {
      ...
      if (sizeTmp > sizeBlock)
        throw new Exception(
            "LinkLocation Network Share Information size ("
                + sizeTmp //$NON-NLS-1$
      ...
      if (offNetShare < sizeTmp) {
        String strTmp =
            toStr(b, iniNet + offNetShare, sizeTmp - 20, (byte) 0X00, "ISO8859_1"); // $NON-NLS-1$
        if (strTmp != null) {
          objItem.setNetShare(strTmp);
        }
      }

      if ((netFlags & 0x00000001)
          == 0x00000001) { // Flag 0x0001 - ValidDevice - If set the device name contains
        // data
        int offNetDevice = toInt(b, iniNet + 12);
        if (offNetDevice < sizeTmp) {
          String strTmp =
              toStr(
                  b, iniNet + offNetDevice, sizeTmp - 21, (byte) 0X00, "ISO8859_1"); // $NON-NLS-1$
          if (strTmp != null) {
      ...
      if ((netFlags & 0x00000002)
          == 0x00000002) { // Flag 0x0002 - ValidNetType - If set the network provider
        // type contains data
    ...
    String strTmp =
        toStr(b, offset + iniCom, sizeBlock - iniCom, (byte) 0X00, "ISO8859_1"); // $NON-NLS-1$
    if (strTmp != null) {
    ...
    if (sizeHeader > 28 && ((locFlags & 0x0001) == 0x0001)) {
      ...
      offtmp += 4;
      if ((locFlags & 0x0001)
          == 0x0001) { // Flag 0x0001 - VolumeIDAndLocalBasePath - The linked file is on a
  ...
  private static int parseExtraData(byte[] b, int offset, LNKShortcut lnkObj) throws Exception {
    ...
    if (hexStr == null) return offtmp;

    int posExt =
        hexStr.indexOf(
            "60000000030000a058000000"); // tamanho da secao (96), assinatura e //$NON-NLS-1$
  ...
  private static int parseLinkTrackerProperties(byte[] b, int offset, LNKShortcut lnkObj)
      throws Exception {
    int offtmp = offset;
    ...
    if (sizeBlock != 96
        || hexSig == null
        || !(hexSig.equals("030000a0"))
        || sizeTmp != 88
        || verBlock != 0) // $NON-NLS-1$
  ...
  public static void parseExtensionBlock(byte[] b, int posSig, LNKShellItem objItem)
      throws Exception {
    ...
    int verExt =
        toSmall(
            b,
            posSig - 2); // 3: Win XP or 2003; 7: Win Vista (SP0); 8: Win 2008, 7, 8.0; 9: Win 8.1,
    // 10
    ...
    if (hexSig.equals("0400efbe")) { // $NON-NLS-1$
      ...
      if (verExt >= 7) {
        ...
        if (indMft > 0) {
          ...
          fEntry.setNtfsRef(
              "MFT Entry Idx "
                  + String.valueOf(indMft)
                  + " - Seq.Numb. "
                  + String.valueOf(seqNum)); // $NON-NLS-1$ //$NON-NLS-2$
        }
      ...
      if (verExt >= 9) posTmp += 4; // bytes desconsiderados: 4(unknown)

      if (verExt >= 8) posTmp += 4; // bytes desconsiderados: 4(unknown)
  ...
  private static final String toStr(byte[] b, int offset, int length) {
    if (offset + length > b.length || offset < 0 || length < 0) return null;
  ...
  private static final String toStr(
      byte[] b, int offset, int maxlength, byte delim, String charSet) {
    if (offset + maxlength >= b.length || maxlength < 0 || offset < 0) return null;
    // procurando o delimitador
    int i = 0;
    for (i = 0; i <= maxlength; i++) if (b[offset + i] == delim) break;
  ...
  private static final String toStrUTF16(byte[] b, int offset, int maxlength) {
    ...
    for (i = 0; i < maxlength; i += 2)
      if (b[offset + i] == 0X00 && b[offset + i + 1] == 0X00) break;
  ...
  private static final int findEndStr(byte[] b, int offset, int maxlength, boolean utf16) {
    if (offset + maxlength >= b.length || maxlength < 0 || offset < 0) return -1;
    // procurando o delimitador
    int i = 0;
    if (utf16) {
      for (i = 0; i < maxlength; i += 2)
      ...
      for (i = 0; i <= maxlength; i++) if (b[offset + i] == 0X00) break;
    }
    return i;
  }
