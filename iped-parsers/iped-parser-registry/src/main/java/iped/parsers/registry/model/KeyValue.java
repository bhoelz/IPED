...
public class KeyValue extends CellContent {
  ...
  public ArrayList<Integer> getSubCellsOffsets() {
    ...
    if (dataLength
        < 0) { // when most significant bit is set to 1 the data is less than 4 bytes in length
      return result;
    } else {
    ...
    try {
      if (buffer.length == 0) {
        ...
        if ((buffer.length >= 2)
            ...
            && (buffer[buffer.length - 2] == 0)) buffer = Arrays.copyOf(buffer, buffer.length - 2);
        ...
        if (buffer.length >= 8) {
          ...
          if (getValueName().toLowerCase().contains("time")
            ...
            try {
              result +=
                  " ("
                      + dateFormat.get().format(new Date((l) / (long) 10000 - 11644473600000l))
                      + ")";
            } catch (Exception e) {
              // igonra
            }
