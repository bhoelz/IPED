...
public class UserAssistKeyParser extends HtmlKeyParser {
        ...
        if (kvs != null) {
          for (int j = 0; j < kvs.length; j++) {
            ...
            if (keyValue.getValueName().equals("MRUListEx")) {
              ...
              ByteArrayInputStream keyValueStream =
                  new ByteArrayInputStream(generateUserAssistHtml(keyValue, 0, kmeta));
  ...
  private byte[] generateUserAssistHtml(KeyValue keyValue, int startOffset, Metadata kmeta)
    ...
    try {
      ...
      for (int i = 67; i >= 60; i--) {
        int pow = i - 60;
        sum =
            sum.add(BigInteger.valueOf(data[i] & 0xFF).multiply(BigInteger.valueOf(256).pow(pow)));
      }
  ...
  private static String decodeUserAssist(String keyValueName) {
