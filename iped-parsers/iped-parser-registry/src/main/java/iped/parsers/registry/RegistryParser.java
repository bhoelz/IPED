...
public class RegistryParser extends AbstractParser {
    ...
    try {
      if (defaultRegistryKeyParser == null) {
        synchronized (RegistryParser.class) {
          defaultRegistryKeyParser =
              RegistryKeyParserManager.getRegistryKeyParserManager().getDefaultRegistryKeyParser();
        }
      }
      if (!(nome.equals("SYSTEM")
          || nome.equals("SOFTWARE")
          || nome.equals("SAM")
          || nome.equals("SECURITY")
          || nome.equals("NTUSER.DAT"))) return;
      ItemInfo itemInfo = context.get(ItemInfo.class);
      String caminho = itemInfo.getPath().toLowerCase().replace("\\", "/");
      if (!(caminho.contains("system32/config")
          || caminho.contains("users")
          || caminho.contains("settings"))) return;
      ...
      if (extractItems) {
        ...
        recursiveKeyParser(
            kf, "ROOT", "", handler, metadata, context, new HashMap<String, EmbeddedParent>());
      }
    } catch (Exception e) {
  ...
  public void extractCaseData(String nome, String caminho, RegistryFile rf, ParseContext context)
      throws RegistryFileException {
    TimeZone tz = null;
    if (nome.equals("SYSTEM")) {
  ...
  private void keyParser(
      ...
      HashMap<String, EmbeddedParent> parentMap)
      throws TikaException {
    RegistryKeyParser parser =
        RegistryKeyParserManager.getRegistryKeyParserManager().getRegistryKeyParser(keyPath);
    if (parser != null) {
      ...
      if (RegistryKeyParserManager.getRegistryKeyParserManager().hasChildRegistered(keyPath)) {
        String title = keyPath.substring(parentPath.length() + 1);
        EmbeddedParent parent = parentMap.get(parentPath);
        defaultRegistryKeyParser.parse(
            kn, title, hasChildren, keyPath, parent, handler, metadata, context);
        parentMap.put(keyPath, context.get(EmbeddedItem.class));
      }
    }
  }

  private void recursiveKeyParser(
      KeyNode kn,
      String keyPath,
      String parentPath,
      ContentHandler handler,
