...
public class RegistryKeyParserManager implements RegistryKeyParserFilter {
  ...
  private synchronized void loadConfigPath() {
    try {
    ...
    if (files != null) {
      for (int i = 0; i < files.length; i++) {
        try (InputStreamReader reader =
            new InputStreamReader(new FileInputStream(files[i]), "UTF-8")) {
          engine.eval(reader);
  ...
  public static boolean matchesStart(String test, String pattern) {
    ...
    while (wildIndex >= 0) {
      ...
      if (slashIndex >= 0) {
        ...
        patternSubts =
            patternSubts.substring(0, patternSubts.indexOf("*"))
                + meio
