...
public class RegRipperParser extends AbstractParser {
  ...
  private static final String timeBiasStartTag = "Bias           -&gt; ";
  private static final String timeBiasEndTag = " (";
  ...
  public Set<MediaType> getSupportedTypes(ParseContext context) {
    if (SUPPORTED_TYPES != null) return SUPPORTED_TYPES;
    ...
    synchronized (this.getClass()) {
      if (!tested) {
        try {
          ...
          Process p = pb.start();
          p.waitFor();
          if (p.exitValue() != 0) throw new Exception("Rip exit value = " + p.exitValue());

          SUPPORTED_TYPES =
              Collections.singleton(MediaType.application("x-windows-registry")); // $NON-NLS-1$

        } catch (Exception e) {
          StringBuilder sb = new StringBuilder();
          for (String s : cmd) sb.append(s).append(" ");
          String msg =
              "Error testing rip (RegRipper): registry reports will NOT be created." //$NON-NLS-1$
                  + " Command line: "
                  + sb.toString() // $NON-NLS-1$
                  + " Check if perl module 'Parse::Win32Registry' is installed."; //$NON-NLS-1$
          // are we in analysis app?
          if (System.getProperty("iped.javaVersionChecked") != null) log.warn(msg);
          else log.error(msg);
    ...
    try {
      ...
      if (directoryListing != null) {
        for (File child : directoryListing) {
          ...
          command.addAll(
              Arrays.asList(
                  "-f", profiles + "/" + child.getName(), "-r", tempFile.getAbsolutePath()));
  ...
  private void extractCaseTimezone(String nome, String caminho, File htmlFile, ParseContext context)
      throws RegistryFileException, IOException {
    TimeZone tz = null;
      ...
      if (caseData != null) {
        synchronized (caseData) {
          if (nome.equals("SYSTEM")) {
            if (tz != null) {
              HashMap<String, TimeZone> tzs2 =
                  (HashMap<String, TimeZone>) caseData.getCaseObject(ICaseData.TIMEZONE_INFO_KEY);
              if (tzs2 == null) {
  ...
  private void runCmdAndCreateReport(
      List<String> command,
      String reportName,
    ...
    reportMetadata.set(
        ...
        "application/x-windows-registry-report"); //$NON-NLS-1$
  ...
  private Thread readStream(
      final InputStream stream, final OutputStream os, final ContainerVolatile msg) {
    Thread t =
        new Thread() {
          @Override
          public void run() {
  ...
  private void waitFor(Process p, ContentHandler handler, ContainerVolatile msg)
