...
public class RegistryParserTest extends TestCase {
  ...
  @Test
  public void testRegistroParserSecurity()
      throws IOException, SAXException, TikaException, URISyntaxException {
  ...
  @Test
  public void testRegistroParserSAM()
      throws IOException, SAXException, TikaException, URISyntaxException {
  ...
  @Test
  public void testRegistroParserSYSTEM()
      throws IOException, SAXException, TikaException, URISyntaxException {
    RegistryParser parser = new RegistryParser();
    Metadata metadata = new Metadata();
    ...
    try (InputStream stream = getStream("test-files/test_system")) {
      ...
      assertEquals(
          "Computer Name: " + kf.getValue("ComputerName").getValueDataAsString(),
          "Computer Name: WKS-WIN732BITA");
      kf = rf.findKeyNode("Setup");
      assertEquals(
          "Working directory: " + kf.getValue("WorkingDirectory").getValueDataAsString(),
          "Working directory: C:\\Windows\\Panther");
    }
  }

  @Test
  public void testRegistroParserSOFTWARE()
      throws IOException, SAXException, TikaException, URISyntaxException {
    ...
    try (InputStream stream = getStream("test-files/test_software")) {
      ...
      assertEquals(
          "Install Date: " + kf.getValue("InstallDate").getValueDataAsString(),
          "Install Date: 8/28/2011");
      kf = rf.findKeyNode("Microsoft/Windows Defender/");
      assertEquals(
          "Disable Anti Spyware: " + kf.getValue("DisableAntiSpyware").getValueDataAsString(),
          "Disable Anti Spyware: 1");
    }
  }

  @Test
  public void testRegistroParserNTUSER()
      throws IOException, SAXException, TikaException, URISyntaxException {
    ...
    try (InputStream stream = getStream("test-files/test_ntuser.dat")) {
      ...
      assertEquals(
          "Wallpaper: " + kf.getValue("Wallpaper").getValueDataAsString(),
      ...
      for (int i = 0; i < identities.size(); i++) {
        assertEquals(
            "Username: " + identities.get(i).getValue("Username").getValueDataAsString(),
            "Username: Main Identity");
        assertEquals(
            "Username ID: " + identities.get(i).getValue("User ID").getValueDataAsString(),
