...
public class AresParser extends AbstractParser {
  ...
  private static final Set<MediaType> SUPPORTED_TYPES =
      Collections.singleton(MediaType.application("x-ares-galaxy")); // $NON-NLS-1$
  public static final String ARES_MIME_TYPE = "application/x-ares-galaxy"; // $NON-NLS-1$
  public static final String ARES_ENTRY_MIME_TYPE =
      "application/x-ares-galaxy-entry"; //$NON-NLS-1$
  public static final String[] header =
      new String[] {
        Messages.getString("AresParser.Seq"), // $NON-NLS-1$
        Messages.getString("AresParser.Title"),
        Messages.getString("AresParser.Path"), // $NON-NLS-1$ //$NON-NLS-2$
        Messages.getString("AresParser.HashSha1"),
        Messages.getString("AresParser.FileDate"), // $NON-NLS-1$ //$NON-NLS-2$
        Messages.getString("AresParser.Size"),
        Messages.getString("AresParser.Shared"), // $NON-NLS-1$ //$NON-NLS-2$
        Messages.getString("AresParser.Corrupted"),
        Messages.getString("AresParser.Artist"), // $NON-NLS-1$ //$NON-NLS-2$
        Messages.getString("AresParser.Album"),
        Messages.getString("AresParser.Category"), // $NON-NLS-1$ //$NON-NLS-2$
        Messages.getString("AresParser.URL"),
        Messages.getString("AresParser.Comments"), // $NON-NLS-1$ //$NON-NLS-2$
        Messages.getString("AresParser.FoundInPedoHashDB"),
        Messages.getString("AresParser.FoundInCase")
      }; //$NON-NLS-1$ //$NON-NLS-2$
  ...
  public void parse(
    ...
    final DateFormat df =
        new SimpleDateFormat(Messages.getString("AresParser.DateFormat")); // $NON-NLS-1$
    ...
    for (int i = -1; i < l.size(); i++) {
      ...
      if (i == -1) {
        ...
        if (e.isShared()) metadata.add(ExtraProperties.SHARED_HASHES, hash);
        ...
        cells.add(
            hash.substring(0, hash.length() / 2)
                + " "
                + hash.substring(hash.length() / 2)); // $NON-NLS-1$
      ...
      for (int j = 0; j < cells.size(); j++) {
        if (present[j]) {
          ...
          if (j != 1 || e == null) xhtml.characters(s);
          else {
