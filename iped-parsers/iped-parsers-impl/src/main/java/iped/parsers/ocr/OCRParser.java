...
/**
 * Parser OCR para imagens e PDFs via Tesseract. No caso de PDFs, é gerada uma imagem para cada
 * página. Outra opção seria extrair as imagens do PDF, mas alguns softwares de digitalização geram
 * várias imagens por página e algumas linhas de texto são cortadas pelas bordas das imagens.
 *
 * @author Nassif
 */
public class OCRParser extends AbstractParser implements AutoCloseable {
  ...
  private static final String OUTPUT_REGEX =
      "Tesseract Open Source OCR Engine v.* with Leptonica"; //$NON-NLS-1$
  ...
  private static final String CREATE_TABLE =
      "CREATE TABLE IF NOT EXISTS ocr(id TEXT PRIMARY KEY, text TEXT);"; //$NON-NLS-1$

  private static final String INSERT_DATA =
      "INSERT INTO ocr(id, text) VALUES(?,?) ON CONFLICT(id) DO NOTHING"; //$NON-NLS-1$

  private static final String SELECT_EXACT = "SELECT text FROM ocr WHERE id=?;"; // $NON-NLS-1$

  private static final String SELECT_ALL =
      "SELECT id, text FROM ocr WHERE id >= ? AND id < ?"; //$NON-NLS-1$
  ...
  public static final String PROCESS_NON_STANDARD_FORMATS_PROP =
      "ocr.processNonStandard"; //$NON-NLS-1$
  public static final String MAX_CONV_IMAGE_SIZE_PROP = "ocr.maxConvImageSize"; // $NON-NLS-1$

  private boolean ENABLED =
      Boolean.valueOf(System.getProperty(ENABLE_PROP, "false")); // $NON-NLS-1$
  ...
  private boolean SKIP_KNOWN_FILES =
      Boolean.valueOf(System.getProperty(SKIP_KNOWN_FILES_PROP, "false")); // $NON-NLS-1$
  private int MIN_SIZE = Integer.valueOf(System.getProperty(MIN_SIZE_PROP, "10000")); // $NON-NLS-1$
  private long MAX_SIZE =
      Integer.valueOf(System.getProperty(MAX_SIZE_PROP, "100000000")); // $NON-NLS-1$
  private List<String> bookmarksToOCR =
      Arrays.asList(
          System.getProperty(SUBSET_TO_OCR, SUBSET_SEPARATOR)
              .split(SUBSET_SEPARATOR)); // $NON-NLS-1$;
  private boolean PROCESS_NON_STANDARD_FORMATS =
      Boolean.valueOf(System.getProperty(PROCESS_NON_STANDARD_FORMATS_PROP, "true")); // $NON-NLS-1$
  private int MAX_CONV_IMAGE_SIZE =
      Integer.valueOf(System.getProperty(MAX_CONV_IMAGE_SIZE_PROP, "3000")); // $NON-NLS-1$
  ...
  private static final Map<String, Set<MediaType>> librarySupportedTypes =
      getLibrarySupportedTypes();
  ...
  public OCRParser() {
    ...
    String[] cmd = {
      tesseractPath, INPUT_FILE_TOKEN, OUTPUT_FILE_TOKEN, "-l", LANGUAGE, "-psm", PAGESEGMODE
    }; //$NON-NLS-1$ //$NON-NLS-2$
    this.command = cmd;
    ...
    try {
      synchronized (checked) {
        if (ENABLED && !checked.getAndSet(true)) {
          ...
          if (!info.isEmpty()) tessVersion = info.get(0);
          ...
          if (info.size() <= 1) {
            ...
            LOGGER.info(
                "Tesseract optional image libraries: "
                    + info.subList(1, info.size())); // $NON-NLS-1$
          ...
          LOGGER.info(
              "Process non-standard image formats {}.", //$NON-NLS-1$
  ...
  private boolean isFromBookmarkToOCR(ItemInfo ocrContext) {
    ...
    if (bookmarksToOCR.size() == 0) return true;

    for (String group : bookmarksToOCR) {
      for (String bookmark : ocrContext.getBookmarks())
        if (group.equalsIgnoreCase(bookmark)) return true;
      for (String category : ocrContext.getCategories())
        if (group.equalsIgnoreCase(category)) return true;
    }
  ...
  /**
   * Executes the configured external command and passes the given document stream as a simple XHTML
   * document to the given SAX content handler.
   */
  @Override
  public void parse(
    ...
    try {
      ...
      if (outDir != null) outputBase = new File(outDir.getPath(), TEXT_DIR);

      if (size >= MIN_SIZE
          && size <= MAX_SIZE
          && (bookmarksToOCR == null || isFromBookmarkToOCR(itemInfo))
        ...
        if (outputBase != null && itemInfo != null && itemInfo.getHash() != null) {
          ...
          output =
              new File(
                  outputBase, outPath + "/" + outFileName + ".txt"); // $NON-NLS-1$ //$NON-NLS-2$
        }
        ...
        if (output == null || !output.exists()) {
          ...
          tmpOutput =
              new File(
                  outputBase, "ocr-" + random.nextLong() + ".txt"); // $NON-NLS-1$ //$NON-NLS-2$
  ...
  private static void storeOcrTextInDb(String id, String ocrText, File outputBase)
      throws IOException {
  ...
  public static void copyOcrResults(String hash, File inputBase, File outputBase)
      throws IOException {
    File sourceDb =
        new File(inputBase, OCRParser.TEXT_DIR + File.separator + OCRParser.OCR_STORAGE);
    File targetDb =
        new File(outputBase, OCRParser.TEXT_DIR + File.separator + OCRParser.OCR_STORAGE);
    if (!sourceDb.exists()) return;
    try (PreparedStatement ps =
        getConnection(sourceDb.getParentFile()).prepareStatement(SELECT_ALL)) {
      ps.setString(1, hash);
      ps.setString(
          2, hash.substring(0, hash.length() - 1) + (char) (hash.charAt(hash.length() - 1) + 1));
      ResultSet rs = ps.executeQuery();
      while (rs.next()) {
  ...
  private void parseTiff(
      XHTMLContentHandler xhtml, TemporaryResources tmp, File input, File output, String itemPath)
      throws IOException, SAXException, TikaException {
    ...
    try (ImageInputStream iis = ImageIO.createImageInputStream(input)) {
      ...
      for (int page = 0; page < numPages; page++) {
        ...
        try {
          ...
          if (image.getWidth() > MAX_CONV_IMAGE_SIZE || image.getHeight() > MAX_CONV_IMAGE_SIZE)
            image =
                ImageUtil.resizeImage(
                    image, MAX_CONV_IMAGE_SIZE, MAX_CONV_IMAGE_SIZE, BufferedImage.TYPE_3BYTE_BGR);

          imageFile =
              File.createTempFile("iped-ocr", "." + PDFToImage.EXT); // $NON-NLS-1$ //$NON-NLS-2$
          ...
          if (imageText.exists()) {
            if (outputBase != null) IOUtil.copyFile(imageText, output, true);
            imageText.delete();
          }
        } catch (IOException e) {
  ...
  private void parseNonStandard(
      XHTMLContentHandler xhtml, File input, File output, String mediaType, String itemPath)
      throws IOException, SAXException, TikaException {
    File imageFile = null;
    try {
      ...
      if (img != null) {
        ...
        if (img.getWidth() > MAX_CONV_IMAGE_SIZE || img.getHeight() > MAX_CONV_IMAGE_SIZE)
          img =
              ImageUtil.resizeImage(
                  img, MAX_CONV_IMAGE_SIZE, MAX_CONV_IMAGE_SIZE, BufferedImage.TYPE_3BYTE_BGR);

        imageFile =
            File.createTempFile("iped-ocr", "." + PDFToImage.EXT); // $NON-NLS-1$ //$NON-NLS-2$
        ImageIO.write(img, PDFToImage.EXT, imageFile);

        if (imageFile.exists()) parse(xhtml, imageFile, output, itemPath);
      }
    } finally {
      if (imageFile != null) imageFile.delete();
    }
    ...
    try {
      ...
      for (int page = 0; page < pdfConverter.getNumPages(); page++) {
        ...
        try {
          ...
          if (!success || !imageFile.exists()) continue;
          File imageText = new File(imageFile.getAbsolutePath() + ".txt"); // $NON-NLS-1$
          parse(xhtml, imageFile, imageText, itemPath);
          if (imageText.exists()) {
            if (outputBase != null) IOUtil.copyFile(imageText, output, true);
  ...
  private void parse(XHTMLContentHandler xhtml, File input, File output, String itemPath)
    ...
    if (output.exists()) extractOutput(output, xhtml);
  }

  private void extractOutput(File output, XHTMLContentHandler xhtml)
  ...
  private void extractOutput(String ocrText, XHTMLContentHandler xhtml)
      throws SAXException, IOException {
  ...
  private void logStream(final String logType, final InputStream stream, String itemPath) {
    new Thread() {
      ...
      public void run() {
        ...
        if (!msg.isEmpty()) LOGGER.debug("OCR msg from " + itemPath + "\t" + msg);
  ...
  public static List<String> checkVersionInfo(String... checkCmd)
