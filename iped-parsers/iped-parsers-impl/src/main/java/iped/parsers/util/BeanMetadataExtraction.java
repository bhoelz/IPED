...
public class BeanMetadataExtraction {
  ...
  public void extractEmbedded(
      int seq, ParseContext context, Metadata metadata, ContentHandler handler, Object bean)
      throws IOException {
  ...
  protected boolean extractEmbedded(
      int seq,
      ...
      try {
        ...
        if (colObj == null) {
          ...
          if (resolvedReferencedQuery != null) {
            entryMetadata.add(
                ExtraProperties.LINKED_ITEMS, parseQuery(resolvedReferencedQuery, bean));
          }
          ...
          for (PropertyDescriptor pd :
            ...
            if (pd.getReadMethod() != null && !"class".equals(pd.getName())) {
              ...
              if (pd.getDisplayName().equals(resolvedNameProp)) {
                ...
                entryMetadata.add(
                    TikaCoreProperties.TITLE, name); // adds the name property without prefix
              ...
              if (value != null) {
                ...
                if (isBean(value)) {
                  if (shouldMergeCollectionProperty(bean, pd) && isCollectionNotEmpty(value)) {
                    for (Iterator<?> iterator = IteratorUtils.getIterator(value);
                        iterator.hasNext(); ) {
          ...
          entryMetadata.add(
              TikaCoreProperties.TITLE, metadataName); // adds the name property without prefix
        ...
        if (children.size() > 0) {
          for (Iterator<ChildParams> iterator = children.iterator(); iterator.hasNext(); ) {
            ...
            if (this.extractEmbedded(
                childSeq, context, entryMetadata, cp.pd, handler, cp.value, seq)) {
              count++;
            }
          }
        }
  ...
  private void fillMetadataForNonBeanValue(
      Metadata entryMetadata, String metadataName, Object value) {
  ...
  public void registerPropertyNameMapping(
      Class beanClass, String propertyName, String metadataPropName) {
  ...
  public void registerTransformationMapping(
      Class beanClass, String metadataPropName, String transformationExpression) {
