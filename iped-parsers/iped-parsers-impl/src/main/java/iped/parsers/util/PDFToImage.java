...
/**
 * Classe utilitária para converter páginas de arquivos PDF para arquivos de imagem.
 *
 * @author Nassif
 */
@Slf4j
public class PDFToImage implements Closeable {
  ...
  private int RESOLUTION =
      Integer.valueOf(System.getProperty(RESOLUTION_PROP, "250")); // $NON-NLS-1$
  private boolean externalConversion =
      Boolean.valueOf(System.getProperty(EXTERNAL_CONV_PROP, "false")); // $NON-NLS-1$
  private String externalConvMaxMem =
      System.getProperty(EXTERNAL_CONV_MAXMEM_PROP, "512M"); // $NON-NLS-1$
  ...
  static {
    ...
    if (javaVer.startsWith("1.8.0")) { // $NON-NLS-1$
      ...
      if (idx > 0 && Integer.valueOf(javaVer.substring(idx + 1)) < 191)
        System.setProperty(
            "sun.java2d.cmm",
            "sun.java2d.cmm.kcms.KcmsServiceProvider"); //$NON-NLS-1$ //$NON-NLS-2$
    }
    ...
    try {
      if (!externalConversion) {
        if (PDFLIB.equals(PDFBOX)) {
          ...
          iceDoc.paintPage(
              page, g, GraphicsRenderingHints.PRINT, Page.BOUNDARY_CROPBOX, rotation, scale);
          g.dispose();
          success = ImageIO.write(buffImage, EXT, output);
        }
        if (!success)
          throw new IOException(
              "Error: no writer found for image format '" + EXT + "'"); // $NON-NLS-1$ //$NON-NLS-2$
        ...
        String[] cmd = {
          ...
          "-cp",
          classpath,
          "-Xmx" + externalConvMaxMem, // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        ...
        try {
          ...
          throw new TikaException(
              this.getClass().getSimpleName() + " interrupted", e); // $NON-NLS-1$
        }
      }
      ...
      log.warn(
          "{} error creating image of page {} of {}: {}",
