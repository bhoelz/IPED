...
public class MetadataUtil {
  ...
  private static Map<String, Class<?>> typesMap =
      Collections.synchronizedMap(
          new TreeMap<String, Class<?>>(StringUtil.getIgnoreCaseComparator()));
  ...
  private static final Set<String> customMetadataPrefixes =
  ...
  /**
   * Method to add a new metadata prefix. Can be called from parsers to install a new meta prefix.
   *
   * @param metaPrefix
   */
    ...
    try (BufferedReader reader =
        new BufferedReader(
            new InputStreamReader(
                MetadataUtil.class.getResourceAsStream("/AllowedRawMailHeaders.txt"),
  ...
  public static boolean isToAddRawMailHeader(String header) {
    return !BASIC_MAIL_HEADERS.contains(header)
        && (RAW_MAIL_HEADERS.contains(header)
  ...
  private static Map<String, String> getRenameMap() {
    ...
    rename.put(
        ExtraProperties.IMAGE_META_PREFIX + TIFF.EQUIPMENT_MAKE.getName(),
        ExtraProperties.IMAGE_META_PREFIX + "Make");
    rename.put(
        ExtraProperties.IMAGE_META_PREFIX + TIFF.EQUIPMENT_MODEL.getName(),
        ExtraProperties.IMAGE_META_PREFIX + "Model");
    rename.put(
        ExtraProperties.IMAGE_META_PREFIX + TIFF.IMAGE_WIDTH.getName(),
        ExtraProperties.IMAGE_META_PREFIX + "Width");
    rename.put(
        ExtraProperties.IMAGE_META_PREFIX + TIFF.IMAGE_LENGTH.getName(),
        ExtraProperties.IMAGE_META_PREFIX + "Height");
    rename.put(
        ExtraProperties.VIDEO_META_PREFIX + TIFF.IMAGE_WIDTH.getName(),
        ExtraProperties.VIDEO_META_PREFIX + "Width");
    rename.put(
        ExtraProperties.VIDEO_META_PREFIX + TIFF.IMAGE_LENGTH.getName(),
        ExtraProperties.VIDEO_META_PREFIX + "Height");
    rename.put(
        ExtraProperties.UFED_META_PREFIX + "Altitude",
        ExtraProperties.COMMON_META_PREFIX + TikaCoreProperties.ALTITUDE.getName());
  ...
  private static Map<String, String> getRenameOrRemoveMap() {
    // Properties here are renamed if is no value already associated with the new name, otherwise
    // they are simply removed.
    Map<String, String> renameOrRemove = new HashMap<String, String>();
    renameOrRemove.put(
        ExtraProperties.IMAGE_META_PREFIX + "Image Width",
        ExtraProperties.IMAGE_META_PREFIX + "Width");
    renameOrRemove.put(
        ExtraProperties.IMAGE_META_PREFIX + "Image Height",
        ExtraProperties.IMAGE_META_PREFIX + "Height");
  ...
  private static Map<String, Property> getCompositeProps() {
    ...
    for (Property prop : props) map.put(prop.getName(), prop);
    return map;
  }
  ...
  private static void normalizeGPSMeta(Metadata metadata) {
    String lat =
        StringUtils.firstNonBlank(
            metadata.get(Metadata.LATITUDE),
    ...
    String lon =
        StringUtils.firstNonBlank(
            metadata.get(Metadata.LONGITUDE),
  ...
  private static void removeDuplicateValues(Metadata metadata) {
    for (String key : metadata.names()) {
      ...
      String prev = null;
      for (String val : values) {
        if (!val.isEmpty() && !val.equals(prev)) unique.add(val);
        prev = val;
      }
      if (unique.size() != values.length) {
        metadata.remove(key);
        for (String val : unique) metadata.add(key, val);
      }
    }
  }
  ...
  private static void normalizeCase(Metadata metadata) {
    for (String key : metadata.names()) {
      ...
      if (first.equals(key)) continue;
      String[] values = metadata.getValues(key);
      metadata.remove(key);
      for (String val : values) metadata.add(first, val);
    }
  }
  ...
  private static void normalizeKeys2(Metadata metadata) {
    ...
    for (String key : metadata.names()) {
      if (generalKeys.contains(key)) continue;
      String newKey = key;
      int i = key.lastIndexOf(':');
      if (i != -1) newKey = key.substring(i + 1);
      ...
      if (!newkeys.contains(newKey)) {
        ...
        if (!newKey.equals(key)) {
          String[] values = metadata.getValues(key);
          metadata.remove(key);
          for (String val : values) metadata.add(newKey, val);
        }
      } else {
        ...
        if (equal) metadata.remove(key);
      }
    }
  }

  private static void normalizeMSGMetadata(Metadata metadata) {
    if (!metadata.get(Metadata.CONTENT_TYPE).equals(MediaTypes.OUTLOOK_MSG.toString())) return;
  ...
  private static void normalizeRecipients(
      Metadata metadata,
      String destMeta,
      Property recipMetaName,
      Property recipMetaDisplayName,
      Property recipMetaEmail) {
    ...
    int length =
        Math.max(
            Math.max(recipientNames.length, recipientsDisplay.length), recipientsEmails.length);
    for (int i = 0; i < length; i++) {
      String value = "";
      if (i < recipientNames.length && recipientNames[i] != null) value = recipientNames[i].trim();
      if (i < recipientsDisplay.length
          && recipientsDisplay[i] != null
          && !value.toLowerCase().contains(recipientsDisplay[i].toLowerCase().trim()))
        value += " " + recipientsDisplay[i].trim(); // $NON-NLS-1$
      if (i < recipientsEmails.length
          && recipientsEmails[i] != null
  ...
  private static void includePrefix(Metadata metadata, String prefix) {
    ...
    for (String key : keys) {
      ...
      String[] values = metadata.getValues(key);
      metadata.remove(key);
      for (String val : values) metadata.add(prefix + key, val);
    }
  }
  ...
  /**
   * Check if the item is a multiple frame image. It works only after VideoThumbTask is executed.
   */
  ...
  public static boolean isImageType(String mediaType) {
    return mediaType == null
        ...
        : mediaType.startsWith("image/")
            || mediaType.equals("application/coreldraw")
  ...
  private static void prefixDocMetadata(Metadata metadata) {
    ...
    if (contentType.startsWith("message")
        ...
        || BPListDetector.PLIST.toString().equals(contentType)) return;
    ...
    while (mediaType != null) {
      ...
      if (mediaType.toString().equals("application/x-tika-msoffice")
          || //$NON-NLS-1$
          mediaType.toString().equals("application/x-tika-ooxml")
          || //$NON-NLS-1$
  ...
  private static void cleanHtmlMeta(Metadata metadata) {
    ...
    for (String key : keys) {
      ...
      if (key.startsWith("\"")
          && key.endsWith("\"")
          && key.length() > 2) // $NON-NLS-1$ //$NON-NLS-2$
  ...
  public static final boolean isHtmlMediaType(MediaType mediaType) {
    return mediaType != null
        && (mediaType.getBaseType().equals(MediaType.TEXT_HTML)
            || mediaType.getBaseType().equals(MediaType.application("xhtml+xml"))
            || //$NON-NLS-1$
  ...
  public static final boolean isHtmlSubType(MediaType mediaType) {
    do {
      ...
      mediaType = MediaTypes.getParentType(mediaType);
      if (!hasParam && isHtmlMediaType(mediaType)) return true;
