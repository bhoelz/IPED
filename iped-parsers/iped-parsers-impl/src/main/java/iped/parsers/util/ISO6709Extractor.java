...
class ISO6709Extractor implements Serializable {
  ...
  private String getLng(String sign, String integer, String flot) {
    ...
    if (integer.length() == 3) {
      ...
      return calcDecimalDegrees(
          sign, integer.substring(0, 3), integer.substring(3, 5) + flotNormed);
    } else if (integer.length() == 7) {
      return calcDecimalDegrees(
          sign,
          integer.substring(0, 3),
          integer.substring(3, 5),
  ...
  private String getLat(String sign, String integer, String flot) {
    ...
    if (integer.length() == 2) {
      ...
      return calcDecimalDegrees(
          sign, integer.substring(0, 2), integer.substring(2, 4) + flotNormed);
    } else if (integer.length() == 6) {
      return calcDecimalDegrees(
          sign,
          integer.substring(0, 2),
          integer.substring(2, 4),
