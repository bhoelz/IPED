...
public class Util {
  ...
  private static String getContentPreview(InputStream is, Metadata m, String mimeType) {
    ...
    try {
      ...
      if (contentHandler.isWriteLimitReached(se)) limitReached = true;
    }
  ...
  public static String getContentPreview(String content, String mimeType) throws IOException {
    ...
    return getContentPreview(
        new ByteArrayInputStream(content.getBytes("UTF-8")), m, mimeType); // $NON-NLS-1$
  }
  ...
  public static String getExportPath(String hash, String ext) {
    if (hash == null || hash.length() < 2) return ""; // $NON-NLS-1$
    StringBuilder sb = new StringBuilder();
    sb.append("../../")
  ...
  public static String getParentPath(String path) {
    ...
    if (i <= 0) return null;
    if (i == path.length() - 1) return getParentPath(path.substring(0, path.length() - 1));
  ...
  public static String getTrueExtension(String origExt, MediaType mediaType)
    ...
    if (!mediaType.equals(MediaType.OCTET_STREAM)) {
      do {
        boolean first = true;
        for (String ext :
            getTikaConfig().getMimeRepository().forName(mediaType.toString()).getExtensions()) {
          if (first) {
            trueExt = ext;
            first = false;
          }
      ...
      } while (trueExt.isEmpty()
          && !MediaType.OCTET_STREAM.equals(
              (mediaType = getTikaConfig().getMediaTypeRegistry().getSupertype(mediaType))));
    }
