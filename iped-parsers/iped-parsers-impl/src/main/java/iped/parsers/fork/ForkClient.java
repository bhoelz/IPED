...
class ForkClient {
  ...
  private void consumeErrorStream() {
    Thread t =
        new Thread("ErrorStreamConsumer") {
          public void run() {
            int i = 0;
  ...
  public Throwable callInBackground(final String method, final Object... args)
    ...
    try {
      ...
      } else throw new RuntimeException("Unexpected error in fork client", exception);
    }
  }
  ...
  public Throwable clientCall(String method, Object... args) throws IOException, TikaException {
    ...
    for (int i = 0; i < args.length; i++) {
      if (args[i] instanceof Metadata) metadata = (Metadata) args[i];
    }
    for (int i = 0; i < args.length; i++) {
      sendObject(args[i], r);
    }
  ...
  private void sendObject(Object object, List<ForkResource> resources)
    ...
    if (object instanceof InputStream) {
      ...
      resources.add(
          new RecursiveMetadataContentHandlerResource((RecursiveParserWrapperHandler) object));
      object =
      ...
      object =
          new TeeContentHandler(
              (ContentHandler) object, new UniqueMetadataContentHandler(metadata));
      ...
      EmbeddedDocumentExtractor extractor =
          ((ParseContext) object).get(EmbeddedDocumentExtractor.class);
      ...
      } else contextProxy.set(EmbeddedDocumentExtractor.class, extractor);
      object = contextProxy;
    }
  ...
  /**
   * Creates a temporary jar file that can be used to bootstrap the forked server process. Remember
   * to remove the file when no longer used.
   *
