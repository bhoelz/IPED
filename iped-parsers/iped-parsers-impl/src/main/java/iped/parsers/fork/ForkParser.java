...
public class ForkParser extends AbstractParser {
      ...
      synchronized (ForkParser.class) {
        if (instance == null) {
          ...
          instance =
              new ForkParser(
                  getMainJarsPath(),
                  new File(plugin_dir).toPath(),
                  new ParserFactoryFactory(
                      ExternalParsingParserFactory.class.getName(), Collections.EMPTY_MAP));
  ...
  /**
   * If you have a directory with, say, tike-app.jar and you want the child process/server to build
   * a parser and run it from that -- so that you can keep all of those dependencies out of your
   * client code, use this initializer.
   *
   * @param tikaBin directory containing the tika-app.jar or similar -- full jar including tika-core
   *     and all desired parsers and dependencies
   ...
   * @param tikaBin directory containing the tika-app.jar or similar -- full jar including tika-core
   *     and all desired parsers and dependencies
   * @param parserFactoryFactory -- the factory to use to generate the parser factory in the child
   *     process/server
  ...
  /**
   * Sets the command used to start the forked server process. The given command line is split on
   * whitespace and the arguments "-jar" and "/path/to/bootstrap.jar" are appended to it when
   * starting the process. The default setting is "java -Xmx32m".
   *
   * @param java java command line
   * @deprecated since 1.8
   ...
   *
   * <p>If using a RecursiveParserWrapper, there are two options:
   *
   * <p>
   *
   * <ol>
   ...
   *       will act on the class but not proxy back the data. This can be used, for example, if all
   *       you want to do is write to disc, extend {@link AbstractRecursiveParserWrapperHandler} to
   *       write to disc when {@link
   ...
   * org.apache.tika.sax.RecursiveParserWrapperHandler#TIKA_CONTENT} key and then serialize and
   * proxy that data back.
   *
  ...
  private ForkClient newClient() throws IOException, TikaException {
    TimeoutLimits timeoutLimits =
        new TimeoutLimits(serverPulseMillis, serverParseTimeoutMillis, serverWaitTimeoutMillis);
   ...
   * The server will shutdown after this amount of time, and a new server will have to be started by
   * a new client.
   *
  ...
  /**
   * If there is a slowly building memory leak in one of the parsers, it is useful to set a limit on
   * the number of files processed by a server before it is shutdown and restarted. Default value is
   * -1.
   *
