...
class ForkServer implements Runnable {
  ...
  private void call(ClassLoader loader, Object object) throws Exception {
    ...
    try {
      ...
      for (int i = 0; i < args.length; i++) {
        ...
        if (args[i] instanceof ParseContext) updateParsingTimeout((ParseContext) args[i]);
  ...
  /**
   * Deserializes an object from the given stream. The serialized object is expected to be preceded
   * by a size integer, that is used for reading the entire serialization into a memory before
   * deserializing it.
   *
