 ...
 *     <p>Reference: http://moneymvps.org/faq/article/8.aspx
 */
public class OFXParser extends AbstractParser {

  private static final long serialVersionUID = 1L;
  private static Set<MediaType> SUPPORTED_MIMES =
      MediaType.set("application/x-ofx-v1", "application/x-ofx-v2");
  ...
  public void decodeSignon(ResponseEnvelope re, HSSFWorkbook workbook) {
    ...
    if (sr != null) {
      ...
      rowheadSig
          .createCell(cnb++)
          .setCellValue(Messages.getString("OFXParser.FinancialInstitutionId"));
      rowheadSig
          .createCell(cnb++)
      ...
      for (int i = 0; i < cnb; i++) sheetSig.autoSizeColumn(i);
    }
    cell = null;
    cellStyle = null;
    sr = null;
  ...
  public void decodeBank(ResponseEnvelope re, HSSFWorkbook workbook) {
    ...
    if (message != null) {
      ...
      List<BankStatementResponseTransaction> bank =
          ((BankingResponseMessageSet) message).getStatementResponses();
      ...
      if (bank != null) {
        ...
        for (BankStatementResponseTransaction b : bank) {
          ...
          HSSFSheet sheetBank =
              workbook.createSheet(Messages.getString("OFXParser.Bank") + "_" + bankCount);
          ...
          rowheadBank
              .createCell(cnb++)
              .setCellValue(Messages.getString("OFXParser.LedgerBalanceAmount"));
          rowheadBank
              .createCell(cnb++)
              .setCellValue(Messages.getString("OFXParser.LedgerBalanceDate"));
          rowheadBank
              .createCell(cnb++)
              .setCellValue(Messages.getString("OFXParser.AvailableBalanceAmount"));
          rowheadBank
              .createCell(cnb++)
              .setCellValue(Messages.getString("OFXParser.AvailableBalanceDate"));
          rowheadBank
              .createCell(cnb++)
              .setCellValue(Messages.getString("OFXParser.TransactionStartDate"));
          rowheadBank
              .createCell(cnb++)
              .setCellValue(Messages.getString("OFXParser.TransactionEndDate"));
          ...
          if (bsr != null) {
            ...
            for (int i = 0; i < cnb; i++) sheetBank.autoSizeColumn(i);
            ...
            if (tl != null) {
              ...
              HSSFSheet sheetTrans =
                  workbook.createSheet(
                      Messages.getString("OFXParser.Bank")
                          + "_"
                          + bankCount
              ...
              for (int i = 0; i < cnt; i++) sheetTrans.autoSizeColumn(i);
            }
          }
          bankCount++;
        }
  ...
  public void decodeCreditCard(ResponseEnvelope re, HSSFWorkbook workbook) {
    ...
    if (message != null) {
      ...
      List<CreditCardStatementResponseTransaction> cc =
          ((CreditCardResponseMessageSet) message).getStatementResponses();
      ...
      if (cc != null) {
        ...
        for (CreditCardStatementResponseTransaction b : cc) {
          ...
          HSSFSheet sheetType =
              workbook.createSheet(Messages.getString("OFXParser.CreditCard") + "_" + TypeCount);
          ...
          if (bsr != null) {
            ...
            for (int i = 0; i < cnb; i++) sheetType.autoSizeColumn(i);
            ...
            if (tl != null) {
              ...
              HSSFSheet sheetTrans =
                  workbook.createSheet(
                      Messages.getString("OFXParser.CreditCard")
                          + "_"
                          + TypeCount
                          + Messages.getString("OFXParser.Transactions"));
  ...
  public void decodeInvestimentWarn(ResponseEnvelope re, HSSFWorkbook workbook) {
    ...
    if (message != null) {
      List<InvestmentStatementResponseTransaction> cc =
          ((InvestmentStatementResponseMessageSet) message).getStatementResponses();
    ...
    if (message != null) {
      ...
      if (cc != null) {
        ...
        for (InvestmentStatementResponseTransaction b : cc) {
          ...
          HSSFSheet sheetType =
              workbook.createSheet(Messages.getString("OFXParser.Investment_") + TypeCount);

          HSSFRow rowheadType = sheetType.createRow(rnb++);
          rowheadType
              .createCell(cnb++)
              .setCellValue(Messages.getString("OFXParser.DateOfStatement"));
          ...
          if (bsr != null) {
            ...
            if (tl != null) {
              ...
              if (listTrans != null && listTrans.size() > 0) {
                ...
                HSSFSheet sheetTrans =
                    workbook.createSheet(
                        Messages.getString("OFXParser.Investment")
                            + "_"
                            + TypeCount
                            + " "
                ...
                rowheadTrans
                    .createCell(cnt++)
                    .setCellValue(Messages.getString("OFXParser.PayeeId"));
                rowheadTrans.createCell(cnt++).setCellValue(Messages.getString("OFXParser.Name"));
                rowheadTrans.createCell(cnt++).setCellValue(Messages.getString("OFXParser.Payee"));
                rowheadTrans
                    .createCell(cnt++)
              ...
              if (listTransBase != null && listTransBase.size() > 0) {
                ...
                HSSFSheet sheetTrans =
                    workbook.createSheet(
                        Messages.getString(
                            "OFXParser.Investment_" + TypeCount + " BaseInvestment"));
                ...
                rowheadTrans
                    .createCell(cnt++)
                    .setCellValue(Messages.getString("OFXParser.ServerId"));
                rowheadTrans
                    .createCell(cnt++)
                    .setCellValue(Messages.getString("OFXParser.TradeDate"));
                rowheadTrans
                    .createCell(cnt++)
  ...
  public void parse(
    ...
    byte[] orgBytes =
        stream.readNBytes(1 << 24); // Valid OFX files should be much smaller than 16 MB
    Charset cs = null;
    ...
    AggregateUnmarshaller<ResponseEnvelope> aggregate =
    ...
    try {
      ...
      if (!StandardCharsets.UTF_8.equals(cs)) {
        try {
          ...
          byte[] cleanBytes = clean(orgBytes, cs);
          reader =
              new BufferedReader(new InputStreamReader(new ByteArrayInputStream(cleanBytes), cs));
          re = (ResponseEnvelope) aggregate.unmarshal(reader);
          e1 = null; // Success
        } catch (Exception e2) {
        }
    ...
    meta.set(
        ...
        MediaType.parse("application/vnd.ms-excel").toString());
