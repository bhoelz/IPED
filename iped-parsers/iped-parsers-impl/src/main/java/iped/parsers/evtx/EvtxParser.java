...
public class EvtxParser extends AbstractParser {
  ...
  private static final MediaType EVTX_MIME_TYPE =
      MediaType.application("x-elf-file"); // $NON-NLS-1$
  private static final MediaType EVTX_RECORD_MIME_TYPE =
      MediaType.application("x-elf-record"); // $NON-NLS-1$

  private static final Set<MediaType> SUPPORTED_TYPES =
      Collections.singleton(EVTX_MIME_TYPE); // $NON-NLS-1$

  public static final String EVTX_METADATA_PREFIX = "WinEvt:";

  private static final Property RECCOUNT_PROP =
      Property.internalInteger(EVTX_METADATA_PREFIX + "recordCount");
  private static final Property RECID_PROP =
      Property.internalIntegerSequence(EVTX_METADATA_PREFIX + "eventRecordID");
  ...
  class EvtxRecordGroupExtractor {
    ...
    public EvtxRecordGroupExtractor(
        String subKey,
        ArrayList<EvtxRecord> recs,
        ParseContext context,
      ...
      try {
        ...
        if (providerVid == null) {
          ...
          providerMetadata.set(
              StandardParser.INDEXER_CONTENT_TYPE, EVTX_RECORD_MIME_TYPE.toString());
          ...
          providerMetadata.set(
              ...
              currentProvider.substring(currentProvider.lastIndexOf(":") + 1)); // eventtype
        ...
        for (Iterator iterator2 = recs.iterator(); iterator2.hasNext(); ) {
          ...
          String eventKey =
              EVTX_METADATA_PREFIX
                  + evtxRecord.getEventProviderName()
                  + ":"
                  + evtxRecord.getEventId();
          ...
          if (datas != null && datas.size() > 0) {
            for (Iterator<Entry<String, String>> iterator3 = datas.entrySet().iterator();
                iterator3.hasNext(); ) {
  ...
  public void parse(
    ...
    if (extractor.shouldParseEmbedded(metadata)) {
      ...
      HashMap<String, ArrayList<EvtxRecord>> subItens =
          new HashMap<String, ArrayList<EvtxRecord>>();

      EvtxRecordConsumer co =
          new EvtxRecordConsumer() {
            ...
            public void accept(EvtxRecord evtxRecord) {
              ...
              if (recs.size() >= maxEventPerItem) {
                EvtxRecordGroupExtractor ex =
                    new EvtxRecordGroupExtractor(
                        groupValue, recs, context, handler, maxProviderId, totalRecordCount);
                ex.run();
      ...
      for (Iterator<String> iterator = subItens.keySet().iterator(); iterator.hasNext(); ) {
        ...
        if (recs.size() > 0) {
          EvtxRecordGroupExtractor ex =
              new EvtxRecordGroupExtractor(
                  subKey, recs, context, handler, maxProviderId, totalRecordCount);
          ex.run();
        }
      }
