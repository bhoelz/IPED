        ...
        try {
          ...
          if (e instanceof EvtxInvalidChunkHeaderException) {
            if (i < chunckCount) {
              if (!dirty) {
                log.warn(
                    "Invalid chunk header found on non dirty evtx file: {}",
                    ((EvtxInvalidChunkHeaderException) e).getHeader());
              } else {
                log.warn(
                    "Invalid chunk header found before end of chunckcount on evtx file: {}",
                    ((EvtxInvalidChunkHeaderException) e).getHeader());
              }
            }
