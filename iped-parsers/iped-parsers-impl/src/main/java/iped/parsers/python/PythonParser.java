        ...
        for (File file : scripts) {
          ...
          try {
            ...
            Collection<String> mediaTypes =
                (Collection<String>)
                    jep.invoke(parser.getInstanceMethod("getSupportedTypes"), new ParseContext());
          ...
          try {
            Map<String, Number> map =
                (Map<String, Number>)
                    jep.invoke(parser.getInstanceMethod("getSupportedTypesQueueOrder"));
            for (Entry<String, Number> entry : map.entrySet()) {
              mediaTypesToQueueOrder.put(
                  MediaType.parse(entry.getKey()), entry.getValue().intValue());
            }
