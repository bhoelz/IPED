 ...
 *
 * <p>This file is licensed under the the zlib/libpng License.
 *
...
/**
 * A singleton that searches for loaded classes from the JRE and the Java classpath. This is the
 * default ClassEnquirer that is used if no ClassEnquirer is specified when constructing an
 * Interpreter. ClassList is also used by the command line <code>jep</code> script.
 *
 * <p>PS: This is a copy and paste of ClassList class from JEP with a fix for
 * https://github.com/ninia/jep/issues/323. To be removed when we upgrade to fixed JEP (Luis
 * Nassif).
 *
 * @author Mike Johnson
 */
public class JEPClassFinder implements ClassEnquirer {
  ...
  private void loadClassList() throws JepException {
     ...
     */
    ClassLoader[] classloadersToTry =
        new ClassLoader[] {
          Thread.currentThread().getContextClassLoader(), Jep.class.getClassLoader()
        };
    ...
    try {
      ...
      while ((line = reader.readLine()) != null) {
        ...
        for (i = 0; i < parts.length - 1; i++) {
          ...
          if (i < parts.length - 2) pname.append(".");
        }
   ...
   *
   * @param pkg a <code>String</code> value
  ...
  public static synchronized JEPClassFinder getInstance() throws JepException {
    if (JEPClassFinder.inst == null) JEPClassFinder.inst = new JEPClassFinder();
  ...
  public static void main(String argv[]) throws Throwable {
    if (argv.length > 0) {
      for (String arg : argv) {
        for (String c : JEPClassFinder.getInstance().getClassNames(arg)) System.out.println(c);
      }
    } else {
      ...
      // test loadPackages
      for (String c : JEPClassFinder.getInstance().getClassNames("jep")) System.out.println(c);
    }
  }
}
