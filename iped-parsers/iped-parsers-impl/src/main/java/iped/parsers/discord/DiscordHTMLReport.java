...
public class DiscordHTMLReport {
  ...
  private static final String defaultAvatarMe =
      Base64.getEncoder().encodeToString(readResourceAsBytes("discordme.png"));
  private static final String defaultAvatarOther =
      Base64.getEncoder().encodeToString(readResourceAsBytes("discord.png"));
  private static final String lottiejs =
      new String(readResourceAsBytes("lottie-player.js"), Charset.forName("UTF-8"));
  private static final String CSS =
      new String(
          readResourceAsBytes("/iped/parsers/discord/discord.css"), Charset.forName("UTF-8"));
  ...
  public byte[] convertToHTML(List<DiscordRoot> drl, IItemSearcher searcher, boolean includeHeader)
      throws IOException {
    ...
    SimpleDateFormat dateFormat =
        new SimpleDateFormat(Messages.getString("DiscordParser.DateFormat0"));
    ...
    for (DiscordRoot dr : drl) {
      ...
      if (!dr.getAuthor().getId().equals(userID)) {
        ...
        out.println("				<TD>");
        out.println(
            "					<span title='Channel ID="
                + format(dr.getChannel_id())
                + ", UserID="
                + format(dr.getAuthor().getId())
                + "'>"
                + format(dr.getAuthor().getName())
                + "<b>"
                + format(dr.getAuthor().getFullUsername())
                + "</b></span>");
        out.println("				</TD>");
        out.println("				<TD>");
      ...
      out.println(
          "<P>" + (dr.getTimestamp() == null ? "" : formatDate(dr.getTimestamp())) + "</P>");
      out.println(
          "<P>"
              + (dr.getEditedTimestamp() == null
                  ? ""
                  : "("
      ...
      if (dr.getCall() != null) {
        ...
        out.println(
            "		<TD>"
                + (dr.getTimestamp() == null ? "" : dateFormat.format(dr.getTimestamp()))
                + "</TD>");
        out.println("	</TR>");
        out.println("	<TR>");
        ...
        out.println(
            "		<TD>"
                + (dr.getCall().getEndedTimestamp() == null
                    ? ""
      ...
      if (dr.getReactions() != null) {
        ...
        out.println(
            "		<TD>"
                + Messages.getString("DiscordParser.Reactions")
      ...
      if (dr.getAttachments() != null && dr.getAttachments().size() > 0) {
        ...
        out.println("	<TR>");
        out.println(
            "		<TD colspan='2'>" + Messages.getString("DiscordParser.Attachments") + "</TD>");
        out.println("	</TR>");
        ...
        for (DiscordAttachment att : dr.getAttachments()) {
          ...
          if (att.getMediaHash() != null) {
            ...
            if (!item.getMediaTypeString().startsWith("video/")
                && !item.getMediaTypeString().startsWith("audio/")) {
              if (thumb != null) {
                out.println(
                    "       <img src=\"data:image/jpeg;base64,"
                ...
                out.println(
                    "       <div class=\"imageImg\" title=\"" + format(att.getFilename()) + "\">");
              } else {
                out.println(
                    "       <div class=\"attachImg\" title=\"" + format(att.getFilename()) + "\">");
              }
            }
            ...
            if (item.getMediaTypeString().startsWith("video/")) {
              if (thumb != null) {
                out.println(
                    "<img class=\"thumb iped-video\" src=\""
                        + "data:image/jpg;base64,"
                        + iped.parsers.whatsapp.Util.encodeBase64(thumb)
                        + "\""
                        + " data-src1=\""
                        + format(exportPath)
                        + "\""
                        ...
                        + "\"" //$NON-NLS-1$
                        + " title=\""
                        + att.getFilename() // $NON-NLS-1$
                        + "\"/>");
              } else {
                out.println(
                    "<div class=\"videoImg iped-video\" title=\"Video\""
                        + " data-src1=\""
                        + format(exportPath)
                        + "\""
                        + " data-src2=\""
