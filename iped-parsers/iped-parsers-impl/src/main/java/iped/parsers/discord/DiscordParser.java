...
public class DiscordParser extends AbstractParser {
  ...
  public static final MediaType DISCORD_ACCOUNT =
      MediaType.application("x-discord-account"); // $NON-NLS-1$

  private static final Set<MediaType> SUPPORTED_TYPES =
      new HashSet<MediaType>(Arrays.asList(MediaType.parse(CHAT_MIME_TYPE)));
  ...
  // not more desired
  @Deprecated
  private static void addChromeIndexMimeTypeSupport() {
    SUPPORTED_TYPES.add(CacheIndexParser.CHROME_INDEX_MIME_TYPE);
  }

  // this code may be removed when backward parsersconfig.xml compatibility were
  // not more desired
  @Deprecated CacheIndexParser cacheIndexParser;
  ...
  public void parseDiscord(
    ...
    String commonQuery =
        BasicProps.EVIDENCE_UUID
            ...
            + BasicProps.LENGTH
            + ":0 AND "
            + CacheIndexParser.IS_CACHE_INDEX_ENTRY.replace(":", "\\:")
            + ":true";
    ...
    try (InputStream is = TikaInputStream.get(() -> indexFile, new TemporaryResources())) {
        ...
        if (rootNode != null) {
          if (rootNode.isArray()) {
            ...
            if (rootNode.has("author") && rootNode.has("timestamp")) {
              ...
              if (msgsNode.isArray()) {
                for (JsonNode innerNode : msgsNode) {
                  if (innerNode.isArray()) {
                    List<DiscordRoot> innerList =
                        mapper.convertValue(innerNode, new TypeReference<List<DiscordRoot>>() {});
              ...
              log.warn(
                  "JSON object ignored in DiscordParser (not a message or query):"
                      + item.getPath());
            }
          }
        }
      ...
      DiscordAuthor me =
          extractAccount(searcher, commonQuery, mapper, avatarCache, handler, extractor);
      ...
      for (DiscordRoot dr : discordRoot) {
        for (DiscordSticker sticker : dr.getStickers()) {
          try {
            List<IItemReader> stickerItems =
                searcher.search(
                    commonQuery
                        + " AND "
                        + CacheIndexParser.CACHE_URL.replace(":", "\\:")
                        + ":\""
                        + sticker.getId()
                        + ".json\""
                        + " AND "
          ...
          if (sticker.getMediaHash() != null) {
            chatmetadata.add(
                ExtraProperties.LINKED_ITEMS, BasicProps.HASH + ":" + sticker.getMediaHash());
          }
        }
        for (DiscordAttachment da : dr.getAttachments()) {
          ...
          if (da.getMediaHash() != null) {
            chatmetadata.add(
                ExtraProperties.LINKED_ITEMS, BasicProps.HASH + ":" + da.getMediaHash());
          }
        }
      }
  ...
  private DiscordAuthor extractAccount(
      IItemSearcher searcher,
      String commonQuery,
      ObjectMapper mapper,
      HashMap<String, byte[]> avatarCache,
