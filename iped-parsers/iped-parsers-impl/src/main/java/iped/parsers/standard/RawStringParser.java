...
/**
 * Parser que extrai strings brutas de um arquivo qualquer. Útil para binários, tipos desconhecidos
 * e drivefreespace. É utilizada uma heurística para detectar codificações ISO-8859-1, UTF-8 e
 * UTF-16 mescladas num mesmo arquivo.
 */
public class RawStringParser extends AbstractParser {
  ...
  private static char[] getByteToCharMap() {
    ...
    for (int i = Byte.MIN_VALUE; i <= Byte.MAX_VALUE; i++) bytes[i & 0xFF] = (byte) i;
    try {
      char[] chars =
          new String(bytes, 0, bytes.length, "windows-1252").toCharArray(); // $NON-NLS-1$
      return chars;
  ...
  private void flushBuffer() throws SAXException {
    if (tmpPos - bufPos >= MIN_SIZE) bufPos = tmpPos - MIN_SIZE;

    // char[] chars = new String(buf, 0, bufPos, "windows-1252").toCharArray();
    char[] chars = new char[bufPos];
    for (int i = 0; i < bufPos; i++) chars[i] = byteToChar[buf[i] & 0xFF];

    handler.characters(chars, 0, chars.length);

    for (int k = 0; k < tmpPos - bufPos; k++) buf[k] = buf[bufPos + k];
  ...
  /**
   * Cria nova instancia para cada parsing para evitar acesso concorrente às mesmas variáveis locais
   * no caso de parsing concorrente com mesma instancia
   *
  ...
  private void safeParse(
    ...
    if (filterRandomBytes) stream = new RandomFilterInputStream(stream);
    ...
    do {
      ...
      while (inPos < inSize) {
        ...
        if (c == (byte) 0xC3) {
          ...
          if (tmpPos == BUF_SIZE) flushBuffer();
    ...
    // char[] chars = new String(buf, 0, bufPos, "windows-1252").toCharArray();
    char[] chars = new char[bufPos];
    for (i = 0; i < bufPos; i++) chars[i] = byteToChar[buf[i] & 0xFF];

    handler.characters(chars, 0, chars.length);

    if (filterRandomBytes) {
      Double compression = ((RandomFilterInputStream) stream).getCompressRatio();
      if (compression != null) metadata.set(COMPRESS_RATIO, Double.toString(compression));
    }
