...
/**
 * Parser padrão do IPED. Como o AutoDetectParser, detecta o tipo do arquivo e delega o parsing para
 * o parser apropriado. Porém aproveita o CONTENT_TYPE caso informado via metadados, evitando nova
 * detecção. Além disso utiliza o RawStringParser como fallback ou no caso de alguma Exceção durante
 * o parsing padrão. Finalmente, escreve os metadados ao final (inclusive de subitens).
 */
@Slf4j
public class StandardParser extends CompositeParser {
  ...
  public static final String METADATA_HEADER =
      Messages.getString("StandardParser.MetadataTitle"); // $NON-NLS-1$
  ...
  private boolean fallbackParserEnabled =
      Boolean.valueOf(System.getProperty(FALLBACK_PARSER_PROP, "true"));
  private boolean errorParserEnabled =
      Boolean.valueOf(System.getProperty(ERROR_PARSER_PROP, "true"));
  private boolean entropyTestEnabled =
      Boolean.valueOf(System.getProperty(ENTROPY_TEST_PROP, "true"));
  ...
  public StandardParser() {
    super(
        getTikaConfig().getMediaTypeRegistry(),
        ((CompositeParser) getTikaConfig().getParser()).getAllComponentParsers());
  ...
  public Parser getLeafParser(Metadata metadata) {
    ...
    while (parser instanceof CompositeParser || parser instanceof ParserDecorator) {
      ...
      else parser = ((ParserDecorator) parser).getWrappedParser();
    }
    return parser;
  }

  public static boolean isSpecificParser(Parser parser) {
    if (parser instanceof RawStringParser
        || parser instanceof TXTParser
        || parser instanceof EmptyParser
        || parser instanceof TextAndCSVParser) return false;
    else return true;
  }
  ...
  private Parser getBestParser(CompositeParser comp, Metadata metadata) {
    ...
    if (type != null) type = comp.getMediaTypeRegistry().normalize(type);
    while (type != null) {
  ...
  public void parse(
    ...
    if (context.get(Parser.class) == null) context.set(Parser.class, this);
    ...
    if (itemInfo != null) filePath = itemInfo.getPath();
    long length = -1;
    ...
    try {
      ...
      if (contentType == null) {
        try {
          ...
          log.warn(
              "{} Error detecting file type: {} ({} bytes)\t\t{}",
          ...
          if (metadata.get(INDEXER_TIMEOUT) == null) {
            if (canUseForkParser && ForkParser.isEnabled() && hasSpecificParser(metadata))
              ...
              if (contentType.startsWith(
                  MediaType.application(MediaTypes.UFED_MIME_PREFIX).toString())) {
            ...
            if (errorParser != null) errorParser.parse(tis, sch, metadata, context);
          }
        }
      ...
      if (!Thread.currentThread().isInterrupted()
        ...
        if ((e instanceof EncryptedDocumentException
                || (value = metadata.get("pdf:encrypted")) != null
                    && Boolean.valueOf(value)) // $NON-NLS-1$
            || ((value = metadata.get("Security")) != null
                && Integer.valueOf(value) == 1) // $NON-NLS-1$
            // TODO melhorar detecção de arquivos openoffice cifrados
            || (contentType.contains("vnd.oasis.opendocument")
                && e.getCause() != null // $NON-NLS-1$
                && (e.getCause()
                        .toString()
                        .contains("only DEFLATED entries can have EXT descriptor") // $NON-NLS-1$
                    || e.getCause()
                        .toString()
                        .contains("MalformedByteSequenceException")))) { // $NON-NLS-1$

          metadata.set(ENCRYPTED_DOCUMENT, "true"); // $NON-NLS-1$

        } else if (itemInfo != null
            && itemInfo.isCarved()
            && context.get(IgnoreCorruptedCarved.class) != null)
          throw new CorruptedCarvedException(e);
        else {
          if (e.getCause() instanceof IOException && IOUtil.isDiskFull((IOException) e.getCause()))
            log.error(
                "No space on temp folder to process {} ({} bytes)",
                filePath,
                lengthStr); //$NON-NLS-1$
          ...
          log.warn(
              "{} Parsing exception: {} ({} bytes)\t\t{}",
              Thread.currentThread().getName(),
              filePath, //$NON-NLS-1$
              lengthStr,
              e.getCause() != null ? e.getCause().toString() : e.toString());
          log.debug(
              Thread.currentThread().getName() + " Parsing exception: " + filePath,
              e); //$NON-NLS-1$
      ...
      if (printMetadata) {
        ...
        for (String name : names) {
          if (name != null
              ...
              && !name.equals("PLTE PLTEEntry")
              && !name.equals("Chroma Palette PaletteEntry") // $NON-NLS-1$ //$NON-NLS-2$
  ...
  private class NoEndBodyContentHandler extends ContentHandlerDecorator {
    ...
    public void characters(char[] ch, int start, int length) throws SAXException {
      ...
      if (!inStyle || !ignoreStyle) super.characters(ch, start, length);
    }
