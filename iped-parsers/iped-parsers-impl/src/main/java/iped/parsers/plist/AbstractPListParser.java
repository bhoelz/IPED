 ...
 *
 * @see
 *     https://github.com/apache/tika/blob/main/tika-parsers/tika-parsers-standard/tika-parsers-standard-modules/tika-parser-apple-module/src/main/java/org/apache/tika/parser/apple/PListParser.java
 */
public abstract class AbstractPListParser<T> implements Parser {
  ...
  protected abstract void processAndGenerateHTMLContent(NSObject obj, State state)
  ...
  protected void processDictionary(NSDictionary obj, String path, State state, boolean open)
      throws SAXException {
    ...
    for (Entry<String, NSObject> entry : obj.entrySet()) {
      ...
      processObject(
          value, appendPath(path, key), state, false); // nested items are not open by default
  ...
  protected void processPossibleDate(NSNumber number, String path, State state)
      throws SAXException {
