...
public class PListHelper {
  ...
  public static final int APPLE_OFFSET_TIMESTAMP =
      978307200; // difference between 2001-01-01T00:00:00Z and 1970-01-01T00:00:00Z
  ...
  public static Date getPossibleDate(NSNumber number) {
    ...
    if (number.isInteger()) {
      ...
      if (timeMillis > (nowMillis - THIRTY_YEARS_IN_MILLIS)
          && timeMillis < (nowMillis + TWO_YEARS_IN_MILLIS)) {
  ...
  public static Date getNSTimeDate(NSNumber number) {
    try {
      long timeMillis =
          (long)
              ((Double.parseDouble(number.stringValue()) + PListHelper.APPLE_OFFSET_TIMESTAMP)
                  * 1000);
      return new Date(timeMillis);
