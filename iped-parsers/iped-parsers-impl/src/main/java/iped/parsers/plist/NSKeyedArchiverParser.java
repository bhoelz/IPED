...
public class NSKeyedArchiverParser extends AbstractPListParser<NSKeyedArchiverParser.Extra> {
  ...
  private static final Set<MediaType> SUPPORTED_TYPES =
      Collections.singleton(PListDetector.NSKEYEDARCHIVER_PLIST);
  ...
  @Override
  protected void processAndGenerateHTMLContent(NSObject nso, State state)
      throws SAXException, TikaException {

    if (!(nso instanceof NSDictionary) || !PListDetector.isNSKeyedArchiver((NSDictionary) nso)) {
      log.error(
          "NSKeyedArchiver is unexpectedly invalid: {}", state.context.get(IItemReader.class));
    ...
    if (!(objects instanceof NSArray) || !((top instanceof NSDictionary))) {
      log.warn(
          "NSKeyedArchiver has no valid $object or $top: {}", state.context.get(IItemReader.class));
      processAsPList(nso, state);
      return;
    }
    ...
    state.xhtml.characters(
        "Archiver: "
            + archiver
            + ", Version: "
            + (version != null ? version : "N/A") //
            + ", Objects count: "
            + ((NSArray) objects).count());
    state.xhtml.endElement("p");

    try {
      // process NSKeyedArchiver
      Set<Integer> alreadyVisitedUIDs =
          new HashSet<>(); // array to control already written objects and avoid infinite loops
  ...
  public void processAsPList(NSObject nso, State state) throws SAXException {
    ...
    AbstractPListParser<Void>.State plistState =
        plistParser
        .new State(state.xhtml, state.metadata, state.embeddedDocumentExtractor, state.context);
  ...
  @Override
  protected void processObject(NSObject obj, String path, State state, boolean open)
      throws SAXException {
  ...
  protected void processDictionary(
      NSDictionary obj, String path, AbstractPListParser<Extra>.State state, boolean open)
      throws SAXException {
  ...
  private void processKALocation(NSDictionary obj, String path, State state) throws SAXException {
    ...
    if (state.embeddedDocumentExtractor.shouldParseEmbedded(state.metadata)) {
      try {
        ...
        state.embeddedDocumentExtractor.parseEmbedded(
            InputStream.nullInputStream(), state.xhtml, metadata, true);
      } catch (IOException e) {
  ...
  private void processKADictionary(NSDictionary obj, String path, State state) throws SAXException {
    ...
    boolean allKeysAreValid =
        Arrays.asList(indexedKeys.getArray()).stream() //
    ...
    if (indexedKeys.count() == objects.count() && allKeysAreValid) {
      ...
      state.xhtml.endElement("summary");
      processArray(
          objects, appendPath(path, className + METADATA_KEY_SEPARATOR + NS_OBJECTS), state);
      state.xhtml.endElement("details");
    }
