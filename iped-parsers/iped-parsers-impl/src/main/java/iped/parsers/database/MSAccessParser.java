...
public class MSAccessParser extends AbstractParser {
  ...
  private static final Set<MediaType> SUPPORTED_TYPES =
      Collections.singleton(MediaType.application("x-msaccess")); // $NON-NLS-1$
  ...
  public void parse(
    ...
    try {
      ...
      EmbeddedDocumentExtractor embeddedParser =
          context.get(
      ...
      for (String tableName : tableNames) {
        ...
        xhtml.characters(
            Messages.getString("MSAccessParser.Table")
                + tableName
                + "\r\n"); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        for (Row row : table) {
          ...
          for (Column column : table.getColumns()) {
            ...
            xhtml.startElement("td"); // $NON-NLS-1$
            if (value != null)
              if (value instanceof Date) xhtml.characters(df.format(value));
                ...
                try {
                  ...
                  } else blobStream = oleBlob.getBinaryStream();
                  ...
                  if (blobStream != null) IOUtil.closeQuietly(blobStream);
                }

              } else if (column.getType().equals(DataType.BINARY)) {
                embeddedParser.parseEmbedded(
                    new ByteArrayInputStream((byte[]) value), xhtml, new Metadata(), true);

              } else xhtml.characters(value.toString().trim());
