...
public abstract class ExtractorAndroidNew extends Extractor {
  ...
  public ExtractorAndroidNew(
      String itemPath, File databaseFile, WAContactsDirectory contacts, WAAccount account) {
    ...
    try (Connection conn = getConnection()) {
      if (SQLite3DBParser.containsTable("jid_map", conn)) {
        try (Statement stmt = conn.createStatement();
            ResultSet rs = stmt.executeQuery(SELECT_JID_MAP)) {
          while (rs.next()) {
  ...
  private void extractAddOns(Connection conn, Message m, boolean hasReactionTable)
      throws SQLException {
  ...
  private void extractMessages(Connection conn, Map<Long, Chat> idToChat) throws SQLException {
    ...
    boolean hasTemplateTables =
        SQLite3DBParser.containsTable("message_template", conn)
    ...
    boolean hasEphemeralSettingTable =
        SQLite3DBParser.containsTable("message_ephemeral_setting", conn);
    boolean hasSystemChat = SQLite3DBParser.containsTable("message_system_chat_participant", conn);
    boolean hasSystemNumberChangeTable =
        SQLite3DBParser.containsTable("message_system_number_change", conn);
    ...
    try (PreparedStatement stmt = conn.prepareStatement(getSelectMessagesQuery(conn))) {
      ...
      while (rs.next()) {
        ...
        if (remoteResource == null
            || remoteResource.isEmpty()
            || (!c.isGroupOrChannelChat() && !c.isBroadcast())) {
        ...
        m.setMessageType(
            decodeMessageType(
                ...
                rs.getInt("privacyType"),
                m.getMediaMime()));

        if (m.getMessageType() == EPHEMERAL_SETTINGS_NOT_APPLIED
            || m.getMessageType() == IGNORE_MESSAGE) {
          // Ignore these type of message, as they do nothing and are not visible in the application
          // itself.
          continue;
        }
        ...
        if (hasEphemeralSettingTable
            && (m.getMessageType() == EPHEMERAL_DURATION_CHANGED
                || m.getMessageType() == EPHEMERAL_DEFAULT
                || m.getMessageType() == EPHEMERAL_CHANGED)) {
      ...
      if (messagesQuotes != null) {
        ...
        for (Message mq : messagesQuotes) {
          ...
          if (m != null) {
            ...
            if (mq.getRemoteResource() == null
                || mq.getRemoteResource().isBlank()
  ...
  private static final String SELECT_CHATS =
      "SELECT c._id as id, j.raw_string AS contact, c.subject, c.created_timestamp as creation,"
          + " sort_timestamp FROM chat c, jid j WHERE c.jid_row_id = j._id ORDER BY c.sort_timestamp DESC";

  private static final String SELECT_ADD_ONS =
      "SELECT message_add_on_type as type,timestamp, status,jid.raw_string as remoteResource,from_me as fromMe FROM message_add_on m left join jid on jid._id=m.sender_jid_row_id where parent_message_row_id=?";

  private static final String SELECT_ADD_ONS_REACTIONS =
      "SELECT message_add_on_type as type, timestamp, status,"
  ...
  private static final String SELECT_TEMPLATE =
      "SELECT content_text_data as content, footer_text_data as footer FROM message_template where message_row_id=?";

  private static final String SELECT_USERS_GROUP_ACTION =
      "select raw_string from message_system_chat_participant inner join jid on user_jid_row_id = jid._id where message_row_id=? order by _id";

  private static final String SELECT_SYSTEM_NUMBER_CHANGE =
      "select old.raw_string as oldUser, new.raw_string as newUser from message_system_number_change left join jid old on old_jid_row_id = old._id left join jid new on new_jid_row_id = new._id where message_row_id=?";

  private static final String SELECT_REVOKED =
      "select raw_string as admin from message_revoked left join jid on admin_jid_row_id = jid._id where message_row_id=?";

  private static final String SELECT_ORDER =
      "select raw_string as seller, order_title as title,"
  ...
  private static final String SELECT_PRODUCT =
      "select raw_string as seller, title,"
  ...
  private static final String SELECT_QUOTED_PRODUCT =
      "select raw_string as seller, title,"
          + " currency_code as currency, amount_1000 as amount, description"
          + " from message_quoted_product left join jid on business_owner_jid = jid._id where message_row_id=?";

  private static final String SELECT_POLL_OPTION =
      "SELECT option_name as name, vote_total as total FROM message_poll_option where message_row_id=? order by _id";

  private static final String SELECT_EPHEMERAL_SETTING =
      "SELECT setting_duration as duration FROM message_ephemeral_setting where message_row_id=?";

  private static final String SELECT_TEMPLATE_BUTTON =
      "SELECT text_data as text, extra_data as extra FROM message_template_button where message_row_id=? order by _id";

  private static String getSelectMessagesQuery(Connection conn) throws SQLException {
    String captionCol =
        SQLite3DBParser.checkIfColumnExists(conn, "message_media", "media_caption")
            ? "mm.media_caption"
            : "null";

    String sortCol =
        SQLite3DBParser.checkIfColumnExists(conn, "message", "sort_id") ? "m.sort_id" : "0";
    ...
    if (SQLite3DBParser.containsTable("message_system_initial_privacy_provider", conn)) {
      ...
      bizStateTableJoin =
          " left join message_system_initial_privacy_provider msipp on m._id=msipp.message_row_id";
    }
    ...
    if (SQLite3DBParser.containsTable("message_system_business_state", conn)) {
      ...
      privacyTypeTableJoin =
          " left join message_system_business_state msbs on m._id=msbs.message_row_id";
    }
  ...
  private static String getSelectMessagesQuotesQuery(Connection conn) throws SQLException {
    String captionCol =
        SQLite3DBParser.checkIfColumnExists(conn, "message_quoted_media", "media_caption")
            ? "mm.media_caption"
            : "null";
  ...
  private static final String SELECT_CALLS =
      "select log._id as id, log.call_id, log.video_call, log.duration,"
  ...
  private static final String SELECT_GROUP_MEMBERS =
      "select g._id as group_id, g.raw_string as group_name, u._id as user_id, u.raw_string as member "
  ...
  private static final String SELECT_JID_MAP =
      "SELECT a.raw_string AS lid, b.raw_string AS jid FROM jid_map map, jid a, jid b WHERE map.lid_row_id = a._id AND map.jid_row_id = b._id";
}
