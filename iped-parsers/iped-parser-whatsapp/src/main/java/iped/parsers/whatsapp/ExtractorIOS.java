...
public abstract class ExtractorIOS extends Extractor {
    ...
    if (recoverDeletedRecords) {
      try {
        ...
        undelete.addRecordValidator(
            "ZWAGROUPMEMBER", new WAIOSGroupMemberValidator()); // $NON-NLS-1$
        undelete.addTableToRecover("ZWACHATSESSION"); // $NON-NLS-1$
        undelete.addRecordValidator(
            "ZWACHATSESSION", new WAIOSChatSessionValidator()); // $NON-NLS-1$
    ...
    do {
      ...
      try (Connection conn = getConnection();
        ...
        try {
          ...
          hasZSTANZAIDAndZMETADATAColumns =
              SQLite3DBParser.checkIfColumnExists(conn, "ZWAMESSAGE", "ZSTANZAID")
                  && SQLite3DBParser.checkIfColumnExists(conn, "ZWAMEDIAITEM", "ZMETADATA");
          hasZMOVIEDURATIONColumn =
              SQLite3DBParser.checkIfColumnExists(conn, "ZWAMEDIAITEM", "ZMOVIEDURATION");
          hasContactIdentifierColumn =
              SQLite3DBParser.checkIfColumnExists(conn, "ZWACHATSESSION", "ZCONTACTIDENTIFIER");
        ...
        String chatListQuery =
            hasContactIdentifierColumn
                ? SELECT_CHAT_LIST_IDENTIFIER
        ...
        for (Chat c : undeletedChats) {
          if (!activeChats.contains(c.getId())) {
            ...
            if (firstTry && c.isDeleted()) {
              log.info(
                  "Recovered deleted chat for database "
                      + itemPath //$NON-NLS-1$
                      + " :"
                      + c.getSubject()
                      + " (" //$NON-NLS-1$ //$NON-NLS-2$
        ...
        for (Chat c : list) {
          ...
          if (messagesUndeletedTable != null && !undeletedMessages.isEmpty()) {
            mergeUndeletedMessages(
                c, messagesMap, undeletedMessages, mediaItems, groupMembers, firstTry);
          }
          if (c.isGroupChat()) {
            try {
  ...
  private void extractMessages(
      Connection conn, Map<Long, Chat> idToChat, boolean firstTry, boolean isGroupChat)
      throws SQLException {
    ...
    if (!hasMessageInfoTable) {
      sql =
          sql.replace("LEFT JOIN ZWAMESSAGEINFO INFO ON INFO.Z_PK = ZWAMESSAGE.ZMESSAGEINFO ", "");
  ...
  private void mergeUndeletedMessages(
      Chat chat,
  ...
  private void findQuotedMessages(
      List<Message> messages, Map<String, Message> messagesMap, Map<Long, Chat> idToChat) {
    ...
    switch (m.getMessageType()) {
      ...
      case TEMPLATE_MESSAGE:
        ...
        if (m.getMessageTemplate() != null
            && m.getMessageTemplate().getContent().equals(m.getData())) {
          m.setData("");
        }
        break;
  ...
  private Message createMessageFromUndeletedRecord(
      SqliteRow row, Chat chat, Map<Long, SqliteRow> mediaItems, Map<Long, SqliteRow> groupMembers)
      throws SQLException {
    Message m = new Message();
    if (account != null) m.setLocalResource(account.getId());
    ...
    try {
      m.setTimeStamp(
          new Date((row.getIntValue("ZMESSAGEDATE") + 978307200L) * 1000)); // $NON-NLS-1$
    } catch (RuntimeException e) {
    }
  ...
  private List<Chat> undeleteChats(
      SQLiteUndeleteTable undeleteChatsSessions, WAContactsDirectory contacts) {
      ...
      try {
        ...
        if (hasPushNameTable) {
          try (Statement stmt = conn.createStatement();
              ResultSet rs = stmt.executeQuery(SELECT_PUSH_NAMES)) {
            while (rs.next()) {
  ...
  private static final String SELECT_CHAT_LIST_IDENTIFIER =
      "SELECT ZWACHATSESSION.Z_PK as id, ZCONTACTJID AS contact, "
  ...
  private static final String SELECT_MESSAGES_GROUP =
      "SELECT ZWAMESSAGE.Z_PK AS id, ZWAMESSAGE.ZCHATSESSION " //$NON-NLS-1$
  ...
  private static final String SELECT_MESSAGES_USER_NOZTITLE =
      "SELECT ZWAMESSAGE.Z_PK AS id, ZCHATSESSION " //$NON-NLS-1$
          + "as chatId, ZFROMJID AS remoteResource, ZMESSAGESTATUS AS status, ZTEXT AS data, " //$NON-NLS-1$
          + "ZISFROMME AS fromMe, datetime(ZMESSAGEDATE + 978307200, 'unixepoch') AS timestamp, " //$NON-NLS-1$
          + "ZVCARDSTRING as vCardString, ZFILESIZE as mediaSize, ZMEDIALOCALPATH " //$NON-NLS-1$
  ...
  private static final String SELECT_MESSAGES_GROUP_NOZTITLE =
      "SELECT ZWAMESSAGE.Z_PK AS id, ZWAMESSAGE.ZCHATSESSION " //$NON-NLS-1$
          + "as chatId, ZMEMBERJID AS remoteResource, ZMESSAGESTATUS AS status, ZTEXT AS data, " //$NON-NLS-1$
          + "ZISFROMME AS fromMe, datetime(ZMESSAGEDATE + 978307200, 'unixepoch') AS timestamp, " //$NON-NLS-1$
          + "ZVCARDSTRING as vCardString, ZFILESIZE as mediaSize, ZMEDIALOCALPATH " //$NON-NLS-1$
  ...
  private static final Set<MessageType> MEDIA_MESSAGES =
      ImmutableSet.of(
          ...
          DOC_MESSAGE,
          IMAGE_MESSAGE,
          VIEW_ONCE_AUDIO_MESSAGE,
          VIEW_ONCE_IMAGE_MESSAGE,
          VIEW_ONCE_VIDEO_MESSAGE);
  ...
  private static class WAIOSMessageValidator implements SQLiteRecordValidator {
    ...
    public boolean validateRecord(SqliteRow row) {
      try {
        ...
        if (remoteId == null
            || !(remoteId.endsWith(WAContact.waSuffix)
                || remoteId.endsWith(WAContact.waNewsletterSuffix)
