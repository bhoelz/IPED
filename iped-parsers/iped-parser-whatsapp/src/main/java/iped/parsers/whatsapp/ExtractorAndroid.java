...
public abstract class ExtractorAndroid extends Extractor {
  ...
  protected List<Chat> extractChatList() throws WAExtractorException {
    ...
    if (recoverDeletedRecords) {
      try {
        ...
        log.warn(
            "Error recovering deleted records from Android WhatsApp Database " + itemPath,
            e); //$NON-NLS-1$
      }
    }

    Map<String, List<SqliteRow>> undeletedMessages =
        undeletedMessagesTable == null
            ? Collections.emptyMap()
            : undeletedMessagesTable.getTableRowsGroupedByTextCol("key_remote_jid"); // $NON-NLS-1$

    List<Chat> undeletedChats =
    ...
    firstTry =
        recoverDeletedRecords; // if recovery of deleted messages is disable, go straight to the
    // second try
    ...
    do {
      ...
      try (Connection conn = getConnection();
        ...
        try {
          ...
          hasGroupParticiantsTable =
              SQLite3DBParser.containsTable("group_participants", conn); // $NON-NLS-1$
          hasEditVersionCol =
              SQLite3DBParser.checkIfColumnExists(
                  conn, "messages", "edit_version"); // $NON-NLS-1$ //$NON-NLS-2$
          hasMediaCaptionCol =
              SQLite3DBParser.checkIfColumnExists(
                  conn, "messages", "media_caption"); // $NON-NLS-1$ //$NON-NLS-2$
          hasMediaDurationCol =
              SQLite3DBParser.checkIfColumnExists(
                  conn, "messages", "media_duration"); // $NON-NLS-1$ //$NON-NLS-2$
          hasForwardedCol =
              SQLite3DBParser.checkIfColumnExists(
                  conn, "messages", "forwarded"); // $NON-NLS-1$ //$NON-NLS-2$
          hasQuoteCol = SQLite3DBParser.checkIfColumnExists(conn, "messages", "quoted_row_id");
          if (!hasChatView) {
            hasSubjectCol =
                SQLite3DBParser.checkIfColumnExists(
                    conn, "chat_list", "subject"); // $NON-NLS-1$ //$NON-NLS-2$
          }
        ...
        for (Chat c : undeletedChats) {
          ...
          if (!activeChats.contains(remoteId)) {
            ...
            if (firstTry && c.isDeleted()) {
              log.info(
                  "Recovered deleted chat for database " //$NON-NLS-1$
                      + itemPath
                      + " :"
                      + c.getSubject() // $NON-NLS-1$
                      + " ("
                      + c.getRemote().getFullId()
                      + ")"); //$NON-NLS-1$ //$NON-NLS-2$
            }
          }
        }
        ...
        if (recoverDeletedRecords && !firstTry) {
          if (list.size() > 0 && undeletedMessages.size() > 0) {
            log.info(
                "Recovered deleted messages from corrupted database " + itemPath); // $NON-NLS-1$
          } else {
            log.info(
                "Was not able to recover messages from corrupted database "
                    + itemPath); //$NON-NLS-1$
        ...
        if (firstTry && recoverDeletedRecords) {
          ...
          if (isSqliteCorruptException(ex)) {
            ...
            log.warn(
                "Database "
                    + itemPath
                    + " is corrupt. Trying to recover data with fqlite"); //$NON-NLS-1$
            // //$NON-NLS-2$
          }
        }
        if (!tryAgain) {
  ...
  private List<Chat> undeleteChats(
      SQLiteUndeleteTable undeleteChatListTable,
      SQLiteUndeleteTable undeleteChatTable,
      SQLiteUndeleteTable undeleteJIDTable,
      WAContactsDirectory contacts) {
    List<Chat> result = new ArrayList<>();

    if (undeleteChatListTable != null
        && undeleteChatListTable.getTableRows() != null
        && !undeleteChatListTable.getTableRows().isEmpty()) {
    ...
    } else if (undeleteChatTable != null
        && undeleteChatTable.getTableRows() != null
        && !undeleteChatTable.getTableRows().isEmpty()) {
      if (undeleteJIDTable != null
          && undeleteJIDTable.getTableRows() != null
          && !undeleteJIDTable.getTableRows().isEmpty()) {
        // this is the case of a database with the view chat_view, that joins the tables chat and
        // jid
  ...
  private boolean databaseHasSortTimestamp(Connection conn) throws SQLException {
    ...
    try (ResultSet rs =
        md.getColumns(null, null, "chat_list", "sort_timestamp")) { // $NON-NLS-1$ //$NON-NLS-2$
      if (rs.next()) {
        result = true;
      }
    }
  ...
  private boolean databaseHasChatView(Connection conn) throws SQLException {
    ...
    DatabaseMetaData md = conn.getMetaData();
    try (ResultSet rs =
        md.getColumns(null, null, "chat_view", "raw_string_jid")) { // $NON-NLS-1$ //$NON-NLS-2$
      if (rs.next()) {
        result = true;
      }
    }
  ...
  private List<Message> extractMessages(
      ...
      boolean isGroupChat,
      Map<String, List<SqliteRow>> undeletedMessages,
      SQLiteUndeleteTable undeleteTable,
      boolean hasThumbTable,
      boolean hasEditVersionCol,
      boolean firstTry)
      throws SQLException {
    ...
    if (hasQuoteCol) {
      String queryQuote =
          getMessagesQueryQuote(
              hasThumbTable, hasEditVersionCol, hasMediaDurationCol, hasMediaCaptionCol);
      ...
      try (PreparedStatement stmt = conn.prepareStatement(queryQuote)) {
        ...
        try (ResultSet rs = stmt.executeQuery()) {
          while (rs.next()) {
            Message m =
                createMessageFromDBRow(
                    rs, remote, isGroupChat, false, hasThumbTable, hasEditVersionCol);
    ...
    if (recoverDeleted) {
      ...
      SQLiteUndeleteTableResultSetAdapter rs =
          new SQLiteUndeleteTableResultSetAdapter(
              undeletedMessages.getOrDefault(id, Collections.emptyList()),
              undeleteTable.getColumnNames(),
          ...
          if (!firstTry || rs.getCurrentRow().isDeletedRow()) {
            Message m =
                createMessageFromDBRow(
                    ...
                    remote,
                    isGroupChat,
                    rs.getCurrentRow().isDeletedRow(),
                    hasThumbTable,
                    hasEditVersionCol);

            if (!activeMessages.contains(
                new MessageWrapperForDuplicateRemoval(
                    m))) { // do not include deleted message if already there
              if (!activeMessageIds.containsKey(m.getId())
                  || !compareMessagesAlmostTheSame(
                      activeMessageIds.get(m.getId()),
                      m)) { // also remove messages with same id that have the same start text
                // (possibly corrupted recovered record)
                messagesMapIdQuote.put(m.getIdQuote(), m);
                if (m.getUuid() != null && !m.getUuid().isEmpty()) {
                  messagesMapUuid.put(m.getUuid(), m);
                }
    ...
    for (Message mq : messagesQuotes) {
      ...
      if (m != null) { // Has quote
        Message original =
            messagesMapUuid.get(mq.getUuid()); // Try to find orginal message in messages
        ...
        } else { // not found original message reference, get info from message_quotes table, less
          // complete
  ...
  private Message createMessageFromDBRow(
      ResultSet rs,
      WAContact remote,
      boolean isGroupChat,
      boolean deleted,
  ...
  protected Message.MessageType decodeMessageType(
      int messageType, int status, Integer edit_version, String caption, int mediaSize) {
  ...
  /** ** static strings *** */
  private static final String SELECT_CHAT_LIST =
      "SELECT _id as id,key_remote_jid AS contact," //$NON-NLS-1$
          + " subject, creation, sort_timestamp FROM chat_list ORDER BY sort_timestamp DESC"; //$NON-NLS-1$

  private static final String SELECT_CHAT_LIST_NO_SORTTIMESTAMP =
      "SELECT _id as id,key_remote_jid AS contact," //$NON-NLS-1$
          + " subject, creation FROM chat_list ORDER BY creation DESC"; //$NON-NLS-1$

  private static final String SELECT_CHAT_VIEW =
      "SELECT _id as id, raw_string_jid AS contact," //$NON-NLS-1$
          + " subject, created_timestamp as creation, sort_timestamp FROM chat_view ORDER BY sort_timestamp DESC"; //$NON-NLS-1$

  private static final String SELECT_CHAT_LIST_NO_SUBJECT =
      "SELECT _id as id,key_remote_jid AS contact, " //$NON-NLS-1$
          + " null as subject, 1230768000000 as creation FROM chat_list"; //$NON-NLS-1$

  private static String getMessagesQuery(
      boolean hasThumbTable,
      boolean hasEditVersion,
      boolean hasMediaDuration,
      boolean hasMediaCaption,
  ...
  private static String getMessagesQueryQuote(
      boolean hasThumbTable,
      boolean hasEditVersion,
      boolean hasMediaDuration,
      boolean hasMediaCaption) {
   ...
   */
  private static final String SELECT_MESSAGES_NO_THUMBS_TABLE =
      "SELECT _id AS id, key_remote_jid " //$NON-NLS-1$
  ...
  private static final String SELECT_MESSAGES_THUMBS_TABLE =
      "SELECT _id AS id, messages.key_remote_jid " //$NON-NLS-1$
          + "as remoteId, remote_resource AS remoteResource, status, data, " //$NON-NLS-1$
  ...
  private static final String SELECT_GROUP_MEMBERS =
      "select gjid as 'group', jid as member FROM group_participants where `group`=?"; //$NON-NLS-1$

  private static final String SELECT_QUOTES_NO_THUMBS_TABLE =
      "SELECT mq._id AS id, mq.key_remote_jid "
  ...
  private static final String SELECT_QUOTES_THUMBS_TABLE =
      "SELECT mq._id AS id, mq.key_remote_jid " //$NON-NLS-1$
  ...
  private static class WAAndroidJIDValidator implements SQLiteRecordValidator {
    ...
    public boolean validateRecord(SqliteRow row) {
      try {
        ...
        if (raw_string == null
            || !(raw_string.endsWith("whatsapp.net") || raw_string.endsWith("g.us"))) {
          return false;
        }
