...
public class LinkExtractor implements Closeable {
  ...
  private byte[] deriveCipherKey(byte[] mediakey, String mediaType) {
    ...
    byte[] key =
        hkg.expand(hkg.extract(new byte[32], mediakey), mediaType.getBytes(Charsets.UTF_8), 112);
  ...
  public void extractLinks() throws SQLException, DecoderException {
    ...
    try (PreparedStatement stmt =
        con.prepareStatement(sql.replaceAll("\\?", base64Hashes.toString()))) {
      ...
      while (rs.next()) {
        ...
        if (key != null) {
          ...
          LinkDownloader ld =
              new LinkDownloader(link, hash, connTimeout, readTimeout, cipherkey, iv);
  ...
  public static final String sql_ios =
      "SELECT ZMEDIAURL as url, ZVCARDNAME as hash, ZVCARDSTRING as tipo, null as data, ZMESSAGE as _id, ZMEDIAKEY as mediaKey from ZWAMEDIAITEM "
  ...
  public static final String sql_android_old =
      "SELECT m.media_url as url,m.media_hash as hash ,m.media_mime_type as tipo,m.thumb_image as data, m._id, null as mediaKey FROM messages m "
          + "where m.media_url like '%whatsapp%.enc' and m.media_hash is not null and m.media_hash in (?) group by url";

  public static final String sql_android =
      "SELECT message_url as url,file_hash as hash ,mime_type as tipo,null as data, message_row_id as _id, media_key as mediaKey FROM  message_media  "
