...
public class ReportGenerator {
  ...
  private final SimpleDateFormat timeFormat =
      new SimpleDateFormat("yyyy-MM-dd HH:mm:ss XXX"); // $NON-NLS-1$
    ...
    if (contact.getAvatar() != null)
      out.println(
          "<img src=\"data:image/jpg;base64,"
              + Util.encodeBase64(contact.getAvatar()) // $NON-NLS-1$
              + "\" width=\"112\"><br>"); //$NON-NLS-1$
    out.println(
        Messages.getString("WhatsAppReport.ContactID") + format(contact.getId())); // $NON-NLS-1$
    out.println(
        "<br>"
            + Messages.getString("WhatsAppReport.DisplayName")
            + format(contact.getDisplayName())); // $NON-NLS-1$ //$NON-NLS-2$
    out.println(
        "<br>"
            + Messages.getString("WhatsAppReport.WAName")
            + format(contact.getWaName())); // $NON-NLS-1$ //$NON-NLS-2$
    out.println(
        "<br>"
            + Messages.getString("WhatsAppReport.GivenName")
            + format(contact.getGivenName())); // $NON-NLS-1$ //$NON-NLS-2$
    out.println(
        "<br>"
            + Messages.getString("WhatsAppReport.NickName")
            + format(contact.getNickName())); // $NON-NLS-1$ //$NON-NLS-2$
    out.println(
        "<br>"
            + Messages.getString("WhatsAppReport.SortName")
            + format(contact.getSortName())); // $NON-NLS-1$ //$NON-NLS-2$
    out.println(
        "<br>"
            + Messages.getString("WhatsAppReport.Status")
            + format(contact.getStatus())); // $NON-NLS-1$ //$NON-NLS-2$
    if (contact.isDeleted()) {
      out.println(
          "<br><i>"
              + Messages.getString("WhatsAppReport.ContactDeleted")
              + "</i>"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    }
  ...
  public byte[] generateAccountHtml(WAAccount account) throws UnsupportedEncodingException {
    ...
    if (account.getAvatar() != null)
      out.println(
          "<img src=\"data:image/jpg;base64,"
              + Util.encodeBase64(account.getAvatar()) // $NON-NLS-1$
              + "\" width=\"112\"><br>"); //$NON-NLS-1$
    out.println(
        Messages.getString("WhatsAppReport.AccountID") + format(account.getId())); // $NON-NLS-1$
    out.println(
        "<br>"
            + Messages.getString("WhatsAppReport.WAName")
            + format(account.getWaName())); // $NON-NLS-1$ //$NON-NLS-2$
    out.println(
        "<br>"
            + Messages.getString("WhatsAppReport.Status")
            + format(account.getStatus())); // $NON-NLS-1$ //$NON-NLS-2$
  ...
  private static final String format(String s) {
    if (s == null || s.trim().isEmpty()) return "-"; // $NON-NLS-1$
  ...
  public byte[] generateNextChatHtml(
      Chat c,
    ...
    if ((!firstFragment && currentMsg == 0)
        || (currentMsg > 0 && currentMsg == c.getMessages().size())) return null;

    firstFragment = false;

    ByteArrayOutputStream chatBytes = new ByteArrayOutputStream();
    PrintWriter printWriter =
        new PrintWriter(new OutputStreamWriter(chatBytes, StandardCharsets.UTF_8)); // $NON-NLS-1$

    printMessageFile(
        printWriter,
        c.getTitle(),
        c.getPrintId(),
        c.getRemote().getAvatar(),
        c.isDeleted(),
        c.isBroadcast(),
        () -> {
          ByteArrayOutputStream bout = new ByteArrayOutputStream();
          PrintWriter out =
              new PrintWriter(new OutputStreamWriter(bout, StandardCharsets.UTF_8)); // $NON-NLS-1$
          if (c.getRecoveredFrom() != null) {
            out.println(
                "<div class=\"linha\"><div class=\"date\">" //$NON-NLS-1$
                    + Messages.getString("WhatsAppReport.RecoveredFrom")
                    + " "
                    + format(c.getRecoveredFrom())
                    + "</div></div>");
          }
          if (currentMsg > 0)
            out.println(
          ...
          if (histFrag.length() == 0) {
            histFrag.append(
                "<input type=\"hidden\" id=\"fragMessageChat\" value=\""
                    + Messages.getString("WhatsAppReport.ChatFragment")
                    + "\">");
            histFrag.append(
                "<input type=\"hidden\" id=\"fragMessageId\" value=\""
                    + Messages.getString("WhatsAppReport.ReferenceId")
                    + "\">");
            histFrag.append(
                "<input type=\"hidden\" id=\"fragMessageClose\" value=\""
                    + Messages.getString("WhatsAppReport.Close")
                    + "\">");
          }

          histFrag.append(
              "<input type=\"hidden\" id=\"frag" + frag + "\" value=\"" + lastId + "\">");
  ...
  private synchronized void printMessage(
    ...
    String bubbleFromSpecial =
        "<div class=\"bbls\"><div class=\"aw\"><div class=\"awls\"></div></div><div class=\"specialmessage from\">";
    ...
    switch (message.getMessageType()) {
      case UNKNOWN_MESSAGE:
        ...
        out.println(
            "<i>"
                + Messages.getString("WhatsAppReport.UnknownMessage")
                + " [ID: "
                + message.getId() // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                + "]</i>");
        break;
      case AI_THIRD_PARTY:
      ...
      case EPHEMERAL_DURATION_CHANGED:
        ...
        if (seconds == 0) {
          ...
          String duration =
              days > 1
                  ? days + " " + Messages.getString("WhatsAppReport.Days")
      ...
      case BUSINESS_META_SECURE_SERVICE:
        out.println("<div class=\"systemmessage\">"); // $NON-NLS-1$
        out.println(
            Messages.getString("WhatsAppReport.BusinessSecureService") + "<br>"); // $NON-NLS-1$
        break;
      case BUSINESS_TO_STANDARD:
        out.println("<div class=\"systemmessage\">"); // $NON-NLS-1$
        out.println(
            Messages.getString("WhatsAppReport.BusinessToStandard") + "<br>"); // $NON-NLS-1$
        break;
      ...
      case MESSAGES_NOW_ENCRYPTED:
        ...
        if (isGroupOrChannel) {
          out.println(
              Messages.getString("WhatsAppReport.GroupNowEncrypted") + "<br>"); // $NON-NLS-1$
        } else {
          out.println(
              Messages.getString("WhatsAppReport.ChatNowEncrypted") + "<br>"); // $NON-NLS-1$
        }
        break;
      ...
      case VIDEO_CALL:
        ...
        if (message.getDuration() > 0) {
          out.println(
              "<br>"
                  + Messages.getString("WhatsAppReport.Duration")
      ...
      case VOICE_CALL:
        ...
        out.println(
            Messages.getString("WhatsAppReport.VoiceCall") + "<br>"); // $NON-NLS-1$ //$NON-NLS-2$
        out.println(
            Messages.getString("WhatsAppReport.Duration")
                + ": "
                + formatMMSS(message.getDuration())); // $NON-NLS-1$ //$NON-NLS-2$
        break;
      case GROUP_CREATED:
        out.println("<div class=\"systemmessage\">");
      ...
      case COMMUNITY_NOT_AVAILABLE:
        out.println("<div class=\"systemmessage\">");
        out.println(
            name + " " + Messages.getString("WhatsAppReport.CommunityNotAvailable") + ".<br>");
        break;
      case GROUP_NOT_PART_OF_COMMUNITY:
        out.println("<div class=\"systemmessage\">");
        out.println(
            name + " " + Messages.getString("WhatsAppReport.GroupNotPartOfCommunity") + ".<br>");
        break;
      ...
      case ANY_COMMUNITY_MEMBER_CAN_JOIN_GROUP:
        out.println("<div class=\"systemmessage\">");
        out.println(
            Messages.getString("WhatsAppReport.AnyCommunityMemberCanJoinThisGroup") + "<br>");
        break;
      ...
      case USER_REMOVED_FROM_GROUP:
        ...
        if (users.size() == 0) {
          if (message.getMessageType() == MessageType.USER_ADDED_TO_GROUP
              || message.getMessageType() == MessageType.USER_ADDED_TO_COMMUNITY) {
          ...
          user =
              getBestContactName(user == null || user.isBlank(), user, contactsDirectory, account);
        ...
        if (!selfAction) {
          ...
          for (int i = 0; i < users.size(); i++) {
            ...
            String contactName =
                getBestContactName(
                    user == null || user.isBlank(), user, contactsDirectory, account);
      ...
      case GROUP_DESCRIPTION_CHANGED:
        out.println("<div class=\"systemmessage\">"); // $NON-NLS-1$
        out.println(
            Messages.getString("WhatsAppReport.GroupDescriptionChanged") + "<br>"); // $NON-NLS-1$
        break;
      ...
      case SECURITY_NOTIFICATIONS_NO_LONGER_AVAILABLE:
        out.println("<div class=\"systemmessage\">");
        out.println(
            Messages.getString("WhatsAppReport.SecurityNotificationsNoLongerAvailable") + "<br>");
        break;
      ...
      case GROUP_CHANGED_ALL_MEMBERS_CAN_ADD:
        ...
        out.println(
            name
                + " "
                + Messages.getString("WhatsAppReport.GroupChangedAllMembersCanAdd")
                + "<br>");
        break;
      case GROUP_CHANGED_ONLY_ADMINS_CAN_ADD:
        ...
        out.println(
            name
                + " "
                + Messages.getString("WhatsAppReport.GroupChangedOnlyAdminsCanAdd")
                + "<br>");
        break;
      case GROUP_CHANGED_ALL_MEMBERS_CAN_SEND:
        ...
        out.println(
            name
                + " "
                + Messages.getString("WhatsAppReport.GroupChangedAllMembersCanSend")
                + "<br>");
        break;
      case GROUP_CHANGED_ONLY_ADMINS_CAN_SEND:
        ...
        out.println(
            name
                + " "
                + Messages.getString("WhatsAppReport.GroupChangedOnlyAdminsCanSend")
                + "<br>");
        break;
      case GROUP_CHANGED_ALL_MEMBERS_CAN_EDIT:
        ...
        out.println(
            name
                + " "
                + Messages.getString("WhatsAppReport.GroupChangedAllMembersCanEdit")
                + "<br>");
        break;
      case GROUP_CHANGED_ONLY_ADMINS_CAN_EDIT:
        ...
        out.println(
            name
                + " "
                + Messages.getString("WhatsAppReport.GroupChangedOnlyAdminsCanEdit")
                + "<br>");
        break;
      ...
      case CHANGED_NUMBER_TO:
        ...
        if (users.size() >= 2) {
          out.println(
              getBestContactName(false, users.get(0), contactsDirectory, account)
                  + " "
                  + Messages.getString("WhatsAppReport.ChangedNumberTo")
                  + " "
                  + getBestContactName(false, users.get(1), contactsDirectory, account)
                  + ".<br>");
        }
        break;
      case CHANGED_NUMBER_CHATTING_WITH_NEW:
        ...
        out.println(
            name
                + " "
                + Messages.getString("WhatsAppReport.ChangedNumberChattingWithNew")
                + "<br>");
        break;
      case CHANGED_NUMBER_CHATTING_WITH_OLD:
        ...
        out.println(
            name
                + " "
                + Messages.getString("WhatsAppReport.ChangedNumberChattingWithOld")
                + "<br>");
        break;
      ...
      default:
        ...
        switch (message.getMessageType()) {
          ...
          case ORDER_MESSAGE:
            ...
            if (product != null) {
              String seller =
                  getBestContactName(false, product.getSeller(), contactsDirectory, account);
          ...
          case UNKNOWN_MEDIA_MESSAGE:
            if (message.getMediaCaption() != null) {
              out.print(
                  "<i>" + Messages.getString("WhatsAppReport.UnknownMediaMessage") + "</i><br>");
          ...
          case DELETED_MESSAGE:
            out.println(
                "<i>"
                    + deletedIcon
                    + Messages.getString("WhatsAppReport.MessageDeleted")
                    + "</i><br>"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            break;
          case DELETED_BY_ADMIN:
            out.print(
                "<i>" + deletedIcon + Messages.getString("WhatsAppReport.MessageDeletedByAdmin"));
            if (!message.getUsersAction().isEmpty()) {
              out.print(
                  " ("
                      + getBestContactName(
                          false, message.getUsersAction().get(0), contactsDirectory, account)
                      + ")");
            }
            out.println("</i><br>");
            break;
          case DELETED_BY_SENDER:
            out.println(
                "<i>"
                    + deletedIcon
                    + Messages.getString(
                        "WhatsAppReport.MessageDeletedBySender") //$NON-NLS-1$ //$NON-NLS-2$
                    + "</i><br>"); //$NON-NLS-1$
            break;
          case WAITING_MESSAGE:
            out.println(
                "<i>"
                    + Messages.getString("WhatsAppReport.WaitingMessage")
                    + "</i><br>"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            break;
          case AUDIO_MESSAGE:
          ...
          case STICKER_MESSAGE:
            ...
            if (mediaItem != null) {
              exportPath = Util.getExportPath(mediaItem);
            }

            if (query != null) {
              if (mediaItem != null) {
                out.println(
                    "<input class=\"check\" type=\"checkbox\" onclick=\"app.check("
                        + query
                        + ",this.checked)\""
                        + " name=\""
                        + mediaItem.getHash()
              ...
              if (mediaItem != null) {
                String href = Util.getReportHref(mediaItem);
            ...
            if (mediaItem != null) { // media file found
              ...
              if (generatedThumb != null) thumb = generatedThumb;
              ...
              if (message.getMessageType() == MessageType.AUDIO_MESSAGE
                ...
                String source = Util.getSourceFileIfExists(mediaItem).orElse("");
                if (message.getMessageType() == MessageType.AUDIO_MESSAGE
                    || message.getMessageType() == MessageType.VIEW_ONCE_AUDIO_MESSAGE) {
                  out.println(
                      Messages.getString("WhatsAppReport.AudioMessageTitle")
                          + "<br>"); //$NON-NLS-1$
                  out.println(
                      "<div class=\"audioImg iped-audio\" "
                          + " title=\"Audio\" "
                          + "data-src1=\""
                          + format(exportPath)
                          + "\" "
                  ...
                  out.println(
                      Messages.getString("WhatsAppReport.VideoMessageTitle")
                          + "<br>"); //$NON-NLS-1$
                  if (thumb != null) {
                    out.print("<img class=\"thumb iped-video\" src=\""); // $NON-NLS-1$
                    out.print(
                        "data:image/jpg;base64,"
                            + Util.encodeBase64(thumb)
                            + "\""); //$NON-NLS-1$ //$NON-NLS-2$
                    out.print(" data-src1=\"" + format(exportPath) + "\"");
                    out.print(" data-src2=\"" + format(source) + "\"");
                    out.println(
                        " title=\"" + getTitle(message) + "\">"); // $NON-NLS-1$ //$NON-NLS-2$

                  } else {
                    out.println(
                        "<div class=\"videoImg iped-video\" title=\"Video\""); //$NON-NLS-1$
                ...
                if (transcription != null) {
                  out.print(
                      Messages.getString("ReportGenerator.TranscriptionTitle")); // $NON-NLS-1$
    ...
    if (!mao.isEmpty()) {
      ...
      for (MessageAddOn a : mao) {
        if (a.getReaction() != null) {
          ...
          String reactionName =
              getBestContactName(a.isFromMe(), a.getRemoteResource(), contactsDirectory, account);
          aoDetails.append(' ');
    ...
    if (edit != null) {
      out.print(
          "<br>" + Messages.getString("WhatsAppReport.EditedOn") + " " + timeFormat.format(edit));
      if (hasStatus) {
    ...
    if (message.isDeleted()) {
      ...
      out.println(
          "<i>"
              + Messages.getString("WhatsAppReport.MessageDeletedRecovered")
              + "</i>"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    ...
    if (message.getRecoveredFrom() != null) {
      ...
      out.print(
          Messages.getString("WhatsAppReport.RecoveredFrom")
              + " "
              + format(message.getRecoveredFrom()));
      out.println("</span>"); // $NON-NLS-1$
    }

    if (isTo) out.println("</div><div class=\"aw\"><div class=\"awr\"></div></div>");
    if (isFrom) out.println("</div>");
    if (isToSpecial) out.println("</div><div class=\"aw\"><div class=\"awrs\"></div></div>");
    if (isFromSpecial) out.println("</div>");
  ...
  private String formatLocation(Message message) {
    ...
    String key =
        message.getMessageType() == MessageType.LOCATION_MESSAGE
            ? "LocationMessage"
            : "SharedLocationMessage";
    sb.append("<b>")
        .append(locationIcon)
        .append(Messages.getString("WhatsAppReport." + key))
        .append("</b><br>");
  ...
  private void printQuote(
      PrintWriter out, Message message, WAContactsDirectory contactsDirectory, WAAccount account) {
    ...
    if (messageQuote != null) {
      ...
      switch (messageQuote.getMessageQuotedType()) {
        case QUOTE_NOT_FOUND:
          quoteEnd =
              "</span><br><span style=\"float:none\" class=\"recovered\"><div class=\"deletedIcon\"></div><i>"
                  + Messages.getString("WhatsAppReport.QuoteNotFound")
                  + "</i>"
                  + quoteEnd;
          break;
        case QUOTE_STATUS:
          quoteEnd =
              "</span><br><span style=\"float:none\" class=\"outside\"><div class=\"statusIcon\"></div><i>"
                  + Messages.getString("WhatsAppReport.QuoteStaus")
                  + "</i>"
                  + quoteEnd;
          break;
        case QUOTE_CATALOG:
          quoteEnd =
              "</span><br><span style=\"float:none\" class=\"outside\"><div class=\"catalogIcon\"></div><i>"
                  + Messages.getString("WhatsAppReport.QuoteCatalog")
                  + "</i>"
                  + quoteEnd;
          break;
        case QUOTE_PRIVACY_GROUP:
          quoteEnd =
              "</span><br><span style=\"float:none\" class=\"outside\"><div class=\"privacyIcon\"></div><i>"
                  + Messages.getString("WhatsAppReport.QuotePrivacy")
                  + "</i>"
                  + quoteEnd;
          if (privateGroupName != null && !privateGroupName.isEmpty()) {
            String ms =
                Messages.getString("WhatsAppReport.QuotePrivacyMessage")
                    + ": "
                    + privateGroupName
                    + "</br> "
                    + Messages.getString("WhatsAppReport.ReferenceId")
        ...
        case QUOTE_PRIVACY_GROUP_NOT_FOUND:
          quoteEnd =
              "</span><br><span style=\"float:none\" class=\"recovered\"><div class=\"privacyDeleteIcon\"></div><i>"
                  + Messages.getString("WhatsAppReport.QuotePrivacyNotFound")
                  + "</i>"
                  + quoteEnd;
          String ms = "";
          if (privateGroupName != null && !privateGroupName.isEmpty()) {
        ...
        case AUDIO_MESSAGE:
          ...
          out.print(
              "<div class=\""
                  ...
                  + "\" "
                  + quoteClick
                  + "><div style=\"display:table-cell;\"><span class=\"quoteUser\">"
                  + quoteUser
        ...
        case GIF_MESSAGE:
          ...
          out.print(
              "<div class=\""
                  ...
                  + "\" "
                  + quoteClick
                  + "><div class=\"quoteTop\"><span class=\"quoteUser\">"
                  + quoteUser
                  + "</span><br><span class=\"quoteMsg\">"
                  + quoteIcon
                  + " "
  ...
  private String getBestContactName(
      Message message, WAContactsDirectory contactsDirectory, WAAccount account) {
    return getBestContactName(
        message.isFromMe(), message.getRemoteResource(), contactsDirectory, account);
  }
  ...
  private void printMessageFile(
    ...
    if (isDeleted) {
      deletedDiv =
          "<div class=\"linha\"><div class=\"recoveredChat\">"
