...
public class WhatsAppParser extends SQLite3DBParser {
  ...
  public static final MediaType WA_USER_XML =
      MediaType.application("x-whatsapp-user-xml"); // $NON-NLS-1$

  public static final MediaType WA_USER_PLIST = PListDetector.WA_USER_PLIST;

  public static final MediaType WHATSAPP_ACCOUNT =
      MediaType.application("x-whatsapp-account"); // $NON-NLS-1$

  public static final MediaType MSG_STORE = MediaType.application("x-whatsapp-db"); // $NON-NLS-1$

  public static final MediaType MSG_STORE_2 =
      MediaType.application("x-whatsapp-db-f"); // $NON-NLS-1$

  public static final MediaType WA_DB = MediaType.application("x-whatsapp-wadb"); // $NON-NLS-1$

  public static final MediaType CHAT_STORAGE =
      MediaType.application("x-whatsapp-chatstorage"); // $NON-NLS-1$

  public static final MediaType CHAT_STORAGE_2 =
      MediaType.application("x-whatsapp-chatstorage-f"); // $NON-NLS-1$

  public static final MediaType CONTACTS_V2 =
      MediaType.application("x-whatsapp-contactsv2"); // $NON-NLS-1$

  public static final MediaType WHATSAPP_CHAT =
      MediaType.parse("application/x-whatsapp-chat"); // $NON-NLS-1$

  public static final MediaType WHATSAPP_CONTACT =
      MediaType.parse("contact/x-whatsapp-contact"); // $NON-NLS-1$

  public static final MediaType WHATSAPP_MESSAGE =
      MediaType.parse("message/x-whatsapp-message"); // $NON-NLS-1$

  public static final MediaType WHATSAPP_ATTACHMENT =
      MediaType.parse("message/x-whatsapp-attachment"); // $NON-NLS-1$

  public static final MediaType WHATSAPP_CALL =
      MediaType.parse("call/x-whatsapp-call"); // $NON-NLS-1$
  ...
  private static final Pattern MSGSTORE_BKP =
      Pattern.compile("msgstore-\\d{4}-\\d{2}-\\d{2}"); // $NON-NLS-1$
  ...
  private static Set<MediaType> SUPPORTED_TYPES =
      MediaType.set(
          MSG_STORE, WA_DB, CHAT_STORAGE, CONTACTS_V2, WA_USER_PLIST, MSG_STORE_2, CHAT_STORAGE_2);

  private static final Map<String, WAContactsDirectory> contactsDirectoriesMap =
  ...
  public Set<MediaType> getSupportedTypes(ParseContext arg0) {
    if (!hashDependenciesChecked.getAndSet(true)) {
      ...
      if (!Boolean.valueOf(System.getProperty(SHA256_ENABLED_SYSPROP, "false"))) { // $NON-NLS-1$
        log.error(
            "SHA-256 is disabled. WhatsAppParser needs it to link attachments to chats!"); //$NON-NLS-1$
      }
    }
  ...
  @Field
  public void setLinkMediasByNameAndApproxSizeFallback(
      boolean linkMediasByNameAndApproxSizeFallback) {
  ...
  public void parse(
    ...
    String mimetype = metadata.get(ParserConstants.INDEXER_CONTENT_TYPE);
    if (mimetype == null) {
    ...
    try (TemporaryResources tmp = new TemporaryResources()) {
      ...
      if (mimetype.equals(WA_USER_PLIST.toString())) {
        ...
        if (mergeBackups || isDownloadMediaFilesEnabled())
          parseAndCheckIfIsMainDb(
              stream, handler, metadata, context, new ExtractorAndroidFactory());
        else
        ...
        mergeParsedDBsAndOutputResults(
            stream, handler, metadata, context, new ExtractorAndroidFactory());
  ...
  private void createReport(
    ...
    for (Chat c : chatList) {
      ...
      searchMediaFilesForMessagesInBatches(
          c.getMessages(), searcher, handler, extractor, dbPath, context, null);
      int frag = 0;
      int firstMsg = 0;
      ...
      int histFragCount = 0;
      byte[] bytes =
          reportGenerator.generateNextChatHtml(c, contacts, account, histFragCount, histFrag);
      while (bytes != null) {
        histFragCount++;
        ...
        if (frag > 0 || nextBytes != null) chatName += "_" + frag++; // $NON-NLS-1$
        ...
        chatMetadata.set(
            ExtraProperties.COMMUNICATION_PREFIX + "isEmpty", Boolean.valueOf(isEmpty).toString());
  ...
  private void parseWhatsAppMessages(
      InputStream stream,
      ContentHandler handler,
      Metadata metadata,
      ParseContext context,
      ExtractorFactory extFactory)
      throws IOException, SAXException, TikaException {
    ...
    if (extractor.shouldParseEmbedded(metadata)) {
      ...
      try {
        ...
        WAContactsDirectory contacts =
            getWAContactsDirectoryForPath(filePath, searcher, extFactory.getClass());

        String dbPath = ((ItemInfo) context.get(ItemInfo.class)).getPath();
        WAAccount account =
            getUserAccount(searcher, dbPath, extFactory instanceof ExtractorAndroidFactory);

        File tempDbFile = tis.getFile();
        extFactory.setConnectionParams(tis, metadata, context, this);
        Extractor waExtractor =
            extFactory.createMessageExtractor(
                filePath, tempDbFile, contacts, account, recoverDeletedRecords);
        List<Chat> chatList = waExtractor.getChatList();
        createReport(
            chatList, searcher, contacts, handler, extractor, account, tis.getFile(), context);

      } catch (Exception e) {
        sqliteParser.parse(tis, handler, metadata, context);
        if (e instanceof TikaException) throw (TikaException) e;
        else throw new TikaException("WAExtractorException Exception", e); // $NON-NLS-1$
  ...
  private void parseDB(
    ...
    try {
      IItemSearcher searcher = context.get(IItemSearcher.class);
      WAContactsDirectory contacts =
          getWAContactsDirectoryForPath(
              wcontext.getItem().getPath(), searcher, extFactory.getClass());

      WAAccount account =
          getUserAccount(
              searcher,
              wcontext.getItem().getPath(),
              extFactory instanceof ExtractorAndroidFactory);

      wcontext.setChalist(
          extractChatList(wcontext, extFactory, metadata, context, contacts, account));
  ...
  private void parseAndCheckIfIsMainDb(
      InputStream stream,
      ContentHandler handler,
      Metadata metadata,
      ParseContext context,
    ...
    if (isDownloadMediaFilesEnabled()) {
      ...
      if (downloadedFiles.get() > 0) {
        log.info(
            "Downloaded {} files from {}", downloadedFiles.get(), wcontext.getItem().getName());
      }
    }

    checkIfIsMainDBAndStore(wcontext);

    metadata.set(ParserConstants.INDEXER_CONTENT_TYPE, extFactory.getType2().toString());
  }
  ...
  private static boolean checkIfIsMainDBAndStore(WhatsAppContext wcontext) {
    ...
    if (!MSGSTORE_BKP.matcher(item.getName()).find()
        && !item.getPath().contains(MSGSTORE_CRYPTO)
  ...
  private List<Chat> extractChatList(
      WhatsAppContext wcontext,
      ExtractorFactory extFactory,
      Metadata metadata,
      ParseContext context,
    ...
    try (TemporaryResources tmp = new TemporaryResources()) {
      TikaInputStream tis =
          TikaInputStream.get(() -> wcontext.getItem().getSeekableInputStream(), tmp);
      ...
      Extractor waExtractor =
          extFactory.createMessageExtractor(
              filePath, tempFile, contacts, account, recoverDeletedRecords);
  ...
  private static synchronized void findOtherDBS(IItemSearcher searcher) {
    ...
    List<IItemReader> result =
        (searcher == null ? Collections.emptyList() : searcher.search(query));
  ...
  private void addBackupMessage(WhatsAppContext item, IItemReader main, XHTMLContentHandler xhtml)
    ...
    ((Metadata) i.getMetadata())
        .set(IS_BACKUP_FROM, main.getExtraAttribute(ExtraProperties.GLOBAL_ID).toString());
    xhtml.startDocument();
    xhtml.characters("Backup from " + main.getPath());
    xhtml.endDocument();
  }

  private void mergeParsedDBsAndOutputResults(
      InputStream stream,
      ContentHandler handler,
      Metadata metadata,
      ParseContext context,
    ...
    for (WhatsAppContext other : dbsFound.values().toArray(new WhatsAppContext[0])) {
      if (other == wcontext || other.getParsingError()) continue;
      synchronized (other) {
        if (other.getChalist() == null && !other.getParsingError()) {
          ...
          try {
            ...
            log.warn(
                "Could not parse DB {} ({} bytes): {}",
                other.getItem().getPath(),
                other.getItem().getLength(),
                e.toString());
            log.debug("", e);
          }
        }
    ...
    Collections.sort(
        dbsFoundList,
        new Comparator<WhatsAppContext>() {
          @Override
    ...
    try (TemporaryResources tmp = new TemporaryResources()) {
      ...
      WAContactsDirectory contacts =
          getWAContactsDirectoryForPath(DB.getPath(), searcher, extFactory.getClass());
      ...
      if (wcontext.isBackup() && wcontext.getMainDBItem() != null) {
        ...
        addBackupMessage(
            wcontext, wcontext.getMainDBItem(), new XHTMLContentHandler(handler, metadata));
        return;
      }
      ...
      for (WhatsAppContext db : dbsFoundList) {
        ...
        if (cm.isBackup(other.getChalist(), contacts)) {
          if (wcontext == mainDb) {
            ...
            log.info(
                "Recovered {} messages from {}",
                numMsgRecovered,
                other.getItem().getPath()); // $NON-NLS-1$
      ...
      if (dbsFound.size() == backupsMerged.get() + dbsSearchedForAndAdded) {
        ...
        dbsFound.values().stream()
            .filter(wacontext -> wacontext.getChalist() != null)
            .forEach(wacontext -> wacontext.getChalist().clear());
      }
    }
  }

  private void parseWhatsAppAccount(
      InputStream is, ParseContext context, ContentHandler handler, boolean isAndroid)
  ...
  private void parseEmbeddedWhatsAppAccount(
      WAAccount account, ParseContext context, ContentHandler handler)
      throws SAXException, IOException, TikaException {

    Metadata meta = new Metadata();
    meta.set(ParserConstants.INDEXER_CONTENT_TYPE, WHATSAPP_ACCOUNT.toString());
    ...
    if (extractor.shouldParseEmbedded(meta)) {
      ByteArrayInputStream bais =
          new ByteArrayInputStream(reportGenerator.generateAccountHtml(account));
      extractor.parseEmbedded(bais, handler, meta, false);
    }
  }

  private void fillAccountWithContactData(
      WAAccount account, IItemSearcher searcher, String dbPath) {
    if (account == null || account.getWaName() != null) {
      return;
    }
    try {
      WAContactsDirectory contacts =
          getWAContactsDirectoryForPath(dbPath, searcher, ExtractorAndroidFactory.class);
  ...
  private String getInternationalPhone(String id) {
    ...
    if (m.matches()) return "+" + id; // $NON-NLS-1$
    else return null;
  }
  ...
  private WAAccount getUserAccount(IItemSearcher searcher, String dbPath, boolean isAndroid) {
    ...
    if (searcher != null) {
      ...
      if (isAndroid) {
        names =
            new String[] {
              "com.whatsapp.w4b_preferences_light.xml",
              "com.whatsapp_preferences_light.xml",
              "com.whatsapp.w4b_preferences.xml",
  ...
  /**
   * Return a list of possible matches ordered by "priority" (first longer path matches, then the
   * index of names array).
   */
    ...
    while (!result.isEmpty()) {
      ...
      for (int i = 0; i < names.length; i++) {
        for (int j = 0; j < result.size(); j++) {
          ...
          if (isWABusiness(item.getPath()) == isWABusiness
              && item.getPath().startsWith(path)
  ...
  private void extractMessages(
    ...
    for (iped.parsers.whatsapp.Message m : messages) {
      ...
      meta.set(ParserConstants.INDEXER_CONTENT_TYPE, WHATSAPP_MESSAGE.toString());
      ...
      if (m.getMessageType() == MessageType.LOCATION_MESSAGE
        ...
        meta.set(
            ExtraProperties.LOCATIONS, m.getLatitude() + ";" + m.getLongitude()); // $NON-NLS-1$
      }
      ...
      if (m.isCall()) {
        ...
        meta.set(
            ConversationConstants.MESSAGE_DURATION, ReportGenerator.formatMMSS(m.getDuration()));
      }
      ...
      if (m.getVcards() != null && !m.getVcards().isEmpty()) {
        meta.set(ParserConstants.INDEXER_CONTENT_TYPE, VCardParser.VCARD_MIME.toString());
  ...
  private void storeLinkedHashes(
      List<Message> messages, Metadata metadata, IItemSearcher searcher) {
    ...
    if (searcher != null && contact.getAvatar() == null) {
      List<IItemReader> result =
          searcher.search(
              BasicProps.NAME
                  + ":\""
                  + escape(searcher, contact.getFullId())
                  + ".j\""); //$NON-NLS-1$ //$NON-NLS-2$
      if (result.isEmpty()) {
        if (contact.getAvatarPath() != null) {
          String avatarFileBase = contact.getAvatarPath();
          if (avatarFileBase.contains("/")) { // $NON-NLS-1$
            avatarFileBase =
                avatarFileBase.substring(avatarFileBase.lastIndexOf('/') + 1); // $NON-NLS-1$
          }
          avatarFileBase = escape(searcher, avatarFileBase);
          // Try file .jpg
          result =
              searcher.search(
                  BasicProps.NAME + ":\"" + avatarFileBase + ".jpg\""); // $NON-NLS-1$ //$NON-NLS-2$
          if (result.isEmpty()) {
            // Try file .thumb
            result =
                searcher.search(
                    BasicProps.NAME
                        + ":\""
                        + avatarFileBase
                        + ".thumb\""); //$NON-NLS-1$ //$NON-NLS-2$
          }
        }
      }
      if (result.isEmpty()) {
        if (contact.getId() != null && !contact.getId().isEmpty()) {
          Iterable<IItemReader> resultIterable =
              searcher.searchIterable(
                  BasicProps.NAME
                      + ":(\""
                      + escape(searcher, contact.getId())
                      + "\" AND (jpg thumb))"); //$NON-NLS-1$ //$NON-NLS-2$
  ...
  private String escape(IItemSearcher searcher, String string) {
    if (searcher != null) return searcher.escapeQuery(string);
    else return string;
  }
  ...
  private void parseWhatsAppContacts(
      InputStream stream,
      ContentHandler handler,
      Metadata metadata,
      ParseContext context,
    ...
    if (extractor.shouldParseEmbedded(metadata)) {
      ...
      try {
        WAContactsExtractor waExtractor =
            extFactory.createContactsExtractor(contactDbFile, recoverDeletedRecords);
        ...
        for (WAContact c : waExtractor.getContactsDirectory().contacts()) {
          ...
          cMetadata.set(ParserConstants.INDEXER_CONTENT_TYPE, WHATSAPP_CONTACT.toString());
          ...
          if (extractor.shouldParseEmbedded(cMetadata)) {
            ByteArrayInputStream chatStream =
                new ByteArrayInputStream(reportGenerator.generateContactHtml(c));
  ...
  private WAContactsDirectory getWAContactsDirectoryForPath(
      String path, IItemSearcher searcher, Class<?> extFactoryClass)
      throws IOException,
          WAExtractorException,
  ...
  private WAContactsDirectory getContacts(
      String path, IItemSearcher searcher, Class<?> extFactoryClass)
      throws IOException,
          WAExtractorException,
          InstantiationException,
    ...
    String query =
        BasicProps.PATH + ":\"" + searcher.escapeQuery(path) + "\""; // $NON-NLS-1$ //$NON-NLS-2$
    query +=
        " && "
            + BasicProps.CONTENTTYPE
            + ":(\""
            + WA_DB.toString()
            + "\" || \""
            + CONTACTS_V2.toString() // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            + "\")"; //$NON-NLS-1$
    ...
    ExtractorFactory extFactory =
        (ExtractorFactory) extFactoryClass.getDeclaredConstructor().newInstance();

    try (InputStream is = item.getBufferedInputStream()) {
      extFactory.setConnectionParams(is, null, context, this);
      WAContactsExtractor waExtractor =
          extFactory.createContactsExtractor(item.getTempFile(), recoverDeletedRecords);
      waExtractor.extractContactList();
      return waExtractor.getContactsDirectory();
    }
  }

  private abstract static class ExtractorFactory {
    ...
    abstract Extractor createMessageExtractor(
        String itemPath,
        File file,
        WAContactsDirectory directory,
        WAAccount account,
    ...
    private void setConnectionParams(
        InputStream is, Metadata metadata, ParseContext context, WhatsAppParser connFactory) {
      this.is = is;
  ...
  protected static class ExtractorAndroidFactory extends ExtractorFactory {
    ...
    @Override
    public Extractor createMessageExtractor(
        String itemPath,
        File file,
        WAContactsDirectory directory,
        WAAccount account,
    ...
    public WAContactsExtractor createContactsExtractor(File file, boolean recoverDeletedRecords) {
      return new WAContactsExtractorAndroid(
          file, new WAContactsDirectory(), recoverDeletedRecords) {
        @Override
        protected Connection getConnection() throws SQLException {
          return ExtractorAndroidFactory.this.getConnection();
        }
  ...
  private List<Future<?>> searchMediaFilesForMessagesInBatches(
      List<Message> messages,
      IItemSearcher searcher,
      ContentHandler handler,
      EmbeddedDocumentExtractor extractor,
    ...
    for (List<Message> listToProcess : listsToProcess) {
      futures.addAll(
          searchMediaFilesForMessages(
              listToProcess, searcher, handler, extractor, dbPath, context, downloadedFiles));
    }
    return futures;
  }

  private void setItemToMessage(
      IItemReader item,
  ...
  private List<Future<?>> searchMediaFilesForMessages(
      List<Message> messages,
      IItemSearcher searcher,
      ContentHandler handler,
      EmbeddedDocumentExtractor extractor,
    ...
    if (!hashesToSearchFor.isEmpty()) {
      ...
      List<IItemReader> result =
          (searcher == null ? Collections.emptyList() : searcher.search(allHashesQuery));
      for (IItemReader item : result) {
        ...
        setItemToMessage(
            item, messageList, hash == null ? null : "sha-256:" + hash, true, saveItemRef);
      }
    }
    ...
    if (!fileNameAndSizeToSearchFor.isEmpty()) {
      ...
      List<IItemReader> result =
          (searcher == null ? Collections.emptyList() : searcher.search(fileNameAndSizeQuery));
      for (IItemReader item : result) {
        if (item.getName() != null
          ...
          String query =
              BasicProps.NAME
                  + ":\""
                  + searcher.escapeQuery(fileName)
                  + "\" AND " //$NON-NLS-1$ //$NON-NLS-2$
                  + BasicProps.LENGTH
                  + ":"
                  + fileSize;
    ...
    if (!hashesToSearchFor.isEmpty()
      ...
      for (List<Message> messageList : hashesToSearchFor.values()) {
        for (Message m : messageList) {
          ...
          if (linkMediasByLongPathFallback > 0) {
            ...
            if (path != null
                && path.indexOf('/') > 0
                && path.length() >= linkMediasByLongPathFallback) {
      ...
      if (!fileNamesToSearchFor.isEmpty() || !pathsToSearchFor.isEmpty()) {
        ...
        List<IItemReader> result =
            (searcher == null ? Collections.emptyList() : searcher.search(fallBackQuery));
        for (IItemReader item : result) {
            ...
            if (fileName != null && item.getLength() != null && item.getLength() > 0) {
              ...
              if (messageList != null) {
                    ...
                    messageList.stream()
                        .filter(
                            m -> {
                              ...
                              long fileSize = item.getLength();
                              return (fileSize >= mediaSize + 1
                                  && fileSize <= mediaSize + 15
                ...
                setItemToMessage(
                    item,
                    messageList,
                    item.getHash() == null ? null : BasicProps.HASH + ":" + item.getHash(),
                    true,
                    saveItemRef);
              }
            }
    ...
    if (isDownloadMediaFilesEnabled() && downloadedFiles != null) {
      if (!hashesToSearchFor.isEmpty()) {
        ...
        try (LinkExtractor le =
            new LinkExtractor(
                dbPath,
                new HashSet<String>(hashesToSearchFor.keySet()),
  ...
  /**
   * Check it the media file is padded with zeros (check if all bytes beyond mediaSize are zeros)
   *
   * @param item
   * @param mediaSize
   * @return
