...
public abstract class WAContactsExtractorIOS extends WAContactsExtractor {
  ...
  private static final String SELECT_CONTACT_NAMES =
      "SELECT * FROM ZWAADDRESSBOOKCONTACT WHERE ZWHATSAPPID IS NOT NULL"; //$NON-NLS-1$

  private static final String SELECT_CONTACT_NAMES_1 =
      "SELECT * FROM ZWAPHONE " //$NON-NLS-1$
  ...
  public WAContactsExtractorIOS(
      File database, WAContactsDirectory directory, boolean recoverDeletedRecords) {
  ...
  public void extractContactList() throws WAExtractorException {
    ...
    if (recoverDeletedRecords) {
      try {
        ...
        undelete.addRecordValidator(
            "ZWAADDRESSBOOKCONTACT", new WAIOSContactValidator()); // $NON-NLS-1$
        undeletedContactsTable =
            undelete.undeleteData().get("ZWAADDRESSBOOKCONTACT"); // $NON-NLS-1$
      } catch (Exception e) {
        log.warn("Error recovering deleted records from IOS WhatsApp Contacts Database", e);
      }
    }
    ...
    try (Connection conn = getConnection();
      ...
      while (rs.next()) {
        ...
        if (given_name == null) given_name = getString(rs, "ZFIRSTNAME"); // $NON-NLS-1$
        c.setGivenName(given_name);
        String status = getString(rs, "ZABOUTTEXT"); // $NON-NLS-1$
        if (status == null) status = getString(rs, "ZTEXT"); // $NON-NLS-1$
        c.setStatus(status);
      }
    ...
    if (undeletedContactsTable != null) {
      for (var row : undeletedContactsTable.getTableRows()) {
        ...
        if (!directory.hasContact(
            Util.getNameFromId(id))) { // only recover contact if it does not exist already
          WAContact c = directory.getContact(id);
          ...
          if (given_name == null) given_name = row.getTextValue("ZFIRSTNAME"); // $NON-NLS-1$
          c.setGivenName(nullToEmpty(given_name));
          String status = row.getTextValue("ZABOUTTEXT"); // $NON-NLS-1$
          if (status == null) status = row.getTextValue("ZTEXT"); // $NON-NLS-1$
