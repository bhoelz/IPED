  ...
  public void parse(
    ...
    try {
      ...
      for (String key : keys) {
        ...
        metadata.remove(key);
        for (String val : values) metadata.add(ExtraProperties.VIDEO_META_PREFIX + key, val);
      }
      if (e != null) metadata.add(TikaCoreProperties.TIKA_META_EXCEPTION_WARNING, e.toString());
    }
  }
}
