...
public class EDBParser extends AbstractParser {
    ...
    synchronized (this.getClass()) {
      if (!tested)
        try {
          ...
          log.error(
              "Error testing esedbexport (libesedb): EDB databases will NOT be parsed!"); //$NON-NLS-1$
  ...
  public void parse(
    ...
    try {
      ...
      String[] cmd = {
        TOOL_PATH + TOOL_NAME, "-t", tmpFile.getAbsolutePath(), file.getAbsolutePath()
      }; //$NON-NLS-1$
      ...
      if (tables != null)
        for (File table : tables) {
          ...
          tableMetadata.set(
              ...
              TABLE_PREFIX + "-" + table.getName()); // $NON-NLS-1$
      ...
      if (e instanceof IOException && file == null) throw (IOException) e;
      e.printStackTrace();
  ...
  private File processTable(
    ...
    try (BufferedReader reader = new BufferedReader(new FileReader(tabFile));
        BufferedWriter writer =
            new BufferedWriter(new OutputStreamWriter(new FileOutputStream(html), charset)); ) {
          ...
          if (value.length() <= 128 || !isHexBinary(value))
            ...
            String name =
                TABLE_PREFIX
                    ...
                    + colnames.get(i)
                    + "_row"
                    + row //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                    + ".data"; //$NON-NLS-1$
