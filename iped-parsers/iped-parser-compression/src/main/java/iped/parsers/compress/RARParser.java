...
/**
 * RAR file parser. No support for Rar5 format as of 2020-10-11 because of junrar limitation.
 * Currently this parser is disabled and SevenZipParser handles RAR files.
 *
 * @author Nassif
 */
public class RARParser extends AbstractParser {
  ...
  private static final Set<MediaType> SUPPORTED_TYPES =
      Collections.singleton(MediaType.application("x-rar-compressed")); // $NON-NLS-1$
  ...
  public void parse(
    ...
    try {
      ...
      if (rar.isEncrypted()) throw new EncryptedDocumentException();
      ...
      while (header != null && !Thread.currentThread().isInterrupted()) {
         ...
         */
        if (header.isDirectory()) folderMap.put(header.getFileNameString(), header);
        header = rar.nextFileHeader();
      }
      // processa os diretórios em profundidade
      for (FileHeader dirheader : folderMap.values()) {
        String parent = CompressionUtil.getParentPath(dirheader.getFileNameString());
      ...
      do {
        ...
        if (header != null && !header.isDirectory()) {
          String parent = CompressionUtil.getParentPath(header.getFileNameString());
      ...
      tmp.close();
      xhtml.endDocument();
      if (entryEncrypted) throw new EncryptedDocumentException();
    }
  }

  private void parseSubitem(
      Archive rar,
      FileHeader header,
      String parent,
      ContentHandler handler,
      EmbeddedDocumentExtractor extractor)
      throws RarException, IOException, SAXException {
    ...
    try {
      ...
      entrydata.set(
          ...
          header.getFileNameString().replace("\\", "/")); // $NON-NLS-1$ //$NON-NLS-2$
