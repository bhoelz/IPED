...
public class SevenZipParser extends AbstractParser {
    ...
    try {
      ...
      inArchive =
          SevenZip.openInArchive(
              null, new RandomAccessFileInStream(randomAccessFile), "password"); // $NON-NLS-1$
      ...
      for (int i = 0; i < simpleInArchive.getNumberOfItems(); i++) {
        ...
        if (item.isFolder()) folderList.add(i);
        else fileList.add(i);
      }

      TreeMap<String, SubFile> fileMap = new TreeMap<>();
      MyExtractCallback extractCallback =
          new MyExtractCallback(simpleInArchive, tmp, handler, extractor, fileMap);
      ...
      for (SubFile subFile : fileMap.values().toArray(new SubFile[0])) {
        try (InputStream is =
            new BufferedInputStream(
                subFile.file != null
                    ? new FileInputStream(subFile.file)
      ...
      throw new TikaException(
          this.getClass().getSimpleName() + ": " + e1.getMessage(), e1); // $NON-NLS-1$
      ...
      try {
        if (item.isFolder()) {
          if (tmpFile == null && bufPos > 0) {
            tmpFile = tmp.createTemporaryFile();
            Files.copy(
                new ByteArrayInputStream(tmpBuf, 0, bufPos),
                tmpFile.toPath(),
        ...
        if (extractFiles) {
          if (tmpFile == null) {
            parseSubitem(
                new ByteArrayInputStream(tmpBuf, 0, bufPos), item, handler, extractor, fileMap);
          } else {
  ...
  private static void parseSubitem(
      InputStream is,
      ISimpleInArchiveItem item,
      ContentHandler handler,
      EmbeddedDocumentExtractor extractor,
    ...
    try {
      ...
      if (item.isFolder()) entrydata.set(ExtraProperties.EMBEDDED_FOLDER, "true"); // $NON-NLS-1$
  ...
  private static void parseMissingSubFolder(
      InputStream is,
      String path,
      ContentHandler handler,
      EmbeddedDocumentExtractor extractor,
