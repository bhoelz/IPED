 ...
 *
 * @author Nassif (better handling of encrypted zips and processing of XPS and generic OOXML)
 */
public class PackageParser extends AbstractParser {
  ...
  public static final Set<MediaType> SUPPORTED_TYPES =
  ...
  public void parse(
    ...
    try {
      ...
      if (sne.getFormat().equals(ArchiveStreamFactory.SEVEN_Z)) {
        ...
        throw new TikaException(
            "Unknown non-streaming format " + sne.getFormat(), sne); // $NON-NLS-1$
      }
    ...
    try {
      ...
      if (ais instanceof ZipArchiveInputStream && zfe.getFeature() == Feature.DATA_DESCRIPTOR) {
        parseZipFile(
            stream, xhtml, context, tmp, parentMap, zipSubitemList, isCarved, encrypted, encoding);
      } else throw new TikaException("UnsupportedZipFeature", zfe); // $NON-NLS-1$
      ...
      if (e.getMessage() != null
          && e.getMessage().contains("Unexpected record signature")) { // $NON-NLS-1$
      ...
      if (encrypted.bool) throw new EncryptedDocumentException();
    }
  }

  private void handleEntry(
      EntryInputStreamFactory factory,
      ArchiveEntry entry,
      boolean isCarved,
      BooleanWrapper encrypted,
      HashSet<String> parentMap,
      List<String> zipSubitemList,
  ...
  private InputStream getNewInputStream(InputStream stream, ParseContext context)
      throws IOException {
    try {
      stream.reset();
      return stream;
  ...
  private void createParent(
      String name, String parent, EmbeddedDocumentExtractor extractor, XHTMLContentHandler xhtml)
      throws SAXException, IOException {
  ...
  private void parseEntry(
    ...
    if (entry instanceof ZipArchiveEntry) {
      ...
      if (!factory.canReadEntryData()) {
        if (zae.getGeneralPurposeBit().usesEncryption()) entryEncrypted.bool = true;
        ...
        if (usesDataDescriptor && zae.getMethod() == ZipEntry.STORED) {
          throw new UnsupportedZipFeatureException(
              UnsupportedZipFeatureException.Feature.DATA_DESCRIPTOR, zae);
        }
      }
      for (ZipExtraField zef : zae.getExtraFields()) {
        ...
        if (zef instanceof X5455_ExtendedTimestamp) {
          entrydata.set(
              ExtraProperties.ACCESSED, ((X5455_ExtendedTimestamp) zef).getAccessJavaTime());
          entrydata.set(
              TikaCoreProperties.CREATED, ((X5455_ExtendedTimestamp) zef).getCreateJavaTime());
        }
      }
    }
    entrydata.set(ExtraProperties.PARENT_VIRTUAL_ID, parent); // $NON-NLS-1$
    if (entry.isDirectory()) entrydata.set(ExtraProperties.EMBEDDED_FOLDER, "true"); // $NON-NLS-1$
