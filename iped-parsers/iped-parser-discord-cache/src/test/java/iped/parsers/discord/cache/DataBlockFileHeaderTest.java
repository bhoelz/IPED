...
class DataBlockFileHeaderTest {
    ...
    private static byte[] buildHeader(long signature, long version,
                                      int fileNumber, int nextFileNumber,
                                      int blockSize, int entriesNumber, int entriesMax) {
        byte[] buf = new byte[HEADER_SIZE];
        int off = 0;
        writeLE32(buf, off, signature); off += 4;
        writeLE32(buf, off, version);   off += 4;
        writeLE16(buf, off, fileNumber);      off += 2;
        writeLE16(buf, off, nextFileNumber);  off += 2;
        writeLE32(buf, off, blockSize);  off += 4;
        writeLE32(buf, off, entriesNumber); off += 4;
        writeLE32(buf, off, entriesMax);    off += 4;
    ...
    void constructor_emptyStream_completesWithGarbageData() throws IOException {
        ...
        DataBlockFileHeader header = new DataBlockFileHeader(new ByteArrayInputStream(new byte[HEADER_SIZE]));
