...
public class CacheEntry {
  ...
  public String getRequestURL() {
    try {
      ...
      if (key == null) {
        ...
        if (longKeyAddress > 0) {
          try (InputStream is =
              longKeyAddressCacheAddress.getInputStream(dataFiles, externalFiles, null)) {
  ...
  public Map<String, String> getHttpResponse() {
    ...
    try (InputStream is = getResponseInfo()) {
      ...
      log.warn(
          "Truncated/invalid cache entry. File: {} | URL: {} | Content-Type: {}",
          fileName,
          url,
          contentType);
    }
  ...
  public String toString() {
    return "CacheEntry [hash="
        ...
        + longKeyAddress
        + ", dataStreamSize="
        + Arrays.toString(dataStreamSize)
        + ", dataStreamAdresses[ResponseInfo]="
        + dataStreamAdresses[0].toString()
        + ", dataStreamAdresses[ResponseRawDataStream]="
        + dataStreamAdresses[1].toString()
        + ", flags="
        + flags
        + ", paddings="
        + Arrays.toString(paddings)
        + ", selfHash="
        + selfHash
        + ", requestURL="
        + getRequestURL()
