...
public class LibpffPSTParser extends AbstractParser {
  ...
  private static String[] types = {
    ...
    "Task",
    "Appointment",
    "Activity", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$
    // //$NON-NLS-7$
    "Note",
    "Feed",
    "Document"
  ...
  private SimpleDateFormat df =
      new SimpleDateFormat("MMM dd, yyyy HH:mm:ss", Locale.US); // $NON-NLS-1$
    ...
    synchronized (this.getClass()) {
      if (!tested)
        try {
          ...
          String version =
              reader.readLine().replace(TOOL_NAME + " ", ""); // $NON-NLS-1$ //$NON-NLS-2$
          ...
          log.error(
              "Error testing pffexport (libpff): Outlook OST 2013 files will NOT be parsed!"); //$NON-NLS-1$
  ...
  public void parse(
    ...
    if (SUPPORTED_TYPES.isEmpty()) return;
  ...
  public void safeparse(
    ...
    try {
      ...
      if (extractOnlyDeleted) extractionMode = "recovered"; // $NON-NLS-1$
      else if (extractOnlyActive) extractionMode = "items"; // $NON-NLS-1$
      ...
      String[] cmd = {
        ...
        "html",
        "-t",
        tmpFile.getAbsolutePath(), // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      ...
      if (extractOnlyDeleted)
        log.error(
            "Recovery of deleted emails was interrupted on "
                + fileName
                + " "
  ...
  private void walkFolder(File dir, int parent)
  ...
  private void handleObject(File file, int parent)
  ...
  private int handleMessage(File file, int parent) throws IOException, SAXException {
    ...
    if (!attachNames.isEmpty()) {
      preview.append(
          "<b>"
              + Messages.getString("LibpffPSTParser.Attachments")
              + " ("
    ...
    for (int i = 1; i < types.length; i++) {
      ...
      if (item.exists()) {
        ...
        metadata.set(
              ...
              OutlookPSTParser.OUTLOOK_CONTACT_MIME); // $NON-NLS-1$
            ...
            if (!value.isEmpty()) {
              preview.append(
                  "<b>"
                      + l[0]
                      + ":</b> "
                      + SimpleHTMLEncoder.htmlEncode(value)
  ...
  private void writeRecipients(File file, Metadata metadata, StringBuilder preview) {
    ...
    if (recipients.exists()) {
      ...
      if (to.size() > 0) {
        ...
        preview.append(
            "<b>TO:</b> " //$NON-NLS-1$
                + SimpleHTMLEncoder.htmlEncode(to.stream().collect(Collectors.joining("; ")))
      ...
      if (cc.size() > 0) {
        ...
        preview.append(
            "<b>CC:</b> " //$NON-NLS-1$
                + SimpleHTMLEncoder.htmlEncode(cc.stream().collect(Collectors.joining("; ")))
                + "<br>"); //$NON-NLS-1$ //$NON-NLS-2$
      }
      if (bcc.size() > 0) {
        ...
        preview.append(
            "<b>bcc:</b> " //$NON-NLS-1$
                + SimpleHTMLEncoder.htmlEncode(bcc.stream().collect(Collectors.joining("; ")))
                + "<br>"); //$NON-NLS-1$ //$NON-NLS-2$
      }
    }
  }
  ...
  private void writeHeader(File file, Metadata metadata, StringBuilder preview) {
    ...
    if (outlookHeader.exists()) {
      ...
      String from = "",
          fromAddr = "",
          subject = Messages.getString("LibpffPSTParser.NoSubject"); // $NON-NLS-1$ //$NON-NLS-2$
      for (String line : lines) {
        String[] l = line.split(":", 2); // $NON-NLS-1$
        if (l.length > 1) {
          String value = l[1].trim();
  ...
  private List<String> getAttachNames(File file) {
    ...
    if (attachments.isDirectory())
      for (File attach : attachments.listFiles()) names.add(getAttachName(attach));
    return names;
  }
  ...
  private List<String> readAllLines(File file) {
    try {
      ...
      for (String newLine : newLines) {
        ...
        if (lines.length > 1) return Arrays.asList(lines);
      }
    } catch (IOException e) {
    }
