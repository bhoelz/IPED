...
public class IncrediMailParser extends AbstractParser {
  ...
  private static final Set<MediaType> SUPPORTED_TYPES =
      Collections.singleton(MediaType.application("x-incredimail")); // $NON-NLS-1$
      ...
      if (extractor.shouldParseEmbedded(metadata)) {
        ...
        while (((line = reader.readLine()) != null) && !Thread.currentThread().isInterrupted()) {
          ...
          try {
              ...
              if (line.startsWith("--") && line.endsWith("--")) { // $NON-NLS-1$ //$NON-NLS-2$
                ...
                if (line2 != null) {
                  ...
                  if (tmpStr.startsWith("return-path:")
                      || tmpStr.startsWith("mime-version:") // $NON-NLS-1$ //$NON-NLS-2$
                      || tmpStr.startsWith("delivered-to:")
                      || tmpStr.startsWith("message-Id") // $NON-NLS-1$ //$NON-NLS-2$
                      || tmpStr.startsWith("subject:")
                      || tmpStr.startsWith("from:") // $NON-NLS-1$ //$NON-NLS-2$
            ...
            log.warn(
                "Fail to extract email {} from {}\t{}", count, name, t.toString()); // $NON-NLS-1$
          }
        }
      }
    } finally {
