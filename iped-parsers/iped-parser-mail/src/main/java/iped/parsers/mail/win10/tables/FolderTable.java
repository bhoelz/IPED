...
public class FolderTable extends AbstractTable {
  ...
  public FolderTable(
    ...
    createTimePos =
        EsedbManager.getColumnPosition(
            esedbLibrary, ColumnCodes.CREATE_TIME, errorPointer, tablePointer, filePath);
  }
  ...
  private void addToParentFolder(FolderEntry subfolder) {
    ArrayList<FolderEntry> parentFolderSubfolders =
        parentFolderToSubfoldersMap.computeIfAbsent(
            subfolder.getParentFolderId(), k -> new ArrayList<FolderEntry>());

    // add if not already present
    if (!parentFolderSubfolders.stream()
        .map(f -> f.getRowId())
        .anyMatch(id -> id == subfolder.getRowId())) {
      parentFolderSubfolders.add(subfolder);
    }
  }

  private FolderEntry extractFolder(
    ...
    Date createTime =
        EsedbManager.getFileTime(
            esedbLibrary, createTimePos, recordPointerRef, filePath, errorPointer);
