...
public class StoreTable extends AbstractTable {
  ...
  private int rowIdPos,
      ...
      displayNamePos2,
      protocolPos,
      downloadNewEmailMinsPos,
      downloadEmailFromDaysPos,
      incomingEmailServerPos,
      outgoingEmailServerPos,
      outgoingEmailServerUsernamePos,
      contactsServerPos,
      calendarServerPos;
  ...
  public StoreTable(
    ...
    rowIdPos =
        EsedbManager.getColumnPosition(
            esedbLibrary, ColumnCodes.ROW_ID, errorPtr, tablePtr, filePath);
    addressPos =
        EsedbManager.getColumnPosition(
            esedbLibrary, ColumnCodes.EMAIL_ADDRESS, errorPtr, tablePtr, filePath);
    displayNamePos =
        EsedbManager.getColumnPosition(
            esedbLibrary, ColumnCodes.DISPLAY_NAME_1, errorPtr, tablePtr, filePath);
    displayNamePos2 =
        EsedbManager.getColumnPosition(
            esedbLibrary, ColumnCodes.DISPLAY_NAME_2, errorPtr, tablePtr, filePath);
    protocolPos =
        EsedbManager.getColumnPosition(
            esedbLibrary, ColumnCodes.PROTOCOL, errorPtr, tablePtr, filePath);
    downloadNewEmailMinsPos =
        EsedbManager.getColumnPosition(
            esedbLibrary, ColumnCodes.DOWNLOAD_EMAIL_FROM, errorPtr, tablePtr, filePath);
    downloadEmailFromDaysPos =
        EsedbManager.getColumnPosition(
            esedbLibrary, ColumnCodes.DOWNLOAD_NEW_EMAIL, errorPtr, tablePtr, filePath);
    incomingEmailServerPos =
        EsedbManager.getColumnPosition(
            esedbLibrary, ColumnCodes.INCOMING_EMAIL_SERVER, errorPtr, tablePtr, filePath);
    outgoingEmailServerPos =
        EsedbManager.getColumnPosition(
            esedbLibrary, ColumnCodes.OUTGOING_EMAIL_SERVER, errorPtr, tablePtr, filePath);
    outgoingEmailServerUsernamePos =
        EsedbManager.getColumnPosition(
            esedbLibrary, ColumnCodes.OUTGOING_EMAIL_SERVER_USERNAME, errorPtr, tablePtr, filePath);
    contactsServerPos =
        EsedbManager.getColumnPosition(
            esedbLibrary, ColumnCodes.CONTACTS_SERVER, errorPtr, tablePtr, filePath);
    calendarServerPos =
        EsedbManager.getColumnPosition(
            esedbLibrary, ColumnCodes.CALENDAR_SERVER, errorPtr, tablePtr, filePath);
  }
  ...
  private StoreEntry extractStoreEntry(
      int row, PointerByReference errorPtr, PointerByReference tablePointerRef) {
    int result = 0;
    ...
    result =
        esedbLibrary.libesedb_table_get_record(
            tablePointerRef.getValue(), row, recordPointerRef, errorPointer);
    ...
    int rowId =
        EsedbManager.getInt32Value(esedbLibrary, rowIdPos, recordPointerRef, filePath, errorPtr);
    String protocol =
        EsedbManager.getUnicodeValue(
            esedbLibrary, protocolPos, recordPointerRef, filePath, errorPtr);
    String address =
        EsedbManager.getUnicodeValue(
            esedbLibrary, addressPos, recordPointerRef, filePath, errorPtr);
    String displayName =
        EsedbManager.getUnicodeValue(
            esedbLibrary, displayNamePos, recordPointerRef, filePath, errorPtr);
    if (displayName.isEmpty())
      displayName =
          EsedbManager.getUnicodeValue(
              esedbLibrary, displayNamePos2, recordPointerRef, filePath, errorPtr);
    long downloadNewEmailMins =
        EsedbManager.getInt32Value(
            esedbLibrary, downloadNewEmailMinsPos, recordPointerRef, filePath, errorPtr);
    long downloadEmailFromDays =
        EsedbManager.getInt32Value(
            esedbLibrary, downloadEmailFromDaysPos, recordPointerRef, filePath, errorPtr);
    String incomingEmailServer =
        EsedbManager.getUnicodeValue(
            esedbLibrary, incomingEmailServerPos, recordPointerRef, filePath, errorPtr);
    String outgoingEmailServer =
        EsedbManager.getUnicodeValue(
            esedbLibrary, outgoingEmailServerPos, recordPointerRef, filePath, errorPtr);
    String outgoingEmailServerUsername =
        EsedbManager.getUnicodeValue(
            esedbLibrary, outgoingEmailServerUsernamePos, recordPointerRef, filePath, errorPtr);
    String contactsServer =
        EsedbManager.getUnicodeValue(
            esedbLibrary, contactsServerPos, recordPointerRef, filePath, errorPtr);
    String calendarServer =
        EsedbManager.getUnicodeValue(
            esedbLibrary, calendarServerPos, recordPointerRef, filePath, errorPtr);
  ...
  public static XHTMLContentHandler emitStoreHeader(ContentHandler handler, Metadata metadata)
      throws SAXException {
    String[] colNames = {
      "Id",
      "DisplayName",
      "Address",
      "Protocol",
      "DownloadNewEmail (Mins)",
      "DownloadEmailFrom (Days)",
      "IncomingEmailServer",
      "OutgoingEmailServer",
      "OutgoingEmailServerUsername",
      "ContactsServer",
      "CalendarServer"
    };
