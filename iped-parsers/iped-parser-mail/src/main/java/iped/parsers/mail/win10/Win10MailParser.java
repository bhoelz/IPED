...
/**
 * Parses Windows Mail App store.vol edb file. Extracts folders, emails, attachments, and
 * appointments from the database. Searches for additional information (e.g. attachment files and
 * body of the emails) in the case. Generates a subitem in the case for each table entry.
 *
 * <p>More info:
 * https://eprints.whiterose.ac.uk/133161/1/Navigating_the_Windows_Mail_database_accepted.pdf
 *
...
public class Win10MailParser extends AbstractParser {
    ...
    try {
      if (params.extractor.shouldParseEmbedded(metadata)) {
        ...
        for (FolderEntry folder : params.folderTable.getFolders()) {
          ...
          FolderEntry uniqueParentFolder =
              params.folderTable.getUniqueFoldersMap().get(folder.getParentFolderId());
          int parentFolderId =
              uniqueParentFolder != null
                  ? uniqueParentFolder.getRowId()
          ...
          ArrayList<ContactEntry> childContacts =
              params.contactTable.getFolderChildContacts(folder);
        ...
        try (FileOutputStream tmpStoreFile = new FileOutputStream(storeTableFile)) {
          ...
          XHTMLContentHandler xhtmlStore =
              StoreTable.emitStoreHeader(storeTableHandler, metadataStore);
        ...
        for (AbstractTable table : tables) closeTablePointer(table.getTablePointer(), params);
        closeFilePointer(filePointerReference, params);
      }
    } catch (Exception e) {
      log.warn(
          "Exception parsing Win10 Mail DB, using generic parser for " + params.itemInfo.getPath(),
          e);
   ...
   */
  protected List<AbstractTable> setMailTables(
      String filePath, PointerByReference filePointerReference, Parameters params)
      throws Win10MailException {
    ...
    try {
      ...
      synchronized (lock) {
        ...
        if (result < 0) EsedbManager.printError("File Initialize", result, dbPath, errorPointer);
        ...
        if (result == 0) throw new Win10MailException("File does not contain an ESEDB");

        result =
            esedbLibrary.libesedb_file_open(
                filePointerReference.getValue(), filePath, accessFlags, errorPointer);
        if (result < 0) EsedbManager.printError("File Open", result, dbPath, errorPointer);

        result =
            esedbLibrary.libesedb_file_get_number_of_tables(
                filePointerReference.getValue(), numTablesRef, errorPointer);
        if (result < 0)
      ...
      for (int tableIdx = 0; tableIdx < numTables; tableIdx++) {
        ...
        result =
            esedbLibrary.libesedb_file_get_table(
                filePointerReference.getValue(), tableIdx, tablePointer, errorPointer);
        if (result < 0) EsedbManager.printError("File Get Table", result, dbPath, errorPointer);

        result =
            esedbLibrary.libesedb_table_get_utf8_name_size(
                tablePointer.getValue(), tableNameSize, errorPointer);
        if (result < 0)
          EsedbManager.printError("Table Get UTF8 Name Size", result, dbPath, errorPointer);

        result =
            esedbLibrary.libesedb_table_get_utf8_name(
                tablePointer.getValue(), tableNameRef, tableNameSize.getValue(), errorPointer);
        if (result < 0)
        ...
        result =
            esedbLibrary.libesedb_table_get_number_of_records(
                tablePointer.getValue(), numberOfRecords, errorPointer);
        if (result < 0)
        ...
        if (tableName.equals("Message")) {
          params.messageTable =
              new MessageTable(esedbLibrary, dbPath, tablePointer, errorPointer, numRecords);
          table = params.messageTable;
        } else if (tableName.equals("Recipient")) {
          params.recipientTable =
              new RecipientTable(esedbLibrary, dbPath, tablePointer, errorPointer, numRecords);
          table = params.recipientTable;
        } else if (tableName.equals("Attachment")) {
          params.attachTable =
              new AttachmentTable(esedbLibrary, dbPath, tablePointer, errorPointer, numRecords);
          table = params.attachTable;
        } else if (tableName.equals("Folders")) {
          params.folderTable =
              new FolderTable(esedbLibrary, dbPath, tablePointer, errorPointer, numRecords);
          table = params.folderTable;
        } else if (tableName.equals("Appointment")) {
          params.apptTable =
              new AppointmentTable(esedbLibrary, dbPath, tablePointer, errorPointer, numRecords);
          table = params.apptTable;
        } else if (tableName.equals("Contact")) {
          params.contactTable =
              new ContactTable(esedbLibrary, dbPath, tablePointer, errorPointer, numRecords);
          table = params.contactTable;
        } else if (tableName.equals("Store")) {
          params.storeTable =
              new StoreTable(esedbLibrary, dbPath, tablePointer, errorPointer, numRecords);
   ...
   * @throws IOException
   */
  private void processFolder(FolderEntry folder, long parentId, Parameters params)
    ...
    folderMetadata.set(
        ExtraProperties.ITEM_VIRTUAL_ID, FOLDER_VIRTUAL_ID_PREFIX + folder.getRowId());
    ...
    params.extractor.parseEmbedded(
        InputStream.nullInputStream(), params.xhtml, folderMetadata, true);
  }
   ...
   * @throws IOException
   */
  private void processAppointment(AppointmentEntry appointment, long parentId, Parameters params)
      throws SAXException, IOException {
    ...
    appointMetadata.set(
        ExtraProperties.ITEM_VIRTUAL_ID, APPT_VIRTUAL_ID_PREFIX + appointment.getRowId());
    ...
    appointMetadata.set(
        ExtraProperties.MESSAGE_BODY, Util.getContentPreview(body, MediaType.TEXT_HTML.toString()));
    ...
    try (ByteArrayInputStream stream =
        new ByteArrayInputStream(preview.toString().getBytes(charset))) {
   ...
   * @throws IOException
   */
  private void processContact(ContactEntry contact, long parentId, Parameters params)
      throws SAXException, IOException {
    Metadata contactMetadata = new Metadata();

    contactMetadata.set(ExtraProperties.DECODED_DATA, Boolean.TRUE.toString());
    contactMetadata.set(
        ExtraProperties.ITEM_VIRTUAL_ID, APPT_VIRTUAL_ID_PREFIX + contact.getRowId());
    ...
    FileTag[] contactTags =
        new FileTag[] {
          FileTag.ASCII,
          FileTag.ASCII_PAIRS,
          FileTag.CONTACT_JPEG_1,
          FileTag.CONTACT_JPEG_2,
          FileTag.CONTACT_JPEG_3
        };
    for (FileTag contactTag : contactTags) {
      ...
      if (item != null) {
        if (contactTag == FileTag.CONTACT_JPEG_1
            || contactTag == FileTag.CONTACT_JPEG_2
            || contactTag == FileTag.CONTACT_JPEG_3) {
  ...
  private String htmlPairLine(String key, String value) {
    if (value != null && !value.isEmpty()) return key + ": " + value + "<br>";
    return "";
  }
   ...
   * @throws IOException
   */
  private String getAppointmentBody(AppointmentEntry appointment, Parameters params)
      throws IOException {
  ...
  private String getMessageBody(MessageEntry email, Parameters params) throws IOException {
    ...
    for (FileTag messageTag : messageTags) {
      ...
      Pair<IItemReader, String> itemQueryPair =
          searchItemInCase(contentPath, email.getMessageSize(), params);
      if (itemQueryPair.getLeft() != null) {
        item = itemQueryPair.getLeft();
    ...
    if (item != null) {
      ...
      try {
        ...
        utf16Reader =
            new InputStreamReader(new CloseShieldInputStream(is), StandardCharsets.UTF_16LE);
  ...
  private void processEmail(MessageEntry email, long parentId, Parameters params) {
    ...
    emailMetadata.add(
        Message.MESSAGE_PREFIX + "ConversationId", Long.toString(email.getConversationId()));
    ...
    try {
      ...
      preview.append(
          "<b>"
              + Messages.getString("OutlookPSTParser.Subject")
              + ": "
              + SimpleHTMLEncoder.htmlEncode(subject)
              + "</b><br>");

      // From:
      String from =
          OutlookPSTParser.formatNameAndAddress(
              email.getSenderName(), email.getSenderEmailAddress());
      if (!from.isEmpty()) {
        emailMetadata.set(Message.MESSAGE_FROM, from);
        preview.append(
            "<b>"
                + Messages.getString("OutlookPSTParser.From")
                + ":</b> "
                + SimpleHTMLEncoder.htmlEncode(from)
                + "<br>");
      }

      // To, BCC, CC:
      Object[][] recipTypes = {
        {RecipientEntry.RecipientType.TO, Messages.getString("OutlookPSTParser.To")},
        {RecipientEntry.RecipientType.CC, "CC:"},
        {RecipientEntry.RecipientType.BCC, "BCC:"}
      };
      ...
      for (int k = 0; k < recipTypes.length; k++) {
        ...
        List<RecipientEntry> recipients =
            params.recipientTable.getMessageRecipients(email.getRowId());
        if (recipients != null) {
          for (int i = 0; i < recipients.size(); i++) {
            ...
            if (recipTypes[k][0] == recipient.getType()) {
              String recipName =
                  OutlookPSTParser.formatNameAndAddress(
                      recipient.getDisplayName(), recipient.getEmailAddress());
        ...
        if (!recipientNames.isEmpty()) {
          ...
          preview.append(
              "<b>"
                  + recipTypes[k][1]
                  + "</b> "
                  + SimpleHTMLEncoder.htmlEncode(
                      recipientNames.stream().collect(Collectors.joining("; ")))
                  + "<br>");
        }
      }
      ...
      if (sent != null) {
        ...
        preview.append(
            "<b>"
                + Messages.getString("OutlookPSTParser.Sent")
                + ":</b> "
                + params.df.format(sent)
                + " (UTC) <br>");
      }

      // Attachments:
      ArrayList<AttachmentEntry> emailAttachments =
          params.attachTable.getMessageAttachments(email.getRowId());
      int noOfAttachments = emailAttachments.size();
      if (noOfAttachments > 0) {
        preview.append(
            "<b>"
                + Messages.getString("OutlookPSTParser.Attachments")
                + " ("
                + noOfAttachments
                + "):</b><br>");
        for (AttachmentEntry attach : emailAttachments) {
          String contentPath =
              Win10MailParser.getEntryLocation(attach, ATTACH_CATEGORY, FileTag.ANY);
          attach.setOriginalFileName(StringUtils.substringAfterLast(contentPath, "/"));
          Pair<IItemReader, String> itemQueryPair =
              searchItemInCase(contentPath, attach.getAttachSize(), params);
          ...
          if (item != null) {
            ...
            preview.append(
                "<a href=\""
                    ...
                    + queryHTML
                    + "');\">"
                    + SimpleHTMLEncoder.htmlEncode(attach.getFileName())
                    + "</a> <i>("
                    + desc
                    + ")</i><br>");
          } else {
            preview.append(
                SimpleHTMLEncoder.htmlEncode(attach.getFileName())
                    + " <i>("
                    + Messages.getString("Win10Mail.NotFound")
                    + ", "
                    + desc
                    + ")</i><br>");
          }
      ...
      try (ByteArrayInputStream stream =
          new ByteArrayInputStream(preview.toString().getBytes(charset))) {
        if (params.extractor.shouldParseEmbedded(emailMetadata)) params.xhtml.characters("\t");
      ...
      log.warn(
          "Exception extracting email: subject='"
              + email.getSubject()
              + "' rowid='"
              + email.getRowId()
              + "' DB='"
              + params.itemInfo.getPath()
              + "'",
          e);
    }
  }
  ...
  private void processAttachment(
      AttachmentEntry attachment, MessageEntry email, Parameters params) {
    ...
    try {
      ...
      attachMetadata.set(
          TikaCoreProperties.ORIGINAL_RESOURCE_NAME, attachment.getOriginalFileName());
      ...
      attachMetadata.set(
          Message.MESSAGE_PREFIX + "AttachmentSize", Long.toString(attachment.getAttachSize()));

      if (params.extractor.shouldParseEmbedded(attachMetadata)) {
        params.xhtml.characters("\t");
        params.extractor.parseEmbedded(
            InputStream.nullInputStream(), params.xhtml, attachMetadata, true);
      }

    } catch (Exception e) {
      log.warn(
          "Exception extracting attachment: name='"
              + filename
              + "' email='"
              + email.getSubject()
              + "' rowid='"
  ...
  /**
   * Encodes the file path in the same way microsoft does in order to generate the file location of
   * the .dat file in the case
   *
    ...
    entryPath.append(
        "/"
            + "abcdefghijklmnopqrstuvwxyz"
                .charAt(Integer.parseInt("" + hexRowId.charAt(hexRowId.length() - 1), 16))
            + "/");
   ...
   */
  public ImmutablePair<IItemReader, String> searchItemInCase(
      String path, long size, Parameters params) {
   ...
   *
   * @param body email body to replace inline images
   * @param attachments list of email attachments
   * @return new body that handles cid images with associated attachments
   */
  private String handleInlineImages(
      String body, ArrayList<AttachmentEntry> attachments, MessageEntry email, Parameters params) {
      ...
      for (AttachmentEntry attachment : attachments) {
        if (attachment.getAttachCID() != null && attachment.getCaseItem() != null) {
          ...
          try {
            ...
            log.warn(
                "Exception inlining attachment: name='"
