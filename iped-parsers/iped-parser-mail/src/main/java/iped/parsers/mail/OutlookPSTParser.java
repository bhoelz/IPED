...
/**
 * Parser para arquivos PST. Extrai emails, anexos, contatos, tarefas, etc. O Outlook fragmenta o
 * email em diversos objetos, então é gerado e extraído um preview HTML do email.
 *
 * @author Nassif
 */
@Slf4j
public class OutlookPSTParser extends AbstractParser {
  ...
  public static Set<MediaType> SUPPORTED_TYPES =
      Collections.singleton(MediaType.application("vnd.ms-outlook-pst")); // $NON-NLS-1$
  ...
  private SimpleDateFormat df =
      new SimpleDateFormat(Messages.getString("OutlookPSTParser.DateFormat")); // $NON-NLS-1$
  ...
  private void safeParse(
    ...
    if (itemInfo != null) fileName = itemInfo.getPath();
    ...
    try {
      ...
      if (useLibpffParser && pstFile.getPSTFileType() == PSTFile.PST_TYPE_2013_UNICODE) {
        throw new TikaException(
            "current java-libpst support for OST 2013 format is broken,"
      ...
      log.error(
          "Extraction of emails was interrupted on "
              + fileName
              + " "
      ...
      if (e instanceof IOException && tmpFile == null) {
        log.error(
            "Tempfile creation and processing failed on "
                + fileName
                + " "
                + e.toString()); // $NON-NLS-1$ //$NON-NLS-2$
        throw (IOException) e;
        ...
        if (useLibpffParser && !libpffCalled) {
          ...
          if (!recoverDeleted) libpffParser.setExtractOnlyActive(true);
          libpffParser.parse(tis, handler, metadata, context);
        } else {
        ...
        if (e.toString()
            .contains(
                "Only unencrypted and compressable PST files are supported at this time")) //$NON-NLS-1$
      ...
      if (pstFile != null && pstFile.getFileHandle() != null) pstFile.getFileHandle().close();
      tmp.close();
    }
      ...
      if (folder.getContentCount() > 0) {
        ...
        do {
          ...
          if (child != null)
            ...
            } else processPSTObject(child, path, parent);
  ...
  private void processPSTObject(PSTObject obj, String path, long parent) {
    ...
    try {
      ...
      if (obj.getClass().equals(PSTContact.class)) {
        ...
        if (suffix == null || suffix.isEmpty()) suffix = contact.getSMTPAddress();
        if (suffix != null && !suffix.isEmpty()) objName += "-" + suffix; // $NON-NLS-1$
        ...
        fillMetadata(
            ...
            contact.getEmail2EmailAddress(),
            contact.getEmail3EmailAddress());
        fillMetadata(
            metadata,
            ExtraProperties.USER_PHONE,
            contact.getPrimaryTelephoneNumber(),
            contact.getCompanyMainPhoneNumber(),
            contact.getRadioTelephoneNumber(),
            contact.getCarTelephoneNumber(),
            contact.getBusinessTelephoneNumber(),
            contact.getBusiness2TelephoneNumber(),
            contact.getMobileTelephoneNumber(),
            contact.getHomeTelephoneNumber(),
            contact.getOtherTelephoneNumber());
        fillMetadata(
            metadata,
            ExtraProperties.USER_ADDRESS,
        ...
        fillMetadata(
            metadata,
            ExtraProperties.USER_URLS,
            contact.getPersonalHomePage(),
            contact.getBusinessHomePage());
        fillMetadata(metadata, ExtraProperties.USER_NOTES, contact.getNote(), contact.getComment());
      } else metadata.set(StandardParser.INDEXER_CONTENT_TYPE, OUTLOOK_MSG_MIME);
      ...
      for (Method method : methodList) appendValue(obj, method, preview);
  ...
  private String processEmail(PSTMessage email, String path, String parent) {
    ...
    try {
      ...
      Object[][] recipTypes = {
        {PSTRecipient.MAPI_TO, Messages.getString("OutlookPSTParser.To")}, // $NON-NLS-1$
        {PSTRecipient.MAPI_CC, "CC:"},
        {PSTRecipient.MAPI_BCC, "BCC:"}
      }; //$NON-NLS-1$ //$NON-NLS-2$
      ...
      for (int k = 0; k < recipTypes.length; k++) {
        ...
        for (int i = 0; i < email.getNumberOfRecipients(); i++) {
          ...
          if (recipTypes[k][0].equals(recip.getRecipientType())) {
            String recipName =
                formatNameAndAddress(recip.getDisplayName(), recip.getEmailAddress());
          ...
          preview.append(
              "<b>"
                  ...
                  + SimpleHTMLEncoder.htmlEncode(
                      recipients.stream().collect(Collectors.joining("; "))) // $NON-NLS-1$
                  + "<br>"); //$NON-NLS-1$
        }
      }
      ...
      if (!attachNames.isEmpty()) {
        preview.append(
            "<b>"
                ...
                + " ("
                + attachNames.size() // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                + "):</b><br>"); //$NON-NLS-1$
  ...
  private List<String> getAttachNames(PSTMessage email) {
    ...
    for (int i = 0; i < email.getNumberOfAttachments(); i++) {
      try {
        ...
        if (filename.isEmpty()) filename = attach.getFilename();
        PSTMessage attachedEmail = attach.getEmbeddedPSTMessage();
        if (attachedEmail != null) filename = attachedEmail.getSubject();
        if (filename.isEmpty())
  ...
  private void processAttachs(PSTMessage email, String path, String parent) {
    ...
    for (int x = 0; x < numberOfAttachments; x++) {
      ...
      try {
        ...
        log.warn(
            "Exception extracting attachment {}:{}>>{}\t{}",
            x,
            path,
            filename,
            e.toString()); // $NON-NLS-1$
