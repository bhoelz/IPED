...
public class LibpffPSTParserTest extends AbstractPkgTest {
  ...
  public void testLibpffPSTParserOST() throws IOException, SAXException, TikaException {
    ...
    if (pffexportInstalled) {
      try (InputStream stream =
          this.getClass().getResourceAsStream("/test-files/test_sample.ost")) {
        ...
        assertTrue(
            psttracker
                .messagebody
                .toString()
                .contains("This is a test email with two attachment files"));
  ...
  public void testLibpffPSTParserUserInfo() throws IOException, SAXException, TikaException {
    ...
    if (pffexportInstalled) {
      try (InputStream stream =
          this.getClass().getResourceAsStream("/test-files/test_sample.pst")) {
        parser.parse(stream, handler, metadata, pstContext);
        ...
        assertTrue(
            psttracker
                .userphone
                .get(0)
                .matches("\\+55 \\(61\\) 986778855|\\+55 \\(61\\) 980099922"));
        assertEquals(
  ...
  public void testLibpffPSTParserInbox() throws IOException, SAXException, TikaException {
    ...
    if (pffexportInstalled) {
      try (InputStream stream =
        ...
        assertTrue(
            messageSubjects.contains(
                "Re: Solicita documentação para contrato de estágio na DITEC/PF."));
        assertTrue(
            messageBodies.contains(
                "Solved by using my abstrackpackage instead of hardcoding the parser. Sorry,"
        ...
        assertTrue(
            psttracker.attachmentname.contains(
                "lionel-animals-to-follow-on-instagram-1568319926.jpg"));
