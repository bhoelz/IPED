...
import iped.parsers.emule.data.*;
...
public class EmuleCollectionParser extends AbstractParser {
  public static final String EMULE_COLLECTION_MIME_TYPE =
      "application/x-emule-collection"; //$NON-NLS-1$
  private static final Set<MediaType> SUPPORTED_TYPES =
      Collections.singleton(MediaType.parse(EMULE_COLLECTION_MIME_TYPE));
  private static final long MAX_FILE_SIZE = 100 * 1024 * 1024;

  static String ED2K_URL_REGEX =
  ...
  public void parse(
    ...
    final DateFormat df =
        new SimpleDateFormat(Messages.getString("KnownMetParser.DataFormat")); // $NON-NLS-1$
    ...
    try {
      ...
      if (size > MAX_FILE_SIZE)
        throw new TikaException(
            "EmuleCollection is too big ("
                + size
                + " bytes): maximum supported size to parse is "
                + MAX_FILE_SIZE
                + " bytes");
      ...
      if (collection == null) { // tries to find ed2k urls with regex
        ...
        while (m.find()) {
          files.add(
              new ED2KURLCollectionFile(m.group("filename"), m.group("hash"), m.group("size")));
        }
  ...
  public void parse(
      ECollection collection, ContentHandler handler, Metadata metadata, ParseContext context)
      throws IOException, SAXException, TikaException {
    ...
    xhtml.characters(
        ".dt {border-collapse: collapse; border-style: solid; font-family: Arial, sans-serif; width: 1800px; margin-right: 32px; margin-bottom: 32px; width: 100%;  } " //$NON-NLS-1$
