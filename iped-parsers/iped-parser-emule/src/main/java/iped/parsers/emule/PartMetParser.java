...
/**
 * e-Mule "part.met" files parser. These files store information about files being downloaded, which
 * in some case are not present in the main e-Mule control file (known.met).
 *
 * @author Wladimir
 */
public class PartMetParser extends AbstractParser {
  ...
  private static final Set<MediaType> SUPPORTED_TYPES =
      Collections.singleton(MediaType.parse(EMULE_PART_MET_MIME_TYPE));
  ...
  public void parse(
    ...
    if (item != null) P2PUtil.printNameWithLink(xhtml, item, e.getName());
    else xhtml.characters(e.getName());
    xhtml.endElement("td");
    xhtml.endElement("tr");
    xhtml.newline();
    ...
    addLine(
        xhtml,
        Messages.getString("KnownMetParser.LastModDate"),
        toFormatStr(df, e.getLastModified()));
    addLine(
        xhtml,
        Messages.getString("KnownMetParser.LastPubDate"),
        toFormatStr(df, e.getLastPublishedKad()));
    addLine(
        xhtml,
        Messages.getString("KnownMetParser.LastShareDate").replaceAll("-", ""),
        toFormatStr(df, e.getLastShared()));
    addLine(xhtml, Messages.getString("KnownMetParser.Size"), toFormatStr(nf, e.getFileSize()));
    addLine(
        xhtml,
        Messages.getString("KnownMetParser.Requests"),
        toFormatStr(nf, e.getTotalRequests()));
    addLine(
        xhtml,
        Messages.getString("KnownMetParser.AcceptedRequests"),
        toFormatStr(nf, e.getAcceptedRequests()));
    addLine(
        xhtml,
        Messages.getString("KnownMetParser.BytesSent"),
        toFormatStr(nf, e.getBytesTransfered()));
    addLine(xhtml, Messages.getString("KnownMetParser.TempFile"), nvlStr(e.getPartName()));
    addLine(
        xhtml,
        Messages.getString("KnownMetParser.FoundInPedoHashDB"),
        hashSets.isEmpty() ? "" : hashSets.toString());
    addLine(
        xhtml,
        Messages.getString("KnownMetParser.FoundInCase"),
  ...
  private void addLine(XHTMLContentHandler xhtml, String key, String value, String valueClass)
      throws SAXException {
