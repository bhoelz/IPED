...
/**
 * Parser para arquivos known.met do e-Mule, que armazena arquivos conhecidos, com dados de
...
public class KnownMetParser extends AbstractParser {
  ...
  public static final String KNOWN_MET_ENTRY_MIME_TYPE =
      "application/x-emule-known-met-entry"; //$NON-NLS-1$
  private static final Set<MediaType> SUPPORTED_TYPES =
      Collections.singleton(MediaType.parse(EMULE_MIME_TYPE));
  ...
  public void parse(
    ...
    for (int i = -1; i <= l.size(); i++) {
      ...
      if (i == -1) {
        ...
        cells.add(
            e.getLastModified() == null ? " " : df.format(e.getLastModified())); // $NON-NLS-1$
        cells.add(
            e.getLastPublishedKad() == null
                ? " "
                : df.format(e.getLastPublishedKad())); // $NON-NLS-1$
      ...
      for (int j = 0; j < cells.size(); j++) {
        ...
        if (j != 1 || e == null) xhtml.characters(cells.get(j));
        else {
          if (item != null) {
