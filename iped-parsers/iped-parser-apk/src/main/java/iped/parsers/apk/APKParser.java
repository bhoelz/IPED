...
public class APKParser extends AbstractParser {
  ...
  private void parseEmbeddedCertificate(
      CertificateMeta m, EmbeddedDocumentExtractor extractor, ContentHandler xhtml)
      throws SAXException, IOException {
    CertificateFactory cf;
    try {
      cf = CertificateFactory.getInstance("X.509");
      X509Certificate cert =
          (X509Certificate) cf.generateCertificate(new ByteArrayInputStream(m.getData()));
      Metadata certMetadata = new Metadata();
      certMetadata.add(
          TikaCoreProperties.RESOURCE_NAME_KEY, cert.getSubjectX500Principal().getName());
  ...
  private void add(XHTMLContentHandler xhtml, String prop, String val, boolean isPre)
      throws SAXException {
    xhtml.startElement("tr");
