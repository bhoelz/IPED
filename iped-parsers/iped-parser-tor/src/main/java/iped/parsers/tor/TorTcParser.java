...
/**
 * Parses TC BUILT and EXTENDED Circuit Status Changes responses fragments on pagefile.sys that are
 * carved by TOR_CIRCUIT_STATUS carver. (Section 4.1.1 of
...
public class TorTcParser extends AbstractParser {
  ...
  static {
    for (String metadataName : dateMetadatas) {
      registerMetadataType(TORTC_PREFIX + metadataName, java.util.Date.class);
    }
  }

  private static void registerMetadataType(String name, Class<?> type) {
    try {
      Class<?> metadataUtilClass = Class.forName("iped.parsers.util.MetadataUtil");
      metadataUtilClass
          .getMethod("setMetadataType", String.class, Class.class)
          .invoke(null, name, type);
    } catch (ReflectiveOperationException e) {
      // Metadata type hints are optional outside the monolithic parsers module.
    }
  }
  ...
  public void parse(
    ...
    if (itemText != null) {
      ...
      if (!(itemText.contains("BUILD_FLAGS")
          || itemText.contains("SOCKS")
          || itemText.contains("HS_STATE"))) {
