...
public class SQLiteContainerDetector implements Detector {
  ...
  private static final MediaType GDRIVE_ACCOUNT_INFO =
      MediaType.application("x-gdrive-account-info");
  ...
  private static final MediaType THREEMA_CHAT_STORAGE =
      MediaType.application("x-threema-chatstorage");
  ...
  public MediaType detect(InputStream input, Metadata metadata) throws IOException {
    ...
    if (input == null) return MediaType.OCTET_STREAM;
    ...
    try {
      ...
      if (tis == null) {
        throw new RuntimeException(
            "Just a TikaInputStream can be given to " + this.getClass().getSimpleName());
      }

      byte[] prefix = new byte[32];
      int len = tis.peek(prefix);
      if (len < header.length) return MediaType.OCTET_STREAM;

      for (int i = 0; i < header.length; i++)
        if (prefix[i] != header[i]) return MediaType.OCTET_STREAM;
  ...
  private MediaType detectSQLiteFormat(File file) {
    try (Connection conn =
            DriverManager.getConnection(
                "jdbc:sqlite:" + file.getAbsolutePath(), // $NON-NLS-1$
  ...
  private MediaType detectTableNames(Connection conn, Set<String> tableNames) throws SQLException {
    ...
    if (tableNames.contains("messagesv12")
        && //$NON-NLS-1$
        tableNames.contains("profilecachev8")
        && //$NON-NLS-1$
        (tableNames.contains("conversationsv14") || tableNames.contains("conversationsv13"))
        && //$NON-NLS-1$
    ...
    if (tableNames.contains("Messages")
        ...
        && //$NON-NLS-1$
        tableNames.contains("Contacts")
        && //$NON-NLS-1$
        tableNames.contains("Transfers")
        && //$NON-NLS-1$
        tableNames.contains("Conversations")
        && //$NON-NLS-1$
        tableNames.contains("Chats")
        && //$NON-NLS-1$
    ...
    if ((tableNames.contains("chat_list") || tableNames.contains("chat"))
        ...
        && (tableNames.contains("group_participants")
            || tableNames.contains("group_participant_user"))
        && tableNames.contains("media_refs")
        && tableNames.contains("sqlite_sequence")) return WA_MSG_STORE;

    if (tableNames.contains("wa_contacts")
        && //$NON-NLS-1$
        tableNames.contains("wa_contact_capabilities")
        && //$NON-NLS-1$
        tableNames.contains("sqlite_sequence")) // $NON-NLS-1$
    return WA_DB;

    if (tableNames.contains("ZWACHATSESSION")
        && //$NON-NLS-1$
        tableNames.contains("ZWAMESSAGE")
        && //$NON-NLS-1$
        tableNames.contains("ZWAMEDIAITEM")
        && //$NON-NLS-1$
        tableNames.contains("ZWAGROUPMEMBER")) // $NON-NLS-1$
    return WA_CHAT_STORAGE;

    if (tableNames.contains("ZWAADDRESSBOOKCONTACT")
        || //$NON-NLS-1$
        (tableNames.contains("ZWACONTACT")
            && tableNames.contains("ZWAPHONE"))) // $NON-NLS-1$ //$NON-NLS-2$
    return WA_CONTACTS_V2;

    if (tableNames.contains("moz_places")
        && //$NON-NLS-1$
        tableNames.contains("moz_bookmarks")) // $NON-NLS-1$
    return MOZ_PLACES;

    if (tableNames.contains("history_items")
        && //$NON-NLS-1$
        tableNames.contains("history_visits")) // $NON-NLS-1$
    return SAFARI_SQLITE;

    if (tableNames.contains("downloads")
        && //$NON-NLS-1$
        tableNames.contains("urls")
        && //$NON-NLS-1$
        tableNames.contains("visits")
        && //$NON-NLS-1$
    ...
    if (tableNames.contains("Activity")
        ...
        && tableNames.contains("ActivityOperation")) return WIN10_TIMELINE;

    if (tableNames.contains("events_persisted")
        && tableNames.contains("tag_descriptions")
        && tableNames.contains("provider_groups")) return EVENT_TRANSCRIPT;

    if (tableNames.contains("cloud_graph_entry") && tableNames.contains("cloud_relations"))
    ...
    if (tableNames.contains("cloud_entry")
        ...
        && tableNames.contains("volume_info")) return GDRIVE_SNAPSHOT;

    if (tableNames.contains("global_preferences") || tableNames.contains("data"))
      return GDRIVE_ACCOUNT_INFO;

    if (tableNames.contains("dialogs")
        && tableNames.contains("chats")
        && tableNames.contains("users")
        && (tableNames.contains("messages") || tableNames.contains("messages_v2"))
        && (tableNames.contains("media")
            || tableNames.contains("media_v2")
            || tableNames.contains("media_v3")
            || tableNames.contains("media_v4"))) return TELEGRAM_DB;

    // detection for Telegram iOS DB
    if (tableNames.contains("t0")
        && tableNames.contains("t2")
        && tableNames.contains("t6")
        && tableNames.contains("t7")
        && tableNames.contains("t9")) {
    ...
    if (tableNames.contains("message")
        && tableNames.contains("chat")
        && tableNames.contains("chat_message_join")
        && tableNames.contains("chat_handle_join")) {
      return MediaType.application("x-ios-sms-db");
    }

    if (tableNames.contains("ABPerson")
        && tableNames.contains("ABMultiValue")
    ...
    if (tableNames.contains("Calendar")
        && tableNames.contains("CalendarChanges")
    ...
    if (tableNames.contains("ZCONVERSATION")
        && tableNames.contains("ZMESSAGE")
        && tableNames.contains("ZCONTACT")
        && tableNames.contains("ZFILEDATA")
        && tableNames.contains("ZIMAGEDATA")) {
