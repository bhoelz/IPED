...
public class SkypeParser extends AbstractParser {
  ...
  public static final MediaType SKYPE_MIME_V12 =
      MediaType.application("sqlite-skype-v12"); // $NON-NLS-1$
  ...
  public static final String CONVERSATION_MIME_TYPE =
      "application/x-skype-conversation"; //$NON-NLS-1$
  ...
  public void parse(
    ...
    try {
      ...
      if (extractor.shouldParseEmbedded(metadata)) {
        ...
        if (searcher != null) sqlite.searchMediaCache(searcher);
        ...
        for (SkypeContact c : contatos) {
          ...
          if (extractor.shouldParseEmbedded(cMetadata)) {
            ByteArrayInputStream chatStream =
                new ByteArrayInputStream(r.generateSkypeContactHtml(c));
        ...
        for (SkypeConversation conv : convs) {
          ...
          for (SkypeMessage sm : conv.getMessages()) {
            if (sm.getAutor() != null) {
              ...
              if (user != null
                  && user.getSkypeName() != null
                  && !sm.getAutor().equals(user.getSkypeName())) sm.setAutor(user.getSkypeName());
            }
          ...
          if (extractor.shouldParseEmbedded(chatMetadata)) {
            ByteArrayInputStream chatStream =
                new ByteArrayInputStream(r.generateSkypeConversationHtml(conv));
          ...
          if (extractMessages)
            for (SkypeMessage sm : conv.getMessages()) {
              ...
              meta.set(
                  ConversationConstants.MESSAGE_STATUS, String.valueOf(sm.getChatMessageStatus()));
        ...
        for (SkypeFileTransfer t : transfers) {
          ...
          if (t.getType() == 3
              && t.getConversation().getParticipantes() != null
              && t.getConversation().getParticipantes().size() > 1) {
          ...
          if (t.getItem() != null) {
            ...
            if (t.getItem().getLength() >= 512) {
              tMetadata.add(
                  ExtraProperties.LINKED_ITEMS, BasicProps.HASH + ":" + t.getItem().getHash());
          ...
          if (extractor.shouldParseEmbedded(tMetadata)) {
            ByteArrayInputStream chatStream =
                new ByteArrayInputStream(r.generateSkypeTransferenciaHtml(t, searcher));
  ...
  private String getItemQuery(SkypeFileTransfer c, IItemSearcher searcher) {
    ...
    if (c.getFilePath() != null && !c.getFilePath().trim().isEmpty()) {
      ...
      if (path.indexOf(':') == 1) path = path.substring(3);
      ...
      if (!searcher.search(query).isEmpty()) return query;
    }
