...
public class VCardParser extends AbstractParser {
  ...
  private void extractMetadata(VCard vcard, Metadata metadata) {
    ...
    if (vcard.getNotes() != null) {
      vcard.getNotes().stream()
          .forEach(n -> metadata.add(ExtraProperties.USER_NOTES, n.getValue()));
    }
    ...
    if (vcard.getPhotos() != null) {
      for (Photo p : vcard.getPhotos()) {
        if (p.getData() != null) {
          metadata.set(
              ExtraProperties.THUMBNAIL_BASE64, Base64.getEncoder().encodeToString(p.getData()));
          break;
        }
      }
    }
  ...
  private static class HtmlGenerator {
    ...
    public void generateVcardHtml(VCard vcard) {
      ...
      generatePropertyText(
          Messages.getString("VCardParser.FormattedName"),
          vcard.getFormattedNames()); // $NON-NLS-1$
      generatePropertyName(
          Messages.getString("VCardParser.Name"), vcard.getStructuredNames()); // $NON-NLS-1$
      generatePropertyTextList(
          Messages.getString("VCardParser.Nickname"), vcard.getNicknames()); // $NON-NLS-1$
      generatePropertyText(
          Messages.getString("VCardParser.Email"), vcard.getEmails()); // $NON-NLS-1$
      generatePropertyTelephone(
          Messages.getString("VCardParser.Telephone"), vcard.getTelephoneNumbers()); // $NON-NLS-1$
      generatePropertyTextList(
          Messages.getString("VCardParser.Organization"), vcard.getOrganizations()); // $NON-NLS-1$
      generatePropertyText(
          Messages.getString("VCardParser.Notes"), vcard.getNotes()); // $NON-NLS-1$
    ...
    private void generatePropertyText(String propName, List<? extends TextProperty> props) {
      ...
      for (TextProperty prop : props) {
        ...
        if (prop.getValue() != null) print(prop.getValue(), true);
      }
    }
    ...
    private void generatePropertyTelephone(String propName, List<? extends Telephone> props) {
      ...
      for (Telephone prop : props) {
        ...
        if (prop.getText() != null) print(prop.getText(), true);
      }
    }
    ...
    private void generatePropertyText(String propName, TextProperty prop) {
      if (prop == null || prop.getValue() == null) return;
    ...
    private void generatePropertyTelephone(String propName, Telephone prop) {
      if (prop == null || prop.getText() == null) return;
      ...
      if (data != null) {
        println(
            "<img src=\"data:image/jpg;base64,"
                + Base64.getEncoder().encodeToString(data) // $NON-NLS-1$
                + "\" width=\"112\"/>",
            false); //$NON-NLS-1$
      } else {
        ...
        if (url != null) {
          println(
              "<img src=\"" + SimpleHTMLEncoder.htmlEncode(url) + "\" />",
    ...
    private void generatePhotos(List<Photo> photos) {
      ...
      for (Photo photo : photos) generatePhoto(photo);
    }
