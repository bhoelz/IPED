 ...
 * JDBCRowHandler.
 *
 * <p>This class is not designed to be thread safe (because of DateFormat)! Need to call a new
 * instance for each parse, as AbstractDBParser does.
 *
 * <p>For now, this silently skips cells of type CLOB, because xerial's jdbc connector does not
...
class SQLite3TableReader extends JDBCTableReader {
  ...
  DateFormat df =
      new SimpleDateFormat(
          Messages.getString("SQLite3TableReader.DateFormat"), Locale.ROOT); // $NON-NLS-1$
   ...
   */
  @Override
  protected TikaInputStream getInputStreamFromBlob(
      ResultSet resultSet, int columnIndex, Blob blob, Metadata m) throws SQLException {
  ...
  private static Date minPlausibleDate =
      new Date(System.currentTimeMillis() - 10L * 365 * 24 * 3600 * 1000);
  private static Date maxPlausibleDate =
      new Date(System.currentTimeMillis() + 10L * 365 * 24 * 3600 * 1000);
  ...
  private void detectDateFormat() throws SQLException {
        ...
        if (meta.getColumnType(col) == Types.INTEGER) {
          ...
          do {
            ...
            if (date != null && (date.after(minPlausibleDate) && date.before(maxPlausibleDate))) {
              if (dateFormats[col] == 0) dateFormats[col] = dateFormat;
              if (dateFormats[col] == dateFormat) numPlausibleDates[col]++;
              break;
            }
    ...
    for (int col : intCols) {
      if (numPlausibleDates[col] < (row - zeroes[col]) * 0.8 && row >= 5) dateFormats[col] = 0;
    }
    results.close();
  }
  ...
  @Override
  protected String handleInteger(
      ResultSetMetaData rsmd, ResultSet rs, int col, ContentHandler handler)
      throws SQLException, SAXException {

    String text = null;

    if (dateFormats == null) detectDateFormat();
