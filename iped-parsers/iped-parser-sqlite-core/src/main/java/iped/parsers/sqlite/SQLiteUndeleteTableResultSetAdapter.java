...
public class SQLiteUndeleteTableResultSetAdapter implements ResultSet {
  ...
  public SQLiteUndeleteTableResultSetAdapter(
      SQLiteUndeleteTable table, Predicate<SqliteRow> filter) {
    this(table, Collections.emptyMap(), filter);
  }

  public SQLiteUndeleteTableResultSetAdapter(
      SQLiteUndeleteTable table, Map<String, String> columnNamesMap, Predicate<SqliteRow> filter) {
  ...
  public SQLiteUndeleteTableResultSetAdapter(
      List<SqliteRow> rows, List<String> columnNames, Map<String, String> columnNamesMap) {
    this(rows, columnNames, columnNamesMap, (x) -> true);
  }

  public SQLiteUndeleteTableResultSetAdapter(
      List<SqliteRow> rows, List<String> columnNames, Predicate<SqliteRow> filter) {
  ...
  public boolean absolute(int row) throws SQLException {
    if (row > table.getTableRows().size() || row < 0) return false;
    idx = row;
    return true;
  }
