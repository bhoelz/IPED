...
public class ContactHandler extends AccountableHandler<Contact> {
  ...
  private void loadReferencedContact(IItemSearcher searcher) {
    ...
    String query =
        searcher.escapeQuery(ExtraProperties.UFED_ID)
            + ":\""
            + model.getId()
            + "\" && "
            + BasicProps.SUBITEM
  ...
  public String getTitle() {
    ...
    List<String> contactEntries =
        model.getContactEntries().values().stream()
            .flatMap(List::stream)
            .map(ContactEntry::getValue)
            .collect(Collectors.toList());

    String data =
        Stream.of(Arrays.asList(name), contactEntries)
            .flatMap(List::stream)
