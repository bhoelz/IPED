...
public class UfedAccountableParser extends AbstractParser {
  ...
  private static Set<MediaType> SUPPORTED_TYPES =
      Set.of(MediaTypes.UFED_CONTACT_MIME, MediaTypes.UFED_USER_ACCOUNT_MIME);
  ...
  public void parse(
    ...
    try {
      ...
      if (accountable instanceof Contact
          && "Shared".equalsIgnoreCase(((Contact) accountable).getType())
  ...
  private void parserAccountable(Accountable contact, XHTMLContentHandler xhtml)
      throws SAXException, IOException {

    xhtml.startElement("table");

    // Contact Photo
    List<ContactPhoto> validPhotos =
        contact.getPhotos().stream()
    ...
    if (!validPhotos.isEmpty()) {
      ...
      for (ContactPhoto photo : contact.getPhotos()) {
        ...
        if (photoData == null && StringUtils.isNotEmpty(photo.getPhotoNodeId())) {
          photoData =
              IOUtils.resourceToByteArray("/iped/parsers/common/img/avatar-unavailable.png");
        }
        if (photoData != null) {
          String hash = DigestUtils.md5Hex(photoData);
          if (seenPhoto.add(hash)) {
            xhtml.startElement(
                "img", "src", "data:image/jpg;base64," + Base64.encodeBase64String(photoData));
