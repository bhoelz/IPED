...
public class ChatActivityHandler extends BaseModelHandler<ChatActivity> {
  ...
  protected void fillMetadata(String prefix, Metadata metadata) {
    ...
    if (model.getParticipant() != null) {
      new PartyHandler(model.getParticipant(), source)
          .fillMetadata(UFED_META_PREFIX + "Party", metadata);
      metadata.add(
          UFED_META_PREFIX + "Party" + ":isPhoneOwner",
          Boolean.toString(model.getParticipant().isPhoneOwner()));
    }
  }
