...
public class AttachmentHandler extends BaseModelHandler<Attachment> {
  ...
  public void doLoadReferences(IItemSearcher searcher) {
    ...
    if (model.getFileId() != null) {
      String query =
          searcher.escapeQuery(ExtraProperties.UFED_ID) + ":\"" + model.getFileId() + "\"";
    ...
    if (model.getAttachmentExtractedPath() != null) {
      {
        String query =
            BasicProps.EVIDENCE_UUID
                ...
                + "\"" //
                + " && "
                + searcher.escapeQuery(ExtraProperties.UFED_META_PREFIX + "local_path")
                + ":\""
                + model.getAttachmentExtractedPath()
                + "\"";
        List<IItemReader> fileItems = searcher.search(query);
        if (!fileItems.isEmpty()) {
          if (fileItems.size() > 1) {
      ...
      // if referenced file was not found, read the "ufed:attachment_extracted_path" content to
      // handle it
      // PS: this is a rare state, that was seen in UFDR files generated in PA 8.
      // In such situations, the file with file_id was not present and attachment_extracted_path
      // pointed to unreferenced files
    ...
    if (!StringUtils.endsWith(
            model.getFilename(), ".enc") // don´t log ".enc" files not found - e.g. WhatsApp
