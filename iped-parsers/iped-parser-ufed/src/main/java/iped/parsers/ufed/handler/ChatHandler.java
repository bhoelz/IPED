...
public class ChatHandler extends BaseModelHandler<Chat> {
  ...
  public void fillMetadata(String prefix, Metadata metadata) {
    ...
    metadata.set(
        ...
        (int) model.getMessages().stream().filter(m -> !m.isSystemMessage()).count());

    // Chat Account Info
    metadata.set(CONVERSATION_ACCOUNT, model.getAccount());
    model
        .getReferencedAccount()
        .ifPresent(
            a -> {
    ...
    model
        ...
        .forEach(
            p -> {
              new PartyHandler(p, model.getSource())
                  .fillMetadata(CONVERSATION_PARTICIPANTS, metadata);
            });
    if (!model.getParticipants().isEmpty()) {
      metadata.add(
          CONVERSATION_PARTICIPANTS + ":count", Integer.toString(model.getParticipants().size()));
    }

    // Chat Owner Participant
    model
        .getPhoneOwnerParticipant()
        .ifPresent(
            p -> {
              new PartyHandler(p, model.getSource())
                  .fillMetadata(UFED_META_PREFIX + "PhoneOwner", metadata);
            });

    // Chat Group Admin Participants
    model.getParticipants().stream()
        .filter(Party::isGroupAdmin)
        .forEach(
            p -> {
    ...
    if (model.getParticipants().stream()
        .filter(Party::isGroupAdmin)
        .filter(Party::isPhoneOwner)
        .findAny()
        .isPresent()) {
  ...
  public void doLoadReferences(IItemSearcher searcher) {
    ...
    model
        ...
        .forEach(
            participant -> {
              new PartyHandler(participant, model.getSource(), item, participantsCache)
                  .loadReferences(searcher);
            });
  ...
  protected void doAddLinkedItemsAndSharedHashes(
    ...
    model.getPhotos().stream()
        .map(ContactPhoto::getReferencedFile)
    ...
    // references to messages referenced items are done in UfedChatParser.storeLinkedHashes() per
    // chat-preview
  }
  ...
  public String getTitle(boolean outputPrefix, boolean outputOwnerParticipant) {
    ...
    if (outputPrefix) {
      if (StringUtils.isNotBlank(source)) sb.append(source).append(' ');
      else if (StringUtils.isNotBlank(model.getServiceIdentifier()))
        sb.append(model.getServiceIdentifier()).append(' ');
      else sb.append("App:" + Messages.getString("UfedChatParser.Unknown")).append(' ');
          ...
          case Chat.TYPE_BROADCAST:
            if (model.getParticipants().size() == 1
                && StringUtils.containsAnyIgnoreCase(
                    source, Chat.SOURCE_TELEGRAM, Chat.SOURCE_WHATSAPP)) {
    ...
    if (name != null) {
      ...
      String otherParticipantStr =
          new PartyHandler(otherParticipants.get(0), model.getSource()).getTitle();
      boolean otherSurroundedByBrackets =
          otherParticipantStr.startsWith("[") && otherParticipantStr.endsWith("]");
      ...
      if (outputOwnerParticipant) {
        boolean ownerSurroundedByBrackets =
            ownerParticipantStr.startsWith("[") && ownerParticipantStr.endsWith("]");
  ...
  private void loadChatAccountReference(IItemSearcher searcher) {
    ...
    String query =
        AccountableHandler.createAccountableQuery(
            account, model.getSource(), MediaTypes.UFED_USER_ACCOUNT_MIME, model, item, searcher);
