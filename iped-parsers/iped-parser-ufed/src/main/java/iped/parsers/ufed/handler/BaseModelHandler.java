...
public class BaseModelHandler<T extends BaseModel> {
  ...
  // if set to true, the item will be linked on addLinkedItems even if the item id is already set in
  // jumpTargets
  private boolean skipJumpTargetsCheckOnAddLinkedItems = false;

  private static final HashSet<String> ignoreAttrs =
      new HashSet<>(
          Arrays.asList( //
              "type", //
              "path", //
  ...
  public static final HashSet<String> ignoreFields =
      new HashSet<>(
          Arrays.asList( //
              "Tags", //
              "CreationTime", //
  ...
  public void setSkipJumpTargetsCheckOnAddLinkedItems(
      boolean skipJumpTargetsCheckOnAddLinkedItems) {
  ...
  /**
   * Fills all relevant metadata for the model into the Tika Metadata object. This is the main entry
   * point for a handler.
   */
  ...
  public final void addLinkedItemsAndSharedHashes(
  ...
  public static void updateLinkedItemsAndSharedHashes(
      Metadata metadata, Set<String> newLinkedItems, Set<String> newSharedHashes) {

    if (!newLinkedItems.isEmpty()) {
      Property linkedItemsProperty = Property.internalTextBag(ExtraProperties.LINKED_ITEMS);
      HashSet<String> linkedItems =
          new HashSet<>(Arrays.asList(metadata.getValues(linkedItemsProperty)));
    ...
    if (!newSharedHashes.isEmpty()) {
      ...
      HashSet<String> sharedHashes =
          new HashSet<>(Arrays.asList(metadata.getValues(sharedHashesProperty)));
  ...
  protected final void addLinkedItem(
      Set<String> linkedItems, IItemReader referencedItem, IItemSearcher searcher) {
    ...
    if (StringUtils.isNotBlank(referencedUfedId)
        && (skipJumpTargetsCheckOnAddLinkedItems
            || !model.getJumpTargets().stream()
                .anyMatch(j -> referencedUfedId.equals(j.getId())))) {
      String linkedItemQuery =
          searcher.escapeQuery(ExtraProperties.UFED_ID) + ":\"" + referencedUfedId + "\"";
                  ...
                  child -> {
                    child
                        ...
                        .forEach(
                            (key, value) -> {
                              fillFieldMetadata(
                                  fieldName + ":" + key, value, metadata, Collections.emptySet());
                            });
                  });
            });
  ...
  protected final void fillFieldMetadata(
      String key, Object value, Metadata metadata, Set<String> fieldsToIgnore) {
  ...
  private void postProcessMetadata(Metadata metadata) {
    if (StringUtils.equalsAny(
        model.getModelType(), "InstantMessage", "Email", "Call", "SMS", "MMS")) {
