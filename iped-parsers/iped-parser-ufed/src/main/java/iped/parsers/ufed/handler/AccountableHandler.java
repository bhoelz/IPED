...
public class AccountableHandler<T extends Accountable> extends BaseModelHandler<T> {
  ...
  protected static String createAccountableQuery(
      String identifier,
      String source,
      MediaType contentType,
      BaseModel relatedModel,
      IItemReader relatedItem,
    ...
    return BasicProps.CONTENTTYPE
        ...
        + ":\""
        + relatedItem.getDataSource().getUUID()
        + "\""
        + " && "
        + searcher.escapeQuery(UFED_META_PREFIX + "extractionId")
        + ":"
        + relatedModel.getExtractionId()
        + " && "
        + searcher.escapeQuery(UFED_META_PREFIX + "Source")
        + ":\""
        + source
        + "\""
        + " && ("
        + searcher.escapeQuery(UFED_META_PREFIX + "UserID")
        + ":\""
        + identifier
        + "\"" //
        + " || "
        + searcher.escapeQuery(UFED_META_PREFIX + "Username")
        + ":\""
        + identifier
        + "\"" //
        + " || "
        + searcher.escapeQuery(UFED_META_PREFIX + "PhoneNumber")
        + ":("
        + possiblePhoneNumbers.stream().collect(Collectors.joining(" ", "\"", "\""))
        + ")" //
        + ")";
  }
}
