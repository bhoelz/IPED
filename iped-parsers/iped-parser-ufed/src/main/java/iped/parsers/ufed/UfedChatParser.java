...
public class UfedChatParser extends AbstractParser {
  ...
  public static final MediaType UFED_CHAT_PREVIEW_MIME =
      MediaType.application("x-ufed-chat-preview");

  public static final Map<String, MediaType> appToMime =
      Map.ofEntries( //
  ...
  @Override
  public void parse(
      InputStream inputStream, ContentHandler handler, Metadata metadata, ParseContext context)
    ...
    try {
      ...
      int messagesCount =
          (int) chat.getMessages().stream().filter(m -> !m.isSystemMessage()).count();
      ...
      if (extractor.shouldParseEmbedded(metadata)) {
        ...
        while (bytes != null) {
          ...
          for (String name : metadata.names()) {
            if (name.startsWith(ExtraProperties.UFED_META_PREFIX)
                || name.startsWith(ExtraProperties.CONVERSATION_PREFIX)
          ...
          chatPreviewMeta.set(ParserConstants.INDEXER_CONTENT_TYPE, previewMime.toString());
  ...
  private void extractMessages(
      List<InstantMessage> subList,
      String chatVirtualId,
    ...
    for (InstantMessage message : subList) {
      ...
      messageMeta.set(ParserConstants.INDEXER_CONTENT_TYPE, message.getMediaType().toString());
  ...
  private void extractActivityLog(
      Chat chat, String chatVirtualId, ContentHandler handler, EmbeddedDocumentExtractor extractor)
      throws SAXException, IOException {
  ...
  private void storeLinkedHashes(
      List<InstantMessage> messages, Metadata chatMetadata, IItemSearcher searcher) {
    ...
    messages.parallelStream()
        .forEach(
            message -> {
              ...
              messageHandler.addLinkedItemsAndSharedHashes(
                  newLinkedItems, newSharedHashes, searcher);
            });

    BaseModelHandler.updateLinkedItemsAndSharedHashes(
        chatMetadata, newLinkedItems, newSharedHashes);
  }
}
