...
public class PartyHandler extends BaseModelHandler<Party> {
  ...
  protected PartyHandler(
      Party model, String source, IItemReader parentItem, Map<String, IItemReader> cache) {
  ...
  protected void fillMetadata(String prefix, Metadata metadata) {
    ...
    model
        ...
        .ifPresentOrElse(
            ref -> {
              metadata.add(
                  prefix + CONVERSATION_SUFFIX_ID,
                  StringUtils.firstNonBlank(model.getIdentifier(), ref.getUserID()));
              metadata.add(
                  prefix + CONVERSATION_SUFFIX_NAME,
                  StringUtils.firstNonBlank(model.getName(), ref.getName()));
    ...
    if (model.isPhoneOwner()) {
      query =
          AccountableHandler.createAccountableQuery(
              identifier, source, MediaTypes.UFED_USER_ACCOUNT_MIME, model, item, searcher);
    } else {
      query =
          AccountableHandler.createAccountableQuery(
              identifier, source, MediaTypes.UFED_CONTACT_MIME, model, item, searcher);
    }
    List<IItemReader> results =
        searcher.search(query).stream()
            .filter(
                item ->
                    source.equals(
                        item.getMetadataValue(
                            ExtraProperties.UFED_META_PREFIX
                                + "Source"))) // filter for exact source
        ...
        Collections.sort(
            ...
            (item1, item2) -> {
              return Integer.compare(
                  ((Metadata) item2.getMetadata()).names().length,
                  ((Metadata) item1.getMetadata()).names().length);
            });
        log.warn(
            "Found more than 1 party reference: size=[{}] \t query=[{}] \t items=[{}]",
            results.size(),
            query,
            StringUtils.truncate(results.toString(), 0, 1000));
      }
  ...
  private boolean isValidPartyIdentifier(String identifier) {
    ...
    if (StringUtils.isAlphanumeric(identifier)
        || EmailValidator.getInstance(true, true).isValid(identifier)) {
      return true;
    }
  ...
  public String getTitle() {
    return PartyStringBuilderFactory.getBuilder(source).withParty(model).build();
  }
}
