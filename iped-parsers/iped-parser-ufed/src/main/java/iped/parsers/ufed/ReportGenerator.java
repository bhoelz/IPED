...
public class ReportGenerator {
  ...
  private String formatLocation(InstantMessage message) {
    ...
    DivTag div =
        div(img(attrs(".location")), b(Messages.getString("UfedChatParser.Location.Title")), br());
    ...
    for (Contact msgContact : message.getSharedContacts()) {
      ...
      if (contact.isPresent()) {
        ...
        if (isNotBlank(username)) {
          table.with(
              tr(td(Messages.getString("UfedChatParser.SharedContact.Username")), td(username)));
        }

        if (isNotBlank(phone)) {
          table.with(
              tr(td(Messages.getString("UfedChatParser.SharedContact.PhoneNumber")), td(phone)));
        }
      }
  ...
  public byte[] generateNextChatHtml() throws UnsupportedEncodingException {
    ...
    if ((!firstHtml && currentMsg == 0)
        || (currentMsg > 0 && currentMsg == chat.getMessages().size())) {
      return null;
    }
    ...
    byte[] firstPhotoData =
        chat.getPhotos().stream()
            .map(ContactPhoto::getReferencedFile) // Optional<ReferencedFile>
            .flatMap(
                opt ->
                    opt.map(Stream::of).orElseGet(Stream::empty)) // unwrap Optional<ReferencedFile>
    ...
    if (currentMsg > 0) {
      out.println(
          "<div class=\"linha\"><div class=\"date\">"
    ...
    while (currentMsg < chat.getMessages().size()) {
      ...
      String thisDate =
          m.getTimeStamp() != null
              ? dateFormat.format(m.getTimeStamp())
  ...
  private void printMessage(
      PrintWriter out, InstantMessage message, boolean group, boolean chatDeleted) {
    ...
    if (message.isForwardedMessage()) {
      ...
      if (originalSender != null) {
        forwardedFrom =
            Messages.getString("UfedChatParser.Forwarded.From")
                + " "
                + new PartyHandler(originalSender, message.getSource()).getTitle();
      }
      out.println(
          "<img class=\"fwd\"><span class=\"fwd\">"
              + Messages.getString("UfedChatParser.Forwarded")
              + " "
              + forwardedFrom
              + "</span><br/>");
  ...
  private void printMessageContent(PrintWriter out, InstantMessage message) {
    ...
    for (Attachment attachment : message.getAttachments()) {
      ...
      if (attachment.getReferencedFile() != null) {
        ...
        exportPath = Util.getExportPath(attachment.getReferencedFile().getItem());
        source = Util.getSourceFileIfExists(attachment.getReferencedFile().getItem()).orElse("");
        ...
        if (fileHash != null) {
          ...
          String href = Util.getReportHref(attachment.getReferencedFile().getItem());
          out.println(
      ...
      if (contentType != null) {
        if (contentType.startsWith("audio")) {
          ...
          out.println(
              "<div class=\"audioImg iped-audio\" title=\"Audio\" "
                  + "data-src1=\""
                  + format(exportPath)
                  + "\" "
                  + "data-src2=\""
          ...
          out.println(
              Messages.getString("UfedChatParser.VideoMessageTitle") + "<br>"); // $NON-NLS-1$
          if (thumb != null) {
        ...
        if (!attachment.getReferencedFile().getChildPornSets().isEmpty()) {
          out.print(
              "<p><i>"
                  ...
                  + " "
                  + format(attachment.getReferencedFile().getChildPornSets().toString())
                  + "</i></p>");
        }
      }
    ...
    if (isNotBlank(body) || hasSubject) {
      if (hasSubject) out.print(format(message.getSubject()));
      if (isNotBlank(body)) {
        if (hasSubject) out.print("<br/>");

        String formattedBody = format(body);
        if (isWhatsApp) {
          formattedBody =
              iped.parsers.whatsapp.ReportGenerator.convertWhatsAppTagsToHTML(formattedBody);
        }
  ...
  private void printQuote(PrintWriter out, InstantMessage message) {
    ...
    for (Attachment attach : quotedMessage.getAttachments()) {
      ...
      if (attachContentType != null) {
        ...
        if (attachContentType.startsWith("audio")) {
          ...
          if (quoteThumb != null) {
            attachStr
                .append("<div><img class=\"quoteImg\" src=\"data:image/jpg;base64,")
    ...
    if (quotedMessage.isDeleted()) {
      quoteEnd =
          "<br/><span style=\"display:table-cell\" class=\"recovered\"><div class=\"deletedIcon\"></div>"
              + "<i>"
              + Messages.getString("UfedChatParser.MessageDeletedRecovered")
              + "</i></span>";
    }
  ...
  private String formatTitleAndUrl(Attachment attachment, String body) {
    ...
    if ("URL".equalsIgnoreCase(contentType)
        && isNotBlank(fileName)
        && !StringUtils.contains(body, fileName)
        && !StringUtils.contains(title, fileName)) {
      sb.append("<p>").append(format(fileName)).append("</p>");
    }

    String url = attachment.getURL();
    if (isNotBlank(url)
        && !StringUtils.contains(body, url)
        && !StringUtils.contains(title, url)
        && !StringUtils.contains(fileName, url)
  ...
  private static void printMessageFileHeader(
    ...
    out.println(
        "<!DOCTYPE html>\n"
            ...
            + "<meta charset=\"UTF-8\" />\n"
            + "<link rel=\"icon\" href=\""
            + Util.getImageResourceAsEmbedded("img/" + icon + ".png")
            + "\">\n"
            + "<style>\n"
