...
public class UfedMessageParser extends AbstractParser {
  ...
  public void parse(
    ...
    try {
      ...
      if (!item.isSubItem()
          ...
          && chatMessageUfedIds.contains(message.getId())) {
        // ignore orphan messages (not related to chats) whose IDs were from messages already
  ...
  private void parseMessage(IItemReader item, InstantMessage message, XHTMLContentHandler handler)
      throws SAXException {
  ...
  private void extractActivityLog(
      InstantMessage message, ContentHandler handler, EmbeddedDocumentExtractor extractor)
  ...
  private void extractSharedContacts(
      InstantMessage message, ContentHandler handler, EmbeddedDocumentExtractor extractor)
      throws SAXException, IOException {
    ...
    for (Contact sharedContact : message.getSharedContacts()) {
      ...
      shareContactMeta.set(
          ParserConstants.INDEXER_CONTENT_TYPE, sharedContact.getMediaType().toString());
