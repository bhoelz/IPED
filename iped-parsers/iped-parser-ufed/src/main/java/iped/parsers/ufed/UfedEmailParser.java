...
public class UfedEmailParser extends AbstractParser {
  ...
  private static final Set<MediaType> SUPPORTED_TYPES =
      Collections.singleton(MediaTypes.UFED_EMAIL_MIME);

  private static final Set<String> formattingTags =
      new HashSet<>(
          Arrays.asList(
              "b", "strong", "i", "em", "u", "mark", "s", "del", "ins", "code", "kbd", "samp",
              "var", "sub", "sup", "small"));
  ...
  public void parse(
    ...
    try {
      ...
      Metadata xhtmlMetadata = new Metadata();
      for (String key : metadata.names()) {
        for (String val : metadata.getValues(key)) {
          xhtmlMetadata.add(key, val);
        }
      }
  ...
  private void extractAttachments(
      Email email, ContentHandler handler, EmbeddedDocumentExtractor extractor)
  ...
  private void parseHeaders(Email email, XHTMLContentHandler xhtml)
      throws SAXException, IOException {

    xhtml.startElement("table");
    if (email.getFrom().isPresent()) {
      parseParties(
          Messages.getString("UfedEmailParser.From"),
          Collections.singletonList(email.getFrom().get()),
          xhtml);
    }
    ...
    if (timeStamp != null) {
      parseField(
          Messages.getString("UfedEmailParser.Date"),
    ...
    parseAttachments(
        Messages.getString("UfedEmailParser.Attachments"), email.getAttachments(), xhtml);
  ...
  private void parseField(String field, String value, XHTMLContentHandler xhtml)
      throws SAXException {
  ...
  private void parseParties(String field, List<Party> parties, XHTMLContentHandler xhtml)
      throws SAXException {
    if (parties.isEmpty()) {
      return;
    }
  ...
  private void parseAttachments(
      String field, List<Attachment> attachments, XHTMLContentHandler xhtml) throws SAXException {
  ...
  private void parseBody(Email email, XHTMLContentHandler xhtml)
      throws IOException, SAXException, TikaException {
    ...
    /**
     * An extended BodyContentHandler that processes HTML content to embed email attachments
     * directly into the output. It overrides the startElement method to find elements with a 'src'
     * attribute (like <img> tags) that reference an attachment. When a match is found, it replaces
     * the 'src' attribute's value with a Base64-encoded data URI of the attachment's content.
     */
    Matcher MATCHER =
        new XPathParser("xhtml", XHTMLContentHandler.XHTML)
            .parse("/xhtml:html/xhtml:body/xhtml:body/descendant::node()");
    MatchingContentHandler bodyHandler =
        new MatchingContentHandler(xhtml, MATCHER) {

          /**
           * Intercepts the start of an XML element to check for 'src' attributes that can be
           * replaced with inline attachment data.
           *
           * <p>The matching logic for attachments includes: - Exact URL match. - Content-ID (cid:)
           * match with the attachment filename. - A fallback for single-attachment emails with a
           * 'cid:' link. - A special-case comparison for Gmail URLs, ignoring certain parameters.
           */
          @Override
                      ...
                      .filter(
                          a ->
                              src.equals(a.getURL())
                                  ...
                                  || "Gmail".equals(email.getSource())
                                      && StringUtils.substringBefore(src, "&view=")
                                          .equals(
                                              StringUtils.substringBefore(a.getURL(), "&view=")))
                      .map(
                          a -> {
                            ...
                            if (a.getReferencedFile() != null) {
                              try (InputStream is =
                                  a.getReferencedFile().getItem().getSeekableInputStream()) {
                                attachData = IOUtils.toByteArray(is);
                                contentType =
                                    StringUtils.firstNonBlank(
                                        a.getContentType(),
                                        a.getReferencedFile().getItem().getMediaType().toString());
                              } catch (IOException e) {
                            ...
                            if (attachData != null) {
                              return String.format(
                                  ...
                                  Base64.encodeBase64String(attachData));
                            }
                            return null;
                          })
