...
public abstract class AbstractDBParser extends AbstractParser {
  ...
  private int parseTables(ContentHandler handler, ParseContext context, JDBCTableReader reader)
    ...
    do {
      ...
      try (InputStream is = trg.createHtmlReport(HTML_MAX_ROWS, handler, context, tmp)) {
        ...
        tableM.set(INDEXER_CONTENT_TYPE, TABLE_REPORT.toString());
  ...
  /**
   * Override this for special configuration of the connection, such as limiting the number of rows
   ...
   *
   * <p>Include any optimization settings, user name, password, etc.
   *
   * <p>
   *
  ...
  protected abstract String getConnectionString(
      InputStream stream, Metadata metadata, ParseContext parseContext) throws IOException;
   ...
   * @return
   */
  protected abstract JDBCTableReader getTableReader(
      Connection connection, String tableName, ParseContext parseContext);
}
