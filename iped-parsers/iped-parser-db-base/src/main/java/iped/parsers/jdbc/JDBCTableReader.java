...
public class JDBCTableReader {
  ...
  public boolean nextRow(ContentHandler handler, ParseContext context)
    ...
    try {
      ...
      handler.startElement(
          XHTMLContentHandler.XHTML, "tr", "tr", EMPTY_ATTRIBUTES); // $NON-NLS-1$ //$NON-NLS-2$
      for (int i = 1; i <= meta.getColumnCount(); i++) {
        handler.startElement(
            XHTMLContentHandler.XHTML, "td", "td", EMPTY_ATTRIBUTES); // $NON-NLS-1$ //$NON-NLS-2$
  ...
  private String handleCell(
      ResultSetMetaData rsmd, int i, ContentHandler handler, ParseContext context)
  ...
  protected String handleInteger(
      ResultSetMetaData rsmd, ResultSet rs, int columnIndex, ContentHandler handler)
  ...
  protected String handleClob(
    ...
    String name =
        FilenameUtils.normalize(
            FilenameUtils.getName(columnName + "_" + rowNum + ".txt")); // $NON-NLS-1$ //$NON-NLS-2$
    ...
    ex.parseEmbedded(
        new ByteArrayInputStream(s.getBytes("UTF-8")), handler, m, false); // $NON-NLS-1$

    return name;
  }

  protected String handleBlob(
      String tableName,
      String columnName,
      int rowNum,
      ResultSet resultSet,
      ...
      try {
        ...
        if (is.getLength() > MIN_SIZE) {
          if (context.get(Parser.class) != null
              || !(ex instanceof ParsingEmbeddedDocumentExtractor)) {
            String name =
                tableName
                    + "_"
                    + columnName
  ...
  protected TikaInputStream getInputStreamFromBlob(
      ResultSet resultSet, int columnIndex, Blob blob, Metadata metadata) throws SQLException {
