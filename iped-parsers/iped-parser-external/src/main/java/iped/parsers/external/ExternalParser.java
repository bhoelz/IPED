...
public class ExternalParser extends AbstractParser {
  ...
  /**
   * Sets the map of regular expression patterns and Metadata keys. Any matching patterns will have
   * the matching metadata entries set. Set this to null to disable Metadata extraction.
   */
   ...
   * document to the given SAX content handler. Metadata is only extracted if {@link
  ...
  public void parse(
    ...
    if (LOGGER == null) LOGGER = LoggerFactory.getLogger(ExternalParser.class);
  ...
  private void parse(
      TikaInputStream stream, XHTMLContentHandler xhtml, Metadata metadata, TemporaryResources tmp)
      throws IOException, SAXException, TikaException {
    ...
    } else workDir = new File(getRootFolder());
    // Execute
   ...
   * to the given XHTML content handler. The standard output stream is closed once fully processed.
   *
  ...
  private void extractOutput(
      InputStream stream, XHTMLContentHandler xhtml, String origFileName, File tmpFile)
  ...
  /**
   * Starts a thread that sends the contents of the given input stream to the standard input stream
   * of the given process. Potential exceptions are ignored, and the standard input stream is closed
   * once fully processed. Note that the given input stream is <em>not</em> closed by this method.
   *
   * @param process process
   ...
   * process. Potential exceptions are ignored, and the stream is closed once fully processed. Note:
   * calling this starts a new thread and blocks the current(caller) thread until the new thread
   * dies
   *
  ...
  private Thread extractMetadataInBackground(final InputStream stream, final Metadata metadata)
      throws TikaException {
    Thread t =
        new Thread() {
          public void run() {
