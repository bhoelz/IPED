...
public class BitTorrentResumeDatParser extends AbstractParser {
  ...
  private static final Set<MediaType> SUPPORTED_TYPES =
      Collections.singleton(MediaType.application("x-bittorrent-resume-dat")); // $NON-NLS-1$
  public static final String RESUME_DAT_MIME_TYPE =
      "application/x-bittorrent-resume-dat"; //$NON-NLS-1$
  ...
  private static final char[] colAlign =
      new char[] {'b', 'a', 'a', 'a', 'h', 'c', 'c', 'b', 'b', 'b', 'b', 'c', 'c', 'b', 'b'};
  ...
  public void parse(
    ...
    final DateFormat df =
        new SimpleDateFormat(
            Messages.getString("BitTorrentResumeDatParser.DateFormat")); // $NON-NLS-1$
    ...
    BencodedDict dict =
        new BencodedDict(stream, df, (context.get(IgnoreCorruptedCarved.class) == null));

    XHTMLContentHandler xhtml = new XHTMLContentHandler(handler, metadata);
    xhtml.startDocument();
    xhtml.startElement("style"); // $NON-NLS-1$
    xhtml.characters(
        ".dt {border-collapse: collapse; font-family: Arial, sans-serif; } " //$NON-NLS-1$
    ...
    try {
      ...
      for (String torrent : dict.keySet()) {
        ...
        IItemReader item =
            P2PUtil.searchItemInCase(searcher, TorrentFileParser.TORRENT_INFO_HASH, infoHash);
        if (item != null) {
        ...
        xhtml.startElement(
            "tr", "class", a ? "ra" : "rb"); // $NON-NLS-1$ $NON-NLS-2$ $NON-NLS-3$ $NON-NLS-4$
        String[] rowElements =
            new String[] {
