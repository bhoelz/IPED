...
public class TorrentFileParser extends AbstractParser {
  ...
  private static final String TORRENT_CREATION_DATE =
      ExtraProperties.P2P_META_PREFIX + "torrentCreationDate";
  public static final String TORRENT_INFO_HASH =
      ExtraProperties.P2P_META_PREFIX + "torrentInfoHash";
  public static final String TORRENT_FILES_FOUND_IN_CASE =
      ExtraProperties.P2P_META_PREFIX + "torrentFilesFoundInCase";

  private static final long serialVersionUID = 3238363426940179831L;
  private static final Set<MediaType> SUPPORTED_TYPES =
      Collections.singleton(MediaType.application("x-bittorrent")); // $NON-NLS-1$
  public static final String TORRENT_FILE_MIME_TYPE = "application/x-bittorrent"; // $NON-NLS-1$
  private static final String[] header = {
    Messages.getString("TorrentFileDatParser.File"),
  ...
  private static final String strConfirmedPieces =
      Messages.getString("TorrentFileDatParser.ConfirmedPieces");
  ...
  public void parse(
    ...
    final DateFormat df =
        new SimpleDateFormat(Messages.getString("TorrentFileDatParser.DateFormat")); // $NON-NLS-1$
    ...
    for (FileInTorrent file : files) {
      if (!file.md5.isEmpty()) include[3] = true;
      if (!file.sha1.isEmpty()) include[4] = true;
      if (!file.ed2k.isEmpty()) include[5] = true;
    }
    ...
    if (searcher != null
        && !files.isEmpty()
        && info.pieces != null
        && info.pieceLength != null
        && info.pieceLength > 0
        && info.pieceLength < maxPieceLength) {
      long tot = 0;
      for (int i = 0; i < files.size(); i++) {
        FileInTorrent file = files.get(i);
        if (file.item == null) {
          ItemPiecesMatchInfo itemPiecesMatchInfo =
              searchAndMatchFileInCase(
                  searcher,
                  file.length,
                  info.pieceLength,
                  info.pieces,
                  (int) (tot / info.pieceLength),
                  (int) (tot % info.pieceLength),
                  i == files.size() - 1);
    ...
    outputInfo(
        xhtml,
        Messages.getString("TorrentFileDatParser.NumberOfFiles"),
        Long.valueOf(files.size() - paddingEntries));
    if (foundInCase > 0) {
      outputInfo(
          xhtml,
          Messages.getString("TorrentFileDatParser.FilesFoundInCase"),
          Long.valueOf(foundInCase));
    }
    ...
    for (int i = 0; i < files.size(); i++) {
      ...
      String[] rowElements =
          new String[] {
            ...
            file.ed2k,
            "",
            file.item == null ? "" : file.item.getPath()
          };
  ...
  private static void outputMatchInfo(
      XHTMLContentHandler xhtml, ItemPiecesMatchInfo itemPiecesMatchInfo) throws SAXException {
    if (itemPiecesMatchInfo != null) {
  ...
  private static void outputInfo(XHTMLContentHandler xhtml, String key, Object value)
      throws SAXException {
    outputInfo(xhtml, key, value, false);
  }

  private static void outputInfo(
      XHTMLContentHandler xhtml, String key, Object value, boolean isMono) throws SAXException {
  ...
  private static List<FileInTorrent> extractFileList(
      BencodedDict dict, Metadata metadata, IItemSearcher searcher) throws TikaException {
  ...
  private static void linkTorrentToItem(
  ...
  private static ItemPiecesMatchInfo searchAndMatchFileInCase(
      IItemSearcher searcher,
      long fileLength,
      long pieceLength,
      String[] pieces,
    ...
    if (firstPiece + totPieces > pieces.length
        || ((!isLast || offset != 0) && totPieces < minPiecesMultiFile)) {
      return null;
    }
