...
/**
 * InputStream for reading bencoded data. Adaptation from BencodeInputStream from Adam Peck to end
 * without exception when parsing incomplete/broken stream with the information already parsed.
 *
 * @author Adam Peck
...
public class BencodedIncompleteInputStream extends FilterInputStream {
  ...
  /**
   * Creates a new BencodeInputStream that reads from the {@link InputStream} passed and uses the
   * {@link Charset} passed for decoding the data and boolean passed to control String parsing.
   *
   * <p>If useBytes is false, then dictionary values that contain byte string data will be coerced
   * to a {@link String}. if useBytes is true, then dictionary values that contain byte string data
   * will be coerced to a {@link ByteBuffer}.
   *
   ...
   * @since 1.3
   */
  public BencodedIncompleteInputStream(
      final InputStream in, final Charset charset, boolean useBytes) {
  ...
  /**
   * Creates a new BencodeInputStream that reads from the {@link InputStream} passed and uses the
   * {@link Charset} passed for decoding the data and coerces dictionary values to a {@link String}.
   *
   * @param in the {@link InputStream} to read from
   * @param charset the {@link Charset} to use
   * @throws NullPointerException if the {@link Charset} passed is null
  ...
  /**
   * Creates a new BencodeInputStream that reads from the {@link InputStream} passed and uses the
   * UTF-8 {@link Charset} for decoding the data and coerces dictionary values to a {@link String}.
   *
   * @param in the {@link InputStream} to read from
   * @see #BencodeInputStream(InputStream, Charset, boolean)
   */
   ...
   * @throws IOException if the underlying stream throws
   * @throws EOFException if the end of the stream has been reached
   * @throws InvalidObjectException if the next type in the stream is not a List, or the list
   ...
   * @throws IOException if the underlying stream throws
   * @throws EOFException if the end of the stream has been reached
   * @throws InvalidObjectException if the next type in the stream is not a Dictionary, or the list
   *     contains invalid types
   */
  ...
  private Object readObject(final int token) throws IOException {
    ...
    if (type == Type.STRING && !useBytes) return readString();
    if (type == Type.STRING && useBytes) return readStringBytes();
    ...
    throw new InvalidObjectException(
        "Unexpected token '" + new String(Character.toChars(token)) + "'");
  }
