...
public class FirefoxSavedSessionParser extends AbstractParser {
  ...
  private static final MediaType X_FIREFOX_SAVEDSESSION_MIME_TYPE =
      MediaType.application("x-firefox-savedsession");
  private static final int MAX_MEM_BYTES = 1 << 27;
  private static final String MAX_MEM_WARNING =
      "Byte data exceeds max size allowed to load on memory.";
  ...
  private void populateTextTabContent(XHTMLContentHandler xHandler, JsonNode rootNode)
      throws SAXException {
    ...
    try {
        ...
        for (JsonNode tab : tabs) {
          ...
          if (entries != null && entries.isArray()) {
            for (JsonNode entry : entries) {
              ...
              xHandler.characters(
                  entry.get("url") != null ? entry.get("url").toString() : "-"); // $NON-NLS-1$
              xHandler.endElement("td"); // $NON-NLS-1$

              xHandler.startElement("td"); // $NON-NLS-1$
              xHandler.characters(
                  entry.get("title") != null ? entry.get("title").toString() : "-"); // $NON-NLS-1$
              xHandler.endElement("td"); // $NON-NLS-1$
              xHandler.endElement("tr"); // $NON-NLS-1$
              empty = false;
            }
      ...
      if (cookies != null && cookies.isArray()) {
        for (JsonNode tmp : cookies) {
          ...
          xHandler.startElement("td"); // $NON-NLS-1$
          xHandler.characters(
              tmp.get("host") != null ? tmp.get("host").toString() : "-"); // $NON-NLS-1$
          xHandler.endElement("td"); // $NON-NLS-1$
          xHandler.startElement("td"); // $NON-NLS-1$
          xHandler.characters(
              tmp.get("path") != null ? tmp.get("path").toString() : "-"); // $NON-NLS-1$
          xHandler.endElement("td"); // $NON-NLS-1$
          xHandler.startElement("td"); // $NON-NLS-1$
          xHandler.characters(
              tmp.get("name") != null ? tmp.get("name").toString() : "-"); // $NON-NLS-1$
          xHandler.endElement("td"); // $NON-NLS-1$
          xHandler.startElement("td"); // $NON-NLS-1$
          xHandler.characters(
              tmp.get("value") != null ? tmp.get("value").toString() : "-"); // $NON-NLS-1$
          xHandler.endElement("td"); // $NON-NLS-1$
          xHandler.endElement("tr"); // $NON-NLS-1$
          empty = false;
        }
  ...
  private byte[] decompressLZ4Data(InputStream stream, Metadata metadata)
    ...
    try {
       ...
       */
      uncompressedLength =
          ((int) compressedFile[8] & 0xff)
              + (((int) compressedFile[9] & 0xff) * (1 << 8))
              + (((int) compressedFile[10] & 0xff) * (1 << 16))
              + (((int) compressedFile[11] & 0xff) * (1 << 24));
      ...
      myDecompressor.decompress(
          compressedFile, HEADER_OFFSET, compressedLength - HEADER_OFFSET, buffer, 0);
