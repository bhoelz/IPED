...
public class EdgeWebCacheParser extends AbstractParser {
  ...
  public static final MediaType EDGE_WEB_CACHE =
      MediaType.application("x-edge-web-cache"); // $NON-NLS-1$

  public static final MediaType EDGE_HISTORY =
      MediaType.application("x-edge-history"); // $NON-NLS-1$

  public static final MediaType EDGE_HISTORY_REG =
      MediaType.application("x-edge-history-registry"); // $NON-NLS-1$
  ...
  static {
    if (Platform.isWindows()) {
      ...
      try (InputStream is =
          EdgeWebCacheParser.class.getResourceAsStream(
              "/nativelibs/libesedb/" + arch + "/libesedb.dll")) {
        File file = new File(System.getProperty("java.io.tmpdir") + "/libesedb.dll");
        if (!file.exists()) Files.copy(is, file.toPath(), StandardCopyOption.REPLACE_EXISTING);
        System.load(file.getAbsolutePath());

      } catch (Throwable e) {
        log.error(
            "Libesedb dll not loaded properly. "
                + EdgeWebCacheParser.class.getSimpleName()
                + " will be disabled.",
            e);
    ...
    if (!SUPPORTED_TYPES.isEmpty())
      try {
        ...
        log.error(
            "Libesedb JNA not loaded properly. "
                + EdgeWebCacheParser.class.getSimpleName()
                + " will be disabled.");
        e.printStackTrace();
        SUPPORTED_TYPES = Collections.EMPTY_SET;
  ...
  private void printError(
      String function, int result, PointerByReference errorPointer, ItemInfo itemInfo) {
    log.warn(
        "Error decoding "
            + itemInfo.getPath()
            + ": Function '"
            ...
            + result
            + "' Error value: "
            + errorPointer.getValue().getString(0)); // $NON-NLS-1$
        ...
        for (EdgeContainer ec : history) {
          ...
          try (FileOutputStream tmpHistoryFile = new FileOutputStream(webcacheFile)) {
            ...
            historyMetadata.add(
                ...
                "Edge History " + ec.getTableName()); // $NON-NLS-1$
          ...
          for (EdgeVisit ev : ec) {
            ...
            metadataHistory.add(
                TikaCoreProperties.RESOURCE_NAME_KEY, "Edge History Entry " + i); // $NON-NLS-1$
  ...
  private void parseEdgeHistory(
      ContentHandler handler,
      Metadata metadata,
      ParseContext context,
      Iterable<EdgeVisit> edgeHistory)
      throws IOException, SAXException, TikaException {
  ...
  protected List<EdgeContainer> getHistory(
      String filePath, PointerByReference filePointerReference, ItemInfo itemInfo)
    ...
    try {
      ...
      synchronized (lock) {
        ...
        if (result < 0) printError("File Initialize", result, errorPointer, itemInfo);

        result = esedbLibrary.libesedb_check_file_signature(filePath, errorPointer);
        if (result < 0) printError("Check File Signature", result, errorPointer, itemInfo);
        ...
        if (result < 0) printError("File Open", result, errorPointer, itemInfo);

        result =
            esedbLibrary.libesedb_file_get_number_of_tables(
                filePointerReference.getValue(), numberOfTables, errorPointer);
        if (result < 0) printError("File Get Number of Tables", result, errorPointer, itemInfo);
      ...
      for (int tables = 0; tables < numTables; tables++) {
        ...
        result =
            esedbLibrary.libesedb_file_get_table(
                filePointerReference.getValue(), tables, tablePointerReference, errorPointer);
        if (result < 0) printError("File Get Table", result, errorPointer, itemInfo);

        result =
            esedbLibrary.libesedb_table_get_utf8_name_size(
                tablePointerReference.getValue(), tableNameSize, errorPointer);
        if (result < 0) printError("Table Get UTF8 Name Size", result, errorPointer, itemInfo);

        result =
            esedbLibrary.libesedb_table_get_utf8_name(
                tablePointerReference.getValue(),
                tableName,
                tableNameSize.getValue(),
                errorPointer);
        if (result < 0) printError("Table Get UTF8 Name", result, errorPointer, itemInfo);
        String tableNameString = tableName.getString(0);

        result =
            esedbLibrary.libesedb_table_get_number_of_columns(
                tablePointerReference.getValue(), numberOfColumns, columnFlags, errorPointer);
        if (result < 0) printError("Table Get Number of Columns", result, errorPointer, itemInfo);

        result =
            esedbLibrary.libesedb_table_get_number_of_records(
                tablePointerReference.getValue(), numberOfRecords, errorPointer);
        if (result < 0) printError("Table Get Number of Records", result, errorPointer, itemInfo);
        ...
        if (tableNameString.contains("Container_")) {
          ...
          EdgeContainer ec =
              new EdgeContainer(tableNameString, tablePointerReference) {
                @Override
                public Iterator<EdgeVisit> iterator() {
                  return new Iterator<EdgeVisit>() {
                    ...
                    public EdgeVisit next() {
                      return getRecord(
                          i++, errorPointer, tablePointerReference, columnType, itemInfo);
                    }
                  };
                }
              };
  ...
  private void closeTablePointer(PointerByReference tablePointer, ItemInfo itemInfo) {
    ...
    if (result < 0) printError("Table Free", result, errorPointer, itemInfo);
  }
  ...
  private void closeFilePointer(PointerByReference filePointerReference, ItemInfo itemInfo) {
    ...
    if (result < 0) printError("File Close", result, errorPointer, itemInfo);

    result = esedbLibrary.libesedb_file_free(filePointerReference, errorPointer);
    if (result < 0) printError("File Free", result, errorPointer, itemInfo);
  }

  private EdgeVisit getRecord(
      int i,
      PointerByReference errorPointer,
      PointerByReference tablePointerReference,
    ...
    result =
        esedbLibrary.libesedb_table_get_record(
            tablePointerReference.getValue(), i, recordPointerReference, errorPointer);
    if (result < 0) printError("Table Get Record", result, errorPointer, itemInfo);

    result =
        esedbLibrary.libesedb_record_get_number_of_values(
            recordPointerReference.getValue(), recordNumberOfValues, errorPointer);
    if (result < 0) printError("Record Get Number of Values", result, errorPointer, itemInfo);
    ...
    result =
        esedbLibrary.libesedb_record_get_value_64bit(
            recordPointerReference.getValue(), 0, recordValueData, errorPointer);
    if (result < 0) printError("Record Get EntryId Data", result, errorPointer, itemInfo);
    entryId = recordValueData.getValue();
    /* Integer 64bit signed */
    result =
        esedbLibrary.libesedb_record_get_value_64bit(
            recordPointerReference.getValue(), 5, recordValueData, errorPointer);
    if (result < 0) printError("Record Get FileSize Data", result, errorPointer, itemInfo);
    fileSize = recordValueData.getValue();
    /* Integer 32bit unsigned */
    result =
        esedbLibrary.libesedb_record_get_value_32bit(
            recordPointerReference.getValue(), 8, recordValueData32, errorPointer);
    if (result < 0) printError("Record Get AccessCount Data", result, errorPointer, itemInfo);
    ...
    result =
        esedbLibrary.libesedb_record_get_value_64bit(
            recordPointerReference.getValue(), 10, recordValueData, errorPointer);
    if (result < 0) printError("Record Get CreationTime Data", result, errorPointer, itemInfo);
    creation = TimeConverter.filetimeToMillis(recordValueData.getValue());
    /* Integer 64bit signed */
    result =
        esedbLibrary.libesedb_record_get_value_64bit(
            recordPointerReference.getValue(), 12, recordValueData, errorPointer);
    if (result < 0) printError("Record Get ModifiedTime Data", result, errorPointer, itemInfo);
    modified = TimeConverter.filetimeToMillis(recordValueData.getValue());
    /* Integer 64bit signed */
    result =
        esedbLibrary.libesedb_record_get_value_64bit(
            recordPointerReference.getValue(), 13, recordValueData, errorPointer);
    if (result < 0) printError("Record Get AccessedTime Data", result, errorPointer, itemInfo);
    accessed = TimeConverter.filetimeToMillis(recordValueData.getValue());

    /* Large Text */
    result =
        esedbLibrary.libesedb_record_get_column_type(
            recordPointerReference.getValue(), 17, columnType, errorPointer);
    if (result < 0) printError("Record Get Column Type", result, errorPointer, itemInfo);

    result =
        esedbLibrary.libesedb_record_get_value_data_flags(
            recordPointerReference.getValue(), 17, valueDataFlags, errorPointer);
    if (result < 0) printError("Record Get Value Data Flags", result, errorPointer, itemInfo);

    if (valueDataFlags.getValue() == 1) {
      result =
          esedbLibrary.libesedb_record_get_value_utf8_string_size(
              recordPointerReference.getValue(), 17, recordValueDataSize, errorPointer);
      if (result < 0) printError("Record Get URL UTF8 String Size", result, errorPointer, itemInfo);

      if ((recordValueDataSize.getValue() > 0) && (result == 1)) {
        result =
            esedbLibrary.libesedb_record_get_value_utf8_string(
                recordPointerReference.getValue(),
                17,
                recordValueDataUrl,
                recordValueDataSize.getValue(),
                errorPointer);
        if (result < 0) printError("Record Get URL UTF8 String", result, errorPointer, itemInfo);
    ...
    if ((recordValueDataSize.getValue() > 0) && (result == 1)) {
      result =
          esedbLibrary.libesedb_record_get_value_utf8_string(
              recordPointerReference.getValue(),
              18,
              recordValueDataFilename,
              recordValueDataSize.getValue(),
              errorPointer);
      if (result < 0) printError("Record Get FileName UTF8 String", result, errorPointer, itemInfo);
      file = recordValueDataFilename.getString(0);
    }

    result = esedbLibrary.libesedb_record_free(recordPointerReference, errorPointer);
    if (result < 0) printError("Record Free", result, errorPointer, itemInfo);
