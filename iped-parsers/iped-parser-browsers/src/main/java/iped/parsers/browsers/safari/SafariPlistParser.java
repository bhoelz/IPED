 ...
 *
 * <p>https://medium.com/@karaiskc/understanding-apples-binary-property-list-format-281e6da00dbd
...
public class SafariPlistParser extends AbstractParser {
  ...
  public static final MediaType SAFARI_PLIST =
      MediaType.application("x-safari-plist"); // $NON-NLS-1$

  public static final MediaType SAFARI_HISTORY =
      MediaType.application("x-safari-history"); // $NON-NLS-1$

  public static final MediaType SAFARI_HISTORY_REG =
      MediaType.application("x-safari-history-registry"); // $NON-NLS-1$

  public static final MediaType SAFARI_DOWNLOADS =
      MediaType.application("x-safari-downloads"); // $NON-NLS-1$

  public static final MediaType SAFARI_DOWNLOADS_REG =
      MediaType.application("x-safari-downloads-registry"); // $NON-NLS-1$
      ...
      if (extractor.shouldParseEmbedded(metadata)) {
        ...
        if (evidenceFile.contains("History")) {
          ...
          try (FileOutputStream tmpHistoryFile = new FileOutputStream(historyFile)) {
            ...
            historyMetadata.add(
                TikaCoreProperties.RESOURCE_NAME_KEY, "Safari Plist History"); // $NON-NLS-1$
            ...
            parseSafariResumedHistory(
                stream, historyHandler, historyMetadata, context, resumedHistory);
          ...
          for (SafariResumedVisit v : resumedHistory) {
            ...
            metadataHistory.add(
                ...
                "Safari Plist History Entry " + i); // $NON-NLS-1$
        ...
        if (evidenceFile.contains("Downloads")) {
          ...
          try (FileOutputStream tmpDownloadFile = new FileOutputStream(downloadFile)) {
            ...
            ToXMLContentHandler downloadHandler =
                new ToXMLContentHandler(tmpDownloadFile, "UTF-8"); // $NON-NLS-1$
            Metadata downloadMetadata = new Metadata();
            downloadMetadata.add(INDEXER_CONTENT_TYPE, SAFARI_DOWNLOADS.toString());
            downloadMetadata.add(
                TikaCoreProperties.RESOURCE_NAME_KEY, "Safari Plist Downloads"); // $NON-NLS-1$
          ...
          for (Download d : downloads) {
            ...
            metadataDownload.add(INDEXER_CONTENT_TYPE, SAFARI_DOWNLOADS_REG.toString());
            metadataDownload.add(
                TikaCoreProperties.RESOURCE_NAME_KEY,
                "Safari Plist Download Entry " + i); // $NON-NLS-1$
        ...
        if (evidenceFile.contains("Bookmarks")) {
          ...
          try (FileOutputStream tmpBookmarkFile = new FileOutputStream(bookmarkFile)) {
            ...
            ToXMLContentHandler bookmarkHandler =
                new ToXMLContentHandler(tmpBookmarkFile, "UTF-8"); // $NON-NLS-1$
            Metadata bookmarkMetadata = new Metadata();
            bookmarkMetadata.add(INDEXER_CONTENT_TYPE, SAFARI_DOWNLOADS.toString());
            bookmarkMetadata.add(
                TikaCoreProperties.RESOURCE_NAME_KEY, "Safari Plist Bookmarks"); // $NON-NLS-1$
          ...
          for (SafariBookmark b : bookmarks) {
            ...
            metadataBookmark.add(INDEXER_CONTENT_TYPE, SAFARI_DOWNLOADS_REG.toString());
            metadataBookmark.add(
                TikaCoreProperties.RESOURCE_NAME_KEY,
                "Safari Plist Bookmark Entry " + i); // $NON-NLS-1$
  ...
  private void parseSafariResumedHistory(
      InputStream stream,
      ContentHandler handler,
      Metadata metadata,
      ParseContext context,
      List<SafariResumedVisit> resumedHistory)
      throws IOException, SAXException, TikaException {
  ...
  protected List<SafariResumedVisit> getResumedHistory(InputStream is) throws Exception {
    ...
    for (NSObject param : parameters) {
      ...
      Double lastVisitedDate =
          dateStr.isEmpty() ? 0 : ((Double.parseDouble(dateStr) + 978307200) * 1000);
      ...
      resumedHistory.add(
          new SafariResumedVisit(i, title, url, visitCount, lastVisitedDate.longValue()));
    }
  ...
  private void parseSafariDownloads(
      InputStream stream,
      ToXMLContentHandler downloadHandler,
  ...
  private String getString(NSObject nsObj) {
    ...
    else return ((NSString) nsObj).getContent();
  }
  ...
  private void parseChildren(List<SafariBookmark> bookmarks, NSDictionary dict) {
    ...
    if (children != null) {
      ...
      if (array != null)
        for (NSObject child : array) parseChildren(bookmarks, (NSDictionary) child);
    }
  ...
  private void parseSafariBookmarks(
      InputStream stream,
      ToXMLContentHandler bookmarkHandler,
      Metadata bookmarkMetadata,
      ParseContext context,
      List<SafariBookmark> bookmarks)
      throws SAXException {
    XHTMLContentHandler xHandler = null;
