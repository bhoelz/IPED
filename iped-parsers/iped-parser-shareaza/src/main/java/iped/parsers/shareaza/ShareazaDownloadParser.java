...
public class ShareazaDownloadParser extends AbstractParser {
  ...
  private static final Set<MediaType> SUPPORTED_TYPES =
      Collections.singleton(MediaType.parse(SHAREAZA_DOWNLOAD_META));
  ...
  private static final String INCOMPLETE_FILE_EX_MESSAGE =
      "Error during file parsing, possible incomplete or corrupted file.";
  private static final String ERROR_READING_CONTROL_BYTES =
      "Error reading control bytes, data is possibly corrupted";
  ...
  public void parse(
    ...
    try {
      ...
      processSDFile(
          stream, handler, xhtml, searcher, metadata, context, item.getPath(), item.getName());
  ...
  public void processSDFile(
    ...
    try {
      ...
      if (version >= 33) {
        ...
        if (searchTerms.equals("")) searchTerms = "<none present>";
      ...
      if (version >= 29) {
        ...
        addLine(
            xhtml,
            "File Length:             "
                + df.format(fileLenLow)
                + " Bytes ("
                + df2.format(dbFileLen)
                + " MB)");
      ...
      for (int i = 0; i < numberOfSources; i++) {
        ...
        sbSources.append(
            "          Protocol ID:                    " + getProtocolName(protocolId) + "\n");
        ...
        sbSources.append(
            "          Hash Auth:                      " + getBoolStr(hashAuth) + "\n");
        sbSources.append("          Use SHA1:                       " + getBoolStr(useSha1) + "\n");
        sbSources.append(
            "          Use TIGER:                      " + getBoolStr(useTiger) + "\n");
        sbSources.append(
            "          Use EDONKEY:                    " + getBoolStr(useEdonkey) + "\n");
        ...
        sbSources.append(
            "          Nick Name:                      "
                + (nickName.isBlank() ? "<none>" : nickName)
                + "\n");
        sbSources.append("          Country Code:                   " + countryCode + "\n");
        sbSources.append("          Country Name:                   " + countryName + "\n");
        sbSources.append("          Speed:                          " + speed + "\n");
        sbSources.append(
            "          Push Only:                      " + getBoolStr(pushOnly) + "\n");
        sbSources.append(
            "          Close Connection:               " + getBoolStr(closeConn) + "\n");
        sbSources.append(
            "          Read Content                    " + getBoolStr(readContent) + "\n");
        ...
        if (version >= 39) {
          ...
          sbSources.append(
              "          Client Extended:                " + getBoolStr(clientExtended) + "\n");
        }

        if (version >= 42) {
          int metaIgnore = read4Bytes(buffer);
          sbSources.append(
              "          Meta Ignore:                    " + getBoolStr(metaIgnore) + "\n");
        }
      }
      ...
      if (version >= 22) {
        ...
        if (torrentValid > 0) {
          ...
          for (int i = 0; i < fileCount; i++) {
            ...
            sbTorrent.append(
                "              Torrent File Size:              " + torrentFileSize + "\n");
            ...
            if (torrentFile != null && torrentFile.length > 0) {
              ...
              extractor.parseEmbedded(
                  new ByteArrayInputStream(torrentFile), handler, torrentMeta, true);
            }
          }
  ...
  public int getEd2kListSerialSize(int nListSize) {
    int nSize = 0;
    if (nListSize > 0) nSize += 16;
    if (nListSize > 1) nSize += 16 * nListSize;
    return nSize;
  }
  ...
  public void readXMLNode(
      ByteBuffer buffer, XHTMLContentHandler xhtml, int level, StringBuilder sbXML)
      throws BufferUnderflowException, TikaException {
