...
public class Extractor {
  ...
  private int getParticipantsCount(Connection conn, ChatGroup cg) throws SQLException {
    ...
    if (SQLite3DBParser.containsTable(table, conn)
        && SQLite3DBParser.checkIfColumnExists(conn, table, column)) {
  ...
  protected Chat extractMessages() throws Exception {
    ...
    do {
      ...
      if (chat == null) {
        ...
        if (chat == null) {
          log.warn("Chat with id {} not found in chat list, creating a new one", chatId);
          Contact c = getContact(chatId);
      ...
      if (message.getMediaMime() != null) {
        if (message.getMediaMime().startsWith("image")) {
          ...
          loadDocument(
              message, androidDecoder.getDocumentNames(), androidDecoder.getDocumentSize());
        }
      }
        ...
        while (rs.next()) {
          ...
          if (c != null && c.getId() > 0) {
            ...
            if (cont.getAvatar() != null && !cont.getPhotos().isEmpty()) {
              try {
                if (cont.getPhone() != null) searchAvatarFileName(cont, cont.getPhotos());
              } catch (IOException e) {
  ...
  protected void searchAvatarFileName(Contact contact, List<PhotoData> photos) throws IOException {
    if (photos == null || searcher == null) return;
      ...
      if (photo.getName() != null) {
        ...
        if (searcher != null) {
          String query =
              BasicProps.NAME
                  + ":\""
                  + searcher.escapeQuery(name)
                  + "\"  -"
                  + BasicProps.LENGTH
                  + ":0";
          result = (searcher == null ? Collections.emptyList() : searcher.search(query));
        }
        if (result != null && !result.isEmpty()) {
          break;
        }
  ...
  private static final Comparator<MessageMultiMedia> MSG_TIME_COMPARATOR =
      new Comparator<MessageMultiMedia>() {
  ...
  private String getAndroidExtractMessagesSQL() throws SQLException {
    ...
    return "SELECT * FROM (SELECT m.uid, m.mid, m.data, md.data as mediaData, md.mid as mdmid "
        + "FROM "
        + tmessage
        + " m  LEFT JOIN "
        + tmedia
        + " md ON (md.mid=m.mid AND m.uid=md.uid)"
        + "UNION ALL "
        + "SELECT md.uid,m.mid as mid, m.data as data, md.data as mediaData, md.mid as mdmid "
        + "FROM "
        + tmessage
        + " m "
        + "RIGHT JOIN "
  ...
  private static final String CHATS_SQL_V2 =
      "SELECT -c.uid as chatId,null as chatName,null as chatData, c.name as groupName, c.data as groupData ,d.date"
  ...
  private static final String SELECT_CHATS_SETTINGS =
      "SELECT participants_count FROM chat_settings_v2 WHERE uid=?";

  private static final String SELECT_CHANNEL_ADMINS =
      "SELECT uid FROM channel_admins_v3 WHERE did=?";

  private static final String CHATS_SQL_IOS =
      "select substr(key,16,8) as chatid, false as deleted from t9 "
  ...
  private static final String EXTRACT_MESSAGES_SQL_IOS =
      "SELECT t7.key,t7.value FROM t7 where substr(t7.key,1,8)=? and "
