...
public class ReportGenerator {
  ...
  public byte[] genarateContactHtml(Contact contact) {
    ...
    if (contact.getAvatar() != null)
      out.println(
          "<img src=\"data:image/jpg;base64," //$NON-NLS-1$
              + Util.encodeBase64(contact.getAvatar())
              + "\" width=\"112\"/><br>"); //$NON-NLS-1$
    out.println(Messages.getString("TelegramContact.ContactID") + " " + contact.getId());
    out.println(
        "<br>" + Messages.getString("TelegramContact.FirstName") + " " + format(contact.getName()));
    out.println(
        "<br>"
            + Messages.getString("TelegramContact.LastName")
            + " "
            + format(contact.getLastName()));
    out.println(
        "<br>"
            + Messages.getString("TelegramContact.Username")
            + " "
            + format(contact.getUsername()));
    out.println(
        "<br>" + Messages.getString("TelegramContact.Phone") + " " + format(contact.getPhone()));
    if (contact.isGroup()) {
  ...
  public byte[] generateNextChatHtml(Chat c) {
    ...
    if (c.isGroup()) {
      ...
      if (c.getC().getPhone() != null) {
        title +=
            " (" + Messages.getString("TelegramContact.Phone") + " " + c.getC().getPhone() + ")";
    ...
    printMessageFileHeader(
        out, title, c.getC().getAvatar(), c.isGroup(), c.isChannel(), c.isDeleted());
  ...
  private TagHtml getThumbTag(Message m, String classnotfound) {
    ...
    if (searcher != null
        && thumb == null
        && m.getMediaHash() != null
        && !m.getMediaHash().isBlank()) {
      if (!m.getMediaHash().equalsIgnoreCase(emptyMD5)) {
        List<IItemReader> result = Util.getItems("md5:" + m.getMediaHash(), searcher);
    ...
    if (thumb != null) {
      ...
      img.setAtribute("src", "data:image/jpg;base64," + Util.encodeBase64(thumb));
    } else {
      img = new TagHtml("div");
  ...
  private void printVideo(PrintWriter out, Message message) {
    ...
    if (message.getMediaHash() != null) {
      ...
      String reportSource = Util.getExportPath(message.getMediaHash(), message.getMediaExtension());
      String originalSource = Util.getSourceFileIfExists(message.getMediaItem()).orElse("");
  ...
  private void printAudio(PrintWriter out, Message message) {
    ...
    if (message.getMediaHash() != null) {
      ...
      String reportSource = Util.getExportPath(message.getMediaHash(), message.getMediaExtension());
      String originalSource = Util.getSourceFileIfExists(message.getMediaItem()).orElse(null);
      ...
      Metadata mediaMetadata =
          message.getMediaItem() == null ? null : (Metadata) message.getMediaItem().getMetadata();
      if (mediaMetadata != null && mediaMetadata.get(ExtraProperties.TRANSCRIPT_ATTR) != null) {
        ...
        transcriptionText.append(format(mediaMetadata.get(ExtraProperties.TRANSCRIPT_ATTR)));
  ...
  private void printImage(PrintWriter out, Message message, boolean isLink) {
    if (isLink) {
      ...
      if (message.getUrl() != null) {
        out.print(
            Messages.getString("TelegramReport.LinkURL")
                + ": "
      ...
      if (message.getLinkTitle() != null) {
        out.print(
            Messages.getString("TelegramReport.LinkTitle")
                + ": "
                + format(message.getLinkTitle())
                + "<br/>");
      }
    }
    if (message.getMediaHash() != null) {
      ...
      TagHtml link = new TagHtml("a");
      link.setAtribute("onclick", "app.open('hash:" + message.getMediaHash() + "')");
      String ref = Util.getReportHref(message.getMediaItem());
  ...
  private void printCheckbox(PrintWriter out, String hash) {
    out.println(
        "<input class=\"check\" type=\"checkbox\" onclick=\"app.check('hash:"
            + hash
            + "', this.checked)\" name=\""
            + hash
            + "\" />");
  ...
  private void printMessage(PrintWriter out, Message message) {
    ...
    if (poolData != null) {
      out.println(
          "<b>"
              + Messages.getString("TelegramReport.Pool")
              + "</b><br/>"
              + format(poolData.getTitle()));
      out.println("<ul>");
    ...
    if (!message.getChildPornSets().isEmpty()) {
      out.print(
          "<p><i>"
              + Messages.getString("TelegramReport.FoundInPedoHashDB")
              + " "
              + message.getChildPornSets().toString()
              + "</i></p>");
    }
  ...
  private static void printMessageFileHeader(
      PrintWriter out,
      String title,
      byte[] avatar,
      boolean isGroup,
    ...
    out.println(
        "<!DOCTYPE html>\n" //$NON-NLS-1$
            ...
            + "     <meta charset=\"UTF-8\" />\n" //$NON-NLS-1$
            + "<link rel=\"icon\" href=\""
            + Util.getImageResourceAsEmbedded("img/telegram.png")
            + "\">\n"
            + "<style>\n"
            ...
            + "</script>"
            + iped.parsers.vcard.VCardParser.HTML_STYLE
            + "</head>\n" //$NON-NLS-1$
            + "<style>.check {vertical-align: top;}</style>"
            + "<body style='background-image:url(" //$NON-NLS-2$
            + Util.getImageResourceAsEmbedded("img/telegramwallpaper.jpg")
            + ")'>\n"
    ...
    if (avatar != null) {
      out.println(
          "<img src=\"data:image/jpg;base64,"
              + Util.encodeBase64(avatar) // $NON-NLS-1$
