...
public class TelegramParser extends SQLite3DBParser {
  ...
  public static final MediaType TELEGRAM_ACCOUNT =
      MediaType.parse("application/x-telegram-account");
  public static final MediaType TELEGRAM_USER_CONF =
      MediaType.parse("application/x-telegram-user-conf");
  ...
  public static final MediaType TELEGRAM_ATTACHMENT =
      MediaType.parse("message/x-telegram-attachment");
  ...
  private static Set<MediaType> SUPPORTED_TYPES =
      MediaType.set(TELEGRAM_DB, TELEGRAM_USER_CONF, TELEGRAM_DB_IOS);
      ...
      for (Message m : mmm.getMessages()) {
        if (Util.isValidHash(m.getMediaHash())) {
          metadata.add(
              ...
              BasicProps.HASH + ":" + m.getMediaHash()); // $NON-NLS-1$
          if (m.isFromMe()) metadata.add(ExtraProperties.SHARED_HASHES, m.getMediaHash());
        }
      }
    }
  }

  public void parseTelegramDBAndroid(
    ...
    try (Connection conn = getConnection(stream, metadata, context)) {
      ...
      DecoderTelegramInterface d =
          (DecoderTelegramInterface)
              Class.forName(Extractor.DECODER_CLASS).getDeclaredConstructor().newInstance();
      ...
      if (e.getContacts() != null) {
        for (Contact c : e.getContacts().values()) {
          ...
          cMetadata.set(ParserConstants.INDEXER_CONTENT_TYPE, TELEGRAM_CONTACT.toString());
  ...
  private void generateChat(
    ...
    while ((bytes = r.generateNextChatHtml(c)) != null) {
      ...
      if (frag > 0 || nextMsg < c.getMessages().size()) chatName += "_" + frag++; // $NON-NLS-1$

      Metadata chatMetadata = new Metadata();
      chatMetadata.set(TikaCoreProperties.TITLE, chatName);
      chatMetadata.set(ParserConstants.INDEXER_CONTENT_TYPE, TELEGRAM_CHAT.toString());
      ...
      if (c.isGroupOrChannel()) {
        ...
        for (long id : cg.getAdmins()) {
          chatMetadata.add(
              ExtraProperties.COMMUNICATION_PREFIX + "ChannelAdmins", e.getContact(id).toString());
        }
        int participantsCount = cg.getParticipantsCount();
        if (participantsCount > 0) {
          chatMetadata.add(
              ExtraProperties.PARTICIPANTS + "Count", String.valueOf(participantsCount));
        }
      }
  ...
  private void extractMessages(
      String chatName,
      List<MessageMultiMedia> messages,
      Contact account,
      Extractor e,
      long parentId,
      ContentHandler handler,
    ...
    for (MessageMultiMedia m : messages) {
      ...
      meta.set(ParserConstants.INDEXER_CONTENT_TYPE, TELEGRAM_MESSAGE.toString());
      ...
      if (meta.get(org.apache.tika.metadata.Message.MESSAGE_TO) == null) {
        if (m.getToId() != 0) {
          meta.set(
              org.apache.tika.metadata.Message.MESSAGE_TO, e.getContact(m.getToId()).toString());
        } else if (m.isFromMe()) {
      ...
      for (Message media : m.getMessages()) {
        ...
        if (Util.isValidHash(media.getMediaHash())) {
          ...
          meta.add(
              ExtraProperties.LINKED_ITEMS,
              BasicProps.HASH + ":" + media.getMediaHash()); // $NON-NLS-1$
      ...
      if (m.isPhoneCall()) {
        meta.set(ParserConstants.INDEXER_CONTENT_TYPE, TELEGRAM_CALL.toString());
      }
      // system messages
  ...
  public void parseTelegramDBIOS(
      InputStream stream, ContentHandler handler, Metadata metadata, ParseContext context)
      throws IOException, SAXException, TikaException {
    try (Connection conn = getConnection(stream, metadata, context)) {
      IItemSearcher searcher = context.get(IItemSearcher.class);
  ...
  private Contact decodeAndroidAccount(InputStream stream) throws Exception {
    ...
    if (nl.getLength() > 0) {
      ...
      byte[] b = Base64.getDecoder().decode(e.getTextContent().trim());
  ...
  public void parseAndroidAccount(
      InputStream stream, ContentHandler handler, Metadata metadata, ParseContext context)
  ...
  private void createAccountHTML(Contact user, ContentHandler handler, ParseContext context)
    ...
    Metadata meta = new Metadata();
    meta.set(ParserConstants.INDEXER_CONTENT_TYPE, TELEGRAM_ACCOUNT.toString());
    ...
    if (user.getAvatar() != null) {
      meta.set(
          ExtraProperties.THUMBNAIL_BASE64, Base64.getEncoder().encodeToString(user.getAvatar()));
    }
