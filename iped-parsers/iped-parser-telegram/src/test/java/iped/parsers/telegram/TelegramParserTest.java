...
public class TelegramParserTest extends AbstractPkgTest {
  ...
  public void testTelegramParser() throws IOException, SAXException, TikaException {
    ...
    metadata.add(ParserConstants.INDEXER_CONTENT_TYPE, TelegramParser.TELEGRAM_DB.toString());
    try (InputStream stream = getStream("test-files/test_telegramCache4.db")) {
      ...
      assertEquals(
          "Group constelação familiar e barras de access (id:-1459548546)",
          telegramtracker.messageto.get(0));
      assertEquals(
          "Group le chad et les chadettes (id:-1391969250)", telegramtracker.messageto.get(1));
  ...
  public void testTelegramParserAndroidAcc() throws IOException, SAXException, TikaException {
    ...
    metadata.add(
        ParserConstants.INDEXER_CONTENT_TYPE, TelegramParser.TELEGRAM_USER_CONF.toString());
