 ...
 *
 * <p>Jobs are keyed by a UUID string. Each job has a {@link JobEntry} that
 * carries the current status, progress counter, and a list of SSE subscriber
 * queues so {@link JobsSseV2} can push events without polling.
 *
 * <p>Completed/failed jobs are retained for {@value #RETENTION_SECONDS} seconds
 * after reaching a terminal state to allow callers to collect the result before
 * GC. A daemon thread runs cleanup every minute.
 */
public final class JobRegistry {
    ...
    private static void expire() {
        ...
        JOBS.values().removeIf(e -> e.isTerminal() && e.terminatedAt != null
                && e.terminatedAt.isBefore(cutoff));
    }
    ...
    public static final class JobEntry {
        ...
        private void emit(JobEvent e) {
            subscribers.forEach(s -> {
                try { s.accept(e); } catch (Exception ignored) {}
            });
        }
