 ...
 * </pre>
 *
 * <p>Immediately delivers the current status event so the client never has to
 * poll first. Subsequent events are pushed as the job progresses. The stream
...
public class JobsSseV2 {
    ...
    public void events(@PathParam("id") String id, @Context SseEventSink sink) {
        ...
        if (jobOpt.isEmpty()) {
            OutboundSseEvent notFound = sse.newEventBuilder()
                    .name("error")
        ...
        java.util.function.Consumer<JobEvent> consumer = event -> {
            ...
            OutboundSseEvent sseEvent = sse.newEventBuilder()
                    ...
                    .build();
            sink.send(sseEvent);
            if ("completed".equals(event.event()) || "failed".equals(event.event())
        ...
        sink.send(sse.newEventBuilder().comment("connected").build())
                .exceptionally(ex -> { job.unsubscribe(consumer); return null; });
    }
