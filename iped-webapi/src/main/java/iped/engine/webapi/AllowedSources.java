 ...
 * <ol>
 *   <li>{@code iped.webapi.allowed-sources} — comma-separated source IDs</li>
 *   <li>{@code IPED_WEBAPI_ALLOWED_SOURCES} — same format</li>
 * </ol>
 *
 * <p>When neither is set the allow-list is {@code null}, meaning unrestricted access
 * (all sources are allowed). This preserves the open-access default for development.
 *
 * <p>The configured set is resolved once at class-load time and cached for the
 * lifetime of the JVM.
 */
public final class AllowedSources {
    ...
    /**
     * Returns {@code true} when {@code sourceId} may be accessed by the current
     * session (either no allow-list is configured, or the ID is in the list).
     */
    ...
    /**
     * Parses a comma-separated source-ID string into an immutable set.
     * Blank entries and surrounding whitespace are discarded.
     */
