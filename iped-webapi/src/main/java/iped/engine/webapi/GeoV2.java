...
/**
 * v2 GeoJSON endpoint — returns a {@code FeatureCollection} of all items that carry GPS coordinates
 * in the given source.
 *
 * <p>Coordinates are stored during indexing in {@code ExtraProperties.LOCATIONS} ({@code
 * "common:geo:locations"}) as {@code "lat;lon"} pairs (semicolon-separated, multi-valued). Altitude
 * is stored in {@code "common:altitude"}.
 *
 * <h2>Endpoints</h2>
 *
 * <pre>
...
public class GeoV2 {
  ...
  @ApiOperation(
      ...
      notes =
          "Items with multiple location values each contribute multiple GeoJSON features. "
  ...
  public Response sourceGeo(
    ...
    StringBuilder sb =
        startFeatureCollection(page.getTotal(), page.getItems().size() < page.getTotal());
    boolean first = true;
    for (SearchResultItem resultItem : page.getItems()) {
      ...
      for (String loc : locations) {
        ...
        String json =
            buildFeature(featureId, sourceId, resultItem.getId(), item.getName(), loc, meta);
        if (json != null) {
          if (!first) sb.append(',');
          sb.append(json);
          first = false;
  ...
  @Path("items/{id}/geo")
  @Produces(MediaType.APPLICATION_JSON)
  public Response itemGeo(@PathParam("sourceId") String sourceId, @PathParam("id") int id) {
    IIPEDSource source = Sources.getSource(sourceId);
  ...
  /**
   * Builds a GeoJSON Feature for one {@code "lat;lon"} location value, or returns {@code null} when
   * the coordinates cannot be parsed.
   */
  private static String buildFeature(
    ...
    if (altStr != null) {
      try {
        f.append(',').append(Double.parseDouble(altStr.trim()));
