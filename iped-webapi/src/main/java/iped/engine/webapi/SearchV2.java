 ...
 *
 * <p>Unlike the legacy {@code GET /search} endpoint (which returns all matching document IDs with
 * no metadata), this endpoint:
 *
 * <ul>
 *   <li>Supports {@code offset}/{@code limit} pagination (max 1000 per page).
 *   <li>Returns rich item metadata per result so the UI can render the list without additional
 *       round-trips.
 * </ul>
 *
 * <h2>Example</h2>
...
public class SearchV2 {
  ...
  @ApiOperation(
      ...
      notes =
          "Searches all open sources (or a single source when sourceId is given) and "
  ...
  public Response search(
      ...
      @ApiParam(value = "Restrict to a specific source ID; omit for all sources")
          @QueryParam("sourceId")
          String sourceIdParam,
      @ApiParam(value = "Zero-based start index", example = "0")
          @QueryParam("offset")
          @DefaultValue("0")
          int offsetParam,
      @ApiParam(value = "Items per page (1–1000)", example = "50")
          @QueryParam("limit")
          @DefaultValue("50")
          int limitParam)
      throws Exception {
