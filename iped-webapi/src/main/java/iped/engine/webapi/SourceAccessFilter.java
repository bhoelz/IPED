 ...
 *
 * <p>When {@link AllowedSources} is configured (via {@code iped.webapi.allowed-sources} system
 * property or {@code IPED_WEBAPI_ALLOWED_SOURCES} environment variable), this filter rejects
 * requests for source IDs not in the allow-list with {@code 403 Forbidden}.
 *
 * <p>The filter intercepts paths of the form:
 *
 * <ul>
 *   <li>{@code /v2/sources/{sourceId}/…} — v2 item/content/text endpoints
 *   <li>{@code /sources/{sourceID}/…} — v1 source endpoints
 *   <li>{@code /v2/cases/{id}/…} — v2 case-scoped endpoints (case ID = source ID)
 *   <li>{@code /v2/cases/{id}} — single-case GET/DELETE
 * </ul>
 *
 * <p>Runs at {@link Priorities#AUTHORIZATION}{@code + 1} so it executes after the API-key check
 * ({@link ApiKeyAuthFilter}) and after audit logging ({@link AuditLoggingFilter}), ensuring denied
 * source accesses are still audited.
 */
@Provider
...
public class SourceAccessFilter implements ContainerRequestFilter {
  ...
  /**
   * Extracts the source/case ID from a decoded URL path, or {@code null} when the path is not a
   * source-scoped endpoint.
   *
