 ...
 *
 * <p>Mirrors the v1 {@link Text} endpoint at the v2 URL shape ({@code
 * /v2/sources/{sourceId}/items/{id}/text}) and adds:
 *
 * <ul>
 *   <li>{@code ?highlight=term1+term2} — caller-supplied highlight terms are wrapped in {@code
 *       <mark>} tags (plain-text callers receive raw text, callers requesting {@code text/html}
 *       receive the marked-up version).
 *   <li>{@code ?limit=N} — truncates output to the first N bytes (useful for preview snippets).
 * </ul>
 *
 * <p>When the source or item is not found, returns {@code 404} (the v1 endpoint throws an unhandled
 * exception).
 */
@Api(value = "Items v2")
  ...
  public Response text(
    ...
    if (source == null) {
      return Response.status(Response.Status.NOT_FOUND)
          .entity("Source not found: " + sourceId)
          .build();
    }

    var item = source.getItemByID(id);
    if (item == null) {
      return Response.status(Response.Status.NOT_FOUND).entity("Item not found: " + id).build();
    }
    ...
    String[] terms =
        highlight != null && !highlight.isBlank() ? highlight.trim().split("\\s+") : new String[0];
          ...
          try {
            ...
            if (wantHtml && terms.length > 0) {
              ...
              for (String term : terms) {
                text =
                    text.replaceAll(
                        "(?i)" + java.util.regex.Pattern.quote(term), "<mark>$0</mark>");
              }
              writer.write("<pre>");
              writer.write(text);
              writer.write("</pre>");
