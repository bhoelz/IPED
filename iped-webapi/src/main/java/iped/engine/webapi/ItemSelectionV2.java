 ...
 *
 * <p>The existing v1 {@code PUT /selection/add} and {@code PUT /selection/remove} accept batch
 * arrays. These v2 endpoints operate on a single item and follow REST resource semantics (PUT =
 * set, DELETE = unset).
 *
 * <pre>
...
public class ItemSelectionV2 {
  ...
  @GET
  public Response getSelected(@PathParam("sourceId") String sourceId, @PathParam("id") int id) {
  ...
  @PUT
  public Response select(@PathParam("sourceId") String sourceId, @PathParam("id") int id) {
    IIPEDSource handle = Sources.services().sources().getSourceHandle(sourceId);
    if (handle == null) {
      return sourceNotFound(sourceId);
    }
  ...
  @DELETE
  public Response deselect(@PathParam("sourceId") String sourceId, @PathParam("id") int id) {
    IIPEDSource handle = Sources.services().sources().getSourceHandle(sourceId);
    if (handle == null) {
      return sourceNotFound(sourceId);
    }
