 ...
 *
 * <p>Tags are bookmark-backed: each tag name corresponds to a named bookmark that is auto-created
 * on first use. This mirrors the semantics exposed by {@code iped_item_tag} / {@code
 * iped_item_untag} in iped-mcp, providing a REST-ergonomic alternative to the bookmark membership
 * endpoints.
 *
 * <pre>
...
public class ItemTagsV2 {
  ...
  @GET
  public Response listTags(@PathParam("sourceId") String sourceId, @PathParam("id") int id) {
    ...
    return Response.ok(
            Map.of("sourceId", sourceId, "itemId", id, "tags", tags != null ? tags : List.of()))
        .build();
  }
  ...
  public Response addTag(
    ...
    if (tag == null || tag.isBlank()) {
      return Response.status(Response.Status.BAD_REQUEST)
          .entity(Map.of("error", "tag name must not be blank"))
          .build();
    }
    IIPEDSource handle = Sources.services().sources().getSourceHandle(sourceId);
    if (handle == null) {
  ...
  private static Response sourceNotFound(String sourceId) {
    return Response.status(Response.Status.NOT_FOUND)
        .entity(Map.of("error", "Source not found: " + sourceId))
        .build();
  }
}
