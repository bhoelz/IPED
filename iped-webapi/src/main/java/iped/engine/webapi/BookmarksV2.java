 ...
 *
 * <p>Wraps the existing {@link iped.engine.webapi.spi.BookmarkService} SPI with
 * consistent JSON shapes, proper HTTP status codes, and audit logging.
 *
 * <p>Bookmark names are URL-path-encoded; callers must percent-encode names
 * containing slashes or other reserved characters.
 *
 * <pre>
...
public class BookmarksV2 {
    ...
    public Response listItems(@PathParam("name") String name) throws Exception {
        ...
        List<Map<String, Object>> items = Sources.services().bookmarks()
                .listBookmarkDocs(name).stream()
    ...
    private static Response notFound(String name) {
        return Response.status(Response.Status.NOT_FOUND)
                .entity(Map.of("error", "Bookmark not found: " + name)).build();
    }
