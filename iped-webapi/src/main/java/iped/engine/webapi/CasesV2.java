 ...
 *
 * <p>Replaces the v1 {@code /sources} endpoint with case-scoped routes. A "case" in the v2 model is
 * a processed IPED output directory opened as a search source.
 *
 * <h2>Routes</h2>
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Opening a case is idempotent: if the same {@code id} is already open, {@code 409 Conflict} is
 * returned. The {@code id} is caller-chosen; if omitted, a UUID is generated from the path.
 */
@Api(value = "Cases v2")
@Path("v2/cases")
public class CasesV2 {
  ...
  public Response openCase(Map<String, String> body) {
    ...
    String id =
        (body.containsKey("id") && body.get("id") != null && !body.get("id").isBlank())
            ? body.get("id")
    ...
    try {
      ...
      return Response.status(Response.Status.CREATED)
          .entity(Map.of("id", id, "path", path, "status", "open"))
          .build();
    } catch (RuntimeException e) {
      if (e.getMessage() != null && e.getMessage().startsWith("duplicated id")) {
        return Response.status(Response.Status.CONFLICT)
            .entity(Map.of("error", "Case '" + id + "' is already open"))
            .build();
      }
  ...
  public Response getCase(@PathParam("id") String id) throws Exception {
    ...
    if (sd == null) {
      return Response.status(Response.Status.NOT_FOUND)
          .entity(Map.of("error", "Case not found: " + id))
          .build();
    }
  ...
  public Response closeCase(@PathParam("id") String id) {
    try {
      ...
      return Response.serverError()
          .entity(Map.of("error", "Failed to close case: " + e.getMessage()))
          .build();
    }
  }
