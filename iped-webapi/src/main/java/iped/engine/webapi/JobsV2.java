 ...
 *
 * <p>Covers export and report generation — operations too slow for a synchronous
 * HTTP response. Callers submit a job, get a {@code jobId}, poll
 * {@code GET /v2/jobs/{id}} for status, and optionally subscribe to
 * {@code GET /v2/jobs/{id}/events} (SSE) for real-time progress.
 *
 * <h2>Supported job types</h2>
 * <ul>
 *   <li>{@code export} — pack selected items into a ZIP file.
 *       Params: {@code sourceId} (string), {@code itemIds} (int[]),
 *       {@code outputPath} (string, optional — defaults to temp dir).</li>
 *   <li>{@code report} — generate an HTML/PDF report for a case.
 *       Params: {@code sourceId} (string), {@code format} ("html"|"pdf").</li>
 * </ul>
 */
@Api(value = "Jobs v2")
@Path("v2/jobs")
public class JobsV2 {
    ...
    public Response submit(Map<String, Object> body) {
        ...
        switch (type) {
            ...
            default -> {
                ...
                return Response.status(Response.Status.BAD_REQUEST)
                        .entity(Map.of("error", "Unknown type: " + type)).build();
            }
        }

        return Response.status(Response.Status.ACCEPTED)
                .entity(Map.of("jobId", job.id, "status", job.status)).build();
    }
    ...
    public Response getJob(@PathParam("id") String id) {
        return JobRegistry.get(id)
                ...
                .orElseGet(() -> Response.status(Response.Status.NOT_FOUND)
                        .entity(Map.of("error", "Job not found: " + id)).build());
    }
    ...
    public Response cancel(@PathParam("id") String id) {
        return JobRegistry.get(id).map(j -> {
            if (j.isTerminal()) {
                return Response.status(Response.Status.CONFLICT)
                        .entity(Map.of("error", "Job already in terminal state: " + j.status)).build();
            }
            j.cancel();
    ...
    private static void runExport(JobEntry job) {
        try {
            ...
            if (sourceId.isBlank()) { job.fail("'sourceId' is required"); return; }

            job.progress(5, "Resolving source");
            var source = Sources.getSource(sourceId);
            if (source == null) { job.fail("Source not found: " + sourceId); return; }

            String outputPath = (String) job.params.getOrDefault("outputPath",
