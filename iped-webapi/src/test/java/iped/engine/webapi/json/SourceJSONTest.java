...
class SourceJSONTest {
  ...
  void sourceToIDsJSON_fromDocList_groupsBySource() {
    List<DocIDJSON> docs =
        List.of(new DocIDJSON("srcA", 1), new DocIDJSON("srcA", 2), new DocIDJSON("srcB", 3));
    ...
    DocIDGroupJSON srcAGroup =
        groups.stream().filter(g -> "srcA".equals(g.getSource())).findFirst().orElse(null);
