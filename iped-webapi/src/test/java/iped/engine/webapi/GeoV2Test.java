...
class GeoV2Test {
  ...
  void buildFeatureProducesValidGeoJsonPoint() throws Exception {
    Map<String, List<String>> meta = Map.of("dcterms:created", List.of("2024-01-15T10:30:00Z"));
    String json = buildFeature("src:42:0", "src", 42, "photo.jpg", "-22.906;-43.172", meta);
  ...
  void buildFeatureIncludesAltitudeWhenPresent() throws Exception {
    Map<String, List<String>> meta = Map.of("common:altitude", List.of("890.5"));
    String json = buildFeature("src:1:0", "src", 1, "gps.jpg", "-10.0;20.0", meta);
  ...
  void buildFeatureReturnsNullForUnparseableCoordinates() throws Exception {
    String json = buildFeature("src:1:0", "src", 1, "bad.jpg", "NOT_A_NUMBER;ALSO_BAD", Map.of());
    assertNull(json, "unparseable coordinates must yield null");
  }

  @Test
  void buildFeatureReturnsNullForMissingDelimiter() throws Exception {
    String json = buildFeature("src:1:0", "src", 1, "bad.jpg", "51.5074", Map.of()); // no semicolon
    assertNull(json, "missing semicolon must yield null");
  }

  @Test
  void buildFeatureEscapesJsonSpecialCharsInName() throws Exception {
    String json = buildFeature("src:2:0", "src", 2, "file\"name\\path", "51.0;0.0", Map.of());

    assertNotNull(json);
    assertTrue(
        json.contains("file\\\"name\\\\path"),
  ...
  void geoJsonCoordinateOrderIsLonLat() throws Exception {
    ...
    assertEquals(
        -43.172, Double.parseDouble(vals[0].trim()), 1e-6, "first coord must be longitude");
    assertEquals(
        -22.906, Double.parseDouble(vals[1].trim()), 1e-6, "second coord must be latitude");
  }
}
