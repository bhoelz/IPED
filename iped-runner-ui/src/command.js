...
export function buildCommand(s) {
  const t = []
...
export function validateState(s) {
  ...
  if (s.asap && !/\.asap$/i.test(s.asap))
    issues.push({ id: 'asapExt', screen: 'entradas', level: 'error', key: 'asapExt' })
  s.passwords.forEach((p, i) => {
    if (i < s.passwords.length - 1 && !p.trim())
      issues.push({ id: 'pw-' + i, screen: 'entradas', level: 'warn', key: 'passwordEmpty' })
  })
  if (!s.output.trim())
    issues.push({ id: 'needOutput', screen: 'saida', level: 'error', key: 'needOutput' })
  s.extra.forEach((e, i) => {
    if (!e.k.trim() && e.v.trim())
      issues.push({ id: 'xk-' + i, screen: 'avancado', level: 'warn', key: 'extraKeyEmpty' })
  })
  return issues
}
