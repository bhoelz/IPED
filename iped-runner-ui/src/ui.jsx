...
export function Card({ title, flag, desc, icon, action, bar, children, style }) {
  return (
...
export function Field({ label, flag, required, error, hint, children }) {
  return (
    <div className="field">
      {(label || flag) && (
...
export function TextInput({ value, onChange, placeholder, affix, onAffix, invalid, mono = true, type = 'text' }) {
...
export function FilePicker({ fileName, onPick, onClear, buttonLabel, emptyLabel, invalid }) {
  return (
    <div className={`filepick ${invalid ? 'invalid' : ''}`}>
      ...
      <span className={`filepick-name ${fileName ? 'set' : ''}`} title={fileName || ''}>{fileName || emptyLabel}</span>
      {fileName && (
...
export function PasswordRow({ value, onChange, onRemove, placeholder }) {
...
export function Checkbox({ on, onChange }) {
  return (
...
export function FlagSwitch({ name, flag, desc, on, onChange }) {
  return (
...
export function FlagCheck({ name, flag, desc, on, onChange }) {
  return (
...
export function TagsInput({ tags, onChange, placeholder }) {
  ...
  return (
    <div className={`tags ${focus ? 'focus' : ''}`} onClick={() => inputRef.current?.focus()}>
      {tags.map(t => (
      ...
      <input ref={inputRef} className="tags-input" value={draft} placeholder={tags.length ? '' : placeholder}
        ...
        onBlur={() => { commit(); setFocus(false) }} onFocus={() => setFocus(true)} />
    </div>
  )
}

export function NumberPresets({ value, onChange, presets, placeholder }) {
  return (
...
export function KeyValueGrid({ rows, onChange, headK, headV, phK, phV }) {
      ...
      {display.map((r, i) => (
        <div className="kv-row" key={i}>
          <div className="kv-cell">
            <input className="kvinp k" value={r.k} placeholder={phK}
              onChange={e => r._new ? onChange([...rows, { k: e.target.value, v: '' }]) : update(i, 'k', e.target.value)} />
          </div>
          <div className="kv-cell">
            <input className="kvinp" value={r.v} placeholder={phV}
              onChange={e => r._new ? onChange([...rows, { k: '', v: e.target.value }]) : update(i, 'v', e.target.value)} />
          </div>
          <div className="kv-cell">
