...
/**
 * Runs a single IPED processing task standalone against an isolated file or directory, without a
 * case, Lucene index or SQLite database -- see {@link ConfigBootstrap}, {@link
 * StandaloneItemFactory}, {@link TaskExecutor} for how each of those requirements is avoided
 * without modifying any task.
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>Exit code: 0 if every item succeeded, 2 if any item was unsupported, 1 if any item errored (or
...
public class StandaloneTaskCli {
  ...
  @Parameter(
      names = "--task",
      description = "Fully qualified class name of the AbstractTask to run")
  ...
  @Parameter(
      names = "--conf",
      description =
          "IPED release directory (contains conf/, scripts/, tools/, profiles/ as siblings)",
      required = true,
      converter = FileConverter.class)
  ...
  @Parameter(
      names = "--list-tasks",
      description = "List installable tasks and their standalone-compatibility status")
  private boolean listTasks = false;

  @Parameter(
      names = "--extract-children-to",
      description =
          "Carving tasks only: write each carved child as a flat file under this directory "
  ...
  /**
   * Executes the public command-line contract without terminating the JVM. Kept package-private so
   * integration tests and embedders can verify the same argument parsing and exit-code policy that
   * {@link #main(String[])} exposes to shell users.
   */
  ...
  private int run() {
    try {
      ...
      ResultWriter writer =
          "json".equalsIgnoreCase(outputFormat) ? new JsonResultWriter() : new StdoutResultWriter();
      if (outputFile != null) {
