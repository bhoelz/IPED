...
/**
 * Static, module-local classification of which {@code AbstractTask} subclasses can run standalone
 * (no case, no Lucene index, no SQLite, no child-item queue). Built by reading each task's {@code
 * process()}/{@code init()} -- none of the task classes themselves were touched. This is a
 * pre-flight gate only: {@code TaskExecutor} still wraps execution in a runtime guard for tasks not
 * listed here (see {@link Status#UNCLEAR}).
 */
public final class TaskCompatibility {
  ...
  public enum Status {
    ...
    KNOWN_UNSUPPORTED,
    /**
     * Not reviewed (or genuinely conditional, e.g. delegates to user scripts); left to the runtime
     * guard.
     */
    UNCLEAR
  }
  ...
  private static final Map<String, String> DEPENDS_ON_CASE =
      Map.ofEntries(
          Map.entry(
              "iped.engine.task.index.IndexTask", "writes to worker.writer/manager.getIndexTemp()"),
          Map.entry(
              "iped.engine.task.ExportFileTask", "opens SQLite storage connections, uses caseData"),
          Map.entry(
              "iped.engine.task.DuplicateTask",
              "reads/writes caseData, uses worker.getIndexingPort()"),
          Map.entry(
              "iped.engine.task.SkipCommitedTask",
              "uses worker.getIndexingPort(), reads/writes caseData"),
          Map.entry(
              "iped.engine.task.IgnoreHardLinkTask",
              "requires SleuthkitInputStreamFactory/FsContent (disk-image state)"),
          Map.entry(
              "iped.engine.task.EmbeddedDiskProcessTask",
              "uses Manager.getInstance() and the item queue"),
          Map.entry(
              "iped.engine.task.TempFileTask",
              "reads caseData.getCaseObject(CmdLineArgs)/isIpedReport()"),
          Map.entry("iped.engine.task.MinIOTask", "reads caseData.getCaseObject(CmdLineArgs)"),
          Map.entry(
              "iped.engine.task.index.ElasticSearchIndexTask",
              "reads caseData.getCaseObject(CmdLineArgs)/isIpedReport()"),
          Map.entry(
              "iped.engine.graph.GraphTask", "uses caseData.getCaseObject()/IPEDSource/IPEDReader"),
          Map.entry("iped.engine.task.ExportCSVTask", "reads caseData.getCaseObject(CmdLineArgs)"),
          Map.entry(
              "iped.engine.task.HTMLReportTask",
              "opens IPEDSource with worker.writer, accumulates into caseData"));
  ...
  private static final Map<String, String> CREATES_CHILDREN =
      Map.ofEntries(
          Map.entry(
              "iped.engine.task.video.VideoThumbTask",
              "creates frame subitems; also extends ThumbTask"),
          Map.entry(
              "iped.engine.task.ParsingTask",
              "container expansion calls worker.processNewItem(); also extends ThumbTask"));

  /**
   * BaseCarveTask and its subclasses: create child items via {@code worker.processNewItem()}, same
   * as {@link #CREATES_CHILDREN}, but {@code TaskExecutor} recognizes this specific family and,
   * when {@code --extract-children-to <dir>} is given, runs them with {@link CarvingOutputWorker}
   * instead of the plain {@code AdditionalTaskWorker} -- redirecting each carved child to a flat
   * output directory (one level of extraction, no recursive carving/parsing/hashing of the child --
   * see {@link CarvingOutputWorker}'s javadoc). Without that flag, these still fail per-item as
   * UNSUPPORTED via the normal runtime guard, exactly like {@link #CREATES_CHILDREN} tasks.
   */
  public static final Set<String> CARVING_FAMILY =
      Set.of(
  ...
  private static final Set<String> COMPATIBLE_SEED =
      Set.of(
  ...
  private static final Map<String, String> COMPATIBLE_WITH_NOTE =
      Map.ofEntries(
          Map.entry(
          ...
          Map.entry(
              "iped.engine.task.RemoteImageClassifierTask",
  ...
  private static final String CARVING_NOTE =
      "creates child items -- pass --extract-children-to <dir> to capture "
  ...
  public static Classification classify(String taskClassName) {
    ...
    if (childrenReason != null) {
      return new Classification(
          Status.KNOWN_UNSUPPORTED, "creates child items (" + childrenReason + ")");
    }
    String caseReason = DEPENDS_ON_CASE.get(taskClassName);
    if (caseReason != null) {
      return new Classification(
          Status.KNOWN_UNSUPPORTED, "depends on case/index (" + caseReason + ")");
    }
    if (COMPATIBLE_SEED.contains(taskClassName)) {
      return new Classification(
          Status.COMPATIBLE, "reads/writes only its own item's metadata/attributes/content");
    }
