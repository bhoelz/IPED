...
public class StdoutResultWriter implements ResultWriter {
    ...
    public void write(RunResult result, PrintStream out) {
        ...
        for (ItemResult item : result.items()) {
            out.printf("%s | %s%s%n", item.path(), item.status(),
        ...
        long unsupported = result.items().stream().filter(i -> ItemResult.UNSUPPORTED.equals(i.status())).count();
        long errors = result.items().stream().filter(i -> ItemResult.ERROR.equals(i.status())).count();
        out.println();
        out.printf("summary: %d ok, %d unsupported, %d errors (of %d)%n", ok, unsupported, errors, result.items().size());
    }
}
