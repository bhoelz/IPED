...
/**
 * Runs a single {@code AbstractTask} standalone against a list of items, mirroring the wiring
 * {@code iped.engine.task.additional.AdditionalTaskRunner} uses for post-indexing task re-runs
 * (minimal {@link AdditionalTaskWorker}, reflective instantiation, {@code setWorker}/{@code
 * setNextTask}/{@code init}/ {@code processAndSendToNextTask}/{@code finish}) -- adapted to build
 * items from disk (see {@link StandaloneItemFactory}) instead of an {@code IPEDSource}.
 */
public final class TaskExecutor {
  ...
  /**
   * @param childrenDir when non-null and {@code taskClassName} is one of {@link
   *     TaskCompatibility#CARVING_FAMILY}, carved children are written here (one level of
   *     extraction, no recursive re-processing -- see {@link CarvingOutputWorker}) instead of
   *     failing as UNSUPPORTED.
   */
  public static RunResult run(
      String taskClassName, List<IItem> items, File confDir, File childrenDir) {
    return run(
        taskClassName, items, confDir, childrenDir, TaskCompatibility.classify(taskClassName));
  }

  /**
   * Package-private seam for callers that already determined compatibility. The public entry point
   * always obtains that classification from {@link TaskCompatibility}; keeping it explicit here
   * also ensures the per-item failure policy uses the same classification as the pre-flight gate.
   */
  static RunResult run(
    ...
    if (classification.status() == TaskCompatibility.Status.KNOWN_UNSUPPORTED) {
      return new RunResult(
          taskClassName, describeInput(items), unsupportedForAll(items, classification.reason()));
    }
    ...
    try {
      ...
      return new RunResult(
          taskClassName,
          describeInput(items),
          errorForAll(items, "could not instantiate task: " + e));
    }

    CollectingSinkTask sink = new CollectingSinkTask();
    AdditionalTaskWorker worker =
        (childrenDir != null && TaskCompatibility.CARVING_FAMILY.contains(taskClassName))
    ...
    for (IItem item : items) {
      ...
      try {
        ...
        results.add(
            ItemResult.ok(
                item.getPath(), elapsed, sink.takeMetadata(), sink.takeExtraAttributes()));
      } catch (Throwable t) {
        long elapsed = System.currentTimeMillis() - start;
        Failure f = classifyFailure(t, classification);
  ...
  private static Failure classifyFailure(
      Throwable t, TaskCompatibility.Classification classification) {
    Throwable root = t;
    ...
    if (root instanceof NullPointerException
        && classification.status() != TaskCompatibility.Status.COMPATIBLE) {
      return new Failure(
