...
> **Status note (2026-06-21):** despite the "✅ COMPLETO" markers throughout this and the
> sibling FASE2–7 documents, none of the classes described below
> (`ComponentProvider`, `ComponentRegistry`, `ComponentTaskLoader`, `CarverProvider`,
> `ParserProvider`, `DatabaseProvider`, `EventDispatcher`, `MetadataSchemaProvider`, etc.)
> exist anywhere in the codebase as of this date. This is a design proposal for a generic,
> cross-component plugin SPI (carvers, parsers, databases, webhooks, metadata, i18n) that
> was apparently drafted but never implemented — it is **separate from and broader than**
> the actual, shipped task-only SPI in this module (`TaskProvider`/`TaskDescriptor`, see
> `PLUGINS_IMPLEMENTATION_PLAN.md` and `PLUGIN_TEMPLATE.md` in this same directory, which
> *is* real and complete per [iped-tasks-ROADMAP.md](../../docs/roadmaps/iped-tasks-ROADMAP.md)
> Phase 2). See [COMPONENT-PLUGIN-SYSTEM-STATUS.md](COMPONENT-PLUGIN-SYSTEM-STATUS.md) for
> the corrected, code-verified status of this whole proposal series.
