...
/**
 * Enforces that iped-tasks.spi depends only on iped-api — it must never pull in engine internals,
 * task implementations, parsers, or the app layer.
 */
class TaskSpiArchTest {

  private static final JavaClasses SPI_CLASSES =
  ...
  void spiMustNotImportEngineInternals() {
    ArchRule rule =
        noClasses()
            ...
            .because(
                "the task SPI is a public extension point — it must depend only on "
