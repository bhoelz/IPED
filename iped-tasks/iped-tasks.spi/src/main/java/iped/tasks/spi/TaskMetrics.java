...
/**
 * Optional interface that task implementations may also implement to surface
 * per-instance runtime metrics to the observability stack.
 *
 * <p>The engine discovers this interface via {@code instanceof} checks on each
 * object returned by {@link TaskProvider#createTask()} and collects snapshots
 * at a configurable interval. All values are cumulative since {@code init()}.
 *
 * <p>Implementations must be thread-safe: the engine may read these counters
 * from a monitoring thread while {@code process()} runs on the worker thread.
 * Use {@code AtomicLong} or equivalent for the backing fields.
 *
 * <h3>Example</h3>
 * <pre>{@code
...
public interface TaskMetrics {
    ...
    /**
     * Cumulative wall-clock milliseconds spent inside {@code process()} for this
     * task instance since {@code init()}, including items that later failed.
     */
    long getProcessingMillis();

    /**
     * Human-readable task name, typically matching
     * {@link TaskDescriptor#displayName()}.
     */
    String getTaskName();
