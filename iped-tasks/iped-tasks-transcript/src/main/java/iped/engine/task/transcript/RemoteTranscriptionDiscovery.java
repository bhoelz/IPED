...
public class RemoteTranscriptionDiscovery {
  ...
  private static final File statsFile =
      new File(System.getProperty("user.home"), "transcription.stats");
  ...
  private static void register(Socket client, BufferedReader reader, PrintWriter writer)
      throws IOException {
  ...
  private static void getStats(Socket client, BufferedReader reader, PrintWriter writer)
      throws IOException {
    String ip = client.getInetAddress().getHostAddress();
    String port = reader.readLine();
    String address = ip + ":" + port;
      ...
      public void run() {
        ...
        while (true) {
          ...
          if (seconds == 30) {
            ...
            System.out.println(
                "Online Time: " + df.format((System.currentTimeMillis() - startTime) / 1000) + "s");
            System.out.println("Transcribed Audios: " + df.format(audiosTranscripted));
            System.out.println(
                "Transcribed Audios Duration: " + df.format(audiosDuration.get() / 1000) + "s");
            System.out.println(
                "Transcription Time (cpu): " + df.format(transcriptionTimeCpu.get() / 1000) + "s");
            System.out.println(
                "Transcription Time (real): "
                    + df.format(transcriptionTimeReal.get() / 1000)
                    + "s");
            System.out.println(
                "Wav Conversion Time (cpu): " + df.format(conversionTimeCpu.get() / 1000) + "s");
            System.out.println(
                "Wav Conversion Time (real): " + df.format(conversionTimeReal.get() / 1000) + "s");
          ...
          while (it.hasNext()) {
            ...
            if (System.currentTimeMillis() - servers.get(server) >= PING_TIMEOUT * 1000) {
              System.out.println(
                  new Date()
                      + " No PING received for the last "
                      + PING_TIMEOUT
                      + "s, removing server from list: "
                      + server);
              it.remove();
