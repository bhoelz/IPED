...
public class Wav2Vec2TranscriptTask extends AbstractTranscriptTask {
  ...
  protected Server startServer0(int device) throws IOException {
    ...
    if (model == null) {
      throw new StartupException(
          "You must configure '"
              + AudioTranscriptConfig.HUGGING_FACE_MODEL
    ...
    if (!HUGGINGSOUND_LOADED.equals(line)) {
      throw new StartupException(
          "'huggingsound' python lib not loaded correctly. Have you installed it?");
    }
  ...
  protected boolean ping(Server server) {
    try {
      ...
      log.warn(
          "Fail to ping transcription process pid={} exception={}",
          server.process.pid(),
          e.toString());
    }
    return false;
