 ...
 *
 * <p>TODO: This and @ExportFileTask should extend a common abstract class, and the implementation
 * should be chosen depending on configuration.
 *
 * @author Nassif
 */
@Slf4j
public class MinIOTask extends AbstractTask {
  ...
  public void init(ConfigurationManager configurationManager) throws Exception {
    ...
    if (!credentialsLoaded.getAndSet(true)) {
      ...
      if (paramBucket != null) {
        log.error(
            "Passing the bucket as a parameter may prevent removing the evidence from the case.");
      }
    }
  ...
  private static void parseFields(String cmdFields) {
    ...
    for (String entry : entries) {
      ...
      if (ACCESS_KEY.equals(pair[0])) accessKey = pair[1];
      else if (SECRET_KEY.equals(pair[0])) secretKey = pair[1];
      else if (BUCKET_KEY.equals(pair[0])) paramBucket = pair[1];
    }
  }
  ...
  private static void loadCredentials(ICaseData caseData) {
    ...
    if (accessKey == null || secretKey == null) {
      throw new RuntimeException(
          "'MinioCredentials' not set by ENV var, sys prop or cmd line param.");
    }
  }
  ...
  private String insertWithZip(
      IItem i, String hash, SeekableInputStream is, String mediatype, boolean preview)
      throws Exception {
      ...
      try (SeekableFileInputStream fi = new SeekableFileInputStream(zp.zipfile)) {
        ...
        sendFile(
            PutObjectArgs.builder()
                .bucket(bucket)
                .object("/zips/" + getZipName(zp))
                .userMetadata(Collections.singletonMap("x-minio-extrac", "true")),
            fi,
            zp.zipfile.length(),
            Math.max(zp.zipfile.length(), 1024 * 1024 * 5));
      }
    }
  ...
  private void sendFile(Builder builder, SeekableInputStream is, long size, long partSize)
      throws Exception {
    Exception ex = null;
  ...
  private void insertItem(
      String hash, SeekableInputStream is, String mediatype, String bucket, String bucketPath)
      throws Exception {

    try {
      sendFile(
          PutObjectArgs.builder().bucket(bucket).object(bucketPath).contentType(mediatype),
          is,
          -1,
          10 << 20);
  ...
  protected void process(IItem item) throws Exception {
    ...
    if (caseData.isIpedReport() || !item.isToAddToCase()) return;

    String hash = item.getHash();
    if (hash == null || hash.isEmpty() || item.getLength() == null) return;
      ...
      if (is != null) {
        ...
        if (fullPath != null) {
          ((Metadata) item.getMetadata())
              .add(
                  ElasticSearchIndexTask.PREVIEW_IN_DATASOURCE,
  ...
  public static class MinIOInputInputStreamFactory extends SeekableInputStreamFactory {
    ...
    public SeekableInputStream getSeekableInputStream(String identifier) throws IOException {
      ...
      if (minioClient == null) {
        ...
        minioClient =
            MinioClient.builder().endpoint(server).credentials(accessKey, secretKey).build();
  ...
  public static class MinIOSeekableInputStream extends SeekableInputStream {
    ...
    public long size() throws IOException {
      if (size == null) {
        if (id.contains(".zip/")) {
          try (GetObjectResponse res =
              minioClient.getObject(
                  GetObjectArgs.builder()
                      .bucket(bucket)
                      .object(id)
                      .offset(0L)
                      .extraHeaders(Collections.singletonMap("x-minio-extract", "true"))
                      .build())) {
