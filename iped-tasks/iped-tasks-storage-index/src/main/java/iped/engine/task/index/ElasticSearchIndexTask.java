...
public class ElasticSearchIndexTask extends AbstractTask {
  ...
  private static List<ElasticSearchIndexTask> taskInstances =
  ...
  public void init(ConfigurationManager configurationManager) throws Exception {
    ...
    RestClientBuilder clientBuilder =
        RestClient.builder(
                new HttpHost(
                    elasticConfig.getHost(), elasticConfig.getPort(), elasticConfig.getProtocol()))
                ...
                new RestClientBuilder.RequestConfigCallback() {
                  ...
                  public RequestConfig.Builder customizeRequestConfig(
                    ...
                    return requestConfigBuilder
                        .setConnectTimeout(elasticConfig.getConnect_timeout())
          ...
          new HttpClientConfigCallback() {
            ...
            public HttpAsyncClientBuilder customizeHttpClient(
              ...
              if (!elasticConfig.getValidateSSL()) {
                custombuilder
                    .setSSLContext(SSLFix.getUnsecureSSLContext())
  ...
  private void parseCmdLineFields(String cmdFields) {
    if (cmdFields == null) return;
    String[] entries = cmdFields.split(";");
    for (String entry : entries) {
      String[] pair = entry.split(":", 2);
      if (USER_KEY.equals(pair[0])) user = pair[1];
      else if (PASSWORD_KEY.equals(pair[0])) password = pair[1];
      else if (INDEX_NAME_KEY.equals(pair[0])) {
        indexName = pair[1];
      } else cmdLineFields.put(pair[0], pair[1]);
    }
  }
  ...
  private void createIndex(String indexName) throws IOException {
    ...
    Builder builder =
        Settings.builder()
            .put(MAX_FIELDS_KEY, elasticConfig.getMax_fields())
    ...
    if (elasticConfig.isUseCustomAnalyzer()) {
      builder
          .put("analysis.tokenizer.latinExtB.type", "simple_pattern") // $NON-NLS-1$ //$NON-NLS-2$
          .put(
              "analysis.tokenizer.latinExtB.pattern",
              getLatinExtendedBPattern()) //$NON-NLS-1$ //$NON-NLS-2$
          ...
          .putList(
              ...
              "lowercase",
              "asciifolding"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    }
  ...
  private String getLatinExtendedBPattern() {
    ...
    for (int i = 0; i <= 0x24F; i++) {
      if (Character.isLetterOrDigit(i)) sb.append(new String(Character.toChars(i)));
    }
  ...
  private void createFieldMappings(String indexName) throws IOException {
    ...
    properties.put(
        ...
        Collections.singletonMap("type", "keyword")); // $NON-NLS-1$ //$NON-NLS-2$
    ...
    properties.put(
        "extraAttributes." + ImageSimilarityTask.IMAGE_FEATURES,
        Map.of(
            "type",
            "knn_vector",
            "dimension",
            ImageSimilarity.numFeatures,
            "method",
  ...
  public static void commit() throws IOException, InterruptedException {
    ...
    UIPropertyListenerProvider.getInstance()
        .firePropertyChange("mensagem", "", "Commiting to ElasticSearch...");
    for (ElasticSearchIndexTask instance : taskInstances) {
      log.info(
          "Commiting Worker-"
              + instance.worker.id
              + " ElasticSearchTask..."); //$NON-NLS-1$ //$NON-NLS-2$
  ...
  private IndexRequest createIndexRequest(String id, String route, XContentBuilder jsonData)
      throws IOException {
  ...
  protected synchronized void process(IItem item) throws Exception {
    ...
    if (textReader == null) {
      log.warn(
          "Null text reader: "
              + item.getPath()
              + " ("
              + (item.getLength() != null ? item.getLength() : "null")
              + " bytes)");
    ...
    FragmentingReader fragReader =
        new FragmentingReader(
            textReader, indexConfig.getTextSplitSize(), indexConfig.getTextOverlapSize());
    ...
    try {
      ...
      do {
        ...
        XContentBuilder jsonContent =
            getJsonFragmentBuilder(item, fragReader, parentId, contenttrackID, fragNum--);
  ...
  private class BulkResponseListener implements ActionListener<BulkResponse> {
    ...
    private BulkResponseListener(
        HashMap<String, String> itemMap, BulkRequest bulkRequest, int retries) {
  ...
  private XContentBuilder getJsonMetadataBuilder(IItem item) throws IOException {
    ...
    builder
        ...
        .field("document_content", "document")
        ...
        .field(BasicProps.TYPE, item.getType())
        ...
        .field(BasicProps.CONTENTTYPE, item.getMediaTypeString())
    ...
    if (extraAttributes.containsKey(ExtraProperties.FACE_ENCODINGS)
        && extraAttributes.containsKey(ExtraProperties.FACE_LOCATIONS)) {
      List<KnnVector> face_encodings =
          (List<KnnVector>) extraAttributes.get(ExtraProperties.FACE_ENCODINGS);
      List<List<Long>> face_locations =
          (List<List<Long>>) extraAttributes.get(ExtraProperties.FACE_LOCATIONS);
    ...
    builder
        .field(IndexItem.ID_IN_SOURCE, idInSource)
        .field(IndexItem.SOURCE_PATH, sourcePath)
    ...
    for (String key : getMetadataKeys(item)) {
      if (PREVIEW_IN_DATASOURCE.equals(key)) {
        ...
        for (String preview : item.getMetadataValues(key)) {
        ...
        String[] values = item.getMetadataValues(key);
  ...
  private XContentBuilder getJsonFragmentBuilder(
      IItem item, Reader textReader, String parentID, String contenttrackID, int fragNum)
      throws IOException {
    ...
    builder
        ...
        .field(BasicProps.EVIDENCE_UUID, item.getDataSource().getUUID())
        .field(BasicProps.ID, item.getId())
        .field("document_content", document_content)
        .field("contenttrackID", contenttrackID)
        .field(IndexTask.FRAG_NUM, fragNum)
