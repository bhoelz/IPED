...
/**
 * Tests for ExportCSVTask that are exercisable without the full AbstractTask
 * lifecycle (no ConfigurationManager, CaseData, or output directory needed).
 *
 * The full CSV-writing logic is covered by integration tests that run the
 * complete IPED pipeline. Here we focus on the task contract methods and
 * constants that can be verified in isolation.
 */
class ExportCSVTaskTest {
    ...
    void processIgnoredItem_returnsTrue() {
        ...
        assertTrue(task.processIgnoredItem(),
    ...
    void getConfigurables_firstEntry_isEnableTaskProperty() {
        ...
        assertTrue(configurable instanceof iped.engine.config.EnableTaskProperty,
