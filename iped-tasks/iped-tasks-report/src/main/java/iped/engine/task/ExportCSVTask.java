...
public class ExportCSVTask extends AbstractTask {
  ...
  private static final String HEADER =
      Messages.getString("ExportCSVTask.CsvColNames"); // $NON-NLS-1$
  private static final String SEPARATOR =
      Messages.getString("ExportCSVTask.CsvSeparator"); // $NON-NLS-1$
  private static final String LINK_FUNCTION =
      Messages.getString("ExportCSVTask.LinkFunction"); // $NON-NLS-1$
  private static final String LINK_NAME =
      Messages.getString("ExportCSVTask.LinkName"); // $NON-NLS-1$
  ...
  /**
   * Indica que itens ignorados, como duplicados ou conhecidos (hash), devem ser listados no arquivo
   * CSV.
   *
   * @return true
   */
  ...
  protected void process(IItem evidence) throws IOException {
    ...
    value =
        evidence
            .getCategories()
            .replace("" + CategoryTokenizer.SEPARATOR, " | "); // $NON-NLS-1$ //$NON-NLS-2$
    if (value == null) {
      value = ""; // $NON-NLS-1$
    }
    list.append("\"" + escape(value) + "\""); // $NON-NLS-1$ //$NON-NLS-2$
  ...
  public void finish() throws IOException {
    if (exportFileProps && staticList != null) {
      ...
      if (!args.isContinue() && !args.isRestart()) return;
      ...
      try (BufferedWriter writer =
              Files.newBufferedWriter(tmp.toPath(), StandardOpenOption.CREATE);
  ...
  public void init(ConfigurationManager configurationManager) throws Exception {
    ...
    if (csvFile.exists() && commitFile.exists()) {
      ...
      int idx =
          args.isRestart()
              ? 0
              : 8; // position 0-7 keeps first commit point, position 8-15 keeps last commit point
      long size = bytesToLong(bytes, idx);
      try (FileChannel fc =
          FileChannel.open(csvFile.toPath(), StandardOpenOption.WRITE, StandardOpenOption.SYNC)) {
        fc.truncate(size);
      }
      if (args.isAppendIndex() && !args.isContinue() && !args.isRestart()) {
