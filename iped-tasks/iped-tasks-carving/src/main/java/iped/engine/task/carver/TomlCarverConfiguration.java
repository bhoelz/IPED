...
/**
 * Loads carver definitions from {@code CarverConfig.toml}, the TOML successor to the legacy {@code
 * CarverConfig.xml}. Extends {@link XMLCarverConfiguration} so XML user-override files can still be
 * applied on top after TOML is loaded.
 *
 * <p>When a {@code [[carverType]]} entry carries no inline {@code headers} or {@code footers}, the
 * named {@code carverClass} is instantiated and its own {@link Carver#getCarverTypes()} result is
 * used, exactly as the XML reader does.
 */
public class TomlCarverConfiguration extends XMLCarverConfiguration {
      ...
      for (JsonNode ctNode : carverTypesNode) {
        try {
          ...
          throw new IOException(
              "Error processing carverType '"
                  + (nameNode != null ? nameNode.asText() : "<unnamed>")
                  + "'",
              e);
        }
      }
  ...
  private void processCarverTypeNode(JsonNode ctNode) throws Exception {
    ...
    boolean hasSigDefs =
        hasItems(headersNode)
            || hasItems(footersNode)
            || hasItems(escapeFooters)
            || hasItems(lengthRefs)
            || hasItems(controls);
    ...
    if (hasSigDefs) {
      ...
      if (escapeFooters != null) {
        for (JsonNode sig : escapeFooters)
          ct.addSignature(sig.asText(), SignatureType.ESCAPEFOOTER);
      }
      if (lengthRefs != null) {
