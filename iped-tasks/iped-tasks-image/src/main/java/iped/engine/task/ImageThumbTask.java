...
public class ImageThumbTask extends ThumbTask {
  ...
  private ImageThumbAccumulator accum() {
    ...
    ImageThumbAccumulator a =
        (ImageThumbAccumulator) caseData.getCaseObject(ImageThumbAccumulator.KEY);
    if (a == null) {
      synchronized (ImageThumbTask.class) {
  ...
  public void init(ConfigurationManager configurationManager) throws Exception {
    ...
    synchronized (extConvPropInit) {
      if (!extConvPropInit.get()) {
        System.setProperty(
            ExternalImageConverter.enabledProp,
            String.valueOf(imgThumbConfig.isEnableExternalConv()));
        System.setProperty(
            ExternalImageConverter.useGMProp, String.valueOf(imgThumbConfig.isUseGraphicsMagick()));
        System.setProperty(
            ExternalImageConverter.lowDensityProp,
            String.valueOf(imgThumbConfig.getLowResDensity()));
        System.setProperty(
            ExternalImageConverter.highDensityProp,
            String.valueOf(imgThumbConfig.getHighResDensity()));
        System.setProperty(
            ExternalImageConverter.magickAreaLimitProp,
            String.valueOf(imgThumbConfig.getMaxMPixelsInMemory()));
        System.setProperty(
            ExternalImageConverter.minTimeoutProp, String.valueOf(imgThumbConfig.getMinTimeout()));
        System.setProperty(
            ExternalImageConverter.timeoutPerMBProp,
            String.valueOf(imgThumbConfig.getTimeoutPerMB()));

        if (System.getProperty("os.name")
            .toLowerCase()
            .startsWith("windows")) { // $NON-NLS-1$ //$NON-NLS-2$
          System.setProperty(
              ExternalImageConverter.winToolPathPrefixProp, Configuration.getInstance().appRoot);
        }

        File tmpDir =
            new File(System.getProperty("java.io.tmpdir"), "ext-conv"); // $NON-NLS-1$ //$NON-NLS-2$
        tmpDir.mkdirs();
            ...
            while (true) {
              ...
              if (subFiles != null) {
                for (File tmp : subFiles) {
                  if (tmp.lastModified() < t) tmp.delete();
                }
              }
              try {
                Thread.sleep(2000);
    ...
    if (hasThumb(evidence, thumbFile)) {
      File viewFile =
          Util.getFileFromHash(
              new File(output, PreviewConstants.VIEW_FOLDER_NAME), evidence.getHash(), PREVIEW_EXT);
      if (viewFile.exists()) {
  ...
  private void createImageThumb(IItem evidence, File thumbFile) {
    try {
      ...
      if (img == null) {
        ...
        img =
            ImageUtil.getSubSampledImage(
                evidence,
                getThumbSize() * samplingRatio,
                renderException,
      ...
      if (img == null) {
        ...
        if (mime != null && mimesToCreateView.contains(mime)) {
          try (BufferedInputStream stream = evidence.getBufferedInputStream()) {
            img =
                externalImageConverter.getImage(
                    stream, maxViewImageSize, true, evidence.getLength(), true);
          } catch (TimeoutException e) {
            if (stats != null) {
              stats.incTimeouts();
            }
        ...
        if (img == null) {
          ...
          try (BufferedInputStream stream = evidence.getBufferedInputStream()) {
            img =
                externalImageConverter.getImage(
                    stream, getThumbSize(), false, evidence.getLength(), true);
          } catch (TimeoutException e) {
            if (stats != null) {
              stats.incTimeouts();
            }
