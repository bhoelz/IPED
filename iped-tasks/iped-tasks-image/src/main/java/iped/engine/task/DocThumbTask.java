...
public class DocThumbTask extends ThumbTask {
        ...
        if (docThumbsConfig.isEnabled()) {
          ...
          log.info(
              "LibreOffice Conversion: "
                  + (docThumbsConfig.isLoEnabled() ? "enabled" : "disabled"));
          ...
          if (docThumbsConfig.isPdfEnabled()) {
            String conversionMode =
                docThumbsConfig.isExternalPdfConversion() ? "external" : "internal";
            ParsingTaskConfig parsingConfig =
                configurationManager.findObject(ParsingTaskConfig.class);
            if (parsingConfig.isEnableExternalParsing()) {
              ...
              System.setProperty(
                  PDFTextParser.THUMB_SIZE, Integer.toString(docThumbsConfig.getThumbSize()));
    ...
    synchronized (finished) {
      if (!finished.get()) {
        ...
        if (isEnabled()) {
          ...
          if (totalPdf != 0) {
            log.info(
                "Average PDF processing time (ms/item): " + (totalPdfTime.longValue() / totalPdf));
          }
          ...
          if (totalLO != 0) {
            log.info(
                "Average LibreOffice processing time (ms/item): "
  ...
  protected void process(IItem item) throws Exception {
    if (!isEnabled()
        || !item.isToAddToCase()
        || ((externalParsingEnabled
                || !docThumbsConfig.isPdfEnabled()
                || !isPdfType((MediaType) item.getMediaType()))
            && (!docThumbsConfig.isLoEnabled()
                || !isLibreOfficeType((MediaType) item.getMediaType())))
      ...
      try {
        ...
        if (docThumbsConfig.isExternalPdfConversion()) {
          ...
          String[] cmd = {
            ...
            "-cp",
            classpath,
            "-Xmx" + docThumbsConfig.getMaxPdfExternalMemory() + "M",
            PDFToThumb.class.getCanonicalName(),
