...
class QRCodeTaskTest {
  ...
  void qrDecode_pipeline_decodesGeneratedQRCode() throws Exception {
    ...
    BitMatrix matrix =
        writer.encode(text, BarcodeFormat.QR_CODE, 200, 200, Map.of(EncodeHintType.MARGIN, 2));
  ...
  void qrDecode_pipeline_urlContent_decodesCorrectly() throws Exception {
    ...
    QRCodeWriter writer = new QRCodeWriter();
    BitMatrix matrix =
        writer.encode(url, BarcodeFormat.QR_CODE, 300, 300, Map.of(EncodeHintType.MARGIN, 2));
    BufferedImage img = MatrixToImageWriter.toBufferedImage(matrix);
  ...
  void qrDecode_resultPoints_arePresentAndNonNull() throws Exception {
    QRCodeWriter writer = new QRCodeWriter();
    BitMatrix matrix =
        writer.encode("test", BarcodeFormat.QR_CODE, 200, 200, Map.of(EncodeHintType.MARGIN, 2));
    BufferedImage img = MatrixToImageWriter.toBufferedImage(matrix);
