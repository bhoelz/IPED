...
public class PhotoDNALookupConfig extends AbstractTaskPropertiesConfig {
  ...
  public void processProperties(UTF8Properties properties) {
    ...
    if (value != null && !value.trim().isEmpty()) statusHashDBFilter = value.trim();

    value = properties.getProperty(MAX_SIMILARITY_DISTANCE);
    if (value != null && !value.trim().isEmpty()) maxDistance = Integer.valueOf(value.trim());

    value = properties.getProperty(TEST_ROTATED_FLIPPED);
    if (value != null && !value.trim().isEmpty()) rotateAndFlip = Boolean.valueOf(value.trim());

    value = properties.getProperty(MIN_ENTROPY);
    if (value != null && !value.trim().isEmpty()) minEntropy = Double.parseDouble(value.trim());
  }
}
