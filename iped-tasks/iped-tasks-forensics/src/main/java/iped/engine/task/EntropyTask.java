...
public class EntropyTask extends AbstractTask {
  ...
  protected void process(IItem evidence) throws Exception {
    ...
    if (evidence.getMediaType().equals(BaseCarveTask.UNALLOCATED_MIMETYPE)
        || Boolean.TRUE.equals(evidence.getExtraAttribute(ThumbConstants.HAS_THUMB))) return;

    try (RandomFilterInputStream rfis =
        new RandomFilterInputStream(evidence.getBufferedInputStream())) {

      while (rfis.read(buf) != -1)
        ;
      Double compression = rfis.getCompressRatio();
      if (compression != null) evidence.setExtraAttribute(COMPRESS_RATIO, compression);
