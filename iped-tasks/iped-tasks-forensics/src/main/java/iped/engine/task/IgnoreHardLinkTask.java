...
public class IgnoreHardLinkTask extends AbstractTask {
  ...
  private static Map<Long, Map<HardLink, String>> fileSystemOrigMap =
      new HashMap<Long, Map<HardLink, String>>();
  private static Map<Long, Map<HardLink, String>> fileSystemSlackMap =
      new HashMap<Long, Map<HardLink, String>>();
  ...
  protected void process(IItem evidence) throws Exception {
    ...
    if (caseData.isIpedReport()) return;
    ...
    SleuthkitInputStreamFactory factory =
        (SleuthkitInputStreamFactory) evidence.getInputStreamFactory();
    ...
    if (content != null && content instanceof FsContent) {
      ...
      if (!fsType.equals(TSK_FS_TYPE_ENUM.TSK_FS_TYPE_HFS)
          && !fsType.equals(TSK_FS_TYPE_ENUM.TSK_FS_TYPE_HFS_DETECT)) return;
      ...
      if (fsContent instanceof SlackFile) fileSystemMap = fileSystemSlackMap;
      ...
      synchronized (lock) {
        ...
        if (id != null) {
          if (!id.equals(evidence.getIdInDataSource())) ignore = true;
        } else {
      ...
      if (ignore) {
        evidence.setInputStreamFactory(
            new SleuthkitInputStreamFactory(factory.getSleuthkitCase(), null));
