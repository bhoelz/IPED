...
public class PhotoDNALookup extends AbstractTask {
  ...
  private static final String cachePath =
      System.getProperty("user.home") + "/.iped/photodnalookup.cache";
      ...
      if (!init.get()) {
        if (pdnaLookupConfig.isEnabled()) {
          ...
          LocalConfig localConfig =
              (LocalConfig) configurationManager.findObject(LocalConfig.class);
          if (localConfig.getHashDbFile() == null) {
            ...
            if (!hashDBFile.exists() || !hashDBFile.canRead() || !hashDBFile.isFile()) {
              ...
              if (pdnaTree != null) {
                log.info(
                    "Load from cache file {} in {} ms.", cachePath, System.currentTimeMillis() - t);
                taskEnabled = true;
              } else {
                ...
                ArrayList<PhotoDnaItem> photoDNAHashSet =
                    hashDBDataSource.readPhotoDNA(statusFilter);
                if (photoDNAHashSet == null || photoDNAHashSet.isEmpty()) {
                  log.error(
                      "PhotoDNA hashes must be loaded into IPED hashes database to enable PhotoDNALookup.");
                } else {
                  log.info(
                      "{} PhotoDNA hashes loaded in {} ms.",
                      photoDNAHashSet.size(),
  ...
  protected void process(IItem evidence) throws Exception {
    ...
    if (hit != null) {
      ...
      Map<String, List<String>> properties =
          hashDBDataSource.getProperties(hit.nearest.getHashId());
    ...
    try {
      ...
      os =
          new ObjectOutputStream(
              new BufferedOutputStream(new FileOutputStream(cacheFile), 1 << 20));
      os.writeInt(magic);
