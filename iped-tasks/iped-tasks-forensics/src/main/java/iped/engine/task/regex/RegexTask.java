...
public class RegexTask extends AbstractTask {
  ...
  private static final File cacheFile =
      new File(System.getProperty("user.home"), ".iped/regexAutomata.cache");
  ...
  static class Regex implements Serializable {
    ...
    public Regex(
      ...
      this(
          name,
          new RegExp(regex).toAutomaton(new DatatypesAutomatonProvider()),
          ignoreCases,
          ignoreDiacritics);
      this.prefix = prefix;
      this.sufix = sufix;
  ...
  public void init(ConfigurationManager configurationManager) throws Exception {
    ...
    ExportByKeywordsConfig exportConfig =
        configurationManager.findObject(ExportByKeywordsConfig.class);
    ...
    if (regexConfig.isEnabled() && regexList == null) {
      ...
      if (loadCache(regexConfig, exportConfig)) {
        ...
        log.info(
            "Created {} automata for each keyword to export configured.", regexList.size() - num);
  ...
  private void writeCache(RegexTaskConfig regexConfig, ExportByKeywordsConfig exportConfig)
      throws IOException {
  ...
  protected void process(IItem item) throws Exception {
    ...
    if (evidence.getTextCache() == null || !evidence.isToAddToCase()) return;
  ...
  private void processRegex(IItem evidence, Reader reader) throws IOException {
    ...
    while (k != -1) {
      int off = 0;
      k = 0;
      while (k != -1 && (off += k) < cbuf.length) k = reader.read(cbuf, off, cbuf.length - off);
      ...
      while (fullMatcher.find()) {
        ...
        for (Regex regex : regexList) {
          if (regex.pattern.run(fullHit)) {
            ...
            if (regex.ignoreCases) hit = hit.toLowerCase();
      ...
      for (int i = 0; i < regexList.size(); i++) {
        if (hitList.get(i).size() > 0) {
          ...
          if (prevHits == null
              || prevHits.isEmpty()
              || !(prevHits.iterator().next() instanceof RegexHits)) {
          ...
          if (regexList.get(i).name.equals(KEYWORDS_NAME)) evidence.setToExtract(true);
        }
      }
      totalOffset += off;
    }
