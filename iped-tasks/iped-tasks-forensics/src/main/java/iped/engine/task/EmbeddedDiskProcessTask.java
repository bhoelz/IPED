...
public class EmbeddedDiskProcessTask extends AbstractTask {
  ...
  private static Set<MediaType> supportedMimes =
      MediaType.set(
          MediaTypes.VMDK,
          MediaTypes.VMDK_DATA,
          MediaTypes.VMDK_DESCRIPTOR,
          MediaTypes.VHD,
          MediaTypes.VHDX,
          MediaTypes.RAW_IMAGE,
          MediaTypes.EWF_IMAGE,
          MediaTypes.E01_IMAGE,
          MediaTypes.EWF2_IMAGE,
          MediaTypes.EX01_IMAGE);
  ...
  public void init(ConfigurationManager configurationManager) throws Exception {
    ...
    if (enabled && Manager.getInstance().getNumWorkers() == 1) {
      ...
      throw new IPEDException(
          "To enable '" + ENABLE_PARAM + "' you should have more than 1 Worker thread!");
    }
  }
  ...
  private void process(IItem item, boolean enqueueDeleted) throws Exception {
    ...
    if (isFirstOrUniqueImagePart(item)) {
      if (MediaTypes.RAW_IMAGE.equals(item.getMediaType())) {
        ...
        ItemSearcher searcher =
            (ItemSearcher) caseData.getCaseObject(IItemSearcher.class.getName());
        int dotIdx = item.getName().lastIndexOf(".");
        if (dotIdx == -1) dotIdx = item.getName().length();
        String query =
            BasicProps.PARENTID
                + ":"
                + item.getParentId()
    ...
    try (SleuthkitReader reader = new SleuthkitReader(true, caseData, output)) {
      log.info(
          "Decoding embedded disk image {} -> {}", item.getPath(), imageFile.getAbsolutePath());
      ...
      if (reader.hasDecodingError()) {
        ((Metadata) item.getMetadata())
            .set(StandardParser.PARSER_EXCEPTION, Boolean.TRUE.toString());
    ...
    if (item instanceof IItem && IOUtil.hasFile((IItem) item)) {
      ...
      synchronized (lock) {
        if (!imageFile.exists()) {
          ...
          if (trackId.equals(trackFileId)) {
            ...
            imageFile =
                new File(exportDir, trackId + "/" + cleanFileName(item.getName()))
                    .getCanonicalFile();
      ...
      if (!alreadyExported) {
        if (imageFile.length() > 0) {
          log.info(
              "Deleting incomplete exported item {} -> {}",
              item.getPath(),
