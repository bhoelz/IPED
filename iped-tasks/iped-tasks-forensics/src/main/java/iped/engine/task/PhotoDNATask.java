...
public class PhotoDNATask extends AbstractTask {
  ...
  public static final String PDNA_NOT_FOUND_MSG =
      "Optional photoDNA lib not found in plugins folder. If you are law enforcement, ask iped@pf.gov.br";
  ...
  public void init(ConfigurationManager configurationManager) throws Exception {
    ...
    if (!pdnaConfig.isEnabled()) return;
  ...
  protected void process(IItem evidence) throws Exception {
    if (!evidence.isToAddToCase()) return;

    if (pdnaConfig.isSkipHashDBFiles()
        && evidence.getExtraAttribute(ExtraProperties.HASHDB_STATUS) != null) return;

    if (evidence.getLength() != null && evidence.getLength() < pdnaConfig.getMinFileSize()) return;
  ...
  private void processImage(IItem evidence) throws Exception {
    if (evidence.getExtraAttribute(PHOTO_DNA) != null) return;

    byte[] thumb = evidence.getThumb();
    if (thumb == null) return;

    if (pdnaConfig.isUseThumbnail() && thumb.length == 0) return;

    try (InputStream is =
        pdnaConfig.isUseThumbnail()
            ? new ByteArrayInputStream(thumb)
  ...
  private void processVideo(IItem evidence) throws Exception {
    if (evidence.getExtraAttribute(PHOTO_DNA_FRAMES) != null) return;
