...
public class JumpListTask extends AbstractTask {
  ...
  public static final MediaType AUTOMATIC_DESTINATIONS_MIME =
      MediaType.application("x-customdestinations");
  public static final MediaType CUSTOM_DESTINATIONS_MIME =
      MediaType.application("x-automaticdestinations");

  public static final MediaType AUTOMATIC_DESTINATIONS_ENTRY_MIME =
      MediaType.application("x-customdestinations-entry");
  public static final MediaType CUSTOM_DESTINATIONS_ENTRY_MIME =
      MediaType.application("x-automaticdestinations-entry");
  ...
  private void processAutomaticDestinationsEntry(IItem evidence) {
    ...
    if (evidence.getMediaType().equals(AUTOMATIC_DESTINATIONS_ENTRY_MIME)
      ...
      if (StringUtils.endsWithAny(
          parentName, AUTOMATIC_DESTINATIONS_SUFIX, CUSTOM_DESTINATIONS_SUFIX)) {
  ...
  private void processExecutables(IItem evidence) {
    ...
    if (MediaTypes.isInstanceOf(evidence.getMediaType(), EXE_MIME)
        && !evidence.isCarved()
        && !evidence.isDeleted()
        && !evidence.isSubItem()) {
