...
public class LanguageDetectTask extends AbstractTask {
  ...
  public void init(ConfigurationManager configurationManager) throws Exception {
    ...
    if (isEnabled && detector == null) detector = loadModels();
  }
  ...
  protected void process(IItem evidence) throws Exception {
    ...
    if (evidence.getMediaType().equals(MediaType.OCTET_STREAM)) return;
    ...
    try (Reader reader = evidence.getTextReader()) {
      while (i != -1 && (off += i) < MAX_CHARS) i = reader.read(cbuf, off, MAX_CHARS - off);
    }
    ...
    if (start != -1) text = text.substring(0, start);
    ...
    if (!langList.isEmpty()) evidence.setExtraAttribute(LANGUAGE_NAMES, langList);
  }
  ...
  private LanguageDetector loadModels() throws IOException {
    ...
    LanguageDetectorBuilder builder =
        LanguageDetectorBuilder.create(NgramExtractors.standard()).withProfiles(languageProfiles);
