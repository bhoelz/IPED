...
public class TempFileTask extends AbstractTask {
  ...
  public void init(ConfigurationManager configurationManager) throws Exception {
    ...
    isEnabled =
        indexTempOnSSD
            && !"fastmode".equals(args.getProfile())
            && !"triage".equals(args.getProfile())
            && !caseData.isIpedReport();

    if (isEnabled) {
      TempFileTaskConfig tempFileTaskConfig =
          configurationManager.findObject(TempFileTaskConfig.class);
    ...
    if (len != null && len <= maxFileSize) {
      try {
        ...
        if (!IOUtil.hasFile(evidence)
          ...
          if (!evidence
              .isSubItem()) { // should we create temp files for subitems compressed into the sqlite
            // storages?
