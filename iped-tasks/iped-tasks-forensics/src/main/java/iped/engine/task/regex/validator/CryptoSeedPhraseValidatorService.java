...
/**
 * Dynamically validates BIP-39 and Electrum seed phrases for any language defined in the
 * configuration.
 *
...
public class CryptoSeedPhraseValidatorService implements RegexValidatorService {
  ...
  public void init(File confDir) {
    ...
    if (regexTaskConfig == null) {
      log.error(
          "RegexTaskConfig not found in ConfigurationManager. CryptoSeedPhraseValidatorService cannot initialize.");
      return;
    }
    ...
    if (discoveredRegexNames.isEmpty()) {
      log.warn(
          "No crypto seed phrase regexes found in RegexConfig.toml with prefix: {}", REGEX_PREFIX);
    }
  }
  ...
  private boolean validateElectrum(String[] words) {
    try {
      ...
      SecretKeySpec key =
          new SecretKeySpec("Seed version".getBytes(StandardCharsets.UTF_8), "HmacSHA512");
      hmac.init(key);
      ...
      if (hexHash.length() < length) return false;
