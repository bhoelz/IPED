...
public class FragmentLargeBinaryTask extends BaseCarveTask {
  ...
  protected void process(IItem evidence) throws Exception {
    ...
    boolean hadParserException =
        Boolean.valueOf(evidence.getMetadataValue(StandardParser.PARSER_EXCEPTION));

    TextCache textCache = ((Item) evidence).getTextCache();

    if (evidence.getLength() != null
        && evidence.getLength() >= splitConfig.getMinItemSizeToFragment()
        && (evidence.isTimedOut()
            || (!hasSpecificParser
      ...
      for (long offset = 0; offset < evidence.getLength(); offset += fragSize - overlap) {
        long len =
            offset + fragSize < evidence.getLength() ? fragSize : evidence.getLength() - offset;
      ...
      for (long textOffset = 0;
          textOffset < totalTextSize;
          textOffset += TEXT_SPLIT_SIZE - splitConfig.getFragmentOverlapSize()) {
        TextCache textCacheChunk = textCache.clone();
        int textSize =
            textOffset + TEXT_SPLIT_SIZE < totalTextSize
                ? TEXT_SPLIT_SIZE
                : (int) (totalTextSize - textOffset);
  ...
  private void addFragmentFile(
      IItem parentEvidence, long off, long len, int fragNum, TextCache textCache) {
    String name = parentEvidence.getName() + "_" + fragNum; // $NON-NLS-1$
    Item fragFile =
        getOffsetFile(parentEvidence, off, len, name, (MediaType) parentEvidence.getMediaType());
