...
public abstract class ThumbTask extends AbstractTask {
  ...
  private static final String INSERT_THUMB =
      "INSERT INTO thumbs(id, thumb) VALUES(?,?) ON CONFLICT(id) DO UPDATE SET thumb=? WHERE thumb IS NULL;"; //$NON-NLS-1$
  ...
  protected File getThumbFile(IItem evidence) throws Exception {
    ...
    boolean storeThumbsInDisk =
        caseData != null && caseData.containsReport() && htmlReportConfig.isEnabled();
    if (storeThumbsInDisk) {
      String reportSubFolderName =
      ...
      return Util.getFileFromHash(
          new File(reportSubFolder, THUMBS_FOLDER_NAME), evidence.getHash(), THUMB_EXT);
    }
  ...
  protected boolean hasThumb(IItem evidence, File thumbFile) throws Exception {
    ...
    if (thumbFile == null) {
      Connection con =
          ExportFileTask.getSQLiteStorageCon(output, evidence.getHashValue().getBytes());
