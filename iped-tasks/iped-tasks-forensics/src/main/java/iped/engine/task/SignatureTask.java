...
public class SignatureTask extends AbstractTask {
  ...
  private Detector getDetector() {
    if (detector == null) {
      ...
      if (MediaTypes.getParentType(MediaType.parse("message/x-chat-message"))
          .equals(MediaType.OCTET_STREAM)) {
      ...
      try {
        if (processFileSignatures) {
          ...
          try {
            ...
            log.warn(
                "{} Error detecting signature: {} ({} bytes)\t\t{}", //$NON-NLS-1$
        ...
        if (type != null
            && type.toString().equals("application/x-tika-ooxml-protected") // $NON-NLS-1$
        ...
        log.warn(
            "{} Error detecting signature: {} ({} bytes)\t\t{}",
  ...
  public static void installCustomSignatures() {
    ...
    System.setProperty(
        MimeTypesFactory.CUSTOM_MIMES_SYS_PROP, config.getTmpConfigFile().getAbsolutePath());
  }
