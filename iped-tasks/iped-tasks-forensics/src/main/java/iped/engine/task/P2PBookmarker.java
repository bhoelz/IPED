...
public class P2PBookmarker {
  ...
  public void createBookmarksForSharedFiles(File caseDir) {
    ...
    if (isIpedReport) return;

    log.info("Searching for shared items...");

    HashMap<String, P2PProgram> p2pPrograms = new HashMap<String, P2PProgram>();

    P2PProgram progEMule =
        new P2PProgram(HashAlgorithm.EDONKEY.toString(), "Emule", new Color(140, 75, 30));
    p2pPrograms.put(KnownMetParser.EMULE_MIME_TYPE, progEMule);
    p2pPrograms.put(PartMetParser.EMULE_PART_MET_MIME_TYPE, progEMule);

    p2pPrograms.put(
        AresParser.ARES_MIME_TYPE,
        new P2PProgram(HashAlgorithm.SHA1.toString(), "Ares", new Color(238, 173, 0)));

    List<String> shareazaHashes =
        Arrays.asList(
            HashAlgorithm.MD5.toString(),
            HashAlgorithm.SHA1.toString(),
            HashAlgorithm.EDONKEY.toString());
    p2pPrograms.put(
        ShareazaLibraryDatParser.LIBRARY_DAT_MIME_TYPE,
        new P2PProgram(shareazaHashes, "Shareaza", new Color(170, 20, 20)));

    p2pPrograms.put(
        ShareazaDownloadParser.SHAREAZA_DOWNLOAD_META,
        new P2PProgram(shareazaHashes, "Shareaza SD", new Color(170, 20, 20)));

    p2pPrograms.put(
        WhatsAppParser.WHATSAPP_CHAT.toString(),
        new P2PProgram(HashAlgorithm.SHA256.toString(), "WhatsApp", new Color(32, 146, 90)));

    p2pPrograms.put(
        UfedChatParser.UFED_CHAT_PREVIEW_MIME.toString(),
    ...
    p2pPrograms.put(
        TelegramParser.TELEGRAM_CHAT.toString(),
        new P2PProgram(IndexItem.HASH, "Telegram", new Color(120, 190, 250)));

    p2pPrograms.put(
        ThreemaParser.THREEMA_CHAT.toString(),
        new P2PProgram(IndexItem.HASH, "Threema")); // $NON-NLS-1$
    ...
    List<String> torrentHashes =
        Arrays.asList(
            ...
            HashAlgorithm.EDONKEY.toString());
    p2pPrograms.put(
        BitTorrentResumeDatParser.RESUME_DAT_MIME_TYPE,
        new P2PProgram(torrentHashes, "Torrent", new Color(0, 160, 60)));
    p2pPrograms.put(
        BitTorrentResumeDatEntryParser.RESUME_DAT_ENTRY_MIME_TYPE,
        new P2PProgram(torrentHashes, "Torrent", new Color(0, 160, 60)));
    p2pPrograms.put(
        TransmissionResumeParser.TRANSMISSION_RESUME_MIME_TYPE,
        new P2PProgram(torrentHashes, "Transmission", new Color(0, 180, 0)));

    P2PProgram progGDrive = new P2PProgram(HashAlgorithm.MD5.toString(), "GoogleDrive");
    ...
    String queryText =
        ExtraProperties.SHARED_HASHES + ":* OR " + ExtraProperties.SHARED_ITEMS + ":*";
      ...
      for (int i = 0; i < p2pItems.getLength(); i++) {
        ...
        log.info(
            "Items shared by "
                + program.appName
                + " found: "
                + result.getLength()); // $NON-NLS-1$ //$NON-NLS-2$
        if (result.getLength() == 0) continue;

        String bookmarkSufix = program.appName;
        if (MediaTypes.isInstanceOf(
            MediaType.parse(mediaType), UfedChatParser.UFED_CHAT_PREVIEW_MIME)) {
          String source = doc.get(ExtraProperties.UFED_META_PREFIX + "Source");
          if (source != null) bookmarkSufix = source;
          String account = doc.get(ExtraProperties.CONVERSATION_ACCOUNT);
          if (StringUtils.isNotBlank(account)) {
            if (source != null) {
              String formattedAccount =
                  PartyStringBuilderFactory.getBuilder(source)
                      .withUserId(
                          doc.get(ExtraProperties.CONVERSATION_ACCOUNT + CONVERSATION_SUFFIX_ID))
                      .withName(
                          doc.get(ExtraProperties.CONVERSATION_ACCOUNT + CONVERSATION_SUFFIX_NAME))
                      .withPhoneNumber(
                          doc.get(ExtraProperties.CONVERSATION_ACCOUNT + CONVERSATION_SUFFIX_PHONE))
                      .withUsername(
                          doc.get(
                              ExtraProperties.CONVERSATION_ACCOUNT + CONVERSATION_SUFFIX_USERNAME))
                      .build();
        ...
        int labelId =
            ipedSrc
                .getBookmarks()
                .newBookmark(
                    Messages.getString("P2PBookmarker.P2PBookmarkPrefix")
                        + bookmarkSufix); //$NON-NLS-1$
        if (program.color != null) {
          ipedSrc.getBookmarks().setBookmarkColor(labelId, program.color);
        }
        ArrayList<Integer> ids = new ArrayList<Integer>();
        for (int j = 0; j < result.getLength(); j++) ids.add(result.getId(j));
