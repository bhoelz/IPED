...
public class BoletoRegexValidatorService extends AbstractDocRegexValidatorService {
  ...
  private static final int[] WEIGHTS = {
    5, 4, 3, 2, 9, 8, 7, 6, 5, 4, 3, 2, 9, 8, 7, 6, 5, 4, 3, 2, 9, 8, 7, 6, 5, 4, 3, 2, 9, 8, 7, 6,
  ...
  public boolean validate(String boleto) {
    ...
    boleto = SEPARATORS.matcher(boleto).replaceAll("");
    if (isRepeated(boleto)) return false;
    ...
    return boleto.equalsIgnoreCase(
  ...
  private static int calcBoletoDigit(String field) {
    ...
    for (int index = field.length() - 1, digit; index >= 0; index--) {
      ...
      if (partial > 9) {
        sum +=
            Integer.parseInt(Integer.toString(partial).substring(0, 1))
