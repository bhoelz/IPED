...
public class PathToGuidConverter {
  ...
  private final Map<String, List<String>> guidsMap =
      new TreeMap<>(
          (e1, e2) -> {
  ...
  private PathToGuidConverter() {
    ...
    commonProgramFilesX86Guids.add(
        "{F7F1ED05-9F6D-47A2-AAAE-29D317C6F066}"); // FOLDERID_ProgramFilesCommon
    commonProgramFilesX86Guids.add(
        "{DE974D24-D9C6-4D3E-BF91-F4455120B917}"); // FOLDERID_ProgramFilesCommonX86
    ...
    guidsMap.put(
        "%WINDIR%\\SYSWOW64",
        singletonList("{D65231B0-B2F1-4857-A4CE-A8E7C6EA7D27}")); // FOLDERID_SystemX86

    // ProgramData
    guidsMap.put(
        "%PROGRAMDATA%",
        singletonList("{62AB5D82-FDC1-4DC3-A9DD-070D1D495D97}")); // FOLDERID_ProgramData
    guidsMap.put(
        "%ALLUSERSPROFILE%",
        singletonList("{62AB5D82-FDC1-4DC3-A9DD-070D1D495D97}")); // FOLDERID_ProgramData
    ...
    guidsMap.put(
        "%WINDIR%", singletonList("{F38BF404-1D43-42F2-9305-67DE0B28FC23}")); // FOLDERID_Windows
    guidsMap.put(
        "%USERPROFILE%",
        singletonList("{5E6C858F-0E22-4760-9AFE-EA3317B67173}")); // FOLDERID_Profile
    guidsMap.put(
        "%APPDATA%",
        singletonList("{3EB685DB-65F9-4CF6-A03A-E3EF65729F3D}")); // FOLDERID_RoamingAppData
    guidsMap.put(
        "%LOCALAPPDATA%",
        singletonList("{F1B32785-6FBA-4FCF-9D55-7B8E7F157091}")); // FOLDERID_LocalAppData
    guidsMap.put(
        "%PUBLIC%", singletonList("{DFDF76A2-C82A-4D63-906A-5644AC457385}")); // FOLDERID_Public
    ...
    guidsMap.put(
        "%PUBLIC%\\DOWNLOADS",
        singletonList("{3D644C9B-1FB8-4f30-9B45-F670235F79C0}")); // FOLDERID_PublicDownloads
    guidsMap.put(
        "%USERPROFILE%\\DOWNLOADS",
        singletonList("{374DE290-123F-4565-9164-39C4925E467B}")); // FOLDERID_Downloads

    // Desktop
    guidsMap.put(
        "%USERPROFILE%\\DESKTOP",
        singletonList("{B4BFCC3A-DB2C-424C-B029-7FE99A87C641}")); // FOLDERID_Desktop
    guidsMap.put(
        "%PUBLIC%\\DESKTOP",
        singletonList("{C4AA340D-F20F-4863-AFEF-F87EF2E6BA25}")); // FOLDERID_PublicDesktop

    // UserProgramFiles
    guidsMap.put(
        "%LOCALAPPDATA%\\PROGRAMS",
        singletonList("{5CD7AEE2-2219-4A67-B85D-6C9CE15660CB}")); // FOLDERID_UserProgramFiles
    guidsMap.put(
        "%LOCALAPPDATA%\\PROGRAMS\\COMMON",
        singletonList("{BCBD3057-CA5C-4622-B42D-BC56DB0AE516}")); // FOLDERID_UserProgramFilesCommon
  }

  /**
   * Given a generic path starting with a known path key (like an environment variable or a
   * composite path such as "%PUBLIC%\Downloads"), returns a list of possible paths with the key
   * replaced by corresponding Known Folder GUIDs.
   *
   * @param genericPath The generic path string, e.g., "%PROGRAMFILES%\My App\app.exe" or
   *     "%PUBLIC%\Downloads\installer.exe".
   * @return A list of strings with GUID-based paths. Returns an empty list if no mapped path key is
   *     found at the beginning of the path, or if the input path is null/empty.
   */
  @Nonnull
    ...
    for (Map.Entry<String, List<String>> entry : guidsMap.entrySet()) {
      ...
      if (genericPathUpperCase.startsWith(pathKeyUpperCase)) {
        ...
        if (genericPath.length() == pathKey.length()
            || (genericPath.length() > pathKey.length()
