...
public class GraphTask extends AbstractTask {
  ...
  public static final String DB_DATA_DIR = GraphConstants.DB_DATA_DIR;
  public static final String DB_DATA_PATH = GraphConstants.DB_DATA_PATH;
  public static final String CSVS_DIR = GraphConstants.CSVS_DIR;
  public static final String CSVS_PATH = GraphConstants.CSVS_PATH;
  public static final String RELATIONSHIP_ID = GraphConstants.RELATIONSHIP_ID;
  public static final String RELATIONSHIP_SOURCE = GraphConstants.RELATIONSHIP_SOURCE;
  ...
  private static Pattern emailPattern =
      Pattern.compile(
          "[0-9a-zA-Z\\+\\.\\_\\%\\-\\#\\!]{1,64}\\@[0-9a-zA-Z\\-]{2,64}(\\.[0-9a-zA-Z\\-]{2,25}){1,3}");
  private static Pattern whatsappPattern = Pattern.compile("([0-9]{7,20})\\@[sg]\\.whatsapp\\.net");
  private static Pattern oldBRPhonePattern =
      Pattern.compile("(\\+55 \\d\\d )([7-9]\\d{3}\\-\\d{4})");
  ...
  private static String[] contactMimes = {
    ...
    TelegramParser.TELEGRAM_CONTACT.toString(),
  ...
  private static Map<String, SortedSet<String>> formattedPhonesCache =
      Collections.synchronizedMap(
          new LinkedHashMap<String, SortedSet<String>>(16, 0.75f, true) {
  ...
  public void init(ConfigurationManager configurationManager) throws Exception {
    ...
    if (enabled) {
      ...
      if (a.graphFileWriter == null) {
        a.graphFileWriter =
            new GraphFileWriter(new File(output, CSVS_PATH), configuration.getDefaultEntity());
        ...
        if (graphDataDir.exists()) {
          ...
          try {
            ...
            throw new IPEDException(
                "Error appending to graph database, currently it can't be in use!");
          }
        }
      }
    }
  ...
  private void finishGraphGeneration() throws IOException {
    UIPropertyListenerProvider.getInstance()
        .firePropertyChange("mensagem", "", "Generating graph database...");
  ...
  public void finish() throws Exception {
    ...
    if (a.graphFileWriter != null) {
      UIPropertyListenerProvider.getInstance()
          .firePropertyChange("mensagem", "", "Finishing graph CSVs...");
      ...
      UIPropertyListenerProvider.getInstance()
          .firePropertyChange("mensagem", "", "Compressing graph CSVs...");
  ...
  private static String getRelationType(String mediaType) {
    ...
    for (String contactMime : contactMimes) {
      if (contactMime.equals(mediaType)) return "contact";
    }
  ...
  private SortedSet<String> getPhones(String value) {
    ...
    for (PhoneNumberMatch m :
        phoneUtil.findNumbers(
            value, configuration.getPhoneRegion(), Leniency.POSSIBLE, Integer.MAX_VALUE)) {
  ...
  private NodeValues getPhoneNodeValues(String value) {
    ...
    if (!phones.isEmpty()) {
      return new NodeValues(
          DynLabel.label(GraphConfiguration.PHONE_LABEL),
  ...
  private NodeValues getGroupNodeValues(String value) {
    return new NodeValues(
        DynLabel.label(GraphConfiguration.CONTACT_GROUP_LABEL),
        BasicProps.NAME,
  ...
  private NodeValues getEmailNodeValues(String value) {
    ...
    if (!emails.isEmpty()) {
      NodeValues nv =
          new NodeValues(
              DynLabel.label(GraphConfiguration.EMAIL_LABEL),
  ...
  private NodeValues getAccountNodeValues(String value, Metadata meta) {
    ...
    if (service == null
        || WhatsAppParser.WHATSAPP.equalsIgnoreCase(service)) // phone will be decoded from whatsapp
    return null;
    ...
    if (idx != -1 && value.endsWith(")")) {
      ...
      nv =
          new NodeValues(
              DynLabel.label(GraphConfiguration.PERSON_LABEL),
  ...
  private NodeValues getNodeValues(String value, Metadata metadata, boolean detectPhones) {
    ...
    if (nv1 == null && detectPhones) nv1 = getPhoneNodeValues(value);
  ...
  private void processUserAccount(IItem item) throws IOException {
    ...
    String relationType = getRelationType(item.getMediaTypeString());
  ...
  private boolean isToDetectPhones(IItem item) {
    String mime = item.getMediaTypeString();
  ...
  private NodeValues writePersonNode(IItem item, SortedSet<String> msisdnPhones)
    ...
    possibleEmails.addAll(
        Arrays.asList(item.getMetadataValues(ExtraProperties.UFED_META_PREFIX + "EmailAddress")));
    possibleEmails.addAll(
        Arrays.asList(item.getMetadataValues(ExtraProperties.UFED_META_PREFIX + "Username")));
    possibleEmails.addAll(
        Arrays.asList(item.getMetadataValues(ExtraProperties.UFED_META_PREFIX + "UserID")));
    possibleEmails.addAll(Arrays.asList(item.getMetadataValues(ExtraProperties.USER_EMAIL)));
    SortedSet<String> emails = getEmails(possibleEmails.toString());

    List<String> possiblePhones = new ArrayList<>();
    possiblePhones.addAll(
        Arrays.asList(item.getMetadataValues(ExtraProperties.UFED_META_PREFIX + "PhoneNumber")));
    possiblePhones.addAll(
        Arrays.asList(item.getMetadataValues(ExtraProperties.UFED_META_PREFIX + "Username")));
    possiblePhones.addAll(Arrays.asList(item.getMetadataValues(ExtraProperties.USER_PHONE)));
    for (String id : item.getMetadataValues(ExtraProperties.UFED_META_PREFIX + "UserID")) {
      Matcher matcher = whatsappPattern.matcher(id);
      if (matcher.find()) possiblePhones.add(id);
    }
    SortedSet<String> formattedPhones =
        isToDetectPhones(item) ? getPhones(possiblePhones.toString()) : new TreeSet<>();
    if (msisdnPhones == null) msisdnPhones = Collections.emptySortedSet();
    formattedPhones.addAll(msisdnPhones);

    String[] name = item.getMetadataValues(ExtraProperties.UFED_META_PREFIX + "Name");
    if (name.length == 0) name = item.getMetadataValues(ExtraProperties.USER_NAME);

    String[] accounts = item.getMetadataValues(ExtraProperties.USER_ACCOUNT);
    if (accounts.length == 0)
      accounts = item.getMetadataValues(ExtraProperties.UFED_META_PREFIX + "Username");
    if (accounts.length == 0) {
    ...
    String[] accountType = item.getMetadataValues(ExtraProperties.UFED_META_PREFIX + "ServiceType");
    if (accountType.length == 0)
      accountType = item.getMetadataValues(ExtraProperties.UFED_META_PREFIX + "Source");
    if (accountType.length == 0)
      accountType = item.getMetadataValues(ExtraProperties.USER_ACCOUNT_TYPE);

    String[] org = item.getMetadataValues(ExtraProperties.UFED_META_PREFIX + "Organization");
    if (org.length == 0) org = item.getMetadataValues(ExtraProperties.USER_ORGANIZATION);

    String[] address = item.getMetadataValues(ExtraProperties.USER_ADDRESS);
    if (address.length == 0) address = new String[] {getUfedAddress((Metadata) item.getMetadata())};

    String[] notes = item.getMetadataValues(ExtraProperties.UFED_META_PREFIX + "Notes");
    if (notes.length == 0) notes = item.getMetadataValues(ExtraProperties.USER_NOTES);
    ...
    String uniqueId =
        accum()
            .graphFileWriter
            .writeNode(nv1.label, nv1.propertyName, nv1.propertyValue, nv1.props);
  ...
  private NodeValues getGenericOwnerNode(IItem item) throws IOException {
    ...
    synchronized (a) {
      ...
      if (nv1 == null) {
        nv1 =
            new NodeValues(
                DynLabel.label(GraphConfiguration.DATASOURCE_LABEL),
        ...
        List<String> msisdns =
            (List<String>) caseData.getCaseObject(UfedXmlReader.MSISDN_PROP + evidenceUUID);
  ...
  private void processContacts(IItem item) throws IOException {
    ...
    NodeValues nv1 = null;
    String contactOfAccount = item.getMetadataValue(ExtraProperties.CONTACT_OF_ACCOUNT);
  ...
  private void processWifi(IItem item) throws IOException {
    ...
    String bssid = item.getMetadataValue(ExtraProperties.UFED_META_PREFIX + "BSSId");
    String ssid = item.getMetadataValue(ExtraProperties.UFED_META_PREFIX + "SSId");
      ...
      for (Entry<String, Object> entry : entries) {
        ...
        for (Entry<String, Object> entry2 : entries) {
          ...
          processMatches(
              entity,
              metadata,
              (Collection<Object>) entry.getValue(),
              entity2,
              metadata2,
              (Collection<Object>) entry2.getValue(),
              evidence,
              relationsAdded);
        }
      }
  ...
  private void processMatches(
      ...
      GraphEntity entity2,
      GraphEntityMetadata metadata2,
      Collection<Object> matches2,
      IItem evidence,
      Set<String> relationsAdded)
      throws IOException {
    ...
    if (matches != null && matches2 != null) {
      ...
      RelationshipType relationshipType =
          DynRelationshipType.withName(configuration.getDefaultRelationship());
